...
-- Update loot tables
UPDATE `gameobject_loot_template` SET `item`=@EmblemLowCURR WHERE `item` IN (@EmblemLowPREV2, @EmblemLowPREV) AND `entry` NOT IN (27078);
UPDATE `creature_loot_template` SET `item`=@EmblemLowCURR WHERE `item` IN (@EmblemLowPREV2, @EmblemLowPREV) AND `entry` NOT IN (33113);
...
-- FREYA !!!!!!
DELETE FROM `gameobject_loot_template` WHERE `entry` = 27078 AND `item` = 40753;
UPDATE `gameobject_loot_template` SET `mincountOrRef` = 5, `maxcount` = 5 WHERE `entry` = 27078 AND `item` = 45624;
...
-- Titatnium Ore should be prospectable
...
-- Usuri Brightcoin - remove emblem of triumph and emblem of conquest
DELETE FROM `npc_vendor` WHERE `entry` = 35790 AND `item` IN (45624, 47241);
UPDATE `creature` SET `phaseMask` = 1 WHERE `id` = 35790;

-- Disable building coloseum dailies
DELETE FROM `disables` WHERE `sourceType` = 1 AND `entry` IN (13627,13681); 
INSERT INTO `disables` VALUES
(1, 13627, 0, '', '', 'Argent Tournament Quest'),
(1, 13681, 0, '', '', 'Argent Tournament Quest');

SET @OGUID = 300032;
DELETE FROM `gameobject` WHERE `guid` BETWEEN @OGUID AND @OGUID+11 AND `id` = 194461;
SET @CGUID = 303315;
DELETE FROM `creature` WHERE `guid` BETWEEN @CGUID AND @CGUID+25 AND `id` = 33308;
