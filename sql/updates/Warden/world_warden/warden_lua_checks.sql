...
(5, 'if(rcbn==nil and CastSpellByName~=nil)then rcbn=CastSpellByName CastSpellByName=function(a, b) rcbn(a, b) @addonSuccess CastSpellByName=rcbn rcbn=nil end end', 6, 'Client called protected function!', 'Low', 1, 'hooks a sendaddon message when CastSpellByName is called'),
...
(9, 'local c=0 for i,v in pairs(_G) do if (type(v)=="table" and v["PQR_EventFrame"] ~= nil)then c=c+1 if (c>1)then break end end end if(c<2)then @addonSuccess else @addonFailureend', 1, 'PQR CHECK', 'Low', 0, 'check global tables for PQR frames'),
...
(20, 'if(rqai==nil and QueryAuctionItems~=nil)then rqai=QueryAuctionItems QueryAuctionItems=function(a,b,c,d,e,f,g,h,j) rqai(a,b,c,d,e,f,g,h,j) local a=(":"..debugstack(2)) @GlobalTablesEntry=a QueryAuctionItems=rqai rqai=nil end end', 6, 'Client called protected function!', 1, 1, 'hooks a sendaddon message when QueryAuctionItems is called'),
(21, 'local a=@GlobalTablesEntry if (a ~= nil)then @AddonMsgGt @GlobalTablesEntry="Secure client, ignore" end', 7, 'Stack data for rqai', 2, 1, 'retrieves data from global table if any');
