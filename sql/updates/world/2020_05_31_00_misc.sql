-- DB/Quest: Cenarion Beacon for Salve via quests - fix based on https://github.com/SunwellWoW/Sunwell-WOTLK/blob/3e0671bed1294cd503333b3c910808fb7f13932f/sql/sql_archives/world%202018-2020/2018_03_15_scripts_cleansing_felwood.sql
...
(@Arathandris,0, 1,2, 61,0,100,0,    0,0,0,0, 11,15120,2,0,0,0,0,7,0,0,0,0,0,0,0,"Arathandris Silversky - On Gossip 2208 Option 0 selected - Cast 'Cenarion Beacon'"),
...
(@Riverbreeze,0, 1,2, 61,0,100,0,    0,0,0,0, 11,15120,2,0,0,0,0,7,0,0,0,0,0,0,0,"Maybess Riverbreeze - On Gossip 21400 Option 0 Selected - Cast 'Cenarion Beacon'"),
...
(2209,  3046),
(21401, 3047);
DELETE FROM `gossip_menu_option` WHERE `menu_id` IN (2208, 21400) AND `id` = 1;
UPDATE `gossip_menu_option` SET `action_menu_id` = 2209 WHERE `menu_id` = 2208 AND `id` = 0;
UPDATE `gossip_menu_option` SET `action_menu_id`= 21401 WHERE `menu_id`= 21400 AND `id` = 0;
...
(@Riverbreeze,0,0,'Here is a beacon for you, $n.  You must keep it in your possession if you are to find the corrupted items we will need.',12,1,100,25,5000,0,0,'Arathandris Silversky - SAY_BEACON'),
(@Arathandris,0,0,'Here is a beacon for you, $n.  You must keep it in your possession if you are to find the corrupted items we will need.',12,7,100,25,5000,0,0,'Arathandris Silversky - SAY_BEACON');
...
(21400, 0, 0, 'I need a Cenarion beacon.', 1, 1, 21401, 0, 0, 0, '');