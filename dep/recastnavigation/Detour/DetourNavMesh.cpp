...
dtStatus dtNavMesh::getTileAndPolyByRef(const dtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
{
	...
	if (m_tiles[it].salt != salt || m_tiles[it].header == nullptr) return DT_FAILURE;
...
bool dtNavMesh::isValidPolyRef(dtPolyRef ref) const
{
	...
	if (m_tiles[it].salt != salt || m_tiles[it].header == nullptr) return false;
...
const dtOffMeshConnection* dtNavMesh::getOffMeshConnectionByRef(dtPolyRef ref) const
{
	...
	// Get current polygon
	decodePolyId(ref, salt, it, ip);
	if (it >= (unsigned int)m_maxTiles) return nullptr;
	if (m_tiles[it].salt != salt || m_tiles[it].header == nullptr) return nullptr;
	const dtMeshTile* tile = &m_tiles[it];
	if (ip >= (unsigned int)tile->header->polyCount) return nullptr;
	const dtPoly* poly = &tile->polys[ip];
