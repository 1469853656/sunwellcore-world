...
class EventProcessor
{
    public:
        ...
        void AddEventAsync(BasicEvent* event, bool set_addtime = true);
        template<typename T>
        is_lambda_event<T> AddEventAsync(T&& event) { AddEventAtOffset(new LambdaBasicEvent<T>(std::move(event)), 0ms); }
