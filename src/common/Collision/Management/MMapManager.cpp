...
{
    // ######################## MMapManager ########################
    MMapManager::MMapManager() : loadedTiles(0)
    {
        m_dataDir = sConfigMgr->GetStringDefault("DataDir", "./");
    }
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        uint32 pathLen = m_dataDir.length() + strlen("/mmaps/%03i.mmap")+1;
        char *fileName = new char[pathLen];
        snprintf(fileName, pathLen, (m_dataDir + "/mmaps/%03i.mmap").c_str(), mapId);
    ...
    {
        return m_mapLocks[mapId];
    }

    bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y)
    {
        std::unique_lock lock(MMapManagerLock);
        ...
        uint32 pathLen = m_dataDir.length() + strlen("/mmaps/%03i%02i%02i.mmtile") + 1;
        char* fileName = new char[pathLen];
        snprintf(fileName, pathLen, (m_dataDir + "/mmaps/%03i%02i%02i.mmtile").c_str(), mapId, x, y);
        ...
        dtStatus stat;
        {
            std::unique_lock mapLock(GetMMapLock(mapId));
