...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEMPLATE, "SELECT entry, difficulty_entry_1, difficulty_entry_2, difficulty_entry_3, KillCredit1, KillCredit2, modelid1, modelid2, modelid3, modelid4, name, subname, IconName, gossip_menu_id, minlevel, maxlevel, exp, faction, npcflag, speed_walk, speed_run, scale, `rank`, mindmg, maxdmg, dmgschool, attackpower, dmg_multiplier, baseattacktime, rangeattacktime, unit_class, unit_flags, unit_flags2, dynamicflags, family, trainer_type, trainer_spell, trainer_class, trainer_race, minrangedmg, maxrangedmg, rangedattackpower, type, type_flags, lootid, pickpocketloot, skinloot, resistance1, resistance2, resistance3, resistance4, resistance5, resistance6, spell1, spell2, spell3, spell4, spell5, spell6, spell7, spell8, PetSpellDataId, VehicleId, mingold, maxgold, AIName, MovementType, InhabitType, HoverHeight, Health_mod, Mana_mod, Armor_mod, RacialLeader, questItem1, questItem2, questItem3, questItem4, questItem5, questItem6, movementId, RegenHealth, mechanic_immune_mask, flags_extra, ScriptName, MythicTemplateId, PresetGossipFlags FROM creature_template WHERE entry = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_SEL_CHAT_PROCESSOR_FILTERS, "SELECT id,regex,report_type,report_count from chat_processor_filters", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_BOOST_ITEMS, "SELECT `class`, `spec`, `item` FROM `boost_items` ORDER BY `class` ASC", CONNECTION_SYNCH);

        /** Mythic System **/
    PrepareStatement(WORLD_SEL_CREATURE_MYTHIC_TEMPLATES, "SELECT Id, CreatureType, CreatureLevel, EncounterAbilityMask, BaseMythicHealth, BaseMythicPower, "
                                                          "MinimumRawMeleeDamage, MaximumRawMeleeDamage, KeyVarianceMelee, KeyVarianceStats, MythicLootId FROM creature_template_mythic", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MYTHIC_SPELL_DATA, "SELECT CreatureType, SpellId, SpellEffect0Value, SpellEffect1Value,  SpellEffect2Value, SpellDuration, SpellMaxTargets, SpellValueVariance, Timers, TargetType FROM creature_mythic_spells", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MYTHIC_SPELL_VALUES, "SELECT SpellId, SpellEffect0Value, SpellEffect1Value,  SpellEffect2Value, SpellDuration, SpellMaxTargets, SpellValueVariance FROM creature_mythic_spell_values", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MYTHIC_AFFIX_DATA, "SELECT AffixId, AffixName, AffixFlags, AffixChance, AffixCooldown, AffixKeyRequirement, IsInstanceAffix FROM mythic_affix_template", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MYTHIC_PRESETS, "SELECT PresetId, FirstAffix, SecondAffix, ThirdAffix, FourthAffix FROM mythic_affix_presets", CONNECTION_SYNCH);

    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_LEVEL, "UPDATE creature_template_mythic SET CreatureLevel = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_HEALTH, "UPDATE creature_template_mythic SET BaseMythicHealth = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_POWER, "UPDATE creature_template_mythic SET BaseMythicPower = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_MINMELEE, "UPDATE creature_template_mythic SET MinimumRawMeleeDamage = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_MAXMELEE, "UPDATE creature_template_mythic SET MaximumRawMeleeDamage = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_MELEEVAR, "UPDATE creature_template_mythic SET KeyVarianceMelee = ? WHERE Id = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MYTHIC_TEMPLATE_STATVAR, "UPDATE creature_template_mythic SET KeyVarianceStats = ? WHERE Id = ?", CONNECTION_ASYNC);
}
