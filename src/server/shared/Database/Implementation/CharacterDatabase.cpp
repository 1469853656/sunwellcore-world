...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                                         ...
                                         "grantableLevels, autoInvite, xpRate, pvpAnnounces, totalSeasonHonorPoints, isCountedAsMaxLevelPathOfWarCharacter, spellQueueWindow "
    ...
    PrepareStatement(CHAR_REP_CHARACTER_SEASONALQUESTSTATUS, "REPLACE INTO character_queststatus_seasonal (guid, quest, event) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INVENTORY, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, bag, slot, "
                                                   "item, itemEntry, transmog, enchant FROM character_inventory ci JOIN item_instance ii ON ci.item = ii.guid WHERE ci.guid = ? ORDER BY bag, slot",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_INSTANCE_SAVE_DATA, "UPDATE instance SET data=?, CustomDifficultyID=? WHERE id=?", CONNECTION_ASYNC);
    ...
    // Arena teams
    PrepareStatement(CHAR_INS_ARENA_TEAM, "INSERT INTO arena_team (arenaTeamId, name, charName, captainGuid, type, rating, matchmakerRating, maxMMR, backgroundColor, emblemStyle, emblemColor, borderStyle, borderColor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ARENA_TEAM, "DELETE FROM arena_team where arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_STATS, "UPDATE arena_team SET rating = ?, matchmakerRating = ?, maxMMR = ?, weekGames = ?, weekWins = ?, seasonGames = ?, seasonWins = ?, `rank` = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_PLAYER_ARENA_TEAMS, "SELECT arenaTeamId FROM arena_team WHERE captainGuid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_UPD_GM_TICKET_COMPLETE, "UPDATE `gm_tickets` SET `completed` = ?, `response` = ? WHERE `ticketId` = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHARACTER, "INSERT INTO characters (guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                                         ...
                                         "power4, power5, power6, power7, latency, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, autoInvite, xpRate, pvpAnnounces) VALUES "
                                         "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
                                         ...
                                         "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,autoInvite=?,xpRate=?,pvpAnnounces=?,online=?,totalSeasonHonorPoints=?,isCountedAsMaxLevelPathOfWarCharacter=?,spellQueueWindow=? WHERE guid=?",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_GROUP_LOOT_TYPE, "UPDATE `groups` SET lootMethod = ?, masterLooterGuid = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_INSTANCE, "UPDATE character_instance SET instance = ?, permanent = ?, extended = 0, CompletedAscendedEncounters = ? WHERE guid = ? AND instance = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_INSTANCE, "INSERT INTO character_instance (guid, instance, permanent, extended, CompletedAscendedEncounters) VALUES (?, ?, ?, 0, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_DEL_INFO_BY_GUID, "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, extra_flags FROM characters WHERE deleteDate IS NOT NULL AND guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_DEL_INFO, "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, extra_flags FROM characters WHERE deleteDate IS NOT NULL", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_PETITION, "INSERT INTO petition (ownerguid, petitionguid, name, type, petitionid) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UDP_CHAR_HONOR_POINTS, "UPDATE characters SET totalHonorPoints = ?, totalSeasonHonorPoints = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_ACTION, "REPLACE INTO character_action (guid, spec, button, action, type) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_STATS, "INSERT INTO character_stats (guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, maxpower6, maxpower7, strength, agility, stamina, intellect, spirit, "
                                          ...
                                          "spellPower, resilience, achievementPoints, avgItemLevel, penArmor, penSpell, ratExpertise, ratHaste, regMana, spellHit, meleeHit, rangeHit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PET_SLOTS_DETAIL, "SELECT owner, id, entry, level, name, slot, PetType FROM character_pet WHERE owner = ? AND ( slot >= ? AND slot <= ? OR slot = ? ) ORDER BY slot", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UDP_CHAR_HUNTER_PET_SLOT_BY_SLOT, "UPDATE character_pet SET slot = ? WHERE owner = ? AND slot = ? and PetType = 1", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_PET_AURA_BY_ID_AND_OWNER, "DELETE pa.* FROM pet_aura pa LEFT JOIN character_pet cp ON pa.guid=cp.id WHERE pa.spell = ? AND cp.owner = ?", CONNECTION_ASYNC);

    // SAVE LOOT SYSTEM
    PrepareStatement(CHAR_SEL_LOOT_SYSTEM_ITEM_COUNT, "SELECT itemCount FROM group_saved_loot WHERE itemId= ? AND instanceId= ? AND objectId= ? AND isCreature= ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_UPD_LOOT_SYSTEM_ITEM_COUNT, "UPDATE group_saved_loot SET itemCount = ? WHERE instanceId = ? AND itemId = ? AND objectId = ? AND isCreature = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_LOOT_SYSTEM_ITEM, "INSERT INTO group_saved_loot VALUES (?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_LOOT_SYSTEM_CREATURE_LOOT, "DELETE FROM group_saved_loot WHERE objectId = ? AND instanceId = ? AND isCreature = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_LOOT_SYSTEM_ITEM, "DELETE FROM group_saved_loot WHERE instanceId = ? AND itemId = ? AND objectId = ? AND isCreature = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_LOOT_SYSTEM_LOOT, "SELECT itemId, itemCount, playerGuids FROM group_saved_loot WHERE objectId= ? AND instanceId= ? AND isCreature= ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_MAIL_EXTERNAL_WORLD,
        "SELECT id, subject, message, money, item, item_count, receiver FROM mail_external WHERE id > ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MAIL_EXTERNAL_MAX, "SELECT MAX(id) FROM mail_external", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_WARDEN_CHECK_FAILURE, "INSERT INTO warden_lua_failures (AccountId, CharacterGuid, CharacterName, FailureDate, CallStack) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_LOG_AUCTION_HOUSE, "INSERT INTO `log_auction_house` (`date`, `auctionId`, `playerGuid`, `playerName`, `itemEntry`, `itemGuid`, `itemCount`, `action`, `bid`, `buyout`, `comment`) VALUES (CURRENT_TIMESTAMP(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    // Guardian spells
    PrepareStatement(CHAR_REP_GUARDIAN_SPELLS, "REPLACE INTO guardian_spell (player_guid, spell, active, value_type) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_GUARDIAN_SPELLS, "SELECT spell, active, value_type FROM guardian_spell WHERE player_guid = ?", CONNECTION_ASYNC);

    // Mythic system global
    PrepareStatement(CHAR_SEL_MYTHIC_STATE_VALUE, "SELECT StateValue FROM mythic_states WHERE StateType = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_MYTHIC_STATE_VALUE, "REPLACE INTO mythic_states (StateType, StateValue) VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MYTHIC_MAP_DATA, "SELECT MapId, KeyLevel, TimeType, TimeValue, TimeHolders FROM mythic_map_data", CONNECTION_SYNCH);

    // Mythic system per player
    PrepareStatement(CHAR_SEL_MYTHIC_KEY_LEVEL, "SELECT KeyLevel FROM character_mythic_key_levels WHERE PlayerGUID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_MYTHIC_KEY_LEVEL, "REPLACE INTO character_mythic_key_levels (PlayerGUID, KeyLevel) VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_MYTHIC_RECORD, "REPLACE INTO mythic_map_data (MapId, KeyLevel, TimeType, TimeValue, TimeHolders) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    //PrepareStatement(CHAR_UPD_MYTHIC_SILVER_RECORD, "UPDATE mythic_map_data SET SilverRecord = ?, SilverRecordHolders = ? WHERE MapId = ?", CONNECTION_ASYNC);
    //PrepareStatement(CHAR_UPD_MYTHIC_BRONZE_RECORD, "UPDATE mythic_map_data SET BronzeRecord = ?, BronzeRecordHolders = ? WHERE MapId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MYTHIC_REWARDS, "SELECT RewardBracketOne, RewardBracketTwo, RewardBracketThree, MythicPower FROM mythic_player_rewards WHERE PlayerGUID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_MYTHIC_REWARDS, "REPLACE INTO mythic_player_rewards (PlayerGUID, RewardbracketOne, RewardBracketTwo, RewardBracketThree, MythicPower) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_MYTHIC_TRANSMOGS, "SELECT `ItemEntry` FROM `character_mythic_transmogs` WHERE `PlayerGUID` = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_MYTHIC_TRANSMOGS, "INSERT INTO `character_mythic_transmogs` VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_MYTHIC_TRANSMOGS, "DELETE FROM `character_mythic_transmogs` WHERE `PlayerGUID` = ? AND `ItemEntry` = ? LIMIT 1", CONNECTION_ASYNC);

    // Mythic companion
    PrepareStatement(CHAR_SEL_MYTHIC_COMPANION_DATA, "SELECT CompanionExp, CompanionLevel, Race, Gender, "
                                                     "Head, Shoulders, Shirt, Chest, Waist, Pants, Boots, Wrists, Hands, Cloak, Tabard, MainHand, OffHand, RangedWeapon, "
                                                     "ToolBank, ToolGuildBank, ToolAuctionHouse, ToolMassRess, BoughtGoblinRace, BoughtTaunkaRace "
                                                     "FROM mythic_companion_data WHERE PlayerGUID = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_MYTHIC_COMPANION_EXP_LEVEL, "REPLACE INTO mythic_companion_data (PlayerGUID, CompanionExp, CompanionLevel, Race, Gender, Head, Shoulders, Shirt, Chest, Waist, Pants, Boots, Wrists, Hands, Cloak, Tabard, MainHand, OffHand, RangedWeapon, ToolBank, ToolGuildBank, ToolAuctionHouse, ToolMassRess, BoughtGoblinRace, BoughtTaunkaRace) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    // end of companion
    PrepareStatement(CHAR_INS_MYTHIC_KEY_USED, "INSERT INTO `character_mythic_key_store` VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_MYTHICPOWER_LOG, "INSERT INTO `log_mythicpower`(guid, type, data1, data2, value) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    // Multispec
    PrepareStatement(CHAR_SEL_SPEC_DATA, "SELECT SpecID, SpecName FROM character_spec_data WHERE CharacterID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_SPEC_DATA, "REPLACE INTO character_spec_data (CharacterID, SpecID, SpecName) VALUES (?, ?, ?)", CONNECTION_ASYNC);

    // Misc
    PrepareStatement(CHAR_SEL_MAXLVL_CHARACTER_COUNT, "SELECT COUNT(`guid`) FROM `characters` WHERE `account` = ? AND `level` >= ? AND NOT (`extra_flags` & 0x10000000)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_NOT_RESTRICTED_WARPATH_CHARACTER_COUNT, "SELECT COUNT(`guid`) FROM `characters` WHERE `account` = ? AND `level` >= ? AND `extra_flags` & 0x10000000 AND `totalSeasonHonorPoints` >= ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_SOLO_QUEUE_MATCHING_LOG, "INSERT INTO solo_queue_matching_logs (match_id, match_time, teamA_leaderTeamID, teamH_leaderTeamID, teamA_MMR, teamH_MMR, teamA_waitTimer, teamH_waitTimer) VALUES (?, NOW(), ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    // Transmog
    PrepareStatement(CHAR_DEL_ACCOUNT_TRANSMOG, "DELETE FROM `custom_account_transmog` WHERE `accountid` = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_ACCOUNT_TRANSMOG, "SELECT `type`, `entry`, `purchased` FROM `custom_account_transmog` WHERE `accountid` = ? ORDER BY `entry`", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ACCOUNT_TRANSMOG, "REPLACE INTO `custom_account_transmog` (`accountid`, `type`, `entry`, `purchased`) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_PURCHASED_TRANSMOG_SETS, "DELETE FROM `custom_purchased_transmogrification_sets` WHERE owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_PURCHASED_TRANSMOG_SETS, "SELECT `presetid`, `setname`, `setdata` FROM `custom_purchased_transmogrification_sets` WHERE `owner` = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_PURCHASED_TRANSMOG_SETS, "REPLACE INTO `custom_purchased_transmogrification_sets` (`owner`, `presetid`, `setname`, `setdata`) VALUES (?, ?, ?, ?)", CONNECTION_BOTH);
    PrepareStatement(CHAR_SEL_HIGHEST_PURCHASED_TRANSMOG_SET, "SELECT MAX(`presetid`) FROM `custom_purchased_transmogrification_sets` WHERE owner = ?", CONNECTION_SYNCH);
}
