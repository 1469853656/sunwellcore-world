...
static uint32 SizeForType(MYSQL_FIELD* field)
{
    switch (field->type)
    {
        ...
        */
        default:
            sLog->outErrorDb("SQL::SizeForType(): invalid field type %u", uint32(field->type));
            return 0;
    }
}
...
DatabaseFieldTypes MysqlTypeToFieldType(enum_field_types type)
{
    switch (type)
    {
        ...
        default:
            sLog->outErrorDb("MysqlTypeToFieldType(): invalid field type %u", uint32(type));
            break;
    }
...
m_metadataResult(result)
{
    ...
    {
        sLog->outErrorDb("%s:mysql_stmt_store_result, cannot bind result from MySQL server. Error: %s", __FUNCTION__, mysql_stmt_error(m_stmt));
        delete[] m_rBind;
        delete[] m_isNull;
        delete[] m_length;
        return;
    ...
    {
        sLog->outErrorDb("%s:mysql_stmt_bind_result, cannot bind result from MySQL server. Error: %s", __FUNCTION__, mysql_stmt_error(m_stmt));
...
bool ResultSet::NextRow()
{
    ...
    if (!lengths)
    {
        sLog->outErrorDb("%s:mysql_fetch_lengths, cannot retrieve value lengths. Error %s.", __FUNCTION__, mysql_error(_result->handle));
        CleanUp();
        return false;
    }
