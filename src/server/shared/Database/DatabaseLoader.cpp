...
DatabaseLoader::DatabaseLoader(std::string const& logger, DatabaseType const defaultUpdateMask)
    : _logger(logger), _autoSetup(sConfigMgr->GetBoolDefault("Updates.AutoSetup", true)),
    _updateFlags(static_cast<DatabaseType>(sConfigMgr->GetIntDefault("Updates.EnableDatabases", static_cast<int>(defaultUpdateMask))))
{
}
...
DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, std::string const& name)
{
    ...
    _open.push([this, name, updatesEnabledForThis, &pool]() -> bool
    {
        ...
        if (dbString.empty())
        {
            sLog->outErrorDb("Database %s not specified in configuration file!", name.c_str());
            return false;
        }
        ...
        {
            sLog->outErrorDb("%s database: invalid number of worker threads specified. "
    ...
    if (updatesEnabledForThis)
    {
        _populate.push([this, name, &pool]() -> bool
        {
            ...
            {
                sLog->outErrorDb("Could not populate the %s database, see log for details.", name.c_str());
                return false;
            }
            return true;
        });
        ...
        _update.push([this, name, &pool]() -> bool
        {
            ...
            {
                sLog->outErrorDb("Could not update the %s database, see log for details.", name.c_str());
                return false;
            }
            return true;
        });
    ...
    _prepare.push([this, name, &pool]() -> bool
    {
        ...
        {
            sLog->outErrorDb("Could not prepare statements of the %s database, see log for details.", name.c_str());
            return false;
        }
        return true;
    });
...
{
    if (!_updateFlags.HasAnyFlag())
        sLog->outMisc("Automatic database updates are disabled for all databases!");
...
template
DatabaseLoader& DatabaseLoader::AddDatabase<LogDatabaseConnection>(DatabaseWorkerPool<LogDatabaseConnection>&, std::string const&);
template
