...
uint32 MySQLConnection::Open()
{
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            sLog->outMisc("MySQL client library: %s", mysql_get_client_info());
            sLog->outMisc("MySQL server ver: %s ", mysql_get_server_info(m_Mysql));
        ...
        sLog->outMisc("Connected to MySQL database at %s", m_connectionInfo.host.c_str());
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
    ...
    //sLog->outMisc("sql.sql", "[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString().c_str());
...
bool MySQLConnection::_Query(const char* sql, MySQLResult** pResult, MySQLField** pFields, uint64* pRowCount, uint32* pFieldCount)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            uint32 lErrno = mysql_errno(m_Mysql);
            sLog->outMisc("SQL: %s", sql);
            sLog->outErrorDb("[%u] %s", lErrno, mysql_error(m_Mysql));
    ...
    for (auto itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    sLog->outErrorDb("Transaction aborted. %u queries not executed.", (uint32)queries.size());
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        sLog->outErrorDb("Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, std::string const& sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        sLog->outErrorDb("In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql.c_str());
        sLog->outErrorDb("%s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            sLog->outErrorDb("In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql.c_str());
            sLog->outErrorDb("%s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            if (m_Mysql)
            {
                sLog->outErrorDb("Lost the connection to the MySQL server!");
        ...
        {
            sLog->outMisc("Attempting to reconnect to the MySQL server...");
            ...
            if (!lErrno)
            {
                ...
                {
                    sLog->outCrash("Could not re-prepare statements!");
                    std::this_thread::sleep_for(std::chrono::seconds(10));
                    std::abort();
                }

                sLog->outMisc("Successfully reconnected to %s @%s:%s (%s).",
            ...
            if ((--attempts) == 0)
            {
                ...
                sLog->outCrash("Failed to reconnect to the MySQL server, terminating the server to prevent data corruption!");
        ...
        case ER_NO_SUCH_TABLE:
            sLog->outErrorDb("Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        case ER_PARSE_ERROR:
