...
UpdateFetcher::UpdateFetcher(Path const& sourceDirectory,
    ...
    std::function<void(Path const& path)> applyFile,
    std::function<QueryResult(std::string const&)> retrieve) :
        _sourceDirectory(std::make_unique<Path>(sourceDirectory)), _apply(std::move(apply)), _applyFile(std::move(applyFile)),
        _retrieve(std::move(retrieve))
{
}
...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            sLog->outMisc("Added locale file \"%s\".", itr->path().filename().generic_string().c_str());
            ...
            {
                sLog->outCrash("Duplicate filename \"%s\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
...
#ifndef GF_MODULE_CLASSLESS
        if (path.find("deus") != std::string::npos)
            continue;
#endif
        ...
        if (!is_directory(p))
        {
            sLog->outErrorDb("DBUpdater: Given update include directory \"%s\" does not exist, skipped!", p.generic_string().c_str());
            continue;
        }
        ...
        sLog->outMisc("Added applied file \"%s\" from remote.", p.filename().generic_string().c_str());
...
std::string UpdateFetcher::ReadSQLUpdate(Path const& file) const
{
    ...
    if (!in.is_open())
    {
        sLog->outCrash("Failed to open the sql update \"%s\" for reading! "
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
    ...
    for (auto const& availableQuery : available)
    {
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    sLog->outErrorDb(">> It seems like the update \"%s\" \'%s\' was renamed, but the old file is still there! " \
                ...
                {
                    sLog->outMisc(">> Renaming update \"%s\" to \"%s\" \'%s\'.",
            ...
            {
                sLog->outMisc(">> Applying update \"%s\" \'%s\'...",
                    availableQuery.first.filename().string().c_str(), hash.substr(0, 7).c_str());
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            sLog->outMisc(">> Re-hashing update \"%s\" \'%s\'...", availableQuery.first.filename().string().c_str(),
        ...
        else
        {
            ...
            {
                sLog->outMisc(">> Reapplying update \"%s\" \'%s\' -> \'%s\' (it changed)...", availableQuery.first.filename().string().c_str(),
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            sLog->outErrorDb(">> The file \'%s\' was applied to the database, but is missing in" \
                " your update directory now!", entry.first.c_str());

            if (doCleanup)
                sLog->outMisc("Deleting orphaned entry \'%s\'...", entry.first.c_str());
        }
        ...
        {
            sLog->outErrorDb("%s", fmt::format("Cleanup is disabled! There were {} dirty files applied to your database, but they are now missing in your source directory!", applied.size()).c_str());
        }
    }
