...
class DatabaseWorkerPool
{
    ...
    public:
        ...
        */
        ...
        template<typename Format, typename... Args>
        void FMTExecute(Format&& sql, Args&&... args)
        {
            if (Trinity::IsFormatEmptyOrNull(sql))
                return;

            Execute(fmt::format(std::forward<Format>(sql), std::forward<Args>(args)...).c_str());
        }
        ...
        */
        ...
        template<typename Format, typename... Args>
        QueryResult FMTQuery(Format&& sql, Args&&... args)
        {
            if (Trinity::IsFormatEmptyOrNull(sql))
                return QueryResult(nullptr);

            return Query(fmt::format(std::forward<Format>(sql), std::forward<Args>(args)...).c_str());
        }
