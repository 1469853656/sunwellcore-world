...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    sLog->outMisc("Opening DatabasePool '%s'. "
    ...
    if (!error)
    {
        sLog->outMisc("%s", fmt::format("DatabasePool '{}' opened successfully. {} total connections running.", GetDatabaseName(),
                    (_connections[IDX_SYNCH].size() + _connections[IDX_ASYNC].size())).c_str());
    }
...
{
    sLog->outMisc("Closing down DatabasePool '%s'.", GetDatabaseName());

    //! Closes the actualy MySQL connection.
    _connections[IDX_ASYNC].clear();

    sLog->outMisc("Asynchronous connections on DatabasePool '%s' terminated. "
    ...
    sLog->outMisc("All connections on DatabasePool '%s' closed.", GetDatabaseName());
}
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
            sLog->outError("TrinityCore does not support MySQL versions below 5.1");
            return 1;
        }
        else
        {
...
template class DatabaseWorkerPool<CustomDatabaseConnection>;
