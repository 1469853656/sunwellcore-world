...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_RANK);
    stmt->setUInt32(0, m_guildId);
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, const CharacterDatabaseTransaction& trans, bool logOnCreate /* = false */)
{
...
void Guild::RankInfo::SetNameRightsMoney(std::string const& name, uint32 rights, uint32 money)
{
    ...
    if (m_name != name)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_NAME);
    ...
    if (m_rights != rights)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_RIGHTS);
    ...
    if (m_bankMoneyPerDay != money)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RANK_BANK_MONEY);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    ItemRef pItem = NewItemOrBag(proto);
    if (!pItem->LoadFromDB(itemGuid, 0, fields, itemEntry))
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_NONEXISTENT_GUILD_BANK_ITEM);
        stmt->setUInt32(0, m_guildId);
...
void Guild::BankTab::SetInfo(std::string const& name, std::string const& icon)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_TAB_INFO);
    stmt->setString(0, m_name);
...
void Guild::BankTab::SetText(std::string const& text)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_TAB_TEXT);
...
bool Guild::BankTab::SetItem(CharacterDatabaseTransaction& trans, uint8 slotId, ItemRef const& item)
{
    if (slotId >= GUILD_BANK_MAX_SLOTS)
        return false;
...
void Guild::Member::SetPublicNote(std::string const& publicNote)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_PNOTE);
...
void Guild::Member::SetOfficerNote(std::string const& officerNote)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_OFFNOTE);
...
void Guild::Member::ChangeRank(uint8 newRank)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_RANK);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_MEMBER);
    stmt->setUInt32(0, m_guildId);
    stmt->setUInt32(1, GUID_LOPART(m_guid));
    stmt->setUInt8(2, m_rankId);
...
void Guild::Member::UpdateBankWithdrawValue(CharacterDatabaseTransaction& trans, uint8 tabId, uint32 amount)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_EMBLEM_INFO);
...
bool Guild::MoveItemData::CanStore(ItemRef const& pItem, bool swap, bool sendError)
{
    m_vec.clear();
...
ItemRef Guild::PlayerMoveItemData::StoreItem(CharacterDatabaseTransaction& trans, ItemRef& pItem)
{
    ASSERT(pItem);
...
inline InventoryResult Guild::PlayerMoveItemData::CanStore(ItemRef const& pItem, bool swap)
{
...
ItemRef Guild::BankMoveItemData::StoreItem(CharacterDatabaseTransaction& trans, ItemRef& pItem)
{
    if (!pItem)
        return nullptr;
...
ItemRef Guild::BankMoveItemData::_StoreItem(CharacterDatabaseTransaction& trans, BankTab* pTab, ItemRef& pItem, ItemPosCount& pos, bool clone) const
{
...
bool Guild::BankMoveItemData::_ReserveSpace(uint8 slotId, ItemRef const& pItem, ItemRef const& pItemDest, uint32& count)
{
...
void Guild::BankMoveItemData::CanStoreItemInTab(ItemRef const& pItem, uint8 skipSlotId, bool merge, uint32& count)
{
    for (uint8 slotId = 0; (slotId < GUILD_BANK_MAX_SLOTS) && (count > 0); ++slotId)
    {
        ...
        ItemRef pItemDest = m_pGuild->_GetItem(m_container, slotId);
        if (pItemDest == pItem)
            pItemDest = nullptr;

        // If merge skip empty, if not merge skip non-empty
        if (bool(pItemDest) != merge)
            continue;
...
InventoryResult Guild::BankMoveItemData::CanStore(ItemRef const& pItem, bool swap)
{
    ...
    {
        ItemRef pItemDest = m_pGuild->_GetItem(m_container, m_slotId);
...
void Guild::HandleSetMOTD(WorldSession* session, std::string const& motd)
{
    ...
    else
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MOTD);
...
void Guild::HandleSetInfo(WorldSession* session, std::string const& info)
{
    ...
    if (_HasRankRight(session->GetPlayer(), GR_RIGHT_MODIFY_GUILD_INFO))
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_INFO);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
    ...
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::Guild) && pInvitee->GetTeam(CF_DENY) != player->GetTeam(CF_DENY))
    {
...
{
    Player* player = session->GetPlayer();
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::Guild) && player->GetTeam(CF_DENY) != sObjectMgr->GetPlayerTeamIdByGUID(GetLeaderGUID()))
        return;

    player->InvalidateHardcorePath();
...
void Guild::HandleAddNewRank(WorldSession* session, std::string const& name)
{
    ...
    {
        auto trans = CharacterDatabase.BeginTransaction();
        if (_CreateRank(name, GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK, trans))
        {
            char aux[2];
            sprintf(aux, "%u", size);
            _BroadcastEvent(GE_RANK_UPDATED, 0, aux, name.c_str());
...
void Guild::HandleRemoveRank(WorldSession* session, uint8 rankId)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_RIGHTS_FOR_RANK);
    stmt->setUInt32(0, m_id);
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    _BroadcastEvent(GE_BANK_MONEY_SET, 0, Trinity::StringFormat("%016llX", m_bankMoney).c_str());

    if (amount > 10 * GOLD)
        CharacterDatabase.PExecute("INSERT INTO log_money (sender_acc, sender_guid, sender_name, sender_ip, receiver_acc, receiver_name, money, topic, date) VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, \"<GB DEPOSIT> %s (guild id: %u, members: %u, new amount: " UI64FMTD ", leader guid low: %u, char level: %u)\", NOW())", session->GetAccountId(), player->GetGUIDLow(), player->GetName().c_str(), session->GetRemoteAddress().c_str(), 0, "", amount, GetName().c_str(), GetId(), GetMemberCount(), GetTotalBankMoney(), (uint32)(GetLeaderGUID() & 0xFFFFFFFF), player->getLevel());
}
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint32 amount, bool repair)
{
    ...
    if (amount > 10 * GOLD)
        CharacterDatabase.PExecute("INSERT INTO log_money (sender_acc, sender_guid, sender_name, sender_ip, receiver_acc, receiver_name, money, topic, date) VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, \"<GB WITHDRAW> %s (guild id: %u, members: %u, new amount: " UI64FMTD ", leader guid low: %u, char level: %u)\", NOW())", session->GetAccountId(), player->GetGUIDLow(), player->GetName().c_str(), session->GetRemoteAddress().c_str(), 0, "", amount, GetName().c_str(), GetId(), GetMemberCount(), GetTotalBankMoney(), (uint32)(GetLeaderGUID() & 0xFFFFFFFF), player->getLevel());
...
{
    if (session->GetSecurity() && session->GetSecurity() < AccountTypes(10))
        return;
...
bool Guild::Validate()
{
    ...
    {
        m_ranks.clear();
        _CreateDefaultGuildRanks(DEFAULT_LOCALE, trans);
    }
    CharacterDatabase.CommitTransaction(trans);
...
// Members handling
bool Guild::AddMember(uint64 guid, uint8 rankId, CharacterDatabaseTransaction trans)
{
    ...
    else
    {
        ...
        // Player must exist
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_DATA_FOR_GUILD);
    ...
    if (!trans)
        trans = CharacterDatabaseTransaction(nullptr);
...
void Guild::_CreateNewBankTab()
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GUILD_BANK_TAB);
    stmt->setUInt32(0, m_id);
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc, CharacterDatabaseTransaction& trans)
{
    ...
    _CreateRank(sObjectMgr->GetTrinityString(LANG_GUILD_MASTER, loc), GR_RIGHT_ALL, trans);
    _CreateRank(sObjectMgr->GetTrinityString(LANG_GUILD_OFFICER, loc), GR_RIGHT_ALL, trans);
    _CreateRank(sObjectMgr->GetTrinityString(LANG_GUILD_VETERAN, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK, trans);
    _CreateRank(sObjectMgr->GetTrinityString(LANG_GUILD_MEMBER, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK, trans);
    _CreateRank(sObjectMgr->GetTrinityString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK, trans);
}

bool Guild::_CreateRank(std::string const& name, uint32 rights, CharacterDatabaseTransaction& trans)
{
...
bool Guild::_ModifyBankMoney(CharacterDatabaseTransaction& trans, uint64 amount, bool add)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_BANK_MONEY);
...
void Guild::_SetLeaderGUID(Member* pLeader)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_LEADER);
...
inline void Guild::_UpdateMemberWithdrawSlots(CharacterDatabaseTransaction& trans, uint64 guid, uint8 tabId)
{
    if (Member* member = GetMember(guid))
    {
        uint8 rankId = member->GetRankId();
...
void Guild::_LogBankEvent(CharacterDatabaseTransaction& trans, GuildBankEventLogTypes eventType, uint8 tabId, uint32 lowguid, uint32 itemOrMoney, uint16 itemStackCount, uint8 destTabId)
{
...
inline ItemRef Guild::_GetItem(uint8 tabId, uint8 slotId) const
{
...
{
    ItemRef pDestItem = pDest->GetItem();
    bool swap         = bool(pDestItem);

    ItemRef pSrcItem = pSrc->GetItem(splitedAmount);
...
void Guild::LogGuildBankAction(Player* player, uint8 tabId, uint8 slotId, uint8 playerBag, uint8 playerSlotId)
{
    ...
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_LOG_TRADE);
