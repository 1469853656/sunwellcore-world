...
void WorldSession::HandleAutoEquipItemOpcode(WorldPacket& recvData)
{
    ...
    ItemRef pSrcItem = _player->GetItemByPos(srcbag, srcslot);
    if (!pSrcItem)
    ...
    ItemRef pDstItem = _player->GetItemByPos(dest);
...
void WorldSession::HandleDestroyItemOpcode(WorldPacket& recvData)
{
    ...
    ItemRef pItem = _player->GetItemByPos(bag, slot);
    if (!pItem)
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr);
        return;
...
void WorldSession::HandleItemQuerySingleOpcode(WorldPacket& recvData)
{
    ...
    if (pProto)
    {
        ...
        if (loc_idx != DEFAULT_LOCALE)
        {
            ...
            WorldPacket data(SMSG_ITEM_QUERY_SINGLE_RESPONSE, 1);
...
void WorldSession::HandleSellItemOpcode(WorldPacket& recvData)
{
    ...
    if (_player->GetTradeData())
    {
        sLog->outCheat("Exploit: Player %s is trying to sell item while trading.", _player->GetName().c_str());
        return;
    }

    if (isLogingOut())
    {
        sLog->outError("Player %s (guid: %u, account: %u) is trying to sell item while logging out.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow(), GetAccountId());
        return;
    }
    ...
    ItemRef pItem = _player->GetItemByGuid(itemguid);
    if (pItem)
    {
        ...
        if (pProto)
        {
            if (pProto->SellPrice > 0)
            {
                ...
                {
                    ItemRef pNewItem = pItem->CloneItem(count, _player);
                    if (!pNewItem)
                    {
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
    ...
    ItemRef pItem = _player->GetItemFromBuyBackSlot(slot);
    if (pItem)
    {
        uint32 price = _player->GetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 + slot - BUYBACK_SLOT_START);
        if (!_player->HasEnoughMoney(price, _player->IsPathOfWar()))
        {
    ...
    for (uint8 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
                ...
                if (!_player->IsGameMaster() && !sWorld->IsCrossfactionEnabled(Crossfaction::Item) && ((itemTemplate->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY && _player->GetTeam(CF_DENY) == TEAM_ALLIANCE) || (itemTemplate->Flags2 == ITEM_FLAGS_EXTRA_ALLIANCE_ONLY && _player->GetTeam(CF_DENY) == TEAM_HORDE)))
                    continue;
...
void WorldSession::HandleAutoStoreBagItemOpcode(WorldPacket& recvData)
{
    ...
    ItemRef pItem = _player->GetItemByPos(srcbag, srcslot);
    if (!pItem)
        return;
...
void WorldSession::HandleAutoStoreBankItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (_player->IsBankPos(srcbag, srcslot)) // moving from bank to inventory
    {
        ...
        _player->RemoveItem(srcbag, srcslot, true);
        if (ItemRef storedItem = _player->StoreItem(dest, pItem, true))
...
void WorldSession::HandleWrapItemOpcode(WorldPacket& recvData)
{
    ...
    ItemRef gift = _player->GetItemByPos(gift_bag, gift_slot);
    if (!gift)
    {
    ...
    if (isLogingOut())
    {
        sLog->outError("Player %s (guid: %u, account: %u) is trying to wrap item while logging out.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow(), GetAccountId());
        _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, gift, nullptr);
        return;
    }
    ...
    ItemRef item = _player->GetItemByPos(item_bag, item_slot);
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_GIFT);
...
void WorldSession::HandleSocketOpcode(WorldPacket& recvData)
{
    ...
    ItemRef itemTarget = _player->GetItemByGuid(item_guid);
    ...
    ItemRef Gems[MAX_GEM_SOCKETS];
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i)
        Gems[i] = gem_guids[i] ? _player->GetItemByGuid(gem_guids[i]) : ItemRef(nullptr);
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i)
    {
        if (GemEnchants[i])
        {
            ...
            if (ItemRef guidItem = _player->GetItemByGuid(gem_guids[i]))
...
void WorldSession::HandleCancelTempEnchantmentOpcode(WorldPacket& recvData)
{
    ...
    ItemRef item = GetPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, eslot);
...
void WorldSession::HandleItemRefundInfoRequest(WorldPacket& recvData)
{
    ...
    ItemRef item = _player->GetItemByGuid(guid);
    if (!item)
    {
...
void WorldSession::HandleItemTextQuery(WorldPacket& recvData)
{
    ...
    if (ItemRef item = _player->GetItemByGuid(itemGuid))
    {
