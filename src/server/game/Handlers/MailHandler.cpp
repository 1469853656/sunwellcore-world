...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (isLogingOut())
    {
        sLog->outError("Player %s (guid: %u, account: %u) is trying to send mail while logging out.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow(), GetAccountId());
        recvData.rfinish();
        return;
    }
    ...
    {
        sLog->outError("Player %s tried to send mail with a subject that has invalid utf8 sequence, blocking.", GetPlayer()->GetName().c_str());
        return;
    }

    if (!utf8::is_valid(body.begin(), body.end()))
    {
        sLog->outError("Player %s tried to send mail with a body that has invalid utf8 sequence, blocking.", GetPlayer()->GetName().c_str());
        return;
    }

    bool const subjectInvalid = (subject.find("| |") != std::string::npos) || subject.size() > MAIL_SUBJECT_MAXIMUM_SIZE;
    bool const bodyInvalid    = (body.find("| |") != std::string::npos) || body.size() > MAIL_BODY_MAXIMUM_SIZE;
    if (subjectInvalid || bodyInvalid)
        return;
    ...
    {
        ChatHandler(player->GetSession()).SendSysMessage("Characters in Path of War are unable to send in-game mail.");
        return;
    }
    ...
    bool rc_isHardcore = false;
    bool isHardcore = player->HasHardcoreEnabled();

    if (receive)
    {
        rc_teamId   = receive->GetTeam(CF_DENY);
        mails_count = receive->GetMailSize();
        rc_isHardcore = receive->HasHardcoreEnabled();
    }
    else
    {
        ...
        if (GlobalPlayerData const* playerData = sWorld->GetGlobalPlayerData(GUID_LOPART(rc)))
        {
            ...
            rc_isHardcore = playerData->IsHardcore;
        }
    }
    ...
    if (isHardcore && !rc_isHardcore)
    {
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantSendMailsToNormal))
    ...
    if (!isHardcore && rc_isHardcore)
    {
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantSendMailsToHardcore))
            ChatHandler(this).SendSysMessage(message->c_str());
    ...
    for (uint8 i = 0; i < items_count; ++i)
    {
        ItemRef item = player->GetItemByGuid(itemGUIDs[i]);
        if (item)
        {
    ...
    if (rc_isHardcore && accountBound)
    {
        ChatHandler(player->GetSession()).SendSysMessage("You can't send account bound items to hardcore characters!");
        player->SendMailResult(0, MAIL_SEND, MAIL_ERR_NOT_YOUR_TEAM);
        return;
    }
    ...
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::Mail) && !accountBound && player->GetTeam(CF_DENY) != rc_teamId && AccountMgr::IsPlayerAccount(GetSecurity()))
    {
        player->SendMailResult(0, MAIL_SEND, MAIL_ERR_NOT_YOUR_TEAM);
        return;
    }

    if (sWorld->getBoolConfig(CONFIG_MAIL_SPAM_ENABLE) && !player->IsFriendOfMine(rc, true) && GetAccountId() != rc_account && player->GetTotalPlayedTime() < 12 * DAY)
    {
...
void WorldSession::HandleMailReturnToSender(WorldPacket& recvData)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_MAIL_BY_ID);
...
void WorldSession::HandleMailTakeItem(WorldPacket& recvData)
{
    ...
    {
        sLog->outError("Player (%s)(GUID: " UI64FMTD ") is taking an item with GUID (%u) but he already has an item with that GUID!", _player->GetName().c_str(), _player->GetGUID(), itemGuidLow);
    ...
    for (Mail const& mail : mails)
    {
        ...
        switch (mail.messageType)
        {
            case MAIL_NORMAL: // sender guid
                data << uint64(MAKE_NEW_GUID(mail.sender, 0, HIGHGUID_PLAYER));
                break;
            case MAIL_CREATURE:
        ...
        {
            ItemRef item = player->GetMItem(mail.items[i].item_guid);
            // item index (0-6?)
            data << uint8(i);
            // item guid low?
...
void WorldSession::HandleQueryNextMailTime(WorldPacket& /*recvData*/)
{
    ...
    if (_player->unReadMails > 0)
    {
        ...
        PlayerMails const mails = _player->GetMails();
        for (Mail const& mail : mails)
        {
            ...
            data << uint64(mail.messageType == MAIL_NORMAL ? mail.sender : 0); // player guid
