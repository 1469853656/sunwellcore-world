...
void WorldSession::SendShowBank(uint64 guid)
{
    ...
    {
        ChatHandler(this).SendSysMessage("This action is blocked for Path of War characters.");
        return;
    }
...
void WorldSession::SendTrainerList(uint64 guid, const std::string& strTitle)
{
    ...
    for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin(); itr != trainer_spells->spellList.end(); ++itr)
    {
        ...
        bool const _isClassTrainer = ci->trainer_type == TRAINER_TYPE_CLASS
            && _player->getClass() == ci->trainer_class;
        uint32 _spellCost = static_cast<uint32>(tSpell->spellCost * fDiscountMod);

        if (sCustomMgr->CheckRealmPhase(RealmPhase::REALM_PHASE_PTR) || (sWorld->getBoolConfig(CONFIG_FREE_SPELLS) && _isClassTrainer))
            _spellCost = 0;
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    CreatureTemplate const* cTemplate = unit->GetCreatureTemplate();
    if (!cTemplate)
        return;

    bool const _hasEnoughMoney = _player->HasEnoughMoney(nSpellCost, _player->IsPathOfWar());
    bool const _isClassTrainer = cTemplate->trainer_type == TRAINER_TYPE_CLASS
        && _player->getClass() == cTemplate->trainer_class;

    //! Check if player has enough money
    //! Do not check if realm is PTR or free spells enabled and trainer is just a class trainer
    //! not a riding trainer etc
    bool const _doMoneyCheck = sCustomMgr->CheckRealmPhase(RealmPhase::REALM_PHASE_PTR) || (sWorld->getBoolConfig(CONFIG_FREE_SPELLS) && _isClassTrainer) ? false : true;

    if (_doMoneyCheck && !_hasEnoughMoney)
        return;
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
    ...
    */
    ...
    if (unit->GetEntry() == Mythic::MYTHIC_COMPANION_ENTRY && _player->GetMythicCompanionGUID() != unit->GetGUID())
        return;
...
void WorldSession::HandleStablePet(WorldPacket& recvData)
{
    ...
    if (!pet || pet->getPetType() != HUNTER_PET || !pet->IsAlive() || pet->isBeingLoaded())
    {
        SendStableResult(STABLE_ERR_STABLE);
        return;
    }
        ...
        if (stableSlot != nullptr && stableSlot->Empty)
        {
            for (PetSaveMode slot2 : { PET_SAVE_AS_CURRENT, PET_SAVE_NOT_IN_SLOT })
            {
                if (PetSlotData* currSlot = GetPlayer()->GetPetSlotData(slot2))
                {
                    ...
                    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UDP_CHAR_HUNTER_PET_SLOT_BY_SLOT);
