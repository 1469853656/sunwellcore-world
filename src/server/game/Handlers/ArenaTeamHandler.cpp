...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket& recvData)
{
    ...
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::ArenaTeam) && player->GetTeam() != GetPlayer()->GetTeam())
    {
...
void WorldSession::HandleArenaTeamAcceptOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::ArenaTeam) && _player->GetTeam(CF_DENY) != sObjectMgr->GetPlayerTeamIdByGUID(arenaTeam->GetCaptain()))
    {
        SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", "", ERR_ARENA_TEAM_NOT_ALLIED);
        return;
    }

    if (sWorld->HasRecentlyLeftArenaTeam(_player->GetGUID()))
    {
        SendNotification("You have recently left an arena team, please wait before joining new one.");
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket& recvData)
{
    ...
    sWorld->AddToArenaActivity(_player->GetGUIDLow());
