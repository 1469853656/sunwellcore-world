...
class CharacterInfoQueryHolder : public CharacterDatabaseQueryHolder
{
private:
    uint32 m_accountId;
    uint64 m_guid;
...
public:
    CharacterInfoQueryHolder(uint32 accountId, uint64 guid)
...
bool CharacterInfoQueryHolder::Initialize()
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUARDIAN_SPELLS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_GET_GUARDIAN_SPELLS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAIL_EXTERNAL);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_EXTERNAL_MAILS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_KEY_LEVEL);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MYTHIC_KEY_LEVEL, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_COMPANION_DATA);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MYTHIC_COMPANION_DATA, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_REWARDS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MYTHIC_REWARDS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_TRANSMOGS);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_MYTHIC_TRANSMOGS, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_SPEC_DATA);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_SPEC_DATA, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAXLVL_CHARACTER_COUNT);
    stmt->setUInt32(0, m_accountId);
    stmt->setUInt8(1, static_cast<uint8>(DEFAULT_MAX_LEVEL));
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_MAX_LVL_CHARACTER_COUNT, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_NOT_RESTRICTED_WARPATH_CHARACTER_COUNT);
    stmt->setUInt32(0, m_accountId);
    stmt->setUInt8(1, static_cast<uint8>(DEFAULT_MAX_LEVEL));
    stmt->setUInt32(2, sCustomMgr->GetIntConfig(CONFIG_PATH_OF_WAR_HONOR_REQUIRED));
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_NOT_RESTRICTED_WARPATH_CHARACTER_COUNT, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_PURCHASED_TRANSMOG_SETS);
    stmt->setUInt32(0, m_accountId);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_PURCHASED_TRANSMOG_SETS, stmt);
...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (Player::BuildEnumData(result->Fetch(), &data))
            {
    ...
    if (_warden)
        _warden->SendCharacterScreenScript();
...
void WorldSession::HandleCharEnumOpcode(WorldPacket& /*recvData*/)
{
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback([this]<typename T>(T&& PH1) { HandleCharEnum(std::forward<T>(PH1)); }));
}
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    recvData >> createInfo->Gender >> createInfo->Skin >> createInfo->Face;
    recvData >> createInfo->HairStyle >> createInfo->HairColor >> createInfo->FacialHair >> createInfo->OutfitId;

    WorldPacket data(SMSG_CHAR_CREATE, 1); // returned with diff.values in all cases

    if (!Player::ValidateAppearance(createInfo->Race, createInfo->Class, createInfo->Gender, createInfo->HairStyle, createInfo->HairColor, createInfo->Face, createInfo->FacialHair, createInfo->Skin, true))
    {
        sLog->outCheat("Account %u is trying to create character with invalid appearance!", GetAccountId());
    ...
    if (!classEntry)
    {
        ...
        SendPacket(&data);
        sLog->outError("Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo->Class, GetAccountId());
        return;
    }
    ...
    if (!raceEntry)
    {
        data << (uint8)CHAR_CREATE_FAILED;
        SendPacket(&data);
        sLog->outError("Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", createInfo->Race, GetAccountId());
        return;
    }
    ...
    if (raceEntry->expansion > Expansion())
    {
        ...
        sLog->outError("Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, createInfo->Race);
        SendPacket(&data);
        return;
    }
    ...
    if (classEntry->expansion > Expansion())
    {
        ...
        sLog->outError("Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, createInfo->Class);
        SendPacket(&data);
        return;
    }
    ...
    if (!utf8::is_valid(createInfo->Name.begin(), createInfo->Name.end()))
    {
    ...
    stmt->setString(0, createInfo->Name);
     ...
     _queryProcessor.AddCallback(
        ...
        .WithChainingPreparedCallback([this](QueryCallback& queryCallback, PreparedQueryResult selCheckNameResult)
        {
            if (selCheckNameResult)
            {
                WorldPacket data(SMSG_CHAR_CREATE, 1);
                data << static_cast<uint8>(CHAR_CREATE_NAME_IN_USE);
                SendPacket(&data);
                return;
            }

            auto const sumRealmCharactersStmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_SUM_REALM_CHARACTERS);
            sumRealmCharactersStmt->setUInt32(0, GetAccountId());
            queryCallback.SetNextQuery(LoginDatabase.AsyncQuery(sumRealmCharactersStmt));
        })
        .WithChainingPreparedCallback([this](QueryCallback& queryCallback, PreparedQueryResult sumRealmCharactersResult)
        {
            uint16 acctCharCount = 0;
            if (sumRealmCharactersResult)
            {
                Field const* fields = sumRealmCharactersResult->Fetch();
            ...
            {
                WorldPacket data(SMSG_CHAR_CREATE, 1);
                data << static_cast<uint8>(CHAR_CREATE_ACCOUNT_LIMIT);
                SendPacket(&data);
                return;
            }

            auto sumCharsStmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_SUM_CHARS);
            sumCharsStmt->setUInt32(0, GetAccountId());
            queryCallback.SetNextQuery(CharacterDatabase.AsyncQuery(sumCharsStmt));
        })
        .WithChainingPreparedCallback([this, createInfo](QueryCallback& queryCallback, PreparedQueryResult sumCharsResult)
        {
            if (sumCharsResult)
            {
                Field const* fields         = sumCharsResult->Fetch();
                createInfo->CharCount = static_cast<uint8>(fields[0].GetUInt64()); // SQL's COUNT() returns uint64 but it will always be less than uint8.Max

                if (createInfo->CharCount >= sWorld->getIntConfig(CONFIG_CHARACTERS_PER_REALM))
                {
                    WorldPacket data(SMSG_CHAR_CREATE, 1);
                    data << static_cast<uint8>(CHAR_CREATE_SERVER_LIMIT);
                    SendPacket(&data);
                    return;
                }
            }
            ...
            std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo](PreparedQueryResult result)
            {
                ...
                bool allowTwoSideAccounts = sWorld->IsCrossfactionEnabled(Crossfaction::CharacterCreation);
                ...
                sWorld->AddGlobalPlayerData(newChar->GetGUIDLow(), GetAccountId(), newChar->GetName(), newChar->getGender(),
                    newChar->getRace(), newChar->getClass(), newChar->getLevel(), 0, 0, AccountMgr::GetName(GetAccountId()), false, false);

                auto const getCharactersDataStmt = CustomDatabase.GetPreparedStatement(CUSTOM_SEL_CHARACTER_DATA);
                getCharactersDataStmt->setUInt32(0, newChar->GetGUIDLow());

                GetQueryProcessor().AddCallback(CustomDatabase.AsyncQuery(getCharactersDataStmt).WithPreparedCallback([characterID = newChar->GetGUIDLow()](PreparedQueryResult characterDataResult)
                {
                    if (!characterDataResult)
                        sCustomMgr->SaveCharacterCustomData(characterID, false); // data not found, create new row in database
                }));
            };

            bool const allowTwoSideAccounts = sWorld->IsCrossfactionEnabled(Crossfaction::CharacterCreation);
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    /*
    if (sArenaTeamMgr->GetArenaTeamByCaptain(guid) || sSoloQueueMgr->GetSoloQueueTeamByOwner(guid))
    {
        WorldPacket data(SMSG_CHAR_DELETE, 1);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    /***                 NAME RESERVATION                  ***/
    /*********************************************************/
    sCustomMgr->KickPlayerWhileNameReservationIsEnabled(this);
    ...
    std::shared_ptr<CharacterInfoQueryHolder> charHolder = std::make_shared<CharacterInfoQueryHolder>(GetAccountId(), playerGuid);
    if (!charHolder->Initialize())
    {
        m_playerLoading = false;
        return;
    }

    AddQueryHolderCallback(CharacterDatabase.DelayQueryHolder(charHolder)).AfterComplete([this](SQLQueryHolderBase const& holder)
    {
        HandlePlayerLoginFromDB(static_cast<CharacterInfoQueryHolder const&>(holder));
    });
}

void WorldSession::HandlePlayerLoginFromDB(CharacterInfoQueryHolder const& holder)
{
    ...
    // Send MOTD
    {
        ...
        if (sWorld->getIntConfig(CONFIG_ENABLE_SINFO_LOGIN) == 1)
            chH.SendSysMessage(GitRevision::GetFullVersion());
    ...
    if (!pCurrChar->getCinematic())
    {
        ...
        if (ChrClassesEntry const* cEntry = sChrClassesStore.LookupEntry(pCurrChar->getClass()))
        {
            ...
            if (!sWorld->GetNewCharString().empty())
                chH.SendSysMessage(sWorld->GetNewCharString().c_str());
        }
    }
    ...
    auto loginStmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_ONLINE);
    loginStmt->setUInt8(0, 1);
    ...
    if (pCurrChar->HasAtLoginFlag(AT_LOGIN_FIRST))
    {
        ...
        sCustomMgr->HandleFirstLogin(pCurrChar);
    }
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_FREE_NAME);
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback([this, newName]<typename T>(T&& param) { HandleChangePlayerNameOpcodeCallBack(std::forward<T>(param), newName); }));
}
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_NAME);
...
void WorldSession::HandleSetPlayerDeclinedNames(WorldPacket& recvData)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_DECLINED_NAME);
    stmt->setUInt32(0, GUID_LOPART(guid));
    trans->Append(stmt);
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    if (!_player->HasEnoughMoney(cost, _player->IsPathOfWar()))
    {
        WorldPacket data(SMSG_BARBER_SHOP_RESULT, 4);
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_AT_LOGIN);
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    // xinef: zomg! sync query
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_AT_LOGIN_TITLES_MONEY);
    stmt->setUInt32(0, lowGuid);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);
    ...
    if (level < 10 && money <= 0 || level > 10 && level <= 30 && money <= 3000000 || level > 30 && level <= 50 && money <= 10000000 || level > 50 && level <= 70 && money <= 50000000 || level > 70 && money <= 200000000)
        valid = true;
    if (!valid)
    {
        WorldPacket data(SMSG_CHAR_FACTION_CHANGE, 1);
    ...
    if (oldRace != race)
    {
        ...
        CustomDatabase.PExecute("UPDATE characters_data SET CurrentRacial = %u WHERE CharacterID = %u", race, GUID_LOPART(guid));
        ...
        sCustomMgr->RemoveRacialShift(lowGuid);
        ...
        if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
        {
            ...
            if (!sWorld->IsCrossfactionEnabled(Crossfaction::Guild))
            {
                // Reset guild
            ...
            // Delete Friend List
            if (!sWorld->IsCrossfactionEnabled(Crossfaction::FriendList))
            {
            ...
            if (knownTitlesStr)
            {
                ...
                for (uint32 index = 0; index < ktcount; ++index)
                    knownTitles[index] = std::atol(tokensTitles[index]);
    ...
    if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
    {
        auto stmnt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ADD_AT_LOGIN_FLAG);
