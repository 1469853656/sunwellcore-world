...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!ticket || ticket->IsClosed())
    {
...

        {
            ...
            chH.SendSysMessage("Ticket has been created. Please remember that tickets that are not specific and do not meet requirements shall be closed without the answer.");
        }
...
void WorldSession::HandleGMSurveySubmit(WorldPacket& recv_data)
{
    ...
    for (uint8 i = 0; i < 10; i++)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GM_SUBSURVEY);
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GM_SURVEY);
...
void WorldSession::HandleReportLag(WorldPacket& recv_data)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_LAG_REPORT);
    stmt->setUInt32(0, GUID_LOPART(GetPlayer()->GetGUID()));
