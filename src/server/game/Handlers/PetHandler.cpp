...
uint8 WorldSession::HandleLoadPetFromDBFirstCallback(PreparedQueryResult result, uint8 asynchLoadType)
{
    ...
    auto holder = std::make_shared<LoadPetFromDBQueryHolder>(pet_number, current, uint32(time(nullptr) - fields[14].GetUInt32()), fields[13].GetString(), savedhealth, savedmana);
    ...
    uint32 petlevel = ( uint32 )fields[4].GetUInt8();
    ...
    switch (pet->getPetType())
    {
        case SUMMON_PET:
            ...
            else if (pet->GetEntry() == NPC_FELGUARD)
                pet->SetUInt32Value(UNIT_FIELD_BYTES_0, 0x100); // class = warrior
            else
    ...
    if (petSlot)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UDP_CHAR_PET_SLOT_BY_SLOT_EXCLUDE_ID);
...
void WorldSession::HandleLoadPetFromDBSecondCallback(LoadPetFromDBQueryHolder const& holder) const
{
    if (!GetPlayer())
        return;
...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint16 spellid, uint16 flag, uint64 guid2)
{
    ...
    if (!charmInfo)
    {
        sLog->outError("WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u, owner name: %s) is considered pet-like but doesn't have a charminfo!",
            guid1, guid2, spellid, flag, pet->GetGUIDLow(), pet->GetTypeId(), (pet->GetCharmerOrOwner() ? pet->GetCharmerOrOwner()->GetName().c_str() : ""));
        return;
    }
            ...
            switch (spellid)
            {
                ...
                case COMMAND_ATTACK:                        //spellid=1792  //ATTACK
                {
                    ...
                    if (Creature* creaturePet = pet->ToCreature())
                        if (!creaturePet->_CanDetectFeignDeathOf(TargetUnit) || !creaturePet->CanCreatureAttack(TargetUnit, true) || creaturePet->isTargetNotAcceptableByMMaps(TargetUnit->GetGUID(), sWorld->GetGameTime(), TargetUnit))
                            return;

                    // Not let attack through obstructions
                    bool checkLos = !World::IsPathfindingEnabled(pet->GetMap()) || (TargetUnit->GetTypeId() == TYPEID_UNIT && (TargetUnit->ToCreature()->isWorldBoss() || TargetUnit->ToCreature()->IsDungeonBoss()));
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (!spellInfo)
            {
                if (pet && pet->GetCharmerOrOwner())
                    sLog->outReleaseDebug("World HandlePetActionHelper: Unknown PET spell Id %u GetcharmerOrOwner: %s And Guid: " UI64FMTD, spellid, pet->GetCharmerOrOwner()->GetName().c_str(), pet->GetCharmerOrOwner()->GetGUID());
                else
...
void WorldSession::HandlePetRename(WorldPacket & recvData)
{
    ...
    if (isdeclined)
    {
        ...
        {
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_PET_DECLINEDNAME);
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_PET_NAME);
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    {
        if (GetPlayer())
            sLog->outError("World HandlePetCastSpellOpcode: Unknown PET Spell Id %i Player: (GUID: " UI64FMTD " Name: %s).", spellId, GetPlayer()->GetGUID(), GetPlayer()->GetName().c_str());
        else
