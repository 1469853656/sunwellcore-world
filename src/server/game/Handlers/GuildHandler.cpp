...
void WorldSession::HandleGuildInviteOpcode(WorldPacket& recvPacket)
{
    ...
    if (!ValidateInputString(invitedName, MAX_PLAYER_NAME))
    {
...
void WorldSession::HandleGuildRemoveOpcode(WorldPacket& recvPacket)
{
    ...
    if (!ValidateInputString(playerName, MAX_PLAYER_NAME))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());
...
void WorldSession::HandleGuildLeaderOpcode(WorldPacket& recvPacket)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "CMSG_GUILD_LEADER [%s]: Target: %s", GetPlayerInfo().c_str(), playerName.c_str());

    if (normalizePlayerName(playerName))
        if (Guild* guild = GetPlayer()->GetGuild())
            guild->HandleSetLeader(this, playerName);
}
...
void WorldSession::HandleGuildMOTDOpcode(WorldPacket& recvPacket)
{
    ...
    if (!ValidateInputString(motd, 128))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());

        sLog->outError("Player %s tried to set Guild MOTD but string validation failed.", guidStr.c_str());
        return;
    }
...
void WorldSession::HandleGuildSetPublicNoteOpcode(WorldPacket& recvPacket)
{
    ...
    bool const playerValid = ValidateInputString(playerName, MAX_PLAYER_NAME);
    bool const noteValid   = ValidateInputString(note, 31);

    if (!playerValid || !noteValid)
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());

        sLog->outError("Player %s tried to set public note but string validation failed.", guidStr.c_str());
        return;
    }
...
void WorldSession::HandleGuildSetOfficerNoteOpcode(WorldPacket& recvPacket)
{
    ...
    if (!playerValid || !noteValid)
    {
        ...
        sLog->outError("Player %s tried to set a officer note with invalid utf8 sequence - blocked", guidStr.c_str());
        return;
    }
...
void WorldSession::HandleGuildRankOpcode(WorldPacket& recvPacket)
{
    ...
    bool const rankNameValid = ValidateInputString(rankName, 15);
    if (!rankNameValid)
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());

        sLog->outError("Player %s tried to set guild rank but string validation failed.", guidStr.c_str());
        return;
    }
...
void WorldSession::HandleGuildAddRankOpcode(WorldPacket& recvPacket)
{
    ...
    if (!ValidateInputString(rankName, 15))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());
...
void WorldSession::HandleGuildChangeInfoTextOpcode(WorldPacket& recvPacket)
{
    ...
    if (!ValidateInputString(info, 500))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());
        sLog->outError("Player %s tried to set guild rank but string validation failed.", guidStr.c_str());
        return;
...
void WorldSession::HandleSaveGuildEmblemOpcode(WorldPacket& recvPacket)
{
    ...
    //! Should we validate if emblem info is correct and exists?

    sLog->outDebug(LOG_FILTER_GUILD, "MSG_SAVE_GUILD_EMBLEM [%s]: Guid: [" UI64FMTD "] Style: %d, Color: %d, BorderStyle: %d, BorderColor: %d, BackgroundColor: %d", GetPlayerInfo().c_str(), vendorGuid, emblemInfo.GetStyle(), emblemInfo.GetColor(), emblemInfo.GetBorderStyle(), emblemInfo.GetBorderColor(), emblemInfo.GetBackgroundColor());
...
void WorldSession::HandleGuildBankerActivate(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "CMSG_GUILD_BANKER_ACTIVATE [%s]: Go: [" UI64FMTD "] AllSlots: %u", GetPlayerInfo().c_str(), guid, sendAllSlots);
    ...
    if (GetPlayer()->CaninteractWithGuildBank(guid))
...
void WorldSession::HandleGuildBankQueryTab(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "CMSG_GUILD_BANK_QUERY_TAB [%s]: Go: [" UI64FMTD "], TabId: %u, ShowTabs: %u", GetPlayerInfo().c_str(), guid, tabId, full);
...
void WorldSession::HandleGuildBankWithdrawMoney(WorldPacket& recvData)
{
    ...
    if (money && GetPlayer()->CaninteractWithGuildBank(guid))
        if (Guild* guild = GetPlayer()->GetGuild())
...
void WorldSession::HandleGuildBankUpdateTab(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "CMSG_GUILD_BANK_UPDATE_TAB [%s]: Go: [" UI64FMTD "], TabId: %u, Name: %s, Icon: %s", GetPlayerInfo().c_str(), guid, tabId, name.c_str(), icon.c_str());

    bool const IsNameValid = ValidateInputString(name, 16);
    bool const IsIconValid = ValidateInputString(icon, 128);

    if (!IsNameValid || !IsIconValid)
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());

        sLog->outError("Player %s tried to update guild bank but string validation failed.", guidStr.c_str());
        return;
    }
...
void WorldSession::HandleSetGuildBankTabText(WorldPacket& recvData)
{
    ...
    if (!ValidateInputString(text, 500))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());

        sLog->outError("Player %s tried to set guild bank text but string validation failed.", guidStr.c_str());
        return;
    }
