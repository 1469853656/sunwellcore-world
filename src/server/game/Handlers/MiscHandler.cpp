...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recv_data)
{
    ...
    else
    {
        if (unit)
        {
            ...
            unit->AI()->sGossipSelectSenderAction(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), _player->PlayerTalkClass->GetGossipOptionAction(gossipListId));
...
void WorldSession::HandleWhoOpcode(WorldPacket& recv_data)
{
    ...
    TeamId teamId = _player->GetTeam(CF_DENY);
    // uint32 security = GetSecurity();
    bool allowTwoSideWhoList = sWorld->IsCrossfactionEnabled(Crossfaction::WhoCommand);
    ...
    {
        // player can see member of other team only if CONFIG_CROSSFACTION_WHO
        if (!allowTwoSideWhoList && (*itr).teamId != teamId)
            continue;
        ...
        {
            isFakeWhoResult = sCustomMgr->GetBoolConfig(CONFIG_POPULATION_BOOSTER_FAKE_WHO_LEVEL) && (lvl >= sCustomMgr->GetIntConfig(CONFIG_POPULATION_BOOSTER_MIN_LEVEL));
            continue;
        }
        ...
        {
            isFakeWhoResult = sCustomMgr->GetBoolConfig(CONFIG_POPULATION_BOOSTER_FAKE_WHO_CLASS);
            continue;
        }
        ...
        if (!(racemask & (1 << race)))
        {
            isFakeWhoResult = sCustomMgr->GetBoolConfig(CONFIG_POPULATION_BOOSTER_FAKE_WHO_RACE);
            continue;
        }
        ...
        {
            isFakeWhoResult = sCustomMgr->GetBoolConfig(CONFIG_POPULATION_BOOSTER_FAKE_WHO_GUILD);
            continue;
        }
    ...
    /***                POPULATION BOOSTER                 ***/
    /*********************************************************/
    matchcount = sCustomMgr->BoostPopulationByValue(matchcount, isFakeWhoResult);

    data.put(0, displaycount);                                                                      // insert right count, count displayed
    data.put(4, sCustomMgr->BoostPopulationByValue(sWorld->GetPlayerCount(), true) /*matchcount*/); // insert right count, count of matches
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recv_data*/)
{
    ...
    LogoutRequest(time(nullptr), instantLogout ? 3 : 20);
}
...
void WorldSession::HandleAddFriendOpcode(WorldPacket& recv_data)
{
    ...
    if (!utf8::is_valid(friendNote.begin(), friendNote.end()))
    {
        std::string guidStr = std::to_string(GetPlayer()->GetGUID());
        sLog->outError("Player %s tried to add to friends player with an invalid UTF8 sequence note - blocked", guidStr.c_str());
        return;
    }
    ...
    if (!AccountMgr::IsPlayerAccount(GetSecurity()) || sWorld->getBoolConfig(CONFIG_ALLOW_GM_FRIEND) || AccountMgr::IsPlayerAccount(AccountMgr::GetSecurity(friendAccountId, realmID)))
    {
        if (friendGuid)
        {
            ...
            else if (!sWorld->IsCrossfactionEnabled(Crossfaction::FriendList) && GetPlayer()->GetTeam(CF_DENY) != teamId && AccountMgr::IsPlayerAccount(GetSecurity()))
...
void WorldSession::HandleLoadActionsSwitchSpecCallback(PreparedQueryResult result) const
{
    if (!GetPlayer())
        return;

    if (result)
        GetPlayer()->_LoadActions(std::move(result));
...
void WorldSession::HandleSetContactNotesOpcode(WorldPacket& recv_data)
{
    ...
    if (!utf8::is_valid(note.begin(), note.end()))
    {
        auto info = GetPlayerInfo();
        sLog->outError("WorldSession::HandleSetContactNotesOpcode: invalid UTF8 sequence - blocking. %s", info.c_str());
        return;
    }
...
void WorldSession::HandleBugOpcode(WorldPacket& recv_data)
{
    ...
    bool const _validContent = utf8::is_valid(content.begin(), content.end());
    bool const _validType    = utf8::is_valid(type.begin(), type.end());
    if (!_validContent || !_validType)
    {
        auto info = GetPlayerInfo();
        sLog->outError("WorldSession::HandleBugOpcode: invalid UTF8 sequence - blocking. %s", info.c_str());
        return;
    }
...
void WorldSession::HandleMoveUnRootAck(WorldPacket& recv_data)
{
    ...
    (LOG_FILTER_PACKETIO, "WORLD: CMSG_FORCE_MOVE_UNROOT_ACK");
...
void WorldSession::HandleWhoisOpcode(WorldPacket& recv_data)
{
    ...
    auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_WHOIS);
...
void WorldSession::HandleHearthAndResurrect(WorldPacket& /*recv_data*/)
{
    ...
    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetZoneId()))
    {
        ...
        {
            ChatHandler(_player->GetSession()).SendSysMessage("You can't leave Wintergrasp right now (currently in combat)");
            return;
        }
        else if (_player->WasRecentlyInCombat(5000))
        {
            ChatHandler(_player->GetSession()).SendSysMessage("You can't leave Wintergrasp right now (recently in Combat)");
            return;
        }
...
{
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Received opcode CMSG_COMMENTATOR_ENABLE");
