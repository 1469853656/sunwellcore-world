...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    // can't group with
    if (!invitingPlayer->IsGameMaster() && !sWorld->IsCrossfactionEnabled(Crossfaction::Group) && invitingPlayer->GetTeam() != invitedPlayer->GetTeam())
    {
    ...
    bool const hasHardcoreEnabled = invitingPlayer->HasHardcoreEnabled();
    bool const invitedHasHardcoreEnabled = invitedPlayer->HasHardcoreEnabled();
    if (hasHardcoreEnabled && !invitedHasHardcoreEnabled)
    {
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantInviteNonHardcore))
    ...
    if (!hasHardcoreEnabled && invitedHasHardcoreEnabled)
    {
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantInviteHardcore))
            ChatHandler(this).SendSysMessage(message->c_str());
