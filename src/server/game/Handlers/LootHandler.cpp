...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPacket& recvData)
{
    ...
    {
        ItemRef pItem = player->GetItemByGuid(lguid);
...
void WorldSession::HandleLootMoneyOpcode(WorldPacket& /*recvData*/)
{
    ...
    switch (GUID_HIPART(guid))
    {
        ...
        case HIGHGUID_ITEM:
        {
            if (ItemRef item = player->GetItemByGuid(guid))
            {
                loot       = &item->loot;
                shareMoney = false;
            }
...
void WorldSession::HandleLootOpcode(WorldPacket& recvData)
{
    ...
    _player->InvalidateHardcorePath();
    if (sWorld->getBoolConfig(CONFIG_LOOT_CHECK_GOBJECT_LOCK) && IS_GAMEOBJECT_GUID(guid))
    {
        GameObject* go = GetPlayer()->GetMap()->GetGameObject(guid);
        if (go && go->GetGOInfo()->GetLockId())
        {
            sLog->outCheat("%s is trying to loot locked gobject " UI64FMTD " %u", GetPlayer()->GetName().c_str(), guid, go->GetGOInfo()->GetLockId());
            return;
        }
    }
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
    ...
    if (item.is_looted)
    {
        sLog->outCheat("MasterLootItem: Player %s is trying to get already looted item Id: %u", GetPlayer()->GetName().c_str(), item.itemid);
        return;
    }

    // Currency tokens are auto-stored in Loot::FillNotNormalLootFor
    // Players shouldn't be able to get it via Master Loot
    if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(item.itemid))
    {
        if (proto->IsCurrencyToken())
        {
            sLog->outCheat("MasterLootItem: Player %s is trying to loot currency token Id: %u", GetPlayer()->GetName().c_str(), item.itemid);
            return;
        }
    }
    ...
    ItemRef newitem = target->StoreNewItem(dest, item.itemid, true, item.randomPropertyId, looters);
    ...
    loot->SetItemLooted(&item, target);
