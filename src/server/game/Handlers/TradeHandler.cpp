...
void WorldSession::SendUpdateTrade(bool trader_data /*= true*/)
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i)
    {
        ...
        if (ItemRef item = view_trade->GetItem(TradeSlots(i)))
        {
...
void WorldSession::moveItems(ItemRef myItems[], ItemRef hisItems[])
{
...
static void setAcceptTradeMode(TradeData* myTrade, TradeData* hisTrade, ItemRef* myItems, ItemRef* hisItems)
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        if (ItemRef item = myTrade->GetItem(TradeSlots(i)))
        {
        ...
        if (ItemRef item = hisTrade->GetItem(TradeSlots(i)))
        {
...
static void clearAcceptTradeMode(ItemRef* myItems, ItemRef* hisItems)
{
    // clear 'in-trade' flag
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    ItemRef myItems[TRADE_SLOT_TRADED_COUNT]  = { nullptr, nullptr, nullptr, nullptr, nullptr, nullptr };
    ItemRef hisItems[TRADE_SLOT_TRADED_COUNT] = { nullptr, nullptr, nullptr, nullptr, nullptr, nullptr };
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        if (ItemRef item = my_trade->GetItem(TradeSlots(i)))
        {
        ...
        if (ItemRef item = his_trade->GetItem(TradeSlots(i)))
        {
            if (!item->CanBeTraded(false, true))
            {
                SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
    ...
    if (his_trade->IsAccepted())
    {
        ...
        if (uint32 my_spell_id = my_trade->GetSpell())
        {
            ...
            ItemRef castItem            = my_trade->GetSpellCastItem();
        ...
        if (uint32 his_spell_id = his_trade->GetSpell())
        {
            ...
            ItemRef castItem            = his_trade->GetSpellCastItem();
        ...
        trader->InvalidateHardcorePath();
        // execute trade: 2. store
        moveItems(myItems, hisItems);

        if (my_trade->GetMoney() >= 10 * GOLD)
        {
            CharacterDatabase.PExecute("INSERT INTO log_money (sender_acc, sender_guid, sender_name, sender_ip, receiver_acc, receiver_name, money, topic, date) VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, \"<TRADE>\", NOW())", GetAccountId(), _player->GetGUIDLow(), _player->GetName().c_str(), GetRemoteAddress().c_str(), trader->GetSession()->GetAccountId(), trader->GetName().c_str(), my_trade->GetMoney());
        }
        if (his_trade->GetMoney() >= 10 * GOLD)
        {
            CharacterDatabase.PExecute("INSERT INTO log_money (sender_acc, sender_guid, sender_name, sender_ip, receiver_acc, receiver_name, money, topic, date) VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, \"<TRADE>\", NOW())", trader->GetSession()->GetAccountId(), trader->GetGUIDLow(), trader->GetName().c_str(), trader->GetSession()->GetRemoteAddress().c_str(), GetAccountId(), _player->GetName().c_str(), his_trade->GetMoney());
        }
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    bool const hasHardcoreEnabled = _player->HasHardcoreEnabled();
    bool const otherHasHardcoreEnabled = pOther->HasHardcoreEnabled();
    if (hasHardcoreEnabled && !otherHasHardcoreEnabled)
    {
        SendTradeStatus(TRADE_STATUS_NO_TARGET);
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantTradeWithNormal))
    ...
    {
        SendTradeStatus(TRADE_STATUS_NO_TARGET);
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantTradeWithHardcore))
            ChatHandler(this).SendSysMessage(message->c_str());
    ...
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::Trade) && pOther->GetTeam(CF_DENY) != _player->GetTeam(CF_DENY))
    {
        if (_player->GetArenaType() != ARENA_TYPE_SOLO_QUEUE || _player->GetBgTeam() != pOther->GetBgTeam())
        {
            SendTradeStatus(TRADE_STATUS_WRONG_FACTION);
            return;
        }
...
void WorldSession::HandleSetTradeGoldOpcode(WorldPacket& recvPacket)
{
    ...
    if (gold > 0 && (_player->IsPathOfWar() || my_trade->GetTrader()->IsPathOfWar()))
    {
        SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
        return;
    }
    ...
    {
        // Trades between factions are enabled on soloqueue, don't allow them to trade gold
        if (gold && pOther->GetTeam(CF_DENY) != _player->GetTeam(CF_DENY))
        {
            SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
            return;
        }
...
void WorldSession::HandleSetTradeItemOpcode(WorldPacket& recvPacket)
{
    ...
    ItemRef item = _player->GetItemByPos(bag, slot);
    ...
    Player* pOther = my_trade->GetTrader();
    // don't allow to put item in nontraded slot in crossfaction soloqueue arena
    if (pOther)
    {
        if (tradeSlot == TRADE_SLOT_NONTRADED && _player->GetArenaType() == ARENA_TYPE_SOLO_QUEUE && _player->GetTeam(CF_DENY) != pOther->GetTeam(CF_DENY))
        {
            SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
            return;
        }
    ...
    if ((_player->HasHardcoreEnabled() || (pOther && pOther->HasHardcoreEnabled())) && item->IsBoundAccountWide())
    {
        SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
        return;
    }
    ...
    if ((_player->IsPathOfWar() || my_trade->GetTrader()->IsPathOfWar()) && !item->IsConjuredConsumable())
    {
        SendTradeStatus(TRADE_STATUS_ONLY_CONJURED);
        return;
    }
