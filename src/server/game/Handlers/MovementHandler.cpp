...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    if (_player->InBattleground())
    {
        ...
        if (!mEntry->IsBattlegroundOrArena())
            _player->SetBattlegroundId(0, BATTLEGROUND_TYPE_NONE, PLAYER_MAX_BATTLEGROUND_QUEUES, false, false, TEAM_NEUTRAL, ARENA_TYPE_NONE);
        // everything ok
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    if (opcode == MSG_MOVE_SET_WALK_MODE && mover->IsCharmed())
        return;
    ...
    if (movementInfo.flags & MOVEMENTFLAG_ONTRANSPORT)
    {
        ...
        if (movementInfo.transport.pos.GetPositionX() > 75.0f || movementInfo.transport.pos.GetPositionY() > 75.0f || movementInfo.transport.pos.GetPositionZ() > 75.0f || movementInfo.transport.pos.GetPositionX() < -75.0f || movementInfo.transport.pos.GetPositionY() < -75.0f || movementInfo.transport.pos.GetPositionZ() < -75.0f)
        {
            recvData.rfinish(); // prevent warnings spam
            return;
        }
    ...
    if (plrMover && plrMover->GetSession()->GetSecurity() < SEC_MODERATOR)
...
void WorldSession::HandleSummonResponseOpcode(WorldPacket& recvData)
{
    ...
    if (agree && _player->IsSummonAsSpectator())
    {
        ...
        if (Player* summoner = ObjectAccessor::FindPlayer(summoner_guid))
            ...
            chc.SendSysMessage("Requested player not found.");
