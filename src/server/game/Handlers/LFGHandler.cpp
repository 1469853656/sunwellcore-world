...
void WorldSession::HandleLfgJoinOpcode(WorldPacket& recvData)
{
    ...
    {
        if (std::string const* message = sCustomMgr->GetCustomString(CustomString::HardcoreModeCantUseLFG))
...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    for (lfg::LfgDungeonSet::const_iterator it = randomDungeons.begin(); it != randomDungeons.end(); ++it)
    {
        ...
        {
            // Call To Arms
            uint32 callToArmsRoles = sLFGMgr->GetCallToArmsRole();
            uint32 playerPossibleRoles = _player->GetPossibleLFGRoles();
            bool ctaEligible = false; 
            bool callToArmsEnabled = sWorld->getBoolConfig(CONFIG_CALL_TO_ARMS_ENABLE) && (*it & 0x00FFFFFF) == lfg::LFG_RANDOM_LICH_KING_HEROIC && !_player->GetGroup();
            if (callToArmsEnabled)
                ctaEligible = (callToArmsRoles & playerPossibleRoles) != 0;

            data << uint8(done);
            data << uint32(quest->GetRewOrReqMoney());
            data << uint32(_player->getLevel() == DEFAULT_MAX_LEVEL ? 0 : sCustomMgr->CalculateBonusXPWithRateUpdate(_player, quest->XPValue(_player)));
            data << uint32(0);
            data << uint32(0);
            data << uint8(quest->GetRewItemsCount() + (ctaEligible ? 1 : 0));
            ...
            if (callToArmsEnabled)
            {
                auto SendMessage = [this, &callToArmsRoles, &playerPossibleRoles](bool enabled) 
                {
                    if (!_player->CanDisplayCallToArmsMessages())
                        return;

                    std::string message;
                    if (!enabled)
                        message = "You are no longer eligible to Call to Arms!";
                    else
                    {
                        std::ostringstream ss;
                        ss << "Call to Arms! Complete Random Heroic Dungeon as \n";
                        if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_TANK)
                        {
                            ss << "Tank |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:0:19:22:41|t";
                            if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_HEALER)
                            {
                                if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_DAMAGE)
                                    ss << ", Healer |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t or DPS |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t\n";
                                else
                                    ss << " or Healer |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t\n";
                            }
                            else if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_DAMAGE)
                                ss << " or DPS |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t\n";
                            else
                                ss << "\n";
                        }
                        else if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_HEALER)
                        {
                            ss << "Healer |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:1:20|t";
                            if (callToArmsRoles & playerPossibleRoles & lfg::PLAYER_ROLE_DAMAGE)
                                ss << " or DPS |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t\n";
                            else
                                ss << "\n";
                            ...
                            ss << "DPS |TInterface/LFGFrame/UI-LFG-ICON-PORTRAITROLES.blp:16:16:0:0:64:64:20:39:22:41|t\n";

                        ss << " and receive Satchel of Exotic Mysteries |TInterface/icons/INV_Misc_Bag_26_Spellfire:16|t!";
                        message = ss.str();
                    }

                    WorldPacket data;
                    ChatHandler::BuildChatPacket(data, CHAT_MSG_RAID_BOSS_EMOTE, LANG_UNIVERSAL, _player, _player, message);
                    SendPacket(&data);
                };

                std::pair<bool, time_t>& callToArmsStatus = _player->GetCallToArmsMessageStatus();
                if (ctaEligible)
                {
                    ItemTemplate const* item = sObjectMgr->GetItemTemplate(lfg::LFG_CTA_REWARD_ITEM_ID);
                    data << uint32(item ? item->ItemId : 0);
                    data << uint32(item ? item->DisplayInfoID : 0);
                    data << uint32(1);

                    time_t now = sWorld->GetGameTime();
                    if (!callToArmsStatus.first)
                    {
                        callToArmsStatus = std::make_pair(true, now + sWorld->getIntConfig(CONFIG_CALL_TO_ARMS_MESSAGE_COOLDOWN));
                        SendMessage(true);
                    }
                    else if (callToArmsStatus.second < now)
                    {
                        callToArmsStatus.second = now + sWorld->getIntConfig(CONFIG_CALL_TO_ARMS_MESSAGE_COOLDOWN);
                        SendMessage(true);
                    }
                }
                else if (callToArmsStatus.first)
                {
                    callToArmsStatus = std::make_pair(false, time_t(0));
                    SendMessage(false);
                }
            }
        }
...
void WorldSession::SendLfgPlayerReward(lfg::LfgPlayerRewardData const& rewardData)
{
    ...
    data << uint32(_player->getLevel() == DEFAULT_MAX_LEVEL ? 0 : sCustomMgr->CalculateBonusXPWithRateUpdate(_player, rewardData.quest->XPValue(_player)));
    data << uint32(0);
    data << uint32(0);
    data << uint8(itemNum);
    if (itemNum)
