...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
    ...
    // pussywizard
    uint32 relationTime = (sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_RELATIVE_TIMESTAMP) + sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_HOUR) * HOUR + sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_MINUTE) * MINUTE); // set point in time (default 29.12.2005) + X hours + X minutes
...
void WorldSession::HandleCalendarGetEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_GET_EVENT. Player [" UI64FMTD "] Event [" UI64FMTD "]", _player->GetGUID(), eventId);
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_UPDATE_EVENT [" UI64FMTD "] EventId [" UI64FMTD "], InviteId [" UI64FMTD "] Title %s, Description %s, type %u "
                                         "Repeatable %u, MaxInvites %u, Dungeon ID %d, Time %u "
                                         "Time2 %u, Flags %u",
        guid, eventId, inviteId, title.c_str(),
...
void WorldSession::HandleCalendarCopyEvent(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_COPY_EVENT [" UI64FMTD "], EventId [" UI64FMTD "] inviteId [" UI64FMTD "] Time: %u", guid, eventId, inviteId, eventTime);
...
void WorldSession::HandleCalendarEventInvite(WorldPacket& recvData)
{
    ...
    if (_player->GetTeam() != inviteeTeamId && !sWorld->IsCrossfactionEnabled(Crossfaction::Calendar))
    {
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_EVENT_RSVP [" UI64FMTD "] EventId [" UI64FMTD "], InviteId [" UI64FMTD "], status %u", guid, eventId,
        inviteId, status);
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_EVENT_REMOVE_INVITE [" UI64FMTD "] EventId [" UI64FMTD "], ownerInviteId [" UI64FMTD "], Invitee ([" UI64FMTD "] id: [" UI64FMTD "])",
...
void WorldSession::HandleCalendarEventStatus(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_EVENT_STATUS [" UI64FMTD "] EventId [" UI64FMTD "] ownerInviteId [" UI64FMTD "], Invitee ([" UI64FMTD "] id: [" UI64FMTD "], status %u", guid, eventId, ownerInviteId, invitee, inviteId, status);
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_EVENT_MODERATOR_STATUS [" UI64FMTD "] EventId [" UI64FMTD "] ownerInviteId [" UI64FMTD "], Invitee ([" UI64FMTD "] id: [" UI64FMTD "], rank %u", guid, eventId, ownerInviteId, invitee, inviteId, rank);
...
void WorldSession::HandleCalendarComplain(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_COMPLAIN [" UI64FMTD "] EventId [" UI64FMTD "] guid [" UI64FMTD "]", guid, eventId, complainGUID);
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket& /*recvData*/)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_CALENDAR_GET_NUM_PENDING: [" UI64FMTD "] Pending: %u", guid, pending);
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& recvData)
{
    ...
    // update in db
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_INSTANCE_EXTENDED);
