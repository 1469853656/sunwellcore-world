...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (isLogingOut())
    {
        sLog->outError("Player %s (guid: %u, account: %u) is trying to use item while logging out.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow(), GetAccountId());
        pUser->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr);
        return;
    }
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (isLogingOut())
    {
        sLog->outError("Player %s (guid: %u, account: %u) is trying to open item while logging out.", GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow(), GetAccountId());
        pUser->SendEquipError(EQUIP_ERR_YOU_ARE_DEAD, nullptr, nullptr);
        return;
    }
    ...
    {
        auto const stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_GIFT_BY_ITEM);
        stmt->setUInt32(0, item->GetGUIDLow());
        GetQueryProcessor().AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback([this, bagIndex, slot, itemGuidLow = item->GetGUIDLow()](PreparedQueryResult characterGiftResult)
        {
            if (!characterGiftResult)
                return;

            ItemRef wrappedItem = GetPlayer()->GetItemByPos(bagIndex, slot);
            if (!wrappedItem)
                return;

            if (wrappedItem->GetGUIDLow() != itemGuidLow || !wrappedItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED)) // during getting result, gift was swapped with another item
                return;

            Field const* fields = characterGiftResult->Fetch();
            uint32 const entry = fields[0].GetUInt32();
            uint32 const flags = fields[1].GetUInt32();

            wrappedItem->SetUInt64Value(ITEM_FIELD_GIFTCREATOR, 0);
            wrappedItem->SetEntry(entry);
            wrappedItem->SetUInt32Value(ITEM_FIELD_FLAGS, flags);
            wrappedItem->SetState(ITEM_CHANGED, GetPlayer());

            auto trans = CharacterDatabase.BeginTransaction();
            GetPlayer()->SaveInventoryAndGoldToDB(trans);

            auto const delGiftStmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GIFT);
            delGiftStmt->setUInt32(0, itemGuidLow);
            trans->Append(delGiftStmt);
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    {
        if (int32 const maxSpellQueueDelay = player->GetSpellQueueWindow())
        {
            Spell const* channelSpell = player->GetCurrentSpell(CURRENT_CHANNELED_SPELL);
            if (!channelSpell && !(spellInfo->AttributesEx4 & SPELL_ATTR4_CAN_CAST_WHILE_CASTING) && !spellInfo->HasEffect(SPELL_EFFECT_PROSPECTING))
            {
                int32 const gcd = player->GetGlobalCooldownMgr().GetGlobalCooldown(spellInfo);

                int32 const cd = player->GetSpellCooldownDelay(spellInfo->Id);

                Spell const* genericSpell = player->GetCurrentSpell(CURRENT_GENERIC_SPELL);
                int32 const castTime = genericSpell ? genericSpell->GetTimer() : 0;

                if ((gcd > 0 || cd > 0 || castTime > 0) && gcd <= maxSpellQueueDelay && cd <= std::max(castTime, gcd) && castTime <= maxSpellQueueDelay)
                {
                    spell->m_targets = targets;
                    player->SetSpellQueue(spell);
                    return;
                }
            }
        }
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    {
        if (_player)
            sLog->outError("WORLD HandlePetCancelAuraOpcode: Unknown PET spell id %u, player(GUID: " UI64FMTD " Name: %s)", spellId, _player->GetGUID(), _player->GetName().c_str());
        else
...
void WorldSession::HandleSelfResOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (_player->GetUInt32Value(PLAYER_SELF_RES_SPELL))
    {
        ...
        if (spellInfo)
        {
            _player->CastSpell(_player, spellInfo, true, nullptr);
