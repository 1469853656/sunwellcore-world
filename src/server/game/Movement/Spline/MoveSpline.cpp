...
struct FallInitializer
{
    FallInitializer(float _start_elevation)
        : start_elevation(_start_elevation)
    {
    }
...
struct CommonInitializer
{
    CommonInitializer(float _velocity)
        : velocityInv(1000.f / _velocity)
        , time(minimal_duration)
    {
    }
    float velocityInv;
    int32 time;
...
#define CHECK(exp)                                                                                                                                                                                                                                  \
    if (!(exp))                                                                                                                                                                                                                                     \
    {                                                                                                                                                                                                                                               \
        sLog->outDebug(LOG_FILTER_UNITS, "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetDBTableGUIDLow(), unit->GetEntry()); \
        return false;                                                                                                                                                                                                                               \
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.1f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    ...
    {
        constexpr uint32 MAX_OFFSET = (1 << 11) / 2;
