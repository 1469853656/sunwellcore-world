...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z, bool generatePath, bool forceDestination, MovementSlot slot)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        Mutate(new PointMovementGenerator<Player>(id, x, y, z, 0.0f, nullptr, generatePath, forceDestination), slot);
    }
    else
    {
        ; //sLog->outStaticDebug("Creature (Entry: %u GUID: %u) targeted point (ID: %u X: %f Y: %f Z: %f)",
        //    _owner->GetEntry(), _owner->GetGUIDLow(), id, x, y, z);
        Mutate(new PointMovementGenerator<Creature>(id, x, y, z, 0.0f, nullptr, generatePath, forceDestination), slot);
    }
}
...
void MotionMaster::MoveAlongSplineChain(uint32 pointId, uint16 dbChainId, bool walk)
{
    ...
    if (!owner)
    {
        sLog->outError("MotionMaster::MoveAlongSplineChain: non-creature " UI64FMTD " tried to walk along db spline chain. Ignoring.", _owner->GetGUID());
        return;
    }
...
bool MotionMaster::IsCharging() const
{
    return top()->IsCharging();
}
