...
public:
    ...
    explicit SplineChainMovementGenerator(SplineChainResumeInfo const& info)
        : _id(info.PointID)
        , _chain(*info.Chain)
        , _chainSize(info.Chain->size())
        , _walk(info.IsWalkMode)
        , finished(info.SplineIndex >= info.Chain->size())
    ...
    MovementGeneratorType GetMovementGeneratorType() override { return SPLINE_CHAIN_MOTION_TYPE; }
