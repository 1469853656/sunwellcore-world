...
#include "CellImpl.h"
 ...
 ////////////////// PathGenerator //////////////////
PathGenerator::PathGenerator( PathGeneratorContext const* ctx, bool destroyContext )
    :m_context( ctx )
    ...
    , _navMeshQuery( nullptr )
{
    ...
    {
        MMAP::IMMapManager* mmap = MMAP::MMapFactory::createOrGetMMapManager();

        std::shared_lock lock( mmap->GetManagerLock() );
        _navMesh = mmap->GetNavMesh( map->GetId() );

        //! #TODO: profile me;
        _navMeshQuery = dtAllocNavMeshQuery();
        _navMeshQuery->init( _navMesh, 1024 );
    }
...
bool PathGenerator::CalculatePath( float destX, float destY, float destZ, bool forceDest )
{
    ...
    std::shared_mutex& mmapLock = MMAP::MMapFactory::createOrGetMMapManager()->GetMMapLock(m_context->GetMap()->GetId());
    std::shared_lock lock(mmapLock);
    ...
    if (!_navMesh || !_navMeshQuery || m_context->IsIgnoringPathFinding() || m_context->GetSourceSize() >= SIZE_OF_GRIDS / 2.0f || (end - start).squaredLength() >= (SIZE_OF_GRIDS * SIZE_OF_GRIDS / 4.0f) || !HaveTile(start) || !HaveTile(end))
    {
        BuildShortcut();
    ...
    BuildPolyPath( start, end, std::move(lock) );
    return true;
}
...
{
    // Ignore normalization on transports
    if (m_context->IsOnTransport())
        return;

    std::ranges::for_each(_pathPoints, [this](G3D::Vector3& point)
    {
        GridCoord const gridCoord = Trinity::ComputeGridCoord(point.x, point.y);
        if (!m_context->GetMap()->getNGrid(gridCoord.x_coord, gridCoord.y_coord))
            return;

        WorldObject::UpdateAllowedPositionZ(m_context, point.x, point.y, point.z);
    });
}
...
{
public:
    MutexReleaser(MUTEX_TYPE& mutex)
        : _mutex(mutex)
    {
    }
    ~MutexReleaser()
...
void PathGenerator::BuildPolyPath( G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, std::shared_lock<std::shared_mutex>&& mmapLock )
{
    bool endInWaterFar = false;
    bool cutToFirstHigher = false;

    Map const* baseMap = m_context->GetBaseMap();
    ...
    bool sourceCanWalk = m_context->CanSourceWalk();
    ...
    if ( farFromStartPoly )
    {
        ...
        if ( sourceCanSwim )
        {
            if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( startPos.x, startPos.y, startPos.z, MAP_ALL_LIQUIDS, nullptr) )
            {
                ...
                if ( farFromEndPoly )
                {
                    if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( endPos.x, endPos.y, endPos.z, MAP_ALL_LIQUIDS, nullptr) )
                    {
                        BuildShortcut();
                        _type = PATHFIND_NOPATH;
                        return;
            ...
            else if ( LIQUID_MAP_NO_WATER == baseMap->getLiquidStatus( endPos.x, endPos.y, endPos.z, MAP_ALL_LIQUIDS, nullptr) )
            {
                if ( farFromEndPoly )
                {
                    BuildShortcut();
    ...
    if ( _type == PATHFIND_NORMAL && cutToFirstHigher ) // starting in water, far from bottom, target is on the ground (above starting Z) -> update beginning points that are lower than starting Z
    {
        ...
        for ( ; i < size; ++i )
        {
            if ( _pathPoints[ i ].z >= startPos.z + 0.1f )
                break;
        }

        if ( i && i != size && LIQUID_MAP_NO_WATER != baseMap->getLiquidStatus( _pathPoints[ i - 1 ].x, _pathPoints[ i - 1 ].y, _pathPoints[ i - 1 ].z, MAP_ALL_LIQUIDS, nullptr) )
        {
            for ( uint32 j = 0; j < i; ++j )
                _pathPoints[ j ].z = startPos.z;
        }
    }
    ...
    if ( !_forceDestination )
    {
        if ( uint32 lastIdx = _pathPoints.size() )
        {
            ...
            {
                //! relocated to MoveSplineInit::MoveByPath( VMAP operations are not cross-thread safe, due to dynamic tree rebalance on each getHeight call )
                //UpdateAllowedPositionZ( _pathPoints[ lastIdx ] );
            }
        }
    }
        ...
        if ( i < size )
        {
            ...
            if ( ( _pathPoints[ i ] - _pathPoints[ 0 ] ).squaredLength() > 15.0f*15.0f )
                ...
                for ( uint32 j = 1; j < i; ++j )
                {
                    ...
                    float oSize = m_context->GetSourceSize();
...
{
    uint32_t pointPathLimit = std::min<uint32>( uint32( m_context->GetPathLengthLimit() / SMOOTH_PATH_STEP_SIZE ), MAX_POINT_PATH_LENGTH );

    float pathPoints[ MAX_POINT_PATH_LENGTH*VERTEX_SIZE ];
    uint32 pointCount = 0;
    dtStatus dtResult = dtResult = FindSmoothPath(
        startPoint,         // start position
        endPoint,           // end position
        ...
        ( int* )&pointCount,
        pointPathLimit );    // maximum number of points
...
void PathGenerator::UpdateFilter()
{
    ...
    if ( m_context->IsSourceInWater() )
    {
        G3D::Vector3 currPos = m_context->GetStartPosition();

        uint16 includedFlags = _filter.getIncludeFlags();
        includedFlags |= GetNavTerrain( currPos.x, currPos.y, currPos.z );
...
NavTerrain PathGenerator::GetNavTerrain( float x, float y, float z )
{
    ...
    ZLiquidStatus liquidStatus = m_context->GetBaseMap()->getLiquidStatus( x, y, z, MAP_ALL_LIQUIDS, &data );
...
bool PathGenerator::HaveTile( const G3D::Vector3& p ) const
{
    ...
    return ( _navMesh->getTileAt( tx, ty ) != nullptr);
}
...
void PathGenerator::UpdateAllowedPositionZ( G3D::Vector3 & pos )
{
    auto map = m_context->GetMap();

    float minAllowedZ = pos.z;
    float maxAllowedZ = INVALID_HEIGHT;

    if ( !m_context->CanSourceFly() )
    {
        bool canSwim = m_context->CanSourceSwim();
        ...
        {
            minAllowedZ = map->GetHeight( m_context->GetPhaseMask(), pos.x, pos.y, pos.z, true );
        }
    }
    else
    {
    ...
    if ( maxAllowedZ > INVALID_HEIGHT && pos.z > maxAllowedZ )
    {
        pos.z = maxAllowedZ;
    }
    else if ( minAllowedZ > INVALID_HEIGHT && pos.z < minAllowedZ )
    {
        pos.z = minAllowedZ;
    }
}
...
G3D::Vector3 GetStartPositionForUnit( Unit const* unit )
{
    G3D::Vector3 start;
    if ( !unit->movespline->Finalized() && unit->movespline->Initialized() )
    {
        start = unit->movespline->ComputePosition();
    }
    else
    {
        unit->GetPosition( start.x, start.y, start.z );
    }
...
{
    m_pathLengthLimit = limit;
}

float PathGeneratorContext::GetPathLengthLimit() const
{
    return m_pathLengthLimit;
}

G3D::Vector3 PathGeneratorContext::GetStartPosition() const
{
    return GetStartPositionForUnit( m_source );
}

bool PathGeneratorContext::CanSourceSwim() const
{
...
bool PathGeneratorContext::CanSourceFly() const
{
    if ( m_source->HasUnitMovementFlag( MOVEMENTFLAG_CAN_FLY | MOVEMENTFLAG_FLYING ) )
        return true;

    if ( m_source->HasUnitMovementFlag( MOVEMENTFLAG_DISABLE_GRAVITY ) && !m_source->HasUnitMovementFlag( MOVEMENTFLAG_SWIMMING ) )
        return true;
...
bool PathGeneratorContext::CanSourceWaterWalk() const
{
    return m_source->HasAuraType( SPELL_AURA_WATER_WALK );
}

bool PathGeneratorContext::IsIgnoringPathFinding() const
{
    return m_source->HasUnitState( UNIT_STATE_IGNORE_PATHFINDING );
}

bool PathGeneratorContext::IsSourceInWater() const
{
    return m_source->IsInWater() || m_source->IsUnderWater();
}

float PathGeneratorContext::GetSourceSize() const
{
    return m_source->GetObjectSize();
}

uint32_t PathGeneratorContext::GetPhaseMask() const
{
    return m_source->GetPhaseMask();
}

Map const* PathGeneratorContext::GetBaseMap() const
{
    return m_source->GetBaseMap();
}

Map const* PathGeneratorContext::GetMap() const
{
    return m_source->GetMap();
}

AsyncPathGeneratorContext::AsyncPathGeneratorContext( Unit const* owner, G3D::Vector3 const& destPos, bool forceShortcut )
    ...
    , m_isForcingShortcut( forceShortcut )
{
    ...
    m_phaseMask = PathGeneratorContext::GetPhaseMask();
    m_sourceMap = PathGeneratorContext::GetBaseMap();
    m_canSourceSwim = PathGeneratorContext::CanSourceSwim();
    m_canSourceFly = PathGeneratorContext::CanSourceFly();
    m_canSourceWalk = PathGeneratorContext::CanSourceWalk();
    ...
    _sourceGUID = PathGeneratorContext::GetSourceGUID();
    _isOnTransport = PathGeneratorContext::IsOnTransport();
...
{
    return m_canSourceSwim;
}

bool AsyncPathGeneratorContext::CanSourceFly() const
{
    return m_canSourceFly;
}

bool AsyncPathGeneratorContext::CanSourceWalk() const
{
    return m_canSourceWalk;
}

bool AsyncPathGeneratorContext::CanSourceWaterWalk() const
{
    return m_canSourceWaterWalk;
}

bool AsyncPathGeneratorContext::IsIgnoringPathFinding() const
{
    return m_isIgnoringPathFinding;
}

bool AsyncPathGeneratorContext::IsSourceInWater() const
{
    return m_isSourceInWater;
}

float AsyncPathGeneratorContext::GetSourceSize() const
{
    return m_sourceSize;
}

G3D::Vector3 AsyncPathGeneratorContext::GetStartPosition() const
{
    return m_startPosition;
}

uint32_t AsyncPathGeneratorContext::GetPhaseMask() const
{
    return m_phaseMask;
}

Map const* AsyncPathGeneratorContext::GetBaseMap() const
{
    return m_sourceMap->GetParent();
}

Map const* AsyncPathGeneratorContext::GetMap() const
{
    return m_sourceMap;
}

G3D::Vector3 const& AsyncPathGeneratorContext::GetEndPosition() const
{
