...
bool FleeingMovementGenerator<T>::_getPoint(T* owner, float& x, float& y, float& z)
{
    ...
    bool const _isZulfarrak = _map ? _map->GetId() == 209 : false;
    Creature* cZulfarrak    = owner->ToCreature();
    if (_isZulfarrak && cZulfarrak)
    {
        Position zulfarrakPosition{ x, y, z };
        cZulfarrak->MovePositionToFirstCollision(zulfarrakPosition, 5.f, i_cur_angle);
        x = zulfarrakPosition.m_positionX;
        y = zulfarrakPosition.m_positionY;
        z = _map->GetWaterOrGroundLevel(cZulfarrak->GetPhaseMask(), x, y, z);

        i_to_distance_from_caster = 0.0f;
        i_nextCheckTime.Reset(2500);
        return true;
    }
