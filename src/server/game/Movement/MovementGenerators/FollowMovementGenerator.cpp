...
namespace Movement {
    ...
    static Optional<float> GetVelocity(Unit const* owner, Unit const* target, G3D::Vector3 const& dest, bool playerPet)
    {
        // some vehicles should update their follow speed
        bool const isVehicle = owner->IsVehicle();
        Optional<float> speed = {};
        if (!owner->IsInCombat() && !isVehicle && !owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
            (owner->IsPet() || owner->IsGuardian() || owner->GetGUID() == target->GetCritterGUID() || owner->GetCharmerOrOwnerGUID() == target->GetGUID()))
        {
            ...
            if (playerPet)
            {
                float distance = std::min<float>(owner->GetDistance2d(dest.x, dest.y), 10.0f) - (*speed / 2.f);
                if (distance > 0.f)
                {
    ...
    bool FollowMovementGenerator::DoUpdate( Unit* owner, uint32 diff )
    {
        ...
        if ( !m_forceNewPath && !owner->movespline->Finalized() )
        {
            ...
            if ( distance <= 0.25f )
                return ResetTimerAndReturn( ( owner->movespline->timeElapsed() > ( int32 )FOLLOW_UPDATE_TIMER ) ? FOLLOW_UPDATE_TIMER : FOLLOW_START_TIMER );
        }
    ...
    {
        if ( World::IsPathfindingEnabled( owner->FindMap() ))
        {
    ...
    void FollowMovementGenerator::MoveByPath( Unit* owner, Movement::Path & path )
    {
        ...
        bool petIsReturning = false;
        bool isPlayerPet = false;
        if (owner->IsGuardian())
        {
            if (CharmInfo* charmInfo = owner->GetCharmInfo())
                petIsReturning = charmInfo->IsReturning();

            isPlayerPet = i_target->IsPlayer();
        }

        if (!petIsReturning || isPlayerPet)
            if (Optional<float> velocity = GetVelocity(owner, i_target->ToUnit(), path.points.back(), isPlayerPet))
                spline.SetVelocity(*velocity);
