...
public:
    ...
    virtual uint64 GetSourceGUID() const { return m_source->GetGUID(); }
    virtual bool IsOnTransport() const { return m_source->GetTransport(); }
...
public:
    ...
    uint64 GetSourceGUID() const override { return _sourceGUID; }
    bool IsOnTransport() const override { return _isOnTransport; }

    void SetFallbackOrigin(G3D::Vector3 const& position);
    bool GetFallbackOrigin(G3D::Vector3& position) const override;
...
protected:
    ...
    uint64 _sourceGUID = 0;
    bool _isOnTransport = false;
...
private:
    ...
    void BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos, std::shared_lock<std::shared_mutex>&& lock);
