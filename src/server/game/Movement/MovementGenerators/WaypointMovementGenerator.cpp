...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path)
    {
        ...
        sLog->outErrorDb("WaypointMovementGenerator::LoadPath: creature %s (Entry: %u GUID: %u) doesn't have waypoint path id: %u", creature->GetName().c_str(), creature->GetEntry(), creature->GetDBTableGUIDLow(), path_id);
        return;
    }
...
void WaypointMovementGenerator<Creature>::DoInitialize(Creature* creature)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_ACTIVE_WAYPOINT_CREATURES_ENABLE))
...
bool FlightPathMovementGenerator::DoUpdate(Player* player, uint32 /*diff*/)
{
    ...
    if (pointId > i_currentNode)
    {
        ...
        do
        {
            if (i_currentNode >= i_path.size())
            {
                sLog->outMisc("TAXI NODE WAS GREATER THAN PATH SIZE, GUID: %u, POINTID: %u, NODESIZE: " SIZEFMTD ", CURRENT: %u", player->GetGUIDLow(), pointId, i_path.size(), i_currentNode);
            ...
            {
                sLog->outMisc("Player on different map, curmap: %u, pointmap: %u, nodesize: " SIZEFMTD ", currentnode: %u", player->GetMapId(), i_path[i_currentNode]->mapid, i_path.size(), i_currentNode);
                player->CleanupAfterTaxiFlight();
                return false;
            }
