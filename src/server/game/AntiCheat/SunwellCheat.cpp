...
void SunwellCheat::buildOpcodeReport(Player* player, uint16 opCode)
{
    ...
    if (opCounter >= opLimit)
    {
        ...
        {
            case SUN_ACTION_LOG:
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been reported due to possible DOS flooding! opCode: %u count: %u",
                    player->GetName().c_str(), player->GetGUID(), player->GetSession()->GetAccountId(), opCode, opCounter);
                break;
            case SUN_ACTION_KICK:
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been kicked due to possible DOS flooding! opCode: %u count: %u",
                    player->GetName().c_str(), player->GetGUID(), player->GetSession()->GetAccountId(), opCode, opCounter);
            ...
            case SUN_ACTION_BAN:
            {
                ...
                sWorld->BanAccount(bm, nameOrIp, duration, "DOS (Packet Flooding or Spoofing)", "[Cheat]");
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been banned for %u minutes due to possible DOS flooding! opCode: %u count: %u",
...
void SunwellCheat::buildCastReport(Player* player, uint32 spellId)
{
    ...
    if (castCounter >= castLimit)
    {
        ...
        {
            case SUN_ACTION_LOG:
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been reported due to casted spell: %u over: %u times! Possible gold cheater!",
                    player->GetName().c_str(), player->GetGUID(), player->GetSession()->GetAccountId(), spellId, castCounter);
                break;
            case SUN_ACTION_KICK:
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been kicked due to casted spell: %u over: %u times! Possible gold cheater!",
                    player->GetName().c_str(), player->GetGUID(), player->GetSession()->GetAccountId(), spellId, castCounter);
                player->GetSession()->KickPlayer();
                break;
            case SUN_ACTION_BAN:
            {
                ...
                sWorld->BanAccount(bm, nameOrIp, duration, "DOS (Packet Flooding or Spoofing)", "[Cheat]");
                sLog->outCheat("[Cheat] Player %s (GUID: " UI64FMTD ") (Account ID: %u) has been banned for %u due to casted spell: %u over: %u times! Possible gold cheater!",
