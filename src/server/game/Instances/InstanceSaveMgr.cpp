...
bool InstanceSaveManager::DeleteInstanceSaveIfNeeded(InstanceSave* save, bool skipMapCheck, bool deleteSave)
{
    ...
    if (!lock_instLists && save && save->m_playerList.empty() && (skipMapCheck || !sMapMgr->FindMap(save->GetMapId(), save->GetInstanceId())))
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INSTANCE_BY_INSTANCE);
        stmt->setUInt32(0, save->GetInstanceId());
        CharacterDatabase.Execute(stmt);

        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_LOOT_SYSTEM_LOOT);
        stmt->setUInt32(0, save->GetInstanceId());
        CharacterDatabase.Execute(stmt);
...
InstanceSave::InstanceSave(uint16 MapId, uint32 InstanceId, Difficulty difficulty, time_t resetTime, time_t extendedResetTime)
    ...
    , m_mapid(MapId)
    , m_difficulty(MapId == 631 || MapId == 724 ? Difficulty(difficulty % 2) : difficulty)
    , m_canReset(true)
    , m_instanceData("")
...
void InstanceSave::InsertToDB()
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_INSTANCE_SAVE);
...
bool InstanceSave::RemovePlayer(uint32 guidLow, InstanceSaveManager* ism, bool deleteSave)
{
    bool saveRemoved = false;
    {
        std::lock_guard lg(_lock);
        m_playerList.remove(guidLow);
        // ism passed as an argument to avoid calling via singleton (might result in a deadlock)
        saveRemoved = ism->DeleteInstanceSaveIfNeeded(this, false, false);
    }
    if (saveRemoved && deleteSave)
        delete this;
...
void InstanceSaveManager::LoadInstances()
{
    ...
    // Clean from group_saved_loot
    CharacterDatabase.DirectExecute("DELETE FROM group_saved_loot WHERE instanceId > 0 AND instanceId NOT IN (SELECT id FROM instance)");
...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    uint32 diff = (sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_HOUR) * HOUR + sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_HOUR) * MINUTE) - (sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_ZONE) * HOUR);
...
{
    QueryResult result = CharacterDatabase.Query("SELECT id, map, resettime, difficulty, completedEncounters, data, CustomDifficultyID FROM instance ORDER BY id ASC");
    if (result)
    {
        do
        {
            ...
            auto customDifficulty = static_cast<CustomDifficulty>(fields[6].GetUInt32());
            ...
            if (save)
            {
                ...
                save->SetCustomDifficulty(customDifficulty);
                if (resettime > 0)
...
void InstanceSaveManager::LoadCharacterBinds()
{
    ...
    QueryResult result = CharacterDatabase.Query("SELECT guid, instance, permanent, extended, CompletedAscendedEncounters FROM character_instance");
    if (result)
    {
        do
        {
            ...
            uint32 completedAscendedEncounters = fields[4].GetUInt32();
            ...
            if (InstanceSave* save = GetInstanceSave(instanceId))
            {
                ...
                if (bind.save) // pussywizard: another bind for the same map and difficulty! may happen because of mysql thread races
                {
                    ...
                    {
                        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE_GUID);
                ...
                bind.CompletedAscendedEncounters = completedAscendedEncounters;
                save->AddPlayer(guid);
                if (perm)
...
void InstanceSaveManager::_ResetSave(InstanceSaveHashMap::iterator& itr)
{
    ...
    if (pList.empty())
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE);
    ...
    else
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE_NOT_EXTENDED);
        stmt->setUInt32(0, itr->second->GetInstanceId());
        trans->Append(stmt);
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
    ...
    if (!warn)
    {
        ...
        uint32 diff = (sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_HOUR) * HOUR + sWorld->getIntConfig(CONFIG_SERVER_RESET_TIME_MINUTE) * MINUTE) - (sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_ZONE) * HOUR);
        ...
        // update it in the DB
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GLOBAL_INSTANCE_RESETTIME);
...
InstancePlayerBind* InstanceSaveManager::PlayerBindToInstance(uint32 guidLow, InstanceSave* save, bool permanent, Player* player /*= NULL*/, uint32 ascendedEncounterIndexMask /* = 0 */)
{
    ...
    uint32 encounterMask = bind.CompletedAscendedEncounters | ascendedEncounterIndexMask;
    ...
    if (bind.save)
    {
        if (save != bind.save || permanent != bind.perm)
        {
            ...
            stmt->setUInt32(2, encounterMask);
    ...
    else
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_GUID_MAP_DIFF); // DELETE ci FROM character_instance ci JOIN instance i ON ci.instance = i.id WHERE ci.guid = ? AND i.map = ? AND i.difficulty = ?
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_INSTANCE);
        stmt->setUInt32(0, guidLow);
        stmt->setUInt32(1, save->GetInstanceId());
        stmt->setBool(2, permanent);
        stmt->setUInt32(3, encounterMask);
        CharacterDatabase.Execute(stmt);
    }
    ...
    bind.save = save;
    bind.perm = permanent;
    bind.CompletedAscendedEncounters = encounterMask;
...
void InstanceSaveManager::PlayerUnbindInstance(uint32 guidLow, uint32 mapid, Difficulty difficulty, bool deleteFromDB, Player* player /*= NULL*/)
{
    ...
    if (itr != w->m[difficulty].end())
    {
        if (deleteFromDB)
        {
            ...
            // Save Loot System
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_LOOT_SYSTEM_LOOT);
            stmt->setUInt32(0, itr->second.save->GetInstanceId());
            CharacterDatabase.Execute(stmt);
        }
