...
int32 ReputationMgr::GetBaseReputation(FactionEntry const* factionEntry) const
{
    ...
    uint32 raceMask  = _player->getRaceMask(CF_DENY);
    uint32 classMask = _player->getClassMask();
    for (int i = 0; i < 4; i++)
    {
        if ((factionEntry->BaseRepRaceMask[i] & raceMask || (factionEntry->BaseRepRaceMask[i] == 0 && factionEntry->BaseRepClassMask[i] != 0)) && (factionEntry->BaseRepClassMask[i] & classMask || factionEntry->BaseRepClassMask[i] == 0))
...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
        ...
        if (new_rank >= REP_FRIENDLY && IsAtWar(factionEntry) && new_rank > old_rank)
...
void ReputationMgr::SetVisible(FactionTemplateEntry const* factionTemplateEntry)
{
    ...
    if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(factionTemplateEntry->faction))
        ...
        if (!(factionEntry->BaseRepRaceMask[1] & _player->getRaceMask(CF_DENY) && factionEntry->BaseRepValue[1] == Reputation_Bottom))
...
void ReputationMgr::SaveToDB(CharacterDatabaseTransaction& trans)
{
    for (FactionStateList::iterator itr = _factions.begin(); itr != _factions.end(); ++itr)
    {
        ...
        {
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CHAR_REPUTATION_BY_FACTION);
