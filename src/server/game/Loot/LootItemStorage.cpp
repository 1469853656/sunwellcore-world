...
void LootItemStorage::LoadStorageFromDB()
{
    ...
    auto stmt    = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ITEMCONTAINER_ITEMS);
...
void LootItemStorage::RemoveEntryFromDB(uint32 containerId, uint32 itemid, uint32 count)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEMCONTAINER_SINGLE_ITEM);
...
void LootItemStorage::AddNewStoredLoot(Loot* loot, Player* player)
{
    ...
    auto trans    = CharacterDatabase.BeginTransaction();
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_ITEMCONTAINER_SINGLE_ITEM);
...
void LootItemStorage::RemoveStoredLoot(uint32 containerId)
{
    ...
    auto trans    = CharacterDatabase.BeginTransaction();
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEMCONTAINER_CONTAINER);
    stmt->setUInt32(0, containerId);
    trans->Append(stmt);
