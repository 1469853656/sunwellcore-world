...
struct AdditionalLootSettings
{
    bool IsInInstanceOrNorthrend = false;
};
...
struct LootStoreItem
{
    ...
    bool Roll(bool rate, std::optional<AdditionalLootSettings> = std::nullopt) const; // Checks if the entry takes it's chance (at loot generation)
...
struct LootItem
{
    ...
    void SetAllowedLooters(AllowedLooterSet looters) { allowedGUIDs = looters; }
...
typedef std::map<uint32, std::shared_ptr<QuestItemList>> QuestItemMap;
...
public:
    ...
    void Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId = 0, std::optional<AdditionalLootSettings> settings = std::nullopt) const;
...
struct Loot
{
    ...
    bool FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError = false, uint16 lootMode = LOOT_MODE_DEFAULT, WorldObject* worldObject = nullptr);
    ...
    void SetLootedGUID(WorldObject* object);

    void FillLootFromDB(WorldObject* object);
    bool LoadedFromDB() { return m_lootLoadedFromDB; }
    ...
    void ProcessAOELoot(Loot& otherLoot, Player* lootOwner);

private:
    void RemoveSavedLootFromDB(CharacterDatabaseTransaction transaction, WorldObject* object);
    void FillNotNormalLootFor(Player* player, bool presentAtLooting, Player const* lootOwner, std::vector<LootItem>* succeededAOEItems = nullptr);
    void FillFFALoot(Player const* player, std::vector<LootItem>* succeededAOEItems);
    void FillQuestLoot(Player* player, std::vector<LootItem>* succeededAOEItems);
    void FillNonQuestNonFFAConditionalLoot(Player* player, bool presentAtLooting, std::vector<LootItem>* succeededAOEItems);
