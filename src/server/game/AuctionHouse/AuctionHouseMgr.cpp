...
AuctionHouseObject* AuctionHouseMgr::GetAuctionsMap(uint32 factionTemplateId)
{
    ...
    if (!u_entry || sWorld->IsCrossfactionEnabled(Crossfaction::AuctionHouse))
...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, ItemRef const& pItem, uint32 count)
{
    ...
    uint32 depPct    = sWorld->IsCrossfactionEnabled(Crossfaction::AuctionHouse) ? 5 : entry->depositPercent;
    float multiplier = CalculatePct(float(depPct), 3);
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, CharacterDatabaseTransaction& trans)
{
    ...
    SendAuctionWonMail(auction, pItem->GetItem(), trans);
}

void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, ItemRef& pItem, CharacterDatabaseTransaction& trans)
{
...
void AuctionHouseMgr::SendAuctionOutbiddedMail(AuctionEntry* auction, uint32 oldBidderGUID, uint32 oldPrice, Player* newBidder, CharacterDatabaseTransaction& trans)
{
    uint64 oldBidder_guid = MAKE_NEW_GUID(oldBidderGUID, 0, HIGHGUID_PLAYER);
    ...
    if (oldBidder || oldBidder_accId)
    {
        if (oldBidder && newBidder)
            oldBidder->GetSession()->SendAuctionBidderNotification(auction->GetHouseId(), auction->Id, newBidder->GetGUID(), auction->bid, auction->GetAuctionOutBid(), auction->item_template);

        MailDraft(auction->BuildAuctionMailSubject(AUCTION_OUTBIDDED), AuctionEntry::BuildAuctionMailBody(auction->owner, oldPrice, auction->buyout, auction->deposit, auction->GetAuctionCut()))
            .AddMoney(oldPrice)
            .SendMailTo(trans, MailReceiver(oldBidder, oldBidderGUID), auction, MAIL_CHECK_MASK_COPIED);
    }
}
...
{
    uint32 houseid = 7; // neutral/goblin auction house

    if (!sWorld->IsCrossfactionEnabled(Crossfaction::AuctionHouse))
    {
...
AuctionHouseEntry const* AuctionHouseMgr::GetAuctionHouseEntry(Creature const* auctioneer)
{
    if (auctioneer->GetEntry() == CustomConstants::HardcoreAuctionHouseEntry)
        return sAuctionHouseStore.LookupEntry(1); // use human auction house, we use goblin (neutral) auction house for other players

    return GetAuctionHouseEntry(auctioneer->getFaction());
}
...
{
    uint32 cutPct = sWorld->IsCrossfactionEnabled(Crossfaction::AuctionHouse) ? 5 : auctionHouseEntry->cutPercent;
    int32 cut = int32(CalculatePct(bid, cutPct) * sWorld->getRate(RATE_AUCTION_CUT));
...
void AuctionEntry::DeleteFromDB(const CharacterDatabaseTransaction& trans) const
{
...
void AuctionEntry::SaveToDB(const CharacterDatabaseTransaction& trans) const
{
