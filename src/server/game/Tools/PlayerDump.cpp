...
DumpReturn PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32 guid)
{
    ...
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHECK_GUID);
    ...
    while (!feof(fin))
    {
        ...
        if ((line.substr(nw_pos, 16) == "== START DUMP ==") || (line.substr(nw_pos, 15) == "IMPORTANT NOTE:") || (line.substr(nw_pos, 14) == "== END DUMP =="))
            continue;
        ...
        */
        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                ...
                race        = uint8(std::atol(getnth(line, 4).c_str()));
                playerClass = uint8(std::atol(getnth(line, 5).c_str()));
                gender      = uint8(std::atol(getnth(line, 6).c_str()));
                level       = uint8(std::atol(getnth(line, 7).c_str()));
                if (name == "")
                {
    ...
    sWorld->AddGlobalPlayerData(guid, account, name, gender, race, playerClass, level, mails.size(), 0, AccountMgr::GetName(account), false, false);
