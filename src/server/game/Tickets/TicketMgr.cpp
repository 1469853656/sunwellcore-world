...
GmTicket::GmTicket(Player* player)
    ...
    , _lastModifiedTime(time(nullptr))
    , _closedBy(0)
    , _assignedTo(0)
    , _completed(false)
    , _escalatedStatus(TICKET_UNASSIGNED)
...
void GmTicket::SaveToDB(CharacterDatabaseTransaction& trans) const
{
    ...
    uint8 index             = 0;
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_GM_TICKET);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GM_TICKET);
...
void GmTicket::SetCompleted(uint64 completedBy)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GM_TICKET_COMPLETE);
...
void TicketMgr::ResetTickets()
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ALL_GM_TICKETS);
...
void TicketMgr::LoadTickets()
{
    ...
    auto stmt    = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GM_TICKETS);
