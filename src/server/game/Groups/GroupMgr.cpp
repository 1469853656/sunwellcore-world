...
{
    {
        ...
        CharacterDatabase.DirectExecute("DELETE `groups` FROM `groups` LEFT JOIN (SELECT guid, count(*) as cnt FROM group_member GROUP BY guid) as t ON `groups`.guid = t.guid WHERE t.guid IS NULL OR (t.cnt<=1 AND `groups`.groupType <> 12)");
