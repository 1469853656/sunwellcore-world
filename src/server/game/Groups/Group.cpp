...
bool Group::Create(Player* leader)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GROUP);
...
bool Group::LoadGroupFromDB(Field* fields)
{
    ...
    if (!sObjectMgr->GetPlayerNameByGUID(fields[0].GetUInt32(), m_leaderName))
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GROUP);
...
void Group::LoadMemberFromDB(uint32 guidLow, uint8 memberFlags, uint8 subgroup, uint8 roles)
{
    ...
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GROUP_MEMBER);
...
void Group::ConvertToLFG()
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_TYPE);
...
void Group::ConvertToRaid()
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_LOOT_TYPE);

        stmt->setUInt8(0, uint8(MASTER_LOOT));
        stmt->setUInt32(1, GetLeaderGUID());
        stmt->setUInt64(2, GetLowGUID());
...
bool Group::AddInvite(Player* player)
{
    ...
    // Players cannot receive Call to Arms reward if they manually invite somebody
    if (isLFGGroup())
        sLFGMgr->SetCallToArmsEligible(GetGUID(), false);
...
static std::string PrepareNinjaMessage(NinjaMessage msgType, std::string const& ninjaName, uint32 postId, bool skipHeader)
{
    ...
    msg << "\n * <" << ninjaName << "> was added to the ninja looter list due to the following proofs: number of topic: " << postId << "/ sunwell-community.com/forum/160-ninja-looters-list-of-proofs/";
    return msg.str();
}
    ...
    if (QueryResult result = CharacterDatabase.PQuery(query.c_str()))
    {
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* member = itr->GetSource())
            {
                ...
                {
                    ChatHandler(member->GetSession()).SendSysMessage(msgNinjasHereStr.c_str());
                }
                else if (!group->isRaidGroup() && !msgNinjaAddedStr.empty())
                {
                    ChatHandler(member->GetSession()).SendSysMessage(msgNinjaAddedStr.c_str());
                }
            }
        }
    }
...
bool Group::AddMember(Player* player)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_GROUP_MEMBER);
...
void Group::ChangeLeader(uint64 newLeaderGuid)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_LEADER);
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    if (InstanceScript* instance = pLootedObject->GetInstanceScript())
        ignoreThreshold = instance->IsMythic();
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (ignoreThreshold || item->Quality >= uint32(m_lootThreshold))
        {
            ...
            for (GroupReference* itr = GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                {
                    bool isPlayerAllowedToLoot = true;
                    if (pLootedObject->IsCreature() && pLootedObject->ToCreature()->HasPlayersAllowedToLoot())
                        isPlayerAllowedToLoot = pLootedObject->ToCreature()->IsPlayerAllowedToLoot(member);
                    else if (pLootedObject->IsGameObject() && pLootedObject->ToGameObject()->HasPlayersAllowedToLoot())
                        isPlayerAllowedToLoot = pLootedObject->ToGameObject()->IsPlayerAllowedToLoot(member);

                    if (i->AllowedForPlayer(member) && isPlayerAllowedToLoot)
                    {
    ...
    for (std::vector<LootItem>::iterator i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            for (GroupReference* itr = GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                {
                    bool isPlayerAllowedToLoot = true;
                    if (lootedObject->IsCreature() && lootedObject->ToCreature()->HasPlayersAllowedToLoot())
                        isPlayerAllowedToLoot = lootedObject->ToCreature()->IsPlayerAllowedToLoot(playerToRoll);
                    else if (lootedObject->IsGameObject() && lootedObject->ToGameObject()->HasPlayersAllowedToLoot())
                        isPlayerAllowedToLoot = lootedObject->ToGameObject()->IsPlayerAllowedToLoot(playerToRoll);

                    if (isPlayerAllowedToLoot && i->AllowedForPlayer(playerToRoll))
                    {
                        r->totalPlayersRolling++;
...
void Group::MasterLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    bool allowedToLootListEmpty = false;
    if (!pLootedObject || (pLootedObject->IsCreature() && !pLootedObject->ToCreature()->HasPlayersAllowedToLoot()) || (pLootedObject->IsGameObject() && !pLootedObject->ToGameObject()->HasPlayersAllowedToLoot()))
    {
        allowedToLootListEmpty = true;
        // Show log for all gameobjects and creatures which are world/dungeon bosses
        bool showLog = false;
        if (pLootedObject)
        {
            if (pLootedObject->IsCreature())
            {
                if (pLootedObject->ToCreature()->isWorldBoss() || pLootedObject->ToCreature()->IsDungeonBoss())
                    showLog = true;
            }
            else if (pLootedObject->IsGameObject())
                showLog = true;
        }
        if (showLog)
            sLog->outError("Group::MasterLoot %s (Entry: %u, Guid: %u, InstanceId: %u) has no players allowed to loot", (pLootedObject->IsCreature() ? "Creature" : "Gameobject"), pLootedObject->GetEntry(), pLootedObject->GetGUIDLow(), pLootedObject->GetInstanceId());
    }
    ...
    for (GroupReference* itr = GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        if (!allowedToLootListEmpty)
        {
            if ((pLootedObject->IsCreature() && pLootedObject->ToCreature()->IsPlayerAllowedToLoot(looter) || (pLootedObject->IsGameObject() && pLootedObject->ToGameObject()->IsPlayerAllowedToLoot(looter))))
            {
                data << uint64(looter->GetGUID());
                ++real_count;
            }
        }
        else if (looter->IsAtGroupRewardDistance(pLootedObject))
        {
            data << uint64(looter->GetGUID());
            ++real_count;
        }
...
void Group::CountTheRoll(Rolls::iterator rollI, Map* allowedMap)
{
    ...
    std::unordered_map<uint64, std::vector<ProblematicItemsData>> problematicItems;
    ...
    if (roll->totalNeed > 0)
    {
        ...
        {
            std::multimap<uint8, uint64> rollValues;
            for (Roll::PlayerVote::const_iterator itr = roll->playerVote.begin(); itr != roll->playerVote.end(); ++itr)
            {
                if (itr->second != NEED)
                    continue;
                ...
                if (!player || (allowedMap && player->FindMap() != allowedMap))
                {
                    --roll->totalNeed;
                    continue;
                }

                uint32 const rollValue = urand(1, 100);
                SendLootRoll(0, itr->first, static_cast<uint8>(rollValue), ROLL_NEED, *roll);
                rollValues.emplace(rollValue, itr->first);
            }

            if (!rollValues.empty())
            {
                uint8 const maxRoll = std::prev(rollValues.end())->first;                           // Get max roll from all players
                auto const idx      = urand(0, static_cast<uint32>(rollValues.count(maxRoll)) - 1); //...and select random player from max roll pool

                if (uint64 const selectedPlayerGUID = std::next(rollValues.equal_range(maxRoll).first, idx)->second) // pussywizard: added condition
                {
                    SendLootRollWon(0, selectedPlayerGUID, maxRoll, ROLL_NEED, *roll);
                    if (Player* player = ObjectAccessor::FindPlayer(selectedPlayerGUID))
                    {
                        player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED_ON_LOOT, roll->itemid, maxRoll);

                        ItemPosCountVec dest;
                        LootItem* item = &(roll->itemSlot >= roll->getLoot()->items.size() ? roll->getLoot()->quest_items[roll->itemSlot - roll->getLoot()->items.size()] : roll->getLoot()->items[roll->itemSlot]);
                        if (InventoryResult msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, roll->itemid, item->count); msg == EQUIP_ERR_OK)
                        {
                            item->is_looted = true;
                        ...
                        else
                        {
                            ...
                            ProblematicItemsData data;
                            data.ItemID           = roll->itemid;
                            data.ItemCount        = item->count;
                            data.RandomPropertyID = item->randomPropertyId;
                            problematicItems[player->GetGUID()].emplace_back(data);
                        }
                    }
                }
                else
    ...
    if (roll->totalNeed == 0 && roll->totalGreed > 0) // pussywizard: if (roll->totalNeed == 0 && ...), not else if, because numbers can be modified above if player is on a different map
    {
        if (!roll->playerVote.empty())
        {
            std::multimap<uint8, std::pair<uint64, RollVote>> rollValues;
            ...
            for (itr = roll->playerVote.begin(); itr != roll->playerVote.end(); ++itr)
            {
                ...
                uint32 const rollValue = urand(1, 100);
                SendLootRoll(0, itr->first, static_cast<uint8>(rollValue), itr->second, *roll);
                rollValues.emplace(rollValue, std::make_pair(itr->first, itr->second));
            }
            ...
            if (!rollValues.empty())
            {
                uint8 const maxRoll                        = std::prev(rollValues.end())->first;                           // Get max roll from all players
                auto const idx                             = urand(0, static_cast<uint32>(rollValues.count(maxRoll)) - 1); //...and select random player from max roll pool
                auto const& [selectedPlayerGUID, rollType] = std::next(rollValues.equal_range(maxRoll).first, idx)->second;

                if (selectedPlayerGUID) // pussywizard: added condition
                {
                    SendLootRollWon(0, selectedPlayerGUID, maxRoll, rollType, *roll);

                    if (Player* player = ObjectAccessor::FindPlayer(selectedPlayerGUID))
                    {
                        player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT, roll->itemid, maxRoll);

                        LootItem* item = nullptr;
                        if (roll->itemSlot < roll->getLoot()->items.size())
                            item = &(roll->getLoot()->items[roll->itemSlot]);
                        else if (roll->itemSlot < roll->getLoot()->items.size() + roll->getLoot()->quest_items.size())
                            item = &(roll->getLoot()->quest_items[roll->itemSlot - roll->getLoot()->items.size()]);
    ...
    for (auto const& [guid, itemData] : problematicItems)
    {
        if (Player* player = ObjectAccessor::FindPlayer(guid))
        {
            ...
            for (auto const& individualItemData : itemData)
            {
                if (Item* item = Item::CreateItem(individualItemData.ItemID, individualItemData.ItemCount))
                {
                    // if (individualItemData.RandomPropertyID)
                    // item->SetItemRandomProperties(individualItemData.RandomPropertyID);
                    item->SaveToDB(trans);
                    draft.AddItem(item);
                }
            }
...
void Group::SendUpdateToPlayer(uint64 playerGUID, MemberSlot* slot)
{
    ...
    if (GetMembersCount() - 1)
    {
        ...
        if (m_raidDifficulty >= 2 && player->GetMapId() == 249)
        {
            data << uint8(1); // Raid Difficulty
...
void Group::ChangeMembersGroup(uint64 guid, uint8 group)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_MEMBER_SUBGROUP);
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const* bgTemplate, BattlegroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/, bool isRated, uint32 arenaSlot)
{
    ...
    // Arenas only at max level
    if (isRated && reference->getLevel() != DEFAULT_MAX_LEVEL)
        return ERR_BATTLEGROUND_JOIN_TIMED_OUT;

    TeamId teamId = reference->GetTeam();
    ...
    for (GroupReference* itr = GetFirstMember(); itr != nullptr; itr = itr->next(), ++memberscount)
    {
        ...
        if (member->GetTeam() != teamId)
            return ERR_BATTLEGROUND_JOIN_TIMED_OUT;

        // don't let join rated matches if the arena team id doesn't match
        if (isRated && !member->GetArenaTeamId(arenaSlot))
            return ERR_BATTLEGROUND_JOIN_FAILED;

        // Arenas only at max level
        if (isRated && member->getLevel() != DEFAULT_MAX_LEVEL)
            return ERR_BATTLEGROUND_JOIN_TIMED_OUT;
...
void Group::SetDungeonDifficulty(Difficulty difficulty)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_DIFFICULTY);
...
void Group::SetRaidDifficulty(Difficulty difficulty)
{
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_RAID_DIFFICULTY);
...
void Group::SetGroupMemberFlag(uint64 guid, bool apply, GroupMemberFlags flag)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GROUP_MEMBER_FLAG);
...
bool const Group::IsMythicGroup() const
{
    return m_mythicGroup;
}

void Group::SetMythicGroup(bool toggle)
{
    m_mythicGroup = toggle;
}
