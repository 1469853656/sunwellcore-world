...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite)
{
    ...
    else if (serverwide_evt)
    {
        ...
        if (overwrite || data.state != GAMEEVENT_WORLD_FINISHED)
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ALL_GAME_EVENT_CONDITION_SAVE);
...
{
    {
        ...
        do
        {
            ...
            {
                sLog->outErrorDb("`game_event` game event id(%i) has out of range holidayStage %u.", event_id, pGameEvent.holidayStage);
...
void GameEventMgr::Initialize()
{
    ...
    if (result)
    {
        ...
        uint16 maxEventId = fields[0].GetUInt8();
...
{
    std::map<uint32, std::set<uint64>> creaturesByMap;

    // go through the creatures whose npcflags are changed in the event
    for (auto const& gameEventNPCFlag : mGameEventNPCFlags[event_id])
    {
        CreatureData const* data = sObjectMgr->GetCreatureData(gameEventNPCFlag.first);
        if (!data)
            continue;

        // get the creature data from the low guid to get the entry, to be able to find out the whole guid
        creaturesByMap[data->mapid].insert(MAKE_NEW_GUID(gameEventNPCFlag.first, data->id, HIGHGUID_UNIT));
    }

    for (auto const& pair : creaturesByMap)
    {
        sMapMgr->DoForAllMapsWithMapId(pair.first, [this, &pair](Map const* map)
        {
            for (uint64 creatureGUID : pair.second)
            {
                if (Creature* creature = map->GetCreature(creatureGUID))
                {
    ...
    if (itr != mQuestToEventConditions.end())
    {
        ...
        if (citr != mGameEvent[event_id].conditions.end())
        {
            ...
            if (citr->second.done < citr->second.reqNum)
            {
                ...
                auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GAME_EVENT_CONDITION_SAVE);
...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GAME_EVENT_SAVE);
    stmt->setUInt8(0, uint8(event_id));
    trans->Append(stmt);
...
void GameEventMgr::OverrideEventStartTime(uint16 event_id, time_t newStartTime)
{
    if (event_id >= mGameEvent.size())
        return;

    GameEventData& eventData = mGameEvent[event_id];
    if (!eventData.isValid())
        return;

    eventData.start = newStartTime;

}

void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
    sMapMgr->DoForAllMaps([event_id, activate](Map* map) { map->ScheduleDelayedGameEventAIHook(event_id, activate); });
}
...
void GameEventMgr::SetHolidayEventTime(GameEventData& event)
{
    ...
    switch (int(holiday->CalendarFilterType))
    {
        case -1:                             // Yearly
