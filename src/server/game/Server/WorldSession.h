...
class InstanceSave;
class Item;
class CharacterInfoQueryHolder;
...
enum AccountFlags
{
    ...
    ACCOUNT_FLAG_CHARACTER_BOOSTED = 0x04,
};
...
//! maximum length the client can send at once is 255 characters
//! everything else is cheating attempt with wowless client
constexpr uint32 CHAT_MESSAGE_MAXIMUM_SIZE{ 255 };
...
{
public:
    WorldSession(uint32 id, std::shared_ptr<WorldSocket> sock, AccountTypes sec, uint8 expansion, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter, bool skipQueue, bool hasBoostedCharacter);
    ~WorldSession();
    ...
    bool HasBoostedCharacter() const { return _hasBoostedCharacter; }
    void AddBoostedCharacterFlag();
    ...
    void LogoutRequest(time_t requestTime, uint32_t delay = 20)
    {
        _logoutTime = requestTime + (requestTime > 0 ? delay : 0);
    }

    /// Is logout cooldown expired?
    bool ShouldLogOut(time_t currTime) const
    {
        return _transactionCallbacks.empty() && (_logoutTime > 0 && currTime >= _logoutTime);
    }
    ...
    void SendInvalidatePlayer(uint64 guid);
    ...
    void SaveTutorialsData(const CharacterDatabaseTransaction& trans);
    ...
    {
        if (time_t(diff) > m_timeOutTime.load(std::memory_order_relaxed))
            m_timeOutTime.store(0, std::memory_order_relaxed);
        else
            m_timeOutTime -= diff;
    }
    void ResetTimeOutTime()
    {
        m_timeOutTime.store(sWorld->getIntConfig(CONFIG_SOCKET_TIMEOUTTIME), std::memory_order_relaxed);
    }
    ...
    // Premium services
    auto const& GetPremiumServices() const { return _premiumServices; }
    void ResetPremiumServices() { _premiumServices = {}; }
    time_t GetPremiumService(PremiumServices const serviceID) const { return _premiumServices[to_underlying(serviceID)]; }
    bool HasPremiumService(PremiumServices serviceID) const;
    void SetPremiumService(PremiumServices const serviceID, time_t const serviceTime) { _premiumServices[to_underlying(serviceID)] = serviceTime; }

    // Packets cooldown
    time_t GetCalendarEventCreationCooldown() const { return _calendarEventCreationCooldown; }
    void SetCalendarEventCreationCooldown(time_t cooldown) { _calendarEventCreationCooldown = cooldown; }

    void SetMaxLevelCharacterCount(uint32 const characterCount) { _maxLevelCharacterCount = characterCount; }
    uint32 GetMaxLevelCharacterCount() const { return _maxLevelCharacterCount; }
...
public:                                                 // opcodes handlers
    ...
    void HandlePlayerLoginFromDB(CharacterInfoQueryHolder const& holder);
    ...
    void HandleLoadActionsSwitchSpecCallback(PreparedQueryResult result) const;
    ...
    void HandleLoadPetFromDBSecondCallback(LoadPetFromDBQueryHolder const& holder) const;
    ...
    void HandleAuctionListOwnerItemsEvent(uint64 guid, uint32 listfrom);
    ...
    // Premium services
    std::array<time_t, to_underlying(PremiumServices::Max)> _premiumServices = { };
    ...
    void ScheduleMuteHistoryQuery(uint32 accountID);
...
private:
    ...
    uint32 _accountId;
    uint8 m_expansion;
    bool _hasBoostedCharacter;

    uint32 _maxLevelCharacterCount = 0;
...
private:
    ...
    uint32 recruiterId;
    bool isRecruiter;
    Threading::LockedQueue<WorldPacket*> _recvQueue;
