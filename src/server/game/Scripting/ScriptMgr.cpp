...
template<class TScript>
void ScriptRegistry<TScript>::AddScript(TScript* const script)
{
    ASSERT(script);
...
// Gets a script by its ID (assigned by ObjectMgr).
template<class TScript>
TScript* ScriptRegistry<TScript>::GetScriptById(uint32 id)
{
    ScriptMapIterator it = ScriptPointerList.find(id);
    if (it != ScriptPointerList.end())
        return it->second;
...
void ScriptMgr::OnGossipSelect(Player* player, ItemRef const& item, uint32 sender, uint32 action)
{
    ASSERT(player);
    ASSERT(item);
...
void ScriptMgr::OnGossipSelectCode(Player* player, ItemRef const& item, uint32 sender, uint32 action, const char* code)
{
    ASSERT(player);
    ASSERT(item);
