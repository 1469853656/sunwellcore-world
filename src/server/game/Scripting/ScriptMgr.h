...
public:
    ...
    virtual WorldZoneScript* GetWorldZoneScript() const { return nullptr; }
};
...
public:
    ...
    virtual void OnGossipSelect(Player* /*player*/, ItemRef const& /*item*/, uint32 /*sender*/, uint32 /*action*/) { }

    // Called when a player selects an option in an item gossip window
    virtual void OnGossipSelectCode(Player* /*player*/, ItemRef const& /*item*/, uint32 /*sender*/, uint32 /*action*/, const char* /*code*/) { }
};
...
class ScriptMgr : public Sunwell::Singleton<ScriptMgr>
{
    friend class Sunwell::Singleton<ScriptMgr>;
...
public: /* ServerScript */
    ...
    void OnSocketClose(WorldSocket* socket);
...
public: /* ItemScript */
    ...
    void OnGossipSelect(Player* player, ItemRef const& item, uint32 sender, uint32 action);
    void OnGossipSelectCode(Player* player, ItemRef const& item, uint32 sender, uint32 action, const char* code);
...
{
public:
    SpellScriptLoaderEx(char const* name)
...
{
public:
    AuraScriptLoaderEx(char const* name)
        : SpellScriptLoader(name)
    {
    }
...
{
public:
    SpellAuraScriptLoaderEx(char const* name)
        : SpellScriptLoader(name)
    {
    }
...
{
public:
    CreatureAILoader(char const* name)
        : CreatureScript(name)
    {
    }
...
{
public:
    InstanceMapScriptLoader(char const* name, uint32 mapId)
        : InstanceMapScript(name, mapId)
    {
    }
...
{
public:
    GameObjectAILoader(char const* name)
...
{
public:
    GenericSpellScriptLoader(char const* name)
        : SpellScriptLoader(name)
    {
    }
...
{
public:
    GenericAuraScriptLoader(char const* name)
        : SpellScriptLoader(name)
    {
    }
...
{
public:
    GenericSpellAndAuraScriptLoader(char const* name)
        : SpellScriptLoader(name)
    {
    }
    SpellScript* GetSpellScript() const override { return new S(); }
