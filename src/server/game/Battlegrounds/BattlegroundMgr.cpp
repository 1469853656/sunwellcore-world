...
BattlegroundMgr::BattlegroundMgr()
    : randomBgDifficultyEntry(999, 0, 80, 80, 0)
    , m_lastClientVisibleInstanceId(0)
    ...
    , m_Testing(false)
{
    ...
    {
        if (qtype <= BATTLEGROUND_QUEUE_SOLO_QUEUE)
            ...
            m_BattlegroundQueues[qtype].SetBgTypeIdAndArenaType(BGTemplateId(BattlegroundQueueTypeId(qtype - BATTLEGROUND_QUEUE_SOLO_QUEUE)), BGArenaType(BattlegroundQueueTypeId(qtype)), true);
    }
}
...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    {
        // for rated arenas
        for (uint32 qtype = BATTLEGROUND_QUEUE_2v2; qtype <= BATTLEGROUND_QUEUE_SOLO_QUEUE; ++qtype)
            for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
                m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(BattlegroundBracketId(bracket), 0x03, true); // pussywizard: 0 for rated means looking for opponents for every team
        ...
        for (uint8 action = 1; action <= 2; ++action)
            for (uint32 qtype = BATTLEGROUND_QUEUE_AV; qtype <= BATTLEGROUND_QUEUE_SOLO_QUEUE; ++qtype)
                for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
                    m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(BattlegroundBracketId(bracket), action);
    ...
    if (sWorld->getBoolConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS))
    {
        if (sWorld->GetGameTime() > m_NextAutoDistributionTime)
        {
            ...
            m_NextAutoDistributionTime = sWorld->GetNextTimeWithDayAndHourAndMinute(sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_DAY), sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_HOUR), sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_MINUTE));
    ...
    *data << uint8(type); // type (battleground=0/arena=1)
    ...
    if (type) // arena
    {
        ...
        for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE; iTeamId = TeamId(iTeamId + 1))
        {
            ...
            uint32 MatchmakerRating = bg->GetArenaTeamMatchmakerRating(Battleground::GetOtherTeamId(iTeamId));
        ...
        for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE; iTeamId = TeamId(iTeamId + 1))
        {
            std::string name = (iTeamId == TEAM_ALLIANCE) ? "Green Team" : "Gold Team";
            *data << name;
        }
    }
    ...
    *data << uint32(scoreCount); // placeholder
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        *data << uint32(itr2->second->KillingBlows);
        ...
        {
            *data << uint8(itr2->second->player->GetBgTeam() == TEAM_ALLIANCE ? 1 : 0); // green or yellow
        }
...
const bool BattlegroundMgr::GetArenaListByType(std::vector<Battleground*>& arenaContainer, ArenaType type, bool onlyRated) const
{
    for (auto& bg : m_Battlegrounds)
        if (bg.second->isArena() && bg.second->GetArenaType() == type && (!onlyRated || bg.second->isRated()))
            arenaContainer.push_back(bg.second);
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    m_NextAutoDistributionTime = wstime ? wstime : sWorld->GetNextTimeWithDayAndHourAndMinute(sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_DAY), sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_HOUR), sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_MINUTE));
    if (!wstime)
        sWorld->setWorldState(WS_ARENA_DISTRIBUTION_TIME, uint64(m_NextAutoDistributionTime));
}
    ...
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_RV:
            switch (arenaType)
            {
                ...
                case ARENA_TYPE_SOLO_QUEUE:
                    return BATTLEGROUND_QUEUE_SOLO_QUEUE;
                default:
...
BattlegroundTypeId BattlegroundMgr::BGTemplateId(BattlegroundQueueTypeId bgQueueTypeId)
{
    switch (bgQueueTypeId)
    {
        ...
        case BATTLEGROUND_QUEUE_SOLO_QUEUE:
...
uint32 BattlegroundMgr::GetMaxRatingDifference(uint32 mmrRating) const
{
    uint32 diff = sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE);
    if (mmrRating)
    {
        if (mmrRating >= 3000)
            diff = sWorld->getIntConfig(CONFIG_SOLO_QUEUE_MAX_RATING_DIFFERENCE_3000);
        else if (mmrRating >= 2000)
            diff = sWorld->getIntConfig(CONFIG_SOLO_QUEUE_MAX_RATING_DIFFERENCE_2000);
        else
            diff = sWorld->getIntConfig(CONFIG_SOLO_QUEUE_MAX_RATING_DIFFERENCE_1500);
    }
...
uint32 BattlegroundMgr::GetRatingDiscardTimer(BattlegroundQueueTypeId queueTypeID) const
{
    if (queueTypeID == BATTLEGROUND_QUEUE_SOLO_QUEUE)
        return sWorld->getIntConfig(CONFIG_SOLOQUEUE_ARENA_RATING_DISCARD_TIMER);
...
void BattlegroundMgr::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, TeamId teamId)
{
    ...
    if (bg->isRated() && bg->isArena())
        bg->SetArenaTeamIdForTeams(ginfo->teamId, ginfo->arenaTeams);
    ...
    for (std::set<uint64>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
    {
        ...
        if (bg->isArena() && bg->isRated())
            bg->ArenaLogEntries[player->GetGUID()].Fill(player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId(), player->GetSession()->GetRemoteAddress());
    }
}

RandomBattlegroundSystem::RandomBattlegroundSystem()
    : m_CurrentRandomBg(BATTLEGROUND_TYPE_NONE)
    , m_SwitchTimer(0)
{
}
    ...
    if (m_SwitchTimer <= diff)
    {
        if (m_BgOrder.empty())
        {
            ...
            */
            ...
            std::vector<BattlegroundTypeId> bigBg, smallBg;
            bigBg.push_back(BATTLEGROUND_AV);
            bigBg.push_back(BATTLEGROUND_IC);
            smallBg.push_back(BATTLEGROUND_WS);
            smallBg.push_back(BATTLEGROUND_EY);
            smallBg.push_back(BATTLEGROUND_AB);
            smallBg.push_back(BATTLEGROUND_SA);

            std::shuffle(bigBg.begin(), bigBg.end(), GetGenerator());
            std::shuffle(smallBg.begin(), smallBg.end(), GetGenerator());

            m_BgOrder.push_back(smallBg.back());
            smallBg.pop_back();
            m_BgOrder.push_back(smallBg.back());
            smallBg.pop_back();
            m_BgOrder.push_back(smallBg.back());
            smallBg.pop_back();
            m_BgOrder.push_back(bigBg.back());
            bigBg.pop_back();
            m_BgOrder.push_back(smallBg.back());
            smallBg.pop_back();
            m_BgOrder.push_back(bigBg.back());
