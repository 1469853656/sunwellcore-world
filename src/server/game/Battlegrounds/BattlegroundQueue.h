...
struct GroupQueueInfo // stores information about the group in queue (also used when joined as solo!)
{
    ...
    bool IsRated;                     // rated
    uint8 ArenaType;                  // 2v2, 3v3 or 0 when BG
    uint32 LeaderArenaTeamId;         // team id if rated match
    ...
    bool IsTwink;
    bool IsCrossfactionAllowed;
    uint32 GroupGUID; // for Solo Queue

    // pussywizard: for internal use
    uint8 _bracketId;
    uint8 _groupType;

    std::unordered_map<uint64, uint32> arenaTeams;
...
public:
    ...
    void BattlegroundQueueUpdate(BattlegroundBracketId bracket_id, uint8 actionMask, bool isRated = false);
    void UpdateEvents(uint32 diff);

    bool FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start = false, PvPDifficultyEntry const* bracketEntry = nullptr);
    bool FillXPlayersToBGWithSpecific(Battleground* bg, const int32 aliFree, const int32 hordeFree, BattlegroundBracketId thisBracketId, BattlegroundQueue* specificQueue, BattlegroundBracketId specificBracketId);
    ...
    GroupQueueInfo* AddGroup(Player* leader, Group* group, PvPDifficultyEntry const* bracketEntry, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 LeaderArenaTeamId);
    GroupQueueInfo* AddSoloQueueArenaGroup(Player* leader, PvPDifficultyEntry const* bracketEntry, std::vector<SoloQueue> const& soloQueues, uint8 ArenaType, uint32 ArenaRating, uint32 MatchmakerRating, uint32 leaderArenaTeamId, std::set<uint64> const& list);
    ...
    uint32 GetAverageQueueWaitTime(GroupQueueInfo* ginfo, BattlegroundBracketId bracketId = BG_BRACKET_ID_FIRST) const;
    void StartInstantSoloQueueArena(OptimizedSoloQueue const& firstOptimizedSoloQueue, OptimizedSoloQueue const& secondOptimizedSoloQueue, PvPDifficultyEntry const* bracketEntry);

    void SetBgTypeIdAndArenaType(BattlegroundTypeId b, uint8 a, bool isTwinkQueue)
    {
        m_bgTypeId     = b;
        m_arenaType    = ArenaType(a);
        m_isTwinkQueue = isTwinkQueue;
    } // pussywizard
...
private:
    bool IsCrossfactionEnabledForBracket(PvPDifficultyEntry const* bracketEntry);
