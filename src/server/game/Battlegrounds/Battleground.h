...
enum ArenaType
{
    ...
    ARENA_TYPE_2v2        = 2,
    ARENA_TYPE_3v3        = 3,
    ARENA_TYPE_SOLO_QUEUE = 5,

    ARENA_TYPE_MAX        = ARENA_TYPE_SOLO_QUEUE
};
...
enum class ValidArenaResult : uint32
{
    RESULT_OK = 0,
    RESULT_OVERTIME,
    RESULT_SAME_IP_ADDRESS_IN_OPPOSITE_TEAM,
    RESULT_REPEATING_ARENA_OPPONENTS,
    RESULT_WRONG_HOUR
};

struct BattlegroundScore
{
    BattlegroundScore(Player* player)
        : KillingBlows(0)
        , Deaths(0)
        , HonorableKills(0)
        , BonusHonor(0)
...
public:
    ...
    void Fill(const char* name, uint32 guid, uint32 acc, std::string ip)
    {
...
public:
    ...
    void RemoveToBeTeleported(uint64 spectator)
    {
        ToBeTeleportedMap::iterator itr = m_ToBeTeleported.find(spectator);
        if (itr != m_ToBeTeleported.end()) m_ToBeTeleported.erase(itr);
    }
    ...
    ValidArenaResult ValidateRatedArena(TeamId winnerTeamId) const;
    ...
    Optional<WorldPacket> BuildBattlegroundPlayerPositionPacket(Player* player);
    ...
    void SetArenaTeamRatingChangeForTeam(TeamId teamId, int32 RatingChange);
    int32 GetArenaTeamRatingChangeForTeam(TeamId teamId) const;
    void SetArenaTeamMatchmakerRating(TeamId teamId, uint32 MMR);
    uint32 GetArenaTeamMatchmakerRating(TeamId teamId) const;
    void SetArenaTeamRating(TeamId teamId, uint32 Rating);
    uint32 GetArenaTeamRating(TeamId teamId) const;
    void SetArenaTeamIdForTeams(TeamId teamId, std::unordered_map<uint64, uint32> arenaTeams);
    std::unordered_map<uint64, uint32> const& GetArenaTeamIdsForTeam(TeamId teamId) const;
    void SetArenaTeamLastDPSList(TeamId teamId, std::set<uint64> const& list);
    std::set<uint64> const& GetArenaTeamLastDPSList(TeamId teamId) const;
    ...
    bool IsFlagPicker(uint64 guid) const { return GetFlagPickerGUID(TEAM_ALLIANCE) == guid || GetFlagPickerGUID(TEAM_HORDE) == guid; }
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_WS)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_AB)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_NA)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_BE)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_EY)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_RL)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_DS)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_RV)
    ...
    {
        if (GetBgTypeID() == BATTLEGROUND_IC)
    ...
    void SetArenaFightID(uint32 arenaFightID);
...
private:
    ...
    */
    ...
    std::unordered_map<uint64, uint32> m_ArenaTeamIds[BG_TEAMS_COUNT];

    int32 m_ArenaTeamRatingChanges[BG_TEAMS_COUNT];
    uint32 m_ArenaTeamMMR[BG_TEAMS_COUNT];
    uint32 m_ArenaTeamRating[BG_TEAMS_COUNT];
    std::set<uint64> m_ArenaTeamLastDPSList[BG_TEAMS_COUNT]; // Only SoloQueue
