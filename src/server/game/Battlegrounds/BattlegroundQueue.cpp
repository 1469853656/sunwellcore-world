...
void GroupQueueInfo::SetLastDPSList(std::set<uint64> const& list)
{
    if (!list.empty())
    {
        LastDPSList.clear();
...
std::set<uint64> const& GroupQueueInfo::GetLastDPSList() const
{
    return LastDPSList;
}
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, PvPDifficultyEntry const* bracketEntry, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    ginfo->LeaderArenaTeamId         = arenateamid;
    ginfo->IsRated                   = isRated;
    ginfo->IsInvitedToBGInstanceGUID = 0;
    ginfo->JoinTime                  = World::GetGameTimeMS();
    ginfo->RemoveInviteTime          = 0;
    ginfo->teamId                    = leader->GetTeam(CF_DENY);
    ginfo->oTeam                     = ginfo->teamId;
    ginfo->ArenaTeamRating           = ArenaRating;
    ginfo->ArenaMatchmakerRating     = MatchmakerRating;
    ginfo->OpponentsTeamRating       = 0;
    ginfo->OpponentsMatchmakerRating = 0;
    ginfo->IsTwink                   = m_isTwinkQueue;
    ginfo->IsCrossfactionAllowed     = leader->IsCrossfactionBgAllowed();
    ginfo->GroupGUID                 = 0;
    ...
    if (!isRated && ginfo->teamId == TEAM_HORDE)
        index++;

    if (IsCrossfactionEnabledForBracket(bracketEntry) && m_arenaType == 0)
    ...
    if (grp)
    {
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            if (m_arenaType != ARENA_TYPE_NONE)
                ginfo->arenaTeams[member->GetGUID()] = member->GetArenaTeamId(ArenaTeam::GetSlotByType(m_arenaType));
        }
    }
    else
    {
        ...
        if (m_arenaType != ARENA_TYPE_NONE)
            ginfo->arenaTeams[leader->GetGUID()] = leader->GetArenaTeamId(ArenaTeam::GetSlotByType(m_arenaType));
    }

    //add GroupInfo to m_QueuedGroups
    m_QueuedGroups[bracketId][index].push_back(ginfo);

    bool battlegroundQueueAnnouncer = sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE);
    if (!isRated && !isPremade && battlegroundQueueAnnouncer)
        if (Battleground* bgt = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
        {
            ...
            if (IsCrossfactionEnabledForBracket(bracketEntry))
            {
                int qPlayers   = 0;
                int MinPlayers = bgt->GetMinPlayersPerTeam() * 2;
                ...
                {
                    ChatHandler(leader->GetSession()).PSendSysMessage("Queue status for %s (Lvl: %u to %u) Queued players: %u + specific queue.", bgName, q_min_level, q_max_level, qPlayers);
                }
                else
                {
                    ChatHandler(leader->GetSession()).PSendSysMessage("Queue status for %s (Lvl: %u to %u) Queued players: %u (Need at least %u more)", bgName, q_min_level, q_max_level, qPlayers, MinPlayers <= qPlayers ? 0 : (MinPlayers - qPlayers));
                }
            }
            else
            {
    ...
    if (isRated && sWorld->getBoolConfig(CONFIG_ARENA_WORLD_ANNOUNCER) && leader->CanSendArenaAnnounce())
        if (Battleground* bgt = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
        {
            if (bgt->isArena())
            {
                ...
                {
                    case ARENA_TYPE_2v2:
                        sWorld->SendPvPWorldText(false, LANG_ARENA_2V2_ANNOUNCE);
                        sWorld->SendGMText(LANG_ARENA_ANNOUNCE_GM, "2v2", leader->GetName().c_str());
...
GroupQueueInfo* BattlegroundQueue::AddSoloQueueArenaGroup(Player* leader, PvPDifficultyEntry const* bracketEntry, std::vector<SoloQueue> const& soloQueues, uint8 ArenaType, uint32 ArenaRating, uint32 MatchmakerRating, uint32 leaderArenaTeamId, std::set<uint64> const& list)
{
    // create new ginfo
    GroupQueueInfo* ginfo = new GroupQueueInfo;
    ginfo->BgTypeId = BATTLEGROUND_AA;
    ginfo->ArenaType = ArenaType;
    ginfo->LeaderArenaTeamId = leaderArenaTeamId;
    ginfo->IsRated = true;
    ginfo->IsInvitedToBGInstanceGUID = 0;
    ginfo->JoinTime = World::GetGameTimeMS();
    ginfo->RemoveInviteTime = 0;
    ...
    // pussywizard: store indices at which GroupQueueInfo is in m_QueuedGroups
    ginfo->_bracketId = bracketId;
    ginfo->_groupType = BG_QUEUE_PREMADE_ALLIANCE;

    //add players from group to ginfo
    for (SoloQueue const& soloQueue : soloQueues)
    {
        ASSERT(m_QueuedPlayers.count(soloQueue.ownerGUID) == 0);
        m_QueuedPlayers[soloQueue.ownerGUID] = ginfo;
        ginfo->Players.insert(soloQueue.ownerGUID);
        ginfo->arenaTeams[soloQueue.ownerGUID] = soloQueue.arenaTeamId;
    }

    //add GroupInfo to m_QueuedGroups
    m_QueuedGroups[bracketId][BG_QUEUE_PREMADE_ALLIANCE].push_back(ginfo);

    if (sWorld->getBoolConfig(CONFIG_ARENA_WORLD_ANNOUNCER) && leader->CanSendArenaAnnounce())
    {
        sWorld->SendPvPWorldText(false, LANG_ARENA_5V5_ANNOUNCE);
        sWorld->SendGMText(LANG_ARENA_ANNOUNCE_GM, "SoloQueue", leader->GetName().c_str());
        leader->AddArenaAnnounceCooldown();
    }
...
uint32 BattlegroundQueue::GetAverageQueueWaitTime(GroupQueueInfo* ginfo, BattlegroundBracketId bracketId) const
{
    // team_index: bg alliance - TEAM_ALLIANCE, bg horde - TEAM_HORDE, arena skirmish - TEAM_ALLIANCE, arena rated - TEAM_HORDE
    ...
    if (ginfo)
        bracketId = (BattlegroundBracketId)ginfo->_bracketId;
    ...
    if (m_WaitTimes[team_index][bracketId][COUNT_OF_PLAYERS_TO_AVERAGE_WAIT_TIME - 1])
    {
        ...
        for (uint32 i = 0; i < COUNT_OF_PLAYERS_TO_AVERAGE_WAIT_TIME; ++i)
            sum += m_WaitTimes[team_index][bracketId][i];
...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool sentToBg, uint32 playerQueueSlot)
{
    ...
    {
        auto itr = groupInfo->arenaTeams.find(guid);
        if (itr != groupInfo->arenaTeams.end())
        {
            if (ArenaTeam* at = sArenaTeamMgr->GetArenaTeamById(itr->second))
            {
                if (Player* player = ObjectAccessor::FindPlayer(guid))
                {
                    at->MemberLost(player, groupInfo->ArenaTeamRating, groupInfo->OpponentsMatchmakerRating);

                    if (groupInfo->ArenaType == ARENA_TYPE_SOLO_QUEUE && !player->HasAura(26013))
                    {
                    ...
                    at->OfflineMemberLost(guid, groupInfo->ArenaTeamRating, groupInfo->OpponentsMatchmakerRating);
    ...
    if (groupInfo->IsRated && !groupInfo->IsInvitedToBGInstanceGUID)
    {
        ...
        if (Player* plr = ObjectAccessor::FindPlayerInOrOutOfWorld(*(groupInfo->Players.begin())))
        {
            ...
            // if Solo Queue, add to preparation queue again
            if (bgQueueTypeId == BATTLEGROUND_QUEUE_SOLO_QUEUE)
            {
                WorldPacket data;
                sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_NONE, 0, 0, ARENA_TYPE_SOLO_QUEUE, TEAM_NEUTRAL);
                plr->GetSession()->SendPacket(&data);

                PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), plr->getLevel());
                if (!sSoloQueueMgr->AddToPreparationQueue(plr, bg, this, bracketEntry, groupInfo->JoinTime))
                {
                    WorldPacket data;
            ...
            {
                WorldPacket data;
                sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_NONE, 0, 0, groupInfo->ArenaType, TEAM_NEUTRAL);
                plr->GetSession()->SendPacket(&data);
            }
        }
...
bool BattlegroundQueue::IsCrossfactionEnabledForBracket(PvPDifficultyEntry const* /*bracketEntry*/)
{
    if (!sWorld->IsCrossfactionEnabled(Crossfaction::Battlegrounds) /* || m_bgTypeId == BATTLEGROUND_RB*/)
        return false;

    return true;
    //return bracketEntry->minLevel >= sCustomMgr->GetIntConfig(CONFIG_CROSSFACTION_BG_MINLVL) && bracketEntry->maxLevel <= sCustomMgr->GetIntConfig(CONFIG_CROSSFACTION_BG_MAXLVL);
}
...
void BattlegroundQueue::FillPlayersToBG(const int32 aliFree, const int32 hordeFree, BattlegroundBracketId bracket_id)
{
    ...
    if (!sBattlegroundMgr->isTesting())
        if (aliFree > hordeFree && m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE].empty() || hordeFree > aliFree && m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].empty())
            return;
...
void BattlegroundQueue::FillPlayersToBGWithSpecific(const int32 aliFree, const int32 hordeFree, BattlegroundBracketId thisBracketId, BattlegroundQueue* specificQueue, BattlegroundBracketId specificBracketId)
{
    ...
    // quick check if nothing we can do:
    if (!sBattlegroundMgr->isTesting())
        if (m_QueuedGroups[thisBracketId][BG_QUEUE_NORMAL_ALLIANCE].empty() && specificQueue->m_QueuedGroups[specificBracketId][BG_QUEUE_NORMAL_ALLIANCE].empty() || m_QueuedGroups[thisBracketId][BG_QUEUE_NORMAL_HORDE].empty() && specificQueue->m_QueuedGroups[specificBracketId][BG_QUEUE_NORMAL_HORDE].empty())
            return;
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* bgTemplate, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers)
{
    ...
    if (bgTemplate->GetBgTypeID() == BATTLEGROUND_RB)
    {
        ...
        if (!FillXPlayersToBGWithSpecific(specificTemplate, specificTemplate->GetMaxPlayersPerTeam(), specificTemplate->GetMaxPlayersPerTeam(), bracket_id, &specificQueue, BattlegroundBracketId(specificBracket->bracketId)))
    ...
    {
        if (bgTemplate->isArena() || !FillXPlayersToBG(bracket_id, bgTemplate, false))
...
struct BgEmptinessComp
{
    bool operator()(Battleground* const& bg1, Battleground* const& bg2) const { return ((float)bg1->GetMaxFreeSlots() / (float)bg1->GetMaxPlayersPerTeam()) > ((float)bg2->GetMaxFreeSlots() / (float)bg2->GetMaxPlayersPerTeam()); }
};
typedef std::set<Battleground*, BgEmptinessComp> BattlegroundNeedSet;

void BattlegroundQueue::BattlegroundQueueUpdate(BattlegroundBracketId bracket_id, uint8 actionMask, bool isRated /*= false*/)
{
    // if no players in queue - do nothing
    if (m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].empty() && m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].empty() && m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE].empty() && m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].empty() && m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].empty())
        return;
    ...
    if ((actionMask & 0x01) && !BattlegroundMgr::IsArenaType(m_bgTypeId))
    {
        ...
        for (auto&& it : bgList)
        {
            ...
            bool checkBattleground = !BattlegroundMgr::IsArenaType(bg->GetBgTypeID());
            checkBattleground &= bg->GetBgTypeID() == m_bgTypeId || m_bgTypeId == BATTLEGROUND_RB;
            checkBattleground &= bg->HasFreeSlots() && bg->GetMinLevel() <= bracketEntry->minLevel && bg->GetMaxLevel() >= bracketEntry->maxLevel;
            checkBattleground &= m_isTwinkQueue == bg->isTwink();
        ...
        for (Battleground* bg : bgsToCheck)
        {
            // call a function that fills whatever we can from normal queues
            if (bg_template->isArena() || !FillXPlayersToBG(bracket_id, bg, false, bracketEntry))
                FillPlayersToBG(bg->GetFreeSlotsForTeam(TEAM_ALLIANCE), bg->GetFreeSlotsForTeam(TEAM_HORDE), bracket_id);

            // invite players
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
                for (GroupQueueInfo* group : m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups)
                    BattlegroundMgr::InviteGroupToBG(group, bg, group->teamId);
        }
    }
    ...
    {
        MinPlayersPerTeam = sBattlegroundMgr->isArenaTesting() ? 1 : (m_arenaType == ARENA_TYPE_SOLO_QUEUE ? 3 : m_arenaType);
        MaxPlayersPerTeam = m_arenaType == ARENA_TYPE_SOLO_QUEUE ? 3 : m_arenaType;
    }
    ...
    if (bg_template->isBattleground())
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            if (bg->HasFreeSlots())
            {
                if (bg->isArena() || !FillXPlayersToBG(bracket_id, bg, false))
                    // call a function that fills whatever we can from normal queues
                    FillPlayersToBG(bg->GetFreeSlotsForTeam(TEAM_ALLIANCE), bg->GetFreeSlotsForTeam(TEAM_HORDE), bracket_id);
    ...
    {
        BattlegroundQueueTypeId queueTypeID = BattlegroundMgr::BGQueueTypeId(m_bgTypeId, m_arenaType, false);
        uint32 maxRatingDifference = sBattlegroundMgr->GetMaxRatingDifference();
        uint32 discardRatingTime = sBattlegroundMgr->GetRatingDiscardTimer(queueTypeID);
        ...
        for (GroupQueueInfo* groupQueueInfoA : m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE])
        {
            if (!groupQueueInfoA || groupQueueInfoA->IsInvitedToBGInstanceGUID)
                continue;

            uint32 arenaRatingTeamA = groupQueueInfoA->ArenaMatchmakerRating;
            uint32 arenaMinRatingTeamA = (arenaRatingTeamA <= maxRatingDifference) ? 0 : arenaRatingTeamA - maxRatingDifference;
            uint32 arenaMaxRatingTeamA = arenaRatingTeamA + maxRatingDifference;
            bool instantArenaTeamA = false;

            // If team is waiting longer than time set in config start increasing the range of teams it can fight
            if (groupQueueInfoA->JoinTime < discardTime)
            {
                uint32 timeOverDiscardTime = discardTime - groupQueueInfoA->JoinTime;
                uint32 timeInMinutes = 1 + (timeOverDiscardTime / 60000);

                // If team is waiting longer than 5 minutes connect it with any other team
                if (timeInMinutes > 5)
                    instantArenaTeamA = true;
                else
                {
                    arenaMinRatingTeamA = (arenaRatingTeamA <= (maxRatingDifference * timeInMinutes)) ? 0 : arenaRatingTeamA - (maxRatingDifference * timeInMinutes);
                    arenaMaxRatingTeamA = arenaRatingTeamA + (maxRatingDifference * timeInMinutes);
                }
            }

            std::array<std::vector<std::pair<uint32, GroupQueueInfo*>>, 2> groupsByDiffMMR;
            for (GroupQueueInfo* groupQueueInfoH : m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE])
            {
                if (!groupQueueInfoH || groupQueueInfoH->IsInvitedToBGInstanceGUID)
                    continue;

                if (groupQueueInfoA->LeaderArenaTeamId == groupQueueInfoH->LeaderArenaTeamId)
                    continue;

                uint32 arenaRatingTeamB = groupQueueInfoH->ArenaMatchmakerRating;
                uint32 mmrDiff = std::abs(static_cast<int32>(arenaRatingTeamA - arenaRatingTeamB));
                if (instantArenaTeamA)
                {
                    groupsByDiffMMR[0].emplace_back(mmrDiff, groupQueueInfoH);
                    continue;
                }

                uint32 arenaMinRatingTeamB = (arenaRatingTeamB <= maxRatingDifference) ? 0 : arenaRatingTeamB - maxRatingDifference;
                uint32 arenaMaxRatingTeamB = arenaRatingTeamB + maxRatingDifference;
                bool instantArenaTeamB = false;

                if (groupQueueInfoH->JoinTime < discardTime)
                {
                    uint32 timeOverDiscardTime = discardTime - groupQueueInfoH->JoinTime;
                    uint32 timeInMinutes = 1 + (timeOverDiscardTime / 60000);
                    if (timeInMinutes > 5)
                        instantArenaTeamB = true;
                    else
                    {
                        arenaMinRatingTeamB = (arenaRatingTeamB <= (maxRatingDifference * timeInMinutes)) ? 0 : arenaRatingTeamB - (maxRatingDifference * timeInMinutes);
                        arenaMaxRatingTeamB = arenaRatingTeamB + (maxRatingDifference * timeInMinutes);
                    }
                }

                if (instantArenaTeamB
                    || ((arenaMinRatingTeamB >= arenaMinRatingTeamA && arenaMinRatingTeamB <= arenaMaxRatingTeamA)
                    || (arenaMaxRatingTeamB >= arenaMinRatingTeamA && arenaMaxRatingTeamB <= arenaMaxRatingTeamA)
                    || (arenaMinRatingTeamA >= arenaMinRatingTeamB && arenaMinRatingTeamA <= arenaMaxRatingTeamB)
                    || (arenaMaxRatingTeamA >= arenaMinRatingTeamB && arenaMaxRatingTeamA <= arenaMaxRatingTeamB)))
                {
                    if (mmrDiff <= sWorld->getIntConfig(CONFIG_ARENA_MAX_MMR_DIFF))
                        groupsByDiffMMR[1].emplace_back(0, groupQueueInfoH);
            ...
            if (!groupsByDiffMMR[0].empty())
            {
                ...
                else
                {
                    ...
                    hTeam = groupsByDiffMMR[0].begin()->second;
                }
        ...
        if (aTeam && hTeam)
        {
            ...
            arena->SetArenaTeamMatchmakerRating(TEAM_ALLIANCE, aTeam->ArenaMatchmakerRating);
            arena->SetArenaTeamMatchmakerRating(TEAM_HORDE, hTeam->ArenaMatchmakerRating);

            arena->SetArenaTeamRating(TEAM_ALLIANCE, aTeam->ArenaTeamRating);
            arena->SetArenaTeamRating(TEAM_HORDE, hTeam->ArenaTeamRating);

            if (queueTypeID == BATTLEGROUND_QUEUE_SOLO_QUEUE)
            {
                arena->SetArenaTeamLastDPSList(TEAM_ALLIANCE, aTeam->GetLastDPSList());
                arena->SetArenaTeamLastDPSList(TEAM_HORDE, hTeam->GetLastDPSList());
            }
...
void BattlegroundQueue::StartInstantSoloQueueArena(OptimizedSoloQueue const& firstOptimizedSoloQueue, OptimizedSoloQueue const& secondOptimizedSoloQueue, PvPDifficultyEntry const* bracketEntry)
{
    Player* player1 = ObjectAccessor::FindPlayerInOrOutOfWorld(firstOptimizedSoloQueue.playerGUID);
    Player* player2 = ObjectAccessor::FindPlayerInOrOutOfWorld(secondOptimizedSoloQueue.playerGUID);

    // Players must be in world!
    ASSERT(player1);
    ASSERT(player2);

    Battleground* arena = sBattlegroundMgr->CreateNewBattleground(m_bgTypeId, bracketEntry->minLevel, bracketEntry->maxLevel, ARENA_TYPE_SOLO_QUEUE, true);
    ASSERT(arena);

    GroupQueueInfo* aTeam = AddSoloQueueArenaGroup(player1, bracketEntry, firstOptimizedSoloQueue.acceptedList, ARENA_TYPE_SOLO_QUEUE, firstOptimizedSoloQueue.averageRating,
        firstOptimizedSoloQueue.averageMMRRating, firstOptimizedSoloQueue.leaderArenaTeamID, firstOptimizedSoloQueue.GetLastDPSList());
    GroupQueueInfo* hTeam = AddSoloQueueArenaGroup(player2, bracketEntry, secondOptimizedSoloQueue.acceptedList, ARENA_TYPE_SOLO_QUEUE, secondOptimizedSoloQueue.averageRating,
        secondOptimizedSoloQueue.averageMMRRating, secondOptimizedSoloQueue.leaderArenaTeamID, secondOptimizedSoloQueue.GetLastDPSList());
    ...
    uint32 arenaFightID = sArenaTeamMgr->GetNextArenaLogId();
    arena->SetArenaFightID(arenaFightID);

    uint32 now = World::GetGameTimeMS();

    auto logStmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_SOLO_QUEUE_MATCHING_LOG);
    logStmt->setUInt32(0, arenaFightID);
    logStmt->setUInt32(1, aTeam->LeaderArenaTeamId);
    logStmt->setUInt32(2, hTeam->LeaderArenaTeamId);
    logStmt->setUInt32(3, aTeam->ArenaMatchmakerRating);
    logStmt->setUInt32(4, hTeam->ArenaMatchmakerRating);
    logStmt->setUInt32(5, now - aTeam->JoinTime);
    logStmt->setUInt32(6, now - hTeam->JoinTime);
    CharacterDatabase.Execute(logStmt);
