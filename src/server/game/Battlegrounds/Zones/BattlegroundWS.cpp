...
void BattlegroundWS::SetFlagPicker(uint64 guid, TeamId teamId)
{
    auto prevGuid        = _flagKeepers[teamId];
    _flagKeepers[teamId] = guid;

    if (sWorld->IsCrossfactionEnabled(Crossfaction::Battlegrounds))
    {
        Player* prevPicker = ObjectAccessor::FindPlayer(prevGuid);
        Player* currPicker = ObjectAccessor::FindPlayer(guid);

        if (prevPicker != nullptr)
            prevPicker->UpdateObjectVisibility(true);

        if (currPicker != nullptr)
            currPicker->UpdateObjectVisibility(true);
    }
}
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    AddPoints(player->GetBgTeam(), 1);
    SetFlagPicker(0, GetOtherTeamId(player->GetBgTeam()));
    UpdateFlagState(GetOtherTeamId(player->GetBgTeam()), BG_WS_FLAG_STATE_ON_BASE);
    if (player->GetBgTeam() == TEAM_ALLIANCE)
    {
    ...
    UpdateWorldState(player->GetBgTeam() == TEAM_ALLIANCE ? BG_WS_FLAG_CAPTURES_ALLIANCE : BG_WS_FLAG_CAPTURES_HORDE, GetTeamScore(player->GetBgTeam()));
    UpdatePlayerScore(player, SCORE_FLAG_CAPTURES, 1); // +1 flag captures
    _lastFlagCaptureTeam = player->GetBgTeam();
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
    ...
    if (player->GetBgTeam() == TEAM_HORDE && GetFlagState(TEAM_ALLIANCE) == BG_WS_FLAG_STATE_ON_BASE && BgObjects[BG_WS_OBJECT_A_FLAG] == gameObject->GetGUID())
    {
    ...
    if (player->GetBgTeam() == TEAM_ALLIANCE && GetFlagState(TEAM_HORDE) == BG_WS_FLAG_STATE_ON_BASE && BgObjects[BG_WS_OBJECT_H_FLAG] == gameObject->GetGUID())
    {
    ...
    if (GetFlagState(TEAM_HORDE) == BG_WS_FLAG_STATE_ON_GROUND && player->IsWithinDistInMap(gameObject, 10.0f) && gameObject->GetEntry() == BG_OBJECT_H_FLAG_GROUND_WS_ENTRY)
    {
        ...
        if (player->GetBgTeam() == TEAM_HORDE)
        {
...
{
    if (GetStatus() == STATUS_IN_PROGRESS)
        return sWorldSafeLocsStore.LookupEntry(player->GetBgTeam() == TEAM_ALLIANCE ? WS_GRAVEYARD_MAIN_ALLIANCE : WS_GRAVEYARD_MAIN_HORDE);
    else
        return sWorldSafeLocsStore.LookupEntry(player->GetBgTeam() == TEAM_ALLIANCE ? WS_GRAVEYARD_FLAGROOM_ALLIANCE : WS_GRAVEYARD_FLAGROOM_HORDE);
}
...
{
    if ((GetFlagPickerGUID(TEAM_ALLIANCE) == 0 && GetFlagState(TEAM_ALLIANCE) != BG_WS_FLAG_STATE_ON_GROUND) || (GetFlagPickerGUID(TEAM_HORDE) == 0 && GetFlagState(TEAM_HORDE) != BG_WS_FLAG_STATE_ON_GROUND) || _bgEvents.GetNextEventTime(BG_WS_EVENT_BOTH_FLAGS_KEPT10) > 0)
        return 0;
