    ...
    for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i)
    {
        ...
        if (nodePoint[i].needChange)
        {
            if (nodePoint[i].timer <= diff)
            {
                ...
                TeamId teamId = nodePoint[i].faction == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE;
...
void BattlegroundIC::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    if (unit->IsVehicle())
    {
        ...
        if (entry == NPC_DEMOLISHER || entry == NPC_SIEGE_ENGINE_H || entry == NPC_SIEGE_ENGINE_A || entry == NPC_GLAIVE_THROWER_A || entry == NPC_GLAIVE_THROWER_H || entry == NPC_CATAPULT)
...
WorldSafeLocsEntry const* BattlegroundIC::GetClosestGraveyard(Player* player)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES + 2; ++i)
    {
        if (!sWorldSafeLocsStore.LookupEntry(BG_IC_GraveyardIds[i]))
            continue;
    ...
    if (!nodes.empty())
    {
        ...
        for (uint8 index : nodes)
        {
            WorldSafeLocsEntry const* entry = sWorldSafeLocsStore.LookupEntry(BG_IC_GraveyardIds[index]);
            float curDist                   = player->GetDistance2d(entry->x, entry->y);
            if (mindist > curDist)
            {
                mindist    = curDist;
                good_entry = entry;
