...
{
    auto prevGuid   = _flagKeeperGUID;
    _flagKeeperGUID = guid;
...
void BattlegroundEY::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
    ...
    PSendMessageToAll(LANG_BG_EY_HAS_TAKEN_FLAG, player->GetTeam() == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE, nullptr, player->GetName().c_str());
...
void BattlegroundEY::FillInitialWorldStates(WorldPacket& data)
{
    ...
    data << uint32(MAGE_TOWER_ALLIANCE_CONTROL) << uint32(_capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(TEAM_ALLIANCE));
    data << uint32(MAGE_TOWER_HORDE_CONTROL) << uint32(_capturePointInfo[POINT_MAGE_TOWER].IsUnderControl(TEAM_HORDE));
