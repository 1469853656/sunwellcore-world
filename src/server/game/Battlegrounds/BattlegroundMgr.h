...
constexpr uint32 WS_ARENA_DISTRIBUTION_TIME{ 20001 }; // Custom worldstate
constexpr uint32 BATTLEGROUND_REPORT_AVAILABILITY_TIMER{ 150000 };
...
class BattlegroundMgr : public Sunwell::Singleton<BattlegroundMgr>
{
    friend class Sunwell::Singleton<BattlegroundMgr>;
...
public:
    ...
    Battleground* CreateNewBattleground(BattlegroundTypeId bgTypeId, uint32 minLevel, uint32 maxLevel, uint8 arenaType, bool isRated, bool isTwink = false);
    ...
    static BattlegroundQueueTypeId BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType, bool isTwink);
    ...
    uint32 GetMaxRatingDifference(uint32 mmrRating = 0) const;
    uint32 GetRatingDiscardTimer(BattlegroundQueueTypeId queueTypeID) const;
    ...
    const bool GetArenaListByType(std::vector<Battleground*>& arenaContainer, ArenaType type, bool onlyRated = true) const;
