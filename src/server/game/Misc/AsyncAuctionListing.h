...
{
public:
    AuctionListOwnerItemsDelayEvent(uint64 guid, uint32 listfrom, uint64 playerGuid, bool o)
        : m_guid(guid)
        , m_listfrom(listfrom)
...
public:
    AuctionListItemsDelayEvent(uint32 msTimer, uint64 playerguid, uint64 creatureguid, std::string searchedname, uint32 listfrom, uint8 levelmin, uint8 levelmax, uint8 usable, uint32 auctionSlotID, uint32 auctionMainCategory, uint32 auctionSubCategory, uint32 quality, uint8 getAll)
        ...
        , _searchedname(std::move(searchedname))
        , _listfrom(listfrom)
        , _levelmin(levelmin)
        , _levelmax(levelmax)
        , _usable(usable)
        , _auctionSlotID(auctionSlotID)
        , _auctionMainCategory(auctionMainCategory)
        , _auctionSubCategory(auctionSubCategory)
        , _quality(quality)
        , _getAll(getAll)
    {
    }
