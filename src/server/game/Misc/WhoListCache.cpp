...
void WhoListCacheMgr::Update()
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        m_whoOpcodeList.push_back(WhoListPlayerInfo(itr->second->GetTeam(CF_DENY), itr->second->GetSession()->GetSecurity(), itr->second->getLevel(), itr->second->getClass(), itr->second->getRace(), ((itr->second->IsSpectator() || itr->second->IsInDodgeMode()) ? 4395 /*Dalaran*/ : itr->second->GetZoneId()), itr->second->getGender(), wpname, wgname, aname, pname, gname));
    }
}
