...
void HostileReference::addThreat(float modThreat)
{
    ...
    {
        Unit* victimOwner = getTarget()->IsTotem() ? nullptr : getTarget()->GetCharmerOrOwner();
        if (victimOwner && victimOwner->IsAlive() && !getTarget()->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED))
