...
namespace BattlefieldConstants
{
    constexpr uint32 BATTLEFIELD_OBJECTIVE_UPDATE_INTERVAL = 1000;
}
...
struct BattlefieldPlayerInviteRequest
{
    uint64 m_playerGUID;
};

struct BattlefieldPlayerLeaveRequest
{
    uint64 m_playerGUID;
};

struct BattlefieldAcceptInvite
{
    enum InviteType
    {
        Queue,
    ...
    uint64      m_playerGUID = 0u;
    InviteType  m_type;
};

struct BattlefieldRemovePlayer
{
    uint64 m_playerGUID = 0u;
};

class Battlefield : public ZoneScript, public EventBus
{
...
public:
    ...
    bool CanBeInvitedToWar(Player* player, bool checkQueue = false) const;
    ...
    /// Return if battlefield is grouping
    bool IsGrouping() const { return m_StartGrouping; }
    ...
    bool   IsPlayerInWar(Player* player) const;
    bool   IsPlayerInvitedToWar(Player* player) const;
...
protected:
    ...
    void InvitePlayerToQueueOrWar(Player* player, bool checkQueue );

    void InvitePlayerToQueue(Player* player);
    void InvitePlayerToWar(Player* player);

    void RemovePlayerFromQueue(uint64_t playerGuid);

    void HandleBattlefieldAcceptInvite(const BattlefieldAcceptInvite& ev);
    void HandleBattlefieldRemovePlayer(const BattlefieldRemovePlayer& ev);

    void HandleBattlefieldPlayerInviteRequest(const BattlefieldPlayerInviteRequest& ev);
    void HandleBattlefieldPlayerLeaveRequest(const BattlefieldPlayerLeaveRequest& ev);
    ...
    mutable std::shared_mutex m_PlayersInQueueMutex;
