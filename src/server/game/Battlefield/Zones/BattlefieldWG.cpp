...
bool BattlefieldWG::SetupBattlefield()
{
    ...
    _map = sMapMgr->FindMap(m_MapId, 0);
    ...
    for (uint8 i = 0; i < WG_MAX_WORKSHOP; i++)
    {
        ...
        if (i < BATTLEFIELD_WG_WORKSHOP_SE)
...
void BattlefieldWG::OnGameObjectCreate(GameObject* go)
{
    ...
    for (Workshop::const_iterator itr = WorkshopsList.begin(); itr != WorkshopsList.end(); ++itr)
    {
        if (WGWorkshop* workshop = (*itr))
        {
            ...
            {
                WintergraspCapturePoint* capturePoint = new WintergraspCapturePoint(this, ((workshop->workshopId < BATTLEFIELD_WG_WORKSHOP_SE) ? GetAttackerTeam() : GetDefenderTeam()));
...
void BattlefieldWG::OnPlayerEnterZone(Player* player)
{
    ...
    // Kick player from WG if he joined other faction, prevent exploiting
    if (sWorld->IsCrossfactionEnabled(Crossfaction::Group))
        if (Group const* plrGroup = player->GetGroup())
            if (Player const* leader = ObjectAccessor::GetPlayer(*player, plrGroup->GetLeaderGUID()))
                if (leader->GetTeam(CF_DENY) != player->GetTeam(CF_DENY))
...
{
    for (uint8 team = 0; team < 2; team++)
        for (GuidSet::iterator itr = m_playersInZone[team].begin(); itr != m_playersInZone[team].end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
                SendInitWorldStatesTo(player);
}
    ...
    if (team == GetAttackerTeam())
    {
        ...
        for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
            {
                ...
                player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_LEANING_TOWER_ACHIEVEMENT, 0, nullptr);
            }
    ...
    if (newStack)
    {
        ...
        for (GuidSet::const_iterator itr = m_vehicles[team].begin(); itr != m_vehicles[team].end(); ++itr)
        {
            if (Creature* creature = GetCreature(*itr))
            {
                if (creature->IsVehicle())
                {
                    creature->SetAuraStack(SPELL_TENACITY_VEHICLE, creature, newStackAbs);
                    ...
                    if (creature->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_ALLIANCE || creature->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_HORDE)
                    {
                        // Add Tenacity to Wintergrasp Siege Turret
