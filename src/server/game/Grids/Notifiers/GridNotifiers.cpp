...
{
    auto visitor = [this](Player* source) {
        vis_guids.erase(source->GetGUID());
        i_player.UpdateVisibilityOf(source, i_data, i_visibleNow);
    };

    if (BattlegroundNotifier::VisitPlayers(&i_player, visitor))
        return;

    for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
        visitor(iter->GetSource());
}
...
{
    auto visitor = [this](Player* player) {
        if (player == &i_object)
            return;
        ...
        if (player->HasSharedVision())
            for (SharedVisionList::const_iterator i = player->GetSharedVisionList().begin(); i != player->GetSharedVisionList().end(); ++i)
                if ((*i)->m_seer == player)
                    (*i)->UpdateVisibilityOf(&i_object);
    };

    if (Player* player = i_object.ToPlayer())
    {
        if (BattlegroundNotifier::VisitPlayers(player, visitor))
            return;
    }
