...
struct BattlegroundNotifier
{
    template<typename Visitor>
    bool VisitPlayers(Player* source, Visitor&& visitor)
    {
        if (m_bgPlayersVisited)
            return true;
        ...
        auto bg = source->GetBattleground();
        if (bg == nullptr)
            return false;

        m_bgPlayersVisited = true;

        for (auto& [guid, player] : bg->GetPlayers())
            visitor(player);

        return true;
    }

    bool m_bgPlayersVisited = false;
};

struct VisibleNotifier : public BattlegroundNotifier
{
    Player& i_player;
...
struct VisibleChangesNotifier : public BattlegroundNotifier
{
...
struct PlayerRelocationNotifier : public VisibleNotifier
{
    PlayerRelocationNotifier(Player& player)
        : VisibleNotifier(player, false)
    {
    }

    template<class T>
    void Visit(GridRefManager<T>& m) { VisibleNotifier::Visit(m); }
    void Visit(PlayerMapType&);
};
...
public:
    ...
    bool operator()(GameObject* go)
    {
        if (go->isSpawned() && go->GetEntry() == i_entry && i_obj.IsWithinDistInMap(go, i_range))
        {
            i_range = i_obj.GetDistance(go); // use found GO range as new range limit for next check
            return true;
        }
...
{
public:
    MostHPMissingInRange(Unit const* obj, float range, uint32 hp, bool _excludePlayers)
        : i_obj(obj)
        , i_range(range)
        , i_hp(hp)
        , i_excludePlayers(_excludePlayers)
    {
    }

    bool operator()(Unit* u)
    {
        if (i_excludePlayers && u->IsPlayer())
            return false;
...
public:
    ...
    bool operator()(Unit* u)
    {
        if (u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) && (i_funit->GetTypeId() != TYPEID_UNIT || !i_funit->ToCreature()->IsAvoidingAOE())) // pussywizard
            return true;
        else
            return false;
    }
...
{
public:
    NearestUnfriendlyNoTotemUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range)
        : i_obj(obj)
        , i_funit(funit)
        , i_range(range)
    {
...
class NearestUnfriendlyInCombatWithOwnerNoTotemUnitInObjectRangeCheck
{
public:
    NearestUnfriendlyInCombatWithOwnerNoTotemUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, Unit const* owner, float range)
        : i_obj(obj)
        , i_funit(funit)
        , i_owner(owner)
        , i_range(range)
...
public:
    ...
    bool operator()(Unit* u)
    {
        if (u->isTargetableForAttack(true, i_funit) && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) && i_funit->CanSeeOrDetect(u))
        {
...
public:
    AnyAoETargetUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range, bool _targetPlayers = false)
        : _onlyTargetPlayers(_targetPlayers)
        , i_obj(obj)
        , i_funit(funit)
        , _spellInfo(nullptr)
        , i_range(range)
...
{
public:
    AllGameObjectsRange(const WorldObject* object, float maxRange)
        : m_pObject(object)
        , m_fRange(maxRange)
    {
    }
...
class AllCreaturesRange
{
public:
    AllCreaturesRange(const WorldObject* object, float maxRange)
        : m_pObject(object)
        , m_fRange(maxRange)
    {
    }
...
{
public:
    ObjectGUIDCheck(uint64 GUID, bool equals)
        : _GUID(GUID)
        , _equals(equals)
    {
    }
