...
template<>
void ObjectGridLoader::SetObjectCell(GameObject* obj, CellCoord const& cellCoord)
{
    Cell cell(cellCoord);
    obj->SetCurrentCell(cell);
}
...
void AddObjectHelper(CellCoord& cell, CreatureMapType& m, uint32& count, Map* map, Creature* obj)
{
    ...
    // Call only for idle creatures
    if (obj->GetDefaultMovementType() == IDLE_MOTION_TYPE)
    {
        // call MoveInLineOfSight for nearby creatures
        Trinity::AIRelocationNotifier notifier(*obj);
        obj->VisitNearbyObject(VisibilityConstants::AI_RELOCATION_NOTIFIER_RANGE, notifier);
    }
