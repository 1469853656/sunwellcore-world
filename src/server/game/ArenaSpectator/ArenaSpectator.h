...
template<class T>
inline void SendCommand(T* o, const char* format, ...) ATTR_PRINTF(2, 3);
...
template<class T>
inline void SendCommand_String(T* p, uint64 targetGUID, const char* prefix, const std::string& c);
template<class T>
...
void SendCommand_String(T* o, uint64 targetGUID, const char* prefix, const char* c)
{
    if (!IS_PLAYER_GUID(targetGUID))
        return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=%s;", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, c);
}
...
void SendCommand_UInt32Value(T* o, uint64 targetGUID, const char* prefix, uint32 t)
{
    if (!IS_PLAYER_GUID(targetGUID))
        return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=%u;", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, t);
}
...
void SendCommand_GUID(T* o, uint64 targetGUID, const char* prefix, uint64 t)
{
    if (!IS_PLAYER_GUID(targetGUID))
        return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=0x" XI64FMTD ";", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, t);
}
...
void SendCommand_Spell(T* o, uint64 targetGUID, const char* prefix, uint32 id, int32 casttime)
{
    if (!IS_PLAYER_GUID(targetGUID))
        return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=%u,%i;", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, id, casttime);
}
...
void SendCommand_Cooldown(T* o, uint64 targetGUID, const char* prefix, uint32 id, uint32 dur, uint32 maxdur)
{
    ...
    if (const SpellInfo* si = sSpellMgr->GetSpellInfo(id))
        if (si->SpellIconID == 1)
            return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=%u,%u,%u;", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, id, dur, maxdur);
}
...
void SendCommand_Aura(T* o, uint64 targetGUID, const char* prefix, uint64 caster, uint32 id, bool isDebuff, uint32 dispel, int32 dur, int32 maxdur, uint32 stack, bool remove)
{
    if (!IS_PLAYER_GUID(targetGUID))
        return;
    SendCommand(o, "%s0x" XI64FMTD ";%s=%u,%u,%i,%i,%u,%u,%u,0x" XI64FMTD ";", SPECTATOR_ADDON_PREFIX, targetGUID, prefix, remove ? 1 : 0, stack, dur, maxdur, id, dispel, isDebuff ? 1 : 0, caster);
}
...
void HandleResetCommand(Player* p)
{
    ...
    for (Battleground::BattlegroundPlayerMap::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
    {
        ...
        SendCommand_UInt32Value(p, itr->first, "TEM", plr->GetBgTeam() == TEAM_ALLIANCE ? ALLIANCE : HORDE);
...
bool ShouldSendAura(Aura* aura, uint8 effMask, uint64 targetGUID, bool remove)
{
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (effMask & (1 << i))
        {
            ...
            if (aura->GetEffect(i)->GetAmount() && (aura->GetSpellInfo()->IsPositive() || targetGUID != aura->GetCasterGUID()) || at == SPELL_AURA_MECHANIC_IMMUNITY || at == SPELL_AURA_EFFECT_IMMUNITY || at == SPELL_AURA_STATE_IMMUNITY || at == SPELL_AURA_SCHOOL_IMMUNITY || at == SPELL_AURA_DISPEL_IMMUNITY)
                return true;
        }
    }
    return false;
