...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    do
    {
        ...
        AddLocaleString(title, locale, data.SubName);

    } while (result->NextRow());

    sLog->outString(">> Loaded " SIZEFMTD " creature locale strings in %u ms", _creatureLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT entry, difficulty_entry_1, difficulty_entry_2, difficulty_entry_3, KillCredit1, KillCredit2, modelid1, modelid2, modelid3, "
                                             ...
                                             " questItem6, movementId, RegenHealth, mechanic_immune_mask, flags_extra, ScriptName, MythicTemplateId, PresetGossipFlags, detection_range, Experience_mod "
...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.MythicTemplateId   = fields[84].GetUInt32();
    creatureTemplate.PresetGossipFlags  = fields[85].GetUInt64();
    creatureTemplate.detection_range    = fields[86].GetFloat();
    creatureTemplate.Experience_mod     = fields[87].GetFloat();
}
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    do
    {
        ...
        {
            SpellInfo const* AdditionalSpellInfo = sSpellMgr->GetSpellInfo(uint32(std::atol(*itr)));
            if (!AdditionalSpellInfo)
            {
                sLog->outErrorDb("Creature (Entry: %u) has wrong spell %u defined in `auras` field in `creature_template_addon`.", entry, uint32(std::atol(*itr)));
                continue;
            }
            creatureAddon.auras[i++] = uint32(std::atol(*itr));
        }
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    do
    {
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            if (!AdditionalSpellInfo)
            {
                sLog->outErrorDb("Creature (GUID: %u) has wrong spell %u defined in `auras` field in `creature_addon`.", guid, uint32(std::atol(*itr)));
                continue;
            }
            creatureAddon.auras[i++] = uint32(std::atol(*itr));
        }
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_EQUIPMENT_ITEMS; ++i)
        {
            ...
            if (dbcItem->InventoryType != INVTYPE_WEAPON && dbcItem->InventoryType != INVTYPE_SHIELD && dbcItem->InventoryType != INVTYPE_RANGED && dbcItem->InventoryType != INVTYPE_2HWEAPON && dbcItem->InventoryType != INVTYPE_WEAPONMAINHAND && dbcItem->InventoryType != INVTYPE_WEAPONOFFHAND && dbcItem->InventoryType != INVTYPE_HOLDABLE && dbcItem->InventoryType != INVTYPE_THROWN && dbcItem->InventoryType != INVTYPE_RANGEDRIGHT)
            {
...
bool ObjectMgr::SetCreatureLinkedRespawn(uint32 guidLow, uint32 linkedGuidLow)
{
    ...
    if (!linkedGuidLow) // we're removing the linking
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_CRELINKED_RESPAWN);
    ...
    _linkedRespawnStore[guid] = linkedGuid;
    auto stmt   = WorldDatabase.GetPreparedStatement(WORLD_REP_CREATURE_LINKED_RESPAWN);
    stmt->setUInt32(0, guidLow);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (float(tmp.QuestExplored.Distance) > VisibilityConstants::DEFAULT_VISIBILITY_DISTANCE)
                {
                ...
                if (tmp.QuestExplored.Distance && float(tmp.QuestExplored.Distance) > VisibilityConstants::DEFAULT_VISIBILITY_DISTANCE)
                {
                    sLog->outErrorDb("Table `%s` has too large distance (%u) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, max distance is %f or 0 for disable distance check",
                        tableName.c_str(), tmp.QuestExplored.Distance, tmp.id, VisibilityConstants::DEFAULT_VISIBILITY_DISTANCE);
                    continue;
                }
...
void ObjectMgr::LoadWaypointScripts()
{
    ...
    auto stmt    = WorldDatabase.GetPreparedStatement(WORLD_SEL_WAYPOINT_DATA_ACTION);
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    do
    {
        ...
        if (creditType == ENCOUNTER_CREDIT_KILL_CREATURE)
            _dungeonEncounterEntryToEncounterMaskMap[creditEntry] = 1 << dungeonEncounter->encounterIndex;
    } while (result->NextRow());
...
void ObjectMgr::ReturnOrDeleteOldMails(bool serverUp)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_EXPIRED_MAIL);
    ...
    do
    {
        Field* fields     = result->Fetch();
        Mail mail;
        mail.messageID    = fields[0].GetUInt32();
        mail.messageType  = fields[1].GetUInt8();
        mail.sender       = fields[2].GetUInt32();
        mail.receiver     = fields[3].GetUInt32();
        bool has_items    = fields[4].GetBool();
        mail.expire_time  = time_t(fields[5].GetUInt32());
        mail.deliver_time = 0;
        mail.COD          = fields[6].GetUInt32();
        mail.checked      = fields[7].GetUInt8();
        mail.mailTemplateId = fields[8].GetInt16();

        if (serverUp && ObjectAccessor::FindPlayerInOrOutOfWorld(MAKE_NEW_GUID(mail.receiver, 0, HIGHGUID_PLAYER)))
            continue;
        ...
        if (has_items)
        {
            ...
            mail.items.swap(itemsCache[mail.messageID]);

            // don't return if: is mail from non-player, or sent to self, or already returned, or read and isn't COD
            if (mail.messageType != MAIL_NORMAL || mail.receiver == mail.sender || (mail.checked & (MAIL_CHECK_MASK_COD_PAYMENT | MAIL_CHECK_MASK_RETURNED)) ||
                ((mail.checked & MAIL_CHECK_MASK_READ) && !mail.COD))
            {
                for (MailItemInfoVec::iterator itr2 = mail.items.begin(); itr2 != mail.items.end(); ++itr2)
                {
            ...
            else
            {
                ...
                sWorld->UpdateGlobalPlayerMails(mail.sender, 1);
                sWorld->UpdateGlobalPlayerMails(mail.receiver, -1);
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveyard(float x, float y, float z, uint32 MapId, TeamId teamId)
{
    ...
    if (!zoneId)
    {
        // Special case for Icecrown Citadel after falling from Lich King platform
        if (MapId == 631)
            zoneId = 4812;
        else if (z > -500)
        {
...
bool ObjectMgr::AddGraveyardLink(uint32 id, uint32 zoneId, TeamId teamId, bool persist /*= true*/)
{
    ...
    if (persist)
    {
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_GRAVEYARD_ZONE);
...
void ObjectMgr::RemoveGraveyardLink(uint32 id, uint32 zoneId, TeamId teamId, bool persist /*= false*/)
{
    ...
    if (persist)
    {
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_GRAVEYARD_ZONE);
...
uint32 ObjectMgr::GenerateMailID()
{
    ...
    std::lock_guard lg(_mailIdMutex);
    return _mailId++;
}
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    sLog->outString(">> Loaded " SIZEFMTD " gameobject locale strings in %u ms", _gameObjectLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
{
    std::lock_guard lg(_hiPetNumberMutex);
    return ++_hiPetNumber;
}
...
void ObjectMgr::LoadCreatureOutfits()
{
    ...
    if (!result)
    {
        sLog->outError(">> Loaded 0 creature outfits. DB table `creature_template_outfits` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError(">> Outfit entry %u in `creature_template_outfits` has too low entry (entry <= %u). Ignoring.", entry, CreatureOutfit::max_real_modelid);
            continue;
        }
        ...
        {
            sLog->outError(">> Outfit entry %u in `creature_template_outfits` has incorrect npcsoundsid (%u). Using 0.", entry, co->npcsoundsid);
            co->npcsoundsid = 0;
        }
        ...
        if (!rEntry)
        {
            sLog->outError(">> Outfit entry %u in `creature_template_outfits` has incorrect race (%u).", entry, uint32(co->race));
            continue;
        }
        ...
        if (!cEntry)
        {
            sLog->outError(">> Outfit entry %u in `creature_template_outfits` has incorrect class (%u).", entry, uint32(co->Class));
            continue;
        }
        ...
        switch (co->gender)
        {
            ...
            default:
                sLog->outError(">> Outfit entry %u in `creature_template_outfits` has invalid gender %u", entry, uint32(co->gender));
                continue;
        }
        ...
        for (EquipmentSlots slot : CreatureOutfit::item_slots)
        {
            ...
            if (displayInfo > 0) // entry
            {
                ...
                {
                    sLog->outError(">> Outfit entry %u in `creature_template_outfits` has invalid item entry: %u. Ignoring.", entry, item_entry);
    ...
    sLog->outString(">> Loaded %u creature outfits in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::AddGameTele(GameTele& tele)
{
    ...
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_GAME_TELE);
...
bool ObjectMgr::DeleteGameTele(const std::string& name)
{
    ...
    for (GameTeleContainer::iterator itr = _gameTeleStore.begin(); itr != _gameTeleStore.end(); ++itr)
    {
        if (itr->second.wnameLow == wname)
        {
            auto stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_GAME_TELE);
...
int ObjectMgr::LoadReferenceVendor(int32 vendor, int32 item, std::set<uint32>* skip_vendors)
{
    ...
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_NPC_VENDOR_REF);
...
constexpr std::array<std::pair<uint32, uint32>, 10> ItemIDQuestIDPairs{ {
    { 1015, 90 },
    { 3476, 496 },
    { 3477, 496 },
    { 23676, 9454 },
...
bool ObjectMgr::IsQuestItem(ItemTemplate const* itemProto, Player const* player)
{
    if (!itemProto)
        return false;

    if (itemProto->Class == 12 && itemProto->Bonding == 4)
        return true;

    for (auto const& [itemID, questID] : ItemIDQuestIDPairs)
    {
        if (itemProto->ItemId == itemID)
        {
            if (!questID)
                return true;
...
void ObjectMgr::AddVendorItem(uint32 entry, uint32 item, int32 maxcount, uint32 incrtime, uint32 extendedCost, bool persist /*= true*/)
{
    ...
    if (persist)
    {
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_NPC_VENDOR);
...
bool ObjectMgr::RemoveVendorItem(uint32 entry, uint32 item, bool persist /*= true*/)
{
    ...
    if (persist)
    {
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_NPC_VENDOR);
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 item_id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    {
        if (player)
            ChatHandler(player->GetSession()).SendSysMessage("MaxCount == 0 but IncrTime<>= 0");
        else
...
void ObjectMgr::RequestItemDestroy(Item* item)
{
    if (d_debugItemDestroy)
    {
        ...
        {
            sLog->outBagCrash("Item: [%p, %d], owner: [%p, " UI64FMTD "], IsBag: %d, BagSlot: `%d`, Slot: `%d`, Pos: `%d`, ref count: `%d`!", this, item->GetGUIDLow(), item->GetOwner(), item->GetOwnerGUID(), item->IsBag(), item->GetBagSlot(), item->GetSlot(), item->GetPos(), item->m_refCounter.load());
        }
    }
...
std::string ObjectMgr::FormatItemLink(uint32 itemID, std::array<uint32, 3> const& gemIDs /* = {} */, uint32 enchantmentID /* = 0 */)
{
    if (ItemTemplate const* temp = sObjectMgr->GetItemTemplate(itemID))
        return fmt::format("|c{:x}|Hitem:{}:{}:{}:{}:{}:0:0:0:0:0|h[{}]|h|r", ItemQualityColors[temp->Quality], itemID, enchantmentID, gemIDs[0], gemIDs[1], gemIDs[2], temp->Name1);

    return "";
}

std::string ObjectMgr::FormatSpellLink(uint32 spellID)
{
    if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellID))
        return fmt::format("|cff71d5ff|Hspell:{}|h[{}]|h|r", spellID, spellInfo->SpellName[0]);

    return "";
}

std::string ObjectMgr::FormatGlyphLink(uint32 glyphID, uint32 glyphIndex)
{
    if (GlyphPropertiesEntry const* glyphProperties = sGlyphPropertiesStore.LookupEntry(glyphID))
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(glyphProperties->SpellId))
            return fmt::format("|cff66bbff|Hglyph:{}:{}|h[{}]|h|r", glyphIndex, glyphProperties->Id, spellInfo->SpellName[0]);

    return "";
}

uint32 ObjectMgr::GetDungeonEncounterIndexMaskFromEntry(uint32 entry) const
{
    auto const it = _dungeonEncounterEntryToEncounterMaskMap.find(entry);
    if (it == _dungeonEncounterEntryToEncounterMaskMap.end())
        return 0;
