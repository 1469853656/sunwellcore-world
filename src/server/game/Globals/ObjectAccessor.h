...
{
public:
    DelayedCorpseAction(Corpse* corpse, uint8 action, uint32 mapId, uint32 instanceId)
        : _corpse(corpse)
        , _action(action)
        , _mapId(mapId)
...
class ObjectAccessor : public Sunwell::Singleton<ObjectAccessor>
{
    friend class Sunwell::Singleton<ObjectAccessor>;
...
public:
    ...
    template<class T>
    static T* GetObjectInOrOutOfWorld(uint64 guid, T* /*typeSpecifier*/)
    {
    ...
    template<class T>
    static T* GetObjectInWorld(uint64 guid, T* /*typeSpecifier*/)
    {
        return HashMapHolder<T>::Find(guid);
    }
    ...
    template<class T>
    static T* GetObjectInMap(uint64 guid, Map* map, T* /*typeSpecifier*/)
    {
        ASSERT(map);
    ...
    {
        std::lock_guard lg(i_objectLock);
...
private:
    ...
    std::vector<DelayedCorpseAction> _delayedCorpseActions;
    mutable std::mutex _delayedCorpseActionsLock;
};
