...
ObjectAccessor::ObjectAccessor() = default;
...
{
    return u.GetMap()->GetPlayer(guid);
}
...
{
    std::shared_lock lock(i_corpseLock);
...
void ObjectAccessor::RemoveCorpse(Corpse* corpse, bool final)
{
    ...
    if (!final)
    {
        std::unique_lock lock(i_corpseLock);
...
Corpse* ObjectAccessor::ConvertCorpseForPlayer(uint64 player_guid, bool insignia /*=false*/)
{
    ...
    if (map && corpse->IsPositionValid() && inWorld && (insignia || (map->IsBattlegroundOrArena() ? sWorld->getBoolConfig(CONFIG_DEATH_BONES_BG_OR_ARENA) : sWorld->getBoolConfig(CONFIG_DEATH_BONES_WORLD))) && !map->IsRemovalGrid(corpse->GetPositionX(), corpse->GetPositionY()))
    {
...
{
    std::lock_guard lg(_delayedCorpseActionsLock);
    _delayedCorpseActions.push_back(DelayedCorpseAction(corpse, action, mapId, instanceId));
}

void ObjectAccessor::ProcessDelayedCorpseActions()
{
    decltype(_delayedCorpseActions) delayedCorpseActions;
    {
        std::lock_guard lg(_delayedCorpseActionsLock);
        delayedCorpseActions.swap(_delayedCorpseActions);
    }

    for (DelayedCorpseAction const& action : delayedCorpseActions)
    {
        switch (action._action)
        {
            case 0: // remove corpse
                RemoveCorpse(action._corpse, true);
                break;
            case 1: // add bones
                if (Map* map = sMapMgr->FindMap(action._mapId, action._instanceId))
                {
                    if (!map->IsRemovalGrid(action._corpse->GetPositionX(), action._corpse->GetPositionY()))
                    {
                        action._corpse->SetMap(map);
                        map->AddToMap(action._corpse);
                    }
                }
...
template<class T>
std::unordered_map<uint64, T*> HashMapHolder<T>::m_objectMap;
template<class T>
