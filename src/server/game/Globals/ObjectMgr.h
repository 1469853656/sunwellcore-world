...
struct TempSummonGroupKey
{
    TempSummonGroupKey(uint32 summonerEntry, SummonerType summonerType, uint8 group)
        : _summonerEntry(summonerEntry)
        , _summonerType(summonerType)
        , _summonGroup(group)
    {
    }
...
struct QuestPOI
{
    ...
    QuestPOI(uint32 id, int32 objIndex, uint32 mapId, uint32 areaId, uint32 floorId, uint32 unk3, uint32 unk4)
        : Id(id)
...
class ObjectMgr : public Sunwell::Singleton<ObjectMgr>
{
    friend class PlayerDumpReader;
    friend class Sunwell::Singleton<ObjectMgr>;
...
public:
    ...
    static bool IsQuestItem(ItemTemplate const* itemProto, Player const* player = nullptr);
    ...
    static std::string FormatItemLink(uint32 itemID, std::array<uint32, 3> const& gemIDs = {}, uint32 enchantmentID = 0);
    static std::string FormatSpellLink(uint32 spellID);
    static std::string FormatGlyphLink(uint32 glyphID, uint32 glyphIndex);

    uint32 GetDungeonEncounterIndexMaskFromEntry(uint32 entry) const;
...
private:
    ...
    std::unordered_map<uint32, uint32> _dungeonEncounterEntryToEncounterMaskMap;
