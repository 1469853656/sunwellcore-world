...
void MapManager::Update(uint32 diff)
{
    ...
    Threading::BackgroundGroupUpdater updater;
    ...
    bool const _alwaysUpdateInstanceable = sWorld->getBoolConfig(CONFIG_UPDATE_INSTANCE_BATTLEGROUNDS_EVERY_UPDATE);
    {
        ...
        for (Map* map : maps)
        {
            ...
            else
            {
                ...
                l_diff = uint32(fullUpdate ? i_timer[mapUpdateStep].GetCurrent() : 0);
            }

           updater.Run([&, diff, l_diff, map]
           {
                map->Update(l_diff, diff);
           });
        }
    }
