...
MapInstanced::MapInstanced(uint32 id)
    : Map(id, 0, DUNGEON_DIFFICULTY_NORMAL)
{
...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
    ...
    else
    {
        ...
        uint32 destInstId   = sInstanceSaveMgr->PlayerGetDestinationInstanceId(player, GetId(), realdiff);
         ...
         bool _createMythic = player->GetGroup() && player->GetGroup()->IsMythicGroup() && sMythicMgr.IsMythicAvailableForMap(mapId);
        //! You can enter mythic on normal only!
        if (realdiff > DUNGEON_DIFFICULTY_NORMAL)
            _createMythic = false;
        ...
        if (destInstId)
        {
            ...
            if (!map)
                map = CreateInstance(destInstId, pSave, realdiff, _createMythic);
            else if ((mapId == 631 || mapId == 724) && !map->HavePlayers() && map->GetDifficulty() != realdiff)
            {
                ...
                for (InstancedMaps::iterator i = m_InstancedMaps.begin(); i != m_InstancedMaps.end(); ++i)
                    ...
                    {
                        DestroyInstance(i);
                        map = CreateInstance(destInstId, pSave, realdiff, 0);
                        break;
                    }
            }
        }
        else
        {
            ...
            map             = CreateInstance(newInstanceId, nullptr, diff, _createMythic);
        }
    }
...
InstanceMap* MapInstanced::CreateInstance(uint32 InstanceId, InstanceSave* save, Difficulty difficulty, bool createMythic)
{
    // load/create a map
    ...
    if (save)
        map->CreateInstanceScript(true, save->GetInstanceData(), save->GetCompletedEncounterMask(), false, save->GetCustomDifficulty());
    else
        map->CreateInstanceScript(false, "", 0, createMythic);
...
bool MapInstanced::DestroyInstance(InstancedMaps::iterator& itr)
{
    ...
    sAchievementMgr->OnInstanceDestroyed(itr->second->GetInstanceId());
