...
enum class CustomDifficulty : uint32
{
    Disabled            = 0,
    Ascended10          = 1,
};
...
public:
    ...
    bool IsDungeonOrRaid() const { return IsRaid() || IsDungeon(); }
    ...
    TempSummon* SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties = nullptr, uint32 duration = 0, Unit* summoner = nullptr, uint32 spellId = 0, uint32 vehId = 0, uint32 phaseMask = 0);
    ...
    GameObject* GetGameObject(uint64 guid) const;
    Transport* GetTransport(uint64 guid) const;
    DynamicObject* GetDynamicObject(uint64 guid) const;
    Pet* GetPet(uint64 guid) const;
    Corpse* GetCorpse(uint64 guid) const;
    Unit* GetUnit(uint64 guid) const;

    void InsertToObjectsStore(Creature* object);
    void InsertToObjectsStore(GameObject* object);
    void InsertToObjectsStore(DynamicObject* object);
    void InsertToObjectsStore(Pet* object);
    void InsertToObjectsStore(Player* object);
    void InsertToObjectsStore(Corpse* object);

    void RemoveFromObjectsStore(Creature const* object);
    void RemoveFromObjectsStore(GameObject const* object);
    void RemoveFromObjectsStore(DynamicObject const* object);
    void RemoveFromObjectsStore(Pet const* object);
    void RemoveFromObjectsStore(Player const* object);
    void RemoveFromObjectsStore(Corpse const* object);

    void DoOnAllCreatures(std::function<void(Creature*)> func) const;

    void ScheduleDelayedGameEventAIHook(uint16 eventID, bool activated);
    void ProcessGameEventRequests();
    ...
    const BattlegroundMap* ToBattlegroundMap() const
    {
        if (IsBattlegroundOrArena()) return reinterpret_cast<BattlegroundMap const*>(this);
        return nullptr;
    }
    ...
    float GetGameObjectFloor(uint32 phasemask, float x, float y, float z, float maxSearchDist = 50.0f) const;
    bool isInLineOfSight(float x1, float y1, float z1, float x2, float y2, float z2, uint32 phasemask, LineOfSightChecks checks = LINEOFSIGHT_ALL_CHECKS) const;
    ...
    void ApplyDynamicModeRespawnScaling(WorldObject const* obj, uint32* respawnDelay = nullptr, uint32* corpseDelay = nullptr) const;
...
protected:
    ...
    mutable std::mutex _dynamicTreeLock;
...
private:
    ...
    using MapObjectContainer = std::unordered_map<uint64 /*guid*/, MapObject>;

    MapObjectContainer<Creature*> _creatureStore;
    MapObjectContainer<GameObject*> _gameobjectStore;
    MapObjectContainer<Corpse*> _corpseStore;
    MapObjectContainer<DynamicObject*> _dynamicObjectStore;
    MapObjectContainer<Pet*> _petStore;
    MapObjectContainer<Player*> _playerStore;

    ZoneDynamicInfoMap _zoneDynamicInfo;
    uint32 _defaultLight;

    std::vector<std::pair<uint16, bool>> _delayedGameEventAIHookRequestData;
};
...
public:
    ...
    void CreateInstanceScript(bool load, std::string data, uint32 completedEncounterMask, bool mythicInstance = false, CustomDifficulty customDifficulty = CustomDifficulty::Disabled);
    bool Reset(uint8 method, std::list<uint32>* globalSkipList = nullptr);
    uint32 GetScriptId() { return i_script_id; }
    InstanceScript* GetInstanceScript() const { return instance_script; }
    void PermBindAllPlayers(Creature const* source = nullptr);
    ...
    void SummonObjectsForLoot(Player* player);
...
private:
    ...
    bool m_summmonedObjectsForLoot;
...
inline void Map::Visit(Cell const& cell, TypeContainerVisitor<T, CONTAINER>& visitor)
{
    ...
    GridCoord coord(x, y);
    if (!coord.IsCoordValid())
        return;

    if (!cell.NoCreate() || IsGridLoaded(coord))
    {
