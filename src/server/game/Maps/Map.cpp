...
{
    if (!World::IsPathfindingEnabled(this, true)) // pussywizard
        return;
    ...
    switch (mmapLoadResult)
    {
        case MMAP::MMAP_LOAD_RESULT_OK:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "MMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case MMAP::MMAP_LOAD_RESULT_ERROR:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "Could not load MMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case MMAP::MMAP_LOAD_RESULT_IGNORED:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "Ignored MMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
    }
}
...
void Map::LoadVMap(int gx, int gy)
{
    ...
    switch (vmapLoadResult)
    {
        case VMAP::VMAP_LOAD_RESULT_OK:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "VMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_ERROR:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "Could not load VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_IGNORED:
            sLog->outDebug(DebugLogFilters::LOG_FILTER_MAPS, "Ignored VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
    }
}
...
Map::Map(uint32 id, uint32 InstanceId, uint8 SpawnMode, Map* _parent)
    ...
    , m_unloadTimer(0)
    , m_VisibleDistance(VisibilityConstants::DEFAULT_VISIBILITY_DISTANCE)
    , _instanceResetPeriod(0)
...
void Map::Update( const uint32 t_diff, const uint32 s_diff, bool thread )
{
    ...
    {
        PROFILE_SCOPE( "VisitActiveObjects" );
...
{
    PROFILE_SCOPE( "HandleDelayedVisibility" );
...
bool GridMap::loadHeightData(FILE* in, uint32 offset, uint32 /*size*/)
{
    ...
    if (header.flags & MAP_HEIGHT_HAS_FLIGHT_BOUNDS)
    {
        ...
        if (fread(_maxHeight, sizeof(int16), 3 * 3, in) != 3 * 3 || fread(_minHeight, sizeof(int16), 3 * 3, in) != 3 * 3)
            return false;
    }
...
bool Map::IsOutdoors(uint32 phaseMask, float x, float y, float z) const
{
    uint32 mogpFlags;
...
bool Map::GetAreaInfo(uint32 phaseMask, float x, float y, float z, uint32& flags, int32& adtId, int32& rootId, int32& groupId) const
{
    ...
    if (phaseMask)
    {
        std::lock_guard lock(_dynamicTreeLock);
        hasDynamicAreaInfo = phaseMask && _dynamicTree.getAreaInfo(x, y, dynamic_z, phaseMask, dflags, dadtId, drootId, dgroupId);
    }
    ...
    if (hasVmapAreaInfo || hasDynamicAreaInfo)
    {
        ...
        if (GridMap* gmap = const_cast<Map*>(this)->GetGrid(x, y))
        {
            ...
            if (z + 2.0f > mapHeight  &&  mapHeight > vmap_z)
                return false;
        }
        return true;
    }
...
uint32 Map::GetAreaId(float x, float y, float z, bool* isOutdoors, uint32 phaseMask) const
{
    uint32 mogpFlags;
    int32 adtId, rootId, groupId;
...
ZLiquidStatus Map::getLiquidStatus(float x, float y, float z, uint8 ReqLiquidType, LiquidData* data) const
{
    ...
    if (vmgr->GetLiquidLevel(GetId(), x, y, z, liquid_level, ground_level, liquid_type))
    {
        ;//sLog->outDebug(LOG_FILTER_MAPS, "getLiquidStatus(): vmap liquid level: %f ground: %f type: %u", liquid_level, ground_level, liquid_type);
        // Check liquid flags, liquid level and ground level
        if ((!ReqLiquidType || (GetLiquidFlags(liquid_type) & ReqLiquidType)) && liquid_level > ground_level && z > ground_level - 2)
        {
            // All ok in water -> store data
            if (data)
            {
...
bool Map::isInLineOfSight(float x1, float y1, float z1, float x2, float y2, float z2, uint32 phasemask, LineOfSightChecks checks /* = LINEOFSIGHT_ALL_CHECKS */) const
{
    if (checks & LINEOFSIGHT_CHECK_VMAP && !VMAP::VMapFactory::createOrGetVMapManager()->isInLineOfSight(GetId(), x1, y1, z1, x2, y2, z2))
        return false;

    if (checks & LINEOFSIGHT_CHECK_GOBJECT)
    {
        std::lock_guard lock(_dynamicTreeLock);
        if (!_dynamicTree.isInLineOfSight(x1, y1, z1, x2, y2, z2, phasemask))
            return false;
    }
...
bool Map::getObjectHitPos(uint32 phasemask, float x1, float y1, float z1, float x2, float y2, float z2, float& rx, float& ry, float& rz, float modifyDist) const
{
    ...
    {
        std::lock_guard lock(_dynamicTreeLock);
        result = _dynamicTree.getObjectHitPos(phasemask, startPos, dstPos, resultPos, modifyDist);
    }
...
float Map::GetGameObjectFloor(uint32 phasemask, float x, float y, float z, float maxSearchDist) const
{
    std::lock_guard lock(_dynamicTreeLock);
...
InstanceMap::InstanceMap(uint32 id, uint32 InstanceId, uint8 SpawnMode, Map* _parent)
    ...
    , m_summmonedObjectsForLoot(false)
...
bool InstanceMap::CanEnter(Player* player, bool loginCheck)
{
    ...
    if (GetId() == 624)
        if (Battlefield* battlefield = sBattlefieldMgr->GetBattlefieldToZoneId(4197)) // wintergrasp
            if (!sWorld->IsCrossfactionEnabled(Crossfaction::Group) && player->GetTeam(CF_DENY) != battlefield->GetDefenderTeam())
            {
    ...
    bool checkProgress = (IsRaid() || GetId() == 44 || GetId() == 668 /*HoR*/ || GetId() == 650 /*toc5*/ || GetId() == 230/*Hellforge*/);
    InstanceScript* instance = GetInstanceScript();
    if (instance && instance->IsMythic())
        checkProgress = true;

    if (checkProgress && instance && instance->IsEncounterInProgress())
    {
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
    ...
    if (instance_script)
    {
        ...
        if (instance_script->IsMythic())
            instance_script->OnPlayerEnterMythic(player);
    }
...
void InstanceMap::Update(const uint32 t_diff, const uint32 s_diff, bool /*thread*/)
{
    ...
    if (!m_summmonedObjectsForLoot && !GetPlayers().isEmpty())
        SummonObjectsForLoot(GetPlayers().begin()->GetSource());
}
...
void InstanceMap::CreateInstanceScript(bool load, std::string data, uint32 completedEncounterMask, bool mythicInstance /* = false*/, CustomDifficulty customDifficulty /*= CustomDifficulty::Disabled */)
{
    ...
    if (mythicInstance)
    {
        instance_script->EnableMythic();
        instance_script->HandleMythicAffix(Mythic::MYTHIC_PROC_FLAG_INITIALIZE_MYTHIC, nullptr);
    }
    ...
    if (load)
    {
        ...
        instance_script->Create();

    instance_script->SetCustomDifficulty(customDifficulty);
}
...
void InstanceMap::PermBindAllPlayers(Creature const* source /* = nullptr */)
{
    if (!IsDungeon())
        return;
    ...
    uint32 ascendedEncounterIndexMask = source ? sObjectMgr->GetDungeonEncounterIndexMaskFromEntry(source->GetEntry()) : 0;
    InstanceScript* instance = GetInstanceScript();
    if (!instance || instance->GetCustomDifficulty() != CustomDifficulty::Ascended10)
        ascendedEncounterIndexMask = 0;
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        if (!bind || !bind->perm)
        {
            ...
            data << uint32(0);
            player->GetSession()->SendPacket(&data);
            sInstanceSaveMgr->PlayerBindToInstance(player->GetGUIDLow(), save, true, player, ascendedEncounterIndexMask);
        }
...
void InstanceMap::SummonObjectsForLoot(Player* player)
{
    m_summmonedObjectsForLoot = true;
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT DISTINCT objectId, summonPositionString, isCreature FROM group_saved_loot WHERE summonPositionString IS NOT NULL AND instanceId = %d AND mapId = %d", GetInstanceId(), GetId());
    ...
    if (result)
    {
        do
        {
            ...
            uint32 objectId = fields[0].GetUInt32();
            char const* summonPositionString = fields[1].GetCString();
            bool isCreature = fields[2].GetBool();

            if (!summonPositionString)
                continue;

            std::istringstream loadStream(summonPositionString);
            ...
            loadStream >> x >> y >> z >> orient;

            if (isCreature)
            {
                if (Creature* creature = player->SummonCreature(objectId, x, y, z, orient, TEMPSUMMON_MANUAL_DESPAWN, WEEK * IN_MILLISECONDS))
                    creature->loot.FillLootFromDB(creature);
            }
            else
            {
                if (GameObject* go = player->SummonGameObject(objectId, x, y, z, orient, 0, 0, 0, 0, WEEK * IN_MILLISECONDS))
                    go->loot.FillLootFromDB(go);
            }
...
Player* Map::GetPlayer(uint64 guid) const
{
    auto const itr = _playerStore.find(guid);
    if (itr == _playerStore.end())
        return nullptr;

    return itr->second;
}

Creature* Map::GetCreature(uint64 guid) const
{
    auto const itr = _creatureStore.find(guid);
    if (itr != _creatureStore.end())
        return itr->second;

    return nullptr;
}

GameObject* Map::GetGameObject(uint64 guid) const
{
    auto const itr = _gameobjectStore.find(guid);
    if (itr == _gameobjectStore.end())
        return nullptr;

    return itr->second;
}

Transport* Map::GetTransport(uint64 guid) const
{
...
DynamicObject* Map::GetDynamicObject(uint64 guid) const
{
    auto const itr = _dynamicObjectStore.find(guid);
    if (itr == _dynamicObjectStore.end())
        return nullptr;

    return itr->second;
}

Pet* Map::GetPet(uint64 guid) const
{
    auto const itr = _petStore.find(guid);
    if (itr == _petStore.end())
        return nullptr;

    return itr->second;
}

Corpse* Map::GetCorpse(uint64 guid) const
{
    auto const itr = _corpseStore.find(guid);
    if (itr == _corpseStore.end())
        return nullptr;

    return itr->second;
}

Unit* Map::GetUnit(uint64 guid) const
{
...
void Map::InsertToObjectsStore(Creature* object)
{
    _creatureStore[object->GetGUID()] = object;
}

void Map::InsertToObjectsStore(GameObject* object)
{
    _gameobjectStore[object->GetGUID()] = object;
}

void Map::InsertToObjectsStore(DynamicObject* object)
{
    _dynamicObjectStore[object->GetGUID()] = object;
}

void Map::InsertToObjectsStore(Pet* object)
{
    _petStore[object->GetGUID()] = object;
}

void Map::InsertToObjectsStore(Player* object)
{
    _playerStore[object->GetGUID()] = object;
}

void Map::InsertToObjectsStore(Corpse* object)
{
    _corpseStore[object->GetGUID()] = object;
}

void Map::RemoveFromObjectsStore(Creature const* object)
{
    _creatureStore.erase(object->GetGUID());
}

void Map::RemoveFromObjectsStore(GameObject const* object)
{
    _gameobjectStore.erase(object->GetGUID());
}

void Map::RemoveFromObjectsStore(DynamicObject const* object)
{
    _dynamicObjectStore.erase(object->GetGUID());
}

void Map::RemoveFromObjectsStore(Pet const* object)
{
    _petStore.erase(object->GetGUID());
}

void Map::RemoveFromObjectsStore(Player const* object)
{
    _playerStore.erase(object->GetGUID());
}

void Map::RemoveFromObjectsStore(Corpse const* object)
{
    _corpseStore.erase(object->GetGUID());
}

void Map::DoOnAllCreatures(std::function<void(Creature*)> func) const
{
    for (auto const& itr : _creatureStore)
        func(itr.second);
}

void Map::ScheduleDelayedGameEventAIHook(uint16 eventID, bool activated)
{
    _delayedGameEventAIHookRequestData.emplace_back(std::make_pair(eventID, activated));
}

void Map::ProcessGameEventRequests()
{
    if (!_delayedGameEventAIHookRequestData.empty())
    {
        // Do not call AI hook directly here, because it can cause deadlock
        for (auto const& requestPair : _delayedGameEventAIHookRequestData)
        {
            uint16 eventID = requestPair.first;
            bool activated = requestPair.second;
            for (auto const& pair : _creatureStore)
            {
                Creature* creature = pair.second;
                if (!creature->IsInWorld())
                    continue;

                creature->ScheduleDelayedGameEventAIHook(eventID, activated);
            }

            for (auto const& pair : _gameobjectStore)
            {
                GameObject* go = pair.second;
                if (!go->IsInWorld())
                    continue;

                go->ScheduleDelayedGameEventAIHook(eventID, activated);
            }
        }

        _delayedGameEventAIHookRequestData.clear();
    }
}
...
void Map::SaveCreatureRespawnTime(uint32 dbGuid, time_t& respawnTime)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CREATURE_RESPAWN);
...
void Map::RemoveCreatureRespawnTime(uint32 dbGuid)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CREATURE_RESPAWN);
    stmt->setUInt32(0, dbGuid);
...
void Map::SaveGORespawnTime(uint32 dbGuid, time_t& respawnTime)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_GO_RESPAWN);
    stmt->setUInt32(0, dbGuid);
    stmt->setUInt32(1, uint32(respawnTime));
    stmt->setUInt16(2, GetId());
    stmt->setUInt32(3, GetInstanceId());
...
void Map::RemoveGORespawnTime(uint32 dbGuid)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GO_RESPAWN);
    stmt->setUInt32(0, dbGuid);
    stmt->setUInt16(1, GetId());
    stmt->setUInt32(2, GetInstanceId());
    CharacterDatabase.Execute(stmt);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CREATURE_RESPAWNS);
...
{
    auto const it = _zonePlayerCountMap.find(zoneId);
    return it != _zonePlayerCountMap.end() ? it->second : 0;
}
...
bool ShouldDynamicScalingApply(GameObject const* go)
{
    GameObjectTemplate const* goTemplate = sObjectMgr->GetGameObjectTemplate(go->GetEntry());
    if (!goTemplate || !(goTemplate->flags & 4))
        return false;

    return std::ranges::any_of(goTemplate->questItems, [](uint32 const questItemID) { return questItemID != 0; });
}

bool ShouldDynamicScalingApply(Creature const* creature)
{
    if (creature->GetAreaId() == 4197) // wintergrasp
        return false;

    switch (creature->GetEntry())
    {
        // Clouds harvested by engineers for materials.
        case 32522:
        case 32544:
            return false;
        default:
    ...
    CreatureTemplate const* creaTemplate = creature->GetCreatureTemplate();

    if (!creaTemplate)
        return false;

    if (creaTemplate->rank > CREATURE_ELITE_ELITE)
        return false;

    if (creaTemplate->type == CREATURE_TYPE_ELEMENTAL)
        return false;

    return true;
}

void Map::ApplyDynamicModeRespawnScaling(WorldObject const* obj, uint32* respawnDelay, uint32* corpseDelay) const
{
    if (!sCustomMgr->GetBoolConfig(CONFIG_DYNAMIC_RESPAWN_ENABLED) || obj->GetMap() != this || !obj->IsInWorld() || IsBattlegroundOrArena() || Instanceable() || (!respawnDelay && !corpseDelay))
        return;

    if (IsDungeon() || IsRaid())
        return;

    TypeID const typeId = obj->GetTypeId();
    if (typeId != TYPEID_UNIT && typeId != TYPEID_GAMEOBJECT)
        return;

    if (typeId == TYPEID_GAMEOBJECT && !ShouldDynamicScalingApply(obj->ToGameObject()))
        return;

    if (typeId == TYPEID_UNIT && !ShouldDynamicScalingApply(obj->ToCreature()))
        return;

    auto const it = _zonePlayerCountMap.find(obj->GetZoneId());
    ...
    if (respawnDelay)
    {
        float const adjustFactor = std::max(0.0f, 1.0f - ((sCustomMgr->GetFloatConfig(typeId == TYPEID_GAMEOBJECT ? CONFIG_RESPAWN_DYNAMICRATE_GAMEOBJECT : CONFIG_RESPAWN_DYNAMICRATE_CREATURE) / 100.0f) * playerCount));
        uint32 const timeMinimum = sCustomMgr->GetIntConfig(typeId == TYPEID_GAMEOBJECT ? CONFIG_RESPAWN_DYNAMICMINIMUM_GAMEOBJECT : CONFIG_RESPAWN_DYNAMICMINIMUM_CREATURE);
        if (*respawnDelay > timeMinimum)
        {
            uint32 const newDelay = std::max<uint32>(ceil(*respawnDelay * adjustFactor), timeMinimum);
            if (newDelay < *respawnDelay)
                *respawnDelay = newDelay;
        }
    }

    if (sCustomMgr->GetBoolConfig(CONFIG_DYNAMIC_RESPAWN_ENABLE_CORPSEDELAY_CALC) && corpseDelay && typeId == TYPEID_UNIT)
    {
        float const adjustFactor = std::max(0.0f, 1.0f - ((sCustomMgr->GetFloatConfig(CONFIG_RESPAWN_DYNAMICRATE_CREATURE_CORPSEDELAY) / 100.0f) * playerCount));
        uint32 const timeMinimum = sCustomMgr->GetIntConfig(CONFIG_RESPAWN_DYNAMICMINIMUM_CREATURE_CORPSEDELAY);
        if (*corpseDelay > timeMinimum)
        {
            uint32 const newDelay = std::max<uint32>(ceil(*corpseDelay * adjustFactor), timeMinimum);
            if (newDelay < *corpseDelay)
                *corpseDelay = newDelay;
        }
    }
}
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CREATURE_RESPAWN_BY_INSTANCE);
    ...
    for (DungeonEncounterList::const_iterator itr = encounters->begin(); itr != encounters->end(); ++itr)
    {
        ...
        if (encounter->creditType == type && encounter->creditEntry == creditEntry)
        {
            if (source)
            {
                ...
                {
                    uint32 encounterIndexMask = 1 << encounter->dbcEntry->encounterIndex;
                    instanceScript->SetCompletedEncountersMask((encounterIndexMask) | instanceScript->GetCompletedEncounterMask(), true);
                    if (instanceScript->GetCustomDifficulty() == CustomDifficulty::Ascended10)
                    {
                        for (auto const& ref : GetPlayers())
                        {
                            ...
                            InstancePlayerBind* instancePlayerBind = sInstanceSaveMgr->PlayerGetBoundInstance(player->GetGUIDLow(), GetId(), GetDifficulty());
                            if (!instancePlayerBind)
                                continue;

                            if (instancePlayerBind->CompletedAscendedEncounters & encounterIndexMask)
                                continue;

                            instancePlayerBind->CompletedAscendedEncounters |= encounterIndexMask;
                            if (instancePlayerBind->save)
                            {
                                auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_INSTANCE);
                                stmt->setUInt32(0, instancePlayerBind->save->GetInstanceId());
                                stmt->setBool(1, instancePlayerBind->perm);
                                stmt->setUInt32(2, instancePlayerBind->CompletedAscendedEncounters);
                                stmt->setUInt32(3, player->GetGUIDLow());
                                stmt->setUInt32(4, instancePlayerBind->save->GetInstanceId());
                                CharacterDatabase.Execute(stmt);
                            }
                        }
                    }
...
void Map::LogEncounterFinished(EncounterCreditType type, uint32 creditEntry)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_LOG_ENCOUNTER);
    stmt->setUInt32(0, GetId());
    stmt->setUInt32(1, GetDifficulty());
    stmt->setUInt32(2, type);
    stmt->setUInt32(3, creditEntry);
    stmt->setString(4, playersInfo);
