...
std::vector<ChatCommand> const& ChatHandler::getCommandTable()
{
    ...
    if (LoadCommandTable())
    {
        ...
        auto stmt    = WorldDatabase.GetPreparedStatement(WORLD_SEL_COMMANDS);
        ...
        if ((table[i].Handler)(this, aText))
        {
            ...
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel) && !ignore)
            {
                ...
                if (m_session)
                {
                    ...
                    DBLog::OutGameMaster(p, fmt::format("Command: {} (GM position: {}, GM MapID: {}, Selected object type: {}, Selected object name: {}, Selected object GUIDLow: {})", fullcmd, p->ToString(), p->GetMapId(), GetLogNameForGuid(sel_guid), p->GetSelectedUnit() ? p->GetSelectedUnit()->GetName() : "<unknown>", GUID_LOPART(sel_guid)));
                }
            }
        }
...
{
    if (chatType == CHAT_MSG_CHANNEL && channelName.empty())
        return {};
...
uint32 ChatHandler::extractSpellIdFromLink(char* text)
{
    ...
    switch (type)
    {
        ...
        case SPELL_LINK_TALENT: {
            ...
            int32 rank = param1_str ? (uint32)std::atol(param1_str) : 0;
        ...
        case SPELL_LINK_GLYPH: {
            uint32 glyph_prop_id = param1_str ? (uint32)std::atol(param1_str) : 0;
...
uint64 ChatHandler::extractGuidFromLink(char* text)
{
    ...
    switch (type)
    {
        ...
        case SPELL_LINK_CREATURE: {
            uint32 lowguid = (uint32)std::atol(idS);
...
{
    return m_session->GetPlayer()->GetGUIDLow();
}

// WEB COMMAND HANDLER
const char* WebCommandHandler::GetTrinityString(int32 entry) const
{
    return sObjectMgr->GetTrinityStringForDBCLocale(entry);
}
...
std::string WebCommandHandler::GetNameLink() const
{
    std::string name;
...
LocaleConstant WebCommandHandler::GetSessionDbcLocale() const
{
    return sWorld->GetDefaultDbcLocale();
}

int WebCommandHandler::GetSessionDbLocaleIndex() const
{
    return sObjectMgr->GetDBCLocaleIndex();
}
