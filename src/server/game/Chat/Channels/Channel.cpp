    ...
    , _password("")
{
    ...
    else // it's custom channel
    {
        ...
        // pussywizard:
        if (ChannelRights const* rights = sChannelMgr->GetChannelRightsFor(channelDBId))
        {
            _channelRights = *rights;
        ...
        /*********************************************************/
        _announce  = sCustomMgr->GetBoolConfig(CONFIG_ALLOW_CHANNEL_OWNER);
        _ownership = sCustomMgr->GetBoolConfig(CONFIG_ALLOW_CHANNEL_PASSWORD);
        _announce  = sCustomMgr->GetBoolConfig(CONFIG_ALLOW_CHANNEL_NOTIFICATIONS);
        ...
        {
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHANNEL);
...
void Channel::UpdateChannelInDB() const
{
    if (_IsSaved)
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHANNEL);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHANNEL_USAGE);
    stmt->setUInt32(0, _channelDBId);
    CharacterDatabase.Execute(stmt);
}
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHANNEL_BAN);
    stmt->setUInt32(0, _channelDBId);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHANNEL_BAN);
    stmt->setUInt32(0, _channelDBId);
    stmt->setUInt32(1, guid);
    CharacterDatabase.Execute(stmt);
}
...
void Channel::CleanOldChannelsInDB()
{
    if (sWorld->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) > 0)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_OLD_CHANNELS);
...
void Channel::JoinChannel(Player* player, std::string const& pass)
{
    ...
    if (_announce && (!AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) || !sWorld->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL)))
    {
        WorldPacket data;
    ...
    if (!IsConstant())
    {
        ...
        if (_channelRights.IsModerator(player))
        {
...
void Channel::KickOrBan(Player const* player, std::string const& badname, bool ban)
{
    ...
    bool const isGoodConstantModerator = _channelRights.IsModerator(player);
    ...
    bool isBadConstantModerator = Trinity::Containers::Contains(_channelRights.Moderators, badAccId);
...
void Channel::UnBan(Player const* player, std::string const& badname)
{
    ...
    uint64 good                    = player->GetGUID();
    bool const isConstantModerator = _channelRights.IsModerator(player);
    ...
    if (_channelRights.Flags & CHANNEL_RIGHT_CANT_BAN)
        DBLog::OutGameMaster(player, fmt::format("Command: /unban (ChannelName: {}, Unbanned player name: {}, Unbanned player CharacterID: {})", GetName(), badname, GUID_LOPART(victim)));
...
void Channel::Password(Player const* player, std::string const& pass)
{
    ...
    if (!isConstantModerator)
    {
        ChatHandler(player->GetSession()).PSendSysMessage("Channel: Password option is available only for Game Masters.");
        return;
    }
...
void Channel::SetMode(Player const* player, std::string const& p2n, bool mod, bool set)
{
    ...
    if (!victim || !IsOn(victim) ||
        ...
        ((!AccountMgr::IsGMAccount(sec) || !AccountMgr::IsGMAccount(newp->GetSession()->GetSecurity())) && player->GetTeam(CF_DENY) != newp->GetTeam(CF_DENY)))
    {
        WorldPacket data;
    ...
    if (mod)
    {
        bool const isBadConstantModerator = _channelRights.IsModerator(newp);
...
void Channel::SetOwner(Player const* player, std::string const& newname)
{
    ...
    if (!victim || !IsOn(victim) || newp->GetTeam(CF_DENY) != player->GetTeam(CF_DENY))
    {
        WorldPacket data;
...
void Channel::Say(uint64 guid, std::string const& what, uint32 lang, uint64 guidForSelfOnly)
{
    if (what.empty())
        return;

    if (sWorld->IsCrossfactionEnabled(Crossfaction::Channels))
        lang = LANG_UNIVERSAL;
    ...
    if (player && !player->GetSession()->GetSecurity()) // pussywizard: prevent spam on populated channels
    {
        ...
        if (sCustomMgr->GetBoolConfig(CONFIG_ALLOW_CHANNEL_SPEAK_DELAY))
        {
        ...
        {
            std::string const timeStr = secsToTimeString(pinfo.lastSpeakTime + speakDelay - sWorld->GetGameTime());
            if (!_channelRights.SpeakMessage.empty())
    ...
    SendToAll(&data, pinfo.IsModerator() ? 0 : guid, guidForSelfOnly);
}
...
void Channel::Invite(Player const* player, std::string const& newname)
{
    ...
    Player* newp = ObjectAccessor::FindPlayerByName(newname, false);
    if (!newp || !newp->isGMVisible() || player->getLevel() < 70)
    {
        WorldPacket data;
        MakePlayerNotFound(&data, newname);
        SendToOne(&data, guid);
    ...
    if (newp->GetTeam(CF_DENY) != player->GetTeam(CF_DENY))
    {
        WorldPacket data;
...
void Channel::SendToAll(WorldPacket* data, uint64 guid, uint64 guidForSelfOnly)
{
    for (PlayerContainer::const_iterator i = playersStore.begin(); i != playersStore.end(); ++i)
    {
        if (guidForSelfOnly && i->first != guidForSelfOnly)
            continue;
