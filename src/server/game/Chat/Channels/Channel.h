...
struct ChannelRights
{
    ...
    bool IsModerator(Player const* player) const
    {
        if (player->GetSession()->GetSecurity() >= SEC_MODERATOR)
            return true;

        return Trinity::Containers::Contains(Moderators, player->GetSession()->GetAccountId());
    }
};
...
#define CHANNEL_BAN_DURATION            DAY*7
...
class Channel
{
    ...
    public:
        ...
        void SetPassword(std::string&& newPassword) { _password = std::move(newPassword); }
        ...
        void Say(uint64 guid, std::string const& what, uint32 lang, uint64 guidForSelfOnly = 0);
    ...
    private:
        ...
        void SendToAll(WorldPacket* data, uint64 guid = 0, uint64 guidForSelfOnly = 0);
