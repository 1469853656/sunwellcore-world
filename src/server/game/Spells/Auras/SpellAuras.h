...
public:
    ...
    static Aura* TryRefreshStackOrCreate(SpellInfo const* spellproto, uint8 tryEffMask, WorldObject* owner, Unit* caster, int32* baseAmount = nullptr, ItemRef const& castItem = nullptr, uint64 casterGUID = 0, bool* refresh = nullptr, bool periodicReset = false);
    static Aura* TryCreate(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount = nullptr, ItemRef const& castItem = nullptr, uint64 casterGUID = 0);
    static Aura* Create(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, ItemRef const& castItem, uint64 casterGUID);
    explicit Aura(SpellInfo const* spellproto, WorldObject* owner, Unit* caster, ItemRef const& castItem, uint64 casterGUID);
    ...
    void DisableSpellStealAndDispel() { m_isSpellStealAndDispelDisabled = true; }
    bool const IsSpellStealAndDispelDisabled() const { return m_isSpellStealAndDispelDisabled; }
    ...
    const AuraApplication* GetApplicationOfTarget(uint64 guid) const
    {
        ...
        if (itr != m_applications.end()) return itr->second;
        return nullptr;
    }
    AuraApplication* GetApplicationOfTarget(uint64 guid)
    {
...
protected:
    ...
    //! Is not set by aura system itself but by spell system and passed via spells SpellValue on aura creation
    bool m_isSpellStealAndDispelDisabled : 1;
...
{
    friend Aura* Aura::Create(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, ItemRef const& castItem, uint64 casterGUID);

protected:
    explicit UnitAura(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, ItemRef const& castItem, uint64 casterGUID);
...
protected:
    explicit DynObjAura(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, ItemRef const& castItem, uint64 casterGUID);
