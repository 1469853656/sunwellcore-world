    ...
    if (uint32 groupId = aurEff->GetAuraGroup())
    {
        ...
        if (!aurEff->IsPeriodic() && (sFlag & SPELL_GROUP_STACK_FLAG_EFFECT_EXCLUSIVE))
        {
            ...
            for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
            {
                ...
                if (aurEff->GetAuraType() != SPELL_AURA_230 /*SPELL_AURA_MOD_INCREASE_HEALTH_2*/ && aurEff->GetAuraType() != SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK && aurEff->GetMiscValue() != (*iter)->GetMiscValue())
                    continue;
...
Aura::Aura(SpellInfo const* spellproto, WorldObject* owner, Unit* caster, ItemRef const& castItem, uint64 casterGUID)
    ...
    , m_castItemGuid(castItem ? castItem->GetGUID() : 0)
    ...
    , m_isSpellStolen(false)
    , m_isSpellStealAndDispelDisabled(false)
{
        ...
        if (addUnit)
        {
            ...
            if (GetType() == DYNOBJ_AURA_TYPE)
            {
                ...
                switch (GetId())
                {
                    ...
                    case 19099: // blizzard
                    case 52067: // Aura of Regeneration
                    case 57056: // Aura of Regeneration
                    case 29882: // Loose mana
                    {
...
int32 Aura::CalcMaxDuration(Unit* caster) const
{
    ...
    if (m_spellInfo->Id == 25646)
    {
        if (Unit* caster = GetCaster())
        {
            InstanceScript* instance = caster->GetInstanceScript();
            bool isMythic            = instance && instance->IsMythic();
            if (!isMythic)
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 1))
                    {
                        ...
                        int32 const basepoints0 = CalculatePct(devouringPlague->GetSpellInfo()->GetMaxTicks() * static_cast<int32>(damage), aurEff->GetAmount());
                        auto const heal         = static_cast<int32>(CalculatePct(basepoints0, 15));

                        caster->CastCustomSpell(target, 63675, &basepoints0, nullptr, nullptr, true, nullptr, devouringPlague);
                        caster->CastCustomSpell(caster, 75999, &heal, nullptr, nullptr, true, nullptr, devouringPlague);
                    }
                }
                // Power Word: Shield
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    if (aurEff)
                    {
                        ...
                        caster->CastSpell(target, spellId, true, nullptr, GetEffect(0));
                    }
                }
                // Unholy blight
                if (GetId() == 50536)
...
bool Aura::CanStackWith(Aura const* existingAura, bool remove) const
{
    ...
    if (existingAura->GetId() == 62328 || existingAura->GetId() == 62446 && !sameCaster)
        return false;

    // Sanctum Sentry @todo move this shit into DB (spell_group)
    if (existingAura->GetId() == 64381 && !sameCaster)
        return true;
    ...
    if (stackFlags)
    {
        ...
        if (sameCaster && m_spellInfo->SpellFamilyName == existingSpellInfo->SpellFamilyName && (stackFlags & SPELL_GROUP_STACK_FLAG_NOT_SAME_CASTER))
            return false;
    ...
    if (m_spellInfo->IsRankOf(existingSpellInfo))
    {
        ...
        if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && m_spellInfo->SpellFamilyFlags.IsEqual(0x0, 0x80000000, 0x0))
            return true;
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_APPLY_AREA_AURA_PARTY:
                if (!effect.Predicate)
                    effect.Predicate = Trinity::AnyGroupedUnitInObjectRangeCheck(auraOwner, auraOwner, radius, false);
                //! no break
            case SPELL_EFFECT_APPLY_AREA_AURA_RAID:
                if (!effect.Predicate)
                    effect.Predicate = Trinity::AnyGroupedUnitInObjectRangeCheck(auraOwner, auraOwner, radius, true);
                //! no break
            case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
                if (!effect.Predicate)
                    effect.Predicate = Trinity::AnyFriendlyUnitInObjectRangeCheck(auraOwner, auraOwner, radius);
                //! no break
            case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY:
            {
                ...
                query.Enabled   = true;
                query.MaxRadius = std::max(radius, query.MaxRadius);
