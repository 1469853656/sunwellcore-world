...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS] = {
    ...
    &AuraEffect::HandleAuraModIncreaseHealthProportionally,   //230 SPELL_AURA_MOD_INCREASE_HEALTH_2
    ...
    &AuraEffect::HandleAuraModIncreaseHealth_2,               //250 SPELL_AURA_MOD_INCREASE_HEALTH_2
...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    if (!amount && caster)
        if (uint64 itemGUID = GetBase()->GetCastItemGUID())
            if (Player* playerCaster = caster->ToPlayer())
                if (ItemRef castItem = playerCaster->GetItemByGuid(itemGUID))
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_MOD_DAMAGE_PERCENT_DONE:
            ...
            {
                ItemRef item1 = caster->ToPlayer()->GetWeaponForAttack(BASE_ATTACK);
                ItemRef item2 = caster->ToPlayer()->GetWeaponForAttack(OFF_ATTACK);
            ...
            switch (GetMiscValue())
            {
                ...
                case FORM_BEAR:
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELLFAMILY_DRUID, 961, 0))
                    {
                        ...
                        target->CastCustomSpell(target, 62069, &bp, nullptr, nullptr, true, nullptr, this, target->GetGUID());
                    }
                    break;
                case FORM_MOONKIN:
...
{
public:
    DelayedStanceRestoreEvent(Unit& owner, uint32 spellId)
        : BasicEvent()
        , _owner(owner)
        , _spellId(spellId)
    {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        target->RemoveShapeshiftAuras(GetBase(), form);
    ...
    else
    {
        ...
        else if (AuraEffect const* aurEff = target->GetAuraEffectsByType(SPELL_AURA_MOD_SHAPESHIFT).front())
            if (aurEff->GetMiscValue() != form)
                target->SetShapeshiftForm(ShapeshiftForm(aurEff->GetMiscValue()));
    ...
    if (target->getClass() == CLASS_DRUID)
    {
        ...
        {
            if (ItemRef pItem = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND))
            {
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        if (ItemRef pItem = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
        {
...
void AuraEffect::HandleAuraMounted(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (Player* player = target->ToPlayer())
        {
            ...
            if (player->InBattleground() && player->IsPlayingCrossfaction())
            {
...
void AuraEffect::HandleAuraModIncreaseHealthProportionally(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    if (apply)
    {
        target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(GetAmount()), apply);
        target->ModifyHealth(GetAmount());
        target->SetHealth(target->GetMaxHealth() * (healthPct / 100.0f));
    }
    else
    {
        target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(GetAmount()), apply);
        target->SetHealth(std::min(target->GetMaxHealth() - 1, static_cast<uint32>(target->GetMaxHealth() * (healthPct / 100.0f))));
    }
}

void AuraEffect::HandleAuraModIncreaseHealth_2(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    {
        target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(GetAmount()), apply);
        int32 healthToSet = target->GetMaxHealth() * healthPct;
        target->SetHealth(std::max(1, healthToSet));
    }
}
...
void AuraEffect::HandleAuraModWeaponCritPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (int i = 0; i < MAX_ATTACK; ++i)
        if (ItemRef pItem = target->ToPlayer()->GetWeaponForAttack(WeaponAttackType(i), true))
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        for (int i = 0; i < MAX_ATTACK; ++i)
            if (ItemRef item = target->ToPlayer()->GetWeaponForAttack(WeaponAttackType(i), false))
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    ItemRef newitem = plCaster->StoreNewItem(dest, GetSpellInfo()->Effects[m_effIndex].ItemType, true);
    if (!newitem)
    {
...
void AuraEffect::HandleAuraLinked(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL)
    {
        if (apply)
        {
            ...
            //! Mythic broken bones, ToDo: moveme
            if (m_spellInfo->Id == 62354)
            {
                CustomSpellValues val;
                val.AddSpellMod(SPELLVALUE_AURA_DURATION, -1);
                val.AddSpellMod(SPELLVALUE_BASE_POINT0, -20);
                val.AddSpellMod(SPELLVALUE_BASE_POINT1, -20);
                val.AddSpellMod(SPELLVALUE_BASE_POINT2, -20);
                caster->CastCustomSpell(triggeredSpellId, val, target, TRIGGERED_FULL_MASK, NullItemRef, this);
            }
            else
            {
            ...
            case SPELLFAMILY_GENERIC: {
                switch (auraId)
                {
                    ...
                    case 27808:
                        if (caster)
                        {
                            caster->CastCustomSpell(29879, SPELLVALUE_BASE_POINT0, int32(target->CountPctFromMaxHealth(26)), target, true, nullptr, this);
                            if (GetTickNumber() == 1)
                    ...
                    case 59376: {
                        ...
                        target->CastCustomSpell(target, 51132, &dmg, nullptr, nullptr, true);
                        return;
                    }
                }
                break;
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (caster && GetSpellInfo()->Effects[GetEffIndex()].Effect == SPELL_EFFECT_PERSISTENT_AREA_AURA && caster->SpellHitResult(target, GetSpellInfo(), false) != SPELL_MISS_NONE)
        return;
    ...
    if (GetSpellInfo()->Effects[GetEffIndex()].IsAreaAuraEffect() || GetSpellInfo()->Effects[GetEffIndex()].IsTargetingArea() || GetSpellInfo()->Effects[GetEffIndex()].Effect == SPELL_EFFECT_PERSISTENT_AREA_AURA || GetSpellInfo()->HasAttribute(SPELL_ATTR5_TREAT_AS_AREA_EFFECT)) // some persistent area auras have targets like A=53 B=28
    {
    ...
    caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim, procEx, PROC_SPELL_PHASE_HIT, damage, BASE_ATTACK, GetSpellInfo());
}
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->ProcDamageAndSpell(damageInfo.target, procAttacker, procVictim, procEx, PROC_SPELL_PHASE_HIT, damageInfo.damage, BASE_ATTACK, spellProto);
}
...
void AuraEffect::HandleRaidProcFromChargeAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    switch (GetId())
    {
        ...
        case 43593: // Cold Stare
            ...
            break;
        default:; //sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraEffect::HandleRaidProcFromChargeAuraProc: received not handled spell: %u", GetId());
            return;
    }
