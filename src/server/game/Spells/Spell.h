...
struct SpellValue
{
    ...
    int32 ForcedMissResult;
    int32 SpellInterruptFlags;
    bool IsSpellStealAndDispelDisabled;
    bool IsReflectDisabled;
    Optional<float> DamageOverride = {};
    Optional<float> DamagePctOverride = {};
};
...
{
    ChannelTargetData(uint64 cguid, const SpellDestination* dst)
        : channelGUID(cguid)
    {
        if (dst)
            spellDst = *dst;
...
public:
    ...
    bool CanUseOutsideOfRange() const;
    ...
    uint32 GetSpellBatchingEffectDelay(Unit const* target) const;
...
protected:
    ...
    SpellMissInfo DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura, bool wasReflected = false);
    ...
    using HitTriggerSpells = std::vector<std::pair<SpellInfo const*, int32>>;
...
{
public:
    ReflectEvent(uint64 casterGUID, uint64 targetGUID, const SpellInfo* spellInfo, Map const* map)
