    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_SHARE_DAMAGE) || GetSpellValue()->ShareDamage)
                {
                    uint32 count = 0;
                ...
                switch (m_spellInfo->Id) // better way to check unknown
                {
                    ...
                    case 21341:
                    {
                        if (unitTarget->GetTypeId() != TYPEID_PLAYER || unitTarget->getLevel() < 80)
                            return;

                        damage = urand(14800, 15200);
                        break;
                    }
        ...
        // ToGC25 Boost
        if (m_originalCaster && m_originalCaster->GetInstanceScript() && m_originalCaster->GetMapId() == 649 && m_originalCaster->GetInstanceScript()->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC && m_originalCaster->IsCreature() && !m_originalCaster->IsControlledByPlayer())
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 46968: // Shockwave
                    m_damage = urand(5000, 8000);
                    break;
                case 72637: // Swarming Shadows
                    m_damage = 8000;
                    break;
                case 40030: // Demon Fire
                    m_damage = 0;
                    break;
                case 40018: // Eye Blast
                    m_damage = 6000;
                    break;
                case 47834: // Seed of Corruption
                    m_damage = 8000;
                    break;
                case 59637: // Fire Blast
                    m_damage = 4000;
                    break;
                case 59638: // Frostbolt
                    m_damage = 4000;
                    break;
                case 50590: // Immolation
                    m_damage = 2000;
                    break;
                case 53195: // Starfall
                    m_damage = urand(1500, 3000);
                    break;
                case 53190: // Starfall
                    m_damage = 1000;
                    break;
            ...
            case 35729:
            {
                ...
                for (Unit::AuraApplicationMap::iterator iter = Auras.begin(); iter != Auras.end();)
                {
                    ...
                    if (spell->DmgClass == SPELL_DAMAGE_CLASS_NONE && spell->SpellFamilyName == SPELLFAMILY_GENERIC)
                        ...
                        {
                            if ((iter->second->GetEffectMask() & (1 << i)) && spell->Effects[i].ApplyAuraName != SPELL_AURA_PERIODIC_DAMAGE && spell->Effects[i].ApplyAuraName != SPELL_AURA_PERIODIC_TRIGGER_SPELL && spell->Effects[i].ApplyAuraName != SPELL_AURA_DUMMY)
                            {
                                dmgClassNone = false;
                                break;
                            }
                    ...
                    if ((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC || (spell->GetDispelMask() & dispelMask) || dmgClassNone) &&
                        ...
                        !iter->second->IsPositive() && !iter->second->GetBase()->IsPassive() && !spell->HasAttribute(SPELL_ATTR0_CU_IGNORE_REMOVE_MECHANICS) &&
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_spellInfo->Id == PREMIUM_HEARTHSTONE_SPELL_ID && m_caster->IsPlayer())
        sCustomMgr->HandleHearthStoneService(m_caster->ToPlayer());
}
...
void Spell::EffectPowerDrain(SpellEffIndex effIndex)
{
    ...
    if (m_caster != unitTarget)
    {
        ...
        m_caster->EnergizeBySpell(m_caster, m_spellInfo->Id, gain, powerType, false);
    }
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->Id)
    {
        case 37065: //Trachela's Carcass
        {
            if (m_caster && m_caster->GetTypeId() == TYPEID_PLAYER)
                if (Creature* Torgos = m_caster->SummonCreature(18707, -3002.40f, 5629.00f, -3.15f, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 300000))
                    Torgos->AI()->AttackStart(m_caster);
        }
        break;
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    ...
    if (!m_spellAura)
    {
        ...
        float radius = m_spellInfo->Effects[effIndex].CalcRadius(caster) * m_spellValue->RadiusMod;
        if (auto boostSpellValue = sBoostMgr->GetBoostSpellValue(sBoostMgr->GetBoostSpellTypeForEffectRadius(effIndex), m_spellInfo, caster))
        ...
        if (Aura* aura = Aura::TryCreate(m_spellInfo, MAX_EFFECT_MASK, dynObj, caster, &m_spellValue->EffectBasePoints[0]))
        {
            ...
            {
                int32 duration = irand(spellBoostValue->ValueMin, spellBoostValue->ValueMax);
                aura->SetMaxDuration(duration);
                aura->RefreshDuration();
            }
            else if (int32 dur = m_spellValue->AuraDuration)
            {
...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            {
                // exclude mines, herbs, etc.
                // before sending loot
                if (gameObjTarget->getLootState() == GO_READY && gameObjTarget->GetPlayersAllowedToLoot().empty())
                {
                    if (gameObjTarget->GetMap()->IsDungeon() && !gameObjTarget->GetGOInfo()->chest.minSuccessOpens)
                    {
                        if (Group* group = player->GetGroup())
                        {
                            Map::PlayerList const& PlayerList = gameObjTarget->GetMap()->GetPlayers();
                            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                                if (Player* i_pl = i->GetSource())
                                    if (i_pl->GetGroup() == group)
                                        gameObjTarget->AddPlayerAllowedToLoot(i_pl->GetGUID());
                        }
                    }
                }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    // ToGC25 Boost
    if (m_originalCaster->GetMapId() == 649 && m_originalCaster->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC && m_originalCaster->IsCreature() && !m_originalCaster->IsControlledByPlayer())
    {
        switch (m_spellInfo->Id)
        {
        ...
        case SUMMON_CATEGORY_UNK:
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_TOTEM:
                {
                    ...
                    //! Do not scale for mythic totems that were spawned by a creature
                    //! let it get scaled by mythic scaling
                    if (!m_caster->IsPlayer())
                    {
                        InstanceScript* instance = m_caster->GetInstanceScript();
                        if (instance && instance->IsMythic() && summon->m_mythicScaling)
                        {
                            summon->m_mythicScaling->ScaleToKeyLevel();
                            break;
                        }
                    }
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget->ToCreature() && unitTarget->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PICKPOCKET)
        return;
...
void Spell::EffectEnchantItemPerm(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        sTransmogificationMgr->AddToCollection(item_owner, itemTarget);
        sTransmogificationMgr->AddToCollectionEnchant(item_owner, enchant_id);
    }
}
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetEntry() == 33846)
        if (m_caster->GetMap()->Is25ManRaid())
            weaponDamage = urand(30000, 40000);
...
void Spell::EffectHealMaxHealth(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage == 0)
    {
        ...
        if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_PALADIN_LAY_ON_HANDS)
        {
            ...
            { //Fix for Mortal Strike and similar spells
...
void Spell::EffectInterruptCast(SpellEffIndex effIndex)
{
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_AUTOREPEAT_SPELL; ++i)
    {
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
            ...
            uint32 interruptFlags         = curSpellInfo->InterruptFlags;
            if (SpellValue const* spellValue = spell->GetSpellValue())
                if (spellValue->SpellInterruptFlags != -1)
                    interruptFlags = spellValue->SpellInterruptFlags;

            // check if we can interrupt spell
            if ((spell->getState() == SPELL_STATE_CASTING
                    || (spell->getState() == SPELL_STATE_PREPARING && spell->GetCastTime() > 0.0f))
                && curSpellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE
                && ((i == CURRENT_GENERIC_SPELL && interruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT)
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if (pGameObj->GetGoType() == GAMEOBJECT_TYPE_FLAGDROP)
        if (Player* player = m_caster->ToPlayer())
            if (Battleground* bg = player->GetBattleground())
                bg->SetDroppedFlagGUID(pGameObj->GetGUID(), player->GetBgTeam() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 54640:
                {
                    ...
                    {
                        uint32 spellId = m_spellInfo->Effects[effIndex].CalcValue();
...
void Spell::EffectAddComboPoints(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_caster->m_movedByPlayer || !unitTarget || damage <= 0 && m_caster->GetEntry() != 30248)
        return;
...
void Spell::EffectSummonPlayer(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget->ToPlayer()->IsPathOfWar())
        return;
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
    ...
    {
        destTarget->GetPosition(x, y, z);
        if (m_spellInfo->Targets == 0 && m_caster->IsPlayer() && m_caster->isMoving() && !m_caster->IsWalking())
        {
            x = x + (8.0f / 3.0f) * std::cos(m_caster->GetOrientation());
            y = y + (8.0f / 3.0f) * std::sin(m_caster->GetOrientation());
            m_caster->UpdateAllowedPositionZ(x, y, z);
        }
    }
...
void Spell::EffectResurrect(SpellEffIndex effIndex)
{
    ...
    target->setResurrectRequestData(m_caster->GetGUID(), m_caster->GetMapId(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ() + 0.5f, health, mana);
    SendResurrectRequest(target);
}
...
void Spell::EffectAddExtraAttacks(SpellEffIndex effIndex)
{
    ...
    Unit* target = [this]() -> Unit* {
        if (m_triggeredByAuraSpell)
            if (Aura* aura = unitTarget->GetAura(m_triggeredByAuraSpell->Id))
                if (Unit* unit = ObjectAccessor::GetUnit(*unitTarget, aura->ExtraAttackTargetGUID))
                    return unit;

        return unitTarget->GetVictim();
    }();
    ...
    ExtraAttackTargetGUID      = target->GetGUID();
    unitTarget->m_extraAttacks = damage;

    ExecuteLogEffectExtraAttacks(effIndex, target, damage);
}
...
void Spell::EffectSelfResurrect(SpellEffIndex effIndex)
{
    ...
    // Reincarnation reagent taking
    if (m_spellInfo->Id == 21169 && !player->HasAura(58059))
    {
        uint32 reagentID = 17030, count = 1;

        if (player->HasItemCount(reagentID, count, false))
            player->DestroyItemCount(reagentID, count, true);
    }
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        ...
        if (m_pathFinder)
        {
            m_caster->GetMotionMaster()->MoveCharge(m_pathFinder->GetEndPosition().x - correction, m_pathFinder->GetEndPosition().y - correction, m_pathFinder->GetEndPosition().z, 42.0f, EVENT_CHARGE, &m_pathFinder->GetPath());
        }
        else
        {
            Position pos;
            ...
            m_caster->GetMotionMaster()->MoveCharge(pos.m_positionX - correction, pos.m_positionY - correction, pos.m_positionZ + 0.5f);
        }
    }
    ...
    if (effectHandleMode == SPELL_EFFECT_HANDLE_HIT_TARGET)
    {
        ...
        {
            if (m_caster->isFeared() || m_caster->isStuned() || m_caster->IsPolymorphed() || m_caster->IsCharmed() || m_caster->isPossessed())
                return;
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Effects[effIndex].TargetA.GetCheckType() != TARGET_CHECK_ENTRY && m_spellInfo->Effects[effIndex].TargetB.GetCheckType() != TARGET_CHECK_ENTRY && m_spellInfo->Id != 62376 && m_spellInfo->Id != 66683 && m_spellInfo->Id != 67660 && m_spellInfo->Id != 67661 && m_spellInfo->Id != 67662) // massive crash Northrend Beasts
    {
...
void Spell::EffectPullTowards(SpellEffIndex effIndex)
{
    ...
    Position pos;
    m_caster->GetFirstCollisionPosition(pos, m_caster->GetCombatReach(), m_caster->GetRelativeAngle(unitTarget));
    ...
    {
        sLog->outError("Spell %u with SPELL_EFFECT_PULL_TOWARDS called with invalid speedZ.", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPullTowardsDest(SpellEffIndex effIndex)
{
    ...
    if (!m_targets.HasDst())
    {
        sLog->outError("Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST has no dest target.", m_spellInfo->Id);
        return;
    }
    ...
    float posZ = pos->GetPositionZ();
    m_caster->UpdateAllowedPositionZ(pos->GetPositionX(), pos->GetPositionY(), posZ);

    float distZ = posZ - unitTarget->GetPositionZ();
    ...
    if (!std::isfinite(speedZ))
    {
        sLog->outError("Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST called with invalid speedZ.", m_spellInfo->Id);
        return;
    }

    unitTarget->GetMotionMaster()->MoveJump(pos->GetPositionX(), pos->GetPositionY(), posZ, speedXY, speedZ);
}
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if ((aura->GetSpellInfo()->GetDispelMask()) & dispelMask)
        {
            ...
            if (!aurApp->IsPositive() || aura->IsPassive() || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_NOT_STEALABLE)
                || aura->IsSpellStealAndDispelDisabled())
                continue;
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        {
            MotionMaster* mm = summon->GetMotionMaster();

            if (mm->size() <= 1) //Initialization of the summon didn't set Motion Master in any way, set to follow
            {
                mm->Clear(false);
    ...
    if (summon)
    {
        switch (m_spellInfo->Id)
        {
            ...
            case 1122:
                ...
                caster->SetPetGUID(summon->GetGUID());
                // Demonic Knowledge
                for (uint32 auraId : { 35691, 35692, 35693 })
                    if (Aura* aura = caster->GetAura(auraId))
                        if (PetAura const* petSpell = sSpellMgr->GetPetAura(auraId, EFFECT_0))
                            caster->AddPetAura(petSpell);
                break;
...
void Spell::EffectRenamePet(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || !unitTarget->IsPet() || unitTarget->ToPet()->getPetType() != HUNTER_PET)
        return;
...
void Spell::EffectActivateSpec(SpellEffIndex /*effIndex*/)
{
    ...
    uint8 spec = uint8(damage);
    if (uint8 specsGroup = player->GetSpecsGroup())
        spec += specsGroup * CLIENT_MAX_TALENT_SPECS;

    player->ActivateSpec(spec - 1); // damage is 1 or 2, spec is 0 or 1
}
...
void Spell::EffectCastButtons(SpellEffIndex effIndex)
{
    ...
    {
        ActionButton const* ab = caster->GetActionButton(button_id);
        if (!ab || ab->GetType() != ACTION_BUTTON_SPELL)
            continue;

        //! Action button data is unverified when it's set so it can be "hacked"
        //! to contain invalid spells, so filter here.
        if (uint32 spellId = ab->GetAction())
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
                spellIds.emplace_back(spellId);

        auto IsStoneclawSummonSpell = [](uint32 summonSpellId) {
        ...
        // Stoneclaw Totem should be summoned at the end
        auto it = std::find_if(std::begin(spellIds), std::end(spellIds), [&](uint32 itrSpellId) { return IsStoneclawSummonSpell(itrSpellId); });
        if (it != std::end(spellIds))
            std::rotate(it, std::next(it), std::end(spellIds));
    }
    ...
    for (uint32 spellId : spellIds)
    {
        ...
        if (!caster->HasSpell(spellId) || caster->HasSpellCooldown(spellId))
            continue;

        if (!spellInfo->HasAttribute(SPELL_ATTR7_SUMMON_PLAYER_TOTEM))
            continue;

        uint32 cost = spellInfo->CalcPowerCost(caster, spellInfo->GetSchoolMask(), this);
        if (caster->GetPower(POWER_MANA) < cost)
            continue;

        caster->CastSpell(caster, spellId, TriggerCastFlags(TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CAST_IN_PROGRESS | TRIGGERED_CAST_DIRECTLY));
    }
}
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    // Temp! For DK after Atlantiss Transfer
    if (m_spellInfo->Id == 53821 && player->HasSpell(50977))
        return;
