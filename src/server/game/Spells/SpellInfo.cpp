...
SpellImplicitTargetInfo::SpellImplicitTargetInfo(uint32 target)
{
    ...
    {
        sLog->outError("Spell is using target value of %d, but max supported target value is %d", _target, TOTAL_SPELL_TARGETS);
        _target = Targets(0);
    }
}
...
SpellEffectInfo::SpellEffectInfo(SpellEntry const* spellEntry, SpellInfo const* spellInfo, uint8 effIndex)
{
    ...
    {
        sLog->outError("Spell is using effect value of %d, but max supported effect value is %d", Effect, TOTAL_SPELL_EFFECTS);
        Effect = 0;
    }
...
{
    if (Effect == SPELL_EFFECT_APPLY_AREA_AURA_PARTY || Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID || Effect == SPELL_EFFECT_APPLY_AREA_AURA_FRIEND || Effect == SPELL_EFFECT_APPLY_AREA_AURA_ENEMY || Effect == SPELL_EFFECT_APPLY_AREA_AURA_PET || Effect == SPELL_EFFECT_APPLY_AREA_AURA_OWNER)
        return true;
    return false;
}
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target) const
{
    ...
    Unit const* source = caster;
    if (!source)
        source = target;

    if (source)
    {
        std::optional<int32> value;
        if (auto const boostSpellType = sBoostMgr->GetBoostSpellTypeForEffect(static_cast<SpellEffIndex>(_effIndex)); boostSpellType != BoostSpellType::None)
            if (BoostSpellValue const* spellValue = sBoostMgr->GetBoostSpellValue(boostSpellType, _spellInfo, source))
                value = irand(spellValue->ValueMin, spellValue->ValueMax);

        if (value)
            if (auto const boostSpellTypeModifier = sBoostMgr->GetBoostSpellTypeForEffectModifier(static_cast<SpellEffIndex>(_effIndex)); boostSpellTypeModifier != BoostSpellType::None)
                if (BoostSpellValue const* spellValue = sBoostMgr->GetBoostSpellValue(boostSpellTypeModifier, _spellInfo, source))
                    value = AddPct(value.value(), irand(spellValue->ValueMin, spellValue->ValueMax));
    ...
    if (caster)
    {
        ...
        bool isControlledByPlayerAndCanScale = false;

        if (caster->GetTypeId() == TYPEID_PLAYER)
        {
            switch (caster->GetEntry())
            {
                case 28985: // Haiphoon, the Great Tempest
                case 28999: // Haiphoon, the Great Tempest
                    isControlledByPlayerAndCanScale = true;
                    break;
        ...
        if ((_spellInfo->SpellLevel && _spellInfo->SpellLevel != caster->getLevel() && !basePointsPerLevel && _spellInfo->HasAttribute(SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION))
            || isControlledByPlayerAndCanScale)
        {
    ...
    if (source)
        if (auto const boostSpellTypeModifier = sBoostMgr->GetBoostSpellTypeForEffectModifier(static_cast<SpellEffIndex>(_effIndex)); boostSpellTypeModifier != BoostSpellType::None)
            if (BoostSpellValue const* spellValue = sBoostMgr->GetBoostSpellValue(boostSpellTypeModifier, _spellInfo, source))
                value = AddPct(value, irand(spellValue->ValueMin, spellValue->ValueMax));
...
bool SpellInfo::IsSpellWithDelayableEffects() const
{
    if (HasAttribute(SPELL_ATTR0_CU_AURA_CC))
        return true;

    // Flash of Light triggers another spell to do the heal.
    if (SpellFamilyName == SPELLFAMILY_PALADIN && SpellIconID == 242)
        return true;
    ...
    if (SpellFamilyName == SPELLFAMILY_WARRIOR && SpellFamilyFlags.IsEqual(0x20000000, 0x0, 0x0))
        return true;

    if (IsChanneled() || HasAttribute(SPELL_ATTR0_ON_NEXT_SWING) || HasAttribute(SPELL_ATTR0_ON_NEXT_SWING_2) || HasAttribute(SPELL_ATTR0_REQ_AMMO))
        return false;

    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (Effects[i].IsEffect() && !IsDelayableEffect(i))
            return false;

    return true;
}

bool SpellInfo::IsDelayableEffect(uint32 effect) const
{
    switch (Effects[effect].Effect)
    {
        case SPELL_EFFECT_SCHOOL_DAMAGE:
        case SPELL_EFFECT_HEALTH_LEECH:
        case SPELL_EFFECT_HEAL:
        ...
        {
            switch (Effects[effect].ApplyAuraName)
            {
...
bool SpellInfo::CanCastWhileDead() const
{
    return Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD;
}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    // arena limitation
    if (AttributesEx6 & SPELL_ATTR6_ONLY_IN_ARENA)
    {
        MapEntry const* mapEntry = sMapStore.LookupEntry(map_id);
        if (!mapEntry)
            return SPELL_FAILED_INCORRECT_AREA;
...
bool SpellInfo::IsStrongerAuraActive(Unit const* caster, Unit const* target) const
{
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
        {
            ...
            int32 auraValue = (sFlag & SPELL_GROUP_SPECIAL_FLAG_BASE_AMOUNT_CHECK) ? abs((*iter)->GetSpellInfo()->Effects[(*iter)->GetEffIndex()].CalcValue((*iter)->GetCaster())) : abs((*iter)->GetAmount());
...
bool SpellInfo::IsSpellRemovedOnEvade() const
{
    for (AuraType auraType : { SPELL_AURA_FLY, SPELL_AURA_CONTROL_VEHICLE, SPELL_AURA_CLONE_CASTER, SPELL_AURA_INITIALIZE_IMAGES })
        if (HasAura(auraType))
            return false;

    return !HasAttribute(SPELL_ATTR1_CU_PERSIST_AFTER_EVADE);
}
...
bool SpellInfo::ValidateAttribute6SpellDamageMods(const Unit* caster, const AuraEffect* auraEffect, bool isDot) const
{
    ...
    if (Id == 70890 && auraEffect)
    {
        ...
        return auraInfo->SpellIconID == 3086 || (auraInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && (auraInfo->SpellFamilyFlags & flag96(8388608, 64, 16) || auraInfo->SpellIconID == 235 || auraInfo->SpellIconID == 154));
    }
...
AuraStateType SpellInfo::LoadAuraState() const
{
    ...
    if (Id == 9991 || Id == 35331 || Id == 9806 || Id == 35325)
        return AURA_STATE_FAERIE_FIRE;
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell, uint32 manaPercentCu) const
{
    ...
    if (manaPercentCu == 0) manaPercentCu = ManaCostPercentage;
    ...
    if (manaPercentCu)
    {
        switch (PowerType)
        {
            ...
            case POWER_HEALTH:
                powerCost += int32(CalculatePct(caster->GetCreateHealth(), manaPercentCu));
                break;
            case POWER_MANA:
                powerCost += int32(CalculatePct(caster->GetCreateMana(), manaPercentCu));
                break;
            case POWER_RAGE:
            case POWER_FOCUS:
            case POWER_ENERGY:
            case POWER_HAPPINESS:
                powerCost += int32(CalculatePct(caster->GetMaxPower(Powers(PowerType)), manaPercentCu));
                break;
            case POWER_RUNE:
            case POWER_RUNIC_POWER:; //sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "CalculateManaCost: Not implemented yet!");
                break;
            default:
...
SpellInfo const* SpellInfo::GetAuraRankForLevel(uint8 level) const
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (IsPositiveEffect(i) && (Effects[i].Effect == SPELL_EFFECT_APPLY_AURA || Effects[i].Effect == SPELL_EFFECT_APPLY_AREA_AURA_PARTY || Effects[i].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID))
        {
