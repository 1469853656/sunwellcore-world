...
enum SpellFamilyFlag
{
    ...
    // Paladin
    SPELLFAMILYFLAG_PALADIN_LAY_ON_HANDS = 0x00008000,
...
enum ProcFlags
{
    ...
    PROC_FLAG_NO_OVERHEALED = 0x02000000, // 25 Heal with no overhealed
...
enum ProcFlagsExLegacy
{
    ...
    PROC_EX_EX_ONE_TIME_TRIGGER = 0x0020000, // Trigers on first periodic tick
...
const uint32 SPELL_FAKE_DELAY = 40u;
...
struct SpellProcEventEntry
{
    ...
    uint32 procPhase;       // proc phase (cast, hit, finish)
...
class SpellMgr : public Sunwell::Singleton<SpellMgr>
{
    friend class Sunwell::Singleton<SpellMgr>;
    // Constructors
private:
    SpellMgr();
    ~SpellMgr();
...
public:
    ...
    bool IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, uint32 procPhase, bool active) const;
    ...
    std::vector<SpellInfo*> LevelUpSpellsCache;
