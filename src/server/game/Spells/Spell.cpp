...
SpellValue::SpellValue(SpellInfo const* proto)
{
    ...
    ForcedMissResult    = -1;
    SpellInterruptFlags = -1;
    IsSpellStealAndDispelDisabled = false;
    ...
    , m_spellValue(new SpellValue(m_spellInfo))
{
    ...
    if (m_spellValue->IsReflectDisabled)
        m_canReflect = false;

    if (m_caster && m_caster->GetMapId() == 230 /*hellforge*/ && m_caster->IsCreature())
    {
        if (m_caster->GetMap()->GetDifficulty() == DUNGEON_DIFFICULTY_HEROIC)
            m_canReflect = false;
    }
...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32& processedEffectMask)
{
    ...
    switch (targetType.GetSelectionCategory())
    {
        ...
        case TARGET_SELECT_CATEGORY_DEFAULT:
            ...
            break;
        case TARGET_SELECT_CATEGORY_NYI:; //sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "SPELL: target type %u, found in spellID %u, effect %u is not implemented yet!", m_spellInfo->Id, effIndex, targetType.GetTarget());
            break;
        default:
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (auto boostSpellValue = sBoostMgr->GetBoostSpellValue(sBoostMgr->GetBoostSpellTypeForEffectRadius(effIndex), m_spellInfo, m_caster))
        radius = frand(boostSpellValue->ValueMin, boostSpellValue->ValueMax);
...
void Spell::SelectImplicitChainTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, WorldObject* target, uint32 effMask)
{
    ...
    if (maxTargets > 1)
    {
        ...
        std::list<WorldObject*> targets;
        SearchChainTargets(targets, maxTargets - 1, target, targetType.GetObjectType(), targetType.GetCheckType(), targetType.GetSelectionCategory(), m_spellInfo->Effects[effIndex].ImplicitTargetConditions, targetType.GetTarget() == TARGET_UNIT_TARGET_CHAINHEAL_ALLY);
...
uint32 Spell::GetSearcherTypeMask(SpellTargetObjectTypes objType, ConditionList* condList)
{
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR3_ONLY_TARGET_PLAYERS) || m_spellValue->TargetPlayersOnly)
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
    Trinity::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList, m_spellInfo->HasAttribute(SPELL_ATTR1_CU_EXCLUDE_PLAYER_SIZE));
...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/)
{
    ...
    if (m_originalCaster)
    {
        ...
        if (m_spellValue->ForcedMissResult != -1)
            targetInfo.missCondition = static_cast<SpellMissInfo>(m_spellValue->ForcedMissResult);
    }
    else
    ...
    if (m_spellInfo->Speed > 0.0f && m_caster != target)
    {
        ...
        m_delayMoment = targetInfo.timeDelay = GetSpellBatchingEffectDelay(target);
    else
    ...
    if (targetInfo.missCondition == SPELL_MISS_REFLECT)
    {
        ...
        m_caster->m_Events.AddEvent(new ReflectEvent(m_caster->GetGUID(), targetInfo.targetGUID, m_spellInfo, m_caster->FindMap()), m_caster->m_Events.CalculateTime(targetInfo.timeDelay));
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (getState() == SPELL_STATE_DELAYED && !m_spellInfo->IsTargetingArea() && !m_spellInfo->IsPositive() && (World::GetGameTimeMS() - target->timeDelay) <= effectUnit->m_lastSanctuaryTime && World::GetGameTimeMS() < (effectUnit->m_lastSanctuaryTime + 500) && effectUnit->FindMap() && !effectUnit->FindMap()->IsDungeon())
    ...
    if (spellHitTarget)
    {
        SpellMissInfo missInfo2 = DoSpellHitOnUnit(spellHitTarget, mask, target->scaleAura, reflectedSpell);
        ...
        }   /// Teleport player to target when his charge is interrupted by spell and he stops moving
            /// TODO: add more types of spells
        else if (spellHitTarget && spellHitTarget->IsPlayer() && m_caster != spellHitTarget && spellHitTarget->ChargeFinalPos && spellHitTarget->GetMotionMaster()->IsCharging() && m_spellInfo->HasAura(SPELL_AURA_MOD_ROOT))
        {
            Position chargePos      = *spellHitTarget->ChargeFinalPos;
            chargePos.m_orientation = spellHitTarget->GetOrientation();
            spellHitTarget->NearTeleportTo(chargePos, true);
            spellHitTarget->ChargeFinalPos = {};
        }
    }
    ...
    if (m_healing > 0)
    {
        ...
        if (canEffectTrigger)
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, PROC_SPELL_PHASE_HIT, addhealth, m_attackType, m_spellInfo, m_triggeredByAuraSpell);
    }
    ...
    else if (m_damage > 0)
    {
        ...
        if (m_spellValue->DamageOverride)
        {
            m_damage = *m_spellValue->DamageOverride;
            damageInfo.damage = *m_spellValue->DamageOverride;
        }

        if (m_spellValue->DamagePctOverride)
        {
            m_damage *= *m_spellValue->DamagePctOverride;
            damageInfo.damage *= *m_spellValue->DamagePctOverride;
        }
        ...
        if (canEffectTrigger)
        {
            uint32 preMitigationDamage = damageInfo.damage + damageInfo.absorb + damageInfo.resist;
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, PROC_SPELL_PHASE_HIT, damageInfo.damage, m_attackType, m_spellInfo, m_triggeredByAuraSpell, preMitigationDamage);
            if (caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->HasAttribute(SPELL_ATTR0_STOP_ATTACK_TARGET) == 0 && m_spellInfo->HasAttribute(SPELL_ATTR4_CANT_TRIGGER_ITEM_SPELLS) == 0 && (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
            {
                caster->ToPlayer()->CastItemCombatSpell(unitTarget, m_attackType, procVictim, procEx);
                // Mutilate, proc mainhand poison twice
                if (m_spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE && m_spellInfo->SpellFamilyFlags.IsEqual(0x0, 0x2, 0x0))
                    caster->ToPlayer()->CastItemCombatSpell(unitTarget, m_attackType, procVictim, procEx);
            }
        }
    ...
    else
    {
        ...
        if (canEffectTrigger)
        {
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, PROC_SPELL_PHASE_HIT, 0, m_attackType, m_spellInfo, m_triggeredByAuraSpell);
            ...
            if ((procEx & PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT) && caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->HasAttribute(SPELL_ATTR0_STOP_ATTACK_TARGET) == 0 && m_spellInfo->HasAttribute(SPELL_ATTR4_CANT_TRIGGER_ITEM_SPELLS) == 0 && (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
    ...
    if (missInfo != SPELL_MISS_EVADE && !m_caster->IsFriendlyTo(effectUnit) && !m_spellInfo->HasAura(SPELL_AURA_DAMAGE_SHIELD) && !m_spellInfo->HasAura(SPELL_AURA_BIND_SIGHT) && (!m_spellInfo->IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
    {
    ...
    auto GetPvPAttackerForHelper = [](Unit* src) -> Unit*
    {
        if (!src->IsInCombat())
            return nullptr;

        if (Unit* victim = src->GetVictim())
            if (victim->IsInCombat() && (src->getAttackers().find(victim) != std::end(src->getAttackers()) || victim->getAttackers().find(src) != std::end(victim->getAttackers())))
                return victim;

        if (!src->getAttackers().empty())
            return *(src->getAttackers().begin());

        return nullptr;
    };

    if (caster && unitTarget && caster->IsInWorld() && unitTarget->IsInWorld() && missInfo == SPELL_MISS_NONE && caster->IsPlayer() && unitTarget->IsControlledByPlayer() && caster->GetGUID() != unitTarget->GetGUID() && unitTarget->GetOwnerGUID() != caster->GetGUID())
        if (m_caster->IsFriendlyTo(unitTarget) && unitTarget->IsInCombat() && m_spellInfo->IsPositive() && !m_triggeredByAuraSpell && !m_spellInfo->HasAttribute(SPELL_ATTR1_NO_THREAT) && !m_spellInfo->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO))
        {
            unitTarget->getHostileRefManager().threatAssist(m_caster, 0.0f);
            // Needed for PvP combat
            if (unitTarget->IsPvP())
                if (Unit* hostile = GetPvPAttackerForHelper(unitTarget))
                    caster->CombatStartOnCast(hostile, true, GetDelayMoment() + 500);
        }
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura, bool wasReflected)
{
    if (!unit || !effectMask)
        return SPELL_MISS_EVADE;

    // For delayed spells immunity may be applied between missile launch and hit - check immunity for that case
    if (_delayed && ((m_damage > 0 && unit->IsImmunedToDamage(m_spellInfo)) || unit->IsImmunedToSchool(m_spellInfo) || unit->IsImmunedToSpell(m_spellInfo)) || unit->HasAura(57835))
    ...
    if (m_caster != unit)
    {
        ...
        if (_delayed)
        {
            ...
            // allow Cloak of Shadows to resist already flying spells
            constexpr uint32 SPELL_CLOAK_OF_SHADOWS = 31224;
            bool isChaosBolt                        = m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK && m_spellInfo->SpellFamilyFlags.IsEqual(0x0, 0x00020000, 0x0);
            if ((m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_NONE || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC) && m_spellInfo->SchoolMask != SPELL_SCHOOL_MASK_NORMAL && !isChaosBolt)
                if (AuraEffect* aurEff = unit->GetAuraEffect(SPELL_CLOAK_OF_SHADOWS, EFFECT_0))
                    if (urand(0, 100) < std::abs(aurEff->GetBaseAmount()))
        ...
        else if (m_caster->IsFriendlyTo(unit))
        {
            ...
            if (!IsTriggered() && _delayed && unit->GetTypeId() == TYPEID_PLAYER && !m_spellInfo->IsPositive())
                return SPELL_MISS_EVADE;
        ...
        else if (_delayed && unit == m_targets.GetUnitTarget())
        {
    ...
    if (creatureCaster && creatureCaster->m_mythicScaling != nullptr)
        m_diminishGroup = DIMINISHING_NONE;
    else
        m_diminishGroup = GetDiminishingReturnsGroupForSpell(m_spellInfo, m_triggeredByAuraSpell); // xinef: do not increase diminish level for bosses (eg. Void Reaver silence is never diminished)

    if (aura_effmask && m_diminishGroup && (unit != m_caster || wasReflected) && (m_caster->GetTypeId() != TYPEID_UNIT || !m_caster->ToCreature()->isWorldBoss()))
    {
        m_diminishLevel             = unit->GetDiminishing(m_diminishGroup);
        DiminishingReturnsType type = GetDiminishingReturnsGroupType(m_diminishGroup);

        uint32 flagsExtra    = unit->GetTypeId() == TYPEID_UNIT ? unit->ToCreature()->GetCreatureTemplate()->flags_extra : 0;
        bool shouldHaveAllDr = unit->IsCreature() ? unit->ToCreature()->IsSubjectToAllDiminishingReturns() : false;
        ...
        if ((type == DRTYPE_PLAYER
                && (unit->GetCharmerOrOwnerPlayerOrPlayerItself()
                       || shouldHaveAllDr
                       || (m_diminishGroup == DIMINISHING_TAUNT && m_caster->IsPlayer() && (flagsExtra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH))))
            || type == DRTYPE_ALL)
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                if (m_spellValue->IsSpellStealAndDispelDisabled)
                    m_spellAura->DisableSpellStealAndDispel();
                ...
                if (auto const spellBoostValue = sBoostMgr->GetBoostSpellValue(BoostSpellType::AuraDuration, m_spellInfo, m_originalCaster ? m_originalCaster : m_caster))
                    duration = irand(spellBoostValue->ValueMin, spellBoostValue->ValueMax);
                else if (int32 _customDuration = m_spellValue->AuraDuration)
            ...
            if (CanExecuteTriggersOnHit(effMask, i->first) && roll_chance_i(i->second))
             {
                ...
                if (i->first->GetDuration() == -1)
                 {
                    int32 oldDuration = 0;
                    if (Aura* oldTriggeredAura = unit->GetAura(i->first->Id, m_caster->GetGUID()))
                        oldDuration = oldTriggeredAura->GetDuration();

                    m_caster->CastSpell(unit, i->first, true);

                    if (Aura* triggeredAura = unit->GetAura(i->first->Id, m_caster->GetGUID()))
                     {
                        Aura* aur = unit->GetAura(m_spellInfo->Id, m_caster->GetGUID());
                        int32 duration = aur ? aur->GetDuration() : 0;
                        duration = std::max(duration, oldDuration);

                        triggeredAura->SetDuration(duration);
                     }
                 }
                else
                   m_caster->CastSpell(unit, i->first, true);
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    _delayed = m_spellInfo->Speed > 0.0f || (!IsTriggered() && m_caster->IsPlayer() && m_spellInfo->IsSpellWithDelayableEffects());
    ...
    if (m_CastItem) // Fix for Convocation at Zol'Heb
    {
...
void Spell::_cast(bool skipCheck)
{
    ...
    if (m_originalCaster)
    {
        ...
        if (!procAttacker)
        {
            bool IsPositive = GetSpellInfo()->IsPositive();
        ...
        m_originalCaster->ProcDamageAndSpell(m_originalCaster, procAttacker, PROC_FLAG_NONE, procEx, PROC_SPELL_PHASE_CAST, 1, BASE_ATTACK, m_spellInfo, m_triggeredByAuraSpell);
    }
    ...
    if (((_delayed || m_delayMoment) && !m_spellInfo->IsChanneled()) /* xinef: we dont need this shit || m_spellInfo->Id == 14157*/)
    {
    ...
    SetExecutedCurrently(false);
    bool canResetAutoActions = !m_spellInfo->HasAttribute(SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS) && !IsTriggered() && !m_triggeredByAuraSpell;
    // Maelstrom Weapon
    if (canResetAutoActions)
        canResetAutoActions = m_caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_SHAMAN, 0x0, 0x0, 0x00000200, m_caster->GetGUID());

    if (m_casttime && canResetAutoActions)
    {
    ...
    // Remove spellmods if spell is channeled, effects are already applied so we don't need to keep them
    if (IsChannelActive() && m_spellInfo->IsChanneled())
        if (Player* player = m_caster->GetSpellModOwner())
            player->RemoveSpellMods(this);
}
...
uint64 Spell::handle_delayed(uint64 t_offset)
{
    ...
    if (m_caster->IsPlayer() && m_customDelay && sWorld->getBoolConfig(CONFIG_EXPERIMENTAL_FEATURE))
    {
        SpellCastResult castResult = CheckCast(true);
...
void Spell::_handle_finish_phase()
{
    ...
    if (m_caster->m_extraAttacks && GetSpellInfo()->HasEffect(SPELL_EFFECT_ADD_EXTRA_ATTACKS))
        if (Unit* extraAttackTarget = ObjectAccessor::GetUnit(*m_caster, ExtraAttackTargetGUID))
            m_caster->HandleProcExtraAttackFor(extraAttackTarget);
    ...
    m_originalCaster->ProcDamageAndSpell(m_originalCaster, procAttacker, PROC_FLAG_NONE, m_procEx, PROC_SPELL_PHASE_FINISH, 1, BASE_ATTACK, m_spellInfo, m_triggeredByAuraSpell);
}
...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) && m_caster->isMoving() && (m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT) && m_spellState == SPELL_STATE_PREPARING && (m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK || !m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR)))
    {
...
void Spell::finish(bool ok)
{
            ...
            if (m_spellInfo->IsCooldownStartedOnEvent())
            {
                switch (m_castError)
                {
                    ...
                    case SPELL_FAILED_NOT_READY:
                        ...
                        // no break;
                    default:
                        m_caster->ToPlayer()->SendCooldownEvent(m_spellInfo, 0, nullptr, false);
                        break;
                }
            }
        }
    ...
    if (IsAutoActionResetSpell())
    {
        ...
        if (!found && !m_spellInfo->HasAttribute(SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS))
        {
            ...
            if (allow)
                allow = m_caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_SHAMAN, 0x0, 0x0, 0x00000200, m_caster->GetGUID());
...
void Spell::TakePower()
{
    ...
    {
        // Failed hits should take 20% of energy by default
        int32 powerCost = int32(0.2f * -m_powerCost);
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_WARRIOR:
            {
                // Execute should take 50% of rage if not immuned (20% if immuned)
                if (!immune && m_spellInfo->SpellFamilyFlags[0] & 0x20000000)
                    powerCost = int32(0.5f * -m_powerCost);
                break;
            }
            case SPELLFAMILY_DRUID:
            {
                // Feral Druid finishers should refund energy only with talent Primal Precision
                // m_powerCost is already modified if druid has this talent, take full cost here
                if (m_spellInfo->SpellFamilyFlags[0] & 0x00800000 || m_spellInfo->SpellFamilyFlags[1] & 0x10000080)
                    powerCost = -m_powerCost;
                break;
            }
            case SPELLFAMILY_ROGUE:
            {
                // Rogue finishers should refund energy only with talent Quick Recovery
                // Handled in Unit::EnergizeBySpell, take full cost here
                if (m_spellInfo->SpellFamilyFlags[0] & 0x003E0000 || m_spellInfo->SpellFamilyFlags[1] & 0x00000009)
                    powerCost = -m_powerCost;
                break;
            }
        }
...
void Spell::HandleThreatSpells()
{
    ...
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        bool const IsFriendly = m_caster->IsFriendlyTo(target);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE))
    {
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
        {
            //can cast triggered (by aura only?) spells while have this flag, allow to finish already queue next melee swing spells, maybe it should check SPELL_ATTR0_ABILITY?
            if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURASTATE) && m_caster->ToPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_ALLOW_ONLY_ABILITY) && !IsNextMeleeSwingSpell())
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->ToPlayer()->isMoving() && !IsTriggered())
    {
        ...
        if ((!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR) || m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK) && (IsAutoRepeat() || (m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED) != 0))
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            if (target->GetEntry() == 30245 || target->GetEntry() == 31750 || target->GetEntry() == 30249 || target->GetEntry() == 31751 || target->GetEntry() == 33142) // Fix for The Eye of Eternity disc's
            {
    ...
    {
        //Hunger Games Event
        if (m_caster->ToPlayer()->GetZoneId() == 268 && (m_spellInfo->Id == 21169 ||                                         //soulstone
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 48467:
            ...
            {
                ChatHandler(m_caster->ToPlayer()->GetSession()).SendSysMessage("You need to be on top of arena before you can use this spell.");
                return SPELL_FAILED_DONT_REPORT;
            }
            break;
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR4_NOT_USABLE_IN_ARENA) || (m_spellInfo->GetRecoveryTime() >= 10 * MINUTE * IN_MILLISECONDS && !m_spellInfo->HasAttribute(SPELL_ATTR4_USABLE_IN_ARENA)))
    ...
    if (m_caster->IsMounted() && m_caster->GetTypeId() == TYPEID_PLAYER && !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE) && !m_spellInfo->IsPassive() && !m_spellInfo->HasAttribute(SPELL_ATTR0_CASTABLE_WHILE_MOUNTED))
    {
    ...
    if (!hasNonDispelEffect && !hasDispellableAura && dispelMask && !IsTriggered())
    {
        if (Unit* target = m_targets.GetUnitTarget())
        {
            if ((m_caster->IsInSanctuary() || target->IsInSanctuary()) && !m_caster->IsFriendlyTo(target) && !m_caster->IsValidAttackTarget(target))
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            case SPELL_EFFECT_DUMMY:
            {
                ...
                else if (m_spellInfo->Id == 53271) // Master's Call
                {
                    ...
                    if (!target || !pet || pet->isDead() || target->isDead() || pet->isStuned())
                        return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                // pussywizard:
                if (World::IsPathfindingEnabled(m_caster->FindMap(), true) && m_spellInfo->NeedsExplicitUnitTarget())
                {
                    Unit* target = m_targets.GetUnitTarget();
                    if (!target)
                        return SPELL_FAILED_BAD_TARGETS;

                    float angle = m_caster->GetAngle(target) - M_PI;
                    ...
                    if (m_caster->GetMapId() == 618) // pussywizard: 618 Ring of Valor
                        destz = std::max(destz, 28.28f);

                    float maxdist = MELEE_RANGE + m_caster->GetMeleeReach() + target->GetMeleeReach();
                    if (target->GetExactDistSq(destx, desty, destz) > maxdist * maxdist)
                    ...
                    {
                        if (destx < 1275.0f || m_caster->GetPositionX() < 1275.0f) // special case (acid)
                    ...
                    {
                        if (target->GetPositionZ() - m_pathFinder->GetActualEndPosition().z > 5.0f)
                            return SPELL_FAILED_NOPATH;
                    }
                }
                break;
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                InstanceScript* instance = m_caster->GetInstanceScript();
                //! If instance is mythic and is not yet started (timer is not running and instance is not yet finished)
                if (instance && instance->IsMythic() && (!instance->IsMythicTimerRunning() && !instance->IsMythicDone()))
                    return SPELL_FAILED_BAD_TARGETS;

                ItemRef pTempItem = nullptr;
                if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM)
                {
                ...
                if (m_spellInfo->Effects[i].TargetA.GetTarget() == TARGET_GAMEOBJECT_ITEM_TARGET && !m_targets.GetGOTarget() && (!pTempItem || !pTempItem->GetTemplate()->LockID || !pTempItem->IsLocked()))
                    return SPELL_FAILED_BAD_TARGETS;
                ...
                // chance for fail at lockpicking
            ...
            case SPELL_EFFECT_SUMMON_RAF_FRIEND:
            {
                ...
                if (target->ToPlayer()->GetSession()->GetRecruiterId() != m_caster->ToPlayer()->GetSession()->GetAccountId() && target->ToPlayer()->GetSession()->GetAccountId() != m_caster->ToPlayer()->GetSession()->GetRecruiterId())
                    return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if (m_caster->getTransForm())
                    if (SpellInfo const* transformSpellInfo = sSpellMgr->GetSpellInfo(m_caster->getTransForm()))
                        if (transformSpellInfo->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) && !transformSpellInfo->HasAttribute(SpellAttr0(SPELL_ATTR0_CASTABLE_WHILE_MOUNTED | SPELL_ATTR0_NEGATIVE_1)))
                            return SPELL_FAILED_NOT_SHAPESHIFT;
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (m_spellValue->SpellRangeMin > 0.f)
        min_range = m_spellValue->SpellRangeMin;
    ...
    max_range += (strict ? (m_caster->IsPlayer() ? 1.25f : 0.0f) : (m_caster->IsPlayer() ? 6.25f : 2.25f)) + m_caster->GetLeewayBonusRange(target, true);
    ...
    if (target)
    {
        if (target != m_caster)
        {
            ...
            if (range_type == SPELL_RANGE_MELEE)
            {
                ...
                if (!m_caster->IsWithinMeleeRange(target, std::max(real_max_range, 0.0f), true) && !CanUseOutsideOfRange())
            ...
            {
                if (m_caster->IsWithinMeleeRange(target, true))
...
uint32 Spell::GetSpellBatchingEffectDelay(Unit const* target) const
{
    uint32 const spellBatchingDelay = sWorld->getIntConfig(CONFIG_SPELL_BATCHING_DELAY);
    return (spellBatchingDelay && target != m_caster) ? (spellBatchingDelay - (getMSTime() % spellBatchingDelay)) : 0;
}

bool Spell::IsNeedSendToClient(bool go) const
{
    return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] || m_spellInfo->IsChanneled() || m_spellInfo->Speed > 0.0f || (!m_triggeredByAuraSpell && !IsTriggered()) || (go && m_triggeredByAuraSpell && m_triggeredByAuraSpell->IsChanneled());
}
...
bool ReflectEvent::Execute(uint64 e_time, uint32 p_time)
{
    if (_map)
    {
        Unit* caster = _map->GetUnit(_casterGUID);
        Unit* target = _map->GetUnit(_targetGUID);
        if (caster && target && caster->IsInMap(target))
            caster->ProcDamageAndSpell(target, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, PROC_SPELL_PHASE_HIT, 1, BASE_ATTACK, _spellInfo);
    }
...
void Spell::SetSpellValue(SpellValueMod mod, int32 value)
{
    switch (mod)
    {
        ...
        case SPELLVALUE_SPELL_RANGE:
            ...
            break;
        case SPELLVALUE_SPELL_RANGE_MIN:
            m_spellValue->SpellRange = (float)value;
            break;
        ...
        case SPELLVALUE_AURA_STATE:
            ...
            break;
        case SPELLVALUE_FORCED_MISS_RESULT:
            m_spellValue->ForcedMissResult = value;
            break;
        case SPELLVALUE_SPELL_INTERRUPT_FLAGS:
            m_spellValue->SpellInterruptFlags = value;
            break;
        case SPELLVALUE_DISABLE_SPELL_STEAL_DISPEL:
            m_spellValue->IsSpellStealAndDispelDisabled = bool(value);
            break;
        case SPELLVALUE_DISABLE_REFLECT:
            m_spellValue->IsReflectDisabled = bool(value);
            break;
        case SPELLVALUE_DAMAGE_OVERRIDE:
            m_spellValue->DamageOverride = value;
            break;
        case SPELLVALUE_DAMAGE_PCT_OVERRIDE:
            m_spellValue->DamagePctOverride = value;
            break;
        default:
            break;
    }
...
bool Spell::CheckScriptEffectImplicitTargets(uint32 effIndex, uint32 effIndexToCheck)
{
    ...
    for (std::list<SpellScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end(); ++itr)
    {
        ...
        for (; targetSelectHookItr != targetSelectHookEnd; ++targetSelectHookItr)
            if (((*targetSelectHookItr).IsEffectAffected(m_spellInfo, effIndex) && !(*targetSelectHookItr).IsEffectAffected(m_spellInfo, effIndexToCheck)) || (!(*targetSelectHookItr).IsEffectAffected(m_spellInfo, effIndex) && (*targetSelectHookItr).IsEffectAffected(m_spellInfo, effIndexToCheck)))
                return false;
        ...
        for (; areaTargetSelectHookItr != areaTargetSelectHookEnd; ++areaTargetSelectHookItr)
            if (((*areaTargetSelectHookItr).IsEffectAffected(m_spellInfo, effIndex) && !(*areaTargetSelectHookItr).IsEffectAffected(m_spellInfo, effIndexToCheck)) || (!(*areaTargetSelectHookItr).IsEffectAffected(m_spellInfo, effIndex) && (*areaTargetSelectHookItr).IsEffectAffected(m_spellInfo, effIndexToCheck)))
                return false;
    }
    return true;
}
...
void Spell::TriggerGlobalCooldown()
{
    ...
    int32 realMaxGCD = m_caster->IsHunterPet() && GetSpellInfo()->PowerType == POWER_FOCUS ? MAX_GCD_HUNTER_PET : MAX_GCD; // Exception for hunter pet focus abilities, should use 1.25s GCD instead of 1.5s
...
WorldObjectSpellNearbyTargetCheck::WorldObjectSpellNearbyTargetCheck(float range, Unit* caster, SpellInfo const* spellInfo,
    SpellTargetCheckTypes selectionType, ConditionList* condList)
    : WorldObjectSpellTargetCheck(caster, caster, spellInfo, selectionType, condList)
    , _range(range)
    , _position(caster)
{
}
...
WorldObjectSpellAreaTargetCheck::WorldObjectSpellAreaTargetCheck(float range, Position const* position, Unit* caster,
    Unit* referer, SpellInfo const* spellInfo, SpellTargetCheckTypes selectionType, ConditionList* condList, bool excludePlayerTargetSize /*= false*/)
    : WorldObjectSpellTargetCheck(caster, referer, spellInfo, selectionType, condList)
    , _range(range)
    , _position(position)
    , _excludePlayerTargetSize(excludePlayerTargetSize)
{
}
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject* target)
{
    if (GameObject const* go = target->ToGameObject())
    {
        ...
        bool const isInRange = go->IsInRange(_position->GetPositionX(), _position->GetPositionY(), _position->GetPositionZ(), _range);
        if (!isInRange)
            return false;
    }
    else if (Unit const* unit = target->ToUnit())
    {
        bool isInsideCylinder = false;
        if (_spellInfo->HasAttribute(SPELL_ATTR1_CU_INCLUDE_CASTER_SIZE) || (_spellInfo->HasAttribute(SPELL_ATTR5_TREAT_AS_AREA_EFFECT) && _spellInfo->SpellFamilyName != SPELLFAMILY_GENERIC))
        {
            float hitboxSum = _caster->GetCombatReach() + unit->GetCombatReach();
            isInsideCylinder = unit->IsInDist2d(_position, _range + hitboxSum) && std::abs(unit->GetPositionZ() - _position->GetPositionZ()) <= (_range + hitboxSum);
        }
        else
        {
            float targetSize = 0.f;
            if (!_excludePlayerTargetSize || !target->IsPlayer())
                targetSize = unit->GetCombatReach();

            isInsideCylinder = unit->IsInDist2d(_position, _range + targetSize) && std::abs(unit->GetPositionZ() - _position->GetPositionZ()) <= (_range + targetSize);
        }
