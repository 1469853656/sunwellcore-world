...
void DeleteSpellFromAllPlayers(uint32 spellId)
{
    ...
    for (uint8 i = 0; i < 2; i++)
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(stmts[i]);
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, uint32 procPhase, bool active) const
{
    // No extra req need
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    do
    {
        ...
        // PROC_PHASE_NONE is by default PROC_PHASE_HIT
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (spellInfo->Effects[eff].Effect != SPELL_EFFECT_DUMMY && (spellInfo->Effects[eff].Effect != SPELL_EFFECT_APPLY_AURA || spellInfo->Effects[eff].ApplyAuraName != SPELL_AURA_DUMMY))
            {
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        if (SpellEntry const* spellEntry = sSpellStore.LookupEntry(i))
        {
            ...
            if (spellEntry->SpellFamilyName != SPELLFAMILY_GENERIC && spellEntry->powerType != POWER_FOCUS && !Trinity::Containers::Contains(IgnoredSpellsForNewLevel, spellEntry->Id))
                LevelUpSpellsCache.emplace_back(mSpellInfoMap[i]);
        }
    }
...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
                ...
                if (spellInfo->Effects[j].Effect)
                {
                    switch (spellInfo->Effects[j].Effect)
                    {
                        ...
                        case SPELL_EFFECT_APPLY_AREA_AURA_OWNER:
                            if (spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_DAMAGE || spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_DAMAGE_PERCENT || spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_DUMMY || spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_LEECH || spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_HEALTH_FUNNEL || spellInfo->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_DUMMY)
                                continue;
                        default:
        ...
        ApplySpellFix({ 58690, // Cyanigosa, Tail Sweep
                          59283 },
            [](SpellInfo* spellInfo) {
                spellInfo->AttributesCu |= SPELL_ATTR0_CU_CONE_BACK;
            });
        ...
        ApplySpellFix({
                          ...
                          25646, // Other type of mortal wound, used in a lot of places
                          17148  // Brain
                      },
            [](SpellInfo* spellInfo) {
        ...
        // Test spell for Mythic Lost Npcs event
        ApplySpellFix({ 70522 }, [](SpellInfo* spellInfo) {
            spellInfo->RecoveryTime         = 10 * IN_MILLISECONDS;
            spellInfo->_requireCooldownInfo = true;
        });

        // Test spell for Mythic Alchemist event
        ApplySpellFix({ 16872 }, [](SpellInfo* spellInfo) {
            spellInfo->RecoveryTime         = MINUTE * IN_MILLISECONDS;
            spellInfo->_requireCooldownInfo = true;
        });

        /* ICC Releasde Patch 3.3 remove cooldown
        ApplySpellFix({
        ...
        ApplySpellFix({ 60350, // Transmute Titanium
                      },
            [](SpellInfo* spellInfo) {
                if (sWorld->PatchNotes(PATCH_30X, PATCH_330))
                {
        ...
        */
        ...
        // @CustomMgr: Test Whisper - Spell used for Royal Features
        ApplySpellFix({ 53374 }, [](SpellInfo* spellInfo) {
        ...
        ApplySpellFix({
                          ...
                          8601,  // Slowing Poison
                          8876,  // Thrash
                          9205,  // Hate to Zero (Hate to Zero)
                          9347,  // Mortal Strike
                          9460,  // Corrosive Ooze
                          ...
                          10095, // Hate to Zero (Hate to Zero)
                          11838, // Hate to Zero (Hate to Zero)
                          11919, // Poison Proc
                          11966, // Fire Shield
                          11984, // Immolate
                          12099, // Shield Spike
                          ...
                          13767, // Hate to Zero (Hate to Zero)
                          14178, // Sticky Tar
                          15088, // Flurry
                          15097, // Enrage
                          15876, // Ice Blast
                          16140, // Exploding Cadaver (Exploding Cadaver)
                          ...
                          19194, // Double Attack
                          19195, // Hate to 90% (Hate to 90%)
                          19396, // Incinerate (Incinerate)
                          19626, // Fire Shield (Fire Shield)
                          19640, // Pummel (Pummel)
                          19817, // Double Attack
                          19818, // Double Attack
                          ...
                          22781, // Thornling
                          22788, // Grow
                          22856, // Ice Lock (Guard Slip'kik ice trap in Dire Maul)
                          25592, // Hate to Zero (Hate to Zero)
                          26341, // Saurfang's Rage
                          27987, // Unholy Aura
                          28126, // Spirit Particles (purple)
                          29406, // Shadowform
                          29526, // Hate to Zero (Hate to Zero)
                          31332, // Dire Wolf Visual
                          31690, // Putrid Mushroom
                          31792, // Bear Form (Shapeshift)
                          32007, // Mo'arg Engineer Transform Visual
                          ...
                          35841, // Draenei Spirit Visual
                          35850, // Draenei Spirit Visual 2
                          36006, // Fel Fire Aura
                          ...
                          37497, // Shadowmoon Ghost Invisibility (Ghostrider of Karabor in SMV)
                          37509, // Ghostly Facade
                          37816, // Shadowform
                          37863, // Disease Cloud
                          38844, // Unholy Aura
                          38847, // Diminish Soul
                          39102, // Glowy (Black)
                          39311, // Scrapped Fel Reaver transform aura that is never removed even on evade
                          39918, // Soulgrinder Ritual Visual ( in progress)
                          39920, // Soulgrinder Ritual Visual ( beam)
                          40453, // Aura of Fear
                          40816, // Saber Lash - Mother Shahraz
                          41634, // Invisibility and Stealth Detection
                          42459, // Dual Wield (Passive)
                          44537, // Fel Lightning
            ...
            [](SpellInfo* spellInfo) {
                spellInfo->AttributesCu1 |= SPELL_ATTR1_CU_PERSIST_AFTER_EVADE;
            });
...
void SpellMgr::LoadDbcDataCorrections()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        if (std::ranges::any_of(spellInfo->Effect, [](uint32 effect) { return effect == SPELL_EFFECT_CLEAR_QUEST; }))
        {
            spellInfo->Attributes |= SPELL_ATTR0_CASTABLE_WHILE_MOUNTED | SPELL_ATTR0_CASTABLE_WHILE_DEAD | SPELL_ATTR0_CASTABLE_WHILE_SITTING;
            spellInfo->AttributesEx4 |= SPELL_ATTR4_CAN_CAST_WHILE_CASTING;
            spellInfo->AttributesEx5 |= SPELL_ATTR5_USABLE_WHILE_STUNNED | SPELL_ATTR5_USABLE_WHILE_CONFUSED | SPELL_ATTR5_USABLE_WHILE_FEARED;
        ...
        switch (spellInfo->Id)
        {
            ...
            case 42835: // Spout, remove damage effect, only anim is needed
                ...
                break;
            // Arena - Dampening until content 3.2.0
            // Battleground - Dampening until content 3.2.0
            case 74410:
            case 74411:
                if (!sWorld->PatchNotes(PATCH_332))
                {
                    spellInfo->Effect[0] = 0;
            ...
            case 9633: // Whirlwind
                ...
                spellInfo->SpellFamilyFlags = flag96(0x0, 0x4, 0x0);
                break;
            ...
            case 29809: // Desecration Arm - 36 instead of 37 - typo? :/
                ...
                break;
            case 46034: // Summon Reprogrammed 55-D Collect-a-Tron
            case 46058: // Summon Reprogrammed Defendo-tank 66D
            case 46063: // Summon Reprogrammed Scavenge-bot 004-A8
            case 46023: // Summon Reprogrammed Scavenge-bot 005-B6
            case 46068: // Summon Reprogrammed Sentry Bot 57-K
            ...
            case 54836: // Wrath of the Plaguebringer
                spellInfo->EffectImplicitTargetB[0]   = TARGET_UNIT_SRC_AREA_ALLY;
                spellInfo->EffectImplicitTargetB[1]   = TARGET_UNIT_SRC_AREA_ALLY;
                spellInfo->EffectBasePoints[EFFECT_1] = spellInfo->EffectBasePoints[EFFECT_1] * 1.5;
                break;
            ...
            case 48776: // Mount Speed
                spellInfo->EffectApplyAuraName[EFFECT_0] = SPELL_AURA_MOD_MOUNTED_SPEED_ALWAYS;
                spellInfo->EffectApplyAuraName[EFFECT_1] = SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS;
                break;
            /////////////////////////////////////////////
            /////////////////CLASS SPELLS////////////////
            /////////////////////////////////////////////
        ...
        */
            ...
            case 53338:
                spellInfo->DmgClass = SPELL_DAMAGE_CLASS_MAGIC;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT | SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
                break;
            // Cobra Strikes
            case 53257:
                spellInfo->procCharges = 2;
        ...
        case 49938:
            ...
            case 47511:
            case 47515:
                spellInfo->SpellFamilyFlags = flag96(0x00000000, 0x00000000, 0x00000020);
                break;
            // Shadow Weaving
            case 15257:
            case 15331:
            ...
            ////Prowl
            case 5215:
                spellInfo->AuraInterruptFlags = AURA_INTERRUPT_FLAG_DIRECT_DAMAGE | AURA_INTERRUPT_FLAG_SPELL_ATTACK | AURA_INTERRUPT_FLAG_MELEE_ATTACK | AURA_INTERRUPT_FLAG_TALK | AURA_INTERRUPT_FLAG_USE | AURA_INTERRUPT_FLAG_CAST;
                break;
            // Owlkin Frenzy
            case 48391:
                spellInfo->Attributes |= SPELL_ATTR0_NOT_SHAPESHIFT;
            ...
            case 59685: // hurl dagger
                ...
                break;
            case 60766: // Essence of the Storm
            ...
            case 30174: //Riding Turtle
                spellInfo->EffectApplyAuraName[1] = SPELL_AURA_MOD_INCREASE_SWIM_SPEED;
                spellInfo->Effect[EFFECT_1]       = SPELL_EFFECT_APPLY_AURA;
                break;
            ...
            case 40018:  // Eye Blast
                ...
                break;
            case 7897: // Flames
                spellInfo->EffectRadiusIndex[EFFECT_0] = EFFECT_RADIUS_0_YARDS;
                break;
            // Spotted Panther
            case 10792:
                spellInfo->EffectBasePoints[EFFECT_1] = 99;
            ...
            //////////////////////////////////////////
            ////////// Old Scarlet Monastery & Custom
            //////////////////////////////////////////
            case 35089: // Vendor test - blizz test psell
            {
                spellInfo->Effect[EFFECT_0] = SPELL_EFFECT_SUMMON;
                spellInfo->EffectImplicitTargetA[EFFECT_0] = TARGET_DEST_CASTER_SUMMON;
                spellInfo->EffectMiscValue[EFFECT_0]       = Mythic::MYTHIC_COMPANION_ENTRY;
                spellInfo->EffectMiscValueB[EFFECT_0]      = 41;
                spellInfo->StartRecoveryTime               = 10000;
                break;
            }
            case 17148: // Brain hacker
            {
                spellInfo->StackAmount = 12;
                break;
            }
            case 53468: // leeching swarm for mythics
            case 51103:                                                        // Frostbomb
            case 34172:                                                        // Arcane Orb
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS; // OLDSM
                break;
            case 50220: // mythic
                spellInfo->Attributes |= (SPELL_ATTR0_NEGATIVE_1 | SPELL_ATTR0_PASSIVE);
                break;
            ...
            case 28526:
                spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_TARGET_ANY;
                break;
            case 62356: // broken bones linked
            case 62354: // broken bones, used for mythic avoidance reduction
            {
                spellInfo->Attributes |= (SPELL_ATTR0_NEGATIVE_1 | SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY);
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;

                for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    spellInfo->EffectImplicitTargetA[i] = TARGET_UNIT_TARGET_ANY;

                if (spellInfo->Id == 62354)
                    spellInfo->EffectBasePoints[EFFECT_1] = 0;

                if (spellInfo->Id == 62356)
                {
                    spellInfo->Effect[EFFECT_2]              = SPELL_EFFECT_APPLY_AURA;
                    spellInfo->EffectApplyAuraName[EFFECT_2] = SPELL_AURA_MOD_PARRY_PERCENT;
                    spellInfo->EffectBasePoints[EFFECT_2]    = 0;
                }
            ...
            //  Disruption
            case 29310:
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_30_YARDS;
                break;
            // Wail of Souls
            case 28459:
                spellInfo->MaxAffectedTargets = 3;
            ...
            case 47592:
                spellInfo->EffectAmplitude[0] = 200;
                break;
            case 47579: // Freezing Cloud
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_0_5_YARDS;
                spellInfo->EffectRadiusIndex[2] = EFFECT_RADIUS_0_5_YARDS;
                break;
            ...
            // Arcane Pulse
            case 57432:
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_35_YARDS; // Instead of 30yd
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
            ...
            case 63493:
                ...
                spellInfo->InterruptFlags = 0;
                break;
            case 69845: // frost bomb (used in mythics)
            ...
            case 64619: // Ulduar, Mimiron, Emergency Fire Bot, Water Spray
                ...
                break;
            case 31984: // Finger of death
                spellInfo->AttributesEx3 &= ~SPELL_ATTR3_ONLY_TARGET_PLAYERS;
                spellInfo->EffectImplicitTargetA[EFFECT_0] = TARGET_UNIT_TARGET_ANY;
                break;
            case 68442: // kneel
            case 62997: // Plasma blast, required for mythic
                // doesnt change anything for mimiron encounter since he is targeting specific players with spell
                spellInfo->EffectImplicitTargetA[EFFECT_0] = TARGET_UNIT_TARGET_ANY;
                break;
            ...
            case 64582:
                ...
                spellInfo->EffectBasePoints[EFFECT_1] = 29;
                break;
            ...
            // Ulduar, Hodir, Ice Shards
            case 62457:
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_3_YARDS; // 3yd
                spellInfo->EffectRadiusIndex[1] = EFFECT_RADIUS_3_YARDS; // 3yd
            ...
            case 64396: // Guardian Swarm
                ...
                spellInfo->EffectAmplitude[EFFECT_0] = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 200 : 500;
                break;
            ...
            case 67181:
            case 67513:
                spellInfo->EffectBasePoints[EFFECT_2] = -101;
                break;
        ...
        ! i suspected its caused by effect execution order which is messed up*/
            ...
            case 66240:
                ...
                spellInfo->DmgClass = SPELL_DAMAGE_CLASS_MAGIC;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_ONLY_TARGET_PLAYERS | SPELL_ATTR3_CANT_TRIGGER_PROC | SPELL_ATTR3_NO_DONE_BONUS;
                break;
            ...
            case 65974: // lava lash
                ...
                break;
            case 33335: // Shadow bolt, used for mythic spellbounce
                spellInfo->Effect[EFFECT_0]           = SPELL_EFFECT_SCHOOL_DAMAGE;
                spellInfo->EffectBasePoints[EFFECT_0] = 0;
                spellInfo->Attributes &= ~SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION;
                break;
            case 35923: // Water bubble head
                spellInfo->Attributes |= SPELL_ATTR0_NEGATIVE_1;
                break;
            ...
            case 69428:
            case 69426:
            case 58847: // green beam Mythic
                spellInfo->InterruptFlags        = 0;
                spellInfo->AuraInterruptFlags    = 0;
                spellInfo->ChannelInterruptFlags = 0;
                break;
            case 45845: // Fire channeling
            ...
            case 74637:
                spellInfo->speed = 0;
                break;
            // Summon Clone (Baltharus the Warborn)
            case 74511:
                spellInfo->EffectImplicitTargetA[EFFECT_0] = TARGET_DEST_CASTER;
                break;
            ...
            case 71024:
                ...
                break;
            // Heavily Perfumed Pulse (Love in the air event)
            case 71520:
            ...
            // Throw Mug visual
            case 50291:
            case 20698: // wrath, used for mythics
            case 42300:
                spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_TARGET_ANY;
                break;
            ...
            case 57778: // Shadow Word: Pain
                spellInfo->Dispel = DISPEL_NONE;
                break;
            case 49549: // Ice beam (for mythic)
            ...
            case 54409: // Mana Sapphire
                spellInfo->SpellFamilyName = SPELLFAMILY_MAGE;
                break;
            case 54758: // Hyperspeed Acceleration
        ...
        switch (spellInfo->Id)
        {
            ...
            case 62935: // Tidal wave 25
                ...
                spellInfo->PreventionType = SPELL_PREVENTION_TYPE_NONE;
                break;
            case 62438: // Iron roots
            case 62861: // Iron roots
                spellInfo->AttributesEx4 |= SPELL_ATTR4_IGNORE_RESISTANCES;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_HIT_RESULT;
                break;
    ...
    rSuffix->prefix[1]             = 6000;
    rSuffix                        = const_cast<ItemRandomSuffixEntry*>(sItemRandomSuffixStore.LookupEntry(80));
    rSuffix->prefix[1]             = 6000;
    rSuffix                        = const_cast<ItemRandomSuffixEntry*>(sItemRandomSuffixStore.LookupEntry(83));
    rSuffix->prefix[1]             = 6000;
