    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_TEAM: {
            if (Player* player = object->ToPlayer())
            {
                ...
                uint32 teamOld = player->GetTeam(CF_DENY) == TEAM_ALLIANCE ? ALLIANCE : HORDE;
        ...
        case CONDITION_RACE: {
            if (Unit* unit = object->ToUnit())
                condMeets = unit->getRaceMask(CF_DENY) & ConditionValue1;
            break;
        }
