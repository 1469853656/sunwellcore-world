...
{
    CharacterDatabaseTransaction dummy = CharacterDatabaseTransaction(nullptr);
...
void CalendarMgr::RemoveInvite(uint64 inviteId, uint64 eventId, uint64 /*remover*/)
{
    ...
    auto trans    = CharacterDatabase.BeginTransaction();
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CALENDAR_INVITE);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CALENDAR_EVENT);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_CALENDAR_INVITE);
    stmt->setUInt64(0, invite->GetInviteId());
...
CalendarEventStore CalendarMgr::GetPlayerEvents(uint64 guid)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayerInOrOutOfWorld(guid))
        if (player->GetGUID() && player->GetGuildId())
...
{
    WorldPacket data(SMSG_CALENDAR_EVENT_UPDATED_ALERT, 1 + 8 + 4 + 4 + 4 + 1 + 4 + calendarEvent.GetTitle().size() + calendarEvent.GetDescription().size() + 1 + 4 + 4);
    data << uint8(1); // unk
