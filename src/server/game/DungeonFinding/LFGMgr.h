...
enum LFGMgrEnum
{
    ...
    LFG_CTA_REWARD_ITEM_ID      = 69903,
    LFG_RANDOM_LICH_KING_HEROIC = 262
};
...
{
    RBEntryInfo() {}
    RBEntryInfo(uint8 _roles, std::string const& _comment)
        : roles(_roles)
        , comment(_comment)
    {
    }
...
struct RBInternalInfo
{
    ...
    RBInternalInfo(uint64 guid, std::string const& comment, bool isGroupLeader, uint64 groupGuid, uint8 roles, uint32 encounterMask, uint64 instanceGuid,
        ...
        , _class(_class)
        , _race(_race)
        , _avgItemLevel(_avgItemLevel)
        , _talents0(_talents[0])
        , _talents1(_talents[1])
        , _talents2(_talents[2])
        , _area(_area)
        , _armor(_armor)
        , _spellDamage(_spellDamage)
        , _spellHeal(_spellHeal)
        , _critRatingMelee(_critRatingMelee)
        , _critRatingRanged(_critRatingRanged)
        , _critRatingSpell(_critRatingSpell)
        , _mp5(_mp5)
        , _mp5combat(_mp5combat)
        , _attackPower(_attackPower)
        , _agility(_agility)
        , _health(_health)
        , _mana(_mana)
        , _defenseSkill(_defenseSkill)
        , _dodgeRating(_dodgeRating)
        , _blockRating(_blockRating)
        , _parryRating(_parryRating)
        , _hasteRating(_hasteRating)
        , _expertiseRating(_expertiseRating)
    {
    }
...
struct LFGDungeonData
{
    ...
    LFGDungeonData(LFGDungeonEntry const* dbc)
        ...
        , seasonal(dbc->flags & LFG_FLAG_SEASONAL)
        , x(0.0f)
        , y(0.0f)
        , z(0.0f)
        , o(0.0f)
...
class LFGMgr : public Sunwell::Singleton<LFGMgr>
{
    friend class Sunwell::Singleton<LFGMgr>;
...
public:
    ...
    /// Get first found online player in group
    uint64 GetAnyOnlinePlayerFound(uint64 guid);
    ...
    // Handles incrementing/decrementing role for Call to Arms system
    void QueueRole(uint64 guid, uint8 roles, bool joinLFG);
    // Updates the status of which role is the most needed
    void UpdateCallToArmsStatus();
    // Sets which role is currently most needed
    void SetCallToArmsRole(uint8 role);
    // Returns Call to Arms role
    uint8 GetCallToArmsRole();
     // Sets bonus roles for a dungeon group
    void SetBonusRoles(uint64 guid, uint8 roles);
    // Return Lfg dungeon bonus roles
    uint8 GetBonusRoles(uint64 guid);
    // Checks if group players can receive Call to Arms reward
    bool IsCallToArmsEligible(uint64 gguid);
    // Enables or disables Call to Arms for a group
    void SetCallToArmsEligible(uint64 gguid, bool enable);
...
private:
    ...
    uint32 _tanksInQueue;
    uint32 _healersInQueue;
    uint32 _dpsesInQueue;
    uint8 _beneficientRoles;
    std::vector<uint64> _callToArmsQueue;
};
