...
LFGMgr::LFGMgr()
    ...
    , _healersInQueue(0)
    , _dpsesInQueue(0)
    , _beneficientRoles(PLAYER_ROLE_NONE)
{
    new LFGPlayerScript();
    new LFGGroupScript();
...
void LFGMgr::_SaveToDB(uint64 guid)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_LFG_DATA);
...
void LFGMgr::InitializeLockedDungeons(Player* player, uint8 level /* = 0 */)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        else if (DisableMgr::IsDisabledFor(DISABLE_TYPE_MAP, dungeon->map, player) || player->IsPathOfWar())
        ...
        else if (AccessRequirement const* ar = sObjectMgr->GetAccessRequirement(dungeon->map, Difficulty(dungeon->difficulty)))
        {
            ...
            else if (player->GetTeam(CF_DENY) == TEAM_ALLIANCE && ar->quest_A && !player->GetQuestRewardStatus(ar->quest_A))
                lockData = LFG_LOCKSTATUS_QUEST_NOT_COMPLETED;
            else if (player->GetTeam(CF_DENY) == TEAM_HORDE && ar->quest_H && !player->GetQuestRewardStatus(ar->quest_H))
                lockData = LFG_LOCKSTATUS_QUEST_NOT_COMPLETED;
            else if (ar->item)
            {
...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons, const std::string& comment)
{
    ...
    if (grp && (grp->isBGGroup() || grp->isBFGroup() || grp->IsMythicGroup()))
        return;
    ...
    else // Add player to queue
    {
        ...
        SetRoles(guid, roles);
        SetBonusRoles(guid, GetCallToArmsRole());
        QueueRole(guid, roles, true);
    ...
    for (LfgRolesMap::const_iterator it = roleCheck.roles.begin(); it != roleCheck.roles.end(); ++it)
    {
        ...
        switch (roleCheck.state)
        {
            ...
            case LFG_ROLECHECK_FINISHED:
                QueueRole(pguid, sLFGMgr->GetRoles(pguid), true);
        ...
        if (Player* player = ObjectAccessor::FindPlayer(*it))
        {
            ...
            for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
            {
                if (BattlegroundQueueTypeId bgQueueTypeId = player->GetBattlegroundQueueTypeId(i))
                {
                    ...
                    queue.RemovePlayer(player->GetGUID(), false, i);
                }
            }

            // Remove from SoloQueue
            sSoloQueueMgr->RemoveFromPreparationQueue(player);
            ...
            QueueRole(*it, GetRoles(*it), false);
    ...
    for (LfgProposalPlayerContainer::iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
    {
        if (it->second.accept == LFG_ANSWER_DENY)
        {
            if (Player* plr = sObjectAccessor->FindPlayer(it->first))
            {
                ...
                {
                    if (aura->GetDuration() < 150 * IN_MILLISECONDS)
                    {
                        aura->SetMaxDuration(150 * IN_MILLISECONDS);
                        aura->RefreshDuration();
                    }
                }
                else if (Aura* aura = plr->AddAura(LFG_SPELL_DUNGEON_COOLDOWN, plr))
...
void LFGMgr::FinishDungeon(uint64 gguid, const uint32 dungeonId, const Map* currMap)
{
    ...
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (Group* group = player->GetGroup())
            if (IsCallToArmsEligible(gguid) && GetBonusRoles(guid) & group->GetLfgRoles(guid))
            {
                ItemPosCountVec dest;
                if (player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, LFG_CTA_REWARD_ITEM_ID, 1) == EQUIP_ERR_OK)
                {
                    ItemRef item = player->StoreNewItem(dest, LFG_CTA_REWARD_ITEM_ID, true);
                ...
                else
                {
                    ...
                    if (Item* item = Item::CreateItem(LFG_CTA_REWARD_ITEM_ID, 1))
                    {
                        item->SaveToDB(trans);
                        draft.AddItem(item);
                    }
        ...
        if (player->GetQuestStatus(26035) == QUEST_STATUS_INCOMPLETE)
            player->KilledMonsterCredit(360001, 0);
        ...
        if (sWorld->getBoolConfig(CONFIG_ARENA_REWARD_ENABLE) && dungeonDone->difficulty == DUNGEON_DIFFICULTY_HEROIC)
        {
            ...
            if (player->AddItem(rewardId, count))
            {
                ...
                handler.PSendSysMessage(LANG_ARENA_WIN_REWARD, "dungeon");
            }
        }
    }
}
...
void LFGMgr::SetTeam(uint64 guid, TeamId teamId)
{
    if (sWorld->IsCrossfactionEnabled(Crossfaction::DungeonFinder))
    {
        ...
        {
            //if (player->getLevel() >= sCustomMgr->GetIntConfig(CONFIG_CROSSFACTION_RDF_MINLVL) && player->getLevel() <= sCustomMgr->GetIntConfig(CONFIG_CROSSFACTION_RDF_MAXLVL))
            teamId = TEAM_NEUTRAL;
        }*/
    }
...
uint64 LFGMgr::GetAnyOnlinePlayerFound(uint64 guid)
{
    for (uint64 guid : GroupsStore[guid].GetPlayers())
...
void LFGMgr::QueueRole(uint64 guid, uint8 roles, bool joinLFG)
{
    if (!sWorld->getBoolConfig(CONFIG_CALL_TO_ARMS_ENABLE))
        return;
    ...
    if (joinLFG)
    {
        ...
        if (rDungeonId != lfg::LFG_RANDOM_LICH_KING_HEROIC)
            return;
        ...
        _callToArmsQueue.push_back(guid);
    }
    else
    {
        std::vector<uint64>::iterator it = std::find(_callToArmsQueue.begin(), _callToArmsQueue.end(), guid);
        if (it == _callToArmsQueue.end())
            return;

        _callToArmsQueue.erase(it);
    ...
    UpdateCallToArmsStatus();
}

void LFGMgr::UpdateCallToArmsStatus()
{
    uint32 totalPlayersInQueue = _tanksInQueue + _healersInQueue + _dpsesInQueue;
    uint8 role = PLAYER_ROLE_NONE;

    if (totalPlayersInQueue >= 5)
    {
        if (_tanksInQueue)
    ...
    SetCallToArmsRole(role);
}

void LFGMgr::SetCallToArmsRole(uint8 role) 
{ 
    _beneficientRoles = role; 
}

uint8 LFGMgr::GetCallToArmsRole() 
{ 
    return _beneficientRoles; 
}

void LFGMgr::SetBonusRoles(uint64 guid, uint8 roles)
{
    PlayersStore[guid].SetBonusRoles(roles);
}

uint8 LFGMgr::GetBonusRoles(uint64 guid)
{
    uint8 roles = PlayersStore[guid].GetBonusRoles();
    return roles;
}

void LFGMgr::SetCallToArmsEligible(uint64 gguid, bool enable)
{
    return GroupsStore[gguid].SetCallToArmsEligible(enable);
}

bool LFGMgr::IsCallToArmsEligible(uint64 gguid)
{
    return GroupsStore[gguid].IsCallToArmsEligible();
}
