...
void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
    ...
    if (!triggerFrom)
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_POOL_QUEST_SAVE);
...
void PoolMgr::LoadFromDB()
{
    ...
    sLog->outString("Loading Quest Pooling Data...");
    {
        ...
        auto stmt    = WorldDatabase.GetPreparedStatement(WORLD_SEL_QUEST_POOLS);
...
void PoolMgr::SaveQuestsToDB(bool daily, bool weekly, bool other)
{
    ...
    for (PoolGroupQuestMap::iterator itr = mPoolQuestGroups.begin(); itr != mPoolQuestGroups.end(); ++itr)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_QUEST_POOL_SAVE);
    ...
    for (SearchMap::iterator itr = mQuestSearchMap.begin(); itr != mQuestSearchMap.end(); ++itr)
        if (deletedPools.find(itr->second) != deletedPools.end())
            ...
            {
                auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_QUEST_POOL_SAVE);
...
{
    return pool_id <= max_pool_id && mPoolGameobjectGroups[pool_id].CheckPool() && mPoolCreatureGroups[pool_id].CheckPool() && mPoolPoolGroups[pool_id].CheckPool() && mPoolQuestGroups[pool_id].CheckPool();
}
