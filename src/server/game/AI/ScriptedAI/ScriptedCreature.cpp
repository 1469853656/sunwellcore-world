...
Unit* ScriptedAI::DoSelectLowestHpFriendly(float range, uint32 minHPDiff, bool excludePlayers /* false */)
{
    Unit* unit = nullptr;
    Trinity::MostHPMissingInRange u_check(me, range, minHPDiff, excludePlayers);
...
void BossAI::_JustDied()
{
    ...
    if (instance)
    {
        sCustomMgr->HandleCooldownResetOnEncounterDone(instance);
...
{
    sCustomMgr->HandleCooldownResetOnWipe(instance, GetEncounterElapsedTime());
    ScriptedAI::EnterEvadeMode();
}

Milliseconds BossAI::GetEncounterElapsedTime() const
{
    if (!instance || instance->GetBossState(_bossId) != IN_PROGRESS)
        return 0s;

    return std::chrono::duration_cast<Milliseconds>(std::chrono::steady_clock::now() - _encounterStartTime);
};
