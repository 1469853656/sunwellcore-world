...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
            ...
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    else if (IsPlayer(*itr) && me)
                    {
                        ...
                        sCreatureTextMgr->SendChat(me, uint8(e.action.talk.textGroupID), IsPlayer(templastInvoker) ? templastInvoker : nullptr, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false, (*itr)->ToPlayer());
        ...
        case SMART_ACTION_CAST: {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                else if (me && (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell)))
                {
                    ...
                    if ((e.action.cast.flags & SMARTCAST_COMBAT_MOVE) && GetCasterMaxDist() > 0.0f)
                    {
                        if (!_smartCombatMoveSpells.contains(e.action.cast.spell))
                        {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST: {
            ...
            {
                TrinityStringTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_FLEE, LANG_UNIVERSAL, nullptr);
        ...
        case SMART_ACTION_UPDATE_TEMPLATE: {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsCreature(*itr))
                    (*itr)->ToCreature()->UpdateEntry(e.action.updateTemplate.creature, nullptr, !e.action.updateTemplate.doNotChangeLevel);
        ...
        case SMART_ACTION_CALL_FOR_HELP: {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsCreature(*itr))
                {
                    ...
                    {
                        TrinityStringTextBuilder builder(*itr, CHAT_MSG_MONSTER_EMOTE, LANG_CALL_FOR_HELP, LANG_UNIVERSAL, nullptr);
        ...
        case SMART_ACTION_SET_UNIT_MOVEMENT_FLAGS: {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsUnit(*itr))
                {
                    (*itr)->ToUnit()->SetUnitMovementFlags((MovementFlags)e.action.movementFlag.flag);
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_CREATURE_GUID:
        {
            ...
            {
                if (Creature* target = ObjectAccessor::GetCreature(scriptTrigger ? *scriptTrigger : *GetBaseObject(), MAKE_NEW_GUID(e.target.unitGUID.dbGuid, e.target.unitGUID.entry, HIGHGUID_UNIT)))
                    l->push_back(target);
            }
            else
            {
        ...
        case SMART_TARGET_GAMEOBJECT_GUID:
        {
            ...
            {
                if (GameObject* target = ObjectAccessor::GetGameObject(scriptTrigger ? *scriptTrigger : *GetBaseObject(), MAKE_NEW_GUID(e.target.goGUID.dbGuid, e.target.goGUID.entry, HIGHGUID_GAMEOBJECT)))
                    l->push_back(target);
            }
            else
            {
...
void SmartScript::UpdateSmartCombatMoveSpells(uint32 diff)
{
    ...
    if (_smartCombatMoveUpdateTimer <= diff)
    {
        ...
        _smartCombatMoveUpdateTimer -= diff;
}
...
bool SmartScript::CanCastCombatMoveSpell(SpellInfo const* spellInfo)
{
    if (spellInfo)
    {
        ...
        int32 const powerCost = spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask());
        if (powerCost != 0 && currentPowerAmount < powerCost)
            return false;
        ...
        if (spellInfo->RangeEntry)
            if (Unit const* victim = me->GetVictim())
                if (!me->IsInRange(victim, spellInfo->RangeEntry->minRangeHostile, spellInfo->RangeEntry->maxRangeHostile))
                    return false;
