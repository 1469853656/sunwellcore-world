...
void SmartWaypointMgr::LoadFromDB()
{
    ...
    auto stmt    = WorldDatabase.GetPreparedStatement(WORLD_SEL_SMARTAI_WP);
...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    auto stmt    = WorldDatabase.GetPreparedStatement(WORLD_SEL_SMART_SCRIPTS);
    PreparedQueryResult result = WorldDatabase.Query(stmt);
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_RANDOM_PHASE:
        {
            ...
            if (e.action.randomPhase.phase1 == 0 && e.action.randomPhase.phase2 == 0 && e.action.randomPhase.phase3 == 0 && e.action.randomPhase.phase4 == 0 && e.action.randomPhase.phase5 == 0 && e.action.randomPhase.phase6 == 0)
            {
                sLog->outErrorDb("SmartAIMgr: Entry %d SourceType %u Event %u Action %u attempts to set invalid phase, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                return false;
            }
        ...
        case SMART_ACTION_REMOVE_AURAS_BY_TYPE:
        {
            ...
            {
                sLog->outError("SmartScript: Entry %u SourceType %u Event %u Action %u uses invalid data type %u (value range 0-TOTAL_AURAS), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.auraType.type);
                return false;
            }
            break;
        }
