...
void VehicleAI::UpdateAI(uint32 diff)
{
    ...
    // Custom handling for professor putricide abomination
    // TODO: fix properly
    if (me->GetOwner() && me->GetOwner()->IsPlayer() && (me->GetEntry() == 38285 || me->GetEntry() == 37672))
    {
        if (me->HasUnitState(UNIT_STATE_CASTING))
            return;

        Unit* victim = ObjectAccessor::GetUnit(*me, me->GetOwner()->GetTarget());
