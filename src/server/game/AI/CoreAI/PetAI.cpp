...
bool PetAI::_needToStop()
{
    ...
    {
        // Gothik
        if (GameObject* VaccuumDoor = me->FindNearestGameObject(181170, 100.0f))
        {
            if (VaccuumDoor->GetGoState() == GO_STATE_READY)
            {
                if (me->GetPositionY() >= -3359.5f && me->GetVictim()->GetPositionY() <= -3361.0f)
                    return true;
                if (me->GetPositionY() <= -3361.0f && me->GetVictim()->GetPositionY() >= -3359.5f)
                    return true;
            }
        }

        // Noth
        if (GameObject* ExitDoor = me->FindNearestGameObject(181201, 100.0f))
        {
            if (ExitDoor->GetGoState() == GO_STATE_READY)
            {
                if (me->GetPositionY() >= -3559.0f && me->GetVictim()->GetPositionY() <= -3557.5f)
                    return true;
                if (me->GetPositionY() <= -3557.5f && me->GetVictim()->GetPositionY() >= -3559.0f)
                    return true;
            }
        }
    }
...
{
    return me->GetEntry() != 416 /*ENTRY_IMP*/ && me->GetEntry() != 510 /*ENTRY_WATER_ELEMENTAL*/ && me->GetEntry() != 37994 /*ENTRY_WATER_ELEMENTAL_PERM*/;
}
    ...
    if (!me->IsCasting())
    {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive())
            {
                ...
                // Lesser Invisibility
                if (spellInfo->Id == 7870)
                    if (auto const lastCastTime = GetLastCastTime(7870))
                        if (sWorld->GetGameTime() - *lastCastTime < 5)
                            continue;
        ...
        if (!targetSpellStore.empty())
        {
            ...
            _lastCastTime[spell->m_spellInfo->Id] = sWorld->GetGameTime();
        }
...
std::optional<time_t> PetAI::GetLastCastTime(uint32 spellID) const
{
    auto const itr = _lastCastTime.find(spellID);
    if (itr != _lastCastTime.end())
        return itr->second;
...
bool PetAI::CanAttack(Unit* target, const SpellInfo* spellInfo)
{
    ...
    if (me->HasReactState(REACT_PASSIVE))
    {
        CharmInfo* cInfo = me->GetCharmInfo();
        if (me->GetOwner() && me->GetOwner()->ToPlayer())
        {
            if (me->GetOwner()->ToPlayer()->getClass() == CLASS_WARLOCK)
                return cInfo->IsCommandAttack();

            else
                return !(cInfo->IsAtStay() || cInfo->IsFollowing() || cInfo->IsCommandFollow());
        }
        else
            return false;
    }
