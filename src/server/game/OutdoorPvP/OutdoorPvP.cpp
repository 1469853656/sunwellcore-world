...
bool OPvPCapturePoint::HandlePlayerEnter(Player* player)
{
    ...
    return m_activePlayers[player->GetTeam(CF_DENY)].insert(player->GetGUID()).second;
}
...
void OPvPCapturePoint::HandlePlayerLeave(Player* player)
{
    ...
    m_activePlayers[player->GetTeam(CF_DENY)].erase(player->GetGUID());
}
...
{
    m_players[player->GetTeam(CF_DENY)].insert(player->GetGUID());
}
...
void OutdoorPvP::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    ...
    m_players[player->GetTeam(CF_DENY)].erase(player->GetGUID());
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    for (std::list<Player*>::iterator itr = players.begin(); itr != players.end(); ++itr)
    {
        ...
        {
            if (m_activePlayers[player->GetTeam(CF_DENY)].insert(player->GetGUID()).second)
...
{
    PlayerSet const& plSet = m_activePlayers[player->GetTeam(CF_DENY)];
...
{
    PlayerSet const& plSet = m_players[player->GetTeam(CF_DENY)];
    return plSet.find(player->GetGUID()) != plSet.end();
}
