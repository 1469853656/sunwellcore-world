...
class CreatureTextMgr : public Sunwell::Singleton<CreatureTextMgr>
{
    friend class Sunwell::Singleton<CreatureTextMgr>;
    CreatureTextMgr() { }
...
public:
    ...
    void SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType, WorldObject const* whisperTarget = nullptr, CreatureTextRange range = TEXT_RANGE_NORMAL, TeamId teamId = TEAM_NEUTRAL, bool gmOnly = false) const;
...
public:
    ...
    void SetWhisperTarget(uint64 guid)
    {
        m_whisperTargetGuid = guid;
    }
    ...
    void operator()(Player* player)
    {
        ...
        TextBuilderResult customizer;

        // create if not cached yet
        if (!_packetCache[loc_idx])
        {
            messageTemplate = new WorldPacket();
            customizer      = _builder(messageTemplate, loc_idx);
            ASSERT(messageTemplate->GetOpcode() != MSG_NULL_ACTION);
            _packetCache[loc_idx] = new std::pair<WorldPacket*, TextBuilderResult>(messageTemplate, customizer);
        }
        else
        {
            messageTemplate = _packetCache[loc_idx]->first;
            customizer      = _packetCache[loc_idx]->second;
        }

        WorldPacket data(*messageTemplate);
        customizer.SetLanguage(&data, player->ConvertLanguage(Language(_builder._language)));
        customizer.SetReceiverGuid(&data, m_whisperTargetGuid);

        switch (_msgType)
        {
            case CHAT_MSG_MONSTER_WHISPER:
            case CHAT_MSG_RAID_BOSS_WHISPER:
                customizer.SetReceiverGuid(&data, player->GetGUID());
                break;
            default:
                break;
        }
...
private:
    std::vector<std::pair<WorldPacket*, TextBuilderResult>*> _packetCache;
    Builder const& _builder;
    ChatMsg _msgType;
    uint64 m_whisperTargetGuid = 0u;
};
...
void CreatureTextMgr::SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType, WorldObject const* whisperTarget /*= NULL*/, CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, TeamId teamId /*= TEAM_NEUTRAL*/, bool gmOnly /*= false*/) const
{
    ...
    if (whisperTarget != nullptr)
        localizer.SetWhisperTarget(whisperTarget->GetGUID());
    ...
    switch (range)
    {
        case TEXT_RANGE_AREA: {
            ...
            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                if (itr->GetSource()->GetAreaId() == areaId && (teamId == TEAM_NEUTRAL || itr->GetSource()->GetTeam() == teamId) && (!gmOnly || itr->GetSource()->IsGameMaster()))
        ...
        case TEXT_RANGE_ZONE: {
            ...
            Map::PlayerList const& players = source->GetMap()->GetPlayers();
            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                if (itr->GetSource()->GetZoneId() == zoneId && (teamId == TEAM_NEUTRAL || itr->GetSource()->GetTeam() == teamId) && (!gmOnly || itr->GetSource()->IsGameMaster()))
                    localizer(itr->GetSource());
            return;
        }
        case TEXT_RANGE_MAP: {
            Map::PlayerList const& players = source->GetMap()->GetPlayers();
            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                if ((teamId == TEAM_NEUTRAL || itr->GetSource()->GetTeam() == teamId) && (!gmOnly || itr->GetSource()->IsGameMaster()))
                    localizer(itr->GetSource());
            return;
        }
        case TEXT_RANGE_WORLD: {
            ...
            for (SessionMap::const_iterator itr = smap.begin(); itr != smap.end(); ++itr)
                if (Player* player = itr->second->GetPlayer())
                    if ((teamId == TEAM_NEUTRAL || player->GetTeam(CF_DENY) == teamId) && (!gmOnly || player->IsGameMaster()))
                        localizer(player);
            return;
        }
    ...
    if (whisperTarget != nullptr)
        worker.i_do.SetWhisperTarget(whisperTarget->GetGUID());
