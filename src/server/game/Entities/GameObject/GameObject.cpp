...
{
public:
    explicit GameObjectModelOwnerImpl(GameObject const* owner)
        : _owner(owner)
    {
    }
    ...
    virtual float GetScale() const override { return _owner->GetFloatValue(OBJECT_FIELD_SCALE_X); }
    virtual void DebugVisualizeCorner(G3D::Vector3 const& corner) const override { _owner->SummonCreature(1, { corner.x, corner.y, corner.z }); }
...
void GameObject::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        Map* map = GetMap();
        map->InsertToObjectsStore(this);

        if (m_model)
        {
            m_model->UpdatePosition();
            map->InsertGameObjectModel(*m_model);
        }
...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        map->RemoveFromObjectsStore(this);
    }
}
        ...
        case GO_READY: {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        if (owner->IsInSanctuary() && owner->GetTypeId() == TYPEID_PLAYER && !owner->ToPlayer()->duel && owner->GetAreaId() != 876 /* GM ISLAND*/)
                            return;

                        UnitList targets;
                        Trinity::NearestUnfriendlyNoTotemUnitInObjectRangeCheck u_check(this, owner, radius);
                        Trinity::UnitListSearcher<Trinity::NearestUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, targets, u_check);
                        ...
                        if (!targets.empty())
                        {
                            auto it = std::min_element(std::begin(targets), std::end(targets), [this](Unit const* left, Unit* const right) { return left->GetDistance(this) < right->GetDistance(this); });
...
bool GameObject::LoadGameObjectFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_CHEST && map->IsDungeon() && !GetGOInfo()->chest.minSuccessOpens)
...
void GameObject::Use(Unit* user)
{
    ...
    if (Player* playerUser = user->ToPlayer())
    {
        ...
        if (playerUser->IsPathOfWar() && GetGoType() == GAMEOBJECT_TYPE_GUILD_BANK)
            return;
    ...
    Spell* spell = new Spell(spellCaster, spellInfo, TRIGGERED_NONE, 0);
...
void GameObject::CastSpell(Unit* target, uint32 spellId)
{
    ...
    else
    {
        ...
        if (target)
            trigger->SetInFront(target);
        trigger->CastSpell(target ? target : trigger, spellInfo, true, nullptr, nullptr, target ? target->GetGUID() : 0);
    }
}
...
void GameObject::SetLootRecipient(Player* player, Creature* source /*= nullptr*/)
{
    ...
    if (source)
        source->FillPlayersAllowedToLoot(&m_playersAllowedToLoot);
}
...
bool GameObject::IsLootAllowedFor(Player const* player) const
{
    ...
    if (!m_playersAllowedToLoot.empty() && m_playersAllowedToLoot.find(player->GetGUID()) == m_playersAllowedToLoot.end())
        return false;
...
void GameObject::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    {
        if (_fieldNotifyFlags & flags[index] || ((updateType == UPDATETYPE_VALUES ? _changesMask.GetBit(index) : m_uint32Values[index]) && (flags[index] & visibleFlag)) || (index == GAMEOBJECT_FLAGS && forcedFlags))
        {
...
void GameObject::CreateObjectModel()
{
