...
class GameObject : public WorldObject, public GridObject<GameObject>, public MovableMapObject
{
    public:
        ...
        void SetLootRecipient(Player* player, Creature* source = nullptr);
        bool IsLootAllowedFor(Player const* player) const;
        bool HasLootRecipient() const { return m_lootRecipient || m_lootRecipientGroup; }
        bool HasPlayersAllowedToLoot() const { return !m_playersAllowedToLoot.empty(); }
        bool IsPlayerAllowedToLoot(Unit const* unit) const { return m_playersAllowedToLoot.find(unit->GetGUID()) != m_playersAllowedToLoot.end(); }
        std::set<uint64> GetPlayersAllowedToLoot() const { return m_playersAllowedToLoot; }
        void FillPlayersAllowedToLoot(std::set<uint64>* s) const override { *s = m_playersAllowedToLoot; }
        void AddPlayerAllowedToLoot(uint64 guid) { m_playersAllowedToLoot.insert(guid); }
    ...
    {
        if (GetGOInfo()->type == GAMEOBJECT_TYPE_MO_TRANSPORT || GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT)
    ...
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT) return m_stationaryPosition.GetPositionX();
        return GetPositionX();
    }
    float GetStationaryY() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT) return m_stationaryPosition.GetPositionY();
        return GetPositionY();
    }
    float GetStationaryZ() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT) return m_stationaryPosition.GetPositionZ();
        return GetPositionZ();
    }
    float GetStationaryO() const override
    {
        if (GetGOInfo()->type != GAMEOBJECT_TYPE_MO_TRANSPORT) return m_stationaryPosition.GetOrientation();
