...
void Item::SaveToDB(CharacterDatabaseTransaction& trans)
{
    ...
    switch (uState)
    {
        ...
        {
            uint8 index             = 0;
            auto stmt = CharacterDatabase.GetPreparedStatement(uState == ITEM_NEW ? CHAR_REP_ITEM_INSTANCE : CHAR_UPD_ITEM_INSTANCE);
            ...
            stmt->setUInt32(++index, _transmog);
            stmt->setUInt32(++index, _transmogEnchant);
        ...
        case ITEM_REMOVED:
        {
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEM_INSTANCE);
...
bool Item::LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry, uint32 transmog /* = 0 */, uint32 transmogEnchant /* = 0 */)
{
    ...
    SetEntry(entry);
    SetObjectScale(1.0f);
    SetTransmog(transmog);
    SetTransmogEnchant(transmogEnchant);
    ...
    {
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ITEM_INSTANCE_ON_LOAD);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INVENTORY_BY_ITEM);
    stmt->setUInt32(0, itemGuid);
    trans->Append(stmt);
}
...
bool Item::CanBeTraded(bool mail, bool trade) const
{
    ...
    if (Player* owner = GetOwner())
    {
        ...
        if (trade && owner->GetArenaType() == ARENA_TYPE_SOLO_QUEUE && !IsConjuredConsumable())
            return false;
    }
...
void Item::SaveRefundDataToDB()
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEM_REFUND_INSTANCE);
...
void Item::ClearSoulboundTradeable(Player* currentOwner)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEM_BOP_TRADE);
    stmt->setUInt32(0, GetGUIDLow());
    CharacterDatabase.Execute(stmt);
}
...
void Item::ItemRestore(uint32 pGuidLow, RestoreAction action)
{
    ...
    switch (action)
    {
        ...
        {
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_ITEM_RESTORE);
        ...
        case ITEM_RESTORE_ACTION_SELECT: { /* TODO OR HANDLE BY SENDITEMS COMMAND ? */
        }
        break;
        case ITEM_RESTORE_ACTION_RESTORE: { /* TODO  OR HANDLE BY SENDITEMS COMMAND ? */
        }
        break;
    }
}
