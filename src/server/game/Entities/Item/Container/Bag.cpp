...
bool Bag::LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry, uint32 transmog /* = 0 */, uint32 transmogEnchant /* = 0 */)
{
    if (!Item::LoadFromDB(guid, owner_guid, fields, entry, transmog, transmogEnchant))
        return false;
