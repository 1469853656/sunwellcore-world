...
public:
    ...
    virtual bool LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry, uint32 transmog = 0, uint32 transmogEnchant = 0);
    ...
    bool IsBag() const
    {
        auto iTemplate = GetTemplate();
        return iTemplate ? iTemplate->InventoryType == INVTYPE_BAG : false;
    }
    ...
    void SetTransmog(uint32 entry) { _transmog = GetEntry() == entry ? 0 : entry; }
    void SetTransmogEnchant(uint32 entry) { _transmogEnchant = GetEnchantmentId(PERM_ENCHANTMENT_SLOT) == entry ? 0 : entry; }
    uint32 GetTransmog() const { return _transmog; }
    uint32 GetTransmogEnchant() const { return _transmogEnchant; }
