...
private:
    ...
    uint32 _linkedToQuest = 0;
    int32 _linkedToQuestCheckTimer = 500;
};

class Minion : public TempSummon
{
public:
    Minion(SummonPropertiesEntry const* properties, WorldObject* owner, bool isWorldObject);
    void InitStats(uint32 duration);
    void RemoveFromWorld();
    Unit* GetOwner() const;
    Unit* GetOwnerForInit() const { return m_owner ? m_owner->ToUnit() : nullptr; } // Use only when owner is not yet added to map while creating the Minion
...
protected:
    ...
    WorldObject* const m_owner = nullptr;
    float m_followAngle;
};
...
{
public:
    Guardian(SummonPropertiesEntry const* properties, WorldObject* owner, bool isWorldObject);
    void InitStats(uint32 duration);
    ...
    // Autocast save
    void SaveSpells();
    void SetReactAndCommandState();
};

class Puppet : public Minion
{
public:
    Puppet(SummonPropertiesEntry const* properties, WorldObject* owner);
    void InitStats(uint32 duration);
    void InitSummon();
    void Update(uint32 time);
    void RemoveFromWorld();
