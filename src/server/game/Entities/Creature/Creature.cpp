...
Creature::Creature(bool isWorldObject)
    ...
    , m_inhabitType(INHABIT_ANYWHERE)
    , m_disableChangeAI(false)
    , m_chainPullEnabled(false)
    , m_shouldBeHealedAtEvade(true)
    ...
    , _lastDamagedTime(0)
    , m_respawnRate(1.0f)
    , m_needsMotionInitialize(false)
    , _mechanicImmuneMask(0)
{
...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        GetMap()->InsertToObjectsStore(this);
        ...
        if (GetInstanceScript())
            if (GetInstanceScript()->IsMythic())
                if (!sMythicMgr.ShouldBeSpawned(this))
                    RemoveFromWorld();
    }
}
...
bool Creature::UpdateEntry(uint32 Entry, const CreatureData* data, bool changelevel)
{
    ...
    SetDetectionRange(cInfo->detection_range);

    if (m_mythicScaling)
    {
        m_mythicBoss = sMythicMgr.AmIMythicBoss(Entry);
        m_mythicScaling->Initialize();
        m_mythicScaling->ScaleToKeyLevel();
    }

    sBoostMgr->ApplyBoostsStatOnSpawn(this);
    return true;
}

bool Creature::IsSubjectToAllDiminishingReturns() const
{
    if (InstanceScript* instance = const_cast<Creature*>(this)->GetInstanceScript())
        if (instance->IsMythic() && instance->GetMythicKeyLevel() >= sWorld->getIntConfig(CONFIG_MYTHIC_DIMINISHING_RETURNS_KEY_LEVEL))
            return true;

    if (CreatureTemplate const* creatureTemplate = GetCreatureTemplate())
        return creatureTemplate->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH;
...
void Creature::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        ...
        case POWER_MANA: {
            bool hasCharmerOrOwner = GetCharmerOrOwnerGUID() != 0U;
            // Combat and any controlled creature
            if (IsInCombat() || hasCharmerOrOwner)
            {
                ...
                {
                    uint32 leftToMax = GetMaxPower(POWER_MANA) - GetPower(POWER_MANA);
                    addvalue         = leftToMax * 0.06;
                }
            }
            else
                addvalue = maxValue / 3;
...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    _mechanicImmuneMask = cTemplate->MechanicImmuneMask;

    LoadCreaturesAddon();

    uint32 displayID               = IsNonPetGuardian() ? ObjectMgr::ChooseDisplayId(cTemplate) : GetNativeDisplayId();
    CreatureModelInfo const* minfo = sObjectMgr->GetCreatureModelRandomGender(&displayID);
...
{
    return m_lootRecipient ? ObjectAccessor::FindPlayerInOrOutOfWorld(m_lootRecipient) : nullptr;
}
...
bool Creature::IsLootAllowedFor(Player const* player) const
{
    ...
    if (!IsPlayerAllowedToLoot(player->GetGUID()))
        return false;
...
void Creature::SetLootRecipient(Unit* unit, bool withGroup)
{
    ...
    if (!unit)
    {
        ...
        if (sCustomMgr->GetBoolConfig(CONFIG_AOE_LOOT_ENABLED) && !m_lootRecipient)
            if (Player* player = ObjectAccessor::GetPlayer(*this, m_lootRecipient))
                player->RemoveAvailableCreatureForLoot(GetGUID());
    ...
    if (!player || player->IsPathOfWar()) // normal creature, no player involved
        return;

    m_lootRecipient = player->GetGUID();

    if (sCustomMgr->GetBoolConfig(CONFIG_AOE_LOOT_ENABLED))
        player->AddAvailableCreatureForLoot(GetGUID());

    uint32 encounterIndexMask = sObjectMgr->GetDungeonEncounterIndexMaskFromEntry(GetEntry());
    auto const CanPlayerLoot = [encounterIndexMask](Player const* player)
    {
        if (player->IsPathOfWar())
            return false;

        if (encounterIndexMask && sCustomMgr->IsAscendedModeAllowedMapID(player->GetMapId()) && player->GetMap()->Is25ManRaid())
            if (InstancePlayerBind const* instancePlayerBind = sInstanceSaveMgr->PlayerGetBoundInstance(player->GetGUIDLow(), player->GetMapId(), RAID_DIFFICULTY_10MAN_NORMAL))
                ...
                if (map && map->IsDungeon())
                {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        if (Player* i_pl = i->GetSource())
                            if (i_pl->GetGroup() == group && CanPlayerLoot(i_pl))
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_DEL_CREATURE);
...
void Creature::SelectLevel(bool changelevel)
{
    ...
    bool overrideExpansion = false;
    if (BoostStatValue const* statType = sBoostMgr->GetBoostStat(BoostStatType::Level, this))
    {
        level = urand(statType->ValueMin, statType->ValueMax);
        overrideExpansion = true;
    }
    ...
    uint32 basehp = std::max<uint32>(1, overrideExpansion ? static_cast<uint32>(ceil(stats->BaseHealth[EXPANSION_WRATH_OF_THE_LICH_KING] * cInfo->ModHealth)) : stats->GenerateHealth(cInfo));
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId, const CreatureData* data)
{
    ...
    if (instance && instance->IsMythic())
        m_mythicScaling = std::make_unique<MythicScaling>(this, normalInfo->MythicTemplateId);
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map* map, bool addToMap, bool gridLoad)
{
    ...
    if (m_deathState != ALIVE)
        SetHealth(0);
    //! Do not set curhealth for mythic creatures, we want them always at full health
    //! if you need mythic creature that starts with half health, you've got to script it
    else if (!m_mythicScaling)
        SetHealth(curhealth);
...
bool Creature::CanStartAttack(Unit const* who, bool dueToChainPull /*= false*/) const
{
    ...
    return !dueToChainPull ? IsWithinLOSInMap(who) : true;
}
...
void Creature::setDeathState(DeathState s, bool despawn)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        GetMap()->ApplyDynamicModeRespawnScaling(this, &respawnDelay, &corpseDelay);

        m_corpseRemoveTime = time(nullptr) + corpseDelay;
        //! its quick enough, so we're not modyifing it
        if (respawnDelay < 30U)
            m_respawnTime = time(nullptr) + respawnDelay + corpseDelay;
        else
        {
            uint32 _respawnDelay = std::max(45U, uint32(respawnDelay * m_respawnRate));
            m_respawnTime        = time(nullptr) + _respawnDelay + corpseDelay;
        }
...
void Creature::ForcedDespawn(uint32 timeMSToDespawn, uint32 forceRespawnTimer)
{
    ...
    if (overrideRespawnTime)
    {
        // ApplyDynamicModeRespawnScaling for forced alived creature already processed in Creature::setDeathState(JUST_DIED)
        if (!isAlive)
        {
...
{
    return !IS_PLAYER_GUID(GetOwnerGUID()) && (_mechanicImmuneMask & mask);
}
...
{
    if (radius <= 0.0f || !GetVictim() || !IsAlive() || IsPet() || IsCharmed())
        return;
...
bool Creature::CanCreatureAttack(Unit const* victim, bool skipDistCheck) const
{
    ...
    if (m_respawnedTime && !victim->GetCharmerOrOwnerPlayerOrPlayerItself() && (sWorld->GetGameTime() - m_respawnedTime) < 5 && victim->getAttackers().empty())
        return false;
    ...
    float dist = std::max(GetAggroRange(victim), sWorld->getFloatConfig(CONFIG_CREATURE_EVADE_HOMEDIST)) + m_CombatDistance;
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    if (m_mythicScaling)
    {
        m_mythicScaling->HandleAffixProc(Mythic::MYTHIC_PROC_FLAG_ON_CREATURE_ADDON_LOAD);
        if (m_mythicScaling->LoadCreatureAddon())
            return false;
    }
...
void Creature::SendZoneUnderAttackMessage(Player* attacker)
{
    ...
    sWorld->SendGlobalMessage(&data, nullptr, (attacker->GetTeam() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE));
}
...
bool Creature::IsSpellProhibited(SpellSchoolMask idSchoolMask) const
{
    ...
    if (!IsSubjectToAllDiminishingReturns() && (isWorldBoss() || IsDungeonBoss()))
        return false;
...
void Creature::ScheduleGroupChainPull()
{
    ...
    auto const scheduleTime = m_chainPullEnabled ? 1ms : 8000ms;
...
void NearbyCreatureChainPullDo::operator()(Creature* creature) const
{
    if (!creature || !creature->IsAIEnabled || !attackTarget || !caller || creature == caller || creature->IsTrigger() || creature->IsCritter() || creature->IsPassive())
        return;
    ...
    switch (_moveGen)
    {
        case IDLE_MOTION_TYPE:
            creature->DisableChainPullFor(4500ms);
        ...
        case POINT_MOTION_TYPE:
            creature->DisableChainPullFor(3000ms);
...
bool DelayedAttackStartEvent::Execute(uint64 /*eTime*/, uint32 /*pTime*/)
{
    ...
    InstanceScript* instance = m_owner->GetInstanceScript();
    bool const _hasAI        = m_owner->AI() != nullptr;
    if (_canAttack && _hasAI)
        m_owner->AI()->AttackStart(target);
    else if (!IsInCombat && instance && _hasAI)
    {
        if (!m_owner->AI()->DoZoneInCombat(m_owner, 150.f))
            m_owner->AI()->EnterEvadeMode();
    }
    else if (!IsInCombat)
    {
        m_owner->SetTarget(0);
