...
void CreatureGroup::FormationReset(bool dismiss)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (itr->first != m_leader && itr->first->IsAlive() && !itr->first->IsInCombat())
        {
            if (dismiss)
            {
                // Initializating MotionMaster when creature is in combat might break his current movement, eg. creature will start going to next waypoint when fighting with player
                // MotionMaster will be intialized on creature evade
                if (itr->first->IsInCombat())
                    itr->first->SetNeedsMotionInitialize(true);
                else
