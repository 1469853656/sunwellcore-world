...
{
    /*
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_STATE_VALUE);
...
void MythicMgr::DoMythicShuffle()
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_MYTHIC_STATE_VALUE);
    stmt->setUInt32(0, Mythic::MYTHIC_STATE_NEXT_SHUFFLE);
...
void MythicMgr::LoadCreatureMythicTemplates()
{
    ...
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_CREATURE_MYTHIC_TEMPLATES);
...
void MythicMgr::LoadMythicSpells()
{
    ...
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_MYTHIC_SPELL_DATA);
    PreparedQueryResult result = WorldDatabase.Query(stmt);
...
void MythicMgr::MythicPowerLog(uint32 guid, Mythic::MythicPowerLogType type, uint32 data1, uint32 data2, int32 value)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_MYTHICPOWER_LOG);
...
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MYTHIC_MAP_DATA);
...
{
    auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_MYTHIC_AFFIX_DATA);
    ASSERT(stmt);
    PreparedQueryResult result = WorldDatabase.Query(stmt);
    ASSERT(result);
    ...
    if ( m_currentHandledFuture )
    {
        if (isFutureReady(m_currentHandledFuture.get()))
        {
            ...
            if ( !m_recordStore.empty() )
            {
                ...
                {
                    auto stmt = CharacterDatabase.GetPreparedStatement( CHAR_REP_MYTHIC_RECORD );
