...
Pet::Pet(Player* owner, PetType type)
    ...
    , asynchLoadType(PET_LOAD_DEFAULT)
{
...
bool Pet::LoadPetFromDB(Player* owner, uint8 asynchLoadType, uint32 petentry, uint32 petnumber, bool current, std::shared_ptr<AsynchPetSummon> info)
{
    ...
    owner->GetSession()->GetQueryProcessor().AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback([session = owner->GetSession(), petLoadInfo = std::move(info), asynchLoadType](PreparedQueryResult resultPetLoad)
    {
        ...
        if (player->IsInWorld())
        {
            uint8 const loadResult = session->HandleLoadPetFromDBFirstCallback(std::move(resultPetLoad), asynchLoadType);
            if (loadResult != PET_LOAD_OK)
                HandleAsynchLoadFailed(std::move(petLoadInfo), player, asynchLoadType, loadResult);
        }
    }));
...
void Pet::SavePetToDB(PetSaveMode mode, bool logout, bool playerSave /*= false*/)
{
    ...
    if (mode == PET_SAVE_AS_CURRENT && owner->GetTemporaryUnsummonedPetNumber() && owner->GetTemporaryUnsummonedPetNumber() != m_charmInfo->GetPetNumber())
    {
    ...
    if (mode >= PET_SAVE_AS_CURRENT)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_PET_BY_ID);
    ...
    else
    {
        ...
        sLog->outDebug(DebugLogFilters::LOG_FILTER_PETS, "Deleting pet from database: Id: %u, Entry: %u, Slot: %u, Owner: [" UI64FMTD "], Name: %s, Guid [%u], OwnerPetGuid [" UI64FMTD "], Duration %d",
...
void Pet::GivePetXP(uint32 xp)
{
    ...
    xp = sCustomMgr->CalculateBonusXPWithRateUpdate(GetOwner(), xp);
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case NPC_INFERNAL:
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, 4 * (lowAmt * lowAmt * lowAmt));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, 4 * (highAmt * highAmt * highAmt));
                ...
                case NPC_TREANT: //force of nature
                {
                    if (!pInfo)
                    {
                        SetCreateHealth(30 + 120 * petlevel);
                        SetModifierValue(UNIT_MOD_HEALTH, BASE_VALUE, GetCreateHealth());
                    }
                ...
                {
                    SetCreateHealth(30 + 200 * petlevel);
                    SetModifierValue(UNIT_MOD_HEALTH, BASE_VALUE, GetCreateHealth());
    ...
    if (GetEntry() == NPC_RISEN_GHOUL)
    {
        ...
        if (m_owner->getRace() == RACE_ORC)
            CastSpell(this, SPELL_ORC_RACIAL_COMMAND, true, nullptr, nullptr, m_owner->GetGUID());
...
void Pet::_SaveSpellCooldowns(const CharacterDatabaseTransaction& trans, bool logout)
{
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_PET_SPELL_COOLDOWNS);
    stmt->setUInt32(0, m_charmInfo->GetPetNumber());
    trans->Append(stmt);
...
void Pet::_SaveSpells(const CharacterDatabaseTransaction& trans)
{
...
void Pet::_SaveAuras(const CharacterDatabaseTransaction& trans, bool logout)
{
    ...
    for (AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end(); ++itr)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_PET_AURA);
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        if (state == PETSPELL_UNCHANGED) // spell load case
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INVALID_PET_SPELL);
    ...
    if (!m_loading)
        ...
        {
            if (aura->GetSpellInfo()->CasterAuraState == AURA_STATE_HEALTHLESS_35_PERCENT || aura->GetSpellInfo()->CasterAuraState == AURA_STATE_HEALTH_ABOVE_75_PERCENT || aura->GetSpellInfo()->CasterAuraState == AURA_STATE_HEALTHLESS_20_PERCENT)
    ...
    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i)
        if (UnitActionBarEntry const* ab = m_charmInfo->GetActionBarEntry(i))
            if (ab->GetAction() && ab->IsActionBarForSpell())
            {
                ...
                {
                    ActiveStates state = ACT_PASSIVE;
                    uint32 newSpellId  = ab->GetAction();
                    //checking newer spells
                    do
                    {
                        newSpellId = sSpellMgr->GetNextSpellInChain(newSpellId);
                        if (HasSpell(newSpellId))
                        {
                            state = ab->GetType();
                            break;
                        }
                    ...
                    m_charmInfo->SetActionBar(i, newSpellId, state);
                }
...
void Pet::resetTalentsForAllPetsOf(Player* owner, Pet* online_pet /*= NULL*/)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_PET);
...
void Pet::HandleAsynchLoadSucceed()
{
    ...
    for (AuraApplicationMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
    {
        ...
        if (m_spellInfo->CasterAuraState != AURA_STATE_HEALTHLESS_20_PERCENT && m_spellInfo->CasterAuraState != AURA_STATE_HEALTHLESS_35_PERCENT && m_spellInfo->CasterAuraState != AURA_STATE_HEALTH_ABOVE_75_PERCENT)
            continue;
...
void Pet::HandleAsynchLoadFailed(std::shared_ptr<AsynchPetSummon> info, Player* player, uint8 asynchLoadType, uint8 loadResult)
{
...
void Pet::HandleIcecrownCitadelBuffs()
{
    if (IS_PLAYER_GUID(GetOwnerGUID()) && GetMapId() == 631 && FindMap() && FindMap()->ToInstanceMap() && FindMap()->ToInstanceMap()->GetInstanceScript() && FindMap()->ToInstanceMap()->GetInstanceScript()->GetData(251 /*DATA_BUFF_AVAILABLE*/))
    {
        ...
        if (Unit* owner = GetOwner())
        {
            if (Player* plr = owner->ToPlayer())
            {
                ...
                if (!_spellIdMap.empty())
                {
                    //! [if] map is heroic then we have to select a lowest possible buff (buff value)
                    auto itr = plr->FindMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC ? std::min_element(_spellIdMap.begin(), _spellIdMap.end(), [](const auto& left, const auto& right) {
