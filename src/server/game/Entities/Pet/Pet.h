...
public:
    AsynchPetSummon(uint32 entry, Position position, PetType petType, uint32 duration, uint32 createdBySpell, uint64 casterGUID)
        : pos(position)
        , m_entry(entry)
        , m_createdBySpell(createdBySpell)
        , m_duration(duration)
        , m_petType(petType)
...
public:
    ...
    static bool LoadPetFromDB(Player* owner, uint8 asynchLoadType, uint32 petentry = 0, uint32 petnumber = 0, bool current = false, std::shared_ptr<AsynchPetSummon> info = nullptr);
    ...
    void _SaveAuras(const CharacterDatabaseTransaction& trans, bool logout);
    void _SaveSpells(const CharacterDatabaseTransaction& trans);
    ...
    static void HandleAsynchLoadFailed(std::shared_ptr<AsynchPetSummon> info, Player* player, uint8 asynchLoadType, uint8 loadResult);
