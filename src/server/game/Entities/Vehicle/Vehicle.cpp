...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion, uint8 type, uint32 summonTime)
{
    ...
    if (_status == STATUS_UNINSTALLING)
    {
        sLog->outError("Vehicle GuidLow: %u, Entry: %u attempts to install accessory Entry: %u on seat %d with STATUS_UNINSTALLING! "
                       "Check Uninstall/PassengerBoarded script hooks for errors.",
            _me->GetGUIDLow(), _me->GetEntry(), entry, (int32)seatId);
        return;
    }
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
        ...
        && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    {
        ...
        catch (...)
        {
            ...
            sLog->outString("ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). not null: %u, this not null: %u", _me ? 1 : 0, 1);
            if (!_me)
                return false;
