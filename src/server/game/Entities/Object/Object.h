...
{
    constexpr float LEEWAY_MIN_MOVE_SPEED = 4.97f;
    constexpr float LEEWAY_BONUS_RANGE = 2.66f;
}
...
constexpr float AI_RELOCATION_NOTIFIER_RANGE    = 60.f;
};
...
public:
    ...
    bool GetClosePoint(float& x, float& y, float& z, float size, float distance2d = 0, float angle = 0, const WorldObject* forWho = nullptr, bool force = false, bool additionalHeightCheck = true) const;
    ...
    static void UpdateAllowedPositionZ(PathGeneratorContext const* context, float x, float y, float &z);
    ...
    {
        if (IsInWorld()) SendMessageToSetInRange(data, GetVisibilityRange(), self, true);
    }                                                                                                                                              // pussywizard!
    ...
    {
        if (IsInWorld()) SendMessageToSetInRange(data, GetVisibilityRange(), false, true, skipped_rcvr);
    } // pussywizard!
    ...
    TempSummon* SummonCreature(uint32 id, const Position& pos, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, uint32 vehId = 0, SummonPropertiesEntry const* properties = nullptr, uint32 phaseMask = 0) const;
    TempSummon* SummonCreature(uint32 id, float x, float y, float z, float ang = 0, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, SummonPropertiesEntry const* properties = nullptr, uint32 phaseMask = 0)
    {
        if (!x && !y && !z)
        {
        ...
        return SummonCreature(id, pos, spwtype, despwtime, 0, properties, phaseMask);
    }
    ...
    void GetGameObjectListInGrid(std::list<GameObject*>& lList, float fMaxSearchRange) const;
    ...
    virtual void FillPlayersAllowedToLoot(std::set<uint64>* /*s*/) const { }

    void ScheduleDelayedGameEventAIHook(uint16 eventID, bool enabled);
...
private:
    ...
    virtual bool _IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D = true, bool useBoundingRadius = true) const;
    ...
    std::optional<std::pair<uint16, bool>> _delayedGameEventAIHook;
};
