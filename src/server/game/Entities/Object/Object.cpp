...
void WorldObject::Update(uint32)
{
    if (_delayedGameEventAIHook)
    {
        uint16 const eventID = _delayedGameEventAIHook->first;
        bool const enabled = _delayedGameEventAIHook->second;
        _delayedGameEventAIHook.reset();

        if (IsInWorld())
        {
            if (Creature const* creature = ToCreature(); creature && creature->IsAIEnabled)
                creature->AI()->sOnGameEvent(enabled, eventID);
            else if (GameObject const* go = ToGameObject(); go && go->AI())
                go->AI()->OnGameEvent(enabled, eventID);
        }
    }
}
    ...
    *data << uint16(flags); // update flags
    ...
    if (flags & UPDATEFLAG_LIVING)
    {
        ...
        *data << unit->GetSpeed(MOVE_WALK)
        ...
        {
            if (unit->movespline->_Spline().getPoints(true).empty())
...
void Object::_LoadIntoDataField(std::string const& data, uint32 startOffset, uint32 count)
{
    ...
    {
        m_uint32Values[startOffset + index] = std::atol(tokens[index]);
...
void WorldObject::ScheduleDelayedGameEventAIHook(uint16 eventID, bool enabled)
{
    _delayedGameEventAIHook = std::make_pair(eventID, enabled);
}
...
bool WorldObject::_IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D /* = true */, bool useBoundingRadius /* = true */) const
{
...
float WorldObject::GetLeewayBonusRange(Unit const* target, bool ability) const
{
    ...
    if (Player const* targetPlayer = target->ToPlayer())
        return GetLeewayBonusRangeForTargets(targetPlayer, unit, ability);
...
{
    if (ability)
        return (player->GetXZFlagBasedSpeed() > ObjectConstants::LEEWAY_MIN_MOVE_SPEED && target->GetXZFlagBasedSpeed() > ObjectConstants::LEEWAY_MIN_MOVE_SPEED) ? ObjectConstants::LEEWAY_BONUS_RANGE : 0.0f;

    // auto attacks do not check speed, only flags
    return (player->IsMovingButNotWalking() && target->IsMovingButNotWalking()) ? ObjectConstants::LEEWAY_BONUS_RANGE : 0.0f;
}

float WorldObject::GetLeewayBonusRadius() const
{
    if (Player const* player = ToPlayer())
        if ((player->GetXZFlagBasedSpeed() > ObjectConstants::LEEWAY_MIN_MOVE_SPEED) || player->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING))
            return ObjectConstants::LEEWAY_BONUS_RANGE;
...
void WorldObject::UpdateAllowedPositionZ(PathGeneratorContext const* context, float x, float y, float& z)
{
    if (IS_CRE_OR_VEH_OR_PET_GUID(context->GetSourceGUID()))
    {
        // non fly unit don't must be in air
        // non swim unit must be at ground (mostly speedup, because it don't must be in water and water level check less fast
        if (!context->CanSourceFly())
        {
            bool canSwim   = context->CanSourceSwim();
            float ground_z = z;
            float max_z    = canSwim
                ? context->GetMap()->GetWaterOrGroundLevel(context->GetPhaseMask(), x, y, z, &ground_z, !context->CanSourceWaterWalk())
                : ((ground_z = context->GetMap()->GetHeight(context->GetPhaseMask(), x, y, z, true)));
            if (max_z > INVALID_HEIGHT)
            {
                if (z > max_z)
                    z = max_z;
        ...
        {
            float ground_z = context->GetMap()->GetHeight(context->GetPhaseMask(), x, y, z, true);
            if (z < ground_z)
                z = ground_z;
        }
    }
    else if (IS_PLAYER_GUID(context->GetSourceGUID()))
    {
        // for server controlled moves playr work same as creature (but it can always swim)
        if (!context->CanSourceFly())
        {
            float ground_z = z;
            float max_z    = context->GetMap()->GetWaterOrGroundLevel(context->GetPhaseMask(), x, y, z, &ground_z, !context->CanSourceWaterWalk());
            if (max_z > INVALID_HEIGHT)
            {
                if (z > max_z)
                    z = max_z;
...
float WorldObject::GetVisibilityRange() const
{
    if (isActiveObject() && !ToPlayer())
        ...
        return IsInWintergrasp() ? GetVisibilityRangeForWintergrasp() + VisibilityConstants::VISIBILITY_INC_FOR_GOBJECTS : GetMap()->GetVisibilityRange() + VisibilityConstants::VISIBILITY_INC_FOR_GOBJECTS;
    else
        return IsInWintergrasp() ? GetVisibilityRangeForWintergrasp() : GetMap()->GetVisibilityRange();
}
        ...
        if (ToPlayer())
        {
            if (target)
            {
                if (target->isActiveObject() && !target->ToPlayer())
                    ...
                    return IsInWintergrasp() && target->IsInWintergrasp() ? GetVisibilityRangeForWintergrasp() + VisibilityConstants::VISIBILITY_INC_FOR_GOBJECTS : GetMap()->GetVisibilityRange() + VisibilityConstants::VISIBILITY_INC_FOR_GOBJECTS;
...
bool WorldObject::CanSeeOrDetect(WorldObject const* obj, bool ignoreStealth, bool distanceCheck, bool checkAlert) const
{
    ...
    if (GameObject const* gobj = obj->ToGameObject())
        if (gobj->AI() && ToPlayer() && !gobj->AI()->CanBeSeen(ToPlayer()))
            return false;
    ...
    else if ((IsPlayer() && (ToPlayer()->GetSession()->GetSecurity() == SEC_PLAYER || !ToPlayer()->IsGameMaster())) || (obj->IsPlayer() && obj->ToPlayer()->HasSpymodeEnabled()) || !IsPlayer())
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= NULL*/, uint32 duration /*= 0*/, Unit* summoner /*= NULL*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/, uint32 phaseMask /*= 0*/)
{
    ...
    if (!summon->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), this, phaseMask, entry, vehId, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation()))
    {
        delete summon;
        return nullptr;
    }
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position& pos, TempSummonType spwtype, uint32 duration, uint32 vehId, SummonPropertiesEntry const* properties, uint32 phaseMask /*= 0*/) const
{
    if (Map* map = FindMap())
    {
        if (TempSummon* summon = map->SummonCreature(entry, pos, properties, duration, isType(TYPEMASK_UNIT) ? (Unit*)this : nullptr, 0, 0, phaseMask))
        {
...
void WorldObject::GetGameObjectListInGrid(std::list<GameObject*>& goList, float maxSearchRange) const
{
    CellCoord pair(Trinity::ComputeCellCoord(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Trinity::AllGameObjectsRange check(this, maxSearchRange);
    Trinity::GameObjectListSearcher<Trinity::AllGameObjectsRange> searcher(this, goList, check);
    TypeContainerVisitor<Trinity::GameObjectListSearcher<Trinity::AllGameObjectsRange>, GridTypeMapContainer> visitor(searcher);

    cell.Visit(pair, visitor, *(this->GetMap()), *this, maxSearchRange);
}

void WorldObject::GetCreatureListInGrid(std::list<Creature*>& lList, float fMaxSearchRange) const
{
    CellCoord pair(Trinity::ComputeCellCoord(this->GetPositionX(), this->GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    Trinity::AllCreaturesRange check(this, fMaxSearchRange);
    Trinity::CreatureListSearcher<Trinity::AllCreaturesRange> searcher(this, lList, check);
    TypeContainerVisitor<Trinity::CreatureListSearcher<Trinity::AllCreaturesRange>, GridTypeMapContainer> visitor(searcher);
...
bool WorldObject::GetClosePoint(float& x, float& y, float& z, float size, float distance2d, float angle, const WorldObject* forWho, bool force, bool additionalHeightCheck) const
{
    // angle calculated from current orientation
    GetNearPoint(forWho, x, y, z, size, distance2d, GetOrientation() + angle);

    if ((fabs(this->GetPositionZ() - z) > 3.0f && additionalHeightCheck) || !IsWithinLOS(x, y, z))
    {
        x = this->GetPositionX();
        y = this->GetPositionY();
        z = this->GetPositionZ();
...
void WorldObject::MovePositionToFirstCollision(Position& pos, float dist, float angle)
{
    ...
    UpdateAllowedPositionZ(destx, desty, destz);
    // @Crackaw: if diff is more than 2 change your location on target location and move back a little bit
    //! @Riztazz: Do not make any corrections for flying creatures
    //! ToDo: fix this properly
    if (diff > 2.0f && ToUnit()->ToCreature() && !ToUnit()->IsFlying())
    {
