...
enum SpellValueMod
{
    ...
    SPELLVALUE_SPELL_RANGE_MIN,
    SPELLVALUE_ENABLE_SHARE_DAMAGE,
    SPELLVALUE_MODIFY_CAST_TIME,
    SPELLVALUE_CONE_ANGLE,
    SPELLVALUE_AURA_STATE,
    SPELLVALUE_FORCED_MISS_RESULT,
    SPELLVALUE_SPELL_INTERRUPT_FLAGS,
    SPELLVALUE_DISABLE_SPELL_STEAL_DISPEL,
    SPELLVALUE_DISABLE_REFLECT,
    SPELLVALUE_DAMAGE_OVERRIDE,
    SPELLVALUE_DAMAGE_PCT_OVERRIDE
};
...
public:
    int32 GetGlobalCooldown(SpellInfo const* spellInfo) const;
...
public:
    ...
    auto& GetPetActionBar() { return PetActionBar; }
...
{
public:
    explicit SafeUnitPointer(Unit* defVal)
        : ptr(defVal)
        , defaultValue(defVal)
    {
    }
...
public:
    ...
    void ResetAttackTimers(bool meleeOnly = false);
    ...
    bool IsWithinMeleeRange(const Unit* obj, float dist = MELEE_RANGE, bool ability = false) const;
    ...
    Unit* SelectNearbyTarget(Unit* exclude = nullptr, float dist = NOMINAL_MELEE_RANGE, bool excludeVictim = true) const;
    Unit* SelectNearbyNoTotemTarget(Unit* exclude = nullptr, float dist = NOMINAL_MELEE_RANGE, bool excludeVictim = true) const;
    ...
    uint8 getRace(CrossFactionOverride override = CF_ALLOW) const;
    uint32 getRaceMask(CrossFactionOverride override = CF_ALLOW) const { return 1 << (getRace(override) - 1); }
    ...
    void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 procPhase, uint32 amount, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = nullptr, SpellInfo const* procAura = nullptr, uint32 preMitigationDamage = 0);
    void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, uint32 procPhase, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura = nullptr, uint32 preMitigationDamage = 0);
    ...
    void AttackerStateUpdate(Unit* victim, WeaponAttackType attType = BASE_ATTACK, bool extra = false, bool meleeSwingOnly = false);

    void CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType = BASE_ATTACK, bool forceCrit = false, bool extra = false);
    ...
    float MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId, bool extra) const;
    ...
    MeleeHitOutcome RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType, bool extra) const;
    MeleeHitOutcome RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool extra) const;
    ...
    bool IsServiceProvider() const
    {
        return HasFlag(UNIT_NPC_FLAGS,
            UNIT_NPC_FLAG_VENDOR | UNIT_NPC_FLAG_TRAINER | UNIT_NPC_FLAG_FLIGHTMASTER | UNIT_NPC_FLAG_PETITIONER | UNIT_NPC_FLAG_BATTLEMASTER | UNIT_NPC_FLAG_BANKER | UNIT_NPC_FLAG_INNKEEPER | UNIT_NPC_FLAG_SPIRITHEALER | UNIT_NPC_FLAG_SPIRITGUIDE | UNIT_NPC_FLAG_TABARDDESIGNER | UNIT_NPC_FLAG_AUCTIONEER);
    }
    bool IsSpiritService() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPIRITHEALER | UNIT_NPC_FLAG_SPIRITGUIDE); }
    bool IsGuildBanker() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GUILD_BANKER); }
    ...
    bool IsInCombatWithHardcorePlayer() const;
    ...
    bool IsUnderCrowdControl() const;

    bool HasStealthAura() const { return HasAuraType(SPELL_AURA_MOD_STEALTH); }
    bool HasInvisibilityAura() const { return HasAuraType(SPELL_AURA_MOD_INVISIBILITY); }
    bool isFeared() const { return HasAuraType(SPELL_AURA_MOD_FEAR); }
    bool isStuned() const { return HasAuraType(SPELL_AURA_MOD_STUN); }
    ...
    void EnergizeBySpell(Unit* victim, uint32 SpellID, uint32 Damage, Powers powertype, bool sendLog = true);

    void CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags = TRIGGERED_NONE, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(Unit* victim, uint32 spellId, bool triggered, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags = TRIGGERED_NONE, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(Unit* victim, SpellInfo const* spellInfo, bool triggered, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(Unit* victim, SpellInfo const* spellInfo, TriggerCastFlags triggerFlags = TRIGGERED_NONE, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(float x, float y, float z, uint32 spellId, bool triggered, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastSpell(GameObject* go, uint32 spellId, bool triggered, ItemRef const& castItem = NullItemRef, AuraEffect* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastCustomSpell(Unit* victim, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, bool triggered, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* victim, bool triggered, ItemRef const& castItem = ItemRef(nullptr), AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* victim = nullptr, TriggerCastFlags triggerFlags = TRIGGERED_NONE, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    void CastCustomSpell(uint32 spellId, CustomSpellValues const& value, Unit* victim = nullptr, TriggerCastFlags triggerFlags = TRIGGERED_NONE, ItemRef const& castItem = NullItemRef, AuraEffect const* triggeredByAura = nullptr, uint64 originalCaster = 0);
    ...
    void AddSpellValueForUnit(uint32 spellID, CustomSpellValues&& value);
    void RemoveSpellValueForUnit(uint32 spellID);
    CustomSpellValues const* GetSpellValueForUnit(uint32 spellID) const;
    void ClearSpellValuesForUnit() { return _spellValuesForUnit.clear(); }
    ...
    Aura* _TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount = nullptr, ItemRef const& castItem = nullptr, uint64 casterGUID = 0, bool periodicReset = false);
    ...
    void RemoveAurasDueToDamage(uint32 flag, uint32 damage = 0, uint32 except = 0);
    void RemoveAurasWithAttribute(uint32 flags);
    void RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID);
    void RemoveAurasWithMechanic(uint32 mechanic_mask, AuraRemoveMode removemode = AURA_REMOVE_BY_DEFAULT, uint32 except = 0);
    void RemoveMovementImpairingAuras(bool withRoot);
    void RemoveAurasByShapeShift();
    void RemoveShapeshiftAuras(Aura* except, ShapeshiftForm newForm);
    ...
    int32 GetTotalAuraModifierWithBestOnly(AuraType auratype) const;
    float GetTotalAuraMultiplier(AuraType auratype) const;
    int32 GetMaxPositiveAuraModifier(AuraType auratype);
    int32 GetMaxNegativeAuraModifier(AuraType auratype) const;
    int32 GetTotalAuraModifier(AuraType auraType, std::function<bool(const AuraEffect*)> predicate) const;
    ...
    bool IsParryHasteAllowed() const;
    ...
    static uint32 CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, const uint32 damage, SpellInfo const* spellInfo, uint8 attackerLevel = 0, WeaponAttackType attackType = MAX_ATTACK, float armorDifference = 0.0f);
    ...
    void AddUnitMovementFlag(MovementFlags /*f*/);
    void RemoveUnitMovementFlag(MovementFlags /*f*/);
    bool HasUnitMovementFlag(MovementFlags /*f*/) const;
    MovementFlags GetUnitMovementFlags() const;
    void SetUnitMovementFlags(MovementFlags /*f*/);

    void AddExtraUnitMovementFlag(MovementFlags2 /*f*/);
    void RemoveExtraUnitMovementFlag(MovementFlags2 /*f*/);
    MovementFlags2 HasExtraUnitMovementFlag(MovementFlags2 /*f*/) const;
    MovementFlags2 GetExtraUnitMovementFlags() const;
    void SetExtraUnitMovementFlags(MovementFlags2 /*f*/);
    ...
    bool IsWalkingBackward() const { return m_movementInfo.HasMovementFlag(MOVEMENTFLAG_BACKWARD); }
    ...
    Optional<Position> ChargeFinalPos;

    void SetMythicLocked(bool state) { m_isMythicLocked = state; }
    bool IsMythicLocked() const { return m_isMythicLocked; }
...
private:
    bool IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, uint32 procPhase, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const*& spellProcEvent, ProcEventInfo const& eventInfo);
    bool HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, uint32 preMitigationDamage = 0);
    ...
    bool m_isMythicLocked;

    std::unordered_map<uint32, CustomSpellValues> _spellValuesForUnit;
};
...
{
public:
    ConflagrateAuraStateDelayEvent(uint64 ownerGUID, uint64 casterGUID, Map const* map)
        : BasicEvent()
        , m_owner(ownerGUID)
        , m_caster(casterGUID)
        , _map(map)
...
{
public:
    RedirectSpellEvent(Unit& self, uint64 auraOwnerGUID, AuraEffect* auraEffect)
        : _self(self)
        , _auraOwnerGUID(auraOwnerGUID)
