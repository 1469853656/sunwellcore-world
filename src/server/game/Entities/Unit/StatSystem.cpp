...
{
    m_modMeleeHitChance = (float)GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE, [this](AuraEffect const* aurEff) { return HasItemFitToSpellRequirements(aurEff->GetSpellInfo()); });
    m_modMeleeHitChance += GetRatingBonusValue(CR_HIT_MELEE);
}

void Player::UpdateRangedHitChances()
{
    m_modRangedHitChance = (float)GetTotalAuraModifier(SPELL_AURA_MOD_HIT_CHANCE, [this](AuraEffect const* aurEff) { return HasItemFitToSpellRequirements(aurEff->GetSpellInfo()); });
...
void Creature::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage)
{
    ...
    float totalValue    = GetModifierValue(unitMod, TOTAL_VALUE);
    float totalPct      = addTotalPct ? GetModifierValue(unitMod, TOTAL_PCT) : 1.0f;
    float dmgMultiplier = m_mythicScaling ? 1.f : GetCreatureTemplate()->dmg_multiplier;
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if (IsHunterPet())
    {
        ...
        // Cobra Reflexes: Increases your pet's attack speed, but reduces damage done by half of the bonus attack speed.
        for (AuraEffect const* aurEff : GetAuraEffectsByType(SPELL_AURA_MOD_ATTACKSPEED))
        {
            if (aurEff->GetId() != 61682 && aurEff->GetId() != 61683)
                continue;

            mindamage *= 1.0f - (aurEff->GetAmount() / 100.0f / 2.0f);
            maxdamage *= 1.0f - (aurEff->GetAmount() / 100.0f / 2.0f);
        }
    }
