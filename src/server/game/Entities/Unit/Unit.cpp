...
#pragma warning(default : 4355)
    ...
    m_CanFinishSwingWhileCasting       = false;
...
int32 GlobalCooldownMgr::GetGlobalCooldown(SpellInfo const* spellInfo) const
{
    GlobalCooldownList::const_iterator itr = m_GlobalCooldowns.find(spellInfo->StartRecoveryCategory);
    return (itr != m_GlobalCooldowns.end() && itr->second.duration) ? (int32)itr->second.duration - (int32)getMSTimeDiff(itr->second.cast_time, World::GetGameTimeMS()) : 0;
}
...
void Unit::Update(uint32 p_time)
{
    ...
    if (IsInCombat() && (GetTypeId() == TYPEID_PLAYER || (IsPet() && IsControlledByPlayer()) || GetEntry() == 28851))
    {
    ...
    {
        Spell const* spell = GetCurrentSpell(type);
        if (!spell)
            return false;

        return spell->GetSpellInfo()->HasAttribute(SPELL_ATTR6_DELAY_MELEE_DURING_CAST);
    };

    if (!spellPausesCombatTimer(CURRENT_GENERIC_SPELL) && !spellPausesCombatTimer(CURRENT_CHANNELED_SPELL))
        for (WeaponAttackType const attackType : { BASE_ATTACK, OFF_ATTACK, RANGED_ATTACK })
            if (uint32 const attackTimer = getAttackTimer(attackType))
                setAttackTimer(attackType, attackTimer > p_time ? attackTimer - p_time : 0);
...
public:
    ...
    {
        if ((result & (Movement::MoveSpline::Result_NextSegment | Movement::MoveSpline::Result_JustArrived)) && _unit->GetTypeId() == TYPEID_UNIT && _unit->GetMotionMaster()->GetCurrentMovementGeneratorType() == ESCORT_MOTION_TYPE && _unit->movespline->GetId() == _unit->GetMotionMaster()->GetCurrentSplineId())
        {
...
void Unit::ResetAttackTimers(bool meleeOnly)
{
...
bool Unit::IsWithinMeleeRange(const Unit* obj, float dist, bool ability) const
{
    if (!obj || !IsInMap(obj) || !InSamePhase(obj))
        return false;
    ...
    maxdist += GetLeewayBonusRange(obj, ability);
...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName, flag96 familyFlags) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if (SpellInfo const* iterSpellProto = (*itr)->GetSpellInfo())
            if (iterSpellProto->SpellFamilyName == familyName && iterSpellProto->SpellFamilyFlags.IsEqual(familyFlags))
                return true;
    return false;
}
...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool allowGM)
{
    ...
    // Piootrek: Warrior should get rage even if damage was mitigated
    if (!rage_damage && cleanDamage && cleanDamage->mitigated_damage)
    ...
    {
        InstanceScript* instance = attacker->GetInstanceScript();
        if (instance)
            if (Player* victimPlayer = victim->GetCharmerOrOwnerPlayerOrPlayerItself())
                instance->UpdateEncounterStatsForPlayer(victimPlayer, PlayerStatType::DamageTaken, damage);
        ...
        if (Player* killer = attacker->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            if (instance)
            {
                instance->UpdateEncounterStatsForPlayer(killer, PlayerStatType::EffectiveDamageDone, damage);

                uint32 rawDamage = damage;
                int32 overKill = int32(rawDamage) - int32(victim->GetHealth());
                if (overKill > 0)
                    rawDamage += overKill;

                instance->UpdateEncounterStatsForPlayer(killer, PlayerStatType::DamageDone, rawDamage);
            }
    ...
    bool const isPlayerPet = victim->IsPet() || victim->IsGuardian();
    if (damage > 0 && isPlayerPet)
    {
        bool const isMeleeAttack = damagetype == DIRECT_DAMAGE && cleanDamage && (cleanDamage->attackType == BASE_ATTACK || cleanDamage->attackType == OFF_ATTACK);

        if (!isMeleeAttack)
        {
            Player* playerPetOwner               = victim->GetCharmerOrOwnerPlayerOrPlayerItself();
            InstanceScript* mythicInstanceScript = playerPetOwner ? playerPetOwner->GetInstanceScript() : nullptr;
            if (playerPetOwner && mythicInstanceScript && mythicInstanceScript->IsMythic())
            {
                uint32 _maxHealth           = victim->GetMaxHealth();
                uint32 _maxAllowedMythicHit = _maxHealth * 0.08f;
                damage                      = std::min(_maxAllowedMythicHit, damage);
            }
        }
    }
...
void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    if (!spellInfo)
    {
    ...
    else if (CustomSpellValues const* spellValue = GetSpellValueForUnit(spellInfo->Id))
        for (CustomSpellValues::const_iterator itr = spellValue->begin(); itr != spellValue->end(); ++itr)
            spell->SetSpellValue(itr->first, itr->second);
...
void Unit::CastSpell(Unit* victim, uint32 spellId, bool triggered, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    CastSpell(victim, spellId, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster);
}

void Unit::CastSpell(Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags /*= TRIGGER_NONE*/, ItemRef const& castItem /*= NULL*/, AuraEffect const* triggeredByAura /*= NULL*/, uint64 originalCaster /*= 0*/)
{
...
void Unit::CastSpell(Unit* victim, SpellInfo const* spellInfo, bool triggered, ItemRef const& castItem /*= NULL*/, AuraEffect const* triggeredByAura /*= NULL*/, uint64 originalCaster /*= 0*/)
{
    CastSpell(victim, spellInfo, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster);
}

void Unit::CastSpell(Unit* victim, SpellInfo const* spellInfo, TriggerCastFlags triggerFlags, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
...
void Unit::CastCustomSpell(Unit* target, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, bool triggered, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
...
void Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, bool triggered, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    CustomSpellValues values;
...
void Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, TriggerCastFlags triggerFlags, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    CustomSpellValues values;
    values.AddSpellMod(mod, value);
...
void Unit::CastCustomSpell(uint32 spellId, CustomSpellValues const& value, Unit* victim, TriggerCastFlags triggerFlags, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
...
void Unit::CastSpell(float x, float y, float z, uint32 spellId, bool triggered, ItemRef const& castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
...
void Unit::CastSpell(GameObject* go, uint32 spellId, bool triggered, ItemRef const& castItem, AuraEffect* triggeredByAura, uint64 originalCaster)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType, bool forceCrit, bool extra)
{
    ...
    damageInfo->hitOutCome = RollMeleeOutcomeAgainst(damageInfo->target, damageInfo->attackType, extra);

    if (damageInfo->hitOutCome == MELEE_HIT_NORMAL && forceCrit)
        damageInfo->hitOutCome = MELEE_HIT_CRIT;

    if (damageInfo->hitOutCome == MELEE_HIT_NORMAL || damageInfo->hitOutCome == MELEE_HIT_CRIT)
    {
        if (Creature* actor = ToCreature())
            if (actor->m_mythicScaling)
                actor->m_mythicScaling->HandleAffixProc(Mythic::MYTHIC_PROC_FLAG_MELEE_ATTACK);
    }
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (victimCreature && victimCreature->IsAIEnabled)
    ...
    if (damageInfo->damage)
    {
        ...
        Unit::DealDamageMods(this, largestDamage, nullptr);
        ...
        Unit::DealDamage(victim, this, largestDamage, nullptr, SPELL_DIRECT_DAMAGE, largestDamageShield->GetSchoolMask(), largestDamageShield, true);
    }
}
...
uint32 Unit::CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, const uint32 damage, SpellInfo const* spellInfo, uint8 attackerLevel, WeaponAttackType /*attackType*/, float armorDifference)
{
    uint32 newdamage = 0;
    float armor      = 0;
...
void Unit::CalcAbsorbResist(Unit* attacker, Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo, bool Splited)
{
    ...
    // ToGC25 Boost
    if (victim && spellInfo && victim->GetInstanceScript() && victim->GetMapId() == 649 && victim->GetInstanceScript()->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
    {
        switch (spellInfo->Id)
        {
            case 68510:
                schoolMask = SpellSchoolMask(SPELL_SCHOOL_MASK_FIRE + SPELL_SCHOOL_MASK_FROST);
                break;
            case 70825:
                schoolMask = SpellSchoolMask(SPELL_SCHOOL_MASK_FIRE + SPELL_SCHOOL_MASK_FROST);
                break;
    ...
    if (/*victim != attacker &&*/ !Splited)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin(); (itr != vSplitDamageFlatCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            caster->ProcDamageAndSpellFor(true, caster, procVictim, procEx, PROC_SPELL_PHASE_HIT, BASE_ATTACK, spellInfo, splitted, nullptr);
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra, bool meleeSwingOnly)
{
    ...
    if (!m_CanFinishSwingWhileCasting && HasUnitState(UNIT_STATE_CASTING))
        return;

    if (!victim || !victim->IsAlive())
        return;

    // Hits against sitting units shouls be always a crit
    // Must be set here, because CombatStart() clears standState
    ...
    if (!result)
    {
        ...
        CalculateMeleeDamage(victim, 0, &damageInfo, attType, forceCrit, extra);
        ...
        ProcDamageAndSpell(damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, damageInfo.procEx, PROC_SPELL_PHASE_HIT, damageInfo.damage, damageInfo.attackType);
...
void Unit::HandleProcExtraAttackFor(Unit* victim)
{
    ...
    {
        if (victim->IsWithinMeleeRange(this, MELEE_RANGE, false))
            AttackerStateUpdate(victim, BASE_ATTACK, true);
        --m_extraAttacks;
    }
}

MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType, bool extra) const
{
    // This is only wrapper

    // Miss chance based on melee
    //float miss_chance = MeleeMissChanceCalc(victim, attType);
    float miss_chance = MeleeSpellMissChance(victim, attType, int32(GetWeaponSkillValue(attType, victim)) - int32(victim->GetMaxSkillValueForLevel(this)), 0, extra);

    // Critical hit chance
    float crit_chance = GetUnitCriticalChance(attType, victim);
    if (IsCreature() && ToCreature()->IsDancingRuneWeapon())
        if (Unit* owner = GetOwner())
            crit_chance = std::max(0.0f, owner->GetUnitCriticalChance(attType, victim));
    ...
    MeleeHitOutcome result = RollMeleeOutcomeAgainst(victim, attType, int32(crit_chance * 100), int32(miss_chance * 100), int32(dodge_chance * 100), int32(parry_chance * 100), int32(block_chance * 100), extra);
    return result;
}

MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool extra) const
{
    ...
    else if (victim->GetTypeId() == TYPEID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_DODGE))
    {
        ...
        bool isCastingOrControlled = victim->IsNonMeleeSpellCast(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED);

        if (victim->GetTypeId() == TYPEID_PLAYER && isCastingOrControlled)
            tmp = 0;
        else if (victim->GetTypeId() == TYPEID_UNIT && isCastingOrControlled && !victim->ToCreature()->IsTrainingDummy())
            tmp = 0;
    ...
    if (!extra && attType != RANGED_ATTACK && (GetTypeId() == TYPEID_PLAYER || IsGuardian()) && victim->GetTypeId() != TYPEID_PLAYER && !victim->IsGuardian() && getLevel() < victim->getLevelForTarget(this))
    {
...
uint32 Unit::CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct)
{
    ...
    if (IsCreature() && attType == BASE_ATTACK && GetMap()->Instanceable())
    {
        if (BoostStatValue const* stat = sBoostMgr->GetBoostStat(BoostStatType::MainhandDamage, ToCreature()))
        {
            minDamage = static_cast<float>(stat->ValueMin);
            maxDamage = static_cast<float>(stat->ValueMax);
        }

        if (BoostStatValue const* stat = sBoostMgr->GetBoostStat(BoostStatType::MainhandDamageModifier, ToCreature()))
        {
            minDamage = AddPct(minDamage, stat->ValueMin);
            maxDamage = AddPct(maxDamage, stat->ValueMax);
        }
    }

    if (IsCreature() && attType == OFF_ATTACK && GetMap()->Instanceable())
    {
        if (BoostStatValue const* stat = sBoostMgr->GetBoostStat(BoostStatType::OffhandDamage, ToCreature()))
        {
            minDamage = static_cast<float>(stat->ValueMin);
            maxDamage = static_cast<float>(stat->ValueMax);
        }

        if (BoostStatValue const* stat = sBoostMgr->GetBoostStat(BoostStatType::OffhandDamageModifier, ToCreature()))
        {
            minDamage = AddPct(minDamage, stat->ValueMin);
            maxDamage = AddPct(maxDamage, stat->ValueMax);
        }
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    uint32 missChance = uint32(MeleeSpellMissChance(victim, attType, skillDiff, spell->Id, false) * 100.0f);
    // Roll miss
    ...
    if (canBlock && victim->IsPlayer() && IsCreature() && spell->HasAttribute(SPELL_ATTR0_CU_DIRECT_DAMAGE))
        canBlock = false;
    ...
    for (AuraEffectList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            default:; //sLog->outStaticDebug("Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d", (*i)->GetId(), (*i)->GetMiscValue());
                break;
        }
    }
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (IsPlayer() && spell->HasEffect(SPELL_EFFECT_ATTACK_ME) && victim->IsTotem())
    ...
    if (Unit* owner = GetOwner())
        modHitChance += owner->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_INCREASES_SPELL_PCT_TO_HIT, schoolMask);
    ...
    if (!spell->IsPositive() && !spell->HasAttribute(SPELL_ATTR4_IGNORE_RESISTANCES) && !(spell->HasAttribute(SPELL_ATTR1_CU_IGNORE_RESISTANCES_PVE) && victim->IsCreature()))
    {
        ...
        if (spell->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) && (spell->GetSchoolMask() & (SPELL_SCHOOL_MASK_NORMAL | SPELL_SCHOOL_MASK_HOLY)) == 0)
        {
            ...
            //! Exclude additional bonus resistances for mythic creatures
            //! use it only for partial resists, otherwise they will resist binary spells all over
            Creature* victimCreature = victim->ToCreature();
            InstanceScript* instance = victimCreature ? victimCreature->GetInstanceScript() : nullptr;
            //! victimCreature has to be valid if instance is valid
            if (instance && victimCreature && victimCreature->m_mythicScaling)
                tmp = std::max(static_cast<int32>(0), tmp -= victimCreature->m_mythicScaling->m_additionalResistances);
        }
    }
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
    ...
    if (CanReflect)
    {
        ...
        if (reflectchance > 0 && roll_chance_i(reflectchance))
        {
            ...
            //ProcDamageAndSpell(victim, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, PROC_SPELL_PHASE_HIT, 1, BASE_ATTACK, spell);
...
float Unit::GetUnitParryChance() const
{
    ...
    if (Player const* player = ToPlayer())
    {
        if (player->CanParry())
        {
            ItemRef tmpitem = player->GetWeaponForAttack(BASE_ATTACK, true);
            if (!tmpitem)
...
float Unit::GetUnitBlockChance() const
{
    if (Player const* player = ToPlayer())
    {
        if (player->CanBlock())
        {
            ItemRef tmpitem = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
...
uint32 Unit::GetWeaponSkillValue(WeaponAttackType attType, Unit const* target) const
{
    ...
    if (Player const* player = ToPlayer())
    {
        ItemRef item = player->GetWeaponForAttack(attType, true);
...
void SafeUnitPointer::UnitDeleted()
{
    ...
    if (defaultValue)
    {
        ...
        {
            sLog->outMisc("SafeUnitPointer::UnitDeleted (A1) - %u, %u, %u, %p, %u, %u, %u, %u", p->GetGUIDLow(), p->GetMapId(), p->GetInstanceId(), p->FindMap(), p->IsInWorld() ? 1 : 0, p->IsDuringRemoveFromWorld() ? 1 : 0, p->IsBeingTeleported() ? 1 : 0, p->isBeingLoaded() ? 1 : 0);
            if (ptr)
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, ItemRef const& castItem /*= NULL*/, uint64 casterGUID /*= 0*/, bool periodicReset /*= false*/)
{
...
void Unit::_ApplyAura(AuraApplication* aurApp, uint8 effMask)
{
    ...
    if (AuraStateType aState = spellInfo->GetAuraState())
    {
        ...
        {
            ConflagrateAuraStateDelayEvent* pEvent = new ConflagrateAuraStateDelayEvent(GetGUID(), caster->GetGUID(), caster->GetMap());
...
void Unit::RemoveAurasDueToDamage(uint32 flag, uint32 damage, uint32 except)
{
    ...
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin(); iter != m_interruptableAuras.end();)
    {
        ...
        if (shouldBeRemoved && (aura->GetSpellInfo()->AuraInterruptFlags & flag) && (!except || aura->GetId() != except))
        {
            ...
            constexpr uint32 SPELL_SCATTER_SHOT = 19503;
            bool mechanicImmuneCheck            = HasAuraWithMechanic(MECHANIC_IMMUNE_SHIELD) && !HasAura(SPELL_SCATTER_SHOT);
            if ((aura->HasEffectType(SPELL_AURA_MOD_STEALTH) || mechanicImmuneCheck) && !damage)
                continue;
...
void Unit::RemoveAurasByShapeShift()
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_CU_IGNORE_REMOVE_MECHANICS))
        {
            if ((aura->GetSpellInfo()->GetAllEffectsMechanicMask() & mechanic_mask) && (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_CU_AURA_CC) || (aura->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_WARRIOR && (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x20))))
            {
                RemoveAura(iter);
                continue;
            }
...
void Unit::RemoveShapeshiftAuras(Aura* except, ShapeshiftForm newForm)
{
    auto IsWarriorStanceForm = [&](ShapeshiftForm form)
    {
        switch (form)
        {
    ...
    for (AuraEffectList::iterator iter = m_modAuras[SPELL_AURA_MOD_SHAPESHIFT].begin(); iter != m_modAuras[SPELL_AURA_MOD_SHAPESHIFT].end();)
    {
        Aura* aura = (*iter)->GetBase();
        AuraApplication* aurApp = aura->GetApplicationOfTarget(GetGUID());
        ShapeshiftForm form = ShapeshiftForm((*iter)->GetMiscValue());

        ++iter;
        if (aura != except)
        {
            bool removeForm = true;
            switch (form)
            {
                case FORM_BATTLESTANCE:
                case FORM_DEFENSIVESTANCE:
                case FORM_BERSERKERSTANCE:
                    if (IsWarriorStanceForm(newForm))
                        break;
            ...
            if (removeForm)
            {
                ...
                RemoveAura(aurApp);
                if (m_removedAurasCount > removedAuras + 1)
                    iter = m_modAuras[SPELL_AURA_MOD_SHAPESHIFT].begin();
            }
        }
    }
}
...
void Unit::RemoveEvadeAuras()
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (!aura->GetSpellInfo()->IsSpellRemovedOnEvade() || (aura->IsPassive() && IS_PLAYER_GUID(GetOwnerGUID())))
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
...
void Unit::GetDispellableAuraList(Unit* caster, uint32 dispelMask, DispelChargesList& dispelList)
{
    ...
    for (Unit::VisibleAuraMap::const_iterator itr = visibleAuras->begin(); itr != visibleAuras->end(); ++itr)
    {
        ...
        if (aura->IsPassive() || aura->IsSpellStealAndDispelDisabled())
            continue;
...
int32 Unit::GetTotalAuraModifierWithBestOnly(AuraType auratype) const
{
    AuraEffectList const& mTotalAuraList = GetAuraEffectsByType(auratype);
    if (mTotalAuraList.empty())
        return 0;
    ...
    int32 modifier = 0;
    std::unordered_map<uint32, int32> bestModifiers;
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetAmount() > bestModifiers[(*i)->GetSpellInfo()->Id])
            bestModifiers[(*i)->GetSpellInfo()->Id] = (*i)->GetAmount();
    }

    for (auto const& itr : bestModifiers)
        modifier += itr.second;
...
int32 Unit::GetTotalAuraModifier(AuraType auraType, std::function<bool(const AuraEffect*)> predicate) const
{
    int32 modifier = 0;
    for (AuraEffect const* aurEff : GetAuraEffectsByType(auraType))
        if (predicate(aurEff))
...
void Unit::ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 procPhase, uint32 amount, WeaponAttackType attType, SpellInfo const* procSpell, SpellInfo const* procAura, uint32 preMitigationDamage)
{
    // Not much to do if no flags are set.
    if (procAttacker)
        ProcDamageAndSpellFor(false, victim, procAttacker, procExtra, procPhase, attType, procSpell, amount, procAura, preMitigationDamage);
    // Now go on with a victim's events'n'auras
    // Not much to do if no flags are set or there is no victim
    if (victim && victim->IsAlive() && procVictim)
    {
        victim->ProcDamageAndSpellFor(true, this, procVictim, procExtra, procPhase, attType, procSpell, amount, procAura, preMitigationDamage);
        if ((procAttacker & PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG) != 0)
        {
            Creature* mythicAttacker = ToCreature();
            if (mythicAttacker && mythicAttacker->m_mythicScaling)
                mythicAttacker->m_mythicScaling->HandleAffixProc(Mythic::MYTHIC_PROC_FLAG_SUCCESFUL_MAGIC_SPELL_HIT, procSpell, victim);
        }
    }
}
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, uint32 preMitigationDamage)
{
    ...
    ItemRef castItem = triggeredByAura->GetBase()->GetCastItemGUID() && GetTypeId() == TYPEID_PLAYER
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC: {
            switch (dummySpell->Id)
            {
                ...
                case 62337: {
                    ...
                    int32 dmg = damage;
                    victim->CastCustomSpell(this, 62379, &dmg, nullptr, nullptr, true);
                    return true;
                }
                // Trial of the Champion, Earth Shield
                case 67534: {
                    const int32 dmg = (int32)damage;
                    CastCustomSpell(this, 67535, &dmg, nullptr, nullptr, true, nullptr, triggeredByAura, triggeredByAura->GetCasterGUID());
                    return true;
                }
                ...
                case 69172: // everything except for DoTs
                {
                    ...
                    if (Unit* caster = triggeredByAura->GetCaster())
                    {
                        if (procFlag & (PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_POS))
                        {
                            ...
                            target->CastCustomSpell(caster, 69190, &dmg, nullptr, nullptr, true);
                        }
                        else
                        {
                            if (caster->GetVictim())
                            {
                                int32 dmg = damage;
                                target->CastCustomSpell(caster->GetVictim(), 69189, &dmg, nullptr, nullptr, true);
                            }
                        }
                    }
                    return true;
                ...
                case 70674: {
                    ...
                    {
                        int32 dmg = 3 * damage;
                        caster->CastCustomSpell(caster, 70677, &dmg, nullptr, nullptr, true);
                    }
                    return true;
                }
        ...
        case SPELLFAMILY_WARRIOR: {
            ...
            if (dummySpell->SpellIconID == 1697)
            {
                ...
                if (procSpell == nullptr || !(procEx & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT)) || this == victim)
                    return false;
        ...
        case SPELLFAMILY_PALADIN: {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 20185: {
                    ...
                    victim->CastCustomSpell(victim, 20267, &basepoints0, nullptr, nullptr, true, nullptr, triggeredByAura);
                    return true;
                }
                // Judgement of Wisdom
                case 20186: {
                    ...
                    // 2% of base mana
                    basepoints0 = static_cast<int32>(std::ceil(0.02 * victim->GetCreateMana()));
                    victim->CastCustomSpell(victim, 20268, &basepoints0, nullptr, nullptr, true, nullptr, triggeredByAura);
                    return true;
                }
                    ...
                    */
                    if (Aura* aur = victim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        ...
                        if (stackAmount == 1)
                            ...
                            basepoints = 33;
                        CastCustomSpell(victim, 42463, &basepoints, nullptr, nullptr, true, castItem, triggeredByAura);
                    ...
                    */
                    if (Aura* aur = victim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        ...
                        if (stackAmount == 1)
                            ...
                            basepoints = 33;
                        CastCustomSpell(victim, 53739, &basepoints, nullptr, nullptr, true, castItem, triggeredByAura);
        ...
        case SPELLFAMILY_SHAMAN: {
            ...
            if (dummySpell->SpellIconID == 2018) // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
                ...
                do
                {
                    ...
                    // Cast
                    CastCustomSpell(victim, spell, &dmg, nullptr, nullptr, true, castItem, triggeredByAura);
                        ...
                        if (dmg > 0)
                            if (Aura* a = GetAura(69023))
                                if (Unit* c = a->GetCaster())
                                    CastCustomSpell(c, 69034, &dmg, nullptr, nullptr, true);
                        return true;
                    }
    ...
    switch (trigger_spell_id)
    {
        ...
        case 51699: {
            ...
            if (Player* pTarget = target->ToPlayer())
            {
                ...
                Unit* comboTarget = nullptr;
                if (trigger_spell_id == 51699)
                {
                    comboTarget = ObjectAccessor::GetUnit(*target, pTarget->GetComboTarget());
                    if (!comboTarget)
                        comboTarget = pTarget->GetSelectedUnit();
                }
                else
                    comboTarget = target;
                ...
                if (comboTarget)
                {
                    pTarget->CastSpell(comboTarget, trigger_spell_id, true);
                    return true;
                }
            }
            return false;
...
{
    if (OverrideFactionID != 0 && _oldFactionId == 0) // _oldFactionId not being 0 means that we're currently being controlled by someone else, skip override so Mind Control or similar spells still work
        faction = OverrideFactionID;
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (!HasUnitState(UNIT_STATE_FLEEING) && !(IsCreature() && ToCreature()->IsTrainingDummy()))
...
void Unit::ModifyAuraState(AuraStateType flag, bool apply)
{
    if (apply)
    {
        if (!HasFlag(UNIT_FIELD_AURASTATE, 1 << (flag - 1)))
        {
            ...
            if (flag == AURA_STATE_HEALTHLESS_20_PERCENT)
            {
                Creature* creature = ToCreature();
                if (creature && creature->m_mythicScaling)
                    creature->m_mythicScaling->HandleAffixProc(Mythic::MYTHIC_PROC_FLAG_HEALTHLESS_20_PCT);
            }
...
int32 Unit::DealHeal(Unit* healer, Unit* victim, uint32 addhealth)
{
    ...
    if (instance)
        if (Player* victimPlayer = victim->GetCharmerOrOwnerPlayerOrPlayerItself())
            instance->UpdateEncounterStatsForPlayer(victimPlayer, PlayerStatType::HealingTaken, gain);

    if (Player* player = unit->GetCharmerOrOwnerPlayerOrPlayerItself())
    {
        if (instance)
        {
            instance->UpdateEncounterStatsForPlayer(player, PlayerStatType::EffectiveHealingDone, gain);
            instance->UpdateEncounterStatsForPlayer(player, PlayerStatType::HealingDone, addhealth);
        }
    }
    ...
    for (AuraEffectList::const_iterator i = hitTriggerAuras.begin(); i != hitTriggerAuras.end(); ++i)
    {
        if (Unit* magnet = (*i)->GetBase()->GetCaster())
            if (_IsValidAttackTarget(magnet, spellInfo) && magnet->IsWithinLOSInMap(this)
                && (!spellInfo || (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK && spellInfo->CheckTarget(this, magnet, false, nullptr) == SPELL_CAST_OK)))
...
void Unit::EnergizeBySpell(Unit* victim, uint32 spellID, uint32 damage, Powers powerType, bool sendLog)
{
    if (sendLog)
...
float Unit::SpellPctDamageModsDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
    ...
    // For totems/DRW get damage bonus from owner
    if (IsCreature() && (IsTotem() || ToCreature()->IsDancingRuneWeapon()))
    {
        if (Unit* owner = GetOwner())
        {
            float pct = owner->SpellPctDamageModsDone(victim, spellProto, damagetype);
            return IsTotem() ? pct : pct / 2.0f;
        }
    }
    ...
    {
        if (spellProto->EquippedItemClass == -1 && (*i)->GetSpellInfo()->EquippedItemClass != -1 && (*i)->GetMiscValue() == SPELL_SCHOOL_MASK_NORMAL && !((*i)->GetSpellInfo()->SpellFamilyFlags[1] & 0x20000)) //prevent apply mods from weapon specific case to non weapon specific spells (Example: thunder clap and two-handed weapon specialization)
            continue;
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, float TotalMod, uint32 stack)
{
    ...
    if (!spellProto->HasAttribute(SPELL_ATTR6_LIMIT_PCT_DAMAGE_MODS))
    {
        ...
        DoneTotal += float(GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_FLAT_SPELL_DAMAGE_VERSUS, creatureTypeMask));
    }
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            ...
            if (bonus->ap_dot_bonus > 0)
            {
                ...
                float APbonus            = float(victim->GetTotalAuraModifierWithBestOnly(attType == BASE_ATTACK ? SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS : SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS));
                APbonus += GetTotalAttackPowerValue(attType);
                DoneTotal += float(bonus->ap_dot_bonus * stack * ApCoeffMod * APbonus);
            }
        }
        else
        {
            ...
            if (bonus->ap_bonus > 0)
            {
                ...
                float APbonus            = float(victim->GetTotalAuraModifierWithBestOnly(attType == BASE_ATTACK ? SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS : SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS));
                APbonus += GetTotalAttackPowerValue(attType);
                DoneTotal += float(bonus->ap_bonus * stack * ApCoeffMod * APbonus);
            }
        }
    }
    ...
    if (coeff && DoneAdvertisedBenefit)
    {
        ...
        DoneTotal += float(DoneAdvertisedBenefit * coeff * factorMod);
    }
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto->Id == 66240)
        tmpDamage = std::max(tmpDamage, 250.0f);
...
float Unit::SpellDoneCritChance(Unit const* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, bool skipEffectCheck) const
{
    ...
    if (IsCreature() && ToCreature()->IsDancingRuneWeapon())
        if (Unit* owner = GetOwner())
            return owner->SpellDoneCritChance(victim, spellProto, schoolMask, attackType, skipEffectCheck);
    ...
    {
        case SPELL_DAMAGE_CLASS_MAGIC: {
            if (schoolMask & SPELL_SCHOOL_MASK_NORMAL && !spellProto->HasAttribute(SPELL_ATTR1_CU_ALLOW_CRIT_ON_NORMAL_MAGIC))
                crit_chance = 0.0f;
            // For other schools
...
bool Unit::IsParryHasteAllowed() const
{
    if (!IsCreature())
        return true;

    if ((ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY_HASTEN) == 0)
        return true;
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, float TotalMod, uint32 stack)
{
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            coeff = bonus->dot_damage;
            if (bonus->ap_dot_bonus > 0)
                DoneTotal += int32(bonus->ap_dot_bonus * ApCoeffMod * stack * GetTotalAttackPowerValue((spellProto->IsRangedWeaponSpell() && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
        else
        {
            coeff = bonus->direct_damage;
            if (bonus->ap_bonus > 0)
                DoneTotal += int32(bonus->ap_bonus * ApCoeffMod * stack * GetTotalAttackPowerValue((spellProto->IsRangedWeaponSpell() && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
    }
    else
    {
...
{
    if (spellProto->HasAttribute(SPELL_ATTR0_CU_IGNORE_HEALING_PCT_MODS))
        return healamount;
    ...
    if (spellInfo->Id != 42292 && spellInfo->Id != 59752 && spellInfo->Id != 19574 && spellInfo->Id != 34471)
    {
        ...
        for (SpellImmuneList::const_iterator itr = schoolList.begin(); itr != schoolList.end(); ++itr)
        {
            ...
            if (((itr->type & spellInfo->GetSchoolMask()) == spellInfo->GetSchoolMask())
                    ...
                    && itr->blockType == SPELL_BLOCK_TYPE_ALL
                || (itr->blockType == SPELL_BLOCK_TYPE_NEGATIVE && !spellInfo->IsPositive()))
                return true;
        }
    }
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (attType == RANGED_ATTACK)
    {
        APbonus += victim->GetTotalAuraModifierWithBestOnly(SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS);
    ...
    {
        APbonus += victim->GetTotalAuraModifierWithBestOnly(SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS);
...
void Unit::ApplySpellDispelImmunity(const SpellInfo* spellProto, DispelType type, bool apply)
{
    ...
    if (apply && spellProto->HasAttribute(SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY))
    {
        ...
        for (AuraApplicationMap::iterator itr = auras.begin(); itr != auras.end();)
        {
            ...
            bool const dispelAura  = (spell->GetDispelMask() & dispelMask) != 0 && !itr->second->GetBase()->IsSpellStealAndDispelDisabled();
            if (dispelAura)
            {
                // Dispel aura
                RemoveAura(itr);
...
bool Unit::IsInCombatWithHardcorePlayer() const
{
    // TODO: cache the result when entering combat with hardcore player instead of checking every time
    for (ThreatContainer::StorageType::const_iterator i = m_ThreatManager.getThreatList().begin(); i != m_ThreatManager.getThreatList().end(); ++i)
    {
        HostileReference* ref = (*i);
        if (!ref)
        ...
        GlobalPlayerData const* playerData = sWorld->GetGlobalPlayerData(GUID_LOPART(ref->getUnitGuid()));
        if (!playerData || !playerData->IsHardcore)
            continue;
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if ((IsAIEnabled && creature->AI()->IsEscorted()) || GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE || GetMotionMaster()->GetCurrentMovementGeneratorType() == ESCORT_MOTION_TYPE)
        ...
        if (enemy)
        {
            ...
            if (IS_PLAYER_GUID(enemy->GetGUID()))
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (IsMythicLocked() || target->IsMythicLocked())
        return false;

    if (Player const* player = GetCharmerOrOwnerPlayerOrPlayerItself(); player && !player->HasHardcoreEnabled() && !target->GetCharmerOrOwnerPlayerOrPlayerItself() && target->IsInCombatWithHardcorePlayer())
        return false;
    ...
    if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_CAN_TARGET_INVISIBLE)) && (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea() && !bySpell->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO)) : // second part of ternarny in second line
                !CanSeeOrDetect(target, (bySpell && bySpell->IsAffectingArea() && !bySpell->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO)) || (target->GetTypeId() == TYPEID_PLAYER && target->HasStealthAura() && target->IsInCombat() && IsInCombatWith(target)))))
        return false;
    ...
    if (repThisToTarget == REP_NEUTRAL && repTargetToThis <= REP_NEUTRAL)
    {
        ...
        if (!(target->GetTypeId() == TYPEID_PLAYER && thisUnit->GetTypeId() == TYPEID_PLAYER) && !(target->GetTypeId() == TYPEID_UNIT && thisUnit->GetTypeId() == TYPEID_UNIT))
        {
            ...
            if (FactionTemplateEntry const* factionTemplate = creature->GetFactionTemplateEntry())
            {
                ...
                else if (player->IsPlayingCrossfaction() && player->ConvertFactionForReputationReward(factionTemplate->faction) != factionTemplate->faction) //! crossfaction BG, query our original faction
                {
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    {
        Player const* owner       = GetCharmerOrOwnerPlayerOrPlayerItself();
        Player const* targetOwner = target->GetCharmerOrOwnerPlayerOrPlayerItself();
        if (owner && targetOwner)
        {
            if (owner->HasHardcoreEnabled() != targetOwner->HasHardcoreEnabled())
                return false;

            if (sWorld->IsCrossfactionEnabled(Crossfaction::Group))
                if (Map const* map = FindMap())
                    if (map->Instanceable() && !map->IsBattlegroundOrArena() && (IsInPartyWith(owner) || IsInRaidWith(targetOwner)))
                        return true;
        }
    }
    ...
    switch (mtype)
    {
        ...
        case MOVE_SWIM:
        {
            ...
            for (Unit::AuraEffectList::const_iterator itr = swimAuras.begin(); itr != swimAuras.end(); ++itr)
            {
                ...
                // crackaw: riding turtle too is not affected by any increasing / decreasing effects ~TODO or NOTTODO?

                if ((*itr)->GetId() == 64731 /*SPELL_SEA_TURTLE*/)
                {
                    SetSpeedRate(mtype, AddPct(non_stack_bonus, (*itr)->GetAmount()));
                    return;
                }
                else if ((*itr)->GetId() == 30174 /* SPELL_RIDING_TURTLE*/)
                {
                    SetSpeedRate(mtype, 2.97f);
                    return;
                }
...
void Unit::SetSpeedRate(UnitMoveType mtype, float rate)
{
    ...
    if (m_movedByPlayer) // unit controlled by a player.
    {
        ...
        if (m_movedByPlayer->ToPlayer())
            m_movedByPlayer->ToPlayer()->GetSession()->SendPacket(&self);
    ...
    if (s == JUST_DIED)
    {
        ...
        if (IsPlayer())
        {
            ...
            if (instance && instance->IsMythic())
                instance->HandleMythicPlayerDeath(ToPlayer());
        }
    }
...
bool Unit::CanHaveThreatList() const
{
    ...
    // Piootrek: added exception for Valithria combat start trigger
    if (cWho->IsPet() || cWho->IsTotem() || (cWho->IsTrigger() && cWho->GetEntry() != 38752))
        return false;
...
int32 Unit::ModSpellDuration(SpellInfo const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask) const
{
...
float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32& duration, Unit const* caster, DiminishingLevels Level, int32 limitduration) const
{
    ...
    if (limitduration > 0 && duration > limitduration)
    {
        ...
        if ((target->GetTypeId() == TYPEID_PLAYER
                || target->ToCreature()->IsSubjectToAllDiminishingReturns())
    ...
    if (group == DIMINISHING_TAUNT)
    {
        if (IsCreature() && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH))
        {
            ...
            switch (diminish)
            {
                ...
                case DIMINISHING_LEVEL_5: mod = 0.17850625f; break;
    ...
    // Some diminishings applies to mobs too (for example, Stun)
                 ...
                 && ((targetOwner ? (targetOwner->GetTypeId() == TYPEID_PLAYER) : (GetTypeId() == TYPEID_PLAYER))
                     || (GetTypeId() == TYPEID_UNIT && ToCreature()->IsSubjectToAllDiminishingReturns())))
...
float Unit::GetTotalAttackPowerValue(WeaponAttackType attType, Unit* victim) const
{
    if (attType == RANGED_ATTACK)
    {
        ...
        if (victim)
            ap += victim->GetTotalAuraModifierWithBestOnly(SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS);
    ...
    else
    {
        ...
        if (victim)
            ap += victim->GetTotalAuraModifierWithBestOnly(SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS);
...
bool CharmInfo::AddSpellToActionBar(SpellInfo const* spellInfo, ActiveStates newstate)
{
    ...
    if (_unit->IsGuardian() && _unit->GetUnitTypeMask() & UNIT_MASK_CONTROLABLE_GUARDIAN)
        if (Unit* owner = _unit->GetOwner())
            if (Guardian* guardian = dynamic_cast<Guardian*>(_unit))
                if (owner->IsPlayer() && !owner->ToPlayer()->GetGuardianSpells().empty() && spellInfo->IsAutocastable())
                {
                    for (uint8 i = 0; i < MAX_UNIT_ACTION_BAR_INDEX; ++i)
                    {
                        auto petActionBar = GetPetActionBar();
                        if (!petActionBar[i].GetAction() && petActionBar[i].IsActionBarForSpell())
                        {
                            auto& guardianSpells = owner->ToPlayer()->GetGuardianSpells();
                            auto pair            = guardianSpells.find(spell_id);
                            if (pair != guardianSpells.end())
                            {
                                SetActionBar(i, spell_id, ActiveStates(pair->second));
                                return true;
                            }
                        }
                    }
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (; index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
        ...
        ActiveStates type = ActiveStates(std::atol(*iter));
        ++iter;
        uint32 action = uint32(std::atol(*iter));
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, uint32 procPhase, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura, uint32 preMitigationDamage)
{
    ...
    ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, procPhase, procExtra, nullptr, &damageInfo, &healInfo, procAura);
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr != GetAppliedAuras().end(); ++itr)
    {
        ...
        if (procExtra & PROC_EX_ABSORB && !isVictim)
        {
            if (damage || spellProto->Effects[EFFECT_0].TriggerSpell || spellProto->Effects[EFFECT_1].TriggerSpell || spellProto->Effects[EFFECT_2].TriggerSpell || spellProto->HasAttribute(SPELL_ATTR0_CU_PROC_ON_FULL_ABSORB))
                active = true;
        }
            ...
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK:
                        ...
                        if (procSpell && (procSpell->SpellFamilyName || GetTypeId() != TYPEID_PLAYER) && (procSpell->CalcCastTime() > 0 /*||
                    ...
                    case SPELL_AURA_MOD_POWER_COST_SCHOOL:
                        ...
                        if (procSpell && (procSpell->ManaCost != 0 || procSpell->ManaCostPercentage != 0 || (procSpell->SpellFamilyFlags[1] & 0x2)) && // Cost check, mutilate include
                    ...
                    case SPELL_AURA_TRANSFORM: {
                        ...
                        {
                            int32 const damageLeft = triggeredByAura->GetAmount() - static_cast<int32>(eventInfo.GetDamageInfo()->GetDamage());
                            if (damageLeft <= 0)
                                aurApp->GetTarget()->RemoveAura(aurApp);
                            else
                                triggeredByAura->SetAmount(damageLeft);
                        }
                        break;
                    }
...
{
    for (auto&& aurEff : GetAuraEffectsByType(SPELL_AURA_MOD_DECREASE_SPEED))
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist, bool excludeVictim) const
{
...
Unit* Unit::SelectNearbyNoTotemTarget(Unit* exclude, float dist, bool excludeVictim) const
{
    std::list<Unit*> targets;
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const* spellProto, DamageEffectType damagetype, uint32 CastingTime) const
{
    ...
    {
        if (spellProto->Effects[j].Effect == SPELL_EFFECT_HEALTH_LEECH || (spellProto->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA && spellProto->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_LEECH))
        {
            CastingTime /= 2;
            break;
        }
...
float Unit::GetAPMultiplier(WeaponAttackType attType, bool normalized)
{
    ...
    ItemRef Weapon = ToPlayer()->GetWeaponForAttack(attType, true);
    if (!Weapon)
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, uint32 procPhase, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const*& spellProcEvent, ProcEventInfo const& eventInfo)
{
    SpellInfo const* spellProto = aura->GetSpellInfo();
    ...
    if (IsGeneric && spellProto->HasAttribute(SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED) && procExtra & PROC_EX_INTERNAL_TRIGGERED && !(procFlag & PROC_FLAG_DONE_TRAP_ACTIVATION))
        IsGeneric = false;
    ...
    {
        if (!(EventProcFlag & (PROC_FLAG_KILL | PROC_FLAG_DEATH)) && IsGeneric && (!eventInfo.GetTriggerAuraSpell() || eventInfo.GetTriggerAuraSpell()->SpellFamilyName == SPELLFAMILY_GENERIC))
            return false;
    }

    // Check spellProcEvent data requirements
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProto, spellProcEvent, EventProcFlag, procSpell, procFlag, procExtra, procPhase, active))
        return false;
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (spellProto->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            ...
            ItemRef item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto)
{
    ...
    if (Player* victimPlayer = victim->ToPlayer(); victimPlayer && victimPlayer->HasHardcoreEnabled())
        sCustomMgr->HandleHardcoreModePlayerDeath(victimPlayer, killer);
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature)
        {
            ...
            if (!loot->LoadedFromDB())
            {
                loot->clear();

                bool const _hasLootMode = creature->GetLootMode() != static_cast<uint16>(0);
                if (creature->m_mythicScaling == nullptr)
                {
                    if (uint32 lootid = creature->GetCreatureTemplate()->lootid)
                    {
                        loot->SetLootedGUID(creature);
                ...
                //! else for creatures with loot mode and they're perm spawns in mythic instance
                else if (_hasLootMode && creature->GetDBTableGUIDLow() && creature->GetInstanceScript() && creature->GetInstanceScript()->IsMythic())
                    loot->generateMoneyLoot(10000, 15000);
            }
    ...
    if (killer && (killer->IsPet() || killer->IsTotem()))
        if (Unit* owner = killer->GetOwner())
            owner->ProcDamageAndSpell(victim, PROC_FLAG_KILL, PROC_FLAG_NONE, PROC_EX_NONE, PROC_SPELL_PHASE_HIT, 0, attackType, spellProto);

    if (killer != victim)
        killer->ProcDamageAndSpell(victim, killer ? PROC_FLAG_KILL : 0, PROC_FLAG_KILLED, PROC_EX_NONE, PROC_SPELL_PHASE_HIT, 0, attackType, spellProto);

    // Proc auras on death - must be before aura/combat remove
    victim->ProcDamageAndSpell(nullptr, PROC_FLAG_DEATH, PROC_FLAG_NONE, PROC_EX_NONE, PROC_SPELL_PHASE_HIT, 0, attackType, spellProto);
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_PRIEST && !(victim->GetMapId() == 616 && victim->GetPositionZ() < 0))
    {
    ...
    else // creature died
    {
        ...
        if (creature->IsAIEnabled)
        {
            ...
            if (creature->m_mythicScaling)
                creature->m_mythicScaling->HandleAffixProc(Mythic::MYTHIC_PROC_FLAG_DEATH);
        }
        ...
        if (creature->GetInstanceId())
        {
            ...
            if (instanceMap->IsDungeon() && player && instanceMap->IsRaidOrHeroicDungeon())
                if (creature->IsInstanceBind())
                    instanceMap->ToInstanceMap()->PermBindAllPlayers(creature);
        }
    }
...
void Unit::SetStunned(bool apply)
{
    if (apply)
    {
        const bool initial = (apply && !isStuned());
        // Non-client controlled unit with an AI should drop target
        if (initial && !IsControlledByPlayer() && GetAI())
        {
            if (GetTarget())
                SetTarget(0);
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    else if (GetTypeId() == TYPEID_PLAYER && aurApp && aurApp->GetBase() && aurApp->GetBase()->GetId() != 71289)
    ...
    CombatStop();
    if (aurApp && charmer->IsPlayer() && IsPlayer() && !charmer->ToPlayer()->IsGameMaster() && !ToPlayer()->IsGameMaster())
    {
        int32 auraDuration = aurApp->GetBase()->GetDuration();
        SetInCombatState(charmer->IsPvP(), charmer, auraDuration);
        charmer->SetInCombatState(IsPvP(), this, auraDuration);
    }
...
bool Unit::IsInRaidWith(Unit const* unit) const
{
    ...
    // Xinef: we assume that npcs with the same faction are in party
        ...
        return u1->getFaction() == u2->getFaction();
    else if ((u2->GetTypeId() == TYPEID_PLAYER && u1->GetTypeId() == TYPEID_UNIT && u1->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER) || (u1->GetTypeId() == TYPEID_PLAYER && u2->GetTypeId() == TYPEID_UNIT && u2->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER))
        return true;
    else
        return false;
}
...
float Unit::MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId, bool extra) const
{
    ...
    if (!extra && !spellId && haveOffhandWeapon() && !hasNextMeleeSwingSpellQueued)
        missChance += 19;
...
void Unit::UpdateObjectVisibility(bool forced, bool /*fromUpdate*/)
{
    ...
    else
    {
        ...
        VisitNearbyObject(VisibilityConstants::AI_RELOCATION_NOTIFIER_RANGE, notifier);
    }
}
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && !CanFly() && (vehicle->GetVehicleInfo()->m_ID == 113 || vehicle->GetVehicleInfo()->m_ID == 8 || vehicle->GetVehicleInfo()->m_ID == 290 || vehicle->GetVehicleInfo()->m_ID == 298))
        GetMotionMaster()->MoveFall();
...
{
    return GetXZFlagBasedSpeed(m_movementInfo.flags);
}

float Unit::GetXZFlagBasedSpeed(uint32 moveFlags) const
{
    if (!(moveFlags & (MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_BACKWARD | MOVEMENTFLAG_STRAFE_LEFT | MOVEMENTFLAG_STRAFE_RIGHT)))
        return 0.f;
...
{
public:
    AuraMunchingQueue(Unit& owner, uint64 targetGUID, int32 basePoints, uint32 spellId)
...
void Unit::PetSpellFail(const SpellInfo* spellInfo, Unit* target, uint32 result)
{
    ...
    if ((World::IsPathfindingEnabled(GetMap()) || result != SPELL_FAILED_LINE_OF_SIGHT) && target)
    {
...
bool ConflagrateAuraStateDelayEvent::Execute(uint64 e_time, uint32 p_time)
{
    ...
    if (Unit* owner = _map->GetUnit(m_owner))
...
void Unit::ExecuteDelayedUnitAINotifyEvent()
{
    ...
    Trinity::AIRelocationNotifier notifier(*this);
    this->VisitNearbyObject(VisibilityConstants::AI_RELOCATION_NOTIFIER_RANGE, notifier);
}
...
void Unit::AddSpellValueForUnit(uint32 spellID, CustomSpellValues&& value)
{
    _spellValuesForUnit[spellID] = std::move(value);
}

void Unit::RemoveSpellValueForUnit(uint32 spellID)
{
    _spellValuesForUnit.erase(spellID);
}

CustomSpellValues const* Unit::GetSpellValueForUnit(uint32 spellID) const
{
    auto const it = _spellValuesForUnit.find(spellID);
    if (it == _spellValuesForUnit.end())
        return nullptr;
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    {
        if (_fieldNotifyFlags & flags[index] || ((flags[index] & visibleFlag) & UF_FLAG_SPECIAL_INFO) || ((updateType == UPDATETYPE_VALUES ? _changesMask.GetBit(index) : m_uint32Values[index]) && (flags[index] & visibleFlag)) || (index == UNIT_FIELD_AURASTATE && HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK)))
        {
            updateMask.SetBit(index);

            if (index == UNIT_FIELD_BYTES_0)
            {
                auto appendValue = m_uint32Values[index];
                if (player == target && player->IsPlayingCrossfaction())
                {
                    appendValue &= ~uint32(0xFF);
                    appendValue |= uint32(player->GetInitialRace());
                }
            ...
            {
                bool const isCrossfactionConfigEnabled = sWorld->IsCrossfactionEnabled(Crossfaction::DungeonFinder) || sWorld->IsCrossfactionEnabled(Crossfaction::Group);
                if ((GetMapId() == 616 && IsVehicle()) || (GetCharmerOrOwnerPlayerOrPlayerItself() && isCrossfactionConfigEnabled && target != this && (IsInPartyWith(target) || IsInRaidWith(target) || (target->GetCharmerOrOwnerPlayerOrPlayerItself() && GetInstanceId() && GetInstanceId() == target->GetInstanceId() && !target->InBattleground()))))
                {
                ...
                else if (target->IsSpectator() && target->FindMap() && target->FindMap()->IsBattleArena() && (IsPlayer() || IsUnit() || IsDynObject()))
                {
                    if (index == UNIT_FIELD_BYTES_2)
