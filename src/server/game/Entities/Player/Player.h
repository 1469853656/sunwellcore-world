...
using AppearanceContainer = std::array<std::unordered_map<uint32 /*visualID*/, bool /*purchased*/>, to_underlying(TransmogType::Max)>;
struct PresetData
{
    std::string Name;
    SetTransmogs Transmogs;
};

using PresetContainer = std::vector<PresetData>;
...
struct PlayerTalent
{
    ...
    uint32 talentID;
    bool inSpellBook;
    bool IsInSpec(uint8 spec) const { return (specMask & (1 << spec)); }
};
...
enum RuneCooldowns
{
    ...
    RUNE_MISS_COOLDOWN = 1000, // cooldown applied on runes when the spell misses
};
...
#define PLAYER_TITLE_MASK_ALLIANCE_PVP \
    (PLAYER_TITLE_PRIVATE | PLAYER_TITLE_CORPORAL | PLAYER_TITLE_SERGEANT_A | PLAYER_TITLE_MASTER_SERGEANT | PLAYER_TITLE_SERGEANT_MAJOR | PLAYER_TITLE_KNIGHT | PLAYER_TITLE_KNIGHT_LIEUTENANT | PLAYER_TITLE_KNIGHT_CAPTAIN | PLAYER_TITLE_KNIGHT_CHAMPION | PLAYER_TITLE_LIEUTENANT_COMMANDER | PLAYER_TITLE_COMMANDER | PLAYER_TITLE_MARSHAL | PLAYER_TITLE_FIELD_MARSHAL | PLAYER_TITLE_GRAND_MARSHAL)
...
enum PlayerExtraFlags : uint32
{
    ...
    PLAYER_EXTRA_IGNORE_TRANSMOG                    = 0x0100000,
    PLAYER_EXTRA_STH_HIDE                           = 0x0200000,
    PLAYER_EXTRA_ACCEPT_CHANNEL_INV                 = 0x0400000,
    PLAYER_EXTRA_SPYMODE                            = 0x0800000,
    PLAYER_EXTRA_DISABLE_CFBG                       = 0x1000000,
    PLAYER_EXTRA_HARDCORE                           = 0x2000000,
    PLAYER_EXTRA_HARDCORE_INVALID                   = 0x4000000,
    PLAYER_EXTRA_NO_HARDCORE_MESSAGES               = 0x8000000,
    PLAYER_EXTRA_PATH_OF_WAR                        = 0x10000000,
    PLAYER_EXTRA_DISABLE_CALL_TO_ARMS_MESSAGE       = 0x20000000
};
...
enum PlayerLoginQueryIndex
{
    ...
    PLAYER_LOGIN_QUERY_LOAD_MAIL,
    PLAYER_LOGIN_QUERY_LOAD_BREW_OF_THE_MONTH,
    PLAYER_LOGIN_QUERY_LOAD_PET_SLOTS,
    PLAYER_LOGIN_QUERY_GET_GUARDIAN_SPELLS,
    PLAYER_LOGIN_QUERY_LOAD_EXTERNAL_MAILS,
    PLAYER_LOGIN_QUERY_MYTHIC_KEY_LEVEL,
    PLAYER_LOGIN_QUERY_MYTHIC_COMPANION_DATA,
    PLAYER_LOGIN_QUERY_MYTHIC_REWARDS,
    PLAYER_LOGIN_QUERY_MYTHIC_TRANSMOGS,
    PLAYER_LOGIN_QUERY_LOAD_SPEC_DATA,
    PLAYER_LOGIN_QUERY_LOAD_MAX_LVL_CHARACTER_COUNT,
    PLAYER_LOGIN_QUERY_LOAD_NOT_RESTRICTED_WARPATH_CHARACTER_COUNT,
    PLAYER_LOGIN_QUERY_LOAD_TRANSMOG,
    PLAYER_LOGIN_QUERY_LOAD_TRANSMOG_SETS,
    PLAYER_LOGIN_QUERY_LOAD_PURCHASED_TRANSMOG_SETS,
...
enum TalentTrees
{
    ...
    TALENT_TREE_HUNTER_MARKMANSHIP      = 363,

    TALENT_TREE_ROGUE_COMBAT            = 181,
    TALENT_TREE_ROGUE_ASSASINATION      = 182,
    ...
    TALENT_TREE_SHAMAN_ENCHANCEMENT     = 263,
...
struct BattlegroundData
{
    BattlegroundData()
        ...
        , arenaType(ARENA_TYPE_NONE)
        , bgTeamId(TEAM_NEUTRAL)
        , bgRace(0)
        , bgQueueSlot(PLAYER_MAX_BATTLEGROUND_QUEUES)
        , isInvited(false)
        , bgIsRandom(false)
        , bgCrossfaction(false)
...
public: // constructors
    TradeData(Player* player, Player* trader)
        ...
        , m_spellCastItem(0)
    {
        memset(m_itemsGUID, 0, TRADE_SLOT_COUNT * sizeof(uint64));
    }

    Player* GetTrader() const { return m_trader; }
    TradeData* GetTraderData() const;

    ItemRef GetItem(TradeSlots slot) const;
    bool HasItem(uint64 itemGuid) const;
    TradeSlots GetTradeSlotForItem(uint64 itemGuid) const;
    void SetItem(TradeSlots slot, ItemRef const& item);

    uint32 GetSpell() const { return m_spell; }
    void SetSpell(uint32 spell_id, ItemRef const& castItem = NullItemRef);

    ItemRef GetSpellCastItem() const;
...
private:              // fields
    ...
    uint64 m_itemsGUID[TRADE_SLOT_COUNT]; // traded itmes from m_player side including non-traded slot
};
...
enum GuardianValueType
{
    GUARDIAN_VALUE_TYPE_SPELL,
    GUARDIAN_VALUE_TYPE_REACTSTATE,
    GUARDIAN_VALUE_TYPE_COMMANDSTATE
};

struct GuardianStates
{
...
public:
    ...
    bool IsPlayingCrossfaction() const { return InBattleground() && m_bgData.bgCrossfaction; }

    void SendChatMessage(const char* format, ...);

    bool SendBattleGroundChat(uint32 msgtype, std::string const& message);

    //void UpdateRacialLanguages(bool restoreNative);
    void ConvertForCrossfactionPlay(BattlegroundTypeId bgTypeId);
    void RecoverFromCrossfactionPlay(BattlegroundTypeId bgTypeId);

    uint32 ConvertFactionForReputationReward(uint32 faction_id) const;
    Language ConvertLanguage(Language language) const;

    TeamId GetTeam(CrossFactionOverride type = CF_ALLOW) const { return (type == CF_ALLOW && IsPlayingCrossfaction()) ? m_bgData.bgTeamId : m_team; }
    TeamId GetBgTeam() const { return m_bgData.bgTeamId != TEAM_NEUTRAL ? m_bgData.bgTeamId : m_team; }

    uint8 GetInitialRace() const { return m_race; }

    static bool BuildEnumData(Field* fields, WorldPacket* data);
    BattlegroundData const* GetBgData() const { return &m_bgData; }
    ...
    Creature* GetNPCIfCanInteractWith(uint64 guid, uint32 npcflagmask) const;
    GameObject* GetGameObjectIfCanInteractWith(uint64 guid, GameobjectTypes type) const;
    bool CaninteractWithGuildBank(uint64 guid) const;
    ...
    PlayerTaxi m_taxi;
    void InitTaxiNodesForLevel() { m_taxi.InitTaxiNodesForLevel(getRace(CF_DENY), getClass(), getLevel()); }
    ...
    bool tryWhisperToWebCommand(std::string to, std::string msg);
    ...
    void SetSpymode(bool on)
    {
        if (on)
            m_ExtraFlags |= PLAYER_EXTRA_SPYMODE;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_SPYMODE;
    }
    ...
    void GiveXP(uint32 xp, Unit* victim, float group_rate = 1.0f, bool finishedBgOrDf = false);
    ...
    void SetVirtualItemSlot(uint8 i, ItemRef const& item);
    void SetSheath(SheathState sheathed) override; // overwrite Unit version
    uint8 FindEquipSlot(ItemTemplate const* proto, uint32 slot, bool swap) const;
    uint32 GetItemCount(uint32 item, bool inBankAlso = false, ItemRef const& skipItem = nullptr) const;
    uint32 GetItemCountWithLimitCategory(uint32 limitCategory, ItemRef const& skipItem = nullptr) const;
    ...
    ItemRef GetItemByPos(uint8 bag, uint8 slot) const;
    Bag* GetBagByPos(uint8 slot) const;
    inline ItemRef GetUseableItemByPos(uint8 bag, uint8 slot) const //Does additional check for disarmed weapons
    {
    ...
    bool HasItemFitToSpellRequirements(SpellInfo const* spellInfo, ItemRef const& ignoreItem = nullptr) const;
    bool CanNoReagentCast(SpellInfo const* spellInfo) const;
    bool HasItemOrGemWithIdEquipped(uint32 item, uint32 count, uint8 except_slot = NULL_SLOT) const;
    bool HasItemOrGemWithLimitCategoryEquipped(uint32 limitCategory, uint32 count, uint8 except_slot = NULL_SLOT) const;
    InventoryResult CanTakeMoreSimilarItems(ItemRef const& pItem) const { return CanTakeMoreSimilarItems(pItem->GetEntry(), pItem->GetCount(), pItem); }
    ...
    InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, ItemRef const& pItem, bool swap = false) const
    {
        if (!pItem)
    ...
    InventoryResult CanStoreItems(ItemRef* pItem, int32 count) const;
    InventoryResult CanEquipNewItem(uint8 slot, uint16& dest, uint32 item, bool swap) const;
    InventoryResult CanEquipItem(uint8 slot, uint16& dest, ItemRef const& pItem, bool swap, bool not_loading = true) const;

    InventoryResult CanEquipUniqueItem(ItemRef const& pItem, uint8 except_slot = NULL_SLOT, uint32 limit_count = 1) const;
    InventoryResult CanEquipUniqueItem(ItemTemplate const* itemProto, uint8 except_slot = NULL_SLOT, uint32 limit_count = 1) const;
    InventoryResult CanUnequipItems(uint32 item, uint32 count) const;
    InventoryResult CanUnequipItem(uint16 src, bool swap) const;
    InventoryResult CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, ItemRef const& pItem, bool swap, bool not_loading = true) const;
    InventoryResult CanUseItem(ItemRef const& pItem, bool not_loading = true) const;
    ...
    ItemRef StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update, int32 randomPropertyId = 0, bool refunded = false);
    ItemRef StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update, int32 randomPropertyId, AllowedLooterSet& allowedLooters, bool refunded = false);
    ...
    InventoryResult CanTakeMoreSimilarItems(uint32 entry, uint32 count, ItemRef const& pItem, uint32* no_space_count = nullptr) const;
    InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, uint32 entry, uint32 count, ItemRef const& pItem = nullptr, bool swap = false, uint32* no_space_count = nullptr) const;

    void AddRefundReference(uint32 it);
    void DeleteRefundReference(uint32 it);

    void ApplyEquipCooldown(ItemRef const& pItem);
    ...
    bool MoveItemFromInventory(uint8 bag, uint8 slot, bool update, CharacterDatabaseTransaction& transaction);
    ...
    bool DestroyItemCount(ItemRef& item, uint32 count, bool update, CharacterDatabaseTransaction& transaction);
    void DestroyConjuredItems(bool update);
    void DestroyZoneLimitedItem(bool update, uint32 new_zone);
    void SplitItem(uint16 src, uint16 dst, uint32 count);
    void SwapItem(uint16 src, uint16 dst);
    void AddItemToBuyBackSlot(ItemRef const& pItem);
    ItemRef GetItemFromBuyBackSlot(uint32 slot);
    void RemoveItemFromBuyBackSlot(uint32 slot, bool del);
    uint32 GetMaxKeyringSize() const { return KEYRING_SLOT_END - KEYRING_SLOT_START; }
    void SendEquipError(InventoryResult msg, ItemRef const& pItem, ItemRef const& pItem2 = nullptr, uint32 itemid = 0);
    ...
    {
        ItemRef mainItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
        return mainItem && mainItem->GetTemplate()->InventoryType == INVTYPE_2HWEAPON && !CanTitanGrip();
    }
    void SendNewItem(ItemRef const& item, uint32 count, bool received, bool created, bool broadcast = false);
    ...
    void AddTradeableItem(ItemRef const& item);
    void RemoveTradeableItem(ItemRef const& item);
    void UpdateItemDuration(uint32 time, bool realtimeonly = false);
    void AddEnchantmentDurations(ItemRef const& item);
    void RemoveEnchantmentDurations(ItemRef const& item);
    void RemoveEnchantmentDurationsReferences(ItemRef const& item); // pussywizard
    void RemoveAllTmpEnchantmentDurations();
    void RemoveArenaEnchantments(EnchantmentSlot slot);
    void AddEnchantmentDuration(ItemRef const& item, EnchantmentSlot slot, uint32 duration);
    void ApplyEnchantment(ItemRef const& item, EnchantmentSlot slot, bool apply, bool apply_dur = true, bool ignore_condition = false);
    void ApplyEnchantment(ItemRef const& item, bool apply);
    void UpdateSkillEnchantments(uint16 skill_id, uint16 curr_value, uint16 new_value);
    void SendEnchantmentDurations();
    void BuildEnchantmentsInfoData(WorldPacket* data);
    void AddItemDurations(ItemRef const& item);
    void RemoveItemDurations(ItemRef const& item);
    void SendItemDurations();
    void LoadCorpse();
    void LoadPet();

    ItemRef CreateNewItem(uint32 itemID, uint32 count, int32 randomPropertyId = 0);
    ...
    bool HasEnoughMoney(uint32 amount, bool isPathOfWarCall = false) const;
    bool HasEnoughMoney(int32 amount, bool isPathOfWarCall = false) const;

    void SetMoney(uint32 value, bool scriptCall = false);
    ...
    PlayerMails const GetMails() { return m_mail; }

    uint32 GetFreeSlotsNumber(bool countBags) const;
    ...
    void SendExternalMail(Field* result);
    ...
    bool IsSpellInAllSpeccs(uint32 spellId, uint8 currentSpec);
    void learnSpell(uint32 spellId, bool showError = true);
    void removeSpell(uint32 spellId, uint8 removeSpecMask, bool onlyTemporary);
    void resetSpells();
    void learnDefaultSpells();
    void learnQuestRewardedSpells();
    void learnQuestRewardedSpells(Quest const* quest);
    void learnSpellHighRank(uint32 spellid, bool checkLevel = false, bool showError = true);
    ...
    void LearnTalent(uint32 talentId, uint32 talentRank, bool force = false);
    void LearnPetTalent(uint64 petGuid, uint32 talentId, uint32 talentRank);
    void SetUsedTalentCount(uint32 value) { m_usedTalentCount = value; }
    ...
    uint32 GetMostPointsTalentTree() const;
    bool IsHealerTalentSpec() const;

    // Multispec
    void SetSpecsGroup(uint8 group) { m_specsGroup = group; }
    uint8 GetSpecsGroup() const { return m_specsGroup; }
    ...
    void ResetGlyphsForActiveSpec(bool save);
    ...
    void _SaveSpellCooldowns(const CharacterDatabaseTransaction& trans, bool logout);
    ...
    void SendMessageToSet(WorldPacket* data, bool self) override;                                                                               // pussywizard!
    ...
    void SendMessageToSet(WorldPacket* data, Player const* skipped_rcvr) override;                                                              // pussywizard!
    ...
    void DurabilityLoss(ItemRef const& item, double percent);
    void DurabilityPointsLossAll(int32 points, bool inventory);
    void DurabilityPointsLoss(ItemRef const& item, int32 points);
    ...
    bool IsAtLootRewardDistance(WorldObject const* rewardSource) const;
    ...
    void ModifyHonorPoints(int32 value, CharacterDatabaseTransaction trans = nullptr); //! If trans is specified, honor save query will be added to trans
    void ModifyArenaPoints(int32 value, CharacterDatabaseTransaction trans = nullptr); //! If trans is specified, arena point save query will be added to trans
    uint32 GetMaxPersonalArenaRatingRequirement(uint32 minarenaslot) const;
    void UpdateKnownTitles();
    void GiveTitleReward();
    ...
    void _ApplyWeaponDependentAuraMods(ItemRef const& item, WeaponAttackType attackType, bool apply);
    void _ApplyWeaponDependentAuraCritMod(ItemRef const& item, WeaponAttackType attackType, AuraEffect const* aura, bool apply);
    void _ApplyWeaponDependentAuraDamageMod(ItemRef const& item, WeaponAttackType attackType, AuraEffect const* aura, bool apply);

    void _ApplyItemMods(ItemRef const& item, uint8 slot, bool apply);
    ...
    void ApplyItemEquipSpell(ItemRef const& item, bool apply, bool form_change = false);
    void ApplyEquipSpell(SpellInfo const* spellInfo, ItemRef const& item, bool apply, bool form_change = false);
    void UpdateEquipSpellsAtFormChange();
    void CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx);
    void CastItemUseSpell(ItemRef const& item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex);
    void CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, ItemRef const& item, ItemTemplate const* proto);
    ...
    uint8 GetArenaType() const { return m_bgData.arenaType; }
    ...
    void SetBattlegroundId(uint32 id, BattlegroundTypeId bgTypeId, uint32 queueSlot, bool invited, bool isRandom, TeamId teamId, uint8 arenaType, bool loading = false);
    ...
    bool CanJoinToBattleground(bool isSoloQueue = false) const;
    ...
    void SetSpellQueueWindow(uint32 value) { _spellQueueWindow = value; }
    uint32 GetSpellQueueWindow() const { return _spellQueueWindow; }

    //uint32 GetSaveTimer() const { return m_nextSave; }
    //void   SetSaveTimer(uint32 timer) { m_nextSave = timer; }

    // Transmog
    PresetContainer TransmogPresets;
    PresetContainer PurchasedTransmogPresets;
    AppearanceContainer TransmogAppearances;
    ...
    float GetAverageItemLevelForDF() const;
    ...
    // sitdev: unstuck command cooldown
    void SetUnstuckCooldown(uint32 cooldown) { m_unstuckCooldown = cooldown; };
    ...
    void SetDodgeMode(bool on)
    {
        if (on)
            m_ExtraFlags |= PLAYER_EXTRA_DODGE_LOCATION;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_DODGE_LOCATION;
    };
    ...
    auto& GetGuardianSpells() { return _guardianSpells; }
    auto& GetGuardianStates() { return _guardianStates; }
    void InsertOrUpdateGuardianState(uint32 entry, ReactStates state);
    void InsertOrUpdateGuardianState(uint32 entry, CommandStates state);

    void HandleMissingStances();

    // Multispec
    std::string_view GetSpecName(uint32 const specID) const { return _playerSpecNames[specID]; }
    void SetSpecName(uint32 specID, std::string&& specName);

    // you can't follow while being followed
    void SetGMFollow(uint64 guid)
    {
        m_GMfollow_GUID       = guid;
        m_GMfollowtarget_GUID = 0;
    }
    void SetFollowTarget(uint64 guid)
    {
        m_GMfollowtarget_GUID = guid;
        m_GMfollow_GUID       = 0;
    }
    uint64 GetFollowTarget() { return m_GMfollowtarget_GUID; }
    uint64 GetFollowingGM() { return m_GMfollow_GUID; }

    int32 const GetMythicKeyLevel() const;
    //! Adds {arg} number of levels to current key level
    void UpdateMythicKeyLevel();
    void SetMythicKeyLevel(int32 /*newKeyLevel*/);
    void SaveMythicKeyLevel();
    void AddMythicReward(uint32 /*keylevel*/, uint32 /*type*/, uint32 /*timeElapsed*/, bool /*isNewRecord*/, uint32 /*mapId*/);

    /** Mythic companion **/
    uint64 const GetMythicCompanionGUID() const { return m_companionData->m_mythicCompanionGUID; }
    void SetMythicCompanionGUID(uint64 guid) { m_companionData->m_mythicCompanionGUID = guid; }
    void GiveXPToMythicCompanion(int32 /*value*/);
    void UpdateMythicCompanionLevelAndAI(uint32 /*newLevel*/);
    bool IsMythicCompanionLoaded() const { return m_mythicCompanionLoaded; }
    void SaveMythicRewards();
    Mythic::MythicPlayerRewards& GetMythicRewards() { return m_mythicRewards; }
    const std::vector<uint32>& GetMythicTransmogs() { return m_mythicTransmogs; }

    void AddMythicTransmog(uint32 entry);
    void DeleteMythicTransmog(uint32 entry);

    struct PlayerCompanionData
    {
        uint64 m_mythicCompanionGUID  = 0;
        uint32 m_mythicCompanionLevel = 1;
        int32 m_mythicCompanionXP     = 0;
        uint8 race                    = 0;
        uint8 gender                  = 0;
        uint32 head                   = 10630;
        uint32 shoulders              = 34107;
        uint32 shirt                  = 6384;
        uint32 chest                  = 795;
        uint32 waist                  = 3602;
        uint32 pants                  = 794;
        ...
        uint32 cloak                  = 4798;
        uint32 tabard                 = 0;
        uint32 mainHand               = 2495;
        uint32 offHand                = 0;
        uint32 ranged                 = 0;
        bool toolBank                 = false;
        bool toolGuildBank            = false;
        bool toolAuctionHouse         = false;
        bool toolMassRess             = false;
        bool goblinRaceBought         = false;
        bool taunkaRaceBought         = false;
    };

    //! By copy
    std::string const FormatCompanionRaces() const;
    std::string const FormatCompanionGenders() const;
    std::string const FormatMythicCostString(std::string prefix, uint32 cost) const;

    PlayerCompanionData* m_companionData;
    uint32 GetCompanionItemBySlot(uint8 slot);
    inline bool HasEnoughMythicPower(uint32 value) const { return m_mythicPower >= value; }
    void TakeMythicPower(uint32 power)
    {
        int32 value   = std::max(0, static_cast<int32>(m_mythicPower - power));
        m_mythicPower = value;
    }

    uint32 m_mythicPower;
    void SaveMythicCompanion();
    bool const IsMythicTransactionInProcess() const { return m_mythicTransactionInProcess; }

    // Hardcore mode
    bool HasHardcoreEnabled() const { return m_ExtraFlags & PLAYER_EXTRA_HARDCORE; }
    void SetHardcoreMode()
    {
        m_ExtraFlags |= PLAYER_EXTRA_HARDCORE;
        sWorld->UpdateGlobalPlayerDataHardcore(GetGUIDLow(), true);
    }

    bool CanBeHardcore() const { return (m_ExtraFlags & PLAYER_EXTRA_HARDCORE_INVALID) == 0; }
    void InvalidateHardcorePath()
    {
        if (HasHardcoreEnabled())
            return;

        m_ExtraFlags |= PLAYER_EXTRA_HARDCORE_INVALID;
    }

    void SetDisplayHardcoreMessages(bool enable)
    {
        if (!enable)
            m_ExtraFlags |= PLAYER_EXTRA_NO_HARDCORE_MESSAGES;
        else
            m_ExtraFlags &= ~PLAYER_EXTRA_NO_HARDCORE_MESSAGES;
    }
    bool CanDisplayHardcoreMessages() const { return (m_ExtraFlags & PLAYER_EXTRA_NO_HARDCORE_MESSAGES) == 0; }

    // Path of War
    bool IsPathOfWar() const { return m_ExtraFlags & PLAYER_EXTRA_PATH_OF_WAR; }
    void SetIsPathOfWar(bool on);

    // AOE Loot
    void AddAvailableCreatureForLoot(uint64 guid)
    {
        _lastKilledCreatures.emplace(guid);
    }

    void RemoveAvailableCreatureForLoot(uint64 guid)
    {
        _lastKilledCreatures.erase(guid);
    }

    bool CanLootWithAOE(Creature const* mainLootedCreature, Creature const* creature) const;

    template <typename Action>
    void DoOnAllAvailableCreaturesForLoot(Creature const* mainLootedCreature, Action&& action)
    {
        auto it = _lastKilledCreatures.begin();
        while (it != _lastKilledCreatures.end())
        {
            Creature* creature = ObjectAccessor::GetCreature(*this, *it);
            if (!creature || creature->IsAlive() || creature->loot.isLooted() || creature->LootForBody)
            {
                it = _lastKilledCreatures.erase(it);
                continue;
            }

            if (mainLootedCreature->GetGUID() != creature->GetGUID() && CanLootWithAOE(mainLootedCreature, creature))
            {
                action(creature);
                it = _lastKilledCreatures.erase(it);
                continue;
    ...
    // Spell Queue
    void SetSpellQueue(Spell* spell);
    Spell* GetSpellQueue() const { return _queuedSpell; }
    void CancelSpellQueue();
    void ClearSpellQueue() { _queuedSpell = nullptr; }
    void UpdateSpellQueue();

    uint8 GetPossibleLFGRoles() const;
    std::pair<bool, time_t>& GetCallToArmsMessageStatus();
    void SetDisableCallToArmsMessage(bool disable);
    bool CanDisplayCallToArmsMessages() const;


private:
    bool m_mythicCompanionLoaded;
    bool m_mythicTransactionInProcess;
    Mythic::MythicPlayerRewards m_mythicRewards;
    std::vector<uint32> m_mythicTransmogs;
    PreparedQueryResultFuture m_mythicRewardsCallback;
    PreparedQueryResultFuture m_companionDataCallback;
    void ProcessMythicAsyncQuery();
...
protected:
    ...
    void _LoadGuardianSpellsData(PreparedQueryResult result);
    void _LoadMythicKeyLevel(PreparedQueryResult result);
    void _LoadMythicCompanionData(PreparedQueryResult result);
    void _LoadMythicRewardData(PreparedQueryResult result);
    void _LoadMythicTransmogsData(PreparedQueryResult result);
    void _LoadSpecData(PreparedQueryResult result);
    void _LoadMaxLvlCharacterCount(PreparedQueryResult result);
    void _LoadNotRestrictedWarpathCharacterCount(PreparedQueryResult result);
    void _LoadTransmog(PreparedQueryResult result);
    void _LoadTransmogSets(PreparedQueryResult result);
    void _LoadPurchasedTransmogSets(PreparedQueryResult result);
    ...
    void _SaveLatency(CharacterDatabaseTransaction& trans);
    ...
    // Multispec
    uint8 m_specsGroup;
    std::array<std::string, MAX_TALENT_SPECS / 2> _playerSpecNames;
...
private:
    ...
    TeamId _realTeamID = TEAM_NEUTRAL;
    ...
    ItemRef _LoadItem(CharacterDatabaseTransaction& trans, uint32 zoneId, uint32 timeDiff, Field* fields);

    typedef std::set<uint32> RefundableItemsSet;
    RefundableItemsSet m_refundableItems;
    void SendRefundInfo(ItemRef const& item);
    void RefundItem(ItemRef const& item);
    ...
    uint32 _totalSeasonHonorPoints = 0;
    bool _isCountedAsPathOfWarMaxLevel = false;

    uint32 _spellQueueWindow = 400;
    ...
    std::unordered_map<uint32 /*spellId*/, uint32 /*activeState*/> _guardianSpells;
    std::vector<GuardianStates> _guardianStates;

    uint64 m_GMfollowtarget_GUID;
    uint64 m_GMfollow_GUID;

    int32 m_mythicKeyLevel;

    // AOE Loot
    std::unordered_set<uint64> _lastKilledCreatures;

    // Spell Queue
    Spell* _queuedSpell = nullptr;

    std::pair<bool, time_t> _ctaMessageStatus;
};
...
T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell, bool temporaryPet)
{
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
        ...
        {
            if (!chargedMod || (chargedMod->ownerAura->Priority < mod->ownerAura->Priority))
                chargedMod = mod;
            continue;
        }
