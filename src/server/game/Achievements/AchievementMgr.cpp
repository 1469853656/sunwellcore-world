...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const* source, Unit const* target, uint32 miscvalue1 /*= 0*/) const
{
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_TEAM: {
            ...
            uint32 teamOld = target->ToPlayer()->GetTeam() == TEAM_ALLIANCE ? ALLIANCE : HORDE;
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE: {
            ...
            uint32 score = bg->GetTeamScore(source->GetTeam() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_NTH_BIRTHDAY: {
            ...
            tm birthday_tm = tm_from_time_t(birthday_start);
...
void AchievementMgr::DeleteFromDB(uint32 lowguid)
{
    ...
    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT);
...
void AchievementMgr::SaveToDB(const CharacterDatabaseTransaction& trans)
{
    if (!m_completedAchievements.empty())
    {
        for (CompletedAchievementMap::iterator iter = m_completedAchievements.begin(); iter != m_completedAchievements.end(); ++iter)
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_BY_ACHIEVEMENT);
    ...
    if (!m_criteriaProgress.empty())
    {
        for (CriteriaProgressMap::iterator iter = m_criteriaProgress.begin(); iter != m_criteriaProgress.end(); ++iter)
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_PROGRESS_BY_CRITERIA);
            stmt->setUInt32(0, GetPlayer()->GetGUID());
            stmt->setUInt16(1, iter->first);
            trans->Append(stmt);
    ...
    if (criteriaResult)
    {
        do
        {
            ...
            if (!criteria)
            {
                ...
                auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA);
...
constexpr uint32 MAX_ARENA_SLOT_ACHIEV{ 3 };
static const uint32 achievIdByArenaSlot[MAX_ARENA_SLOT_ACHIEV] = { 1057, 1107, 1108 };
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList->begin(); i != achievementCriteriaList->end(); ++i)
    {
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_DEATH: {
                ...
                bool notfit = false;
                for (int j = 0; j < MAX_ARENA_SLOT_ACHIEV; ++j)
                {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_TEAM_RATING: {
                ...
                {
                    for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT_ACHIEV; ++arena_slot)
                    {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_PERSONAL_RATING: {
                ...
                else // login case
                {
                    for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT_ACHIEV; ++arena_slot)
                    {
                        ...
                        SetCriteriaProgress(achievementCriteria, team->GetRating(), PROGRESS_HIGHEST);
                        break;
                    }
                }
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    if (uint32 titleId = reward->titleId[achievement->ID == 1793 ? GetPlayer()->getGender() : GetPlayer()->GetTeam(CF_DENY)])
        if (CharTitlesEntry const* titleEntry = sCharTitlesStore.LookupEntry(titleId))
            GetPlayer()->SetTitle(titleEntry);
...
bool AchievementMgr::CanUpdateCriteria(AchievementCriteriaEntry const* criteria, AchievementEntry const* achievement)
{
    ...
    if ((achievement->requiredFaction == ACHIEVEMENT_FACTION_HORDE && GetPlayer()->GetTeam(CF_DENY) != TEAM_HORDE) || (achievement->requiredFaction == ACHIEVEMENT_FACTION_ALLIANCE && GetPlayer()->GetTeam(CF_DENY) != TEAM_ALLIANCE))
        return false;
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows(); ++entryId)
    {
        ...
        if (criteria->additionalRequirements[1].additionalRequirement_type != ACHIEVEMENT_CRITERIA_CONDITION_NONE && criteria->additionalRequirements[1].additionalRequirement_type != criteria->additionalRequirements[0].additionalRequirement_type)
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    do
    {
        ...
        if (!achievement)
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INVALID_ACHIEVMENT);
