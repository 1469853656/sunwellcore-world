...
MailReceiver::MailReceiver(Player* receiver)
    : m_receiver(receiver)
    , m_receiver_lowguid(receiver->GetGUIDLow())
{
}

MailReceiver::MailReceiver(Player* receiver, uint32 receiver_lowguid)
    : m_receiver(receiver)
...
void MailDraft::SendMailTo(CharacterDatabaseTransaction& trans, MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    {
        deliver_time = time(nullptr);
        expire_time  = time(nullptr) + 1 * YEAR;
        expire_delay = 0;
    }
    ...
    {
        ItemRef pItem = mailItemIter->second;
...
void Player::SendExternalMail(Field* fields)
{
    if (!fields)
        return;
    ...
    sLog->outDebug(LOG_FILTER_NONE, "EXTERNAL MAIL> Sending mail to " UI64FMTD ", Item:%u", GetGUID(), ItemID);

    auto transaction = CharacterDatabase.BeginTransaction();

    auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_MAIL_EXTERNAL);
    stmt->setUInt64(0, id);
    CharacterDatabase.ExecuteOrAppend(transaction, stmt);

    if (ItemID != 0)
    {
        auto mailItem = Item::CreateItem(ItemID, ItemCount, this);
        mailItem->SaveToDB(transaction);

        MailDraft(subject, message)
            .AddItem(mailItem)
            .AddMoney(money)
            .SendMailTo(transaction, MailReceiver(this), MailSender(MAIL_NORMAL, uint32(0), MAIL_STATIONERY_GM), MAIL_CHECK_MASK_RETURNED);
    }
    else
    {
        MailDraft(subject, message)
