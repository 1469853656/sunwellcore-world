...
{
public: // Constructors
    explicit MailReceiver(uint32 receiver_lowguid)
        : m_receiver(nullptr)
...
struct Mail
{
    Mail() : messageID(0) , messageType(MAIL_NORMAL), stationery(MAIL_STATIONERY_DEFAULT), mailTemplateId(0), sender(0), receiver(0), expire_time(time_t(0)),
        deliver_time(time_t(0)), money(0), COD(0), checked(MAIL_CHECK_MASK_NONE), state(MAIL_STATE_UNCHANGED)
    {
    }
