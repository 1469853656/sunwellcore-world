...
public:
    ...
    struct boss_akilzonAI : public BossAI
    {
        boss_akilzonAI(Creature* creature)
            : BossAI(creature, DATA_AKILZONEVENT)
        {
            ...
            if (StormCount < 10 && StormCount > 1)
            {
                ...
                for (std::list<Unit*>::const_iterator i = tempUnitMap.begin(); i != tempUnitMap.end(); ++i)
                {
                    if (Unit* target = (*i))
                    {
                        if (Cloud && !Cloud->IsWithinDist(target, 6, false))
                            Cloud->CastCustomSpell(target, SPELL_ZAP, &bp0, nullptr, nullptr, true, nullptr, nullptr, me->GetGUID());
                    }
                }
                ...
                for (uint8 i = 0; i < 5 + rand() % 5; ++i)
                {
                    ...
                    if (Unit* trigger = me->SummonTrigger(x, y, z, 0, 2000))
                    {
                        ...
                        if (Cloud)
                            Cloud->CastCustomSpell(trigger, /*43661*/ SPELL_ZAP, &bp0, nullptr, nullptr, true, nullptr, nullptr, Cloud->GetGUID());
                    }
                }
            }
...
public:
    npc_akilzon_eagle()
        : CreatureScript("npc_akilzon_eagle")
    {
    }
