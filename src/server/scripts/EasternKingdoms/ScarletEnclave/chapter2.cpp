            ...
            if (HasEscortState(STATE_ESCORT_PAUSED))
            {
                if (waveTimer <= uiDiff)
                {
                    switch (wave)
                    {
                        ...
                        case 3:
                            Talk(SAY_BREAKOUT6);
                            me->SummonCreature(NPC_HIGH_INQUISITOR_VALROTH, 1642.329f, -6045.818f, 127.583f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
                            waveTimer = 1000;
                            break;
                        case 4:
                        {
...
public:
    npc_scarlet_courier()
        : CreatureScript("npc_scarlet_courier")
    {
    }
...
public:
    ...
    {
        npc_high_inquisitor_valrothAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    spell_q12779_an_end_to_all_things()
        : SpellScriptLoader("spell_q12779_an_end_to_all_things")
    {
    }
