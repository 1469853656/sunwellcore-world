...
public:
    ...
    struct npc_eye_of_acherusAI : public NullCreatureAI
    {
        ...
        void InitializeAI() override
        {
            ...
            events.ScheduleEvent(EVENT_REGAIN_CONTROL, 36s);
...
public:
    spell_q12641_death_comes_from_on_high_summon_ghouls()
        : SpellScriptLoader("spell_q12641_death_comes_from_on_high_summon_ghouls")
    {
    }
...
public:
    npc_scarlet_cannon()
        : CreatureScript("npc_scarlet_cannon")
    {
    }
    ...
    {
        npc_scarlet_cannonAI(Creature* creature)
            : VehicleAI(creature)
        {
            summonAttackers = 0;
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
            {
                ...
                if (Player* starter = ObjectAccessor::GetPlayer(*me, playerGUID))
                    sCreatureTextMgr->SendChat(me, SAY_EVENT_ATTACK, nullptr, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false, starter);
