...
//x, y, z
#define SPAWN_POS pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ()
...
#define CHEAT_TIMER 120000

typedef std::list<uint64> GuidList;

enum Cheats
{
    FIRST_CHEAT_MASK  = 0x1,
    SECOND_CHEAT_MASK = 0x2,
    THIRD_CHEAT_MASK  = 0x4
};

enum SCRIPTTEXTs
{
    SCRIPTTEXT_AT_EVENT_START = 1,
    SCRIPTTEXT_LOSE_KNIGHT_P  = 2,
    SCRIPTTEXT_LOSE_KNIGHT_M  = 3,
    SCRIPTTEXT_LOSE_PAWN_P_1  = 4,
    SCRIPTTEXT_LOSE_PAWN_P_2  = 5,
    SCRIPTTEXT_LOSE_PAWN_P_3  = 6,
    SCRIPTTEXT_LOSE_PAWN_M_1  = 7,
    SCRIPTTEXT_LOSE_PAWN_M_2  = 8,
    SCRIPTTEXT_LOSE_PAWN_M_3  = 9,
    SCRIPTTEXT_LOSE_QUEEN_P   = 10,
    SCRIPTTEXT_LOSE_QUEEN_M   = 11,
    SCRIPTTEXT_LOSE_BISHOP_P  = 12,
    SCRIPTTEXT_LOSE_BISHOP_M  = 13,
    SCRIPTTEXT_LOSE_ROOK_P    = 14,
    SCRIPTTEXT_LOSE_ROOK_M    = 15,
    SCRIPTTEXT_PLAYER_CHECK   = 16,
    SCRIPTTEXT_MEDIVH_CHECK   = 17,
    SCRIPTTEXT_PLAYER_WIN     = 18,
    SCRIPTTEXT_MEDIVH_WIN     = 19,
    SCRIPTTEXT_MEDIVH_CHEAT_1 = 20,
    SCRIPTTEXT_MEDIVH_CHEAT_2 = 21,
    SCRIPTTEXT_MEDIVH_CHEAT_3 = 22
};
...
enum ChessEventStatuses
{
    CHESS_STATUS_TRIGGER_START = 1,
    CHESS_STATUS_TRIGGER_END   = 2
};
...
class move_triggerAI : public NullCreatureAI
{
private:
...
private:
    ...
    uint32 ability1Chance; //chance to cast spell
    uint32 ability2Chance;
    uint32 attackDamage;

    uint32 ability1Timer;
    uint32 ability1Cooldown;
    uint32 ability2Timer;
    uint32 ability2Cooldown;
    uint32 attackTimer;

    uint32 nextTryTimer; //try to cast spell after some time
...
public:
    ...
    void SpellHitTarget(Unit* caster, const SpellInfo* spell) override;
...
private:
    ...
    uint32 chanceToSelfMove;

    ChessTile chessBoard[8][8];
    float hordeSideDeadWP[2][16];
    float allianceSideDeadWP[2][16];

    GuidList medivhSidePieces; //alive pieces guids

    GuidList unusedMedivhPieces; //pieces that was summoned after medivh piece death
    GuidList unusedPlayerPieces; //pieces that was summoned after player piece death
    ...
    uint32 miniEventTimer;
    uint32 endEventTimer;
    uint32 endEventLightningTimer;

    uint32 cheatTimer;
    uint32 currentCheatMask;
    uint32 cheatMask;
    ...
    Position pos;   //location of medivh
    Position tpPos; //location of player teleport point
