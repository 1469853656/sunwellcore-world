...
//trigger AI
move_triggerAI::move_triggerAI(Creature* c)
    : NullCreatureAI(c)
{
...
npc_chesspieceAI::npc_chesspieceAI(Creature* c)
    : ScriptedAI(c)
{
    pInstance = me->GetInstanceScript();
...
void npc_chesspieceAI::UpdateAI(const uint32 diff)
{
    ...
    if ((me->getFaction() == A_FACTION && pInstance->GetData(DATA_CHESS_EVENT_TEAM) == TEAM_HORDE) || (me->getFaction() == H_FACTION && pInstance->GetData(DATA_CHESS_EVENT_TEAM) == TEAM_ALLIANCE))
    {
...
public:
    npc_echo_of_medivh()
        : CreatureScript("npc_echo_of_medivh")
    {
    }
...
boss_MedivhAI::boss_MedivhAI(Creature* c)
    : ScriptedAI(c)
{
...
int boss_MedivhAI::GetCountOfEnemyInMelee(uint64 piece, bool strafe)
{
    ...
    if (strafe)
    {
        for (int i = 0; i < OFFSET8COUNT; ++i)
        {
            ...
            if (tmpOffsetI >= 0 && tmpOffsetI < 8 && tmpOffsetJ >= 0 && tmpOffsetJ < 8 && Enemy(piece, chessBoard[tmpOffsetI][tmpOffsetJ].piece))
                ++tmpCount;
        }
    }
    else
    ...
    switch (range)
    {
        case 25:
            for (i = 0; i < OFFSET25COUNT; i++)
            {
                ...
                if (tmpOffsetI >= 0 && tmpOffsetI < 8 && tmpOffsetJ >= 0 && tmpOffsetJ < 8)
                {
    ...
    switch (range)
    {
        case 25:
            for (i = 0; i < OFFSET25COUNT; i++)
            {
                ...
                if (tmpOffsetI >= 0 && tmpOffsetI < 8 && tmpOffsetJ >= 0 && tmpOffsetJ < 8)
                    if (chessBoard[tmpOffsetI][tmpOffsetJ].piece == to || chessBoard[tmpOffsetI][tmpOffsetJ].trigger == to)
                        return true;
            }
        case 20:
            for (i = 0; i < OFFSET20COUNT; i++)
...
void boss_MedivhAI::RemoveChessPieceFromBoard(Creature* piece)
{
    ...
    {
        --alliancePieces;
        tmpC = me->SummonCreature(tmpEntry, allianceSideDeadWP[0][alliancePieces], allianceSideDeadWP[1][alliancePieces], POSITION_Z + 1.f, ORI_E, TEMPSUMMON_CORPSE_DESPAWN, 0);
        if (tmpC)
            tmpC->AI()->SetData(1, 1);
    }
    else
...
void boss_MedivhAI::ClearBoard()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        for (int j = 0; j < 8; ++j)
        {
            tmpC = ObjectAccessor::GetCreature(*me, chessBoard[i][j].piece);
            if (tmpC)
                tmpC->DespawnOrUnsummon();

            tmpC = ObjectAccessor::GetCreature(*me, chessBoard[i][j].trigger);
            if (tmpC)
                tmpC->DespawnOrUnsummon();
    ...
    {
        tmpC = ObjectAccessor::GetCreature(*me, guid);
        if (tmpC)
            tmpC->DespawnOrUnsummon();
    }
        ...
        switch (GetMoveRange(guid))
        {
            case 25:
                for (i = 0; i < OFFSET25COUNT; i++)
                {
                    ...
                    if (tmpOffsetI >= 0 && tmpOffsetI < 8 && tmpOffsetJ >= 0 && tmpOffsetJ < 8 && !chessBoard[tmpOffsetI][tmpOffsetJ].piece && // if square is empty
