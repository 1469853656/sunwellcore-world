...
struct boss_netherspiteAI : public BossAI
{
    boss_netherspiteAI(Creature* creature)
        : BossAI(creature, BOSS_NETHERSPITE)
    {
    }
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_SET_PORTAL_PHASE:
            {
                ...
                std::shuffle(std::begin(portalPositions), std::end(portalPositions), GetGenerator());
                for (uint8 i = 0; i < 3; ++i)
...
class spell_netherspite_nether_beam_selector_SpellScript : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        Trinity::Containers::EraseIf(targets, [this, boss](WorldObject* target) -> bool {
            if (target->IsPlayer())
