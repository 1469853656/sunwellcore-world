...
struct boss_razorgoreAI : public BossAI
{
    boss_razorgoreAI(Creature* creature)
        : BossAI(creature, BOSS_RAZORGORE)
        , _explosion(false)
    {
    }
...
{
    npc_grethok_the_controllerAI(Creature* creature)
        : ScriptedAI(creature)
    {
        // Visual
...
{
    go_orb_of_dominationAI(GameObject* go)
        : GameObjectAI(go)
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SPAWN_ADDS:
                    Trinity::Containers::EraseIf(_summonGUIDs, [this](uint64 const guid) { return !ObjectAccessor::GetCreature(*go, guid); });
                    if (Creature* razorgore = GetRazorgore())
...
class spell_razorgore_possess_AuraScript : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_razorgore_possess_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_POSSESS, AURA_EFFECT_HANDLE_REAL);
