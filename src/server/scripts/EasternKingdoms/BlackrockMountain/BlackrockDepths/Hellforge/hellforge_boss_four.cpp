...
{
    boss_dwarf_boss_four_AI(Creature* creature)
        : BossAI(creature, DATA_BOSS_FOUR)
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        else if (type == POINT_MOTION_TYPE)
        {
            if (pointId == POINT_BOSS_FOUR_CHANGE_EQUIPMENT_FROST || pointId == POINT_BOSS_FOUR_CHANGE_EQUIPMENT_FINAL)
            {
                ...
                events.ScheduleEvent(pointId == POINT_BOSS_FOUR_CHANGE_EQUIPMENT_FROST ? EVENT_BOSS_FOUR_BEGIN_FROST_PHASE : EVENT_BOSS_FOUR_BEGIN_FINAL_PHASE, 3s);
            }
        }
    }
...
{
    npc_boss_four_lightning_target_AI(Creature* creature)
        : ScriptedAI(creature)
    {
        me->SetPassive();
...
{
    npc_boss_four_blizzard_target_AI(Creature* creature)
        : ScriptedAI(creature)
    {
        HellforgeStatValues val;
...
{
    npc_boss_fiery_orb_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_four_pillar_protection(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_four_hammer_hit_pool(Creature* creature)
        : ScriptedAI(creature)
    {
