...
std::array<const Position, POSITION_CORNER_SIZE> const _fightingAreaCorners = { { { 1390.760f, -400.398f, -94.674f },
...
{
    boss_hellforge_boss_two_AI(Creature* creature)
        : BossAI(creature, DATA_BOSS_TWO)
        , _phaseTwo(false)
        , _mySoakerSpawnPositions(_soakerSpawnPositions)
    {
    }
    ...
    void InitializeMovement()
    {
        scheduler.Schedule(1s, [this](TaskContext func) {
            ...
            sLog->outDebug(LOG_FILTER_TSCR, "Hellforge boss two, function InitializeMovement initializes spline of size (%zu) and duration " UI64FMTD, _path.size(), time.count());
            func.Repeat(time);
        });
    }
...
{
    npc_boss_two_soaker_AI(Creature* creature)
        : ScriptedAI(creature)
    {
        _events.Reset();
...
{
    go_altar_of_sacrifice_hellforge_AI(GameObject* go)
        : GameObjectAI(go)
...
{
    npc_hellforge_boss_two_add_AI(Creature* creature)
        : ScriptedAI(creature)
        , _currentCarrier(0)
...
{
    npc_boss_two_crossfire_trigger_AI(Creature* creature)
        : ScriptedAI(creature)
...
{
    npc_add_poison_spreader_AI(Creature* creature)
        : ScriptedAI(creature)
        , pos(me->GetPosition())
        , _failCounter(0)
        , charmerGUID(0)
...
{
public:
    spell_boss_two_blight_crystal_damage()
        : SpellScriptLoader("spell_boss_two_blight_crystal_damage")
    {
    }
...
{
    go_gong_of_boss_two(GameObject* go)
        : GameObjectAI(go)
    {
        go->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);
    }
