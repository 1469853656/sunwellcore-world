...
{
    boss_hellforge_five_AI(Creature* creature)
        : BossAI(creature, DATA_BOSS_FIVE)
    {
...
{
    npc_boss_five_frosty_worgen_AI(Creature* creature)
        : ScriptedAI(creature)
...
{
    npc_boss_five_melee_explosion_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_five_fearer_AI(Creature* creature)
        : ScriptedAI(creature)
    {
        _instance = me->GetInstanceScript();
    }
    ...
    void LoadBossStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({
            BOSS_FIVE_ADD_MINDMASTER_HEALTH,
            BOSS_FIVE_ADD_MINDMASTER_MANA,
            BOSS_FIVE_ADD_MINDMASTER_WITHER_DAMAGE,
            BOSS_FIVE_ADD_MINDMASTER_WITHER_DOT_DMG,
    ...
    void EnterCombat(Unit* who) override
    {
        ...
        _task.Schedule(2s, [this](TaskContext func) {
            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0U, [this](Unit* object) -> bool {
                    ...
                    Unit* target2 = boss->AI()->SelectTarget(SELECT_TARGET_TOPAGGRO);
                    if (!target2)
                        return false;

                    if (object->GetGUID() == target2->GetGUID())
                        return false;
...
{
    npc_boss_five_burner_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_five_big_add_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_five_bat_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
struct npc_boss_five_flame_spreader_AI : public ScriptedAI
{
    npc_boss_five_flame_spreader_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    void JustDied(Unit* killer) override
