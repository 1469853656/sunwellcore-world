...
{
    npc_boss_six_diablo_AI(Creature* creature)
        : BossAI(creature, DATA_DIABLO)
    {
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_DIABLO_NAPALAM_SHELL_PERCENTAGE,
            STAT_DIABLO_NAPALAM_SHELL_TIMER,
            STAT_DIABLO_NAPALAM_SHELL_READJUST_HEALTH_PERCENTAGE,
            STAT_DIABLO_NAPALAM_SHELL_COUNT,
            STAT_DIABLO_NAPALAM_SHELL_READJUSTED_COUNT,
    ...
    void BeginShadowRealm()
    {
        ...
        _pentagramIndex = 0;
        Unit* target    = SelectTarget(SELECT_TARGET_RANDOM, 1U, [this](Unit* object) {
            return object->IsPlayer() && !object->HasAura(SPELL_DIABLO_MC_INSANE) && !object->HasAura(SPELL_DIABLO_BUFFETTING_WINDS) && !object->HasAura(SPELL_DIABLO_CONVERSION_BEAM);
        });
...
{
    npc_boss_six_flying_demon_AI(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    npc_boss_six_beam_trigger(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_boss_six_beam_trigger_explosion(Creature* creature)
        : ScriptedAI(creature)
    {
        _scheduler.CancelAll();
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_BEAM_FIREBEAM_EXPLOSION_DAMAGE, STAT_BEAM_FIREBEAM_EXPLOSION_RADIUS_RATIO, STAT_BEAM_FIREBEAM_FLAME_PATCH_PCT_DMG,
...
{
    npc_boss_diablo_playerclone(Creature* creature)
        : ScriptedAI(creature)
    {
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_PLAYERCLONE_COMET_COUNT, STAT_PLAYERCLONE_COMET_TIMER, STAT_PLAYERCLONE_COMET_REPEAT_TIMER });
...
{
    npc_boss_diablo_comet_trigger(Creature* creature)
        : ScriptedAI(creature)
    {
        _instance         = me->GetInstanceScript();
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_COMET_METEOR_HIT_TIMER, STAT_COMET_METEOR_HIT_DAMAGE, STAT_COMET_METEOR_RADIUS_RATIO,
...
{
    npc_boss_six_diabolic_infernal(Creature* creature)
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_INFERNAL_HELLFIRE_TIMER, STAT_INFERNAL_HELLFIRE_DAMAGE, STAT_INFERNAL_HELLFIRE_RADIUS_RATIO, STAT_INFERNAL_HELLFIRE_REPEAT_TIMER });
...
{
    npc_boss_six_increased_damage_trigger(Creature* creature)
        : NullCreatureAI(creature)
    {
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_DAMAGE_TRIGGER_CHECK_TIMER, STAT_DAMAGE_TRIGGER_BUFFFET_DURATION, STAT_DAMAGE_TRIGGET_FLAME_BUFFET_DMG, STAT_DAMAGE_TRIGGER_FLAME_BUFFET_RADIUS_RATIO });
...
{
    npc_boss_diablo_fire_elementals(Creature* creature)
        : NullCreatureAI(creature)
    {
        LoadStats();
        me->SetCanMissSpells(false);
    ...
    void LoadStats()
    {
        HellforgeStats _stats = sWorldCache.GetStatValues({ STAT_ELEMENTAL_DIABLO_DISTANCE, STAT_ELEMENTAL_SELF_DAMAGE_RATIO, STAT_ELEMENTAL_CHECK_TIMER,
...
{
    npc_boss_player_flame_sphere(Creature* creature)
        : ScriptedAI(creature)
    {
        me->SetCanMissSpells(false);
        _arrived    = false;
...
{
    npc_boss_diablo_shadow_drake(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_diablo_napalm_shell_trigger(Creature* creature)
        : NullCreatureAI(creature)
    {
    }
...
{
    npc_abyssal_knight_diablo(Creature* creature)
        : ScriptedAI(creature)
    {
    }
