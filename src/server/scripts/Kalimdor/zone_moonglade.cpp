...
public:
    npc_bunthen_plainswind()
        : CreatureScript("npc_bunthen_plainswind")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF + 1:
                ...
                if (player->getClass() == CLASS_DRUID && player->GetTeam(CF_DENY) == TEAM_HORDE)
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        else if (player->getClass() == CLASS_DRUID && player->GetTeam(CF_DENY) == TEAM_HORDE)
        {
...
public:
    npc_great_bear_spirit()
        : CreatureScript("npc_great_bear_spirit")
    {
    }
...
public:
    npc_silva_filnaveth()
        : CreatureScript("npc_silva_filnaveth")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF + 1:
                player->CLOSE_GOSSIP_MENU();
                if (player->getClass() == CLASS_DRUID && player->GetTeam(CF_DENY) == TEAM_ALLIANCE)
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        else if (player->GetTeam(CF_DENY) != TEAM_ALLIANCE)
        {
            if (player->GetQuestStatus(QUEST_SEA_LION_HORDE) == QUEST_STATUS_INCOMPLETE)
        ...
        else if (player->getClass() == CLASS_DRUID && player->GetTeam(CF_DENY) == TEAM_ALLIANCE)
        {
...
public:
    npc_clintar_spirit()
        : CreatureScript("npc_clintar_spirit")
    {
    }
...
public:
    npc_giant_spotlight()
        : CreatureScript("npc_giant_spotlight")
    {
    }
