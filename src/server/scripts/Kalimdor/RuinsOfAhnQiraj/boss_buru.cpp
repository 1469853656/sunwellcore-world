...
struct npc_buru_eggAI : public ScriptedAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (Creature* buru = me->GetMap()->GetCreature(_instance->GetData64(DATA_BURU)))
            if (boss_buruAI* buruAI = dynamic_cast<boss_buruAI*>(buru->AI()))
...
{
    new CreatureAILoader<boss_buruAI>("boss_buru");
    new CreatureAILoader<npc_buru_eggAI>("npc_buru_egg");
    new SpellScriptLoaderEx<spell_egg_explosion_SpellScript>("spell_egg_explosion");
}
