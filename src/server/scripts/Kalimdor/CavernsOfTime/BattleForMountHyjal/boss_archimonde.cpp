...
public:
    npc_ancient_wisp()
        : CreatureScript("npc_ancient_wisp")
    {
    }
...
public:
    npc_doomfire_targetting()
        : CreatureScript("npc_doomfire_targetting")
    {
    }
...
public:
    boss_archimonde()
        : CreatureScript("boss_archimonde")
    {
    }
            ...
            if (MeleeRangeCheckTimer <= diff)
            {
                if (CanUseFingerOfDeath())
                {
                    Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, [&](Unit* obj) {
                        return obj->IsPlayer() && obj->IsHostileTo(me);
                    });
                    if (target)
