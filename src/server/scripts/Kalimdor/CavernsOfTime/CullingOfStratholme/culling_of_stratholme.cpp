        ...
        void WaypointReached(uint32 uiPointId) override
        {
            switch (uiPointId)
            {
                ...
                case 11:
                    ...
                    {
                        cityman->MonsterSay("Oh no...", LANG_UNIVERSAL, nullptr); // missing script_text
                        me->CastSpell(cityman, SPELL_ARTHAS_CRUSADER_STRIKE, true);
                    }
                    me->SetReactState(REACT_DEFENSIVE);
                    SetEscortPaused(true);
            ...
            if (eventInRun)
            {
                ...
                switch (uint32 currentEvent = actionEvents.GetEvent())
                {
                    ...
                    case EVENT_ACTION_PHASE3 + 9:
                        ...
                        summons.DespawnAll();
                        me->MonsterSay("I can't afford to spare you.", LANG_UNIVERSAL, nullptr);
                        me->SetFacingTo(0.0f);
                        ScheduleNextEvent(currentEvent, 5000);
                        break;
...
public:
    npc_crate_helper()
        : CreatureScript("npc_create_helper_cot")
    {
    }
...
{
public:
    npc_cos_chromie_start()
        : CreatureScript("npc_cos_chromie_start")
    {
    }
...
public:
    npc_cos_chromie_middle()
        : CreatureScript("npc_cos_chromie_middle")
    {
    }
...
public:
    ...
    {
        npc_cos_stratholme_citizienAI(Creature* creature)
            : ScriptedAI(creature)
        {
            allowTimer = 0;
