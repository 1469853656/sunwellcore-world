...
public:
    npcs_rutgar_and_frankal()
        : CreatureScript("npcs_rutgar_and_frankal")
    {
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(8304) == QUEST_STATUS_INCOMPLETE && creature->GetEntry() == 15170 && !player->GetReqKillOrCastCurrentCount(8304, TRIGGER_RUTGAR))
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);

        if (player->GetQuestStatus(8304) == QUEST_STATUS_INCOMPLETE && creature->GetEntry() == 15171 && player->GetReqKillOrCastCurrentCount(8304, TRIGGER_RUTGAR))
...
public:
    npc_anachronos_the_ancient()
        : CreatureScript("npc_anachronos_the_ancient")
    {
    }
    ...
    struct npc_anachronos_the_ancientAI : public ScriptedAI
    {
        ...
        {
            Creature* Fandral     = me->FindNearestCreature(C_FANDRAL_STAGHELM, 100.0f);
            Creature* Arygos      = me->FindNearestCreature(C_ARYGOS, 100.0f);
            Creature* Caelestrasz = me->FindNearestCreature(C_CAELESTRASZ, 100.0f);
            Creature* Merithra    = me->FindNearestCreature(C_MERITHRA, 100.0f);

            if (Fandral)
                _dragonGUIDs[C_FANDRAL_STAGHELM] = Fandral->GetGUID();

            if (Arygos)
                _dragonGUIDs[C_ARYGOS] = Arygos->GetGUID();

            if (Caelestrasz)
                _dragonGUIDs[C_CAELESTRASZ] = Caelestrasz->GetGUID();

            if (Merithra)
                _dragonGUIDs[C_MERITHRA] = Merithra->GetGUID();
        }

        Creature* GetDragonCreature(uint32 entry)
        {
            auto it = _dragonGUIDs.find(entry);
            if (it == _dragonGUIDs.end())
                return nullptr;

            return ObjectAccessor::GetCreature(*me, it->second);
        }
        ...
        void HandleAnimation()
        {
            ...
            if (eventEnd == false)
            {
                ...
                {
                    case 0:
                        Talk(ANACHRONOS_SAY_1, GetDragonCreature(C_FANDRAL_STAGHELM));
                        break;
                    case 1: {
                        Creature* Fandral = GetDragonCreature(C_FANDRAL_STAGHELM);
                        if (Fandral)
                        {
                    ...
                    case 2:
                        if (Creature* Fandral = GetDragonCreature(C_FANDRAL_STAGHELM))
                            Fandral->SetTarget(0);

                        if (Creature* Merithra = GetDragonCreature(C_MERITHRA))
                            Merithra->AI()->Talk(MERITHRA_EMOTE_1, Merithra);
                        break;
                    case 3:
                        if (Creature* Merithra = GetDragonCreature(C_MERITHRA))
                        ...
                        break;
                    case 4:
                        if (Creature* Arygos = GetDragonCreature(C_ARYGOS))
                        Arygos->AI()->Talk(ARYGOS_EMOTE_1, Arygos);
                        break;
                    case 5: {
                        Creature* Caelestrasz = GetDragonCreature(C_CAELESTRASZ);
                        Creature* Fandral     = GetDragonCreature(C_FANDRAL_STAGHELM);
                        if (Caelestrasz && Fandral)
                        {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        if (Creature* Caelestrasz = GetDragonCreature(C_CAELESTRASZ))
                    ...
                    case 10:
                        if (Creature* Merithra = GetDragonCreature(C_MERITHRA))
                        {
                            ...
                            Merithra->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
                    ...
                    case 18:
                        if (Creature* Arygos = GetDragonCreature(C_ARYGOS))
                        {
                            ...
                            Arygos->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
                    ...
                    case 26:
                        if (Creature* Caelestrasz = GetDragonCreature(C_CAELESTRASZ))
                        {
                            ...
                            Caelestrasz->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
                    ...
                    case 42:
                        ...
                        if (Creature* Fandral = GetDragonCreature(C_FANDRAL_STAGHELM))
                        {
                            Fandral->AI()->Talk(FANDRAL_EMOTE_1, Fandral);
                            Fandral->SetStandState(UNIT_STAND_STATE_KNEEL);
                        }
                        break;
                    case 43:
                        if (Creature* Fandral = GetDragonCreature(C_FANDRAL_STAGHELM))
                    ...
                    case 50:
                        if (Creature* Fandral = GetDragonCreature(C_FANDRAL_STAGHELM))
                        {
                            Fandral->SetStandState(UNIT_STAND_STATE_STAND);
                            Fandral->AI()->Talk(FANDRAL_EMOTE_2, Fandral);
                            Fandral->CastSpell(-8127, 1525, 17.5f, 33806, true);
                        }
                        break;
                    case 51: {
                        std::list<Creature*> _mobs;
                        uint32 entries[4] = { 15423, 15424, 15414, 15422 };
                        for (uint32 entry : entries)
                            me->GetCreatureListWithEntryInGrid(_mobs, entry, 100.f);

                        for (Creature* creature : _mobs)
                    ...
                    case 61: {
                        ...
                        std::for_each(_lPlayers.begin(), _lPlayers.end(), [&](Player* player) {
    ...
    private:
        std::unordered_map<uint32 /*entry*/, uint64 /*guid*/> _dragonGUIDs;
    };
};
...
public:
    npc_qiraj_war_spawn()
        : CreatureScript("npc_qiraj_war_spawn")
    {
    }
...
public:
    go_crystalline_tear()
        : GameObjectScript("go_crystalline_tear")
    {
    }
    ...
    bool OnQuestAccept(Player* player, GameObject* go, Quest const* quest)
    {
        ...
        {
            if (go->FindNearestCreature(15378, 100.f))
                return false;
            ...
            if (Creature* trigger = go->FindNearestCreature(15454, 100, player))
            {
                ...
                if (Anachronos)
                {
                    if (Anachronos->IsAIEnabled)
                        Anachronos->AI()->DoAction(0);
...
public:
    go_wind_stone()
        : GameObjectScript("go_wind_stone")
    {
    }
