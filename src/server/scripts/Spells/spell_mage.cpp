...
public:
    spell_mage_arcane_blast()
        : SpellScriptLoader("spell_mage_arcane_blast")
    {
    }
...
{
public:
    spell_mage_deep_freeze()
        : SpellScriptLoader("spell_mage_deep_freeze")
    {
    }
...
public:
    spell_mage_burning_determination()
        : SpellScriptLoader("spell_mage_burning_determination")
    {
    }
...
{
public:
    spell_mage_molten_armor()
        : SpellScriptLoader("spell_mage_molten_armor")
    {
    }
...
public:
    spell_mage_mirror_image()
        : SpellScriptLoader("spell_mage_mirror_image")
    {
    }
...
public:
    spell_mage_burnout()
        : SpellScriptLoader("spell_mage_burnout")
    {
    }
...
public:
    spell_mage_burnout_trigger()
        : SpellScriptLoader("spell_mage_burnout_trigger")
    {
    }
...
{
public:
    spell_mage_pet_scaling()
        : SpellScriptLoader("spell_mage_pet_scaling")
    {
    }
...
public:
    spell_mage_brain_freeze()
        : SpellScriptLoader("spell_mage_brain_freeze")
    {
    }
    ...
    class spell_mage_brain_freeze_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            //  Ice Armor - Chilled      Frost Armor - Chilled
            if (spellInfo->Id == 7321 || spellInfo->Id == 6136)
                return false;

            // Rethil: Improved Blizzard, check if chilled. Ranks ID: 12484, 12485, 12486
            if ((spellInfo->Id ^ 0x30C4) < 3)
                return spellInfo->Id;
...
public:
    spell_mage_fire_frost_ward()
        : SpellScriptLoader("spell_mage_fire_frost_ward")
    {
    }
...
public:
    spell_mage_focus_magic()
        : SpellScriptLoader("spell_mage_focus_magic")
    {
    }
...
public:
    spell_mage_ice_barrier()
        : SpellScriptLoader("spell_mage_ice_barrier")
    {
    }
...
public:
    spell_mage_ignite()
        : SpellScriptLoader("spell_mage_ignite")
    {
    }
...
public:
    spell_mage_living_bomb()
        : SpellScriptLoader("spell_mage_living_bomb")
    {
    }
...
public:
    spell_mage_mana_shield()
        : SpellScriptLoader("spell_mage_mana_shield")
    {
    }
...
public:
    spell_mage_master_of_elements()
        : SpellScriptLoader("spell_mage_master_of_elements")
    {
    }
    ...
    class spell_mage_master_of_elements_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            int32 mana                            = int32(eventInfo.GetDamageInfo()->GetSpellInfo()->CalcPowerCost(GetTarget(), eventInfo.GetDamageInfo()->GetSchoolMask(), nullptr, manaPercent));
            mana                                  = CalculatePct(mana, aurEff->GetAmount());
...
public:
    ...
    {
        PrepareSpellScript(spell_mage_summon_water_elemental_SpellScript) bool Validate(SpellInfo const* /*spellEntry*/) override
        {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            ///Not sure why this is here, so I'm just commenting it out ~Monich
            /*if (Creature *pet = ObjectAccessor::GetCreature(*caster, caster->GetPetGUID()))
...
class spell_mage_arcane_missiles : public AuraScript
{
    ...
    {
        if (Unit* caster = GetCaster())
            caster->RemoveAura(44401, caster->GetGUID(), 0, AURA_REMOVE_BY_EXPIRE);
    }

    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_mage_arcane_missiles::AfterRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
};
