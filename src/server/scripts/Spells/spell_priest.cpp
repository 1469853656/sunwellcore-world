...
public:
    spell_pri_shadowfiend_scaling()
        : SpellScriptLoader("spell_pri_shadowfiend_scaling")
    {
    }
...
public:
    spell_pri_circle_of_healing()
        : SpellScriptLoader("spell_pri_circle_of_healing")
    {
    }
...
public:
    spell_pri_divine_aegis()
        : SpellScriptLoader("spell_pri_divine_aegis")
    {
    }
...
public:
    spell_pri_divine_hymn()
        : SpellScriptLoader("spell_pri_divine_hymn")
    {
    }
...
public:
    spell_pri_glyph_of_prayer_of_healing()
        : SpellScriptLoader("spell_pri_glyph_of_prayer_of_healing")
    {
    }
...
public:
    spell_pri_hymn_of_hope()
        : SpellScriptLoader("spell_pri_hymn_of_hope")
    {
    }
...
public:
    spell_pri_item_greater_heal_refund()
        : SpellScriptLoader("spell_pri_item_greater_heal_refund")
    {
    }
...
public:
    spell_pri_lightwell_renew()
        : SpellScriptLoader("spell_pri_lightwell_renew")
    {
    }
        ...
        void HandleRemoveEffect(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                for (Unit* controlled : caster->m_Controlled)
                    if (controlled->GetDisplayId() == MODEL_LIGHTWELL)
                    {
...
public:
    spell_pri_mana_leech()
        : SpellScriptLoader("spell_pri_mana_leech")
    {
    }
...
public:
    ...
    class spell_pri_power_word_shield_AuraScript : public AuraScript
    {
        ...
        {
            // Workaround for Sundial of the Exiled trinket, spell won't proc from only applying aura
            constexpr uint32 SPELL_NOW_IS_THE_TIME         = 60063;
            constexpr uint32 SPELL_NOW_IS_THE_TIME_TRIGGER = 60064;
            if (Unit* caster = GetCaster())
                if (caster->IsPlayer() && caster->HasAura(SPELL_NOW_IS_THE_TIME, caster->GetGUID()) && !caster->HasSpellCooldown(SPELL_NOW_IS_THE_TIME))
                {
                    caster->CastSpell(caster, SPELL_NOW_IS_THE_TIME_TRIGGER, true);
                    if (SpellProcEventEntry const* procEvent = sSpellMgr->GetSpellProcEvent(SPELL_NOW_IS_THE_TIME))
                        caster->ToPlayer()->AddSpellCooldown(SPELL_NOW_IS_THE_TIME, 0, procEvent->cooldown);
                }
        }
...
public:
    spell_pri_prayer_of_mending_heal()
        : SpellScriptLoader("spell_pri_prayer_of_mending_heal")
    {
    }
...
public:
    spell_pri_renew()
        : SpellScriptLoader("spell_pri_renew")
    {
    }
...
public:
    spell_pri_vampiric_touch()
        : SpellScriptLoader("spell_pri_vampiric_touch")
    {
    }
