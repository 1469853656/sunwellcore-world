...
{
public:
    spell_warl_eye_of_kilrogg()
        : SpellScriptLoader("spell_warl_eye_of_kilrogg")
    {
    }
...
{
public:
    spell_warl_shadowflame()
        : SpellScriptLoader("spell_warl_shadowflame")
    {
    }
...
public:
    spell_warl_improved_demonic_tactics()
        : SpellScriptLoader("spell_warl_improved_demonic_tactics")
    {
    }
...
{
public:
    spell_warl_ritual_of_summoning()
        : SpellScriptLoader("spell_warl_ritual_of_summoning")
    {
    }
...
{
public:
    spell_warl_demonic_aegis()
        : SpellScriptLoader("spell_warl_demonic_aegis")
    {
    }
...
public:
    spell_warl_demonic_knowledge()
        : SpellScriptLoader("spell_warl_demonic_knowledge")
    {
    }
...
{
public:
    spell_warl_generic_scaling()
        : SpellScriptLoader("spell_warl_generic_scaling")
    {
    }
    ...
    class spell_warl_generic_scaling_AuraScript : public AuraScript
    {
        ...
        void CalculateAPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            if (Unit* owner = GetUnitOwner()->GetOwner())
            {
                ...
                //Aura apply for this glyph of Felguard
                if (uint64 petGUID = owner->GetPetGUID())
                    if (Unit* pet = ObjectAccessor::GetUnit(*owner, petGUID))
                        if (pet->GetEntry() == NPC_FELGUARD)
                        {
                            if (owner->HasAura(56246) && !pet->HasAura(56246)) pet->AddAura(56246, pet);
                            if (!owner->HasAura(56246) && pet->HasAura(56246)) pet->RemoveAura(56246);
                        }
            }
        }
...
public:
    spell_warl_infernal_scaling()
        : SpellScriptLoader("spell_warl_infernal_scaling")
    {
    }
...
public:
    spell_warl_banish()
        : SpellScriptLoader("spell_warl_banish")
    {
    }
...
public:
    spell_warl_ritual_of_doom_effect()
        : SpellScriptLoader("spell_warl_ritual_of_doom_effect")
    {
    }
...
{
public:
    spell_warl_siphon_life()
        : SpellScriptLoader("spell_warl_siphon_life")
    {
    }
...
public:
    spell_warl_life_tap()
        : SpellScriptLoader("spell_warl_life_tap")
    {
    }
...
public:
    spell_warl_demonic_circle_summon()
        : SpellScriptLoader("spell_warl_demonic_circle_summon")
    {
    }
...
public:
    spell_warl_demonic_circle_teleport()
        : SpellScriptLoader("spell_warl_demonic_circle_teleport")
    {
    }
...
public:
    spell_warl_fel_synergy()
        : SpellScriptLoader("spell_warl_fel_synergy")
    {
    }
...
public:
    spell_warl_haunt()
        : SpellScriptLoader("spell_warl_haunt")
    {
    }
...
public:
    spell_warl_unstable_affliction()
        : SpellScriptLoader("spell_warl_unstable_affliction")
    {
    }
...
public:
    spell_warl_health_funnel()
        : SpellScriptLoader("spell_warl_health_funnel")
    {
    }
...
public:
    spell_warl_shadow_ward()
        : SpellScriptLoader("spell_warl_shadow_ward")
    {
    }
...
public:
    spell_wrl_fire_bolt()
        : SpellScriptLoader("spell_wrl_fire_bolt")
    {
    }
...
{
public:
    spell_warl_everlasting_affliction_aura()
        : SpellScriptLoader("spell_warl_everlasting_affliction_aura")
    {
    }
...
{
public:
    DelayedInterruptEvent(Unit* caster)
        : _caster(caster)
    {
    }
...
// -1120 - Drain Soul
class spell_warl_drain_soul final : public AuraScript
{
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        uint32 everlastingAfflictionSpellOnCaster = 0;
        for (uint32 const everlastingAfflictionSpellID : { 47205, 47204, 47203, 47202, 47201 })
        {
            if (caster->HasAura(everlastingAfflictionSpellID))
            {
                everlastingAfflictionSpellOnCaster = everlastingAfflictionSpellID;
                break;
            }
        }

        if (!everlastingAfflictionSpellOnCaster)
            return;

        SpellInfo const* everlastingAfflictionSpellInfo = sSpellMgr->GetSpellInfo(everlastingAfflictionSpellOnCaster);
        if (!everlastingAfflictionSpellInfo || !roll_chance_i(everlastingAfflictionSpellInfo->ProcChance))
            return;

        if (Unit const* target = GetAura()->GetUnitOwner())
        {
            // Refresh corruption on target
            if (AuraEffect* aur = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_WARLOCK, 0x2, 0, 0, GetCaster()->GetGUID()))
            {
                aur->GetBase()->RefreshTimersWithMods();
                aur->ChangeAmount(aur->CalculateAmount(aur->GetCaster()), false);
            }
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectRemoveFn(spell_warl_drain_soul::OnApply, EFFECT_1, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
};
