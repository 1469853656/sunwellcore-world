...
public:
    spell_dk_wandering_plague()
        : SpellScriptLoader("spell_dk_wandering_plague")
    {
    }
...
public:
    spell_dk_raise_ally()
        : SpellScriptLoader("spell_dk_raise_ally")
    {
    }
...
{
public:
    spell_dk_raise_ally_trigger()
        : SpellScriptLoader("spell_dk_raise_ally_trigger")
    {
    }
...
public:
    spell_dk_aotd_taunt()
        : SpellScriptLoader("spell_dk_aotd_taunt")
    {
    }
...
public:
    spell_dk_death_and_decay()
        : SpellScriptLoader("spell_dk_death_and_decay")
    {
    }
    ...
    class spell_dk_death_and_decay_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* aurEff)
        {
            ...
            if (GetCaster() && GetTarget())
            {
                ...
                GetCaster()->CastCustomSpell(GetTarget(), SPELL_DK_DEATH_AND_DECAY_TRIGGER, &basePoints0, nullptr, nullptr, true, nullptr, aurEff);
            }
        }
...
public:
    spell_dk_master_of_ghouls()
        : SpellScriptLoader("spell_dk_master_of_ghouls")
    {
    }
...
public:
    spell_dk_bloodworms()
        : SpellScriptLoader("spell_dk_bloodworms")
    {
    }
...
{
public:
    spell_dk_summon_gargoyle()
        : SpellScriptLoader("spell_dk_summon_gargoyle")
    {
    }
...
{
public:
    spell_dk_improved_blood_presence_proc()
        : SpellScriptLoader("spell_dk_improved_blood_presence_proc")
    {
    }
...
{
public:
    spell_dk_wandering_plague_aura()
        : SpellScriptLoader("spell_dk_wandering_plague_aura")
    {
    }
...
public:
    spell_dk_rune_of_the_fallen_crusader()
        : SpellScriptLoader("spell_dk_rune_of_the_fallen_crusader")
    {
    }
...
public:
    spell_dk_bone_shield()
        : SpellScriptLoader("spell_dk_bone_shield")
    {
    }
...
public:
    spell_dk_blood_caked_blade()
        : SpellScriptLoader("spell_dk_blood_caked_blade")
    {
    }
...
class spell_dk_dancing_rune_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        if (SpellInfo const* procSpell = eventInfo.GetSpellInfo())
            for (uint32 const spellId : { SPELL_DK_BLOOD_STRIKE, SPELL_DK_ICY_TOUCH, SPELL_DK_PLAGUE_STRIKE, SPELL_DK_HEART_STRIKE, SPELL_DK_DEATH_STRIKE,
                SPELL_DK_BLOOD_BOIL, SPELL_DK_RUNE_STRIKE, SPELL_DK_DEATH_COIL, SPELL_DK_CHAINS_OF_ICE })
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* dancingRuneWeapon = ObjectAccessor::GetUnit(*player, _runeWeaponGUID);
        if (!dancingRuneWeapon)
            return;

        Unit* target = ObjectAccessor::GetUnit(*player, dancingRuneWeapon->GetAI()->GetGUID(0));
        if (!target || target->GetDistance(player) > 30.f)
            return;
        ...
        {
            bool const isChainsOfIce = sSpellMgr->GetFirstSpellInChain(procSpell->Id) == SPELL_DK_CHAINS_OF_ICE;
            uint32 const damage = eventInfo.GetDamageInfo()->GetDamage();
            if (damage > 0 || isChainsOfIce)
            {
                CustomSpellValues values;
                if (!isChainsOfIce)
                {
                    values.AddSpellMod(SPELLVALUE_DAMAGE_OVERRIDE, CalculatePct<int32>(damage, 50) + 1);
                    values.AddSpellMod(SPELLVALUE_FORCED_CRIT_RESULT, eventInfo.GetHitMask() & PROC_EX_CRITICAL_HIT);
                }

                dancingRuneWeapon->CastCustomSpell(procSpell->Id, values, target, TRIGGERED_FULL_MASK, NullItemRef, aurEff, dancingRuneWeapon->GetGUID());
    ...
    void HandleEffectApply1(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        for (Unit* controlled : owner->m_Controlled)
            if (static_cast<int32>(controlled->GetEntry()) == GetSpellInfo()->Effects[EFFECT_0].MiscValue)
            {
    ...
    void HandleEffectApply2(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        // Inform rune weapon about proper target, needed in script
        if (Unit* victim = GetUnitOwner())
            if (Unit* owner = GetCaster())
                for (Unit* runeWeapon : owner->m_Controlled)
                    if (static_cast<int32>(runeWeapon->GetEntry()) == GetSpellInfo()->Effects[EFFECT_0].MiscValue)
                    {
                        runeWeapon->GetAI()->SetGUID(victim->GetGUID());
                        break;
                    }
    }
    ...
    void Register() override
    {
        ...
        OnEffectApply += AuraEffectApplyFn(spell_dk_dancing_rune_weapon::HandleEffectApply1, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        OnEffectApply += AuraEffectApplyFn(spell_dk_dancing_rune_weapon::HandleEffectApply2, EFFECT_2, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_dk_rune_weapon_scaling_02 : public AuraScript
{
    ...
    void CalculateDamageDoneAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
    {
        ...
        {
            pet->SetInt32Value(UNIT_FIELD_ATTACK_POWER, 0);
    ...
    void CalculateAmountMeleeHaste(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
    {
        ...
        if (Player* owner = GetUnitOwner()->GetOwner()->ToPlayer())
            if (owner->m_modAttackSpeedPct[BASE_ATTACK] < 1.0f) // inherit haste only
                amount = static_cast<int32>(((1.0f / owner->m_modAttackSpeedPct[BASE_ATTACK]) - 1.0f) * 100.0f);
    }
...
public:
    spell_dk_scent_of_blood_trigger()
        : SpellScriptLoader("spell_dk_scent_of_blood_trigger")
    {
    }
...
public:
    spell_dk_pet_scaling()
        : SpellScriptLoader("spell_dk_pet_scaling")
    {
    }
...
public:
    spell_dk_blood_gorged()
        : SpellScriptLoader("spell_dk_blood_gorged")
    {
    }
...
{
public:
    explicit CorpseExplosionCheck(uint64 casterGUID)
...
public:
    spell_dk_corpse_explosion()
        : SpellScriptLoader("spell_dk_corpse_explosion")
    {
    }
...
public:
    spell_dk_death_coil()
        : SpellScriptLoader("spell_dk_death_coil")
    {
    }
...
public:
    spell_dk_death_grip()
        : SpellScriptLoader("spell_dk_death_grip")
    {
    }
            ...
            if (Player* player = GetCaster()->ToPlayer())
                for (Unit::ControlSet::const_iterator itr = player->m_Controlled.begin(); itr != player->m_Controlled.end(); ++itr)
                    if (Creature* undeadPet = (*itr)->ToCreature())
                        if (undeadPet->IsAlive() && undeadPet->GetOwnerGUID() == player->GetGUID() && undeadPet->GetCreatureType() == CREATURE_TYPE_UNDEAD && undeadPet->IsWithinDist(player, 100.0f, false))
                            return SPELL_CAST_OK;
...
public:
    spell_dk_death_strike()
        : SpellScriptLoader("spell_dk_death_strike")
    {
    }
...
public:
    spell_dk_ghoul_explode()
        : SpellScriptLoader("spell_dk_ghoul_explode")
    {
    }
...
public:
    spell_dk_icebound_fortitude()
        : SpellScriptLoader("spell_dk_icebound_fortitude")
    {
    }
...
public:
    ...
    class spell_dk_pestilence_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            // LK Encounter Hack, pestilence shouldn't target siphoning Val'kyr Shadowguard
            Trinity::Containers::EraseIf(targets, [](WorldObject* target) { return target->GetEntry() == 36609 && target->ToCreature()->AI()->GetData(1000); });
        }
        ...
        void Register() override
        {
            ...
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_dk_pestilence_SpellScript::FilterTargets, EFFECT_2, TARGET_UNIT_DEST_AREA_ENEMY);
        }
    };
...
public:
    spell_dk_presence()
        : SpellScriptLoader("spell_dk_presence")
    {
    }
...
{
public:
    explicit RaiseDeadCheck(Player const* caster)
        : _caster(caster)
    {
    }
...
public:
    spell_dk_rune_tap_party()
        : SpellScriptLoader("spell_dk_rune_tap_party")
    {
    }
...
public:
    spell_dk_scent_of_blood()
        : SpellScriptLoader("spell_dk_scent_of_blood")
    {
    }
...
public:
    spell_dk_spell_deflection()
        : SpellScriptLoader("spell_dk_spell_deflection")
    {
    }
...
public:
    spell_dk_vampiric_blood()
        : SpellScriptLoader("spell_dk_vampiric_blood")
    {
    }
...
public:
    spell_dk_will_of_the_necropolis()
        : SpellScriptLoader("spell_dk_will_of_the_necropolis")
    {
    }
...
public:
    spell_dk_blood_tap()
        : SpellScriptLoader("spell_dk_blood_tap")
    {
    }
    ...
    class spell_dk_blood_tap_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            return sSpellMgr->GetSpellInfo(SPELL_DK_BLOOD_TAP);
        }
        ...
        void HandleEffectApply()
        {
            ...
            if (Player* player = GetCaster()->ToPlayer())
            {
                if (player->HasAura(45529))
                    player->RemoveAura(45529);

                // script prioritizes the first rune to be taken out of cooldown
                for (uint8 i = 0; i < 2; ++i)
                    if (player->GetCurrentRune(i) == RUNE_BLOOD && player->GetRuneCooldown(i) != 0)
                    {
        ...
        void Register() override
        {
            OnCast += SpellCastFn(spell_dk_blood_tap_SpellScript::HandleEffectApply);
        }
    };
...
class spell_dk_army_of_the_dead final : public SpellScript
{
    ...
    {
        uint32 ghoulEntry = 0;
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(GetSpellInfo()->Effects[EFFECT_0].TriggerSpell))
            ghoulEntry = spellInfo->Effects[EFFECT_1].MiscValue;
        ...
        for (Unit* controlled : caster->m_Controlled)
            if (controlled->IsCreature() && controlled->GetEntry() == ghoulEntry)
                controlled->ToCreature()->DespawnOrUnsummon();
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_dk_army_of_the_dead::HandleScriptEffect, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
void AddSC_deathknight_spell_scripts()
{
    ...
    RegisterAuraScript(spell_dk_dancing_rune_weapon_visual);
