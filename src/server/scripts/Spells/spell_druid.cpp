...
public:
    spell_dru_t10_balance_4p_bonus()
        : SpellScriptLoader("spell_dru_t10_balance_4p_bonus")
    {
    }
...
{
public:
    spell_dru_feral_swiftness()
        : SpellScriptLoader("spell_dru_feral_swiftness")
    {
    }
...
public:
    spell_dru_omen_of_clarity()
        : SpellScriptLoader("spell_dru_omen_of_clarity")
    {
    }
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                if (!foundValidEffect && effectInfo.Effect == SPELL_EFFECT_APPLY_AURA)
                {
                    ...
                    if (auraName != SPELL_AURA_PROC_TRIGGER_SPELL && auraName != SPELL_AURA_PROC_TRIGGER_SPELL_WITH_VALUE)
...
{
public:
    spell_dru_brambles_treant()
        : SpellScriptLoader("spell_dru_brambles_treant")
    {
    }
...
public:
    spell_dru_barkskin()
        : SpellScriptLoader("spell_dru_barkskin")
    {
    }
...
public:
    spell_dru_treant_scaling()
        : SpellScriptLoader("spell_dru_treant_scaling")
    {
    }
...
public:
    spell_dru_dash()
        : SpellScriptLoader("spell_dru_dash")
    {
    }
...
public:
    spell_dru_enrage()
        : SpellScriptLoader("spell_dru_enrage")
    {
    }
...
public:
    spell_dru_idol_lifebloom()
        : SpellScriptLoader("spell_dru_idol_lifebloom")
    {
    }
...
public:
    spell_dru_insect_swarm()
        : SpellScriptLoader("spell_dru_insect_swarm")
    {
    }
...
public:
    spell_dru_lifebloom()
        : SpellScriptLoader("spell_dru_lifebloom")
    {
    }
    ...
    class spell_dru_lifebloom_AuraScript : public AuraScript
    {
        ...
        void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (GetTarget()->HasUnitState(UNIT_STATE_ISOLATED))
                return;
        ...
        void HandleDispel(DispelInfo* dispelInfo)
        {
            if (Unit* target = GetUnitOwner())
            {
                ...
                {
                    Unit* caster               = GetCaster();
                    int32 healAmount           = GetSpellInfo()->Effects[EFFECT_1].CalcValue(caster ? caster : target, nullptr, target) * dispelInfo->GetRemovedCharges();
                    const SpellInfo* finalHeal = sSpellMgr->GetSpellInfo(SPELL_DRUID_LIFEBLOOM_FINAL_HEAL);
                    if (caster)
                    {
                        // healing with bonus
...
{
public:
    spell_dru_living_seed()
        : SpellScriptLoader("spell_dru_living_seed")
    {
    }
...
{
public:
    spell_dru_living_seed_proc()
        : SpellScriptLoader("spell_dru_living_seed_proc")
    {
    }
...
public:
    spell_dru_owlkin_frenzy()
        : SpellScriptLoader("spell_dru_owlkin_frenzy")
    {
    }
...
public:
    spell_dru_predatory_strikes()
        : SpellScriptLoader("spell_dru_predatory_strikes")
    {
    }
...
public:
    spell_dru_rip()
        : SpellScriptLoader("spell_dru_rip")
    {
    }
...
public:
    spell_dru_savage_roar()
        : SpellScriptLoader("spell_dru_savage_roar")
    {
    }
...
public:
    spell_dru_survival_instincts()
        : SpellScriptLoader("spell_dru_survival_instincts")
    {
    }
...
{
public:
    spell_dru_swift_flight_passive()
        : SpellScriptLoader("spell_dru_swift_flight_passive")
    {
    }
...
public:
    spell_dru_tiger_s_fury()
        : SpellScriptLoader("spell_dru_tiger_s_fury")
    {
    }
...
public:
    spell_dru_typhoon()
        : SpellScriptLoader("spell_dru_typhoon")
    {
    }
...
public:
    spell_dru_wild_growth()
        : SpellScriptLoader("spell_dru_wild_growth")
    {
    }
...
{
public:
    spell_dru_wolfshead_helm_energy_dummy()
        : SpellScriptLoader("spell_dru_wolfshead_helm_energy_dummy")
    {
    }
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (actor->GetTypeId() == TYPEID_PLAYER && !actor->ToPlayer()->HasSpellCooldown(SPELL_LEADER_OF_THE_PACK_HEAL))
        {
            if (aurEff->GetCasterGUID() == actor->GetGUID())
            {
                ...
                actor->CastCustomSpell(actor, SPELL_LEADER_OF_THE_PACK_MANA, &basepoints1, nullptr, nullptr, true, nullptr, aurEff);
            }
            int32 basepoints0 = int32(actor->CountPctFromMaxHealth(triggerAmount));
            actor->CastCustomSpell(actor, SPELL_LEADER_OF_THE_PACK_HEAL, &basepoints0, nullptr, nullptr, true);
...
class spell_dru_cat_form : public AuraScript
{
    ...
    void HandleOnEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* owner = GetUnitOwner())
            for (AuraType auraApplyTypes : { SPELL_AURA_TRACK_RESOURCES, SPELL_AURA_TRACK_CREATURES })
                for (AuraEffect* aurEff : owner->GetAuraEffectsByType(auraApplyTypes))
                    _trackingSpells.emplace_back(aurEff->GetSpellInfo()->Id);
    }

    void HandleOnEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* owner = GetUnitOwner())
            for (uint32 spellId : _trackingSpells)
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_dru_cat_form::HandleOnEffectApply, EFFECT_0, SPELL_AURA_MOD_SHAPESHIFT, AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(spell_dru_cat_form::HandleOnEffectRemove, EFFECT_0, SPELL_AURA_MOD_SHAPESHIFT, AURA_EFFECT_HANDLE_REAL);
    }

    private:
        std::vector<uint32> _trackingSpells;
};

// 21178 - Bear Form (Passive2) (Passive)
class spell_dru_bear_form_passive2 : public AuraScript
{
    PrepareAuraScript(spell_dru_bear_form_passive2);
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_dru_bear_form_passive2::AfterApply, EFFECT_0, SPELL_AURA_MOD_THREAT, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
};
...
void AddSC_druid_spell_scripts()
{
    ...
    RegisterAuraScript(spell_dru_bear_form_passive2);
