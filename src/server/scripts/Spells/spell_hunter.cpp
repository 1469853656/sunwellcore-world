...
enum HunterSpells
{
    ...
    SPELL_HUNTER_FREEZING_TRAP_AURA                 = 14311,
    SPELL_HUNTER_GLYPH_OF_ASPECT_OF_THE_VIPER       = 56851,
    SPELL_HUNTER_GLYPH_OF_FREEZING_TRAP             = 56845,
    SPELL_HUNTER_GLYPH_OF_FREEZING_TRAP_EFFECT      = 61394,
...
{
public:
    spell_hun_check_pet_los()
        : SpellScriptLoader("spell_hun_check_pet_los")
    {
    }
...
public:
    spell_hun_cower()
        : SpellScriptLoader("spell_hun_cower")
    {
    }
...
public:
    spell_hun_wyvern_sting()
        : SpellScriptLoader("spell_hun_wyvern_sting")
    {
    }
...
public:
    spell_hun_animal_handler()
        : SpellScriptLoader("spell_hun_animal_handler")
    {
    }
...
public:
    spell_hun_aspect_of_the_beast()
        : SpellScriptLoader("spell_hun_aspect_of_the_beast")
    {
    }
...
public:
    spell_hun_ascpect_of_the_viper()
        : SpellScriptLoader("spell_hun_ascpect_of_the_viper")
    {
    }
    ...
    class spell_hun_ascpect_of_the_viper_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_ASPECT_OF_THE_VIPER_ENERGIZE) || !sSpellMgr->GetSpellInfo(SPELL_HUNTER_GLYPH_OF_ASPECT_OF_THE_VIPER) || !sSpellMgr->GetSpellInfo(SPELL_HUNTER_VIPER_ATTACK_SPEED) || !sSpellMgr->GetSpellInfo(SPELL_HUNTER_VICIOUS_VIPER))
                return false;
            return true;
        }
...
public:
    spell_hun_chimera_shot()
        : SpellScriptLoader("spell_hun_chimera_shot")
    {
    }
    ...
    class spell_hun_chimera_shot_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            ...
            if (Unit* unitTarget = GetHitUnit())
            {
                ...
                if (spellId)
                    caster->CastCustomSpell(unitTarget, spellId, &basePoint, nullptr, nullptr, true);
            }
        }
...
//14311 - Freezing Trap
//56845 - Freezing Trap Glyph
//61394 - Freezing Trap Glyph Effect
class spell_hun_freezing_trap_aura : public SpellScriptLoader
{
public:
    spell_hun_freezing_trap_aura()
        : SpellScriptLoader("spell_hun_freezing_trap_aura")
    {
    }

    class spell_hun_freezing_trap_aura_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hun_freezing_trap_aura_AuraScript);

        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_FREEZING_TRAP_AURA))
                return false;
            return true;
        }

        void GlyphOfFreezingGlyphApply(AuraEffect const* aurEff, AuraEffectHandleModes mode)
        {
            Unit* caster = GetCaster();
            ...
            if (caster && target && caster->HasAura(SPELL_HUNTER_GLYPH_OF_FREEZING_TRAP))
                ...
                target->CastSpell(target, SPELL_HUNTER_GLYPH_OF_FREEZING_TRAP_EFFECT, true, nullptr, aurEff);
        }

        void Register() override
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_hun_freezing_trap_aura_AuraScript::GlyphOfFreezingGlyphApply, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_hun_freezing_trap_aura_AuraScript();
    }
};
...
public:
    spell_hun_improved_mend_pet()
        : SpellScriptLoader("spell_hun_improved_mend_pet")
    {
    }
    ...
    class spell_hun_readiness_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            for (PlayerSpellMap::const_iterator itr = spellMap.begin(); itr != spellMap.end(); ++itr)
            {
                ...
                if (spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && spellInfo->Id != SPELL_HUNTER_READINESS && spellInfo->Id != SPELL_HUNTER_BESTIAL_WRATH && spellInfo->Id != SPELL_DRAENEI_GIFT_OF_THE_NAARU && spellInfo->GetRecoveryTime() > 0)
                {
    ...
    class spell_hun_sniper_training_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* aurEff)
        {
            ...
            if (aurEff->GetAmount() <= 0)
            {
                ...
                if (Unit* target = GetTarget())
                {
                    ...
                    triggerCaster->CastSpell(target, triggeredSpellInfo, true, nullptr, aurEff);
                }
            }
        }
        ...
        void HandleUpdatePeriodic(AuraEffect* aurEff)
        {
            if (Player* playerTarget = GetUnitOwner()->ToPlayer())
            {
                ...
                int32 amount     = playerTarget->isMoving() || aurEff->GetAmount() <= 0 ? playerTarget->CalculateSpellDamage(playerTarget, GetSpellInfo(), aurEff->GetEffIndex(), &baseAmount) : aurEff->GetAmount() - 1;
...
public:
    spell_hun_misdirection()
        : SpellScriptLoader("spell_hun_misdirection")
    {
    }
...
public:
    spell_hun_misdirection_proc()
        : SpellScriptLoader("spell_hun_misdirection_proc")
    {
    }
...
class spell_hun_disengage : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        ...
        auto HasApplyAuraWithMechanicGrip = [caster]() {
            for (auto&& pair : caster->GetAppliedAuras())
            {
                SpellInfo const* spellInfo = pair.second->GetBase()->GetSpellInfo();

                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    if (pair.second->HasEffect(i) && spellInfo->Effects[i].Effect && spellInfo->Effects[i].Mechanic && spellInfo->Effects[i].ApplyAuraName == SPELL_EFFECT_APPLY_AURA)
                        if (MECHANIC_GRIP & (1 << spellInfo->Effects[i].Mechanic))
                            return true;
            }
        ...
        if (HasApplyAuraWithMechanicGrip())
            return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;

        if (MotionMaster* master = caster->GetMotionMaster())
            if (master->GetCurrentMovementGeneratorType() == EFFECT_MOTION_TYPE)
                caster->StopMoving();
...
public:
    spell_hun_viper_attack_speed()
        : SpellScriptLoader("spell_hun_viper_attack_speed")
    {
    }
...
public:
    spell_hun_volley()
        : SpellScriptLoader("spell_hun_volley")
    {
    }

    class spell_hun_volley_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_hun_volley_SpellScript);
        ...
        void Register() override
        {
            BeforeHit += SpellHitFn(spell_hun_volley_SpellScript::OnCast);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_hun_volley_SpellScript();
    }
};
...
public:
    spell_hun_glyph_of_arcane_shot()
        : SpellScriptLoader("spell_hun_glyph_of_arcane_shot")
    {
    }
...
class spell_hun_steady_shot : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetExplTargetUnit())
            if (target->IsDazed())
                damage += GetSpellInfo()->Effects[EFFECT_1].CalcValue();
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_hun_steady_shot::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
void AddSC_hunter_spell_scripts()
{
    ...
    new spell_hun_freezing_trap_aura();
