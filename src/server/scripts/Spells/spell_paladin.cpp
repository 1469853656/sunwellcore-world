...
{
public:
    spell_pal_seal_of_command()
        : SpellScriptLoader("spell_pal_seal_of_command")
    {
    }
    ...
    class spell_pal_seal_of_command_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (const SpellInfo* procSpell = eventInfo.GetDamageInfo()->GetSpellInfo())
                if (procSpell->SpellIconID == 3025 || procSpell->Id == 20467) // Righteous Vengeance, should not proc SoC
                    return false;
...
public:
    spell_pal_divine_intervention()
        : SpellScriptLoader("spell_pal_divine_intervention")
    {
    }
    ...
    class spell_pal_divine_intervention_AuraScript : public AuraScript
    {
        ...
        void HandleRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (!GetTarget()->IsInCombat() && GetTarget()->GetTypeId() == TYPEID_PLAYER)
                GetTarget()->RemoveAurasDueToSpell(GetTarget()->ToPlayer()->GetTeam(CF_DENY) == TEAM_ALLIANCE ? 57723 : 57724);
        }
...
{
public:
    spell_pal_seal_of_light()
        : SpellScriptLoader("spell_pal_seal_of_light")
    {
    }
...
{
public:
    spell_pal_sacred_shield_base()
        : SpellScriptLoader("spell_pal_sacred_shield_base")
    {
    }
    ...
    class spell_pal_sacred_shield_base_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (eventInfo.GetTypeMask() & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS)
            {
                ...
                if (caster && procSpell->SpellFamilyName == SPELLFAMILY_PALADIN && procSpell->SpellFamilyFlags.HasFlag(0x40000000) && caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_PALADIN, 3021, 0)) // need infusion of light
                {
            ...
            if (Aura* aura = GetAura())
            {
                if (aura->IsSpellStolen())
                    eventInfo.GetActionTarget()->CastCustomSpell(eventInfo.GetActionTarget(), triggered_spell_id, &basepoints, nullptr, nullptr, true, nullptr);
                else
                    eventInfo.GetActionTarget()->CastCustomSpell(eventInfo.GetActionTarget(), triggered_spell_id, &basepoints, nullptr, nullptr, true, nullptr, aurEff, aurEff->GetCasterGUID());
            }
        }
    ...
    class spell_pal_ardent_defender_AuraScript : public AuraScript
    {
        ...
        void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
        {
            ...
            if (remainingHealth <= 0 && !victim->ToPlayer()->HasSpellCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL))
            {
                ...
                if (absorbAmount)
                    victim->ToPlayer()->AddSpellCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL, 0, 2 * MINUTE * IN_MILLISECONDS);
            }
...
public:
    spell_pal_avenging_wrath()
        : SpellScriptLoader("spell_pal_avenging_wrath")
    {
    }
    ...
    class spell_pal_blessing_of_sanctuary_AuraScript : public AuraScript
    {
        ...
        void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                // xinef: hack
                int32 value = 9;
                caster->CastCustomSpell(target, SPELL_PALADIN_BLESSING_OF_SANCTUARY_BUFF, &value, &value, nullptr, true);
            }
        }
...
{
public:
    spell_pal_divine_sacrifice()
        : SpellScriptLoader("spell_pal_divine_sacrifice")
    {
    }
...
public:
    spell_pal_divine_storm()
        : SpellScriptLoader("spell_pal_divine_storm")
    {
    }
...
public:
    spell_pal_divine_storm_dummy()
        : SpellScriptLoader("spell_pal_divine_storm_dummy")
    {
    }
...
public:
    spell_pal_exorcism_and_holy_wrath_damage()
        : SpellScriptLoader("spell_pal_exorcism_and_holy_wrath_damage")
    {
    }
...
public:
    spell_pal_eye_for_an_eye()
        : SpellScriptLoader("spell_pal_eye_for_an_eye")
    {
    }
...
public:
    spell_pal_hand_of_salvation()
        : SpellScriptLoader("spell_pal_hand_of_salvation")
    {
    }
...
public:
    spell_pal_item_healing_discount()
        : SpellScriptLoader("spell_pal_item_healing_discount")
    {
    }
...
{
public:
    spell_pal_judgement(char const* scriptName, uint32 spellId)
    ...
    public:
        spell_pal_judgement_SpellScript(uint32 spellId)
            : SpellScript()
            , _spellId(spellId)
        {
        }
...
public:
    spell_pal_righteous_defense()
        : SpellScriptLoader("spell_pal_righteous_defense")
    {
    }
