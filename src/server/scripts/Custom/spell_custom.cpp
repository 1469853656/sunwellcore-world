...
{
public:
    spell_cust_black_qiraji_battle_tank()
        : SpellScriptLoader("spell_cust_black_qiraji_battle_tank")
    {
    }
...
struct LearnSpellsOnLevelUp : public PlayerScript
{
    ...
    void OnLevelChanged(Player* player, uint8 oldLevel) override
    {
        if (oldLevel < player->getLevel())
            sCustomMgr->LearnSpellsForNewLevel(player, oldLevel);
    }
};
