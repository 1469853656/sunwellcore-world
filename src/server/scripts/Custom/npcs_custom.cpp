...
public:
    npc_hunger_games()
        : CreatureScript("npc_hunger_games")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        switch (action)
        {
            case ACTION_ADD_TO_QUEUE:
                ...
                ChatHandler(player->GetSession()).SendSysMessage("You have been added to Hunger Games queue!");
                player->CLOSE_GOSSIP_MENU();
                break;
            case ACTION_LEAVE_QUEUE:
                sCustomEventMgr->removePlayerFromHungerGames(player->GetGUID());
                ChatHandler(player->GetSession()).SendSysMessage("You have been removed from Hunger Games queue!");
                player->CLOSE_GOSSIP_MENU();
                break;
            case ACTION_QUIT:
                player->CLOSE_GOSSIP_MENU();
            ...
            case ACTION_ADD_TO_GMLIST:
                ...
                ChatHandler(player->GetSession()).SendSysMessage("Zostales dodany do listy GMow monitorujacych event");
                player->CLOSE_GOSSIP_MENU();
                break;
            case ACTION_REMOVE_FROM_GMLIST:
                sCustomEventMgr->removeGmFromHungerGames(player->GetGUID());
                ChatHandler(player->GetSession()).SendSysMessage("Juz nie monitoruejsz eventu. Piootrek jest zawiedziony twoja postawa :(");
                player->CLOSE_GOSSIP_MENU();
                break;
        }
        return true;
...
{
public:
    npc_hunger_games_control()
        : CreatureScript("npc_hunger_games_control")
    {
    }
    ...
    {
        npc_hunger_games_controlAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
public:
    player_script_hunger_games()
        : PlayerScript("player_script_hunger_games")
    {
    }
...
{
public:
    npc_gmisl_teleporter()
        : CreatureScript("npc_gmisl_teleporter")
    {
    }
...
public:
    npc_test_server()
        : CreatureScript("npc_test_server")
    {
    }
...
enum ArenaSpectatorData
{
    ...
    ARENAS_PER_PAGE       = 25,
    ARENAS_PER_PAGE_SOLOQ = 5,
};

struct SoloQSpectateInfo
{
    std::stringstream ss;
    std::list<uint32> ratings;
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_SPECTATE_ARENA:
            {
                uint64 playerGuid = sender;
            ...
            case GOSSIP_ACTION_3V3: 
            {
                type = ARENA_TYPE_3v3;
                break;
            }
            case GOSSIP_ACTION_2V2: 
            {
                type = ARENA_TYPE_2v2;
                break;
            }
            case GOSSIP_ACTION_SOLOQ: 
            {
                type = ARENA_TYPE_SOLO_QUEUE;
                break;
            }
        }
        ...
        uint32 size;
        page = sender - 1;
        bool arenasFound = DisplayArenas(player, type, page, size);
        if (arenasFound)
        {
            if (page != 0)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Show previous page", sender - 1, action);
            if (size > static_cast<uint32>((page + 1) * ARENAS_PER_PAGE_SOLOQ))
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Show next page", sender + 1, action);
    ...
    {
        std::vector<Battleground*> arenaContainer;
        ...
        if (sBattlegroundMgr->GetArenaListByType(arenaContainer, type))
        {
            ...
            for (uint32 i = begin; i <= end; ++i)
            {
                if (i >= size)
                    break;
                Battleground* arena = arenaContainer[i];
                ...
                for (auto& arenaPlayer : arena->GetPlayers())
                {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, os.str().c_str(), playerGuid, GOSSIP_ACTION_SPECTATE_ARENA);
            }
            return true;
        }
        return false;
    ...
    void DisplayHelloGossips(Player* player)
    {
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "3v3 SoloQueue", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_SOLOQ);
    }
};
...
{
public:
    npc_arena_spectator_leave()
        : CreatureScript("npc_arena_spectator_leave")
    {
    }
    bool OnGossipHello(Player* player, Creature* creature) override
    {
    ...
    {
        npc_arena_spectator_leaveAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
    npc_custom_guild_bank_companion_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
public:
    ...
    {
        npc_wandererAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
public:
    npc_wanderer_dalaranNPC()
        : CreatureScript("npc_wanderer_dalaranNPC")
    {
    }
    struct npc_wanderer_dalaranNPCAI : public SmartAI
    {
        npc_wanderer_dalaranNPCAI(Creature* creature)
            : SmartAI(creature)
        {
        }
...
{
public:
    npc_wanderer_skeleton()
        : CreatureScript("npc_wanderer_skeleton")
    {
    }
    struct npc_wanderer_skeletonAI : public ScriptedAI
    {
        npc_wanderer_skeletonAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
constexpr uint32 ITEMS_IN_MAIL{ 12 };

constexpr std::array<uint32, 12> SPEC_MASKS    = { 0, BOOST_MDPS | BOOST_TANK, BOOST_MDPS | BOOST_HEAL | BOOST_TANK, BOOST_RDPS, BOOST_MDPS, BOOST_RDPS | BOOST_HEAL, 0 /* dk? */, BOOST_MDPS | BOOST_RDPS | BOOST_HEAL, BOOST_RDPS, BOOST_RDPS, 0, BOOST_MDPS | BOOST_RDPS | BOOST_HEAL };
std::array<WorldLocation, 2> teleportLocations = { WorldLocation(0, -8863.55f, 640.36f, 96.07f), WorldLocation(1, 1612.26f, -4396.45f, 10.51f) };
struct npc_character_boost : public ScriptedAI
{
    npc_character_boost(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    {
        if (!CanUseBoost(player))
            return;

        uint32 specMask = SPEC_MASKS[player->getClass()];
        if (specMask & BOOST_MDPS)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Melee DPS", GOSSIP_SENDER_MAIN, 0);
        if (specMask & BOOST_RDPS)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Ranged DPS", GOSSIP_SENDER_MAIN, 1);
        if (specMask & BOOST_HEAL)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Healer", GOSSIP_SENDER_MAIN, 2);
        if (specMask & BOOST_TANK)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tank", GOSSIP_SENDER_MAIN, 3);
        player->SEND_GOSSIP_MENU(3600004, me->GetGUID());
    }
    ...
    void sGossipSelect(Player* player, uint32 menuId, uint32 actionId) override
    {
        ...
        switch (sender)
        {
            case GOSSIP_SENDER_MAIN: {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Yes", GOSSIP_SENDER_MAIN + 1, action);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "No", GOSSIP_SENDER_MAIN + 2, 0);

                player->SEND_GOSSIP_MENU(360000 * 10 + action, me->GetGUID());
                return;
            }
        ...
        BoostCharacter(player, action);
        player->PlayerTalkClass->SendCloseGossip();
    }

    void BoostCharacter(Player* player, uint32 spec)
    {
        if ((SPEC_MASKS[player->getClass()] & ( 1 << spec)) == 0)
            return;
        ...
        // 2. Send items via mail
        SendBoostItems(player, spec);

        // 3. Learn riding
        player->CastSpell(player, 33389, true); // Apprentice Riding
        player->CastSpell(player, 33392, true); // Journeyman Riding
        if (player->getClass() == CLASS_DRUID)
            player->CastSpell(player, 33950, true); // Teaches flight form and expert riding
        else
            player->CastSpell(player, 34092, true); // Teaches just expert riding

        // 4. Learn required skills
        LearnRequiredSkills(player);

        // 5. Update skills
        player->UpdateSkillsToMaxSkillsForLevel();

        // 6. Teleport to Orgrimmar/Stormwind
        player->TeleportTo(teleportLocations[player->GetTeam(CF_DENY)]);

        // 7. Flag account as already boosted
        player->GetSession()->AddBoostedCharacterFlag();

        // 8. Log
        sLog->outRewards("Player %s (guid: %u, account: %u ip: %s) used character boost. Specialization: %u", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId(), player->GetSession()->GetRemoteAddress().c_str(), spec);
    }

    void SendBoostItems(Player* player, uint32 spec)
    {
        std::vector<uint32> boostItems = sWorldCache.GetBoostItemsForClassAndSpec(player->getClass(), spec);
        if (boostItems.empty())
            return;

        // Add 4x Netherweave Bag
        for (uint32 i = 0; i < 4; ++i)
            boostItems.push_back(21841);

        // Add mounts
        if (player->GetTeam(CF_DENY) == TEAM_ALLIANCE)
        {
            boostItems.push_back(18776);
            boostItems.push_back(25470);
        }
        else
        {
            boostItems.push_back(18796);
            boostItems.push_back(25474);
        }
        ...
        {
            itemsToSend.push_back(item);
            if (itemsToSend.size() >= ITEMS_IN_MAIL)
                SendItems(itemsToSend, player);
        }
        if (!itemsToSend.empty())
            SendItems(itemsToSend, player);
    }

    void SendItems(std::vector<uint32>& items, Player* player)
    {
        /*MailSender sender(MAIL_NORMAL, 0, MAIL_STATIONERY_GM);
        // fill mail
        std::ostringstream ss;
        ss << "Hello " << player->GetName() <<"\nSunwell Team would like to thank you for taking part in our event. You may find the chosen equipment below. Have a nice time and enjoy your journey through the cold steppes of Northrend!";
        MailDraft draft("Character Boost", ss.str());

        auto trans = CharacterDatabase.BeginTransaction();

        for (auto&& i : items)
        {
            if (Item* item = Item::CreateItem(i, 1, player))
            {
        ...
        draft.SendMailTo(trans, MailReceiver(player, player->GetGUIDLow()), sender, MAIL_CHECK_MASK_RETURNED);
        CharacterDatabase.CommitTransaction(trans);
        items.clear();*/
    }

    void LearnRequiredSkills(Player* player)
    {
        ...
        switch (_class)
        {
            ...
            case CLASS_WARRIOR:
            case CLASS_PALADIN: {
                uint8 _playerRace = player->getRace();
                if (_class == CLASS_PALADIN && _playerRace != RACE_HUMAN && _playerRace != RACE_DRAENEI)
                    player->learnSpell(CHECK_SKILL_TWO_HAND_MACE); // two-handed mace

                if (_class == CLASS_WARRIOR)
                {
                    player->learnSpell(CHECK_SKILL_BOWS);
                    player->learnSpell(CHECK_SKILL_GUNS);
                }
    ...
    bool CanUseBoost(Player const* player)
    {
        if (!sWorld->getBoolConfig(CONFIG_CHARACTER_BOOST_ENABLE))
            return false;
        if (player->GetSession()->HasBoostedCharacter())
            return false;
        if (player->getClass() == CLASS_DEATH_KNIGHT)
            return false;
    ...
    {
        return CanUseBoost(player) || player->IsGameMaster();
    }
};
...
static const std::map<uint32, RacialShiftRaceData> RacialShiftData{
    { RACE_HUMAN, { RACIAL_CHOOSE_RACE_HUMAN, +"|TInterface/ICONS/Achievement_character_human_male:30:30:-18:0|t Will of the Human.", +"|TInterface/ICONS/Achievement_character_human_female:30:30:-18:0|t Will of the Human.", CLASSMASK_WARRIOR + CLASSMASK_PALADIN + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_MAGE + CLASSMASK_WARLOCK + CLASSMASK_DEATH_KNIGHT } },
    { RACE_ORC, { RACIAL_CHOOSE_RACE_ORC, +"|TInterface/ICONS/Achievement_character_orc_male:30:30:-18:0|t Bravery of the Orc.", +"|TInterface/ICONS/Achievement_character_orc_female:30:30:-18:0|t Bravery of the Orc.", CLASSMASK_WARRIOR + CLASSMASK_HUNTER + CLASSMASK_ROGUE + CLASSMASK_SHAMAN + CLASSMASK_WARLOCK + CLASSMASK_DEATH_KNIGHT } },
    { RACE_DWARF, { RACIAL_CHOOSE_RACE_DWARF, +"|TInterface/ICONS/Achievement_character_dwarf_male:30:30:-18:0|t Path of the Dwarf.", +"|TInterface/ICONS/Achievement_character_dwarf_female:30:30:-18:0|t Path of the Dwarf.", CLASSMASK_WARRIOR + CLASSMASK_PALADIN + CLASSMASK_HUNTER + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_DEATH_KNIGHT + CLASSMASK_SHAMAN } },
    { RACE_NIGHTELF, { RACIAL_CHOOSE_RACE_NIGHTELF, +"|TInterface/ICONS/Achievement_character_nightelf_male:30:30:-18:0|t Way of the Night Elf.", +"|TInterface/ICONS/Achievement_character_nightelf_female:30:30:-18:0|t Way of the Night Elf.", CLASSMASK_WARRIOR + CLASSMASK_HUNTER + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_DRUID + CLASSMASK_DEATH_KNIGHT } },
    { RACE_UNDEAD_PLAYER, { RACIAL_CHOOSE_RACE_UNDEAD, +"|TInterface/ICONS/Achievement_character_undead_male:30:30:-18:0|t Shadow of the Undead.", +"|TInterface/ICONS/Achievement_character_undead_female:30:30:-18:0|t Shadow of the Undead.", CLASSMASK_WARRIOR + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_MAGE + CLASSMASK_WARLOCK + CLASSMASK_DEATH_KNIGHT + CLASSMASK_PALADIN } },
    { RACE_TAUREN, { RACIAL_CHOOSE_RACE_TAUREN, +"|TInterface/ICONS/Achievement_character_tauren_male:30:30:-18:0|t Nature of the Tauren.", +"|TInterface/ICONS/Achievement_character_tauren_female:30:30:-18:0|t Nature of the Tauren.", CLASSMASK_WARRIOR + CLASSMASK_HUNTER + CLASSMASK_SHAMAN + CLASSMASK_DRUID + CLASSMASK_DEATH_KNIGHT } },
    { RACE_GNOME, { RACIAL_CHOOSE_RACE_GNOME, +"|TInterface/ICONS/Achievement_character_gnome_male:30:30:-18:0|t Mind of the Gnome.", +"|TInterface/ICONS/Achievement_character_gnome_female:30:30:-18:0|t Mind of the Gnome.", CLASSMASK_WARRIOR + CLASSMASK_ROGUE + CLASSMASK_MAGE + CLASSMASK_WARLOCK + CLASSMASK_DEATH_KNIGHT } },
    { RACE_TROLL, { RACIAL_CHOOSE_RACE_TROLL, +"|TInterface/ICONS/Achievement_character_troll_male:30:30:-18:0|t Voodoo of the Troll.", +"|TInterface/ICONS/Achievement_character_troll_female:30:30:-18:0|t Voodoo of the Troll.", CLASSMASK_WARRIOR + CLASSMASK_HUNTER + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_SHAMAN + CLASSMASK_MAGE + CLASSMASK_DEATH_KNIGHT } },
    { RACE_BLOODELF, { RACIAL_CHOOSE_RACE_BLOODELF, +"|TInterface/ICONS/Achievement_character_bloodelf_male:30:30:-18:0|t Secrets of the Blood Elf.", +"|TInterface/ICONS/Achievement_character_bloodelf_female:30:30:-18:0|t Secrets of the Blood Elf.", CLASSMASK_PALADIN + CLASSMASK_HUNTER + CLASSMASK_ROGUE + CLASSMASK_PRIEST + CLASSMASK_MAGE + CLASSMASK_WARLOCK + CLASSMASK_DEATH_KNIGHT } },
    { RACE_DRAENEI, { RACIAL_CHOOSE_RACE_DRAENEI, +"|TInterface/ICONS/Achievement_character_draenei_male:30:30:-18:0|t Wisdom of the Draenei.", +"|TInterface/ICONS/Achievement_character_draenei_female:30:30:-18:0|t Wisdom of the Draenei.", CLASSMASK_WARRIOR + CLASSMASK_PALADIN + CLASSMASK_HUNTER + CLASSMASK_PRIEST + CLASSMASK_SHAMAN + CLASSMASK_MAGE + CLASSMASK_DEATH_KNIGHT } },
};

struct npc_racial_shift final : ScriptedAI
{
    explicit npc_racial_shift(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    bool CanBeSeen(Player const* player) override
    {
        ...
        if (!sCustomMgr->GetBoolConfig(CONFIG_RACIAL_SHIFT_FEATURE_ENABLED))
            return false;

        if (player->GetSession()->HasPremiumService(PremiumServices::WhitemanePlus))
            return true;

        CustomData const* customData = sCustomMgr->GetCustomData(player->GetGUIDLow());
        if (!customData)
            return false;

        return !customData->HasCharacterFlag(CustomPlayerFlags::UsedRacialShift);
    }

    void AddGossipForRace(Player* player, uint8 const race) const
    {
        auto const currentRacial = sCustomMgr->GetCurrentRacialsForPlayer(player);
        if (race == currentRacial)
            return;

        uint32 const racialAllianceMask = sCustomMgr->GetIntConfig(CONFIG_RACIAL_SHIFT_ENABLED_ALLIANCE);
        uint32 const racialHordeMask    = sCustomMgr->GetIntConfig(CONFIG_RACIAL_SHIFT_ENABLED_HORDE);
        TeamId const playerTeamID       = player->GetTeam(CF_DENY);
    ...
    void sGossipHello(Player* player) override
    {
        if (!sCustomMgr->GetBoolConfig(CONFIG_RACIAL_SHIFT_FEATURE_ENABLED) && !player->IsGameMaster())
        {
    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        if (sender == GOSSIP_SENDER_INFO)
        {
            ...
            sCustomMgr->ApplyRacialShift(player, static_cast<Races>(action));
...
inline void LearnQuestsSpell(Player* player)
{
    if (!player)
        return;
...
struct npc_level_up final : ScriptedAI
{
    npc_level_up(Creature* creature) : ScriptedAI(creature)
    {
    }
    ...
    void sGossipHello(Player* player) override
    {
        if (player->getLevel() < 80)
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Level Up!", GOSSIP_SENDER_MAIN, 1);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Reset IDs!", GOSSIP_SENDER_MAIN, 2);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Reset cooldowns!", GOSSIP_SENDER_MAIN, 3);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Add 10000 gold!", GOSSIP_SENDER_MAIN, 4);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Learn armor!", GOSSIP_SENDER_MAIN, 5);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Learn riding!", GOSSIP_SENDER_MAIN, 6);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Reset talents!", GOSSIP_SENDER_MAIN, 7);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Max Weapon Skills!", GOSSIP_SENDER_MAIN, 8);

        if (player->getClass() == CLASS_WARRIOR || player->getClass() == CLASS_WARLOCK || player->getClass() == CLASS_PALADIN || player->getClass() == CLASS_HUNTER || player->getClass() == CLASS_SHAMAN || player->getClass() == CLASS_DRUID)
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Learn Class spells from Quests!", GOSSIP_SENDER_MAIN, 9);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Learn Dual Talent!", GOSSIP_SENDER_MAIN, 10);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Boost Northrend Reputation!", GOSSIP_SENDER_MAIN, 11);

        if (player->getLevel() < 58)
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Level Up (60) to Outland!", GOSSIP_SENDER_MAIN, 12);

        if (player->getLevel() < 68)
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Level Up (70) to Northrend!", GOSSIP_SENDER_MAIN, 13);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Boost Outland Reputation!", GOSSIP_SENDER_MAIN, 14);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Level Up (80)!", GOSSIP_SENDER_MAIN, 18);

      //  AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Add Profession Token!", GOSSIP_SENDER_MAIN, 19);

        SendGossipMenuFor(player, 60000, me);
    }
    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        switch (action)
        {
            ...
            case 10:
                ...
                break;
            case 11:
                for (const uint32 repID : { 1156, 1090, 1106, 1119, 1098, 1091 })
                {
                    if (FactionEntry const* faction = sFactionStore.LookupEntry(repID))
                    {
                        player->GetReputationMgr().SetOneFactionReputation(faction, 50000, false);
                        if (FactionState const* state = player->GetReputationMgr().GetState(faction))
            ...
            case 14:
            {
                std::vector<uint32> repIds;
                if (player->GetTeam() == TEAM_HORDE)
                    repIds = { 934, 932, 935, 947, 1011 };
                else
                    repIds = { 985, 942, 1077, 946, 1011 };
                for (uint32 repID : repIds)
                {
                    if (FactionEntry const* faction = sFactionStore.LookupEntry(repID))
                    {
                        player->GetReputationMgr().SetOneFactionReputation(faction, 50000, false);
            ...
            case 15:
                player->GiveLevel(std::min(player->getLevel() + 1, 80));
                break;
            case 16:
                player->GiveLevel(std::min(player->getLevel() + 5, 80));
                break;
            case 17:
                player->GiveLevel(std::min(player->getLevel() + 10, 80));
                break;
            case 18:
                player->GiveLevel(std::min(player->getLevel() + 79, 80));
                break;
            case 19:
                player->AddItem(41606, 1);
                break;
            default:
                break;
        }
    ...
    void ResetInstanceIDs(Player* player)
    {
        for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
        {
            ...
            for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
            {
                ...
                if (itr->first != player->GetMapId())
                {
                    ...
                    ChatHandler(player->GetSession()).PSendSysMessage("unbinding map: %d, inst: %d, perm: %s, diff: %d, canReset: %s, TTR: %s%s", itr->first, save->GetInstanceId(), itr->second.perm ? "yes" : "no", save->GetDifficulty(), save->CanReset() ? "yes" : "no", timeleft.c_str(), (itr->second.extended ? " (extended)" : ""));
...
enum ItemConversion
{
    NPC_TEXT_NO_ITEMS_FOR_CONVERSION = 961000,
    NPC_TEXT_CONVERSION_DEFAULT
};

struct npc_toc_convert_items : ScriptedAI
{
    explicit npc_toc_convert_items(Creature* creature) : ScriptedAI(creature) { }

    static bool IsToCItemAllowedForConversion(uint32 const itemEntry)
    {
        return itemEntry;
    }

    static std::optional<std::pair<uint32, uint32>> GetFactionReplacementData(TeamId const teamID, uint32 const itemEntry)
    {
        if (teamID == TEAM_HORDE)
        {
            auto const it = sObjectMgr->FactionChangeItems.find(itemEntry);
            if (it != std::end(sObjectMgr->FactionChangeItems))
                return *it;
    ...
    void sGossipHello(Player* player) override
    {
        bool hasAnyItemForConversion = false;
        for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
        {
            ItemRef item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i);
            if (!item || !IsToCItemAllowedForConversion(item->GetEntry()))
                continue;

            if (!GetFactionReplacementData(player->GetTeam(CF_DENY), item->GetEntry()))
                continue;

            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, fmt::format("Convert '{}' item.", item->GetTemplate()->Name1).c_str(), item->GetEntry(), item->GetGUIDLow());
            hasAnyItemForConversion = true;
        }
        ...
        for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
        {
            ...
            {
                ItemRef item = bag->GetItemByPos(j);
                if (!item || !IsToCItemAllowedForConversion(item->GetEntry()))
                    continue;
        ...
        SendGossipMenuFor(player, hasAnyItemForConversion ? NPC_TEXT_CONVERSION_DEFAULT : NPC_TEXT_NO_ITEMS_FOR_CONVERSION, me);
    }

    void sGossipSelectSenderAction(Player* player, uint32 const sender, uint32 const action) override
    {
        CloseGossipMenuFor(player);
        auto const factionReplacementData = GetFactionReplacementData(player->GetTeam(CF_DENY), sender);
        if (!factionReplacementData)
            return;

        uint32 const newItemEntry = sender == factionReplacementData->first ? factionReplacementData->second : factionReplacementData->first;
        if (ItemRef const& item = player->GetItemByGuid(MAKE_NEW_GUID(action, 0, HIGHGUID_ITEM)))
        {
            uint32 count = 1;
            player->DestroyItemCount(item, count, true);
            player->AddItem(newItemEntry, count);
            ChatHandler(player->GetSession()).SendSysMessage("Item changed!");
        }
    }
};

struct npc_hardcore_leveling final : ScriptedAI
{
    npc_hardcore_leveling(Creature* creature) : ScriptedAI(creature)
    {
    }

    static std::set<uint32> GetAllowedItems(Player const* player)
    {
        ...
        {
            for (uint32 itemID : oEntry->ItemId)
            {
                if (itemID <= 0)
                    continue;
                ...
                allowedItems.emplace(itemID);
            }
        }

        if (PlayerInfo const* info = sObjectMgr->GetPlayerInfo(player->getRace(), player->getClass()))
            for (auto const& itemCreateInfo : info->item)
                allowedItems.emplace(itemCreateInfo.item_id);

        return allowedItems;
    }

    void CleanDisallowedItems(Player* player) const
    {
        std::set<uint32> allowedItems = GetAllowedItems(player);
        for (uint8 i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_ITEM_END; i++)
            if (ItemRef const item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
                if (allowedItems.find(item->GetEntry()) == std::end(allowedItems))
    ...
    static bool CanStartHardcorePath(Player const* player)
    {
        if (player->HasHardcoreEnabled())
        {
            ChatHandler(player->GetSession()).SendSysMessage("You already have Hardcore Mode enabled.");
            return false;
        }

        if (player->getLevel() != 1)
        {
            ChatHandler(player->GetSession()).SendSysMessage("You must be level 1 to start Hardcore Mode.");
            return false;
        }

        if (player->GetUInt32Value(PLAYER_XP))
        {
            ChatHandler(player->GetSession()).SendSysMessage("You must have 0 XP to start Hardcore Mode.");
            return false;
        }

        if (player->GetMoney())
        {
            ChatHandler(player->GetSession()).SendSysMessage("You must have 0 gold to start Hardcore Mode.");
            return false;
        }

        if (player->GetTotalPlayedTime() > 5 * MINUTE)
        {
            ChatHandler(player->GetSession()).SendSysMessage("You must have less than 5 minutes played time to start Hardcore Mode.");
            return false;
        }

        if (!player->CanBeHardcore())
        {
            ChatHandler(player->GetSession()).SendSysMessage("You cannot start Hardcore Mode.");
            return false;
        }
    ...
    void StartHardcorePath(Player* player) const
    {
        CleanDisallowedItems(player);
        player->SetHardcoreMode();
        ChatHandler(player->GetSession()).SendSysMessage("Hardcore Mode has been activated. Good luck.");
    }

    bool CanBeSeen(Player const* player) override
    {
        return true;
        //return CanStartHardcorePath(player);
    }

    void sGossipHello(Player* player) override
    {
        if (!CanStartHardcorePath(player))
        {
            SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, me);
            return;
        }

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Activate Hardcore Mode.", GOSSIP_SENDER_MAIN, 1);
        SendGossipMenuFor(player, 4200000, me);
    }
    ...
    void sGossipSelectSenderAction(Player* player, uint32 const sender, uint32 const action) override
    {
        ...
        {
            case 1:
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "I am sure. Activate Hardcore Mode.", GOSSIP_SENDER_MAIN, 2);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "I changed my mind.", GOSSIP_SENDER_MAIN, 3);
                SendGossipMenuFor(player, 4200001, me);
                break;
            case 2:
                StartHardcorePath(player);
                player->PlayerTalkClass->SendCloseGossip();
                break;
            case 3:
                player->PlayerTalkClass->SendCloseGossip();
...
struct npc_preset final : ScriptedAI
{
    npc_preset(Creature* creature) : ScriptedAI(creature) { }

    void sGossipHello(Player* player) override
    {
        sCustomMgr->SendGossipsForPremadeSets(player, me->GetGUID(), me->GetCreatureTemplate()->PresetGossipFlags);
    }
    ...
    void sGossipSelectSenderAction(Player* player, uint32 const sender, uint32 const action) override
    {
        ...
        sCustomMgr->HandlePremadeSetGossipSelect(player, action, me->GetCreatureTemplate()->PresetGossipFlags);
        CloseGossipMenuFor(player);
    }
};

struct npc_ascended final : ScriptedAI
{
    npc_ascended(Creature* creature) : ScriptedAI(creature) { }
    ...
    bool CanBeSeen(Player const* /*player*/) override
    {
        ...
        return !instance->IsEncounterInProgress();
    }
    ...
    void sGossipHello(Player* player) override
    {
        ...
        if (!sCustomMgr->IsAscendedModeAllowedMapID(instance->instance->GetId()))
        {
            player->PlayerTalkClass->SendCloseGossip();
            return;
        }
        ...
        CustomDifficulty const oldDifficulty = instance->GetCustomDifficulty();
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, oldDifficulty == CustomDifficulty::Disabled ? "Activate Ascended Mode" : "Disable Ascended Mode", GOSSIP_SENDER_MAIN, 0);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, me);
    }
    ...
    void sGossipSelectSenderAction(Player* player, uint32 const sender, uint32 const action) override
    {
        ...
        CustomDifficulty const oldDifficulty = instance->GetCustomDifficulty();
        CustomDifficulty const newDifficulty = oldDifficulty == CustomDifficulty::Ascended10 ? CustomDifficulty::Disabled : CustomDifficulty::Ascended10;

        InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(instance->instance->GetInstanceId());
        save->SetCustomDifficulty(newDifficulty);
        instance->SetCustomDifficulty(newDifficulty);

        instance->SaveToDB();
        instance->instance->DoOnAllCreatures([](Creature* creature)  { sBoostMgr->ApplyBoostsStatOnSpawn(creature); });

        const char* message = newDifficulty == CustomDifficulty::Ascended10 ? "Ascended Mode has been enabled." : "Ascended Mode has been disabled.";
        for (auto const& ref : instance->instance->GetPlayers())
        {
            Player const* playerInMap = ref.GetSource();
            if (!playerInMap)
                continue;

            ChatHandler(playerInMap->GetSession()).SendSysMessage(message);
        }
...
std::vector<uint32> BlockedLearnSpells = { 8388, 8387 };
...
static void HandleLearnSkillRecipesHelper(Player* player, uint32 skillId)
{
    ...
    for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
    {
        ...
        if (std::find(std::begin(BlockedLearnSpells), std::end(BlockedLearnSpells), skillLine->spellId) != std::end(BlockedLearnSpells))
            continue;
...
struct npc_mograine final : ScriptedAI
{
    npc_mograine(Creature* creature) : ScriptedAI(creature) { }

    bool CanBeSeen(Player const* /*player*/) override
    {
        return sCustomMgr->CheckRealmPhase(RealmPhase::REALM_PHASE_PTR);
    }

    void sGossipHello(Player* player) override
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Level 80", GOSSIP_SENDER_MAIN, 0);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Teleport to GM Island", GOSSIP_SENDER_MAIN, 1);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Reset raids", GOSSIP_SENDER_MAIN, 2);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Add 5000 gold", GOSSIP_SENDER_MAIN, 3);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Add mount", GOSSIP_SENDER_MAIN, 4);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Teleport to Naxxramas", GOSSIP_SENDER_MAIN, 5);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Teleport to Obsidian Sanctum", GOSSIP_SENDER_MAIN, 6);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Teleport to Eye of Eternity", GOSSIP_SENDER_MAIN, 7);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Professions", GOSSIP_SENDER_MAIN, 8);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, me);
    }
    ...
    void sGossipSelectSenderAction(Player* player, uint32 const sender, uint32 const action) override
    {
        if (sender == GOSSIP_SENDER_MAIN)
        {
            switch (action)
            {
                case 0:
                    ...
                    break;
                case 1:
                    player->TeleportTo(1, 16226.2f, 16257.0f, 13.202f, 1.339f);
                    break;
                case 2:
                    ResetInstanceIDs(player);
                    break;
                ...
                case 4:
                    ...
                    break;
                case 5:
                    player->TeleportTo(571, 3668.719f, -1262.460F, 243.621f, 4.785f);
                    break;
                case 6:
                    player->TeleportTo(571, 3457.005f, 262.283f, -113.770f, 3.260f);
                    break;
                case 7:
                    player->TeleportTo(571, 3861.344f, 6988.958f, 152.042f, 5.763f);
                    break;
                case 8:
                    ClearGossipMenuFor(player);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Mining", 2, 0);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Herbalism", 2, 1);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Skinning", 2, 2);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Enchanting", 2, 3);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Jewelcrafting", 2, 4);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Blacksmithing", 2, 5);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Tailoring", 2, 6);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Leatherworking", 2, 7);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Alchemy", 2, 8);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Engineering", 2, 9);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Inscription", 2, 10);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Cooking", 2, 11);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "First Aid", 2, 12);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Fishing", 2, 13);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Go Back", 2, 14);
                    SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, me);
                    return;
                default:
                    break;
        ...
        else if (sender == 2)
        {
            ...
            if (action != 11 && action != 12 && action != 13 && action != 14)
                canLearnProfession = player->GetFreePrimaryProfessionPoints() > 0;

            if (!canLearnProfession)
            {
                player->GetSession()->SendNotification("You already have 2 professions!");
                CloseGossipMenuFor(player);
                return;
            }
...
void AddSC_npcs_custom()
{
    ...
    RegisterCreatureAI(npc_character_boost);
    RegisterCreatureAI(npc_racial_shift);
    RegisterCreatureAI(npc_level_up);
    RegisterCreatureAI(npc_hardcore_leveling);
    RegisterCreatureAI(npc_preset);
    RegisterCreatureAI(npc_ascended);
    RegisterCreatureAI(npc_mograine);
}
