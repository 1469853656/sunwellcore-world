...
public:
    spell_q10929_fumping()
        : SpellScriptLoader("spell_q10929_fumping")
    {
    }
...
{
public:
    npc_greatfather_aldrimus()
        : CreatureScript("npc_greatfather_aldrimus")
    {
    }
    ...
    {
        npc_greatfather_aldrimusAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
public:
    spell_q10036_torgos()
        : SpellScriptLoader("spell_q10036_torgos")
    {
    }
...
{
public:
    spell_q10898_skywing()
        : SpellScriptLoader("spell_q10898_skywing")
    {
    }
...
public:
    npc_unkor_the_ruthless()
        : CreatureScript("npc_unkor_the_ruthless")
    {
    }
            ...
            if (player && me->HealthBelowPctDamaged(30, damage))
            {
                if (Group* group = player->GetGroup())
                {
                    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
                    {
                        ...
                        if (groupie && groupie->IsInMap(player) && groupie->GetQuestStatus(QUEST_DONTKILLTHEFATONE) == QUEST_STATUS_INCOMPLETE && groupie->GetReqKillOrCastCurrentCount(QUEST_DONTKILLTHEFATONE, 18260) == 10)
                        {
                ...
                else if (player->GetQuestStatus(QUEST_DONTKILLTHEFATONE) == QUEST_STATUS_INCOMPLETE && player->GetReqKillOrCastCurrentCount(QUEST_DONTKILLTHEFATONE, 18260) == 10)
                {
...
public:
    npc_infested_root_walker()
        : CreatureScript("npc_infested_root_walker")
    {
    }
    ...
    {
        npc_infested_root_walkerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_rotting_forest_rager()
        : CreatureScript("npc_rotting_forest_rager")
    {
    }
    ...
    {
        npc_rotting_forest_ragerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_isla_starmane()
        : CreatureScript("npc_isla_starmane")
    {
    }

    struct npc_isla_starmaneAI : public npc_escortAI
    {
        npc_isla_starmaneAI(Creature* creature)
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            switch (waypointId)
            {
                ...
                case 29:
                    ...
                    else if (player->GetTeam(CF_DENY) == TEAM_HORDE)
...
public:
    go_skull_pile()
        : GameObjectScript("go_skull_pile")
    {
    }
...
public:
    npc_cenarion_sparrowhawk()
        : CreatureScript("npc_cenarion_sparrowhawk")
    {
    }
    ...
    {
        npc_cenarion_sparrowhawkAI(Creature* creature)
            : ScriptedAI(creature)
        {
            Initialize();
