...
public:
    npc_natrualist_bite()
        : CreatureScript("npc_natrualist_bite")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        else
        {
            ...
            if (Creature* cr = creature->SummonCreature(17957, -186, -790, 43.8f, 4.2f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                cr->AI()->AttackStart(creature);
            if (Creature* cr = creature->SummonCreature(17960, -188, -783, 43.8f, 4.2f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                cr->AI()->AttackStart(player);
            if (Creature* cr = creature->SummonCreature(17957, -196, -783, 43.8f, 4.4f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                cr->AI()->AttackStart(player);
    ...
    {
        npc_natrualist_biteAI(Creature* creature)
            : ScriptedAI(creature)
        {
            _spoken = 0;
        }
        ...
        uint32 GetData(uint32) const override { return _spoken == 2; }
    };
...
public:
    npcs_ashyen_and_keleth()
        : CreatureScript("npcs_ashyen_and_keleth")
    {
    }
...
public:
    npc_cooshcoosh()
        : CreatureScript("npc_cooshcoosh")
    {
    }

    struct npc_cooshcooshAI : public ScriptedAI
    {
        npc_cooshcooshAI(Creature* creature)
            : ScriptedAI(creature)
        {
...
public:
    ...
    {
        npc_elder_kurutiAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        void Reset() override
...
public:
    npc_mortog_steamhead()
        : CreatureScript("npc_mortog_steamhead")
    {
    }
...
public:
    npc_kayra_longmane()
        : CreatureScript("npc_kayra_longmane")
    {
    }

    struct npc_kayra_longmaneAI : public npc_escortAI
    {
        npc_kayra_longmaneAI(Creature* creature)
...
public:
    npc_timothy_daniels()
        : CreatureScript("npc_timothy_daniels")
    {
    }
