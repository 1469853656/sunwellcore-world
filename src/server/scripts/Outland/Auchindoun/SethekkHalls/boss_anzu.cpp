...
struct boss_anzuAI : public BossAI
{
    ...
    void SummonedCreatureDies(Creature* summon, Unit*) override
    {
        ...
        if (summon->GetEntry() == NPC_BROOD_OF_ANZU)
            if (summons.GetEntryCount(NPC_BROOD_OF_ANZU) == 0)
...
{
    npc_brood_of_anzu_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_anzu_spiritAI(Creature* creature)
        : ScriptedAI(creature)
    {
        Initialize();
    ...
    {
        if (spell && spell->SpellFamilyName == SPELLFAMILY_DRUID && spell->SpellFamilyFlags[0] & 0x1000000050)
        {
