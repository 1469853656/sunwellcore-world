        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_INVOKER_SAY_1:
                    ...
                    {
                        plr->MonsterSay("The Ice Stone has melted!", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_INVOKER_SAY_2:
                    if (Player* plr = ObjectAccessor::GetPlayer(*me, InvokerGUID))
                        plr->MonsterSay("Ahune, your strength grows no more!", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    events.RescheduleEvent(EVENT_INVOKER_SAY_3, 2000);
                    break;
                case EVENT_INVOKER_SAY_3:
                    if (Player* plr = ObjectAccessor::GetPlayer(*me, InvokerGUID))
                        plr->MonsterSay("Your frozen reign will not come to pass!", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    break;
                case EVENT_ATTACK:
                    events.Reset();
                ...
                case EVENT_TOTEMS_ATTACK:
                    ...
                    break;
                case EVENT_SUBMERGE:
                    me->MonsterTextEmote(TEXT_RETREAT, nullptr, true);
                ...
                case EVENT_EMERGE_WARNING:
                    me->MonsterTextEmote(TEXT_RESURFACE, nullptr, true);
                    events.PopEvent();
                    break;
...
public:
    go_ahune_ice_stone()
        : GameObjectScript("go_ahune_ice_stone")
    {
    }
...
{
public:
    npc_ahune_frozen_core()
        : CreatureScript("npc_ahune_frozen_core")
    {
    }
    ...
    {
        npc_ahune_frozen_coreAI(Creature* c)
            : NullCreatureAI(c)
        {
        }
