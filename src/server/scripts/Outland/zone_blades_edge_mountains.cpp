...
{
public:
    npc_deahts_door_wrap_gate()
        : CreatureScript("npc_deahts_door_wrap_gate")
    {
    }
    ...
    {
        npc_deahts_door_wrap_gateAI(Creature* c)
            : ScriptedAI(c)
        {
        }
                ...
                if ((timer >= 0 && timer < 10000) || (timer >= 20000 && timer < 30000))
                {
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* cr = me->SummonCreature((roll_chance_i(50) ? 22474 : 22500), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                            if (Unit* target = cr->SelectNearbyTarget(nullptr, 50.0f))
...
public:
    npc_nether_drake()
        : CreatureScript("npc_nether_drake")
    {
    }
...
public:
    npc_simon_bunny()
        : CreatureScript("npc_simon_bunny")
    {
    }
...
public:
    go_simon_cluster()
        : GameObjectScript("go_simon_cluster")
    {
    }
...
public:
    go_apexis_relic()
        : GameObjectScript("go_apexis_relic")
    {
    }
...
public:
    spell_oscillating_field()
        : SpellScriptLoader("spell_oscillating_field")
    {
    }
