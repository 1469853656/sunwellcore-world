...
{
public:
    ResetEncounterEvent(Unit* caster, uint32 spellId, uint64 otherTransport)
        : _caster(caster)
        , _spellId(spellId)
    ...
    bool Execute(uint64, uint32)
    {
        ...
        if (GameObject* go = _caster->GetMap()->GetGameObject(_otherTransport))
        {
...
public:
    ...
    struct npc_gunshipAI : public NullCreatureAI
    {
        npc_gunshipAI(Creature* creature)
            ...
            , _teamIdInInstance(TeamId(creature->GetInstanceScript()->GetData(DATA_TEAMID_IN_INSTANCE)))
            , _died(false)
            , _summonedFirstMage(false)
            , _fightTimer(0)
        {
            me->SetRegeneratingHealth(false);
            if (_instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
                me->SetMaxHealth(me->GetMaxHealth() * 2.0f);
...
public:
    ...
    struct npc_high_overlord_saurfang_igbAI : public ScriptedAI
    {
        ...
        {
            if (damage && _instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
                damage *= 1.3f;
        }
...
public:
    npc_zafod_boombox()
        : CreatureScript("npc_zafod_boombox")
    {
    }

    struct npc_zafod_boomboxAI : public NullCreatureAI
    {
        npc_zafod_boomboxAI(Creature* creature)
            : NullCreatureAI(creature)
        {
...
public:
    npc_igb_ship_crew()
        : CreatureScript("npc_igb_ship_crew")
    {
    }
...
struct gunship_npc_AI : public ScriptedAI
{
    ...
    void DamageDealt(Unit* /*victim*/, uint32& damage, DamageEffectType /*damageType*/) override
    {
        if (damage && Instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
            damage *= 1.3f;
    }
...
struct npc_gunship_boarding_addAI : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        if (type == POINT_MOTION_TYPE && pointId == EVENT_CHARGE_PREPATH && Slot)
        {
            ...
            if (GameObject* go = Instance->instance->GetGameObject(Instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE)))
...
public:
    npc_gunship_boarding_add()
        : CreatureScript("npc_gunship_boarding_add")
    {
    }
...
public:
    npc_gunship_mage()
        : CreatureScript("npc_gunship_mage")
    {
    }
...
public:
    npc_gunship_gunner()
        : CreatureScript("npc_gunship_gunner")
    {
    }
...
{
public:
    npc_gunship_rocketeer()
        : CreatureScript("npc_gunship_rocketeer")
    {
    }

    struct npc_gunship_rocketeerAI : public gunship_npc_AI
    {
        npc_gunship_rocketeerAI(Creature* creature)
            : gunship_npc_AI(creature)
        {
            creature->m_CombatDistance = 150.0f;
        }
...
public:
    spell_igb_rocket_pack()
        : SpellScriptLoader("spell_igb_rocket_pack")
    {
    }
    ...
    class spell_igb_rocket_pack_AuraScript : public AuraScript
    {
        ...
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_ROCKET_PACK_DAMAGE) || !sSpellMgr->GetSpellInfo(SPELL_ROCKET_BURST))
                return false;
    ...
    class spell_igb_gunship_fall_teleport_SpellScript : public SpellScript
    {
        ...
        void SelectTransport(WorldObject*& target)
        {
            if (InstanceScript* instance = target->GetInstanceScript())
                target = instance->instance->GetGameObject(instance->GetData64(DATA_ICECROWN_GUNSHIP_BATTLE));
        }
...
public:
    spell_igb_explosion()
        : SpellScriptLoader("spell_igb_explosion")
    {
    }
...
{
public:
    IgbTeleportOnVictoryCheck(InstanceScript* inst)
        : _inst(inst)
    {
    }
...
public:
    spell_igb_overheat()
        : SpellScriptLoader("spell_igb_overheat")
    {
    }
...
public:
    spell_igb_cannon_blast()
        : SpellScriptLoader("spell_igb_cannon_blast")
    {
    }
...
public:
    spell_igb_burning_pitch()
        : SpellScriptLoader("spell_igb_burning_pitch")
    {
    }
...
public:
    spell_igb_on_gunship_deck()
        : SpellScriptLoader("spell_igb_on_gunship_deck")
    {
    }
