...
public:
    boss_lord_marrowgar()
        : CreatureScript("boss_lord_marrowgar")
    {
    }

    struct boss_lord_marrowgarAI : public BossAI
    {
        boss_lord_marrowgarAI(Creature* creature)
            : BossAI(creature, DATA_LORD_MARROWGAR)
        {
            ...
            if (instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
                ...
                instance->IncreaseMeleeDmgPct(me, 0.1f);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_BONE_STORM_MOVE:
                {
                    ...
                    if (!unit)
                    {
                        unit = SelectTarget(SELECT_TARGET_TOPAGGRO, 0, 175.0f, true);
                        if (unit && unit->GetPositionX() > -337.0f)
                        {
                            EnterEvadeMode();
                            return;
                        }
...
public:
    spell_marrowgar_coldflame()
        : SpellScriptLoader("spell_marrowgar_coldflame")
    {
    }
