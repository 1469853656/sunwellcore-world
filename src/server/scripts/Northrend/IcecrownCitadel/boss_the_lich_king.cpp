...
{
public:
    LichKingDeathEvent(Creature& owner)
        : _owner(owner)
    {
    }
...
{
public:
    LichKingMovieEvent(Creature& owner)
        : _owner(owner)
    {
    }
...
public:
    boss_the_lich_king()
        : CreatureScript("boss_the_lich_king")
    {
    }
    ...
    struct boss_the_lich_kingAI : public BossAI
    {
            ...
            : BossAI(creature, DATA_THE_LICH_KING)
        {
            ...
            if (auto diff = instance->IsBoostEnabled())
            {
                float mult = 1.0f;
                if (diff == RAID_DIFFICULTY_10MAN_HEROIC)
                    mult = 1.1f;
                else if (diff == RAID_DIFFICULTY_25MAN_HEROIC)
                    mult = 1.2f;
                me->SetMaxHealth(me->GetMaxHealth() * mult);
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType, SpellSchoolMask) override
        {
            ...
            if (!attacker || (_bFrostmournePhase && attacker->GetExactDistSq(495.708f, -2523.76f, 1049.95f) > 40.0f * 40.0f) && attacker->HasAura(SPELL_HARVEST_SOULS_TELEPORT)) // frostmourne room, prevent exploiting (tele hack to get back and damage him)
            {
                damage = 0;
                return;
            }
            ...
            if (_phase == PHASE_OUTRO)
            {
                ...
                if (!_bFordringMustFallYell && me->GetHealth() < 500000)
                {
                    ...
                    if (Creature* tirion = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_HIGHLORD_TIRION_FORDRING)))
                    {
                        tirion->MonsterYell("The Lich King must fall!", LANG_UNIVERSAL, nullptr);
        ...
        void JustSummoned(Creature* summon) override
        {
            switch (summon->GetEntry())
            {
                ...
                case NPC_DRUDGE_GHOUL:
                    ...
                    {
                        summon->SetMaxHealth(summon->GetMaxHealth() * 1.2f);
            ...
            switch (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_OUTRO_SOUL_BARRAGE:
                    if (Creature* theLichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
                    {
                        ...
                        sCreatureTextMgr->SendSound(theLichKing, SOUND_PAIN, CHAT_MSG_MONSTER_YELL, nullptr, TEXT_RANGE_NORMAL, TEAM_NEUTRAL, false);
...
public:
    spell_the_lich_king_quake()
        : SpellScriptLoader("spell_the_lich_king_quake")
    {
    }
...
public:
    spell_the_lich_king_jump()
        : SpellScriptLoader("spell_the_lich_king_jump")
    {
    }
...
public:
    npc_shambling_horror_icc()
        : CreatureScript("npc_shambling_horror_icc")
    {
    }

    struct npc_shambling_horror_iccAI : public ScriptedAI
    {
        npc_shambling_horror_iccAI(Creature* creature)
            : ScriptedAI(creature)
        {
            _frenzied = false;
        }
        ...
        void OnSpellCastInterrupt(SpellInfo const* spell) override
        {
            ...
            if (spell->Id == RAID_MODE<uint32>(72143, 72146, 72147, 72148))
...
public:
    npc_icc_ice_sphere()
        : CreatureScript("npc_icc_ice_sphere")
    {
    }

    struct npc_icc_ice_sphereAI : public ScriptedAI
    {
        npc_icc_ice_sphereAI(Creature* creature)
            : ScriptedAI(creature)
        {
            targetGUID = 0;
            timer      = 250;
    ...
    class spell_the_lich_king_raging_spirit_SpellScript : public SpellScript
    {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (Unit* target = GetHitUnit())
                target->CastSpell(target, uint32(GetEffectValue()), true, nullptr, nullptr, target->GetGUID());
        }
...
public:
    npc_raging_spirit()
        : CreatureScript("npc_raging_spirit")
    {
    }
    ...
    struct npc_raging_spiritAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* /*summoner*/) override
        {
            ...
            // if (Creature* lichKing = me->GetMapId() == 249 /* Onyxias lair - Hellforge */ ? me->FindNearestCreature(261008, 100.f) : ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
...
public:
    npc_valkyr_shadowguard()
        : CreatureScript("npc_valkyr_shadowguard")
    {
    }
...
public:
    spell_the_lich_king_vile_spirit_move_target_search()
        : SpellScriptLoader("spell_the_lich_king_vile_spirit_move_target_search")
    {
    }
...
public:
    spell_the_lich_king_vile_spirit_damage_target_search()
        : SpellScriptLoader("spell_the_lich_king_vile_spirit_damage_target_search")
    {
    }
...
public:
    npc_strangulate_vehicle()
        : CreatureScript("npc_strangulate_vehicle")
    {
    }
...
{
public:
    npc_terenas_menethil()
        : CreatureScript("npc_terenas_menethil")
    {
    }
...
public:
    npc_spirit_warden()
        : CreatureScript("npc_spirit_warden")
    {
    }
...
public:
    npc_icc_lk_checktarget()
        : CreatureScript("npc_icc_lk_checktarget")
    {
    }

    struct npc_icc_lk_checktargetAI : public ScriptedAI
    {
        npc_icc_lk_checktargetAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_lk_spirit_bomb()
        : CreatureScript("npc_lk_spirit_bomb")
    {
    }

    struct npc_lk_spirit_bombAI : public NullCreatureAI
    {
        npc_lk_spirit_bombAI(Creature* creature)
            : NullCreatureAI(creature)
        {
            me->SetReactState(REACT_PASSIVE);
...
{
public:
    npc_lk_wicked_spirit()
        : CreatureScript("npc_lk_wicked_spirit")
    {
    }

    struct npc_lk_wicked_spiritAI : public ScriptedAI
    {
        npc_lk_wicked_spiritAI(Creature* creature)
            : ScriptedAI(creature)
        {
            if (InstanceScript* instance = me->GetInstanceScript())
                if (instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
                {
                    me->SetMaxHealth(me->GetMaxHealth() * 1.2f);
                    me->SetHealth(me->GetMaxHealth());
                }
        }
