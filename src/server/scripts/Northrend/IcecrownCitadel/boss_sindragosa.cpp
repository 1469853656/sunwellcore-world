...
public:
    FrostBombExplosion(Creature* owner, uint64 sindragosaGUID)
        : _owner(owner)
        , _sindragosaGUID(sindragosaGUID)
    {
    }
...
{
public:
    IceTombSummonEvent(Unit* owner, uint64 sindragosaGUID)
        : _owner(owner)
        , _sindragosaGUID(sindragosaGUID)
    {
    }
...
public:
    ...
    bool operator()(Unit const* target) const
    {
        ...
        if (target->IsImmunedToDamageOrSchool(SPELL_SCHOOL_MASK_ALL) || target->HasAura(SPELL_ICE_TOMB_UNTARGETABLE) || target->HasAura(SPELL_FROST_IMBUED_BLADE) || target->HasAura(SPELL_ICE_TOMB_DAMAGE) || target->HasAura(SPELL_TANK_MARKER_AURA) || target->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION))
            return false;
...
public:
    boss_sindragosa()
        : CreatureScript("boss_sindragosa")
    {
    }
    ...
    struct boss_sindragosaAI : public BossAI
    {
            ...
            , _summoned(false)
        {
            ...
            if (instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
                ...
                instance->IncreaseMeleeDmgPct(me, 0.15f);
            }
        }
        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            else if (summon->GetEntry() == NPC_ICE_TOMB)
                summon->AI()->DoAction(_isThirdPhase ? DATA_TRIGGER_ASPHYXIATION_LAST_PHASE : DATA_TRIGGER_ASPHYXIATION_AIR_PHASE);
        }
...
public:
    spell_sindragosa_s_fury()
        : SpellScriptLoader("spell_sindragosa_s_fury")
    {
    }
...
public:
    spell_sindragosa_icy_grip()
        : SpellScriptLoader("spell_sindragosa_icy_grip")
    {
    }
...
{
public:
    explicit MysticBuffetTargetFilter(Unit* caster)
        : _caster(caster)
    {
    }
    ...
    bool operator()(WorldObject* unit) const
    {
        ...
        return unit->GetMap()->isInLineOfSight(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ() + 2.f, ox, oy, oz + 2.f, unit->GetPhaseMask(), LINEOFSIGHT_CHECK_GOBJECT);
    }
...
public:
    npc_spinestalker()
        : CreatureScript("npc_spinestalker")
    {
    }
...
public:
    npc_rimefang()
        : CreatureScript("npc_rimefang_icc")
    {
    }
...
public:
    spell_rimefang_icy_blast()
        : SpellScriptLoader("spell_rimefang_icy_blast")
    {
    }
...
public:
    at_sindragosa_lair()
        : AreaTriggerScript("at_sindragosa_lair")
    {
    }
...
{
public:
    npc_sindragosa_trash()
        : CreatureScript("npc_sindragosa_trash")
    {
    }
...
{
public:
    explicit OrderWhelpTargetSelector(Creature* owner)
        : _owner(owner)
    {
    }
