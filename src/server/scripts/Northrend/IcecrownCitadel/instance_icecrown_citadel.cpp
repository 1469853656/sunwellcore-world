...
public:
    instance_icecrown_citadel()
        : InstanceMapScript(ICCScriptName, 631)
    {
    }
    ...
    struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
    {
        ...
        {
            if (newArea == 4890 /*Putricide's Laboratory of Alchemical Horrors and Fun*/ || newArea == 4891 /*The Sanctum of Blood*/ || newArea == 4889 /*The Frost Queen's Lair*/ || newArea == 4859 /*The Frozen Throne*/ || newArea == 4910 /*Frostmourne*/)
            {
        ...
        void OnPlayerEnter(Player* player) override
        {
            if (TeamIdInInstance == TEAM_NEUTRAL)
                TeamIdInInstance = player->GetTeam();

            uint32 const newFaction   = TeamIdInInstance == TEAM_HORDE ? 2 : 1;
            player->OverrideFactionID = newFaction;
            ...
            switch (entry)
            {
                ...
                case NPC_ZAFOD_BOOMBOX:
                    if (GameObjectTemplate const* go = sObjectMgr->GetGameObjectTemplate(GO_THE_SKYBREAKER_A))
                        if ((TeamIdInInstance == TEAM_ALLIANCE && data->mapid == go->moTransport.mapID) || (TeamIdInInstance == TEAM_HORDE && data->mapid != go->moTransport.mapID))
                            return entry;
                    return 0;
                case NPC_IGB_MURADIN_BRONZEBEARD:
                    if ((TeamIdInInstance == TEAM_ALLIANCE && data->posX > 10.0f) || (TeamIdInInstance == TEAM_HORDE && data->posX < 10.0f))
                        return entry;
                    return 0;
            ...
            switch (go->GetEntry())
            {
                ...
                case GO_CACHE_OF_THE_DREAMWALKER_25H:
                    if (Creature* valithria = instance->GetCreature(ValithriaDreamwalkerGUID))
                        go->SetLootRecipient(valithria->GetLootRecipient(), valithria);
                        ...
                        case DONE:
                            if (GameObject* loot = instance->GetGameObject(DeathbringersCacheGUID))
                            {
                                if (Creature* deathbringer = instance->GetCreature(DeathbringerSaurfangGUID))
                                    loot->SetLootRecipient(deathbringer->GetLootRecipient(), deathbringer);
                                loot->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_LOCKED | GO_FLAG_NOT_SELECTABLE | GO_FLAG_NODESPAWN);
                            }
                            // no break
                        case NOT_STARTED:
        ...
        void Update(uint32 diff) override
        {
            ...
            if (AttemptCooldown <= diff)
                AttemptCooldown = 0;
            else
                AttemptCooldown -= diff;
