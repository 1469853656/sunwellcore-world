...
public:
    boss_blood_queen_lana_thel()
        : CreatureScript("boss_blood_queen_lana_thel")
    {
    }
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (_creditBloodQuickening)
            {
                ...
                for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
                {
                    ...
                    {
                        auto questId = (me->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL || me->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC) ? NPC_INFILTRATOR_MINCHAR_BQ : NPC_BLOOD_QUICKENING_CREDIT_25;
                    ...
                    if (me->GetVictim())
                    {
                        ...
                        if (!myList.empty())
                        {
                            ...
                            if (me->GetVictim()->GetGUID() != _tankGUID || target->GetGUID() != _offtankGUID)
                            {
                                ...
                                if (Is25ManRaid() && target->GetQuestStatus(QUEST_BLOOD_INFUSION) == QUEST_STATUS_INCOMPLETE && target->HasAura(SPELL_UNSATED_CRAVING) && !target->HasAura(SPELL_THIRST_QUENCHED) && !target->HasAura(SPELL_GUSHING_WOUND))
...
public:
    spell_blood_queen_pact_of_the_darkfallen_dmg_target()
        : SpellScriptLoader("spell_blood_queen_pact_of_the_darkfallen_dmg_target")
    {
    }
...
{
public:
    explicit BloodboltHitCheck(LanaThelAI* ai)
        : _ai(ai)
    {
    }
...
{
public:
    achievement_once_bitten_twice_shy(const char* name, uint8 spawnMode, bool wasVampire)
