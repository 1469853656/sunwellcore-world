...
{
public:
    explicit CaptainSurviveTalk(Creature const& owner)
        : _owner(owner)
    {
    }
...
public:
    npc_rotting_frost_giant()
        : CreatureScript("npc_rotting_frost_giant")
    {
    }
...
public:
    npc_frost_freeze_trap()
        : CreatureScript("npc_frost_freeze_trap")
    {
    }
...
public:
    npc_crok_scourgebane()
        : CreatureScript("npc_crok_scourgebane")
    {
    }
...
Position const SvalnaGroundHomePos = { 4356.709f, 2484.330f, 358.439f };

class boss_sister_svalna : public CreatureScript
{
public:
    boss_sister_svalna()
        : CreatureScript("boss_sister_svalna")
    {
    }
    ...
    struct boss_sister_svalnaAI : public BossAI
    {
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            return target->GetDistance(SvalnaGroundHomePos) < 35.0f;
        }
...
public:
    npc_captain_arnath()
        : CreatureScript("npc_captain_arnath")
    {
    }
    ...
    private:
        Creature* FindFriendlyCreature() const
        {
            ...
            Trinity::MostHPMissingInRange u_check(me, 60.0f, 0, false);
...
public:
    npc_captain_brandon()
        : CreatureScript("npc_captain_brandon")
    {
    }
...
public:
    npc_captain_grondel()
        : CreatureScript("npc_captain_grondel")
    {
    }
...
public:
    npc_captain_rupert()
        : CreatureScript("npc_captain_rupert")
    {
    }
...
public:
    npc_frostwing_vrykul()
        : CreatureScript("npc_frostwing_vrykul")
    {
    }
...
public:
    npc_impaling_spear()
        : CreatureScript("npc_impaling_spear")
    {
    }
...
public:
    npc_alchemist_adrianna()
        : CreatureScript("npc_alchemist_adrianna")
    {
    }
...
public:
    npc_arthas_teleport_visual()
        : CreatureScript("npc_arthas_teleport_visual")
    {
    }
    ...
    struct npc_arthas_teleport_visualAI : public NullCreatureAI
    {
        ...
        {
            _events.Reset();
            if (_instance->GetBossState(DATA_PROFESSOR_PUTRICIDE) == DONE && _instance->GetBossState(DATA_BLOOD_QUEEN_LANA_THEL) == DONE && _instance->GetBossState(DATA_SINDRAGOSA) == DONE)
...
public:
    spell_icc_stoneform()
        : SpellScriptLoader("spell_icc_stoneform")
    {
    }
...
public:
    spell_icc_sprit_alarm()
        : SpellScriptLoader("spell_icc_sprit_alarm")
    {
    }
...
{
public:
    spell_icc_geist_alarm()
        : SpellScriptLoader("spell_icc_geist_alarm")
    {
    }
        ...
        void HandleEvent(SpellEffIndex effIndex)
        {
            ...
            if (InstanceScript* instance = GetCaster()->GetInstanceScript())
            {
                ...
                if (Creature* l = instance->instance->SummonCreature(NPC_VENGEFUL_FLESHREAPER, p))
                {
                    ...
                    Unit* target = l->SelectNearestTarget(20.0f);
                    if (target)
                        hasTarget = true;
                    else
                    {
...
public:
    spell_svalna_remove_spear()
        : SpellScriptLoader("spell_svalna_remove_spear")
    {
    }
...
{
public:
    spell_icc_soul_missile()
        : SpellScriptLoader("spell_icc_soul_missile")
    {
    }
...
{
public:
    at_icc_saurfang_portal()
        : AreaTriggerScript("at_icc_saurfang_portal")
    {
    }
...
{
public:
    at_icc_shutdown_traps()
        : AreaTriggerScript("at_icc_shutdown_traps")
    {
    }
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
    {
        // Don't start gauntlet when Sindragosa wing is disabled
        if ((sWorld->getIntConfig(CONFIG_ICC_STAGE) & 0x08) == 0 || player->IsGameMaster())
            return true;

        if (InstanceScript* instance = player->GetInstanceScript())
            if (instance->GetBossState(DATA_SISTER_SVALNA) != DONE)
            {
                if (Creature* svalna = ObjectAccessor::GetCreature(*player, instance->GetData64(DATA_SISTER_SVALNA)))
                {
                    if (!player->InSamePhase(svalna))
                        return true;
                }
...
public:
    spell_icc_web_wrap()
        : SpellScriptLoader("spell_icc_web_wrap")
    {
    }
...
{
public:
    spell_icc_dark_reckoning()
        : SpellScriptLoader("spell_icc_dark_reckoning")
    {
    }
...
{
public:
    spell_icc_yf_frozen_orb()
        : SpellScriptLoader("spell_icc_yf_frozen_orb")
    {
    }
...
public:
    spell_icc_yh_volley()
        : SpellScriptLoader("spell_icc_yh_volley")
    {
    }
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            ...
            if (Unit* c = GetCaster())
                if (Unit* t = GetTarget())
                {
                    ...
                    c->CastCustomSpell(t, 41089, nullptr, &basepoints1, nullptr, true);
                }
        }
...
{
public:
    spell_icc_shattered_bones()
        : SpellScriptLoader("spell_icc_shattered_bones")
    {
    }
...
public:
    ...
    {
        npc_icc_skybreaker_marksmanAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        EventMap events;
...
{
public:
    npc_icc_skybreaker_vicar()
        : CreatureScript("npc_icc_skybreaker_vicar")
    {
    }

    struct npc_icc_skybreaker_vicarAI : public ScriptedAI
    {
        npc_icc_skybreaker_vicarAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        EventMap events;
...
public:
    ...
    {
        npc_icc_skybreaker_luminaryAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        EventMap events;
...
{
public:
    npc_icc_severed_essence()
        : CreatureScript("npc_icc_severed_essence")
    {
    }

    struct npc_icc_severed_essenceAI : public ScriptedAI
    {
        npc_icc_severed_essenceAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        EventMap events;
...
public:
    npc_icc_spire_frostwyrm()
        : CreatureScript("npc_icc_spire_frostwyrm")
    {
    }

    struct npc_icc_spire_frostwyrmAI : public ScriptedAI
    {
        npc_icc_spire_frostwyrmAI(Creature* creature)
            : ScriptedAI(creature)
        {
            me->SetDisableGravity(true);
            me->SetHover(true);
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == EFFECT_MOTION_TYPE && id == 1)
            {
                ...
                {
                    Talk(p->GetTeam(CF_DENY) == TEAM_HORDE ? 1 : 2);
                    AttackStart(p);
                }
            }
        }
...
public:
    npc_icc_buff_switcher()
        : CreatureScript("npc_icc_buff_switcher")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        if (creature->GetEntry() == NPC_GARROSH_HELLSCREAM && player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 11206 || creature->GetEntry() == NPC_KING_VARIAN_WRYNN && player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 11204)
        {
            if (!player->GetGroup() || !player->GetGroup()->isRaidGroup() || !player->GetGroup()->IsLeader(player->GetGUID()))
            {
                ...
                ChatHandler(player->GetSession()).SendSysMessage("Only the raid leader can turn off the buff.");
                return true;
            }
...
public:
    ...
    {
        npc_icc_nerubar_broodkeeperAI(Creature* creature)
            : ScriptedAI(creature)
        {
            me->SetDisableGravity(true);
            me->SetCanFly(true);
...
{
public:
    npc_icc_putricades_trap()
        : CreatureScript("npc_icc_putricades_trap")
    {
    }
...
public:
    npc_darkfallen_blood_knight()
        : CreatureScript("npc_darkfallen_blood_knight")
    {
    }

    struct npc_darkfallen_blood_knightAI : public ScriptedAI
    {
        npc_darkfallen_blood_knightAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
public:
    at_icc_gauntlet_event()
        : AreaTriggerScript("at_icc_gauntlet_event")
    {
    }
...
{
public:
    at_icc_putricide_trap()
        : AreaTriggerScript("at_icc_putricide_trap")
    {
    }
...
public:
    at_icc_spire_frostwyrm()
        : AreaTriggerScript("at_icc_spire_frostwyrm")
    {
    }
...
{
    npc_plague_scientistAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_icc_web_wrapAI(Creature* creature)
        : ScriptedAI(creature)
    {
        me->SetReactState(REACT_PASSIVE);
...
{
public:
    at_icc_lights_hammer_disable_spawn()
        : AreaTriggerScript("at_icc_lights_hammer_disable_spawn")
    {
    }
...
{
    npc_icc_warhawkAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
public:
    explicit ChampionRessurrect(Creature* owner, Unit* killer)
        : _owner(owner)
        , _killer(killer)
    {
    }
...
{
    npc_icc_championAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_icc_argent_championAI(Creature* creature)
        : npc_icc_championAI(creature)
    {
    }
...
{
    npc_icc_ebon_championAI(Creature* creature)
        : npc_icc_championAI(creature)
    {
    }
...
{
    npc_deathbound_wardAI(Creature* creature)
        : ScriptedAI(creature)
    {
        _path = false;
    }
