...
public:
    ...
    {
        return creature->IsAlive() && creature->GetEntry() == NPC_RISEN_ARCHMAGE && creature->GetDBTableGUIDLow() && !creature->IsInCombat();
    }
};

struct ManaVoidSelector
{
    explicit ManaVoidSelector(WorldObject const* source)
        : _source(source)
    {
    }
...
{
public:
    explicit ValithriaDespawner(Creature* creature)
        : _creature(creature)
    {
    }
...
public:
    boss_valithria_dreamwalker()
        : CreatureScript("boss_valithria_dreamwalker")
    {
    }
    ...
    struct boss_valithria_dreamwalkerAI : public ScriptedAI
    {
            ...
            , _portalCount(RAID_MODE<uint32>(3, 8, 3, 8))
        {
            ...
            {
                me->SetMaxHealth(_originalMaxHP * 1.25f);
                _spawnHealth = me->CountPctFromMaxHealth(50.0f);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                if (_instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC && me->GetMaxHealth() != _originalMaxHP * 1.25f)
                {
                    me->SetMaxHealth(_originalMaxHP * 1.25f);
                    _spawnHealth = me->CountPctFromMaxHealth(50.0f);
                }
...
public:
    npc_risen_archmage()
        : CreatureScript("npc_risen_archmage")
    {
    }
...
public:
    npc_valithria_portal()
        : CreatureScript("npc_valithria_portal")
    {
    }
...
public:
    npc_blazing_skeleton()
        : CreatureScript("npc_blazing_skeleton")
    {
    }
...
public:
    npc_blistering_zombie()
        : CreatureScript("npc_blistering_zombie")
    {
    }
...
public:
    npc_gluttonous_abomination()
        : CreatureScript("npc_gluttonous_abomination")
    {
    }
