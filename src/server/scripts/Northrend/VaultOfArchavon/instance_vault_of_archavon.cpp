                    ...
                    if (!bf->IsWarTime())
                    {
                            ...
                            if (!PlayerList.isEmpty())
                                for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                                    if (Player* player = i->GetSource())
                                        player->MonsterTextEmote("This instance will reset in 15 minutes.", nullptr, true);
                        }
                        ...
                        else if (bf->GetTimer() <= (2 * MINUTE * IN_MILLISECONDS) && bf->GetTimer() > (MINUTE * IN_MILLISECONDS))
                        {
                            ...
                            if (!PlayerList.isEmpty())
                                for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                                    if (Player* player = i->GetSource())
                                        player->MonsterTextEmote("This instance is about to reset. Prepare to be removed.", nullptr, true);
                        }
            ...
            switch (criteria_id)
            {
                ...
                case CRITERIA_EARTH_WIND_FIRE_25:
                    if (ArchavonDeath && EmalonDeath && KoralonDeath)
                    {
                        ...
                        return (abs(int(ArchavonDeath - EmalonDeath)) < MINUTE && abs(int(EmalonDeath - KoralonDeath)) < MINUTE && abs(int(KoralonDeath - ArchavonDeath)) < MINUTE);
                    }
                    break;
                default:
                    break;
