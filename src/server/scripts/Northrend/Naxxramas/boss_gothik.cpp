...
static Player* FindEligibleTarget(Creature const* me, bool isGateOpen)
{
    ...
    for (Map::PlayerList::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (player && (isGateOpen || IsOnSameSide(me, player)) && me->CanSeeOrDetect(player) && me->IsValidAttackTarget(player) && player->isInAccessiblePlaceFor(me) && player->GetDistance(GothikMiddleOfTheRoom) < 85.0f)
            return player;
    }
...
public:
    ...
    struct boss_gothikAI : public BossAI
    {
        ...
        {
            if (sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid())
            {
                if (Aura* aura = summon->AddAura(17211, summon))
                {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SUMMON:
                    {
                        ...
                        {
                            sLog->outError("GothikAI: Wave count %d is out of range for difficulty %d.", _waveCount, GetDifficulty());
                            break;
                        }
...
{
public:
    npc_gothik_minion_livingtrainee()
        : CreatureScript("npc_gothik_minion_livingtrainee")
    {
    }

    struct npc_gothik_minion_livingtraineeAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_livingtraineeAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature, SPELL_ANCHOR_1_TRAINEE)
...
{
public:
    npc_gothik_minion_livingknight()
        : CreatureScript("npc_gothik_minion_livingknight")
    {
    }

    struct npc_gothik_minion_livingknightAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_livingknightAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature, SPELL_ANCHOR_1_DK)
...
{
public:
    npc_gothik_minion_livingrider()
        : CreatureScript("npc_gothik_minion_livingrider")
    {
    }

    struct npc_gothik_minion_livingriderAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_livingriderAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature, SPELL_ANCHOR_1_RIDER)
...
{
public:
    npc_gothik_minion_spectraltrainee()
        : CreatureScript("npc_gothik_minion_spectraltrainee")
    {
    }

    struct npc_gothik_minion_spectraltraineeAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_spectraltraineeAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature)
...
{
public:
    npc_gothik_minion_spectralknight()
        : CreatureScript("npc_gothik_minion_spectralknight")
    {
    }

    struct npc_gothik_minion_spectralknightAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_spectralknightAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature)
...
public:
    npc_gothik_minion_spectralrider()
        : CreatureScript("npc_gothik_minion_spectralrider")
    {
    }

    struct npc_gothik_minion_spectralriderAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_spectralriderAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature)
            , _frenzyTimer(urand(2000, 5000))
            , _drainTimer(urand(8000, 12000))
        {
        }
...
{
public:
    npc_gothik_minion_spectralhorse()
        : CreatureScript("npc_gothik_minion_spectralhorse")
    {
    }

    struct npc_gothik_minion_spectralhorseAI : public npc_gothik_minion_baseAI
    {
        npc_gothik_minion_spectralhorseAI(Creature* creature)
            : npc_gothik_minion_baseAI(creature)
            , _stompTimer(urand(10000, 15000))
        {
        }
...
public:
    npc_gothik_trigger()
        : CreatureScript("npc_gothik_trigger")
    {
    }
    ...
    {
        npc_gothik_triggerAI(Creature* creature)
            : ScriptedAI(creature)
        {
