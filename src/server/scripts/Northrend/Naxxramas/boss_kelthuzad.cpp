...
public:
    ...
    struct boss_kelthuzadAI : BossAI
    {
        boss_kelthuzadAI(Creature* c) : BossAI(c, EVENT_KELTHUZAD) { }
        ...
        void Reset() override
        {
            ...
            if (GameObject* go = me->GetMap()->GetGameObject(instance->GetData64(DATA_KELTHUZAD_FLOOR)))
            {
                go->SetPhaseMask(1, true);
                go->SetGoState(GO_STATE_READY);
            }
            if (GameObject* go = me->GetMap()->GetGameObject(instance->GetData64(DATA_KELTHUZAD_GATE)))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_START_SECOND_PHASE:
                    ...
                    events.ScheduleEvent(EVENT_SPELL_SHADOW_FISSURE, sWorld->getBoolConfig(CONFIG_BOOST_NAXXRAMAS) ? RAID_MODE(25000, 15000) : 25000);
                    events.ScheduleEvent(EVENT_SPELL_FROST_BLAST, sWorld->getBoolConfig(CONFIG_BOOST_NAXXRAMAS) ? RAID_MODE(45000, 22000) : 45000);
                ...
                {
                    uint32 count = sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 3 : 1;
                    ...
                    events.RepeatEvent(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 15000 : 25000);
                    break;
                }
                ...
                {
                    for (uint8 i = 0; i < (sWorld->getBoolConfig(CONFIG_BOOST_NAXXRAMAS) ? 4 : 3); ++i)
                    {
                ...
                case EVENT_SECOND_PHASE_HEALTH_CHECK:
                    if (me->HealthBelowPct(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 75 : 40))
                    {
                        events.PopEvent();
                        Talk(SAY_REQUEST_AID);
                        events.DelayEvents(5500);
                        events.ScheduleEvent(EVENT_THIRD_PHASE_LICH_KING_SAY, 5000);
                        instance->SetData(DATA_ACTIVATE_KEL_PORTALS, DATA_ACTIVATE_KEL_PORTALS);
                        break;
                    }
                    events.RepeatEvent(1000);
                    break;
                case EVENT_THIRD_PHASE_LICH_KING_SAY:
                    if (Creature* cr = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_LICH_KING_BOSS)))
                ...
                case EVENT_SUMMON_GUARDIAN_OF_ICECROWN:
                    me->MonsterTextEmote("A Guardian of Icecrown enter the fight!", nullptr, true);
...
{
public:
    boss_kelthuzad_minion()
        : CreatureScript("boss_kelthuzad_minion")
    {
    }
    ...
    {
        boss_kelthuzad_minionAI(Creature* c)
            : ScriptedAI(c)
        {
        }
        ...
        void Reset() override
        {
            ...
            if (me->GetEntry() == NPC_GUARDIAN_OF_ICECROWN)
            {
                ...
                if (is25ManBoostEnabled)
                    if (Aura const* aura = me->AddAura(47106, me))
                        if (AuraEffect* aurEff = aura->GetEffect(0))
                            aurEff->ChangeAmount(-35);
            }
            else if (is25ManBoostEnabled)
            {
                if (Aura* aura = me->AddAura(17211, me))
                {
                    if (AuraEffect* aurEff = aura->GetEffect(0))
                        aurEff->ChangeAmount(200);
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_CALL_HELP_ON)
                ...
                callHelp = false;
            else if (param == ACTION_SECOND_PHASE && !me->IsInCombat())
        ...
        void AttackStart(Unit* who) override
        {
            ...
            if (callHelp && me->GetEntry() != NPC_GUARDIAN_OF_ICECROWN)
            {
        ...
        void HandleScript(AuraEffect const* aurEff)
        {
            ...
            if (int32 mana = int32(target->GetMaxPower(POWER_MANA) / 10))
            {
                ...
                if (target->GetMapId() == 249 /* Onyxia's Lair - Hellforge */)
                {
                    ...
                    target->CastCustomSpell(SPELL_MANA_DETONATION_DAMAGE, SPELLVALUE_BASE_POINT0, val.StatValue, target, true, nullptr, aurEff);
                }
                else
