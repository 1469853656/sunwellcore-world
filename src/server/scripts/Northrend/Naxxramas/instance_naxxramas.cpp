...
struct NaxxEncounterLogData
{
    NaxxEncounterLogData(std::string encounterName, uint64 encounterGUID) : EncounterName(std::move(encounterName)), EncounterGUID(encounterGUID) { }

    std::string EncounterName;
    uint64  EncounterGUID = 0;
};
...
public:
    instance_naxxramas()
        : InstanceMapScript("instance_naxxramas", 533)
    {
    }
    ...
    struct instance_naxxramas_InstanceMapScript : public InstanceScript
    {
        ...
        // NPCs
        uint64 _patchwerkGUID = 0;
        uint64 _gluthGUID = 0;
        uint64 _nothGUID = 0;
        uint64 _anubGUID = 0;
        uint64 _razuviousGUID = 0;
        uint64 _grobbulusGUID = 0;
        uint64 _maexxnaGUID = 0;
        uint64 _faerlinaGUID = 0;
        uint64 _loathebGUID = 0;
        uint64 _heiganGUID = 0;
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            switch (creature->GetEntry())
            {
                ...
                case NPC_PATCHWERK:
                    ...
                    break;
                case NPC_GLUTH:
                    _gluthGUID = creature->GetGUID();
                    break;
                case NPC_NOTH:
                    _nothGUID = creature->GetGUID();
                    break;
                case NPC_GROBBULUS:
                    _grobbulusGUID = creature->GetGUID();
                    break;
                case NPC_MAEXXNA:
                    _maexxnaGUID = creature->GetGUID();
                    break;
                case NPC_FAERLINA:
                    _faerlinaGUID = creature->GetGUID();
                    break;
                case NPC_LOATHEB:
                    _loathebGUID = creature->GetGUID();
                    break;
                case NPC_HEIGAN:
                    _heiganGUID = creature->GetGUID();
                    break;
                case NPC_ANUB:
                    _anubGUID = creature->GetGUID();
                    break;
            }
        }
        ...
        void OnGameObjectCreate(GameObject* pGo) override
        {
            ...
            switch (pGo->GetEntry())
            {
                ...
                case GO_PATCHWERK_GATE:
                    ...
                    if (GetBossState(EVENT_PATCHWERK) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_GLUTH_GATE:
                    _gluthGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_GLUTH) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_NOTH_GATE:
                    _nothGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_NOTH) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_HEIGAN_ENTERANCE_GATE:
                    _heiganGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_HEIGAN) == DONE || GetBossState(EVENT_NOTH) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_HEIGAN_EXIT_GATE:
                    _heiganGateExitGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_HEIGAN) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_LOATHEB_GATE:
                    _loathebGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_LOATHEB) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_ANUB_GATE:
                    _anubGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_ANUB) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                ...
                case GO_FAERLINA_GATE:
                    ...
                    if (GetBossState(EVENT_FAERLINA) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_MAEXXNA_GATE:
                    _maexxnaGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_FAERLINA) == DONE) // faerlina is correct
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_THADDIUS_GATE:
                    _thaddiusGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_GLUTH) == DONE) // gluth is correct
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                ...
                case GO_GOTHIK_EXIT_GATE:
                    ...
                    if (GetBossState(EVENT_GOTHIK) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                case GO_HORSEMAN_GATE:
                    _horsemanGateGUID = pGo->GetGUID();
                    if (GetBossState(EVENT_GOTHIK) == DONE) // correct
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                ...
                case GO_SAPPHIRON_GATE:
                    ...
                    if (GetBossState(EVENT_SAPPHIRON) == DONE)
                        pGo->SetGoState(GO_STATE_ACTIVE);
                    break;
                ...
                case GO_THADDIUS_PORTAL:
                    ...
                    if (GetBossState(EVENT_THADDIUS) == DONE)
                        pGo->SetPhaseMask(1, true);
                    break;
                ...
                case GO_MAEXXNA_PORTAL:
                    ...
                    if (GetBossState(EVENT_MAEXXNA) == DONE)
                        pGo->SetPhaseMask(1, true);
                    break;
                ...
                case GO_HORSEMAN_PORTAL:
                    ...
                    if (GetBossState(EVENT_HORSEMAN) == DONE)
                        pGo->SetPhaseMask(1, true);
                    break;
        ...
        {
            // Horseman are handled separately
            std::map<uint32, NaxxEncounterLogData> bossNames =
            {
                { EVENT_PATCHWERK, NaxxEncounterLogData("Patchwerk", _patchwerkGUID) },
                { EVENT_GLUTH, NaxxEncounterLogData("Gluth", _gluthGUID) },
                { EVENT_NOTH, NaxxEncounterLogData("Noth the Plaguebringer", _nothGUID) },
                { EVENT_ANUB, NaxxEncounterLogData("Anub'rekhan", _anubGUID) },
                { EVENT_RAZUVIOUS, NaxxEncounterLogData("Instructor Razuvious", _razuviousGUID) },
                { EVENT_GOTHIK, NaxxEncounterLogData("Gothik the Harvester", _gothikGUID) },
                { EVENT_GROBBULUS, NaxxEncounterLogData("Grobbulus", _grobbulusGUID) },
                { EVENT_MAEXXNA, NaxxEncounterLogData("Maexxna", _maexxnaGUID) },
                { EVENT_KELTHUZAD, NaxxEncounterLogData("Kel'thuzad", _kelthuzadGUID) },
                { EVENT_FAERLINA, NaxxEncounterLogData("Grand Widow Faerlina", _faerlinaGUID) },
                { EVENT_THADDIUS, NaxxEncounterLogData("Thaddius", _thaddiusGUID) },
                { EVENT_LOATHEB, NaxxEncounterLogData("Loatheb", _loathebGUID) },
                { EVENT_HEIGAN, NaxxEncounterLogData("Heigan the Unclean", _heiganGUID) },
                { EVENT_SAPPHIRON, NaxxEncounterLogData("Sapphiron", _sapphironGUID) },
            };

            auto const it = bossNames.find(bossID);
            if (it != bossNames.end())
            {
                if (state == IN_PROGRESS)
                    StartEncounterStatsCounting(it->second.EncounterGUID, it->second.EncounterName);
                else if (state == DONE)
                    FinishEncounterStatsCounting();
            }
            ...
            switch (bossID)
            {
                ...
                case EVENT_KELTHUZAD:
                    ...
                    for (uint64 const kelthuzadPortalGUID : kelthuzadPortals)
                    {
                        if (GameObject* go = instance->GetGameObject(kelthuzadPortalGUID))
                        {
                            if (state == NOT_STARTED)
                ...
                case EVENT_HORSEMAN:
                    // respawn
                    if (state == NOT_STARTED && _horsemanKilled > 0)
                    {
                        _horsemanKilled = 0;
                        for (uint64 const horsemanGUID :  { _blaumeuxGUID, _rivendareGUID, _zeliekGUID, _korthazzGUID })
                        {
                            if (Creature* cr = instance->GetCreature(horsemanGUID))
                            {
                                if (!cr->IsAlive())
                                {
                    ...
                    {
                        StartEncounterStatsCounting(_blaumeuxGUID, "The Four Horsemen");
                        for (uint64 const horsemanGUID :  { _blaumeuxGUID, _rivendareGUID, _zeliekGUID, _korthazzGUID })
                            if (Creature* cr = instance->GetCreature(horsemanGUID))
                                cr->SetInCombatWithZone();
                    }
                    else if (state == FAIL)
                        sCustomMgr->HandleCooldownResetOnWipe(this, 45s);
            ...
            if (bossID == EVENT_HORSEMAN && state == DONE && _horsemanKilled < 4)
                return false;
                ...
                case DATA_HORSEMAN_CHEST:
                    if (GameObject* chest = instance->GetGameObject(_mChestGUID))
                    {
                        if (instance->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
                            chest->SetLootMode(GetLootModeFor10Man(this));

                        chest->SetVisible(true);
                        chest->EnableCollision(true);

                        if (Creature* cr = instance->GetCreature(_blaumeuxGUID))
                            chest->SetLootRecipient(cr->GetLootRecipient(), cr);
                    }
                    return;
                case DATA_HORSEMEN_DIED:
                    _horsemanTimer++;
                    _horsemanKilled++;
                    if (_horsemanKilled < 4)
                        break;

                    sCustomMgr->HandleCooldownResetOnEncounterDone(this);
                    FinishEncounterStatsCounting();
                    ...
                    SetBossState(EVENT_HORSEMAN, DONE);
                    SaveToDB();
                    return;
            }
        }
...
public:
    boss_naxxramas_misc()
        : CreatureScript("boss_naxxramas_misc")
    {
    }
    ...
    {
        boss_naxxramas_miscAI(Creature* c)
            : NullCreatureAI(c)
        {
            timer = 0;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                if (me->SelectNearestPlayer(0.5f))
...
{
    npc_stoneskin_gargoyleAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
public:
    go_sapphiron_teleport_downstairs()
        : GameObjectScript("go_sapphiron_teleport_downstairs")
    {
    }
