...
public:
    ...
    struct boss_maexxnaAI : BossAI
    {
        boss_maexxnaAI(Creature* c) : BossAI(c, EVENT_MAEXXNA) { }
        ...
        {
            _Reset();
            if (instance->GetBossState(EVENT_FAERLINA) == DONE)
                if (GameObject* go = me->GetMap()->GetGameObject(instance->GetData64(DATA_MAEXXNA_GATE)))
        ...
        {
            instance->DoRemoveAurasDueToSpellOnPlayers(28622);
            _JustDied();
            instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_SPELL_WEB_SPRAY:
                    me->MonsterTextEmote("%s sprays strands of web everywhere!", nullptr, true);
                ...
                case EVENT_SPELL_POISON_SHOCK:
                    ...
                    events.RepeatEvent(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) ? RAID_MODE(20000, 10000) : 40000);
                    break;
                ...
                case EVENT_SUMMON_SPIDERLINGS:
                    me->MonsterTextEmote("Spiderlings appear on the web!", nullptr, true);
                    for (uint8 i = 0; i < 8; ++i)
                ...
                case EVENT_HEALTH_CHECK:
                    ...
                    break;
                case EVENT_WEB_WRAP:
                    me->MonsterTextEmote("%s spins her web into a cocoon!", nullptr, true);
                    for (uint8 i = 0; i < RAID_MODE(1, sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) ? 4 : 3); ++i)
                    {
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0, true, -SPELL_WEB_WRAP))
                        {
                            ...
                            Position posWrap = PosWrap[pos];
                            me->GetNearPositionFromPos(posWrap, 5.0f);
                            if (Creature* wrap = me->SummonCreature(NPC_WEB_WRAP, posWrap, TEMPSUMMON_TIMED_DESPAWN, 60000))
                            {
                                wrap->AI()->SetGUID(target->GetGUID());
                                target->GetMotionMaster()->MoveJump(posWrap, 20, 20);
                            }
                        }
                    }
                    events.RepeatEvent(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && Is25ManRaid() ? 20000 : RAID_MODE(25000, 40000));
                    break;
            }
...
{
public:
    boss_maexxna_webwrap()
        : CreatureScript("boss_maexxna_webwrap")
    {
    }
    ...
    {
        boss_maexxna_webwrapAI(Creature* c)
            : NullCreatureAI(c)
            , victimGUID(0)
        {
        }
