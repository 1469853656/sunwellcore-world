...
public:
    boss_four_horsemen()
        : CreatureScript("boss_four_horsemen")
    {
    }
    ...
    struct boss_four_horsemenAI : BossAI
    {
        boss_four_horsemenAI(Creature* c)
            : BossAI(c, EVENT_HORSEMAN)
        {
            switch (me->GetEntry())
            {
                case NPC_SIR_ZELIEK:
        ...
        void Reset() override
        {
            ...
            if (GameObject* go = me->GetMap()->GetGameObject(instance->GetData64(DATA_HORSEMAN_GATE)))
                go->SetGoState(GO_STATE_ACTIVE);

            // Schedule Events
            events.RescheduleEvent(EVENT_SPELL_MARK_CAST, 24000);
            events.RescheduleEvent(EVENT_CHECK_MARK_COUNT, 60000);
        ...
        void JustDied(Unit* killer) override
        {
            ...
            scheduler.CancelAll();
            summons.DespawnAll();
            instance->SetData(DATA_HORSEMEN_DIED, 1);
            if (instance->GetBossState(EVENT_HORSEMAN) == DONE)
                instance->SetData(DATA_HORSEMAN_CHEST, DATA_HORSEMAN_CHEST);
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            for (uint32 const auraID : { SPELL_MARK_OF_KORTHAZZ, SPELL_MARK_OF_BLAUMEUX, SPELL_MARK_OF_RIVENDARE, SPELL_MARK_OF_ZELIEK, SPELL_MARK_DAMAGE })
                instance->DoRemoveAurasDueToSpellOnPlayers(auraID);
        ...
        void checkMarksToEnrage()
        {
            ...
            {
                if (itr->GetSource()->HasAura(SPELL_MARK_OF_KORTHAZZ))
                    marksCount = marksCount + itr->GetSource()->GetAuraCount(SPELL_MARK_OF_KORTHAZZ);
                if (itr->GetSource()->HasAura(SPELL_MARK_OF_BLAUMEUX))
                    marksCount = marksCount + itr->GetSource()->GetAuraCount(SPELL_MARK_OF_BLAUMEUX);
                if (itr->GetSource()->HasAura(SPELL_MARK_OF_RIVENDARE))
                    marksCount = marksCount + itr->GetSource()->GetAuraCount(SPELL_MARK_OF_RIVENDARE);
                if (itr->GetSource()->HasAura(SPELL_MARK_OF_ZELIEK))
                    marksCount = marksCount + itr->GetSource()->GetAuraCount(SPELL_MARK_OF_ZELIEK);
                if (itr->GetSource()->HasAura(SPELL_MARK_DAMAGE))
                    marksCount = marksCount + itr->GetSource()->GetAuraCount(SPELL_MARK_DAMAGE);
            }
        }
...
{
public:
    spell_four_horsemen_mark()
        : SpellScriptLoader("spell_four_horsemen_mark")
    {
    }
    ...
    class spell_four_horsemen_mark_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    case 1:
                        damage = (sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && caster->GetMap()->Is25ManRaid() ? 2000 : 0);
                        break;
                    case 2:
                        damage = (sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && caster->GetMap()->Is25ManRaid() ? 4000 : 500);
                        break;
                    case 3:
                        damage = (sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && caster->GetMap()->Is25ManRaid() ? 6000 : 1500);
                        break;
                    case 4:
                        damage = (sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && caster->GetMap()->Is25ManRaid() ? 100000 : 4000);
                        break;
                    case 5:
                        damage = 12500;
                        break;
                    ...
                    case 8:
                        ...
                        break;
                    default:
                        damage = 22000 + 1000 * (GetStackAmount() - 8);
                        break;
                }
                if (damage)
