...
public:
    ...
    struct boss_sapphironAI : BossAI
    {
        boss_sapphironAI(Creature* c) : BossAI(c, EVENT_SAPPHIRON) { }
        ...
        void DespawnIceBlocks()
        {
            ...
            me->GetGameObjectListWithEntryInGrid(iceBlockList, GO_ICE_BLOCK, SIZE_OF_GRIDS);

            if (!iceBlockList.empty())
            {
                for (std::list<GameObject*>::const_iterator itr = iceBlockList.begin(); itr != iceBlockList.end(); ++itr)
        ...
        void Reset() override
        {
            ...
            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_FROST_NOVA);
        }
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            {
                events.ScheduleEvent(EVENT_STOMP_BOOST, 8s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SPELL_LIFE_DRAIN:
                    me->CastCustomSpell(RAID_MODE(SPELL_LIFE_DRAIN_10, SPELL_LIFE_DRAIN_25), SPELLVALUE_MAX_TARGETS, sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 8 : RAID_MODE(2, 5), me, false);
                    events.RepeatEvent(24000);
                    return;
                case EVENT_SPELL_BLIZZARD:
                {
                    uint8 count = sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 3 : 1;
                    for (uint8 i = 0; i < count; ++i)
                    {
                        Creature* cr = nullptr;
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 40.0f, true))
                            cr = me->SummonCreature(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? RAID_MODE(NPC_BLIZZARD, NPC_BLIZZARD_BOOST) : NPC_BLIZZARD, *target, TEMPSUMMON_TIMED_DESPAWN, 16000);
                        else
                            cr = me->SummonCreature(sCustomMgr->GetBoolConfig(CONFIG_NAXXRAMAS_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? RAID_MODE(NPC_BLIZZARD, NPC_BLIZZARD_BOOST) : NPC_BLIZZARD, *me, TEMPSUMMON_TIMED_DESPAWN, 16000);
