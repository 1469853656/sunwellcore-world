...
public:
    npc_attracted_reef_bull()
        : CreatureScript("npc_attracted_reef_bull")
    {
    }

    struct npc_attracted_reef_bullAI : public NullCreatureAI
    {
        npc_attracted_reef_bullAI(Creature* creature)
...
public:
    npc_your_inner_turmoil()
        : CreatureScript("npc_your_inner_turmoil")
    {
    }

    struct npc_your_inner_turmoilAI : public ScriptedAI
    {
        npc_your_inner_turmoilAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        ...
        {
            creature->setFaction(player->GetTeam(CF_DENY) == TEAM_ALLIANCE ? FACTION_ESCORTEE_A : FACTION_ESCORTEE_H);
    ...
    {
        npc_Apothecary_HanesAI(Creature* creature)
...
public:
    ...
    {
        npc_plaguehound_trackerAI(Creature* creature)
            : npc_escortAI(creature)
        {
        }
...
{
public:
    explicit ValgardeEventRespawn(Creature& owner)
        : _owner(owner)
    {
    }
...
{
    npc_valgarde_eventAI(Creature* creature)
        : ScriptedAI(creature)
    {
        if (me->isDead())
            me->Respawn();
...
{
    npc_valgarde_event_attackerAI(Creature* creature)
        : npc_valgarde_eventAI(creature)
    {
    }
...
{
    npc_valgarde_event_defenderAI(Creature* creature)
        : npc_valgarde_eventAI(creature)
    {
    }
...
{
    npc_lebronskiAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_riven_widow_cocconAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    void JustDied(Unit* killer) override
    {
        summonSpellId = Trinity::Containers::SelectRandomContainerElement(summonsSpells);
        if (summonSpellId)
        {
