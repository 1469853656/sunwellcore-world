...
public:
    npc_wg_demolisher_engineer()
        : CreatureScript("npc_wg_demolisher_engineer")
    {
    }
        ...
        if (canBuild(creature))
        {
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    creature->CastSpell(player, player->GetTeam() == TEAM_ALLIANCE ? SPELL_BUILD_SIEGE_VEHICLE_FORCE_ALLIANCE : SPELL_BUILD_SIEGE_VEHICLE_FORCE_HORDE, true);
                    break;
            }
...
public:
    npc_wg_spirit_guide()
        : CreatureScript("npc_wg_spirit_guide")
    {
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        for (uint8 i = 0; i < graveyard.size(); i++)
            if (graveyard[i]->GetControlTeamId() == player->GetTeam())
    ...
    bool OnGossipSelect(Player* player, Creature* /*creature */, uint32 /*sender */, uint32 action) override
    {
        ...
        if (wintergrasp)
        {
            ...
            for (uint8 i = 0; i < gy.size(); i++)
                if (action - GOSSIP_ACTION_INFO_DEF == i && gy[i]->GetControlTeamId() == player->GetTeam())
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* /*creature */, uint32 /*sender */, uint32 /*action*/) override
    {
        ...
        wintergrasp->QueueEvent<BattlefieldPlayerInviteRequest>(player->GetGUID());
        return true;
    }

    struct npc_wg_queueAI : public ScriptedAI
    {
        npc_wg_queueAI(Creature* creature)
            : ScriptedAI(creature)
        {
...
public:
    npc_wg_quest_giver()
        : CreatureScript("npc_wg_quest_giver")
    {
    }
    ...
    uint32 GetDialogStatus(Player* player, Creature* creature) override
    {
        ...
        for (QuestRelations::const_iterator i = qir.first; i != qir.second; ++i)
        {
            ...
            QuestStatus status = player->GetQuestStatus(questId);
            if ((status == QUEST_STATUS_COMPLETE && !player->GetQuestRewardStatus(questId)) || (quest->IsAutoComplete() && player->CanTakeQuest(quest, false)))
            {
...
{
public:
    npc_wg_siege_machine()
        : CreatureScript("npc_wg_siege_machine")
    {
    }
...
public:
    go_wg_vehicle_teleporter()
        : GameObjectScript("go_wg_vehicle_teleporter")
    {
    }
    ...
    struct go_wg_vehicle_teleporterAI : public GameObjectAI
    {
        ...
        {
            return ((go->GetUInt32Value(GAMEOBJECT_FACTION) == WintergraspFaction[TEAM_HORDE] && passenger->getRaceMask() & RACEMASK_HORDE) || (go->GetUInt32Value(GAMEOBJECT_FACTION) == WintergraspFaction[TEAM_ALLIANCE] && passenger->getRaceMask() & RACEMASK_ALLIANCE));
        }
...
public:
    go_wg_fortress_door()
        : GameObjectScript("go_wg_fortress_door")
    {
    }

    struct go_wg_fortress_doorAI : public GameObjectAI
    {
        go_wg_fortress_doorAI(GameObject* gameObject)
            : GameObjectAI(gameObject)
        {
        }
            ...
            if (!cheatType.empty())
                if (Vehicle* vehicle = unit->GetVehicleKit())
                    if (Unit* player = vehicle->GetPassenger(0))
                    {
                        ...
                        sLog->outCheat("Wintergrasp possible cheater! Attacking Fortress Door from %s. Nick: %s (guid: " UI64FMTD ")", cheatType.c_str(), player->GetName().c_str(), player->GetGUID());
                    }
        }
    };
...
public:
    spell_wintergrasp_rp_gg()
        : SpellScriptLoader("spell_wintergrasp_rp_gg")
    {
    }
...
{
public:
    spell_wintergrasp_portal()
        : SpellScriptLoader("spell_wintergrasp_portal")
    {
    }
    ...
    class spell_wintergrasp_portal_SpellScript : public SpellScript
    {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            Battlefield* wintergrasp = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_BATTLEID_WG);
            if (!wintergrasp || !target || target->getLevel() < 75 || (wintergrasp->GetDefenderTeam() != target->GetTeam()))
                return;
...
public:
    spell_wintergrasp_water()
        : SpellScriptLoader("spell_wintergrasp_water")
    {
    }
...
public:
    ...
    bool OnCheck(Player* source, Unit* target)
    {
        ...
        if (Unit* vehicle = source->GetVehicleBase())
            if (vehicle->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_ALLIANCE || vehicle->GetEntry() == NPC_WINTERGRASP_SIEGE_ENGINE_HORDE || vehicle->GetEntry() == NPC_WINTERGRASP_CATAPULT || vehicle->GetEntry() == NPC_WINTERGRASP_DEMOLISHER || vehicle->GetEntry() == NPC_WINTERGRASP_TOWER_CANNON)
                return true;
