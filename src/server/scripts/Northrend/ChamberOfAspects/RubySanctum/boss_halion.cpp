        ...
        void JustSummoned(Creature* summon) override
        {
            ...
            switch (summon->GetEntry())
            {
                ...
                case NPC_METEOR_STRIKE_FLAME:
                    if (Is25ManRaid() && IsHeroic() && roll_chance_i(90) && summons.GetEntryCount(NPC_LIVING_EMBER) < _livingEmberCount + 18)
...
public:
    boss_twilight_halion()
        : CreatureScript("boss_twilight_halion")
    {
    }
...
public:
    npc_halion_controller()
        : CreatureScript("npc_halion_controller")
    {
    }
...
public:
    npc_orb_carrier()
        : CreatureScript("npc_orb_carrier")
    {
    }
    ...
    class spell_halion_meteor_strike_marker_AuraScript : public AuraScript
    {
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                if (instance && instance->IsMythic() && caster->ToCreature())
                {
                    Creature* magtheridon = instance->GetCreature(32 /*magtheridon blood furnace*/);
                    if (magtheridon && magtheridon->IsAIEnabled)
                        magtheridon->AI()->SetGUID(caster->GetGUID());
                }
                else if (instance && instance->IsEncounterInProgress())
                {
...
public:
    spell_halion_blazing_aura()
        : SpellScriptLoader("spell_halion_blazing_aura")
    {
    }
...
public:
    spell_halion_damage_aoe_summon(char const* scriptName, uint32 explosionSpell, uint32 auraSpell)
        : SpellScriptLoader(scriptName)
        , _explosionSpell(explosionSpell)
