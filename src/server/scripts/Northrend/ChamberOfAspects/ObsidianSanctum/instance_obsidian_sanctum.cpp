    ...
    struct instance_obsidian_sanctum_InstanceMapScript : public InstanceScript
    {
        ...
        uint32 GetData(uint32 id) const override
        {
            switch (id)
            {
                ...
                case DATA_SARTHARION_ENRAGE:
                    return _sartharionEnraged;
            }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            switch (type)
            {
                case BOSS_SARTHARION_EVENT:
                    ...
                    if (Creature const* sartharion = instance->GetCreature(m_uiSartharionGUID); sartharion && sartharion->IsAIEnabled)
                    {
                        if (data == IN_PROGRESS)
                            StartEncounterStatsCounting(m_uiSartharionGUID, "Sartharion");
                        else if (data == DONE)
                            FinishEncounterStatsCounting(fmt::format(" [{}D]", sartharion->AI()->GetData(DATA_ACHIEVEMENT_DRAGONS_COUNT)));
                    }
