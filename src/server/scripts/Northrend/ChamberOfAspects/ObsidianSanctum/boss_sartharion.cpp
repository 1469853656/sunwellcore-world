...
const Position EggsPos[24] = {
    ...
    { 3249.76f, 664.413f, 87.7281f, 4.12938f },
    { 3248.01f, 658.606f, 86.8737f, 4.12938f },
    { 3248.7f, 651.558f, 85.8179f, 4.12938f },
    { 3240.72f, 652.386f, 85.9625f, 0.897469f },
    { 3259.89f, 653.323f, 85.9177f, 0.897469f },
    ...
    { 3239.24f, 526.20f, 58.95f, 0.0f },
    { 3240.95f, 515.96f, 58.662f, 0.7f },
    { 3247.66f, 521.685f, 58.78f, 0.7f },
    { 3256.64f, 526.6f, 58.811f, 1.966f },
    { 3260.9f, 536.41f, 58.811f, 2.08f },
...
public:
    boss_sartharion()
        : CreatureScript("boss_sartharion")
    {
    }
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        std::chrono::steady_clock::time_point _encounterStartTime = {};
        ...
        void EnterEvadeMode() override
        {
            ...
            sCustomMgr->HandleCooldownResetOnWipe(pInstance, std::chrono::duration_cast<Milliseconds>(std::chrono::steady_clock::now() - _encounterStartTime));
        ...
        void SummonLavaWaves()
        {
            ...
            events.ScheduleEvent(EVENT_SARTHARION_FINISH_LAVA, 12000);
        ...
        void StoreDragons()
        {
            ...
            {
                for (uint8 i = 0; i < 3; ++i)
                    if (Unit* cr = ObjectAccessor::GetUnit(*me, pInstance->GetData64(DATA_TENEBRON + i)))
                    {
                        if (!cr->IsAlive())
                            continue;
                        ...
                        {
                            case DATA_TENEBRON:
                                pInstance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, cr);
                ...
                if (!Is25ManRaid())
                    me->AddLootMode(GetLootModeFor10Man(pInstance));
        ...
        void SetDragons(bool die)
        {
            if (pInstance)
            {
                if (Creature* Dragon01 = me->FindNearestCreature(30449, 300.0f, true))
                {
                    pInstance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, Dragon01);
                    if (die)
                        Dragon01->DisappearAndDie();
                    else
                    {
                        Dragon01->SetHealth(Dragon01->GetMaxHealth());
                        Dragon01->AI()->EnterEvadeMode();
                    }
                }

                if (Creature* Dragon02 = me->FindNearestCreature(30451, 300.0f, true))
                {
                    pInstance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, Dragon02);
                    if (die)
                        Dragon02->DisappearAndDie();
                    else
                    {
                        Dragon02->SetHealth(Dragon02->GetMaxHealth());
                        Dragon02->AI()->EnterEvadeMode();
                    }
                }

                if (Creature* Dragon03 = me->FindNearestCreature(30452, 300.0f, true))
                {
                    pInstance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, Dragon03);
                    if (die)
                        Dragon03->DisappearAndDie();
                    else
                    {
                        Dragon03->SetHealth(Dragon03->GetMaxHealth());
                        Dragon03->AI()->EnterEvadeMode();
                    }
                }
            }
        }
        ...
        void Reset() override
        {
            ...
            if (pInstance)
            {
                ...
                pInstance->SetData(DATA_SARTHARION_ENRAGE, false);
            }
        }
        ...
        void EnterCombat(Unit* pWho) override
        {
            ...
            _encounterStartTime = std::chrono::steady_clock::now();
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (pInstance)
            {
                ...
                sCustomMgr->HandleCooldownResetOnEncounterDone(pInstance);
            }
        }
...
void boss_sartharion::boss_sartharionAI::HandleSartharionAbilities()
{
    ...
    switch (events.GetEvent())
    {
        ...
        case EVENT_SARTHARION_BERSERK:
            ...
            events.PopEvent();
            pInstance->SetData(DATA_SARTHARION_ENRAGE, true);
            break;
    }
}
...
public:
    boss_sartharion_tenebron()
        : CreatureScript("boss_sartharion_tenebron")
    {
    }
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                {
                    ...
                    for (uint8 i = 0; i < 12; ++i)
                        me->SummonCreature(NPC_TWILIGHT_EGG, EggsPos[isSartharion ? i + 12 : i], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000, 0, nullptr, 16);
                    ...
                    for (SummonList::iterator i = summons.begin(); i != summons.end(); ++i)
                    {
                        if (Creature* cr = ObjectAccessor::GetCreature(*me, *i))
                        {
                            ...
                            if (cr->GetEntry() == NPC_TWILIGHT_EGG)
                                if (Creature* summonCreature = me->SummonCreature(NPC_TWILIGHT_WHELP, cr->GetPositionX(), cr->GetPositionY(), cr->GetPositionZ(), cr->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000))
                                    summons2.Summon(summonCreature);
                        }
                    }
...
{
public:
    boss_sartharion_shadron()
        : CreatureScript("boss_sartharion_shadron")
    {
    }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                    ...
                    Creature* cr = me->SummonCreature((isSartharion ? NPC_ACOLYTE_OF_SHADRON : NPC_DISCIPLE_OF_SHADRON), *me, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0, 0, nullptr, 16);
...
{
public:
    boss_sartharion_vesperon()
        : CreatureScript("boss_sartharion_vesperon")
    {
    }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                    ...
                    Creature* cr = me->SummonCreature((isSartharion ? NPC_ACOLYTE_OF_VESPERON : NPC_DISCIPLE_OF_VESPERON), *me, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0, 0, nullptr, 16);
...
public:
    npc_twilight_summon()
        : CreatureScript("npc_twilight_summon")
    {
    }
    ...
    class spell_sartharion_lava_strike_SpellScript : public SpellScript
    {
        ...
        void HandleSchoolDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (InstanceScript* pInstance = GetCaster()->GetInstanceScript())
                ...
                {
                    uint8 rand = urand(1, 4); // 25 % chance to summon
                    uint8 iter = 1;
...
{
    npc_blaze_mistress_sarthrionAI(Creature* creature)
        : ScriptedAI(creature)
        , summons(me)
    {
    }
...
{
    npc_flame_orb_mistressAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
public:
    spell_summon_flame_orb_mistress()
        : SpellScriptLoader("spell_summon_flame_orb_mistress")
    {
    }
...
{
public:
    spell_onyx_brood_avenging_fury()
        : SpellScriptLoader("spell_onyx_brood_avenging_fury")
    {
    }
...
{
public:
    spell_vesperon_shadow_fissure()
        : SpellScriptLoader("spell_vesperon_shadow_fissure")
    {
    }
...
class spell_sartharion_twilight_shift : public AuraScript
{
    PrepareAuraScript(spell_sartharion_twilight_shift)
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*handle*/)
    {
        ...
        target->RemoveAurasDueToSpell(SPELL_TWILIGHT_TORMENT_DEBUFF_SARTHARION, 0, 0, AURA_REMOVE_BY_ENEMY_SPELL);
        target->RemoveAurasDueToSpell(SPELL_TWILIGHT_TORMENT_DEBUFF_ALONE, 0, 0, AURA_REMOVE_BY_ENEMY_SPELL);
    }

    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_sartharion_twilight_shift::OnApply, EFFECT_0, SPELL_AURA_PHASE, AURA_EFFECT_HANDLE_REAL);
    }
};

// 61187 - Twilight Shift
class spell_sartharion_twilight_shift_to_normal : public AuraScript
{
    PrepareAuraScript(spell_sartharion_twilight_shift_to_normal)
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*handle*/)
    {
        ...
        for (auto const& ref : target->GetMap()->GetPlayers())
        {
            Player* player = ref.GetSource();
            if (player->IsGameMaster())
                continue;

            for (uint32 const auraID : { SPELL_TWILIGHT_TORMENT_DEBUFF_SARTHARION, SPELL_TWILIGHT_TORMENT_DEBUFF_ALONE })
            {
                if (player->HasAura(auraID))
                {
                    target->AddAura(auraID, target);
                    break;
                }
            }
        }
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_sartharion_twilight_shift_to_normal::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

// 57557 - Pyrobuffet
class spell_pyrobuffet_damage final : public SpellScript
{
    PrepareSpellScript(spell_pyrobuffet_damage)

    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (InstanceScript const* instance = GetCaster()->GetInstanceScript())
            if (instance->GetData(DATA_SARTHARION_ENRAGE))
                return;

        Trinity::Containers::EraseIf(targets, [caster = GetCaster()](WorldObject const* target) { return target->GetExactDist(3248.231f, 527.584f, 58.936f) < 50.0f; });
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_pyrobuffet_damage::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
...
{
public:
    achievement_realm_first_obsidian_slayer()
        : AchievementCriteriaScript("achievement_realm_first_obsidian_slayer")
    {
    }
...
void AddSC_boss_sartharion()
{
    ...
    RegisterAuraScript(spell_sartharion_twilight_shift);
    RegisterAuraScript(spell_sartharion_twilight_shift_to_normal);
    RegisterSpellScript(spell_pyrobuffet_damage);
