    ...
    struct npc_haiphoonAI : public VehicleAI
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            if (spell->Id == SPELL_DEVOUR_WIND && me->GetCharmerOrOwnerPlayerOrPlayerItself())
                me->UpdateEntry(NPC_SOWAW_WIND_ELEMENTAL);
            else if (spell->Id == SPELL_DEVOUR_WATER && me->GetCharmerOrOwnerPlayerOrPlayerItself())
                me->UpdateEntry(NPC_SOWAW_WATER_ELEMENTAL);
        }
    };
...
public:
    npc_artruis_the_hearthless()
        : CreatureScript("npc_artruis_the_hearthless")
    {
    }
    ...
    struct npc_artruis_the_hearthlessAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (Creature* cr = me->SummonCreature(NPC_JALOOT, 5616.91f, 3772.67f, -94.26f, 1.78f))
            {
                summons.Summon(cr);
                cr->CastSpell(cr, SPELL_TOMB_OF_THE_HEARTLESS, true);
                cr->setFaction(me->getFaction());
            }
            if (Creature* cr = me->SummonCreature(NPC_ZEPIK, 5631.63f, 3794.36f, -92.24f, 3.45f))
            {
                summons.Summon(cr);
                cr->CastSpell(cr, SPELL_TOMB_OF_THE_HEARTLESS, true);
                cr->setFaction(me->getFaction());
        ...
        {
            me->MonsterYell("Ah, the heroes. Your little friends said you would come. This certainly saves me the trouble of hunting you down myself.", LANG_UNIVERSAL, nullptr);
            ...
            if (!summons.empty())
            {
                ...
                for (std::list<uint64>::const_iterator itr = summons.begin(); itr != summons.end(); ++itr)
                {
                    ...
                    if (minion && minion->IsAlive())
                    {
                        ...
                        {
                            minion->MonsterSay("Now you not catch us with back turned! Now we hurt you bad undead. BAD!", LANG_UNIVERSAL, nullptr);
            ...
            switch (events.GetEvent())
            {
                case EVENT_ARTRUIS_HP_CHECK:
                    if (me->GetHealthPct() <= 30)
                    {
                        ...
                        me->MonsterTextEmote("Artruis is shielded. You must choose your side quickly to break his spell.", nullptr, true);
                ...
                case EVENT_ARTRUIS_TALK1:
                    me->MonsterYell("I have weathered a hundred years of war and suffering. Do you truly think it wise to pit your mortal bodies against a being that cannot die? I'd venture you have more to lose.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    events.RescheduleEvent(EVENT_ARTRUIS_TALK2, 10000);
                    break;
                case EVENT_ARTRUIS_TALK2:
                    me->MonsterYell("Even shattered into countless pieces, the crystals all around weaken me... perhaps i should not have underestimated the titans so...", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    break;
                case EVENT_ARTRUIS_TALK3:
                    me->MonsterYell("Arthas once mustered strength... of the very same sort... perhaps he is the path that you will follow.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    break;
...
public:
    npc_still_at_it_trigger()
        : CreatureScript("npc_still_at_it_trigger")
    {
    }
    ...
    struct npc_still_at_it_triggerAI : public NullCreatureAI
    {
        ...
        void Say(const char* text)
        {
            if (Creature* th = ObjectAccessor::GetCreature(*me, thunderbrewGUID))
                th->MonsterSay(text, LANG_UNIVERSAL, nullptr);
            else
                Reset();
        }
        ...
        void SpellHit(Unit* caster, const SpellInfo* spellInfo) override // for banana(51932), orange(51931), papaya(51933) override
        {
            if (running)
            {
                uint8 a = 0;
...
public:
    go_pressure_valve()
        : GameObjectScript("go_pressure_valve")
    {
    }
...
public:
    npc_avatar_of_freya()
        : CreatureScript("npc_avatar_of_freya")
    {
    }
...
public:
    npc_bushwhacker()
        : CreatureScript("npc_bushwhacker")
    {
    }
...
public:
    npc_engineer_helice()
        : CreatureScript("npc_engineer_helice")
    {
    }
...
public:
    npc_jungle_punch_target()
        : CreatureScript("npc_jungle_punch_target")
    {
    }
...
public:
    npc_adventurous_dwarf()
        : CreatureScript("npc_adventurous_dwarf")
    {
    }
...
public:
    spell_q12589_shoot_rjr()
        : SpellScriptLoader("spell_q12589_shoot_rjr")
    {
    }
...
public:
    npc_vics_flying_machine()
        : CreatureScript("npc_vics_flying_machine")
    {
    }
...
public:
    spell_shango_tracks()
        : SpellScriptLoader("spell_shango_tracks")
    {
    }
...
{
public:
    npc_rejek_sholazar_event()
        : CreatureScript("npc_rejek_sholazar_event")
    {
    }

    struct npc_rejek_sholazar_event_AI : public ScriptedAI
    {
        npc_rejek_sholazar_event_AI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
    npc_hardknuckle_matriarchAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
public:
    go_raised_mud_scholazar_river()
        : GameObjectScript("go_raised_mud_scholazar_river")
    {
    }
...
public:
    npc_mystery_crystal()
        : CreatureScript("npc_mystery_crystal")
    {
    }

    struct npc_mystery_crystalAI : public NullCreatureAI
    {
        npc_mystery_crystalAI(Creature* creature)
