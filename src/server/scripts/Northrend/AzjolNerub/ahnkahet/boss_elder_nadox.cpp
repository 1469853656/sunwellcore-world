...
{
    boss_elder_nadoxAI(Creature* creature)
        : BossAI(creature, BOSS_NADOX)
    {
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellSchoolMask /*schoolMask*/) override
    {
        ...
        if (_preNerf)
        {
            healthCheck = (me->HealthBelowPctDamaged(75, damage) && _guardianCount == 0) || (me->HealthBelowPctDamaged(50, damage) && _guardianCount == 1) || (me->HealthBelowPctDamaged(25, damage) && _guardianCount == 2);
        }
        else
...
{
    npc_nadox_addAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_ahnkahar_nerubianAI(Creature* creature)
        : npc_nadox_addAI(creature)
    {
    }
...
{
    npc_nadox_eggAI(Creature* creature)
        : npc_nadox_addAI(creature)
    {
