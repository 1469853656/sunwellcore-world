...
struct boss_hadronoxAI : public BossAI
{
            ...
            if (Unit const* target = hostileRef->getTarget())
                if (target->GetCharmerOrOwnerPlayerOrPlayerItself())
                {
                    ...
                    if (_step < NUM_STEPS - 1)
                        return true;
                    if (!_doorsWebbed && me->GetPositionZ() > 725.7f)
                    {
    ...
    bool AnyPlayerValid() const
    {
        for (auto const& ref : instance->instance->GetPlayers())
        {
            ...
            if (me->GetDistance(player) < 130.0f && player->IsAlive() && !player->IsGameMaster() && me->CanCreatureAttack(player) && me->_CanDetectFeignDeathOf(player))
                return true;
        }
    ...
    {
        events.ScheduleEvent(EVENT_LEECH_POISON, urand(5000, 7000));
        events.ScheduleEvent(EVENT_ACID_CLOUD, urand(7000, 13000));
        events.ScheduleEvent(EVENT_WEB_GRAB, urand(13000, 19000));
        events.ScheduleEvent(EVENT_PIERCE_ARMOR, urand(4000, 7000));
    ...
    void EnterEvadeMode() override
    {
        ...
        me->GetCreatureListWithEntryInGrid(triggers, NPC_WORLDTRIGGER_LARGE, 250.0f);
    ...
    void SetGUID(uint64 guid, int32 /*what*/) override
    {
            ...
            switch (eventId)
            {
                ...
                case EVENT_PLAYER_CHECK:
                    ...
                    if (IsInCombatWithPlayer() != _lastPlayerCombatState)
                    {
                        ...
                        {
                            if (!instance->CheckRequiredBosses(DATA_HADRONOX_EVENT))
                            {
                                EnterEvadeMode();
                                return;
                            }
...
struct npc_hadronox_crusherPackAI : public ScriptedAI
{
    ...
    void EnterEvadeMode() override
    {
        if (Creature* hadronox = _instance->GetCreature(DATA_HADRONOX_EVENT))
            if (hadronox->IsAIEnabled)
                hadronox->AI()->EnterEvadeMode();
    }
...
{
    npc_anub_ar_crusherAI(Creature* creature)
        : npc_hadronox_crusherPackAI(creature, crusherWaypoints)
        , _hadFrenzy(false)
    {
    }
    ...
    {
        _events.ScheduleEvent(EVENT_SMASH, urand(8000, 12000));
    ...
    void DoEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_SMASH:
                ...
                _events.Repeat(urand(13000, 21000));
                break;
        }
    }
    ...
    {
        if (Creature* hadronox = _instance->GetCreature(DATA_HADRONOX_EVENT))
            if (hadronox->IsAIEnabled && _instance->GetBossState(DATA_HADRONOX_EVENT) != DONE)
...
{
    npc_anub_ar_crusher_championAI(Creature* creature)
        : npc_hadronox_crusherPackAI(creature, championWaypoints)
    {
    }
    ...
    void DoEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_REND:
                ...
                _events.Repeat(urand(12000, 16000));
                break;
            case EVENT_PUMMEL:
                DoCastVictim(SPELL_PUMMEL);
                _events.Repeat(urand(12000, 17000));
                break;
        }
    }
    ...
    void _EnterCombat() override
    {
        _events.ScheduleEvent(EVENT_REND, urand(4000, 8000));
        _events.ScheduleEvent(EVENT_PUMMEL, urand(15000, 19000));
    }
};
...
{
    npc_anub_ar_crusher_crypt_fiendAI(Creature* creature)
        : npc_hadronox_crusherPackAI(creature, cryptFiendWaypoints)
    {
    }
    ...
    void DoEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_INFECTED_WOUND:
                ...
                _events.Repeat(urand(16000, 25000));
                break;
        }
    }
    ...
    void _EnterCombat() override
    {
        _events.ScheduleEvent(EVENT_CRUSHING_WEBS, urand(4000, 8000));
        _events.ScheduleEvent(EVENT_INFECTED_WOUND, urand(15000, 19000));
    }
};
...
{
    npc_anub_ar_crusher_necromancerAI(Creature* creature)
        : npc_hadronox_crusherPackAI(creature, necromancerWaypoints)
    {
    }
    ...
    void _EnterCombat() override
    {
        _events.ScheduleEvent(EVENT_SHADOW_BOLT, urand(2000, 4000));
        _events.ScheduleEvent(EVENT_ANIMATE_BONES, urand(37000, 45000));
    }
};
        ...
        if (_nextMovement)
        {
            switch (_nextMovement)
            {
                ...
                case MOVE_HADRONOX_REAL:
                {
                    ...
                    Creature* hadronox         = _instance->GetCreature(DATA_HADRONOX_EVENT);
...
{
    npc_anub_ar_championAI(Creature* creature)
        : npc_hadronox_foeAI(creature)
    {
    }
    ...
    void DoEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_TAUNT:
                ...
                _events.Repeat(urand(15000, 50000));
                break;
        }
    }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        _events.ScheduleEvent(EVENT_REND, urand(4000, 8000));
        _events.ScheduleEvent(EVENT_PUMMEL, urand(15000, 19000));
        _events.ScheduleEvent(EVENT_TAUNT, urand(15000, 50000));
    }
};

struct npc_anub_ar_crypt_fiendAI : public npc_hadronox_foeAI
{
    npc_anub_ar_crypt_fiendAI(Creature* creature)
        : npc_hadronox_foeAI(creature)
    {
    }
...
{
    npc_anub_ar_necromancerAI(Creature* creature)
        : npc_hadronox_foeAI(creature)
    {
    }
...
public:
    ...
    PrepareAuraScript(spell_hadronox_periodic_summon_template_AuraScript)
        ...
        private : bool Validate(SpellInfo const* /*spell*/) override
    {
...
public:
    spell_hadronox_periodic_summon_champion_AuraScript()
        : spell_hadronox_periodic_summon_template_AuraScript(SPELL_SUMMON_CHAMPION_TOP, SPELL_SUMMON_CHAMPION_BOTTOM)
    {
    }
};
...
public:
    spell_hadronox_periodic_summon_crypt_fiend_AuraScript()
        : spell_hadronox_periodic_summon_template_AuraScript(SPELL_SUMMON_CRYPT_FIEND_TOP, SPELL_SUMMON_CRYPT_FIEND_BOTTOM)
    {
    }
};
...
public:
    spell_hadronox_periodic_summon_necromancer_AuraScript()
        : spell_hadronox_periodic_summon_template_AuraScript(SPELL_SUMMON_NECROMANCER_TOP, SPELL_SUMMON_NECROMANCER_BOTTOM)
    {
    }
};
...
{
    new CreatureAILoader<boss_hadronoxAI>("boss_hadronox");
    new CreatureAILoader<npc_anub_ar_crusherAI>("npc_anub_ar_crusher");
    new CreatureAILoader<npc_anub_ar_crusher_championAI>("npc_anub_ar_crusher_champion");
    new CreatureAILoader<npc_anub_ar_crusher_crypt_fiendAI>("npc_anub_ar_crusher_crypt_fiend");
    new CreatureAILoader<npc_anub_ar_crusher_necromancerAI>("npc_anub_ar_crusher_necromancer");
    new CreatureAILoader<npc_anub_ar_championAI>("npc_anub_ar_champion");
    new CreatureAILoader<npc_anub_ar_crypt_fiendAI>("npc_anub_ar_crypt_fiend");
    new CreatureAILoader<npc_anub_ar_necromancerAI>("npc_anub_ar_necromancer");
    new AuraScriptLoaderEx<spell_hadronox_periodic_summon_champion_AuraScript>("spell_hadronox_periodic_summon_champion");
    new AuraScriptLoaderEx<spell_hadronox_periodic_summon_crypt_fiend_AuraScript>("spell_hadronox_periodic_summon_crypt_fiend");
    new AuraScriptLoaderEx<spell_hadronox_periodic_summon_necromancer_AuraScript>("spell_hadronox_periodic_summon_necromancer");
    new AuraScriptLoaderEx<spell_hadronox_leeching_poison_AuraScript>("spell_hadronox_leeching_poison");
    new SpellScriptLoaderEx<spell_hadronox_web_doors_SpellScript>("spell_hadronox_web_doors");
