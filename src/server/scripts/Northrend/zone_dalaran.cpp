...
{
public:
    npc_mei_francis_mount()
        : CreatureScript("npc_mei_francis_mount")
    {
    }

    struct npc_mei_francis_mountAI : public ScriptedAI
    {
        npc_mei_francis_mountAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_shandy_dalaran()
        : CreatureScript("npc_shandy_dalaran")
    {
    }

    struct npc_shandy_dalaranAI : public ScriptedAI
    {
        npc_shandy_dalaranAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_archmage_landalock()
        : CreatureScript("npc_archmage_landalock")
    {
    }
    ...
    {
        npc_archmage_landalockAI(Creature* creature)
            : ScriptedAI(creature)
        {
...
public:
    npc_archmage_timear()
        : CreatureScript("npc_archmage_timear")
    {
    }
    ...
    {
        npc_archmage_timearAI(Creature* creature)
            : ScriptedAI(creature)
        {
            _switchImageTimer = MINUTE * IN_MILLISECONDS;
            _summonGUID       = 0;
...
public:
    npc_mageguard_dalaran()
        : CreatureScript("npc_mageguard_dalaran")
    {
    }
    ...
    struct npc_mageguard_dalaranAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!player || player->IsGameMaster() || player->IsBeingTeleported() || (player->GetPositionZ() > 670 && player->GetVehicle()) ||
                ...
                player->HasAura(SPELL_SUNREAVER_DISGUISE_FEMALE) || player->HasAura(SPELL_SUNREAVER_DISGUISE_MALE) || player->HasAura(SPELL_SILVER_COVENANT_DISGUISE_FEMALE) || player->HasAura(SPELL_SILVER_COVENANT_DISGUISE_MALE))
                return;

            switch (me->GetEntry())
            {
                case NPC_SILVER_COVENANT_GUARDIAN_MAGE:
                    if (player->GetTeam(CF_DENY) == TEAM_HORDE) // Horde unit found in Alliance area
                    {
                ...
                case NPC_SUNREAVER_GUARDIAN_MAGE:
                    if (player->GetTeam(CF_DENY) == TEAM_ALLIANCE) // Alliance unit found in Horde area
                    {
...
public:
    npc_minigob_manabonk()
        : CreatureScript("npc_minigob_manabonk")
    {
    }

    struct npc_minigob_manabonkAI : public ScriptedAI
    {
        npc_minigob_manabonkAI(Creature* creature)
            : ScriptedAI(creature)
        {
            me->setActive(true);
        }
...
enum marciaEnum
{
    NPC_GENERIC_LAB_TRIGGER = 28332,
    DALARAN_ZONE            = 4395,
    SKYBOX_DEFAULT          = 1802,
    SKYBOX_EVENT            = 1773
};

class npc_marcia_dalaran_skybox : public CreatureScript
{
public:
    npc_marcia_dalaran_skybox()
        : CreatureScript("npc_marcia_dalaran_skybox")
    {
    }

    struct npc_marcia_dalaran_skybox_AI : public ScriptedAI
    {
        npc_marcia_dalaran_skybox_AI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        ...
        void SetData(uint32 type, uint32 value) override
        {
            if (type == 1 && value == 1)
            {
                ...
                me->GetCreatureListWithEntryInGrid(tempList, NPC_GENERIC_LAB_TRIGGER, 300.0f);
                for (auto i : tempList)
                    if (Creature* creature = i->ToCreature())
                        creature->setActive(true);
            }
            else if (type == 2 && value == 2)
            {
                WorldPacket data(SMSG_OVERRIDE_LIGHT, 12);
                data << uint32(SKYBOX_DEFAULT); // Light.dbc entry (map default)
                data << uint32(SKYBOX_EVENT);   // Light.dbc entry (override)
                data << uint32(500);
            ...
            else if (type == 3 && value == 3)
            {
                ...
                WorldPacket data(SMSG_OVERRIDE_LIGHT, 12);
                data << uint32(SKYBOX_DEFAULT); // Light.dbc entry (map default)
                data << uint32(0);              // Light.dbc entry (override)
                data << uint32(5000);
                SendPacketToPlayers(&data);
            }
        }
            ...
            if (!players.isEmpty())
                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    if (Player* player = itr->GetSource())
                        if (player->GetZoneId() == DALARAN_ZONE)
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_marcia_dalaran_skybox_AI(creature);
    }
};
