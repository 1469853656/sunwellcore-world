...
public:
    ...
    struct boss_uromAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer) override
        {
            ...
            if (me->GetPositionZ() > 510.0f && x && y && z)
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            switch (spell->Id)
            {
                ...
                case SPELL_SUMMON_MENAGERIE_3:
                {
                    ...
                    uint8 phase = GetPhaseByCurrentPosition();
                    int8 prevPhase = std::max(0, phase - 1);
                    int8 nextPhase = phase + 1;
                    if (nextPhase > 3)
                        nextPhase = 0;

                    for (uint8 i = 0; i < 4; ++i)
                        me->SummonCreature(summons[prevPhase][i], cords[prevPhase][0] + ((i % 2) ? 4.0f : -4.0f), cords[prevPhase][1] + (i < 2 ? 4.0f : -4.0f), cords[prevPhase][2], 0.0f, TEMPSUMMON_TIMED_DESPAWN, 300000);
                    me->SetHomePosition(cords[nextPhase][0], cords[nextPhase][1], cords[nextPhase][2], cords[nextPhase][3]);
