...
public:
    ...
    struct boss_eregosAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            events.Update(diff);
            if (me->HasUnitState(UNIT_STATE_CASTING) || me->HasAura(SPELL_DRAKE_STOP_TIME))
                return;
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SPELL_PLANAR_SHIFT:
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                        if (Unit* t = SelectTarget(SELECT_TARGET_RANDOM, 0, [&](Unit* tar) -> bool { return !tar->IsTotem() && tar->IsVehicle(); }))
