    ...
    struct boss_malygosAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_POWER_SPARK_MALYGOS_BUFF)
            {
                if (!bLockHealthCheck)
                {
                    me->MonsterYell("I AM UNSTOPPABLE!", LANG_UNIVERSAL, nullptr);
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            me->MonsterYell("My patience has reached it's limit, I will be rid of you!", LANG_UNIVERSAL, nullptr);
            ...
            if (pInstance)
            {
                ...
                if (!Is25ManRaid())
                    me->SetLootMode(GetLootModeFor10Man(pInstance));
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SPELL_ARCANE_STORM:
                    me->CastCustomSpell(SPELL_ARCANE_STORM, SPELLVALUE_MAX_TARGETS, RAID_MODE(5, 10), me, true);
                ...
                case EVENT_SUMMON_POWER_SPARK:
                {
                    ...
                    me->MonsterTextEmote("A Power Spark forms from a nearby rift!", nullptr, true);
                ...
                {
                    bLockHealthCheck = true;
                    me->MonsterYell("Watch helplessly as your hopes are swept away...", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_START_PHASE_2:
                    me->MonsterYell("I had hoped to end your lives quickly, but you have proven more...resilient then I had anticipated. Nonetheless, your efforts are in vain, it is you reckless, careless mortals who are to blame for this war! I do what I must...And if it means your...extinction...THEN SO BE IT!", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_START_PHASE_2_MOVE_TO_SIDE:
                    me->MonsterYell("Few have experienced the pain I will now inflict upon you!", LANG_UNIVERSAL, nullptr);
                ...
                {
                    me->MonsterYell("You will not succeed while i draw breath!", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_SURGE_OF_POWER_WARNING:
                    me->MonsterTextEmote("Malygos takes a deep breath.", nullptr, true);
                    events.PopEvent();
                ...
                case EVENT_CHECK_TRASH_DEAD:
                {
                    ...
                    else
                    {
                        ...
                        summons.DespawnAll();
                        // start phase 3
                        me->MonsterYell("ENOUGH! If you intend to reclaim Azeroth\'s magic, then you shall have it...", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_MOVE_TO_PHASE_3_POSITION:
                {
                    ...
                    if (sCustomMgr->GetBoolConfig(CONFIG_EYE_OF_ETERNITY_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid())
                        events.ScheduleEvent(EVENT_HEALING_REDUCE, 5s);
                }
                break;
                case EVENT_SAY_PHASE_3_INTRO:
                    me->MonsterYell("Now your benefactors make their appearance...But they are too late. The powers contained here are sufficient to destroy the world ten times over! What do you think they will do to you?", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_START_PHASE_3:
                    ...
                    events.RescheduleEvent(EVENT_SPELL_STATIC_FIELD, urand(4000, 6000), 1);
                ...
                {
                    uint32 count = sCustomMgr->GetBoolConfig(CONFIG_EYE_OF_ETERNITY_25_BOOST_ENABLED) && me->GetMap()->Is25ManRaid() ? 2 : 1;
                ...
                case EVENT_SPELL_PH3_SURGE_OF_POWER:
                    DoCastSelf(SPELL_PH3_SURGE_OF_POWER);
        ...
        {
            me->MonsterYell("UNTHINKABLE! The mortals will destroy... e-everything... my sister... what have you-", LANG_UNIVERSAL, nullptr);
        ...
        void HandleIntroSpeech(const uint32 diff)
        {
            ...
            if (timer1 <= diff)
            {
                ...
                me->MonsterYell(MalygosIntroTexts[IntroCounter].text, LANG_UNIVERSAL, nullptr);
...
public:
    npc_vortex_ride()
        : CreatureScript("npc_vortex_ride")
    {
    }
                ...
                if (me->GetVehicleKit())
                    if (Unit* pass = me->GetVehicleKit()->GetPassenger(0))
                        if (Player* plr = pass->ToPlayer())
                        {
                            ...
                            data << plr->GetPositionX() << plr->GetPositionY() << plr->GetPositionZ() + plr->GetHoverHeight();
                            data << uint32(World::GetGameTimeMS());
                            data << uint8(0);
                            data << uint32(SPLINEFLAG_FLYING);
                            data << VORTEX_DEFAULT_DIFF * 2;       // Time in between points
                            data << uint32(1);                     // 1 single waypoint
                            data << me->GetPositionX() + dist * cos(arcangle) << me->GetPositionY() + dist * sin(arcangle) << me->GetPositionZ(); // the single waypoint Point B
...
public:
    npc_power_spark()
        : CreatureScript("npc_power_spark")
    {
    }
    ...
    {
        npc_power_sparkAI(Creature* pCreature)
...
public:
    ...
    {
        npc_nexus_lordAI(Creature* pCreature)
        ...
        void Reset() override
        {
            me->SetFloatValue(UNIT_FIELD_COMBATREACH, me->GetCombatReach() * 2.f);
        }
...
public:
    npc_scion_of_eternity()
        : CreatureScript("npc_scion_of_eternity")
    {
    }
    ...
    {
        npc_scion_of_eternityAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            me->SetReactState(REACT_PASSIVE);
            pInstance = me->GetInstanceScript();
...
public:
    npc_alexstrasza()
        : CreatureScript("npc_alexstrasza")
    {
    }
    ...
    {
        npc_alexstraszaAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            events.Reset();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case 1:
                    ...
                    me->MonsterYell("I did what I had to, brother. You gave me no alternative.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    events.RescheduleEvent(2, 6000);
                    break;
                case 2:
                    me->PlayDirectSound(14407);
                    me->MonsterYell("And so ends the Nexus War.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    events.RescheduleEvent(3, 5000);
                    break;
                case 3:
                    me->PlayDirectSound(14408);
                    me->MonsterYell("This resolution pains me deeply, but the destruction, the monumental loss of life had to end. Regardless of Malygos\' recent transgressions, I will mourn his loss. He was once a guardian, a protector. This day, one of the world\'s mightiest has fallen.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    events.RescheduleEvent(4, 22000);
                    break;
                case 4:
                    me->PlayDirectSound(14409);
                    me->MonsterYell("The red dragonflight will take on the burden of mending the devastation wrought on Azeroth. Return home to your people and rest. Tomorrow will bring you new challenges, and you must be ready to face them. Life... goes on.", LANG_UNIVERSAL, nullptr);
                    events.PopEvent();
                    break;
            }
        }
...
{
public:
    npc_eoe_wyrmrest_skytalon()
        : CreatureScript("npc_eoe_wyrmrest_skytalon")
    {
    }
    ...
    {
        npc_eoe_wyrmrest_skytalonAI(Creature* pCreature)
            : VehicleAI(pCreature)
        {
        }
...
{
public:
    go_the_focusing_iris()
        : GameObjectScript("go_the_focusing_iris")
    {
    }
...
{
public:
    go_exit_portal_malygos()
        : GameObjectScript("go_exit_portal_malygos")
    {
    }
