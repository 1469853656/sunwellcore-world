...
constexpr uint32 BetterOfDredRaptorAmount = 6;

class boss_dred : public CreatureScript
{
public:
    boss_dred()
        : CreatureScript("boss_dred")
    {
    }
    ...
    {
        boss_dredAI(Creature* creature)
            : BossAI(creature, DATA_DRED)
        {
        }
        ...
        {
            if (data == me->GetEntry())
                return uint32(_raptorCount >= BetterOfDredRaptorAmount);
...
{
public:
    spell_dred_grievious_bite()
        : SpellScriptLoader("spell_dred_grievious_bite")
    {
    }
...
public:
    spell_dred_raptor_call()
        : SpellScriptLoader("spell_dred_raptor_call")
    {
    }
