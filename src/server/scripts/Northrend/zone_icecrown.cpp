...
{
public:
    npc_black_knight_graveyard()
        : CreatureScript("npc_black_knight_graveyard")
    {
    }

    struct npc_black_knight_graveyardAI : public ScriptedAI
    {
        npc_black_knight_graveyardAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_battle_at_valhalas()
        : CreatureScript("npc_battle_at_valhalas")
    {
    }
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        ...
        switch (quest->GetQuestId())
        {
            ...
            case QUEST_BFV_SIGRID:
                ...
                break;
            case QUEST_BFV_CARNAGE:
                creature->MonsterSay("Carnage is coming! Remember, no matter what you do, do NOT leave the battle ring or I will disqualify you and your group.", LANG_UNIVERSAL, nullptr);
                break;
            case QUEST_BFV_THANE:
                creature->MonsterSay("Look to the southeast and you will see the thane upon the platform near Gjonner the Merciless when he shows himself. Let him come down. Stay within the ring of Valhalas.", LANG_UNIVERSAL, nullptr);
                break;
            case QUEST_BFV_FINAL:
                creature->MonsterSay("It's too late to run now. Do not leave the ring. Die bravely, $N!", LANG_UNIVERSAL, nullptr);
                break;
        }
...
public:
    npc_llod_generic()
        : CreatureScript("npc_llod_generic")
    {
    }

    struct npc_llod_genericAI : public CombatAI
    {
        npc_llod_genericAI(Creature* pCreature)
            : CombatAI(pCreature)
        {
            attackTimer = 0;
            summonTimer = 1;
...
{
public:
    npc_boneguard_footman()
        : CreatureScript("npc_boneguard_footman")
    {
    }

    struct npc_boneguard_footmanAI : ScriptedAI
    {
        npc_boneguard_footmanAI(Creature* creature)
            : ScriptedAI(creature)
        {
            checkTimer = 0;
        }
...
public:
    npc_tirions_gambit_tirion()
        : CreatureScript("npc_tirions_gambit_tirion")
    {
    }
...
{
public:
    spell_fight_fire_bomber()
        : SpellScriptLoader("spell_fight_fire_bomber")
    {
    }
...
public:
    npc_frostbrood_skytalon()
        : CreatureScript("npc_frostbrood_skytalon")
    {
    }

    struct npc_frostbrood_skytalonAI : public VehicleAI
    {
        npc_frostbrood_skytalonAI(Creature* creature)
            : VehicleAI(creature)
        {
        }
...
{
public:
    npc_cauldron_target_basic_chemistry()
        : CreatureScript("npc_cauldron_target_basic_chemistry")
    {
    }

    struct npc_cauldron_target_basic_chemistry_AI : public ScriptedAI
    {
        npc_cauldron_target_basic_chemistry_AI(Creature* creature)
            : ScriptedAI(creature)
            , _summons(me)
        {
        }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell) override
        {
            ...
            if (spell->Id == SPELL_ITEM_SPELL_DOSE && caster->GetTypeId() == TYPEID_PLAYER && (caster->ToPlayer()->GetQuestStatus(QUEST_BASIC_CHEMISTRY_1) == QUEST_STATUS_INCOMPLETE || caster->ToPlayer()->GetQuestStatus(QUEST_BASIC_CHEMISTRY_2) == QUEST_STATUS_INCOMPLETE || caster->ToPlayer()->GetQuestStatus(QUEST_NEUTRALIZING_PLAGUE_1) == QUEST_STATUS_INCOMPLETE || caster->ToPlayer()->GetQuestStatus(QUEST_NEUTRALIZING_PLAGUE_2) == QUEST_STATUS_INCOMPLETE))
            {
                if (!_eventInProgress)
                {
...
{
public:
    npc_blastbolt_brother()
        : CreatureScript("npc_blastbolt_brother")
    {
    }
...
{
    npc_battle_priestAI(Creature* creature)
        : ScriptedAI(creature)
    {
...
struct npc_jotunheim_proto_drake final : ScriptedAI
{
    explicit npc_jotunheim_proto_drake(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
void AddSC_icecrown()
{
    ...
    RegisterCreatureAI(npc_jotunheim_proto_drake);
}
