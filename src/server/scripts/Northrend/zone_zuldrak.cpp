...
public:
    npc_finklestein()
        : CreatureScript("npc_finklestein")
    {
    }
...
{
public:
    go_finklestein_cauldron()
        : GameObjectScript("go_finklestein_cauldron")
    {
    }
...
public:
    npc_feedin_da_goolz()
        : CreatureScript("npc_feedin_da_goolz")
    {
    }
    ...
    {
        npc_feedin_da_goolzAI(Creature* creature)
...
public:
    npc_drakuru_shackles()
        : CreatureScript("npc_drakuru_shackles")
    {
    }
...
public:
    npc_captured_rageclaw()
        : CreatureScript("npc_captured_rageclaw")
    {
    }

    struct npc_captured_rageclawAI : public NullCreatureAI
    {
        npc_captured_rageclawAI(Creature* creature)
            : NullCreatureAI(creature)
        {
        }
...
public:
    npc_crusade_recruit()
        : CreatureScript("npc_crusade_recruit")
    {
    }

    struct npc_crusade_recruitAI : public ScriptedAI
    {
        npc_crusade_recruitAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_storm_cloud()
        : CreatureScript("npc_storm_cloud")
    {
    }

    struct npc_storm_cloudAI : public ScriptedAI
    {
        npc_storm_cloudAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
    npc_overlord_drakuru_midlevel_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_drakkari_skullcrusherAI(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
public:
    spell_q12709_hexed_cache()
        : SpellScriptLoader("spell_q12709_hexed_cache")
    {
    }
...
enum SoFarSoBad
{
    SPELL_SUMMON_ALARMED_BLIGHTGUARD = 52232,

    SPELL_BLIGHTGUARD_INVISIBILITY = 52060,
    SPELL_BLIGHTGUARD_SHADOWSTRIKE = 33914
};

struct npc_alarmed_blightguard final : ScriptedAI
{
    npc_alarmed_blightguard(Creature* creature) : ScriptedAI(creature) { }

    void Reset() override
    {
        _scheduler.CancelAll();
        DoCastSelf(SPELL_BLIGHTGUARD_INVISIBILITY, true);
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        {
            DoCastVictim(SPELL_BLIGHTGUARD_SHADOWSTRIKE, true);
            task.Repeat(6s, 10s);
        });
    }
...
// 52227 - Dilute Blight Cauldron
class spell_dilute_blight_cauldron final : public SpellScript
{
    PrepareSpellScript(spell_dilute_blight_cauldron)
    ...
    void HandleDummyEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        target->m_Events.AddEventAtOffset([target] { target->CastSpell(target, SPELL_SUMMON_ALARMED_BLIGHTGUARD, true); }, 1s);
        target->m_Events.AddEventAtOffset([target] { target->CastSpell(target, SPELL_SUMMON_ALARMED_BLIGHTGUARD, true); }, 2s);
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_dilute_blight_cauldron::HandleDummyEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
void AddSC_zuldrak()
{
    ...
    RegisterCreatureAI(npc_alarmed_blightguard);
    RegisterSpellScript(spell_dilute_blight_cauldron);
}
