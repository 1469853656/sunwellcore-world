                ...
                case 12:
                    ...
                    for (SummonList::iterator itr = _summons.begin(); itr != _summons.end(); ++itr)
                        if (Creature* summon = _instance->instance->GetCreature(*itr))
                        {
                            ...
                            summon->Respawn(true);
                            summon->UpdateEntry(36796, nullptr, false);
...
struct npc_pos_deathwhisper_necrolyteAI : public ScriptedAI
{
    ...
    void InitializeAI() override
    {
        ...
        {
            if (me->GetPositionX() > 490.0f && me->GetPositionX() < 504.0f && me->GetPositionY() > 192.0f && me->GetPositionY() < 206.0f || me->GetPositionX() > 490.0f && me->GetPositionX() < 504.0f && me->GetPositionY() > 240.0f && me->GetPositionY() < 254.0f)
            {
                _immortal = true;
...
{
    npc_pos_icicle_triggerAI(Creature* creature)
        : NullCreatureAI(creature)
    {
        _instance = me->GetInstanceScript();
...
{
    npc_pos_freed_slaveAI(Creature* creature)
        : SmartAI(creature)
    {
...
{
    npc_frostbite_invisible_stalkerAI(Creature* creature)
        : NullCreatureAI(creature)
    {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (_events.ExecuteEvent() == EVENT_FREEZING_CIRCLE_DMG)
        {
            ...
            if (InstanceScript* instance = me->GetInstanceScript())
            {
                ...
                {
                    damage = 1500 * instance->GetMythicKeyLevel();
                    val.AddSpellMod(SPELLVALUE_AURA_DURATION, 5500);
                }
            }

            val.AddSpellMod(SPELLVALUE_BASE_POINT1, damage);
            val.AddSpellMod(SPELLVALUE_RADIUS_MOD, 7500); // match visual on the ground
            me->CastCustomSpell(34779, val, me, TRIGGERED_FULL_MASK);
        }
    }
        ...
        if (Unit* target = GetHitUnit())
            if (target->GetUInt32Value(UNIT_NPC_EMOTESTATE)) // prevent using multiple times
            {
                ...
                if (Creature* slave = target->ToCreature())
                {
                    ...
                    slave->MonsterSay(slaveTexts[urand(0, (slave->getGender() == GENDER_FEMALE ? 22 : 20))], LANG_UNIVERSAL, nullptr);
...
public:
    at_tyrannus_event_starter()
        : AreaTriggerScript("at_tyrannus_event_starter")
    {
    }
...
{
public:
    at_fos_entrance_intro()
        : AreaTriggerScript("at_fos_entrance_intro")
    {
    }
