...
struct boss_falricAI : public BossAI
{
    ...
    float GetHealthPCTForNextHopelessness() const
    {
        switch (_hopelessnessCount)
        {
            case 0:
                return 60;
            case 1:
    ...
    {
        float healthPct = GetHealthPCTForNextHopelessness();
        if (G3D::fuzzyEq(healthPct, 0.0f))
            return;
