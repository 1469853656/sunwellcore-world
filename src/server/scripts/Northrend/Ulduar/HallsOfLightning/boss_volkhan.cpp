...
public:
    npc_molten_golem()
        : CreatureScript("npc_molten_golem")
    {
    }
    ...
    struct npc_molten_golemAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit*, uint32& uiDamage, DamageEffectType, SpellSchoolMask) override
        {
            ...
            {
                me->UpdateEntry(NPC_BRITTLE_GOLEM, nullptr, false);
...
public:
    npc_hol_monument()
        : CreatureScript("npc_hol_monument")
    {
    }
    ...
    {
        npc_hol_monumentAI(Creature* creature)
            : ScriptedAI(creature)
        {
            _attackGUID = 0;
            _isActive   = urand(0, 1);
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            {
                if ((who->GetPositionX() > me->GetPositionX() || who->GetPositionY() < -220.0f) && me->GetDistance2d(who) < 30.0f)
                {
                    _isActive   = false;
