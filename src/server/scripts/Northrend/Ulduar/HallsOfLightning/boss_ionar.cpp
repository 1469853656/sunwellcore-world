...
public:
    ...
    struct boss_ionarAI : public ScriptedAI
    {
        ...
        void Split()
        {
            ...
            for (uint8 i = 0; i < 5; ++i)
            {
                if (Creature* spark = me->SummonCreature(NPC_SPARK_OF_IONAR, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 20000))
                {
...
public:
    npc_spark_of_ionar()
        : CreatureScript("npc_spark_of_ionar")
    {
    }
    ...
    {
        npc_spark_of_ionarAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
