...
public:
    brann_bronzebeard()
        : CreatureScript("brann_bronzebeard")
    {
    }
    ...
    struct brann_bronzebeardAI : public npc_escortAI
    {
        ...
        void DespawnHeads()
        {
            if (Creature* cr = GetAbedneum()) cr->DespawnOrUnsummon();
            if (Creature* cr = GetMarnak()) cr->DespawnOrUnsummon();
            if (Creature* cr = GetKaddrak()) cr->DespawnOrUnsummon();
        ...
        void SwitchHeadVisaul(uint8 headMask, bool activate)
        {
            ...
            if (headMask & 0x1) // Kaddrak
                if (GameObject* go = me->GetMap()->GetGameObject(pInstance->GetData64(GO_KADDRAK)))
                    activate ? go->SendCustomAnim(0) : go->SetGoState(GO_STATE_READY);

            if (headMask & 0x2) // Marnak
                if (GameObject* go = me->GetMap()->GetGameObject(pInstance->GetData64(GO_MARNAK)))
                    activate ? go->SendCustomAnim(0) : go->SetGoState(GO_STATE_READY);

            if (headMask & 0x4) // Abedneum
                if (GameObject* go = me->GetMap()->GetGameObject(pInstance->GetData64(GO_ABEDNEUM)))
                    activate ? go->SendCustomAnim(0) : go->SetGoState(GO_STATE_READY);
        }
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                case ACTION_START_EVENT:
                    Start(false, true, 0, nullptr, true, false);
                    break;
                ...
                case ACTION_START_SJONNIR_FIGHT:
                    me->MonsterYell("Don't worry! Ol' Brann's got yer back! Keep that metal monstrosity busy, and I'll see if I can't sweet talk this machine into helping ye!", LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(14274);
                    SetEscortPaused(false);
                    break;
                case ACTION_SJONNIR_DEAD:
                    me->MonsterYell("Loken? That's downright bothersome... We might've neutralized the iron dwarves, but I'd lay odds there's another machine somewhere else churnin' out a whole mess o' these iron vrykul!", LANG_UNIVERSAL, nullptr);
        ...
        void UpdateEscortAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_TRIBUNAL_END:
                {
                    ...
                    if (pInstance)
                    {
                        ...
                        if (me->GetMap()->IsDungeon() && me->GetMap()->IsRaidOrHeroicDungeon())
                ...
                case EVENT_END:
                {
                    ...
                    me->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP | UNIT_NPC_FLAG_QUESTGIVER);
                    me->MonsterYell("I'll use the forge to make batches o' earthen to stand guard... But our greatest challenge still remains: find and stop Loken!", LANG_UNIVERSAL, nullptr);
            ...
            if (TalkEvent)
            {
                ...
                if (SpeechPause >= Conversation[SpeechCount].timer)
                {
                    ...
                    if (cs)
                    {
                        cs->MonsterYell(Conversation[SpeechCount].text, LANG_UNIVERSAL, nullptr);
...
{
    if (Creature* cr = me->SummonCreature(NPC_KADDRAK, 923.7f, 326.9f, 219.5f, 2.1f, TEMPSUMMON_TIMED_DESPAWN, 580000))
    {
        cr->SetInCombatWithZone();
        KaddrakGUID = cr->GetGUID();
    }
    if (Creature* cr = me->SummonCreature(NPC_MARNAK, 895.974f, 363.571f, 219.337f, 5.5f, TEMPSUMMON_TIMED_DESPAWN, 580000))
    {
        cr->SetInCombatWithZone();
        MarnakGUID = cr->GetGUID();
    }
    if (Creature* cr = me->SummonCreature(NPC_ABEDNEUM, 892.25f, 331.25f, 223.86f, 0.6f, TEMPSUMMON_TIMED_DESPAWN, 580000))
    {
        cr->SetInCombatWithZone();
...
{
public:
    dark_rune_protectors()
        : CreatureScript("dark_rune_protectors")
    {
    }
    ...
    {
        dark_rune_protectorsAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
{
public:
    dark_rune_stormcaller()
        : CreatureScript("dark_rune_stormcaller")
    {
    }
    ...
    {
        dark_rune_stormcallerAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
{
public:
    iron_golem_custodian()
        : CreatureScript("iron_golem_custodian")
    {
    }
    ...
    {
        iron_golem_custodianAI(Creature* c)
            : ScriptedAI(c)
        {
        }
        EventMap events;
...
public:
    spell_hos_dark_matter()
        : SpellScriptLoader("spell_hos_dark_matter")
    {
    }
