        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_CHECK_HEALTH:
                {
                    if (SummonPhase == PHASE_SUMMON_UNFRIENDLY_DWARFES && HealthBelowPct(50))
                    {
                        ...
                        if (pInstance)
                            if (Creature* brann = ObjectAccessor::GetCreature(*me, pInstance->GetData64(NPC_BRANN)))
                            {
                                brann->MonsterYell("What in the name o' Madoran did THAT do? Oh! Wait: I just about got it...", LANG_UNIVERSAL, nullptr);
                    ...
                    if (HealthBelowPct(20))
                    {
                        if (Creature* brann = ObjectAccessor::GetCreature(*me, pInstance->GetData64(NPC_BRANN)))
                        {
                            brann->MonsterYell("Ha, that did it! Help's a-comin'! Take this, ya glowin' iron brute!", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_SUMMON_SPEACH:
                {
                    if (Creature* brann = ObjectAccessor::GetCreature(*me, pInstance->GetData64(NPC_BRANN)))
                    {
                        brann->MonsterYell("This is a wee bit trickier that before... Oh, bloody--incomin'!", LANG_UNIVERSAL, nullptr);
...
public:
    boss_sjonnir_dwarf()
        : CreatureScript("boss_sjonnir_dwarf")
    {
    }
    ...
    {
        boss_sjonnir_dwarfAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
{
public:
    boss_sjonnir_iron_sludge()
        : CreatureScript("boss_sjonnir_iron_sludge")
    {
    }
    ...
    {
        boss_sjonnir_iron_sludgeAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
public:
    ...
    {
        boss_sjonnir_malformed_oozeAI(Creature* c)
            : ScriptedAI(c)
        {
        }
