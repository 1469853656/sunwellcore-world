...
{
    boss_vezaxAI(Creature* creature)
        : BossAI(creature, TYPE_VEZAX)
    {
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_BERSERK:
                    ...
                    me->MonsterYell(TEXT_VEZAX_BERSERK, LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_VEZAX_BERSERK, nullptr);
                    break;
                ...
                case EVENT_SPELL_SUMMON_SARONITE_VAPORS:
                {
                    ...
                    uint32 _count = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 8U : 6U;
                    if (_vaporsCount < _count || !_hardmodeAvailable)
                        events.Repeat(30000);
                    else
                    {
                ...
                case EVENT_SARONITE_VAPORS_SWIRL:
                    if (summons.size())
                    {
                        me->MonsterTextEmote("The saronite vapors mass and swirl violently, merging into a monstrous form!", nullptr, true);
                ...
                case EVENT_SPELL_SUMMON_SARONITE_ANIMUS:
                    if (summons.size())
                    {
                        me->MonsterTextEmote("A saronite barrier appears around General Vezax!", nullptr, true);
    ...
    void KilledUnit(Unit* who) override
    {
        if (who->IsPlayer())
        {
            if (roll_chance_i(50))
            {
                me->MonsterYell(TEXT_VEZAX_SLAIN_1, LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(SOUND_VEZAX_SLAIN_1, nullptr);
            }
            else
            {
                me->MonsterYell(TEXT_VEZAX_SLAIN_2, LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(SOUND_VEZAX_SLAIN_2, nullptr);
            }
        }
    }
...
{
    explicit npc_ulduar_saronite_vaporsAI(Creature* creature)
...
{
    npc_ulduar_saronite_animusAI(Creature* creature)
        : ScriptedAI(creature)
    {
        instance = me->GetInstanceScript();
        if (instance)
...
{
    npc_twilight_frost_mageAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
class spell_mark_of_the_faceless_periodic_AuraScript : public AuraScript
{
        ...
        void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        if (Unit* caster = GetCaster())
            ...
            {
                int32 dmg = 5000;
                caster->CastCustomSpell(GetTarget(), SPELL_MARK_OF_THE_FACELESS_EFFECT, nullptr, &dmg, nullptr, true);
            }
    }
...
{
    npc_faceless_horror_ulduar_AI(Creature* creature)
        : ScriptedAI(creature)
        , summons(me)
        , _isPrenerfEnabled(sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF))
    {
    }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType, SpellSchoolMask) override
    {
        ...
        else if (me->HealthBelowPctDamaged(50, damage) && !_barriers.at(0))
        {
            _barriers.at(0) = true;
            SummonVoidBeast();
        }
