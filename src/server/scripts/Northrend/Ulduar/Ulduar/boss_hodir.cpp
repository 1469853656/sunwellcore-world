...
public:
    ...
    struct boss_hodirAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            me->MonsterYell(TEXT_HODIR_AGGRO, LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(SOUND_HODIR_AGGRO, nullptr);
        ...
        void DamageTaken(Unit*, uint32& damage, DamageEffectType, SpellSchoolMask) override
        {
            if (damage >= me->GetHealth() || me->GetHealth() < 150000)
            {
                ...
                if (!me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE))
                {
                    ...
                    me->MonsterYell(TEXT_HODIR_DEFEATED, LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_HODIR_DEFEATED, nullptr);
                    ...
                    if (GameObject* normalChest = ObjectAccessor::GetGameObject(*me, _normalChest))
                    {
                        ...
                        normalChest->SetLootRecipient(me->GetLootRecipient(), me);
                    }
                    ...
                    if (hardmode)
                    {
                        if (GameObject* hardmodeChest = ObjectAccessor::GetGameObject(*me, _hardmodeChest))
                        {
                            ...
                            hardmodeChest->SetLootRecipient(me->GetLootRecipient(), me);
                        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_BERSERK: {
                        ...
                        me->MonsterYell(TEXT_HODIR_BERSERK, LANG_UNIVERSAL, nullptr);
                        me->PlayDirectSound(SOUND_HODIR_BERSERK, nullptr);
                        break;
                    }
                    case EVENT_HARD_MODE_MISSED: {
                        ...
                        hardmode = false;
                        me->MonsterTextEmote(TEXTEMOTE_HODIR_HARD_MODE_MISSED.c_str(), nullptr);
                        break;
                    }
                    case EVENT_FLASH_FREEZE: {
                        ...
                        me->MonsterTextEmote("Hodir begins to cast Flash Freeze!", nullptr, true);
                        me->MonsterYell(TEXT_HODIR_FLASH_FREEZE, LANG_UNIVERSAL, nullptr);
                        me->PlayDirectSound(SOUND_HODIR_FLASH_FREEZE, nullptr);
                        SmallIcicles(false);
                        events.Repeat(60s);
                    ...
                    case EVENT_FROZEN_BLOWS: {
                        me->MonsterTextEmote("Hodir gains Frozen Blows!", nullptr, true);
                        me->MonsterTextEmote(TEXTEMOTE_HODIR_FROZEN_BLOWS.c_str(), nullptr);
                        me->PlayDirectSound(SOUND_HODIR_FROZEN_BLOWS, nullptr);
        ...
        void KilledUnit(Unit* who) override
        {
            if (who->GetTypeId() == TYPEID_PLAYER)
            {
                if (urand(0, 1))
                {
                    me->MonsterYell(TEXT_HODIR_SLAIN_1, LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_HODIR_SLAIN_1, nullptr);
                }
                else
                {
                    me->MonsterYell(TEXT_HODIR_SLAIN_2, LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_HODIR_SLAIN_2, nullptr);
                }
            }
        }
...
public:
    npc_ulduar_icicle()
        : CreatureScript("npc_ulduar_icicle")
    {
    }
    ...
    {
        npc_ulduar_icicleAI(Creature* pCreature)
...
public:
    npc_ulduar_flash_freeze()
        : CreatureScript("npc_ulduar_flash_freeze")
    {
    }
...
{
public:
    npc_ulduar_toasty_fire()
        : CreatureScript("npc_ulduar_toasty_fire")
    {
    }
...
public:
    npc_ulduar_hodir_priest()
        : CreatureScript("npc_ulduar_hodir_priest")
    {
    }
    ...
    {
        npc_ulduar_hodir_priestAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            pInstance = me->GetInstanceScript();
            events.Reset();
...
public:
    ...
    {
        npc_ulduar_hodir_druidAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            pInstance = me->GetInstanceScript();
            events.Reset();
...
{
public:
    npc_ulduar_hodir_shaman()
        : CreatureScript("npc_ulduar_hodir_shaman")
    {
    }
    ...
    {
        npc_ulduar_hodir_shamanAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            pInstance = me->GetInstanceScript();
            events.Reset();
        ...
        void ScheduleAbilities()
        {
            ...
            uint32 _timer = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? urand(20000, 25000) : 10000;
            events.ScheduleEvent(EVENT_SHAMAN_STORM_CLOUD, _timer);
        }
...
public:
    npc_ulduar_hodir_mage()
        : CreatureScript("npc_ulduar_hodir_mage")
    {
    }
    ...
    struct npc_ulduar_hodir_mageAI : public ScriptedAI
    {
        ...
        void ScheduleAbilities()
        {
            ...
            Seconds _toastyTimer = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 10s : 6s;
            events.ScheduleEvent(EVENT_MAGE_TOASTY_FIRE, _toastyTimer);
    ...
    class spell_hodir_biting_cold_player_aura_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (Unit* target = GetTarget())
            {
                ...
                target->CastCustomSpell(target, SPELL_BITING_COLD_DAMAGE, &dmg, nullptr, nullptr, true);
            }
        }
...
public:
    spell_hodir_flash_freeze()
        : SpellScriptLoader("spell_hodir_flash_freeze")
    {
    }
...
public:
    spell_hodir_storm_power()
        : SpellScriptLoader("spell_hodir_storm_power")
    {
    }
...
{
public:
    spell_hodir_storm_cloud()
        : SpellScriptLoader("spell_hodir_storm_cloud")
    {
    }
...
{
public:
    spell_hodir_toasty_fire()
        : SpellScriptLoader("spell_hodir_toasty_fire")
    {
    }
...
{
public:
    spell_hodir_starlight()
        : SpellScriptLoader("spell_hodir_starlight")
    {
    }
