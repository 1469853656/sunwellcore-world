...
struct boss_thorim : public ScriptedAI
{
        ...
        , summons(me)
    {
        ...
        if (_encounterFinished && m_pInstance)
    ...
    void CloseDoors()
    {
        ...
        if (GameObject* go = GetThorimObject(DATA_THORIM_FIRST_DOORS))
            go->SetGoState(GO_STATE_READY);

        if (GameObject* go = GetThorimObject(DATA_THORIM_SECOND_DOORS))
            go->SetGoState(GO_STATE_READY);
    ...
    void Reset() override
    {
        ...
        if (Player* t = SelectTargetFromPlayerList(1000))
            if (t->GetTeam(CF_DENY) == TEAM_HORDE)
                _isAlly = false;
    ...
    void DoAction(int32 param) override
    {
        ...
        else if (param == ACTION_KILLED_MINI_BOSS)
            ++_miniBossesKilled;
    }
    ...
    void KilledUnit(Unit*) override
    {
        ...
        if (urand(0, 1))
        {
            me->MonsterYell("Can't you at least put up a fight!?", LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(SOUND_SLAY1);
        }
        else
        {
            me->MonsterYell("Pathetic!", LANG_UNIVERSAL, nullptr);
    ...
    {
        if (who && _miniBossesKilled == 2 && who->GetPositionZ() > 430 && who->GetTypeId() == TYPEID_PLAYER)
        {
            _miniBossesKilled     = 0;
            ...
            me->MonsterYell("Impertinent whelps! You dare challenge me atop my pedestal! I will crush you myself!", LANG_UNIVERSAL, nullptr);
        ...
        if (damage >= me->GetHealth())
        {
            ...
            if (!_encounterFinished)
            {
                ...
                me->MonsterYell("Stay your arms! I yield!", LANG_UNIVERSAL, nullptr);
                ...
                if (GameObject* go = me->GetMap()->SummonGameObject(chestId, 2134.73f, -286.32f, 419.51f, 1.67f, 0, 0, 0, 0, 0))
                {
                    go->SetSpellId(1);
                    go->SetUInt32Value(GAMEOBJECT_FLAGS, 0);
                    go->SetLootRecipient(me->GetLootRecipient(), me);
                }
            }
        }
    }
    ...
    void SpawnArenaNPCs()
    {
        ...
        float moveTimeHalf = 20 / Movement::gravity;
        float max_height   = -Movement::computeFallElevation(moveTimeHalf, false, -20);
        for (uint8 i = 0; i < (_spawnCommoners ? 7 : 2); ++i)
        {
            rnd = urand(0, 13);
            if (Creature* cr = me->SummonCreature((_spawnCommoners ? NPC_DARK_RUNE_COMMONER : RollTable[urand(0, 2)]), ArenaNPCs[rnd], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000))
            {
                Movement::MoveSplineInit init(cr);
                init.MoveTo(Middle.GetPositionX() + urand(19, 24) * cos(Middle.GetAngle(cr)),
                    Middle.GetPositionY() + urand(19, 24) * sin(Middle.GetAngle(cr)),
                    Middle.GetPositionZ());
                init.SetParabolic(max_height, 0);
                init.SetVelocity(20);
                int32 jumpTime = init.Launch();
                // Change creature's reactstate to passive, change it back to aggresive after jump
                if (cr->AI())
                    cr->AI()->DoAction(jumpTime);
            }
        }
    }
    ...
    void PlaySpecial()
    {
        ...
        {
            case 0:
                me->MonsterYell("Behold the power of the storms and despair!", LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(SOUND_SPECIAL1);
                break;
            case 1:
                me->MonsterYell("Do not hold back! Destroy them!", LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(SOUND_SPECIAL2);
                break;
            case 2:
                me->MonsterYell("Have you begun to regret your intrusion? ", LANG_UNIVERSAL, nullptr);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (events.GetEvent())
        {
            ...
            case EVENT_THORIM_AGGRO:
                me->MonsterYell("Interlopers! You mortals who dare to interfere with my sport will pay... Wait--you...", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_AGGRO2: {
                me->MonsterYell("I remember you... In the mountains... But you... what is this? Where am--", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_LIGHTNING_ORB: {
                ...
                // No players found
                me->MonsterYell("Failures! Weaklings!", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_BERSERK:
                ...
                events.PopEvent();
                me->MonsterYell("My patience has reached its limit!", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_OUTRO1:
                ...
                if (_hardMode)
                {
                    me->MonsterYell("You! Fiend! You are not my beloved! Be gone!", LANG_UNIVERSAL, nullptr);
                ...
                {
                    me->MonsterYell("I feel as though I am awakening from a nightmare, but the shadows in this place yet linger.", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_OUTRO2:
                ...
                if (_hardMode)
                {
                    me->MonsterYell("Behold the hand behind all the evil that has befallen Ulduar! Left my kingdom in ruins, corrupted my brother and slain my wife!", LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_HARD2);
                    events.ScheduleEvent(EVENT_THORIM_OUTRO3, 12000, 0, 3);
                }
                else
                {
                    me->MonsterYell("Sif... was Sif here? Impossible--she died by my brother's hand. A dark nightmare indeed....", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_THORIM_OUTRO3:
                ...
                if (_hardMode)
                {
                    me->MonsterYell("And now it falls to you, champions, to avenge us all! The task before you is great, but I will lend you my aid as I am able. You must prevail!", LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(SOUND_HARD3);
                }
                else
                {
                    me->MonsterYell("I need time to reflect.... I will aid your cause if you should require it. I owe you at least that much. Farewell.", LANG_UNIVERSAL, nullptr);
...
public:
    boss_thorim_sif()
        : CreatureScript("boss_thorim_sif")
    {
    }

    struct boss_thorim_sifAI : public ScriptedAI
    {
        boss_thorim_sifAI(Creature* pCreature)
            ...
            while (auto eventID = events.ExecuteEvent())
            {
                switch (eventID)
                {
                    case EVENT_SIF_FINISH_DOMINION:
                        ...
                        me->MonsterYell("This pathetic morons are harmless. Relive my station, dispose of them!", LANG_UNIVERSAL, nullptr);
                    ...
                    case EVENT_SIF_START_TALK:
                        ...
                        me->MonsterYell("Thorim, my lord, why else would these invaders have come into your sanctum but to slay you? They must be stopped!", LANG_UNIVERSAL, nullptr);
                        me->PlayDirectSound(SOUND_SIF_START);
                        break;
                    case EVENT_SIF_JOIN_TALK:
                        me->PlayDirectSound(SOUND_SIF_EVENT);
                        me->MonsterYell("Impossible! Lord Thorim, I will bring your foes a frigid death!", LANG_UNIVERSAL, nullptr);
                    ...
                    case EVENT_SIF_FROST_NOVA_START:
                        me->NearTeleportTo(2108.f + frand(0.f, 42.f), -238.f - frand(0.f, 46.f), 420.02f, me->GetAngle(&Middle));
                        events.Repeat(20s);
                        events.DelayEvents(5001, 0);
                        events.ScheduleEvent(EVENT_SIF_FROST_NOVA_CAST, 2.5s);
                        _allowCast = false;
                        return;
...
{
public:
    boss_thorim_lightning_orb()
        : CreatureScript("boss_thorim_lightning_orb")
    {
    }
...
public:
    boss_thorim_trap()
        : CreatureScript("boss_thorim_trap")
    {
    }
    ...
    {
        boss_thorim_trapAI(Creature* pCreature)
...
{
public:
    boss_thorim_sif_blizzard()
        : CreatureScript("boss_thorim_sif_blizzard")
    {
    }
...
public:
    boss_thorim_pillar()
        : CreatureScript("boss_thorim_pillar")
    {
    }
    ...
    {
        boss_thorim_pillarAI(Creature* pCreature)
            : NullCreatureAI(pCreature)
        {
        }
...
public:
    boss_thorim_start_npcs()
        : CreatureScript("boss_thorim_start_npcs")
    {
    }

    struct boss_thorim_start_npcsAI : public ScriptedAI
    {
        boss_thorim_start_npcsAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
...
public:
    boss_thorim_gauntlet_npcs()
        : CreatureScript("boss_thorim_gauntlet_npcs")
    {
    }
...
{
public:
    boss_thorim_runic_colossus()
        : CreatureScript("boss_thorim_runic_colossus")
    {
    }
        ...
        void JustDied(Unit*) override
        {
            if (me->GetInstanceScript())
            {
                if (GameObject* go = ObjectAccessor::GetGameObject(*me, me->GetInstanceScript()->GetData64(DATA_THORIM_FIRST_DOORS)))
                    go->SetGoState(GO_STATE_ACTIVE);
                if (Creature* thorim = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetData64(TYPE_THORIM)))
                    thorim->AI()->DoAction(ACTION_KILLED_MINI_BOSS);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_RC_RUNIC_BARRIER:
                    ...
                    me->MonsterTextEmote("Runic Colossus surrounds itself with a crackling Runic Barrier!", nullptr, true);
                    events.RepeatEvent(20000);
                    break;
                case EVENT_RC_SMASH:
...
public:
    ...
    struct boss_thorim_ancient_rune_giantAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit*) override
        {
            ...
            me->MonsterTextEmote("Ancient Rune Giant fortifies nearby allies with runic might", nullptr, true);
        }
...
public:
    boss_thorim_arena_npcs()
        : CreatureScript("boss_thorim_arena_npcs")
    {
    }
    ...
    {
        boss_thorim_arena_npcsAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
        }
        ...
        void EnterCombat(Unit*) override
        {
            ...
            else if (me->GetEntry() == NPC_DARK_RUNE_CHAMPION)
            {
                ...
                events.ScheduleEvent(EVENT_DR_CHAMPION_CH, urand(2000, 6000));
        ...
        void DoAction(int32 sec) override
        {
            me->SetPassive();
            events.ScheduleEvent(EVENT_DR_SET_AGGRESIVE, sec);
        }
...
public:
    go_thorim_lever()
        : GameObjectScript("go_thorim_lever")
    {
    }
...
{
    npc_thorim_golem_left_right_bunny_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
public:
    spell_thorim_trash_impale()
        : SpellScriptLoader("spell_thorim_trash_impale")
    {
    }
