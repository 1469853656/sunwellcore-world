        ...
        Unit* GetEyeBeamTarget()
        {
            ...
            for (auto& i : threatList)
            {
                if (Unit* unit = i->getTarget())
                {
                    ...
                    {
                        if (player->isDead() || player->HasAura(SPELL_STONE_GRIP_DOT) || player->IsGameMaster() || player->GetDistance(me->GetPosition()) < 20.0f || player->GetDistance(me->GetPosition()) > 60.0f)
                            continue;
...
public:
    boss_kologarn_arms()
        : CreatureScript("boss_kologarn_arms")
    {
    }
    ...
    {
        boss_kologarn_armsAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
{
public:
    boss_kologarn_eyebeam()
        : CreatureScript("boss_kologarn_eyebeam")
    {
    }

    struct boss_kologarn_eyebeamAI : public ScriptedAI
    {
        boss_kologarn_eyebeamAI(Creature* creature)
            : ScriptedAI(creature)
            , _damaged(false)
        {
            chaseTargetGUID = 0;
...
public:
    spell_ulduar_stone_grip()
        : SpellScriptLoader("spell_ulduar_stone_grip")
    {
    }
...
{
public:
    spell_ulduar_squeezed_lifeless_periodic()
        : SpellScriptLoader("spell_ulduar_squeezed_lifeless_periodic")
    {
    }
...
{
public:
    at_kologarn_feet_ulduar()
        : AreaTriggerScript("at_kologarn_feet_ulduar")
    {
    }
