        ...
        void OnGameObjectCreate(GameObject* gameObject) override
        {
            switch (gameObject->GetEntry())
            {
                ...
                case GO_ALGALON_CHEST_HERO:
                    if (!gameObject->loot.LoadedFromDB())
                    {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            switch (type)
            {
                ...
                case EVENT_TOWER_OF_FLAMES_DESTROYED:
                {
                    ...
                    if (GameObject* gobj = instance->GetGameObject(m_leviathanVisualTowers[type - EVENT_TOWER_OF_LIFE_DESTROYED][0]))
                        gobj->SetGoState(GO_STATE_ACTIVE);
                    if (GameObject* gobj = instance->GetGameObject(m_leviathanVisualTowers[type - EVENT_TOWER_OF_LIFE_DESTROYED][1]))
                        gobj->SetGoState(GO_STATE_ACTIVE);
                    return;
                }
                ...
                case DATA_GIFT_OF_THE_OBSERVER: {
                    if (GameObject* go = instance->GetGameObject(m_chestGUID))
                    {
                        ...
                        if (Creature* algalon = instance->GetCreature(m_uiAlgalonGUID))
                            go->SetLootRecipient(algalon->GetLootRecipient(), algalon);
                    }
                    break;
                }
                // Achievement
            ...
            if (type == TYPE_FREYA && data == DONE)
            {
                ...
                if (Creature* freya = instance->GetCreature(GetData64(TYPE_FREYA)))
                {
                    ...
                    for (std::list<GameObject*>::const_iterator itr = goList.begin(); itr != goList.end(); ++itr)
                    {
                        ...
                        (*itr)->SetRespawnDelay(7 * DAY);
                        if ((*itr)->GetRespawnTime())
