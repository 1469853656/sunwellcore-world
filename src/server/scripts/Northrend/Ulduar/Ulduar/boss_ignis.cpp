...
{
    npc_ulduar_iron_constructAI(Creature* creature)
        : ScriptedAI(creature)
    {
    ...
    void SpellHit(Unit* /*caster*/, const SpellInfo* spell) override
    {
        ...
        else if (spell->Id == SPELL_HEAT_BUFF)
        {
            ...
            {
                uint32 _stacksRequired = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 20U : 10U;
                if (aura->GetStackAmount() >= _stacksRequired)
                {
...
{
    npc_scorched_groundAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    boss_ignisAI(Creature* creature)
        : BossAI(creature, TYPE_IGNIS)
    {
    }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        me->MonsterYell(TEXT_AGGRO, LANG_UNIVERSAL, nullptr);
    ...
    void KilledUnit(Unit* victim) override
    {
        if (victim->IsPlayer() && roll_chance_i(25))
        {
            if (urand(0, 1))
            {
                me->MonsterYell(TEXT_SLAY_1, LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(SOUND_SLAY_1);
            }
            else
            {
                me->MonsterYell(TEXT_SLAY_2, LANG_UNIVERSAL, nullptr);
    ...
    void JustDied(Unit* killer) override
    {
        me->MonsterYell(TEXT_DEATH, LANG_UNIVERSAL, nullptr);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SPELL_SCORCH:
                    if (urand(0, 1))
                    {
                        me->MonsterYell(TEXT_SCORCH_1, LANG_UNIVERSAL, nullptr);
                        me->PlayDirectSound(SOUND_SCORCH_1);
                    }
                    else
                    {
                        me->MonsterYell(TEXT_SCORCH_2, LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_SPELL_FLAME_JETS:
                    me->MonsterTextEmote(TEXT_FLAME_JETS.c_str(), nullptr, true);
                ...
                case EVENT_GRAB:
                {
                    ...
                    if (!playerGUIDs.empty())
                    {
                        ...
                        {
                            me->MonsterYell(TEXT_SLAG_POT, LANG_UNIVERSAL, nullptr);
