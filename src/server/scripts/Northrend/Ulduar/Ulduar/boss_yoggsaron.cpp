...
{
    boss_yoggsaron_sara(Creature* pCreature)
    ...
    void InitFight(Unit* target)
    {
        ...
        me->MonsterYell("The time to strike at the head of the beast will soon be upon us! Focus your anger and hatred on his minions!", LANG_UNIVERSAL, nullptr);
    ...
    void SpellSounds()
    {
        ...
        if (urand(0, 1))
        {
            me->MonsterYell(_secondPhase ? "Tremble, mortals, before the coming of the end!" : "Yes! YES! Show them no mercy! Give no pause to your attacks!", LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(_secondPhase ? SARA_P2_CAST2 : SARA_P1_CAST1);
        }
        else
        {
            me->MonsterYell(_secondPhase ? "Suffocate upon your own hate!" : "Let hatred and rage guide your blows!", LANG_UNIVERSAL, nullptr);
    ...
    void KilledUnit(Unit* who) override
    {
        ...
        if (urand(0, 1))
        {
            me->MonsterYell("Could they have been saved?", LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(SARA_P1_KILL2);
        }
        else
        {
            me->MonsterYell("Powerless to act...", LANG_UNIVERSAL, nullptr);
    ...
    void DamageTaken(Unit* who, uint32& damage, DamageEffectType, SpellSchoolMask) override
    {
        if (who && who->GetEntry() == NPC_GUARDIAN_OF_YS && !_secondPhase)
        {
            ...
            if (me->GetHealth() <= damage)
            {
                ...
                _p2TalkTimer++;
                me->MonsterYell("I am the lucid dream.", LANG_UNIVERSAL, nullptr);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (_p2TalkTimer)
        {
            ...
            if (_p2TalkTimer >= 4000 && _p2TalkTimer < 20000)
            {
                ...
                me->MonsterYell("The monster in your nightmares.", LANG_UNIVERSAL, nullptr);
                _p2TalkTimer = 20000;
            }
            else if (_p2TalkTimer >= 25000 && _p2TalkTimer < 40000)
            {
                summons.DespawnEntry(NPC_OMINOUS_CLOUD);
                me->MonsterYell("The fiend of a thousand faces.", LANG_UNIVERSAL, nullptr);
                _p2TalkTimer = 40000;
            }
            else if (_p2TalkTimer >= 44500 && _p2TalkTimer < 60000)
            {
                me->MonsterYell("Cower before my true form.", LANG_UNIVERSAL, nullptr);
                _p2TalkTimer = 60000;
            }
        ...
        switch (events.GetEvent())
        {
            ...
            case EVENT_SARA_P2_MALADY:
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                    if (Aura* aur = target->GetAura(SPELL_SANITY))
                    {
                        uint32 _stackReq = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 30U : 40U;
                        if (aur->GetStackAmount() >= _stackReq)
...
{
    boss_yoggsaron_cloud(Creature* pCreature)
        : ScriptedAI(pCreature)
    {
        Reset();
    }
...
{
    boss_yoggsaron_guardian_of_ys(Creature* creature)
        : ScriptedAI(creature)
    {
        _zeroKeepers = false;
    }
...
{
    boss_yoggsaron(Creature* creature)
        : ScriptedAI(creature)
        , summons(creature)
    {
    ...
    void JustDied(Unit* who) override
    {
        ...
        me->MonsterYell("Your fate is sealed. The end of days is finally upon you and ALL who inhabit this miserable little seedling. Uulwi ifis halahs gag erh'ongg w'ssh.", LANG_UNIVERSAL, nullptr);
    ...
    void DoAction(int32 param) override
    {
        ...
        {
            me->MonsterYell("BOW DOWN BEFORE THE GOD OF DEATH!", LANG_UNIVERSAL, nullptr);
        }
        else if (param == ACTION_YOGG_SARON_OPEN_PORTAL_YELL)
        {
            me->MonsterYell("MADNESS WILL CONSUME YOU!", LANG_UNIVERSAL, nullptr);
        ...
        else if (param == ACTION_YOGG_SARON_START_P3)
        {
            ...
            me->MonsterYell("Look upon the true face of death and know that your end comes soon!", LANG_UNIVERSAL, nullptr);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (events.GetEvent())
        {
            case EVENT_YS_LUNATIC_GAZE:
                me->MonsterYell("Hoohehehahahaha... AHAHAHAHAHAHA!", LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(YS_P3_LUNATIC_GAZE);
                me->CastSpell(me, SPELL_LUNATIC_GAZE_YS, true);
                events.RepeatEvent(12000);
                break;
            case EVENT_YS_DEAFENING_ROAR:
                me->MonsterTextEmote("Yogg-Saron opens his mouth wide!", nullptr, true);
                me->MonsterYell("Eternal suffering awaits!", LANG_UNIVERSAL, nullptr);
            ...
            case EVENT_YS_SHADOW_BEACON:
            {
                ...
                events.RepeatEvent(5000);
                if (!sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) && Is25ManRaid())
                    count = 3;
...
{
    boss_yoggsaron_death_orb(Creature* pCreature)
...
{
    boss_yoggsaron_crusher_tentacle(Creature* pCreature)
        : ScriptedAI(pCreature)
    {
        _zeroKeepers = false;
    ...
    {
        // Judgements
        if (spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && spellInfo->SpellFamilyFlags[0] & 0x00080000)
            me->InterruptNonMeleeSpells(false);
    }
...
{
    boss_yoggsaron_corruptor_tentacle(Creature* creature)
        : ScriptedAI(creature)
    {
        _zeroKeepers = false;
        SetCombatMovement(false);
...
{
    boss_yoggsaron_constrictor_tentacle(Creature* creature)
        : ScriptedAI(creature)
    {
        _zeroKeepers = false;
        SetCombatMovement(false);
...
{
    boss_yoggsaron_keeper(Creature* creature)
        : NullCreatureAI(creature)
    {
...
{
    boss_yoggsaron_influence_tentacle(Creature* creature)
        : NullCreatureAI(creature)
    {
        _zeroKeepers = false;
    ...
    void DamageTaken(Unit*, uint32& damage, DamageEffectType, SpellSchoolMask) override
    {
        if (me->GetEntry() != NPC_INFLUENCE_TENTACLE)
            me->UpdateEntry(NPC_INFLUENCE_TENTACLE, nullptr, false);
...
{
    boss_yoggsaron_immortal_guardian(Creature* creature)
        : ScriptedAI(creature)
    {
        task.CancelAll();
...
{
    boss_yoggsaron_lich_king(Creature* creature)
        : NullCreatureAI(creature)
    {
    }
...
{
    boss_yoggsaron_llane(Creature* creature)
        : NullCreatureAI(creature)
    {
    }
...
{
    boss_yoggsaron_neltharion(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    boss_yoggsaron_voice(Creature* creature)
        : NullCreatureAI(creature)
    {
        _targets.clear();
        _current = 0;
...
public:
    ...
    class spell_yogg_saron_insane_periodic_trigger_SpellScript : public SpellScript
    {
        ...
        void HandleDummyEffect(SpellEffIndex effIndex)
        {
            ...
            caster->MonsterWhisper("Your will is no longer your own...", target, false);
...
public:
    spell_yogg_saron_insane()
        : SpellScriptLoader("spell_yogg_saron_insane")
    {
    }
...
public:
    spell_yogg_saron_constrictor_tentacle()
        : SpellScriptLoader("spell_yogg_saron_constrictor_tentacle")
    {
    }
