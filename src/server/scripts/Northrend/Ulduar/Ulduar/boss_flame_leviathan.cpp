...
public:
    ...
    struct boss_flame_leviathanAI : public ScriptedAI
    {
        ...
        {
            ScheduleEvents();
            me->MonsterYell("Hostile entities detected. Threat assessment protocol active. Primary target engaged. Time minus thirty seconds to re-evaluation.", LANG_UNIVERSAL, nullptr);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_SOUND_BEGINNING:
                    if (_towersCount)
                    {
                        me->MonsterYell("Orbital countermeasures enabled.", LANG_UNIVERSAL, nullptr);
                        me->PlayDirectSound(LV_SOUND_HARD_MODE);
                    }
                    else
                    {
                        me->MonsterYell("Alert! Static defense system failure. Orbital countermeasures disabled.", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_REINSTALL:
                {
                    ThrowPlayers(true);
                    events.PopEvent();
                    me->MonsterTextEmote("Flame Leviathan reactivated. Resumming combat functions.", nullptr, true);
                    return;
                }
                case EVENT_THORIMS_HAMMER:
                    SummonTowerHelpers(TOWER_OF_STORMS);
                    events.RepeatEvent(60000 + rand() % 60000);
                    me->MonsterTextEmote("Flame Leviathan activates Thorim's Hammer.", nullptr, true);
                    me->MonsterYell("'Thorim's Hammer' online. Acquiring target.", LANG_UNIVERSAL, nullptr);
                ...
                case EVENT_FREYA:
                    ...
                    events.PopEvent();
                    me->MonsterTextEmote("Flame Leviathan activates Freya's Ward.", nullptr, true);
                    me->MonsterYell("'Freya's Ward' online. Acquiring target.", LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(LV_SOUND_TOWER_3);
                    return;
                case EVENT_MIMIRONS_INFERNO:
                    SummonTowerHelpers(TOWER_OF_FLAMES);
                    events.PopEvent();
                    me->MonsterTextEmote("Flame Leviathan activates Mimiron's Inferno.", nullptr, true);
                    me->MonsterYell("'Mimiron's Inferno' online. Acquiring target.", LANG_UNIVERSAL, nullptr);
                    me->PlayDirectSound(LV_SOUND_TOWER_2);
                    return;
                case EVENT_HODIRS_FURY:
                    SummonTowerHelpers(TOWER_OF_FROST);
                    events.PopEvent();
                    me->MonsterTextEmote("Flame Leviathan activates Hodir's Fury.", nullptr, true);
                    me->MonsterYell("'Hodir's Fury' online. Acquiring target.", LANG_UNIVERSAL, nullptr);
...
{
    me->GetMap()->ToInstanceMap()->PermBindAllPlayers(me);
}
...
void boss_flame_leviathan::boss_flame_leviathanAI::TurnGates(bool _start, bool _death)
{
    ...
    if (_start)
    {
        ...
        if (GameObject* go = ObjectAccessor::GetGameObject(*me, m_pInstance->GetData64(DATA_LIGHTNING_WALL2)))
            go->SetGoState(GO_STATE_READY);

        if (m_pInstance->GetData(TYPE_LEVIATHAN) == NOT_STARTED)
            if (GameObject* go = ObjectAccessor::GetGameObject(*me, m_pInstance->GetData64(GO_LEVIATHAN_DOORS)))
                go->SetGoState(GO_STATE_ACTIVE_ALTERNATIVE);
    }
    else
    {
        if (_death)
            if (GameObject* go = ObjectAccessor::GetGameObject(*me, m_pInstance->GetData64(DATA_LIGHTNING_WALL1)))
...
void boss_flame_leviathan::boss_flame_leviathanAI::TurnHealStations(bool _apply)
{
    ...
    if (_apply)
    {
        if (GameObject* go = ObjectAccessor::GetGameObject(*me, m_pInstance->GetData64(DATA_REPAIR_STATION1)))
            go->SetLootState(GO_READY);
        if (GameObject* go = ObjectAccessor::GetGameObject(*me, m_pInstance->GetData64(DATA_REPAIR_STATION2)))
            go->SetLootState(GO_READY);
    }
    else
    {
...
void boss_flame_leviathan::boss_flame_leviathanAI::SayPursue()
{
    ...
    {
        case 0:
            me->MonsterYell("Threat re-evaluated. Target assessment complete. Changing course.", LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(LV_SOUND_TARGET1);
            break;
        case 1:
            me->MonsterYell("Pursuit objective modified. Changing course.", LANG_UNIVERSAL, nullptr);
            me->PlayDirectSound(LV_SOUND_TARGET2);
            break;
        case 2:
            me->MonsterYell("Hostile entity stratagem predicted. Rerouting battle function. Changing course.", LANG_UNIVERSAL, nullptr);
...
void boss_flame_leviathan::boss_flame_leviathanAI::SpellHit(Unit* caster, const SpellInfo* spellInfo)
{
    ...
    if (spellInfo->Id == SPELL_SYSTEMS_SHUTDOWN)
    {
        ...
        ThrowPlayers(false);
        me->MonsterTextEmote("Flame Leviathan's circuits overloaded.", nullptr, true);
        me->MonsterTextEmote("Automatic repair sequence initiated.", nullptr, true);

        switch (urand(0, 2))
        {
            case 0:
                me->MonsterYell("System malfunction. Diverting power to support systems.", LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(LV_SOUND_OVERLOAD1);
                break;
            case 1:
                me->MonsterYell("Combat matrix overload. Powering do-o-o-own...", LANG_UNIVERSAL, nullptr);
                me->PlayDirectSound(LV_SOUND_OVERLOAD2);
                break;
            case 2:
                me->MonsterYell("System restart required. Deactivating weapon systems.", LANG_UNIVERSAL, nullptr);
...
void boss_flame_leviathan::boss_flame_leviathanAI::JustDied(Unit* killer)
{
    ...
    me->MonsterYell("Total systems failure. Defense protocols breached. Leviathan Unit shutting down.", LANG_UNIVERSAL, nullptr);
...
public:
    ...
    {
        npc_thorims_hammerAI(Creature* c)
            : NullCreatureAI(c)
        {
        }
...
public:
    npc_lore_keeper_of_norgannon_ulduar()
        : CreatureScript("npc_lore_keeper_of_norgannon_ulduar")
    {
    }
    ...
    {
        npc_lore_keeper_of_norgannon_ulduarAI(Creature* creature)
            : ScriptedAI(creature)
        {
...
public:
    npc_brann_ulduar()
        : CreatureScript("npc_brann_ulduar")
    {
    }
    ...
    {
        npc_brann_ulduarAI(Creature* creature)
            : ScriptedAI(creature)
        {
            instance = me->GetInstanceScript();
            Reset();
        ...
        void DoAction(int32 param) override
        {
            ...
            {
                instance->ProcessEvent(nullptr, EVENT_TOWER_OF_STORM_DESTROYED);
                instance->ProcessEvent(nullptr, EVENT_TOWER_OF_FROST_DESTROYED);
                instance->ProcessEvent(nullptr, EVENT_TOWER_OF_FLAMES_DESTROYED);
                instance->ProcessEvent(nullptr, EVENT_TOWER_OF_LIFE_DESTROYED);
            }
...
public:
    npc_brann_radio()
        : CreatureScript("npc_brann_radio")
    {
    }
...
public:
    npc_storm_beacon_spawn()
        : CreatureScript("npc_storm_beacon_spawn")
    {
    }

    struct npc_storm_beacon_spawnAI : public ScriptedAI
    {
        npc_storm_beacon_spawnAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    ...
    {
        boss_flame_leviathan_safety_containerAI(Creature* c)
            : NullCreatureAI(c)
        {
            _allowTimer = 0;
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (id == me->GetEntry())
            {
                if (Creature* liquid = me->SummonCreature(NPC_LIQUID, *me))
                {
                    ...
                    Seconds _despawnTime = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 60s : 180s;
                    if (sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF))
                        liquid->DespawnOrUnsummon(_despawnTime);
                }
...
public:
    spell_load_into_catapult()
        : SpellScriptLoader("spell_load_into_catapult")
    {
    }
...
public:
    spell_auto_repair()
        : SpellScriptLoader("spell_auto_repair")
    {
    }
...
public:
    spell_systems_shutdown()
        : SpellScriptLoader("spell_systems_shutdown")
    {
    }
...
public:
    spell_pursue()
        : SpellScriptLoader("spell_pursue")
    {
    }
...
public:
    spell_tar_blaze()
        : SpellScriptLoader("spell_tar_blaze")
    {
    }
...
{
public:
    spell_vehicle_grab_pyrite()
        : SpellScriptLoader("spell_vehicle_grab_pyrite")
    {
    }
...
{
public:
    spell_vehicle_chopper_grab_pyrite()
        : SpellScriptLoader("spell_vehicle_chopper_grab_pyrite")
    {
    }
...
{
public:
    spell_orbital_supports()
        : SpellScriptLoader("spell_orbital_supports")
    {
    }
...
public:
    spell_thorims_hammer()
        : SpellScriptLoader("spell_thorims_hammer")
    {
    }
...
{
public:
    spell_shield_generator()
        : SpellScriptLoader("spell_shield_generator")
    {
    }
...
{
public:
    achievement_flame_leviathan_garage(char const* name, uint32 entry1, uint32 entry2)
        : AchievementCriteriaScript(name)
        , _entry1(entry1)
        , _entry2(entry2)
    {
...
public:
    npc_ward_of_life()
        : CreatureScript("npc_ward_of_life")
    {
    }
    ...
    struct npc_ward_of_lifeAI : public ScriptedAI
    {
        ...
        void Reset() override { _lashTimer = 2000; }
        ...
        void JustDied(Unit* /*who*/) override { me->DisappearAndDie(); }
...
{
    npc_liquid_piryte_flame_leviathanAI(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    npc_flame_leviathan_vehicles_AI(Creature* creature)
        : ScriptedAI(creature)
    {
            ...
            switch (GetHitUnit()->GetEntry())
            {
                ...
                case NPC_VEHICLE_CHOPPER:
                {
                    if (Creature* veh = GetHitUnit()->ToCreature())
                    {
                        if (veh->IsAIEnabled)
                        {
                            uint32 hitCount = sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 4U : 1U;
                            if (veh->AI()->GetData(DATA_GET_FLAMES_HIT_COUNT) >= hitCount)
                            {
...
{
    npc_brann_bronzebeard_flame_leviathanAI(Creature* creature)
        : ScriptedAI(creature)
    {
        _rhyrdianGUID    = 0;
