...
public:
    boss_algalon_the_observer()
        : CreatureScript("boss_algalon_the_observer")
    {
    }
    ...
    struct boss_algalon_the_observerAI : public ScriptedAI
    {
        ...
        bool IsValidHeraldItem(const ItemTemplate* item)
        {
            if (!item) // should not happen, but checked in GetAverageItemLevel()
                return true;
            if (item->ItemLevel <= 226 || item->ItemLevel <= 232 && (item->InventoryType == INVTYPE_SHIELD || item->Class == ITEM_CLASS_WEAPON || item->Class == ITEM_CLASS_ARMOR && (item->InventoryType == INVTYPE_RELIC || item->InventoryType == INVTYPE_HOLDABLE)))
                return true;
            return false;
        }
        ...
        void DamageTaken(Unit*, uint32& damage, DamageEffectType, SpellSchoolMask) override
        {
            ...
            else if (me->HealthBelowPctDamaged(sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF) ? 2.0f : 3.5f, damage) && !_fightWon)
            {
                _fightWon = true;
                damage    = 0;
                me->SetReactState(REACT_PASSIVE);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_COSMIC_SMASH:
                {
                    ...
                    uint32 _smashCount = RAID_MODE(1, 3);
                    if (Is25ManRaid() && sWorld->getBoolConfig(CONFIG_ULDUAR_PRE_NERF))
                        _smashCount = 5;
                    me->CastCustomSpell(SPELL_COSMIC_SMASH, SPELLVALUE_MAX_TARGETS, _smashCount, (Unit*)nullptr);
...
public:
    npc_collapsing_star()
        : CreatureScript("npc_collapsing_star")
    {
    }
...
public:
    npc_living_constellation()
        : CreatureScript("npc_living_constellation")
    {
    }
...
{
public:
    npc_algalon_worm_hole()
        : CreatureScript("npc_algalon_worm_hole")
    {
    }

    struct npc_algalon_worm_holeAI : public NullCreatureAI
    {
        npc_algalon_worm_holeAI(Creature* creature)
            : NullCreatureAI(creature)
        {
...
public:
    spell_algalon_phase_punch()
        : SpellScriptLoader("spell_algalon_phase_punch")
    {
    }
...
public:
    spell_algalon_collapse()
        : SpellScriptLoader("spell_algalon_collapse")
    {
    }
...
public:
    ...
    class spell_algalon_cosmic_smash_damage_SpellScript : public SpellScript
    {
        ...
        void RecalculateDamage()
        {
            ...
            if (IsMythic)
                if (GetHitUnit()->IsImmunedToSpell(GetSpellInfo()))
                    PreventHitDamage();
                else
                    SetHitDamage(50000);
            else
...
{
public:
    spell_algalon_big_bang()
        : SpellScriptLoader("spell_algalon_big_bang")
    {
    }
...
public:
    achievement_algalon_he_feeds_on_your_tears()
        : AchievementCriteriaScript("achievement_algalon_he_feeds_on_your_tears")
    {
    }
