...
public:
    npc_ulduar_keeper()
        : CreatureScript("npc_ulduar_keeper_gossip")
    {
    }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 uiSender, uint32 uiAction) override
    {
        ...
        switch (creature->GetEntry())
        {
            case NPC_FREYA_GOSSIP:
                creature->MonsterYell("Eonar, your servant calls for your blessing!", LANG_UNIVERSAL, nullptr);
                creature->PlayDirectSound(15535);
                _keeper = KEEPER_FREYA;
                break;
            case NPC_HODIR_GOSSIP:
                creature->MonsterYell("The veil of winter will protect you, champions!", LANG_UNIVERSAL, nullptr);
                creature->PlayDirectSound(15559);
                _keeper = KEEPER_HODIR;
                break;
            case NPC_MIMIRON_GOSSIP:
                creature->MonsterYell("Combat matrix enhanced. Behold wonderous rapidity!", LANG_UNIVERSAL, nullptr);
                creature->PlayDirectSound(15630);
                _keeper = KEEPER_MIMIRON;
                break;
            case NPC_THORIM_GOSSIP:
                creature->MonsterYell("Golganneth, lend me your strengh! Grant my mortal allies the power of thunder!", LANG_UNIVERSAL, nullptr);
...
{
public:
    spell_ulduar_energy_sap()
        : SpellScriptLoader("spell_ulduar_energy_sap")
    {
    }
...
{
    npc_ulduar_snow_mound(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    npc_white_jormungar_ulduar(Creature* creature)
        : ScriptedAI(creature)
    {
...
public:
    ...
    {
        npc_ulduar_storm_tempered_keeperAI(Creature* pCreature)
...
public:
    ...
    {
        npc_ulduar_arachnopod_destroyerAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
...
{
    npc_boomer_xp500_ulduar_AI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_mechagnome_battletank(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    npc_superheated_winds(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_dark_rune_ravager_thunderer(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    void Reset() override
    {
        uint32 spellId = me->GetEntry() == NPC_DARK_RUNE_THUNDERER_TRASH ? SPELL_THUNDERER_LIGHTNING_BRAND : SPELL_RAVAGER_RAVAGE_ARMOR;
