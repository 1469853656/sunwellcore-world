...
struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
{
    ...
    void OnCreatureCreate(Creature* creature) override
    {
        if (TeamIdInInstance == TEAM_NEUTRAL)
            TeamIdInInstance = GetDominatingInstanceTeamId();
        ...
        switch (creature->GetEntry())
        {
            ...
            case NPC_PALETRESS:
                ...
                // Coliseum Announcer:
            case NPC_JAEREN:
                if (InstanceProgress != INSTANCE_PROGRESS_FINISHED && creature->isDead())
            ...
            case INSTANCE_PROGRESS_ARGENT_CHALLENGE_WITHOUT_SOLDIERS:
                if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                {
                    ...
                    if (Creature* c = instance->GetCreature(NPC_ArgentChampionGUID))
                    {
                        ...
                        {
                            SetCreatureAttackable(boss, true);
                ...
                if (++Counter >= 3)
                {
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* c = instance->GetCreature(NPC_GrandChampionGUID[i]))
                        {
                            ...
                            c->RemoveAurasDueToSpell(67867); // remove trample
                            if (!creditCasted)
                            {
    ...
    void DoSummonGrandChampion(uint32 BossNumber, uint8 BossOrder)
    {
        ...
        if (Creature* pBoss = instance->SummonCreature(CHAMPION_TO_SUMMON, SpawnPos))
        {
            ...
            pBoss->SetHomePosition(748.309f, 619.448f, 411.3f, M_PI / 2);
            SetCreatureAttackable(pBoss, false);
            if (pBoss->IsAIEnabled)
            ...
            for (uint8 i = 0; i < 3; ++i)
                if (Creature* pAdd = instance->SummonCreature(MINION_TO_SUMMON, SpawnPos))
                {
                    ...
                    SetCreatureAttackable(pAdd, false);
                }
        }
            ...
            switch (eventId)
            {
                ...
                case EVENT_GRAND_GROUP_1_ATTACK:
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* creature = instance->GetCreature(NPC_GrandChampionMinionsGUID[1][i]))
                            SetCreatureAttackable(creature, true);
                ...
                case EVENT_GRAND_CHAMPIONS_MOVE_MIDDLE:
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* creature = instance->GetCreature(NPC_GrandChampionGUID[i]))
                        {
                            ...
                            creature->GetMotionMaster()->MovePoint(4, 748.309f + 3.0f * cos(angle), 619.448f + 3.0f * sin(angle), 411.3f);
                        }
                ...
                case EVENT_GRAND_CHAMPIONS_MOUNTS_ATTACK:
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* creature = instance->GetCreature(NPC_GrandChampionGUID[i]))
                        {
                            ...
                            creature->CastSpell(creature, 67865, true); // SPELL_TRAMPLE_AURA
                        }
                    break;
                case EVENT_GRAND_CHAMPIONS_MOVE_SIDE:
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* creature = instance->GetCreature(NPC_GrandChampionGUID[i]))
                        {
                            ...
                            {
                                case 0:
                                    creature->GetMotionMaster()->MovePoint(5, 736.695f, 650.02f, 412.4f);
                                    break;
                                case 1:
                                    creature->GetMotionMaster()->MovePoint(5, 756.32f, 650.05f, 412.4f);
                                    break;
                                case 2:
                                    creature->GetMotionMaster()->MovePoint(5, 746.5f, 650.65f, 411.7f);
                                    break;
                            }
                        }
                ...
                case EVENT_START_ARGENT_CHALLENGE_INTRO:
                    ...
                    for (int8 i = 0; i < 3; ++i)
                    {
                        ...
                        if (Creature* trash = instance->SummonCreature(NPC_ARGENT_LIGHTWIELDER, pos))
                        {
                            if (trash->IsAIEnabled)
                                trash->AI()->SetData(i, 0);
                            NPC_ArgentSoldierGUID[i][0] = trash->GetGUID();
                        }
                        pos.m_positionX = x + (i - 1) * 10.0f;
                        if (Creature* trash = instance->SummonCreature(NPC_ARGENT_MONK, pos))
                        {
                            if (trash->IsAIEnabled)
                                trash->AI()->SetData(i, 0);
                            NPC_ArgentSoldierGUID[i][1] = trash->GetGUID();
                        }
                        pos.m_positionX = x + 2.0f + (i - 1) * 10.0f;
                        if (Creature* trash = instance->SummonCreature(NPC_PRIESTESS, pos))
                        {
                            if (trash->IsAIEnabled)
                                trash->AI()->SetData(i, 0);
                            NPC_ArgentSoldierGUID[i][2] = trash->GetGUID();
                        }
                    }
                ...
                case EVENT_ARGENT_CHALLENGE_SAY_2:
                    if (Creature* champion = instance->GetCreature(NPC_ArgentChampionGUID))
                        if (champion->IsAIEnabled)
                            champion->AI()->Talk(TEXT_PALETRESS_SAY_2);
                    break;
                case EVENT_ARGENT_SOLDIER_GROUP_ATTACK:
                    Counter = 0;
                    for (uint8 i = 0; i < 3; ++i)
                        for (uint8 j = 0; j < 3; ++j)
                            if (Creature* creature = instance->GetCreature(NPC_ArgentSoldierGUID[i][j]))
                                SetCreatureAttackable(creature, true);
                ...
                case EVENT_SUMMON_BLACK_KNIGHT:
                    if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                        if (Creature* bk_vehicle = announcer->SummonCreature(VEHICLE_BLACK_KNIGHT, 769.834f, 651.915f, 447.035f, 0.0f))
                        {
                            ...
                            SetCreatureAttackable(bk_vehicle, false);

                            if (Vehicle* vehicle = bk_vehicle->GetVehicleKit())
                                if (Unit* blackKnight = vehicle->GetPassenger(0))
                                {
                                    NPC_BlackKnightGUID = blackKnight->GetGUID();
                                    blackKnight->SendMovementFlagUpdate(); // put him on vehicle visually
                                    blackKnight->SetImmuneToAll(true);
                                    if (blackKnight->IsUnit())
                                        SetCreatureAttackable(blackKnight->ToCreature(), false);
                                }
                ...
                case EVENT_BLACK_KNIGHT_MOVE_FORWARD:
                    if (Creature* blackKnight = instance->GetCreature(NPC_BlackKnightGUID))
                    {
                        blackKnight->SetUnitMovementFlags(MOVEMENTFLAG_WALKING);
                        blackKnight->GetMotionMaster()->MovePoint(0, 746.81f, 623.15f, 411.42f);
                        if (blackKnight->IsAIEnabled)
                            blackKnight->AI()->Talk(TEXT_BK_LICH);
                    }
                    if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                ...
                case EVENT_BLACK_KNIGHT_SAY_TASK:
                    if (Creature* blackKnight = instance->GetCreature(NPC_BlackKnightGUID))
                    {
                        blackKnight->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                        if (blackKnight->IsAIEnabled)
                            blackKnight->AI()->Talk(TEXT_BK_TASK);
                    }
                    events.ScheduleEvent(EVENT_BLACK_KNIGHT_ATTACK, 5000);
                    break;
                case EVENT_BLACK_KNIGHT_ATTACK:
                    if (Creature* blackKnight = instance->GetCreature(NPC_BlackKnightGUID))
                        SetCreatureAttackable(blackKnight, true);
...
{
    new InstanceMapScriptLoader<instance_trial_of_the_champion_InstanceMapScript>("instance_trial_of_the_champion", 650);
}
