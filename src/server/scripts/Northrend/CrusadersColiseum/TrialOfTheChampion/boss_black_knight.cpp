...
struct boss_black_knightAI : public BossAI
{
    boss_black_knightAI(Creature* creature)
        : BossAI(creature, BOSS_BLACK_KNIGHT)
    {
    }
            ...
            case EVENT_ANNOUNCER_SAY_ZOMBIE:
                if (!summons.empty())
                    if (Creature* ghoul = instance->instance->GetCreature(*summons.begin()))
                        ghoul->MonsterYell("[Zombie] .... . Brains ....", LANG_UNIVERSAL, nullptr);
                break;
...
{
    npc_black_knight_ghoulAI(Creature* creature)
        : ScriptedAI(creature)
    {
    ...
    void SpellHitTarget(Unit* target, const SpellInfo* spell) override
    {
        switch (spell->Id)
        {
            case SPELL_CLAW:
                DoResetThreat();
                if (Unit* randomTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 30.0f))
                {
                    me->AddThreat(randomTarget, 100.0f);
