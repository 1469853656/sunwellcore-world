...
enum ChampionSpells
{
    ...
    SPELL_ROLLING_THROW         = 67560,
    SPELL_ROLLING_THROW_TRIGGER = 67546
};
...
{
    npc_toc5_player_vehicleAI(Creature* creature)
    ...
    void DamageTaken(Unit*, uint32& damage, DamageEffectType, SpellSchoolMask /*mask*/) override
    {
    ...
    bool WasUsedByPlayer() const
    {
        return _usedByPlayer;
    }

    bool IsSelectedByChampionForRemount() const
    {
        return _selectedForRemount;
    }
    ...
    void OnCharmed(bool apply) override
    {
        ...
        if (apply)
        {
            SetCreatureAttackable(me, true);
            me->SetSpeedRate(MOVE_RUN, 2.0f);
            DoCastSelf(SPELL_TRAMPLE_AURA, true);
        }
        else
        {
            SetCreatureAttackable(me, false);
    ...
    void SetData(uint32 const /*type*/, uint32 const data) override
    {
        _selectedForRemount = data;
    }
    ...
    private:
        ...
        bool _respawn;
        bool _usedByPlayer = false;
        bool _selectedForRemount = false;
};

struct npc_toc5_grand_champion_minionAI : public ScriptedAI
{
    npc_toc5_grand_champion_minionAI(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    boss_grand_championAI(Creature* pCreature)
    ...
    void ScheduleAbilitiesEvents()
    {
        ...
        switch (me->GetEntry())
        {
            ...
            case NPC_ZULTORE: // Zul'tore
                _events.RescheduleEvent(EVENT_HUNTER_SPELL_DISENGAGE, 5s, 10s);
            ...
            case NPC_MOKRA: // Mokra the Skullcrusher
                ...
                _events.RescheduleEvent(EVENT_WARRIOR_SPELL_ROLLING_THROW, 5s, 10s);
                break;
            default:
                break;
        }
    ...
    Creature* GetNearestViableMount() const
    {
        std::list<Creature*> mounts;
        me->GetCreatureListWithEntryInGrid(mounts, instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE ? VEHICLE_ARGENT_WARHORSE : VEHICLE_ARGENT_BATTLEWORG, 100.0f);

        Trinity::Containers::EraseIf(mounts, [](Creature const* mount)
        {
            ...
            if (CAST_AI(npc_toc5_player_vehicleAI, mount->AI())->WasUsedByPlayer())
                return true;

            if (CAST_AI(npc_toc5_player_vehicleAI, mount->AI())->IsSelectedByChampionForRemount())
                return true;
        ...
        mounts.sort(Trinity::ObjectDistanceOrderPred(me));
        return mounts.front();
    }
        ...
        if (_mountPhase)
        {
            ...
            else if (damage >= me->GetHealth())
            {
                ...
                if (instance)
                {
                    ...
                    if (Creature* mount = GetNearestViableMount())
                    {
                        _newMountGUID = mount->GetGUID();
                        if (mount->IsAIEnabled)
                            mount->AI()->SetData(0, 1);
    ...
    void SpellHit(Unit* /*caster*/, const SpellInfo* spell) override
    {
        switch (spell->Id)
        {
            case SPELL_TRAMPLE_STUN:
            {
                ...
                me->MonsterTextEmote(buffer, nullptr);
            }
            break;
        }
    }
            ...
            switch (_events.ExecuteEvent())
            {
                case EVENT_FIND_NEW_MOUNT:
                    ...
                    if (instance)
                    {
                        ...
                        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                            if (Player* player = itr->GetSource())
                                if (me->GetExactDist(player) <= 5.0f)
                                    if (Vehicle* vehicle = player->GetVehicle())
                                        if (Unit* mount = vehicle->GetBase())
                                            if (mount->GetTypeId() == TYPEID_UNIT && mount->ToCreature()->GetEntry() == static_cast<uint32>((instance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE ? VEHICLE_ARGENT_BATTLEWORG : VEHICLE_ARGENT_WARHORSE)))
                                            {
                        ...
                        if (trample)
                        {
                            if (Aura* aura = me->GetAura(SPELL_TRAMPLE_STUN))
                    ...
                    if (Creature* mount = ObjectAccessor::GetCreature(*me, _newMountGUID))
                        if (mount->IsAlive() && mount->IsAIEnabled && !CAST_AI(npc_toc5_player_vehicleAI, mount->AI())->WasUsedByPlayer())
                        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_HUNTER_SPELL_MULTI_SHOT:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, [this](Unit const* tar) { return me->IsInRange(tar, 5.0f, 30.0f, false); }))
                    ...
                    case EVENT_WARRIOR_SPELL_INTERCEPT:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, [this](Unit const* tar) { return me->IsInRange(tar, 8.0f, 25.0f, false); }))
...
// 67706 - Fan of Knives
class spell_grand_champions_fan_of_knives : public SpellScript
{
    PrepareSpellScript(spell_grand_champions_fan_of_knives);

    void HandleHit(SpellEffIndex /*effIndex*/)
    {
        // Taken from description - if caster wears two daggers he should deal 1.5x damage, and this spell is used only by creatures with daggers
        SetEffectValue(GetEffectValue() * 1.5f);
    }

    void Register() override
    {
        OnEffectLaunchTarget += SpellEffectFn(spell_grand_champions_fan_of_knives::HandleHit, EFFECT_0, SPELL_EFFECT_WEAPON_PERCENT_DAMAGE);
    }
};

// 67573 - Intercept
class spell_grand_champions_intercept_trigger : public SpellScript
{
    PrepareSpellScript(spell_grand_champions_intercept_trigger)

        void HandleResetThreat(SpellEffIndex /*effIndex*/)
    {
        Unit* caster = GetCaster();
        if (caster->CanHaveThreatList())
            caster->getThreatManager().resetAllAggro();
        if (Unit* unitTarget = GetHitUnit())
            caster->AddThreat(unitTarget, 5.0f);
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_grand_champions_intercept_trigger::HandleResetThreat, EFFECT_0, SPELL_EFFECT_WEAPON_PERCENT_DAMAGE);
    }
};

// 67534 - Hex of Mending
class spell_grand_champions_hex_of_mending : public AuraScript
{
    PrepareAuraScript(spell_grand_champions_hex_of_mending);
    ...
    void HandleProc(ProcEventInfo& eventInfo)
    {
        ...
        if (HealInfo* healInfo = eventInfo.GetHealInfo())
            target->CastCustomSpell(SPELL_HEX_OF_MENDING_HEAL, SPELLVALUE_BASE_POINT0, healInfo->GetHeal(), target, TRIGGERED_FULL_MASK);
    }

    void Register() override
    {
        OnProc += AuraProcFn(spell_grand_champions_hex_of_mending::HandleProc);
    }
};

// 67560 - Rolling Throw
class spell_grand_champions_rolling_throw : public SpellScript
{
    PrepareSpellScript(spell_grand_champions_rolling_throw)

        void HandleTriggerSpell(SpellEffIndex effIndex)
    {
        PreventHitDefaultEffect(effIndex);
        if (Unit* target = GetHitUnit())
            target->CastSpell(target, SPELL_ROLLING_THROW_TRIGGER, true);
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_grand_champions_rolling_throw::HandleTriggerSpell, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
void AddSC_boss_grand_champions()
{
    ...
    new SpellScriptLoaderEx<spell_grand_champions_fan_of_knives>("spell_grand_champions_fan_of_knives");
    new SpellScriptLoaderEx<spell_grand_champions_intercept_trigger>("spell_grand_champions_intercept_trigger");
    new AuraScriptLoaderEx<spell_grand_champions_hex_of_mending>("spell_grand_champions_hex_of_mending");
    new SpellScriptLoaderEx<spell_grand_champions_rolling_throw>("spell_grand_champions_rolling_throw");
}
