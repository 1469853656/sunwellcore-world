...
struct boss_eadricAI : public BossAI
{
    boss_eadricAI(Creature* creature)
        : BossAI(creature, BOSS_ARGENT_CHALLENGE)
    {
    }
    ...
    {
        _Reset();
        SetCreatureAttackable(me, instance->GetData(DATA_INSTANCE_PROGRESS) == INSTANCE_PROGRESS_ARGENT_CHALLENGE_WITHOUT_SOLDIERS);
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_SPELL_RADIANCE:
                ...
                me->MonsterTextEmote(TEXT_RADIATE, nullptr, true);
                events.Repeat(16000);
                break;
...
class spell_eadric_radiance_SpellScript : public SpellScript
{
    ...
    {
        if (GetCaster()->GetInstanceScript() && GetCaster()->GetInstanceScript()->IsMythic())
            return;
