...
SpectatorsInfo constexpr SpectatorData[12] = {
    { 0, 0 },
...
public:
    npc_announcer_toc5()
        : CreatureScript("npc_announcer_toc5")
    {
    }
    ...
    {
        npc_announcer_toc5AI(Creature* pCreature)
            : CreatureAI(pCreature)
        {
        }
                                ...
                                for (Map::PlayerList::const_iterator itr = pList.begin(); itr != pList.end(); ++itr)
                                {
                                    if (rand == 0)
                                    {
                                        ...
                                        if (plr && !plr->IsGameMaster() && plr->IsAlive())
                                        {
                                            ...
                                            {
                                                GetSpectators(spectatorList, id);
                                                Trinity::Containers::RandomResize(spectatorList, urand(6, 10));
                                            }
                                            else
                                            {
                                                me->GetCreatureListWithEntryInGrid(spectatorList, SpectatorData[plr->getRace()].spectatorForAnim, 250.0f);
                                                Trinity::Containers::RandomResize(spectatorList, urand(2, 5));
                                            }
