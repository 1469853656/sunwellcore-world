...
public:
    npc_snobold_vassal()
        : CreatureScript("npc_snobold_vassal")
    {
    }
    ...
    {
        npc_snobold_vassalAI(Creature* pCreature)
            : ScriptedAI(pCreature)
        {
            ...
            {
                me->SetMaxHealth(me->GetMaxHealth() * 1.4f);
                me->SetFullHealth();
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                {
                    if (pInstance && (pInstance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC || !t->IsPlayer()))
                    {
...
class DelayedChangeSnoboldVehicle : public BasicEvent
{
public:
    DelayedChangeSnoboldVehicle(Creature* owner, Player* target)
        : _owner(owner)
        , _target(target)
    {
    }
    ...
    bool Execute(uint64 /*execTime*/, uint32 /*diff*/)
    {
        ...
        CAST_AI(npc_snobold_vassal::npc_snobold_vassalAI, _owner->AI())->TargetGUID = _target->GetGUID();
        _owner->AI()->AttackStart(_target);
        _target->CastSpell(_target, SPELL_CHANGE_VEHICLE, true);
        _owner->EnterVehicle(_target, 0);
        return true;
    }
...
public:
    ...
    struct boss_gormokAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*pKiller*/) override
        {
            if (pInstance && pInstance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
                std::map<Player*, Creature*> SnoboldTargetMap;
                std::list<Creature*> snobolds;
                GetCreatureListWithEntryInGrid(snobolds, me, NPC_SNOBOLD_VASSAL, 250.0f);
                for (Unit* snobold : snobolds)
                    if (snobold->IsAlive())
                    {
                        if (Unit* base = snobold->GetVehicleBase())
                            if (base->IsPlayer())
                                continue;

                        for (auto&& ref : me->GetMap()->GetPlayers())
                            if (Player* player = ref.GetSource())
                                if (player->IsAlive() && !player->GetVehicleKit() && !player->IsMounted() && !player->GetVehicle() && !player->IsGameMaster() && SnoboldTargetMap.find(player) == std::end(SnoboldTargetMap))
                                {
                                    SnoboldTargetMap.emplace(player, snobold->ToCreature());
                                    break;
                                }
                    }

                for (auto&& pair : SnoboldTargetMap)
                {
                    Player* target    = pair.first;
                    Creature* snobold = pair.second;
                    if (target && snobold && snobold->IsAIEnabled)
                        me->m_Events.AddEvent(new DelayedChangeSnoboldVehicle(snobold, target), me->m_Events.CalculateTime(50));
                }
            }
            else
                summons.DoAction(2);
...
{
    boss_jormungarAI(Creature* pCreature)
        : ScriptedAI(pCreature)
    {
        pInstance = pCreature->GetInstanceScript();
        //events2.Reset();
    ...
    void DoAction(int32 param) override
    {
        switch (param)
        {
            ...
            case -2:
                ...
                if (pInstance && pInstance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
                    me->CastCustomSpell(SPELL_ENRAGE, SPELLVALUE_BASE_POINT0, 200, me, TRIGGERED_FULL_MASK);
                else
...
public:
    boss_dreadscale()
        : CreatureScript("boss_dreadscale")
    {
    }
