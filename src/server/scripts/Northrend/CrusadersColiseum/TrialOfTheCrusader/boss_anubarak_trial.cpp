...
enum AnubSpells
{
    ...
    // Boost
    SPELL_BOOST_VISUAL    = 33344,
    SPELL_BOOST_COLDFLAME = 69146
};
...
struct boss_anubarak_trialAI : public BossAI
{
        ...
        : BossAI(creature, TYPE_ANUBARAK)
    {
        ...
        if (instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
        {
            ...
            me->SetFullHealth();
            DoCastSelf(SPELL_BOOST_VISUAL, true);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (!_phase3 && HealthBelowPct(instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC ? 40 : 30) && !me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE) && !me->HasAura(SPELL_SUBMERGE) && !me->HasAura(SPELL_EMERGE))
        {
            _phase3 = true;
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_EMERGE:
                    ...
                    events.CancelEvent(EVENT_BOOST_COLDFLAME);
                ...
                case EVENT_SPELL_SUMMON_SPIKE:
                    if (instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
                        events.RescheduleEvent(EVENT_BOOST_COLDFLAME, 0.5s);
                    DoCastSelf(SPELL_SUMMON_SPIKE, true);
                    break;
                case EVENT_BOOST_COLDFLAME:
                    if (Creature* spike = summons.GetCreatureWithEntry(NPC_SPIKE))
                    {
                        if (spike->GetExactDist(&_lastColdflamePos) > 6.0f)
                        {
                            me->NearTeleportTo(spike->GetPosition());
                            me->CastCustomSpell(SPELL_BOOST_COLDFLAME, SPELLVALUE_AURA_DURATION, 480000, spike, TRIGGERED_FULL_MASK);
                            _lastColdflamePos = me->GetPosition();
                        }
                    }
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
        {
            summon->SetMaxHealth(summon->GetMaxHealth() * 1.5f);
...
private:
    ...
    Position _lastColdflamePos;
};

struct npc_swarm_scarabAI : public ScriptedAI
{
    npc_swarm_scarabAI(Creature* pCreature)
    ...
    void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            if (auto caster = GetCaster())
            {
                ...
                {
                    if (target->GetHealthPct() >= 10.f)
                        lifeLeeched = target->CountPctFromCurHealth(20.f);
                    else
                        lifeLeeched = 0;
                }
                else
                {
                    lifeLeeched = target->CountPctFromCurHealth(aurEff->GetAmount());
                    if (lifeLeeched < 250)
                        lifeLeeched = 250;
                }
            }
            ...
            multiplier += [target]() {
                uint32 addAmount = 0;
                for (AuraEffect const* healingPctAurEff : target->GetAuraEffectsByType(SPELL_AURA_MOD_HEALING_PCT))
                    if (healingPctAurEff->GetAmount() > 0)
                        addAmount += healingPctAurEff->GetAmount();
            ...
            uint32 resist                 = 0;
            uint32 absorb                 = 0;
            SpellInfo const* dmgSpellInfo = sSpellMgr->GetSpellInfo(SPELL_LEECHING_SWARM_DMG);
            GetTarget()->CalcAbsorbResist(caster, target, dmgSpellInfo->GetSchoolMask(), DIRECT_DAMAGE, lifeLeeched, &absorb, &resist, dmgSpellInfo);
            if (target->GetMapId() != 249 /* Onyxia's Lair - Hellforge */ && !target->IsImmunedToDamage(sSpellMgr->GetSpellInfo(SPELL_LEECHING_SWARM_DMG)))
            {
                ...
                caster->CastCustomSpell(caster, SPELL_LEECHING_SWARM_HEAL, &value, nullptr, nullptr, true);
            }
        }
    }
...
class spell_anubarak_acid_drenched_mandibles : public AuraScript
{
    PrepareAuraScript(spell_anubarak_acid_drenched_mandibles);
    ...
    void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        if (InstanceScript* instance = GetTarget()->GetInstanceScript())
        {
            ...
            {
                int32 newDuration = GetMaxDuration() * 2;
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_anubarak_acid_drenched_mandibles::OnApply, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
};
...
void AddSC_boss_anubarak_trial()
{
    ...
    RegisterAuraScript(spell_anubarak_acid_drenched_mandibles);
}