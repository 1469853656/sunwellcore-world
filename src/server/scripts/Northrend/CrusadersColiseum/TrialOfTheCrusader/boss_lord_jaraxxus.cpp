...
{
    boss_jaraxxusAI(Creature* creature)
        : BossAI(creature, TYPE_JARAXXUS)
    {
        ...
        if (instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
        {
            ...
            me->SetFullHealth();
            if (CreatureTemplate const* anubTemplate = sObjectMgr->GetCreatureTemplate(35616))
            {
                me->SetFloatValue(UNIT_FIELD_MINDAMAGE, anubTemplate->mindmg * anubTemplate->dmg_multiplier);
                me->SetFloatValue(UNIT_FIELD_MAXDAMAGE, anubTemplate->maxdmg * anubTemplate->dmg_multiplier);
            }
        }
    }
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_SPELL_TOUCH_OF_JARAXXUS:
            {
                ...
                SelectTargetList(targetList, instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC ? 2 : 1, SELECT_TARGET_RANDOM, 0.0f, true, -SPELL_LEGION_FLAME);
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        if (instance && instance->IsBoostEnabled() == RAID_DIFFICULTY_25MAN_HEROIC)
        {
            summon->SetMaxHealth(summon->GetMaxHealth() * 1.4f);
...
{
    npc_fel_infernalAI(Creature* creature)
        : ScriptedAI(creature)
    {
...
{
    npc_mistress_of_painAI(Creature* pCreature)
