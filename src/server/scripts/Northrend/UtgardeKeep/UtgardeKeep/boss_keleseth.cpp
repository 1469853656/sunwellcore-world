...
public:
    npc_vrykul_skeleton()
        : CreatureScript("npc_vrykul_skeleton")
    {
    }
    ...
    {
        npc_vrykul_skeletonAI(Creature* c)
            : ScriptedAI(c)
        {
            pInstance = c->GetInstanceScript();
        }
...
public:
    spell_frost_tomb()
        : SpellScriptLoader("spell_frost_tomb")
    {
    }
