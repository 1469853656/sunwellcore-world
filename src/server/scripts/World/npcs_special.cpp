...
public:
    npc_elder_clearwater()
        : CreatureScript("npc_elder_clearwater")
    {
    }

    struct npc_elder_clearwaterAI : public ScriptedAI
    {
        npc_elder_clearwaterAI(Creature* c)
            : ScriptedAI(c)
        {
            events.Reset();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_CLEARWATER_ANNOUNCE: {
                    ...
                    {
                        sCreatureTextMgr->SendChat(me, CLEARWATER_SAY_PRE, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_MAP);
                        preWarning = true;
                    }
                    if (!startWarning && strdate.tm_hour == 14 && strdate.tm_min == 0)
                    {
                        sCreatureTextMgr->SendChat(me, CLEARWATER_SAY_START, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_MAP);
                        startWarning = true;
                    }
                    if (!finishWarning && strdate.tm_hour == 15 && strdate.tm_min == 0)
                    {
                        sCreatureTextMgr->SendChat(me, CLEARWATER_SAY_END, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_MAP);
                        finishWarning = true;
                        // no one won - despawn
                        if (!finished)
                        {
...
public:
    npc_riggle_bassbait()
        : CreatureScript("npc_riggle_bassbait")
    {
    }

    struct npc_riggle_bassbaitAI : public ScriptedAI
    {
        npc_riggle_bassbaitAI(Creature* c)
            : ScriptedAI(c)
        {
            events.Reset();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_RIGGLE_ANNOUNCE: {
                    ...
                    if (!startWarning && strdate.tm_hour == 14 && strdate.tm_min == 0)
                    {
                        sCreatureTextMgr->SendChat(me, RIGGLE_SAY_START, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_ZONE);
                        startWarning = true;
                    }
                    if (!finishWarning && strdate.tm_hour == 16 && strdate.tm_min == 0)
                    {
                        sCreatureTextMgr->SendChat(me, RIGGLE_SAY_END, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_ZONE);
                        finishWarning = true;
                        // no one won - despawn
                        if (!finished)
                        {
...
{
public:
    npc_short_john_mirthil()
        : CreatureScript("npc_short_john_mirthil")
    {
    }

    struct npc_short_john_mirthilAI : public NullCreatureAI
    {
        npc_short_john_mirthilAI(Creature* c)
            : NullCreatureAI(c)
        {
            pathPoint    = 0;
            startWarning = false;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_SHORT_JOHN_ACTION: {
                    ...
                    {
                        sCreatureTextMgr->SendChat(me, SAY_SHORT_JOHN_ANNOUNCE, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_ZONE);
                        startWarning = true;
                ...
                case EVENT_SHORT_JOHN_MOVE_BACK:
                    ...
                    sCreatureTextMgr->SendChat(me, SAY_SHORT_JOHN_BATTLE_START, nullptr, CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, TEXT_RANGE_ZONE);
                    StartMovement(true);
                    events.PopEvent();
                    break;
            }
...
public:
    ...
    struct npc_training_dummyAI : ScriptedAI
    {
        ...
        void DamageTaken(Unit*, uint32& damage, DamageEffectType /*damageType*/, SpellSchoolMask) override
        {
            resetTimer = 5000;
            damage = 0;
        }
...
public:
    npc_target_dummy()
        : CreatureScript("npc_target_dummy")
    {
    }

    struct npc_target_dummyAI : ScriptedAI
    {
        npc_target_dummyAI(Creature* creature)
            : ScriptedAI(creature)
        {
            SetCombatMovement(false);
            deathTimer = 15000;
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF + 1)
        {
            ...
            player->AreaExploredOrEventHappens(player->GetTeam(CF_DENY) == TEAM_ALLIANCE ? QUEST_BODY_HEART_A : QUEST_BODY_HEART_H);
        }
        return true;
    }
};
            ...
            switch (emote)
            {
                case TEXT_EMOTE_CHICKEN:
                    if (player->GetQuestStatus(QUEST_CLUCK) == QUEST_STATUS_NONE && rand() % 30 == 1)
                    {
                        ...
                        Talk(player->GetTeam(CF_DENY) == TEAM_HORDE ? EMOTE_HELLO_H : EMOTE_HELLO_A);
                    }
                    break;
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (doSwitch)
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("Can't do that while being in battleground queue.");
                return true;
            }
...
public:
    npc_stable_master()
        : CreatureScript("npc_stable_master")
    {
    }
