...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/)
    {
        if (player->IsAlive() && !player->IsInCombat())
        {
            if (player->GetTeam(CF_DENY) == TEAM_ALLIANCE && player->GetQuestRewardStatus(QUEST_GAINING_ACCESS_A))
            {
                player->CastSpell(player, SPELL_TELE_A_TO, false);
                return true;
            }

            if (player->GetTeam(CF_DENY) == TEAM_HORDE && player->GetQuestRewardStatus(QUEST_GAINING_ACCESS_H))
            {
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
    {
        ...
        {
            if (!(QLR == QUEST_STATUS_INCOMPLETE || QLR == QUEST_STATUS_COMPLETE || QBT == QUEST_STATUS_INCOMPLETE || QBT == QUEST_STATUS_COMPLETE))
                return false;
        }
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
    {
        if (!player->isDead() && (player->GetQuestStatus(QUEST_MEETING_A_GREAT_ONE) != QUEST_STATUS_NONE || (player->GetQuestStatus(QUEST_THE_MAKERS_OVERLOOK) == QUEST_STATUS_REWARDED && player->GetQuestStatus(QUEST_THE_MAKERS_PERCH) == QUEST_STATUS_REWARDED)))
        {
            switch (trigger->id)
            {
                case AT_SHOLAZAR:
...
public:
    AreaTrigger_at_nats_landing()
        : AreaTriggerScript("at_nats_landing")
    {
    }
...
public:
    AreaTrigger_at_area_52_entrance()
        : AreaTriggerScript("at_area_52_entrance")
    {
