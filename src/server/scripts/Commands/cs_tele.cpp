...
public:
    tele_commandscript()
        : CommandScript("tele_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        static std::vector<ChatCommand> teleCommandTable = {
            { "add", SEC_ADMINISTRATOR, CMD_INGAME, &HandleTeleAddCommand, "" },
            { "del", SEC_ADMINISTRATOR, CMD_CLI, &HandleTeleDelCommand, "" },
            { "name", SEC_GAMEMASTER, CMD_CLI, &HandleTeleNameCommand, "" },
            { "group", SEC_GAMEMASTER, CMD_INGAME, &HandleTeleGroupCommand, "" },
            { "", SEC_GAMEMASTER, CMD_INGAME, &HandleTeleCommand, "" }
        };
        static std::vector<ChatCommand> commandTable = {
            { "tele", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", teleCommandTable }
        };
        return commandTable;
    }
    ...
    static bool HandleTeleNameCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (strcmp(teleStr, "$home") == 0) // References target's homebind
        {
            ...
            {
                auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_HOMEBIND);
    ...
    static bool HandleTeleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!sCustomMgr->CheckRealmPhase(RealmPhase::REALM_PHASE_PTR) && handler->GetSession()->GetSecurity() == SEC_PLAYER)
            return false;
