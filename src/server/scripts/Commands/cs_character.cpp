...
public:
    character_commandscript()
        : CommandScript("character_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        ...
        static std::vector<ChatCommand> characterCommandTable = {
            ...
            { "deleted", SEC_GAMEMASTER, CMD_CLI, nullptr, "", characterDeletedCommandTable },
        ...
        static std::vector<ChatCommand> commandTable = {
            { "character", SEC_GAMEMASTER, CMD_CLI, nullptr, "", characterCommandTable },
            { "levelup", SEC_ADMINISTRATOR, CMD_INGAME, &HandleLevelUpCommand, "" },
            { "lvl", SEC_PLAYER, CMD_INGAME, &HandleLvlCommand, "" },
            { "pdump", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", pdumpCommandTable }
        };
        return commandTable;
    }
    ...
    static bool GetDeletedCharacterInfoList(DeletedInfoList& foundList, std::string searchString)
    {
        ...
        if (result)
        {
            do
            {
                ...
                uint32 const extraFlags = fields[4].GetUInt32();
                info.IsHardcore = extraFlags & PLAYER_EXTRA_HARDCORE;
                info.IsPathOfWar = extraFlags & PLAYER_EXTRA_PATH_OF_WAR;
    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        PreparedStatement<CharacterDatabaseConnection>* nameStmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_NAME_DATA);
        ...
        if (result)
        {
            ...
            sWorld->AddGlobalPlayerData(delInfo.lowGuid, delInfo.accountId, nameReserved ? "Default" : delInfo.name,
                gender, race, playerClass, level, 0, 0, AccountMgr::GetName(delInfo.accountId), delInfo.IsHardcore, delInfo.IsPathOfWar);
        }
    }
    ...
    */
    ...
    static bool HandleLvlCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto const newLevel = char_cast<uint32>(*tokens.begin());
        if (!newLevel)
            return false;

        if (newLevel.value() == 0 || newLevel.value() > DEFAULT_MAX_LEVEL)
        {
            handler->SendSysMessage("Level needs to be in 1-80 range.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        HandleCharacterLevel(player, player->GetGUID(), oldLevel, newLevel.value(), handler);   
        return true;
    }
