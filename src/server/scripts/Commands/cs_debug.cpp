...
public:
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        ...
        {
            { "memoryleak",     SEC_ADMINISTRATOR,  CMD_CLI, &HandleDebugMemoryLeak, "" },
            { "outofbounds",    SEC_ADMINISTRATOR,  CMD_CLI, &HandleDebugOutOfBounds, "" }
        };
        static std::vector<ChatCommand> debugCommandTable =
        {
            ...
            { "play",           SEC_GAMEMASTER,      CMD_INGAME, nullptr,              "", debugPlayCommandTable },
            { "send",           SEC_ADMINISTRATOR,  CMD_INGAME, nullptr,              "", debugSendCommandTable },
            ...
            { "asan",           SEC_ADMINISTRATOR,  CMD_CLI,    nullptr,                            "", debugAsanCommandTable },
        };
        static std::vector<ChatCommand> commandTable =
        {
            { "debug", SEC_GAMEMASTER, CMD_CLI, nullptr, "", debugCommandTable },
    ...
    static bool HandleDebugSendSellErrorCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->GetSession()->GetPlayer()->SendSellError(msg, nullptr, 0, 0);
        return true;
    }
    ...
    static bool HandleDebugUlduar(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            handler->SendSysMessage("You're not in Ulduar.");
            return true;
        }
        ...
        if (!instance)
        {
            handler->SendSysMessage("Instance pointer invalid, returning.");
            return true;
        }
    ...
    static bool HandleDebugMoltenCore(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            handler->SendSysMessage("You're not in Molten Core.");
            return true;
        }
        ...
        handler->SendSysMessage(stream.str().c_str());
    ...
    static bool HandleDebugMajordomo(ChatHandler* handler, char const* args)
    {
        ...
        if (Unit* majordomo = instance->GetCreature(8))
        {
            ...
            switch (function)
            {
                ...
                case 1:
                    if (encounterFinished)
                    {
                        ...
                        handler->SendSysMessage("Majordomo has been summoned to Ragnaros room");
                    }
                    else
                        handler->SendSysMessage("Majodromo encounter not finished");
                    return true;
                case 2:
                    if (encounterFinished)
                    {
                        majordomo->ToCreature()->DespawnOrUnsummon();
                        handler->SendSysMessage("Majordomo has been despawned");
                    }
                    else
                        handler->SendSysMessage("Majodromo encounter not finished");
                    return true;
            ...
            handler->SendSysMessage("Majordomo isn't summoned in this instance.");
        return true;
    }
    ...
    static bool HandleDebugHellforge(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            handler->SendSysMessage("You're not in Hellforge.");
            return true;
        }
