...
public:
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        static std::vector<ChatCommand> npcAddCommandTable = {
            { "formation", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcAddFormationCommand, "" },
            { "item", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcAddVendorItemCommand, "" },
            { "move", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcAddMoveCommand, "" },
            { "temp", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcAddTempSpawnCommand, "" },
            //{ TODO: fix or remove this command
            { "weapon", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcAddWeaponCommand, "" },
            //}
            { "", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcAddCommand, "" }
        };
        static std::vector<ChatCommand> npcDeleteCommandTable = {
            { "item", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcDeleteVendorItemCommand, "" },
            { "", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcDeleteCommand, "" }
        };
        static std::vector<ChatCommand> npcFollowCommandTable = {
            { "stop", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcUnFollowCommand, "" },
            { "", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcFollowCommand, "" }
        };
        static std::vector<ChatCommand> npcSetCommandTable = {
            { "allowmove", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcSetAllowMovementCommand, "" },
            { "entry", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcSetEntryCommand, "" },
            { "factionid", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetFactionIdCommand, "" },
            { "flag", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetFlagCommand, "" },
            { "level", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetLevelCommand, "" },
            { "link", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetLinkCommand, "" },
            { "model", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetModelCommand, "" },
            { "movetype", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetMoveTypeCommand, "" },
            { "phase", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetPhaseCommand, "" },
            { "spawndist", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetSpawnDistCommand, "" },
            { "spawntime", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetSpawnTimeCommand, "" },
            { "data", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcSetDataCommand, "" },
            //{ TODO: fix or remove these commands
            { "name", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetNameCommand, "" },
            { "subname", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSetSubNameCommand, "" }
            //}
        };
        static std::vector<ChatCommand> npcCommandTable = {
            { "info", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcInfoCommand, "" },
            { "near", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcNearCommand, "" },
            { "move", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcMoveCommand, "" },
            { "playemote", SEC_ADMINISTRATOR, CMD_INGAME, &HandleNpcPlayEmoteCommand, "" },
            { "say", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcSayCommand, "" },
            { "textemote", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcTextEmoteCommand, "" },
            { "whisper", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcWhisperCommand, "" },
            { "yell", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcYellCommand, "" },
            { "tame", SEC_GAMEMASTER, CMD_INGAME, &HandleNpcTameCommand, "" },
            { "add", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", npcAddCommandTable },
            { "delete", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", npcDeleteCommandTable },
            { "follow", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", npcFollowCommandTable },
            { "set", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", npcSetCommandTable }
        };
        static std::vector<ChatCommand> commandTable = {
            { "npc", SEC_GAMEMASTER, CMD_INGAME, nullptr, "", npcCommandTable }
        };
        return commandTable;
    }
    ...
    static bool HandleNpcAddVendorItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 maxcount = 0;
        if (fmaxcount)
            maxcount = std::atol(fmaxcount);

        char* fincrtime = strtok(nullptr, " "); //add incrtime, default: 0
        uint32 incrtime = 0;
        if (fincrtime)
            incrtime = std::atol(fincrtime);

        char* fextendedcost = strtok(nullptr, " "); //add ExtendedCost, default: 0
        uint32 extendedcost = fextendedcost ? std::atol(fextendedcost) : 0;
    ...
    static bool HandleNpcAddMoveCommand(ChatHandler* handler, const char* args)
    {
        ...
        */
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_MOVEMENT_TYPE);
    ...
    static bool HandleNpcSetFactionIdCommand(ChatHandler* handler, const char* args)
    {
        ...
        // ..and DB
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_FACTION);
    ...
    static bool HandleNpcSetFlagCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_NPCFLAG);
    ...
    static bool HandleNpcNearCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->SendSysMessage("----- DATABASE -----");

        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_CREATURE_NEAREST);
    ...
    static bool HandleNpcMoveCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_POSITION);
    ...
    */
    ...
    static bool HandleNpcSetSpawnDistCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_SPAWN_DISTANCE);
    ...
    static bool HandleNpcSetSpawnTimeCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_SPAWN_TIME_SECS);
    ...
    static bool HandleNpcTextEmoteCommand(ChatHandler* handler, const char* args)
    {
        ...
        creature->MonsterTextEmote(args, nullptr);
    ...
    static bool HandleNpcWhisperCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint64 receiver_guid = std::atol(receiver_str);
    ...
    static bool HandleNpcAddFormationCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_CREATURE_FORMATION);
