...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> commandTable = {
            { "anticheat", SEC_MODERATOR, CMD_CLI, nullptr, "", anticheatCommandTable }
        };
    ...
    static bool HandleAntiCheatDeleteCommand(ChatHandler* handler, const char* args)
    {
        ...
        else
        {
            ...
            if (!player)
                handler->SendSysMessage("Player doesn't exist");
            else
    ...
    static bool HandleAntiCheatGlobalCommand(ChatHandler* handler, const char* /* args */)
    {
        if (!sWorld->getBoolConfig(CONFIG_ANTICHEAT_ENABLE))
        {
            handler->SendSysMessage("The Anticheat System is disabled.");
            return true;
        }
