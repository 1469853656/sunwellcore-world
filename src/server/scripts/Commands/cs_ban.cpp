...
public:
    ban_commandscript()
        : CommandScript("ban_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        ...
        static std::vector<ChatCommand> commandTable = {
            { "ban", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", banCommandTable },
            { "baninfo", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", baninfoCommandTable },
            { "banlist", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", banlistCommandTable },
            { "unban", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", unbanCommandTable }
        };
        return commandTable;
    }
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        switch (sWorld->BanAccount(mode, nameOrIP, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
            case BAN_SUCCESS:
                ...
                {
                    case BAN_ACCOUNT:
                        sLog->outBan("Game Master: [%s] has banned account: [%s] for %s - Reason: %s", handler->GetSession()->GetPlayerName().c_str(), nameOrIP.c_str(), (atoi(durationStr) > 0 ? (secsToTimeString(TimeStringToSecs(durationStr), true).c_str()) : "Infinity (perm ban)"), reasonStr);
                        break;
                    case BAN_CHARACTER:
                        sLog->outBan("Game Master: [%s] has banned character: [%s] for %s - Reason: %s", handler->GetSession()->GetPlayerName().c_str(), nameOrIP.c_str(), (atoi(durationStr) > 0 ? (secsToTimeString(TimeStringToSecs(durationStr), true).c_str()) : "Infinity (perm ban)"), reasonStr);
                        break;
                    case BAN_IP:
                        sLog->outBan("Game Master: [%s] has banned ip address: [%s] for %s - Reason: %s", handler->GetSession()->GetPlayerName().c_str(), nameOrIP.c_str(), (atoi(durationStr) > 0 ? (secsToTimeString(TimeStringToSecs(durationStr), true).c_str()) : "Infinity (perm ban)"), reasonStr);
                        break;
                }
            ...
            case BAN_LONGER_EXISTS:
                handler->SendSysMessage("Unsuccessful! A longer ban is already present on this account!");
                break;
        }
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_BANINFO);
    ...
    {
        auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_DEL_EXPIRED_IP_BANS);
        ...
        if (filter.empty())
        {
            auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BANNED_ALL);
            result                  = LoginDatabase.Query(stmt);
        }
        else
        {
            auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BANNED_BY_USERNAME);
        ...
        return HandleBanListHelper(std::move(result), handler);
    }
        ...
        if (handler->GetSession())
        {
            do
            {
                ...
                if (banResult)
                {
                    ...
                    handler->SendSysMessage(fields2[0].GetCString());
                }
            } while (result->NextRow());
        }
        // Console wide output
            ...
            do
            {
                ...
                if (banInfo)
                {
                    ...
                    do
                    {
                        ...
                        tm tmBan = tm_from_time_t(timeBan);
                        ...
                        else
                        {
                            ...
                            tm tmUnban = tm_from_time_t(timeUnban);
    ...
    static bool HandleBanListCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUID_BY_NAME_FILTER);
        stmt->setString(0, filter);
        PreparedQueryResult result = CharacterDatabase.Query(stmt);
        if (!result)
        {
        ...
        if (handler->GetSession())
        {
            ...
            {
                Field* fields            = result->Fetch();
                auto stmt2 = CharacterDatabase.GetPreparedStatement(CHAR_SEL_BANNED_NAME);
                stmt2->setUInt32(0, fields[0].GetUInt32());
                PreparedQueryResult banResult = CharacterDatabase.Query(stmt2);
                if (banResult)
                    handler->SendSysMessage((*banResult)[0].GetCString());
            } while (result->NextRow());
        }
        // Console wide output
        else
        {
            ...
            do
            {
                ...
                auto stmt2 = CharacterDatabase.GetPreparedStatement(CHAR_SEL_BANINFO_LIST);
                stmt2->setUInt32(0, fields[0].GetUInt32());
    ...
    static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (filter.empty())
        {
            auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED_ALL);
            result = LoginDatabase.Query(stmt);
        }
        else
        {
            auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED_BY_IP);
            stmt->setString(0, filter);
            result = LoginDatabase.Query(stmt);
        }
        ...
        if (handler->GetSession())
        {
            ...
            {
                Field* fields = result->Fetch();
                handler->SendSysMessage( fields[0].GetCString());
            } while (result->NextRow());
        }
        // Console wide output
        else
