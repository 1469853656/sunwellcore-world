...
constexpr uint32 ACCOUNT_HISTORY_LIMIT{ 100 };

class lookup_commandscript : public CommandScript
{
public:
    lookup_commandscript()
        : CommandScript("lookup_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        ...
        static std::vector<ChatCommand> lookupCommandTable = {
            ...
            { "player", SEC_GAMEMASTER, CMD_CLI, nullptr, "", lookupPlayerCommandTable },
            { "skill", SEC_ADMINISTRATOR, CMD_CLI, &HandleLookupSkillCommand, "" },
            { "spell", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", lookupSpellCommandTable },
        ...
        static std::vector<ChatCommand> commandTable = {
            { "lookup", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", lookupCommandTable }
        };
        return commandTable;
    }
    ...
    static bool HandleLookupAreaCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 index = 0; index < sAreaTableStore.GetNumRows(); ++index)
        {
            AreaTableEntry const* areaEntry = sAreaTableStore.LookupEntry(index);
            if (areaEntry)
            {
    ...
    static bool HandleLookupPlayerIpCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BY_IP);
        stmt->setString(0, ip);

        return LookupPlayerSearchCommand(LoginDatabase.Query(stmt), limit, handler);
    }
    ...
    static bool HandleLookupPlayerAccountCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_LIST_BY_NAME);
    ...
    static bool HandleLookupPlayerEmailCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_LIST_BY_EMAIL);
    ...
    static bool HandleLookupPlayerAcchistoryCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            handler->SendSysMessage("You can't view history for this account.");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            if (++count > ACCOUNT_HISTORY_LIMIT)
                return true;
            handler->PSendSysMessage("%u. |cff00ccff[%s] | |cffffff00%s", count, it.first.c_str(), TimeToTimestampStr(it.second).c_str());
        }
    ...
    static bool HandleLookupPlayerIphistoryCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (auto const& it : sortedIpStore)
        {
            ...
            auto stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_INFO);
            stmt->setUInt32(0, accountId);
            PreparedQueryResult result = LoginDatabase.Query(stmt);
            if (!result)
                continue;
            ...
            handler->PSendSysMessage("%u. |cff00ccff[%s (ID: %d)] | |cffffff00%s", count, accName.c_str(), accountId, TimeToTimestampStr(it.second).c_str());
        }
