...
public:
    list_commandscript()
        : CommandScript("list_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const
    {
        ...
        static std::vector<ChatCommand> commandTable = {
            { "list", SEC_ADMINISTRATOR, CMD_CLI, nullptr, "", listCommandTable }
        };
        return commandTable;
    }
    ...
    static bool HandleListPremiumCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            if (player->GetSession()->HasPremiumService(service))
            {
                time_t const serviceTime = player->GetSession()->GetPremiumService(service);
                strftime(buff, 20, "%Y-%m-%d %H:%M:%S", localtime(&serviceTime));
                ChatHandler(handler->GetSession()).PSendSysMessage("|cFFFFD700Service |cffff9933%s |cFFFFD700Enabled|r and will expire at: |cffff9933%s", sCustomMgr->GetPremiumServiceName(service).data(), buff);
            }
            else
                ChatHandler(handler->GetSession()).PSendSysMessage("|cFFFFD700Service  |cffff9933%s |cFFFFD700Disabled", sCustomMgr->GetPremiumServiceName(service).data());
        }
    ...
    static bool HandleListCreatureCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 count   = countStr ? std::atol(countStr) : 10;
    ...
    static bool HandleListItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_INVENTORY_COUNT_ITEM);
    ...
    static bool HandleListObjectCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 gameObjectId = std::atol(id);
        if (!gameObjectId)
        {
