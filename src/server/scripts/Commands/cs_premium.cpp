...
{
public:
    premium_commandscript()
        : CommandScript("premium_commandscript")
    {
    }
    ...
    static bool HandlePremiumCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        constexpr std::string_view TextColor = "|cFFFFD700";
        constexpr std::string_view ContentColor = "|cffff9933";
        time_t const subscriptionTime = session->GetPremiumService(PremiumServices::WhitemanePlus);
        bool const hasSubscription = subscriptionTime > time(nullptr);
        if (hasSubscription)
            ChatHandler(session).SendFormattedSysMessage("{1}Whitemane+ Subscription {0}Enabled|r and will expire at: {1}{2}", TextColor, ContentColor, TimeToHumanReadable(subscriptionTime));
        else
            ChatHandler(session).SendFormattedSysMessage("{1}Whitemane+ Subscription {0}Disabled", TextColor, ContentColor);

        for (PremiumServices const service : { PremiumServices::HealthyResurrection, PremiumServices::IndestructibleArmor, PremiumServices::UnlimitedHearthstone, PremiumServices::InstantFlight, PremiumServices::RapidExperience })
        {
            time_t const serviceTime = session->GetPremiumService(service);
            if (serviceTime > time(nullptr))
                ChatHandler(session).SendFormattedSysMessage("{0}Service {1}\"{2}\" {0}Enabled|r and will expire at: {1}{3}", TextColor, ContentColor, sCustomMgr->GetPremiumServiceName(service), TimeToHumanReadable(serviceTime));
            else if (hasSubscription && service != PremiumServices::RapidExperience)
                    ChatHandler(session).SendFormattedSysMessage("{0}Service {1}\"{2}\" {0}Enabled (from Whitemane+ subscription)|r and will expire at: {1}{3}", TextColor, ContentColor, sCustomMgr->GetPremiumServiceName(service), TimeToHumanReadable(subscriptionTime));
            else
                ChatHandler(session).SendFormattedSysMessage("{0}Service {1}\"{2}\" {0}Disabled", TextColor, ContentColor, sCustomMgr->GetPremiumServiceName(service));
        }
        return true;
    }
    ...
    static bool HandlePremiumTeleportCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            handler->SendSysMessage("You can't do this right now.");
            handler->SetSentErrorMessage(true);
            return false;
        }
...
#pragma region Error handling
        ...
        if (!errors.empty())
        {
            handler->SendSysMessage("To teleport, please fix the following:");
