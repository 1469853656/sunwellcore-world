...
public:
    arena_commandscript()
        : CommandScript("arena_commandscript")
    {
    }
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> commandTable = {
            { "arena", SEC_MODERATOR, CMD_CLI, nullptr, "", arenaCommandTable }
        };
    ...
    static bool HandleArenaSetPersonalRating(ChatHandler* handler, const char* args)
    {
        ...
        if (!player)
        {
            handler->PSendSysMessage("Couldn't find player (%s), either wrong name or player is offline", playerName.c_str());
            return true;
        }
        ...
        {
            handler->SendSysMessage("Invalid arena slot, valid arena slots: 0 for 2s, 1 for 3s and 2 for 5s");
            return true;
        }
        ...
        if (ArenaTeam* at = sArenaTeamMgr->GetArenaTeamById(arenaTeamId))
        {
            if (at->IsFighting())
            {
                handler->PSendSysMessage("Arena team %s is currently fighting, cannot change personal rating.", at->GetName().c_str());
                return true;
            }

            at->ForcePersonalRatingChange(player, newPersonalRating, 0);
            at->SaveToDB();
            handler->PSendSysMessage("Successfuly changed personal rating and matchmaking rating to (%u) for player (%s)", newPersonalRating, player->GetName().c_str());
            return true;
        }
        else
        {
