...
public:
    go_midsummer_bonfire()
        : GameObjectScript("go_midsummer_bonfire")
    {
    }
...
public:
    npc_midsummer_bonfire()
        : CreatureScript("npc_midsummer_bonfire")
    {
    }

    struct npc_midsummer_bonfireAI : public ScriptedAI
    {
        npc_midsummer_bonfireAI(Creature* c)
            : ScriptedAI(c)
        {
            me->IsAIEnabled = true;
            goGUID          = 0;
...
public:
    npc_midsummer_torch_target()
        : CreatureScript("npc_midsummer_torch_target")
    {
    }

    struct npc_midsummer_torch_targetAI : public ScriptedAI
    {
        npc_midsummer_torch_targetAI(Creature* c)
            : ScriptedAI(c)
        {
            teleTimer  = 0;
            startTimer = 1;
        ...
        void SpellHit(Unit* caster, SpellInfo const* spellInfo) override
        {
            ...
            if (spellInfo->Id == 46054 && caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (++counter >= maxCount)
                {
                    caster->CastSpell(caster, (caster->ToPlayer()->GetTeam(CF_DENY) ? 46651 : 45719), true); // quest complete spell
...
public:
    spell_midsummer_ribbon_pole()
        : SpellScriptLoader("spell_midsummer_ribbon_pole")
    {
    }
...
public:
    spell_midsummer_torch_quest()
        : SpellScriptLoader("spell_midsummer_torch_quest")
    {
    }
...
{
public:
    spell_midsummer_fling_torch()
        : SpellScriptLoader("spell_midsummer_fling_torch")
    {
    }
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (Player* target = GetHitPlayer())
            {
                ...
                if (Aura* aur = target->GetAura(SPELL_TORCH_COUNTER))
                {
                    ...
                    uint8 count = 4;
                    if (target->GetQuestStatus(target->GetTeam(CF_DENY) ? 11925 : 11924) == QUEST_STATUS_INCOMPLETE) // More Torch Catching quests
                        count = 10;

                    if (aur->GetStackAmount() >= count)
                    {
                        //target->CastSpell(target, 46711, true); // Set Flag: all torch returning quests are complete
                        target->CastSpell(target, (target->GetTeam(CF_DENY) ? 46654 : 46081), true); // Quest completion
                        aur->SetDuration(1);
                        return;
                    }
                }
...
public:
    spell_midsummer_juggling_torch()
        : SpellScriptLoader("spell_midsummer_juggling_torch")
    {
    }
    ...
    class spell_midsummer_juggling_torch_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            if (Player* target = GetHitPlayer())
                if (!handled && target->GetQuestRewardStatus(target->GetTeam(CF_DENY) == TEAM_ALLIANCE ? 11657 : 11923))
                {
                    handled = true;
