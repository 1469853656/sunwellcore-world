...
public:
    npc_love_in_air_supply_sentry()
        : CreatureScript("npc_love_in_air_supply_sentry")
    {
    }

    struct npc_love_in_air_supply_sentryAI : public ScriptedAI
    {
        npc_love_in_air_supply_sentryAI(Creature* creature)
            : ScriptedAI(creature)
            , lock(0)
        {
        }
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (lock > 1000 && me->GetDistance(who) < 10.0f && who->GetTypeId() == TYPEID_PLAYER && who->HasAura(SPELL_GOBLIN_DISGUISE) && !who->HasAura(SPELL_GOBLIN_CARRY_CRATE))
            {
                ...
                if (who->ToPlayer()->GetTeam(CF_DENY) == TEAM_ALLIANCE)
...
public:
    npc_love_in_air_snivel()
        : CreatureScript("npc_love_in_air_snivel")
    {
    }

    struct npc_love_in_air_snivelAI : public NullCreatureAI
    {
        npc_love_in_air_snivelAI(Creature* creature)
...
{
public:
    npc_love_in_air_snivel_real()
        : CreatureScript("npc_love_in_air_snivel_real")
    {
    }

    struct npc_love_in_air_snivel_realAI : public ScriptedAI
    {
        npc_love_in_air_snivel_realAI(Creature* creature)
            : ScriptedAI(creature)
        {
            actionTimer   = 7000;
            actionCounter = 0;
        ...
        bool Talk(uint32 time)
        {
            switch (me->GetEntry())
            {
                ...
                case NPC_SNIVEL_HORDE: {
                    switch (time)
                    {
                        case 1: me->MonsterSay("What are you staring at? Haven't you ever seen a genius before?", LANG_UNIVERSAL, nullptr); return false;
                        case 2: me->MonsterSay("This'll teach you to mind your own business!", LANG_UNIVERSAL, nullptr); return true;
                    }
                    break;
                }
                ...
                case NPC_SNIVEL_HORDE + 1: {
                    switch (time)
                    {
                        case 1: me->MonsterSay("That's right. I'd like to list some of these 'fireworks'.", LANG_UNIVERSAL, nullptr); return false;
                        case 2: me->MonsterSay("Those'll net me a nice profit when I return from the South Seas.", LANG_UNIVERSAL, nullptr); return false;
                        case 3: me->MonsterSay("You... Don't think I don't see you. Leave me alone!", LANG_UNIVERSAL, nullptr); return true;
                    }
                    break;
                }
                ...
                case NPC_SNIVEL_HORDE + 2: {
                    switch (time)
                    {
                        case 1: me->MonsterSay("Thanks for the great cut and shave, buddy.", LANG_UNIVERSAL, nullptr); return false;
                        case 2: me->MonsterSay("Here's a little something extra since I'll be away.", LANG_UNIVERSAL, nullptr); return false;
                        case 3: me->MonsterSay("On second thought, keep the whole bag. I have to, uh, get going.", LANG_UNIVERSAL, nullptr); return false;
                        case 4: me->MonsterSay("Did you really think you could corner me this easily?", LANG_UNIVERSAL, nullptr); return true;
                    }
                    break;
                }
            }
...
{
public:
    npc_love_in_air_hummel()
        : CreatureScript("npc_love_in_air_hummel")
    {
    }

    struct npc_love_in_air_hummelAI : public ScriptedAI
    {
        npc_love_in_air_hummelAI(Creature* creature)
            : ScriptedAI(creature)
            , summons(me)
        {
        }
        ...
        {
            me->MonsterSay("...please don't think less of me.", LANG_UNIVERSAL, nullptr);
        ...
        void UpdateAI(uint32 diff) override
        {
            if (speachTimer)
            {
                ...
                {
                    me->MonsterSay("Did they bother to tell you who I am and why I am doing this?", LANG_UNIVERSAL, nullptr);
                    speachTimer = 10000;
                }
                else if (speachTimer >= 16000 && speachTimer < 20000)
                {
                    me->MonsterSay("...or are they just using you like they do everybody else?", LANG_UNIVERSAL, nullptr);
                    speachTimer = 20000;
                }
                else if (speachTimer >= 26000 && speachTimer < 30000)
                {
                    me->MonsterSay("But what does it matter. It is time for this to end.", LANG_UNIVERSAL, nullptr);
                    speachTimer = 0;
                    me->setFaction(16);
...
public:
    npc_love_in_air_hummel_helper()
        : CreatureScript("npc_love_in_air_hummel_helper")
    {
    }

    struct npc_love_in_air_hummel_helperAI : public ScriptedAI
    {
        npc_love_in_air_hummel_helperAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        ...
        void JustDied(Unit*) override { me->MonsterSay("...please don't think less of me.", LANG_UNIVERSAL, nullptr); }
...
public:
    spell_love_in_air_perfume_immune()
        : SpellScriptLoader("spell_love_in_air_perfume_immune")
    {
    }
...
{
public:
    spell_love_in_air_periodic_perfumes()
        : SpellScriptLoader("spell_love_in_air_periodic_perfumes")
    {
    }
...
public:
    spell_item_create_heart_candy()
        : SpellScriptLoader("spell_item_create_heart_candy")
    {
    }
...
public:
    spell_love_is_in_the_air_romantic_picnic()
        : SpellScriptLoader("spell_love_is_in_the_air_romantic_picnic")
    {
    }
...
{
public:
    spell_gen_aura_service_uniform()
        : SpellScriptLoader("spell_gen_aura_service_uniform")
    {
    }
