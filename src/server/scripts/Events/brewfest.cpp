...
public:
    npc_coren_direbrew()
        : CreatureScript("npc_coren_direbrew")
    {
    }
    ...
    {
        npc_coren_direbrewAI(Creature* c)
            : ScriptedAI(c)
            , summons(me)
        {
        }
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_START_FIGHT)
            {
                for (int i = 0; i < 3; ++i)
                {
                    ...
                    if (Creature* cr = me->SummonCreature(NPC_ANTAGONIST, me->GetPositionX() + 3 * cos(o), me->GetPositionY() + 3 * sin(o), me->GetPositionZ(), me->GetOrientation()))
                    {
                        if (i == 0)
                            cr->MonsterSay("Time to die.", LANG_UNIVERSAL, nullptr);
...
public:
    npc_coren_direbrew_sisters()
        : CreatureScript("npc_coren_direbrew_sisters")
    {
    }
    ...
    {
        npc_coren_direbrew_sistersAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
public:
    npc_brewfest_keg_thrower()
        : CreatureScript("npc_brewfest_keg_thrower")
    {
    }

    struct npc_brewfest_keg_throwerAI : public ScriptedAI
    {
        npc_brewfest_keg_throwerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
{
public:
    npc_brewfest_keg_reciver()
        : CreatureScript("npc_brewfest_keg_reciver")
    {
    }

    struct npc_brewfest_keg_reciverAI : public ScriptedAI
    {
        npc_brewfest_keg_reciverAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (!player->HasSpellCooldown(SPELL_COOLDOWN_CHECKER) && player->GetQuestRewardStatus(player->GetTeam(CF_DENY) == TEAM_ALLIANCE ? QUEST_THERE_AND_BACK_AGAIN_A : QUEST_THERE_AND_BACK_AGAIN_H))
...
public:
    npc_brewfest_bark_trigger()
        : CreatureScript("npc_brewfest_bark_trigger")
    {
    }

    struct npc_brewfest_bark_triggerAI : public ScriptedAI
    {
        npc_brewfest_bark_triggerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_dark_iron_attack_generator()
        : CreatureScript("npc_dark_iron_attack_generator")
    {
    }

    struct npc_dark_iron_attack_generatorAI : public ScriptedAI
    {
        npc_dark_iron_attack_generatorAI(Creature* creature)
            : ScriptedAI(creature)
            , summons(me)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                ...
                case EVENT_BARTENDER_SAY: {
                    ...
                    {
                        thrown = 0;
                        sayer->MonsterSay("SOMEONE TRY THIS SUPER BREW!", LANG_UNIVERSAL, nullptr);
                    ...
                    {
                        if (urand(0, 1))
                            sayer->MonsterSay("Chug and chuck! Chug and chuck!", LANG_UNIVERSAL, nullptr);
                        else
                            sayer->MonsterSay("Down the free brew and pelt the Guzzlers with your mug!", LANG_UNIVERSAL, nullptr);
                    }
        ...
        void FinishEventDueToLoss()
        {
            if (Creature* herald = me->FindNearestCreature(NPC_DARK_IRON_HERALD, 100.0f))
            {
                ...
                herald->MonsterYell(amount, LANG_UNIVERSAL, nullptr);
            }
        ...
        void PrepareEvent()
        {
            if (me->GetMapId() == 1) // Kalimdor
            {
                if (Creature* cr = me->SummonCreature(NPC_DROHN_KEG, 1183.69f, -4315.15f, 21.1875f, 0.750492f))
                    summons.Summon(cr);
                if (Creature* cr = me->SummonCreature(NPC_VOODOO_KEG, 1182.42f, -4272.45f, 21.1182f, -1.02974f))
                    summons.Summon(cr);
                if (Creature* cr = me->SummonCreature(NPC_GORDOK_KEG, 1223.78f, -4296.48f, 21.1707f, -2.86234f))
                    summons.Summon(cr);
            }
            else if (me->GetMapId() == 0) // Eastern Kingdom
            {
                if (Creature* cr = me->SummonCreature(NPC_BARLEYBREW_KEG, -5187.23f, -599.779f, 397.176f, 0.017453f))
                    summons.Summon(cr);
                if (Creature* cr = me->SummonCreature(NPC_THUNDERBREW_KEG, -5160.05f, -632.632f, 397.178f, 1.39626f))
                    summons.Summon(cr);
                if (Creature* cr = me->SummonCreature(NPC_GORDOK_KEG, -5145.75f, -575.667f, 397.176f, -2.28638f))
                    summons.Summon(cr);
            }

            if (Creature* cr = me->SummonCreature(NPC_DARK_IRON_HERALD, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 300000))
                summons.Summon(cr);
        ...
        bool AllowStart()
        {
            tm strDate = tm_from_time_t(time(nullptr));
...
{
public:
    npc_dark_iron_attack_mole_machine()
        : CreatureScript("npc_dark_iron_attack_mole_machine")
    {
    }

    struct npc_dark_iron_attack_mole_machineAI : public ScriptedAI
    {
        npc_dark_iron_attack_mole_machineAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    npc_dark_iron_guzzler()
        : CreatureScript("npc_dark_iron_guzzler")
    {
    }

    struct npc_dark_iron_guzzlerAI : public ScriptedAI
    {
        npc_dark_iron_guzzlerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        ...
        void SayText()
        {
            if (!urand(0, 20))
            {
                ...
                {
                    case 0:
                        me->MonsterSay("Drink it all boys!", LANG_UNIVERSAL, nullptr);
                        break;
                    case 1:
                        me->MonsterSay("DRINK! BRAWL! DRINK! BRAWL!", LANG_UNIVERSAL, nullptr);
                        break;
                    case 2:
                        me->MonsterSay("Did someone say, \"Free Brew\"?", LANG_UNIVERSAL, nullptr);
                        break;
                    case 3:
                        me->MonsterSay("No one expects the Dark Iron dwarves!", LANG_UNIVERSAL, nullptr);
                        break;
                    case 4:
                        me->MonsterSay("It's not a party without some crashers!", LANG_UNIVERSAL, nullptr);
                        break;
                }
            }
        }
...
public:
    npc_brewfest_super_brew_trigger()
        : CreatureScript("npc_brewfest_super_brew_trigger")
    {
    }

    struct npc_brewfest_super_brew_triggerAI : public ScriptedAI
    {
        npc_brewfest_super_brew_triggerAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
...
public:
    spell_brewfest_main_ram_buff()
        : SpellScriptLoader("spell_brewfest_main_ram_buff")
    {
    }
...
public:
    spell_brewfest_ram_fatigue()
        : SpellScriptLoader("spell_brewfest_ram_fatigue")
    {
    }
...
{
public:
    spell_brewfest_apple_trap()
        : SpellScriptLoader("spell_brewfest_apple_trap")
    {
    }
...
{
public:
    spell_q11117_catch_the_wild_wolpertinger()
...
{
public:
    spell_brewfest_fill_keg()
        : SpellScriptLoader("spell_brewfest_fill_keg")
    {
    }
...
{
public:
    spell_brewfest_unfill_keg()
        : SpellScriptLoader("spell_brewfest_unfill_keg")
    {
    }
...
public:
    spell_brewfest_toss_mug()
        : SpellScriptLoader("spell_brewfest_toss_mug")
    {
    }
    ...
    class spell_brewfest_toss_mug_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            ...
            {
                if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_VOODOO, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
                else if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_DROHN, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
                else if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_GORDOK, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
            }
            else // EK
            {
                if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_THUNDERBREW, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
                else if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_BARLEYBREW, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
                else if (Creature* cr = caster->FindNearestCreature(NPC_NORMAL_GORDOK, 40.0f))
                    cr->CastSpell(caster, SPELL_THROW_MUG_TO_PLAYER, true);
            }
...
{
public:
    spell_brewfest_add_mug()
        : SpellScriptLoader("spell_brewfest_add_mug")
    {
    }
...
public:
    npc_brew_bubble()
        : CreatureScript("npc_brew_bubble")
    {
    }

    struct npc_brew_bubbleAI : public NullCreatureAI
    {
        npc_brew_bubbleAI(Creature* creature)
