...
public:
    spell_hallows_end_trick()
        : SpellScriptLoader("spell_hallows_end_trick")
    {
    }
...
{
public:
    spell_hallows_end_put_costume(const char* name, uint32 maleSpell, uint32 femaleSpell)
        : SpellScriptLoader(name)
        , _maleSpell(maleSpell)
        , _femaleSpell(femaleSpell)
    {
    }
    ...
    {
    public:
        spell_hallows_end_put_costume_SpellScript(uint32 maleSpell, uint32 femaleSpell)
            : _maleSpell(maleSpell)
            , _femaleSpell(femaleSpell)
        {
        }
...
{
public:
    spell_hallows_end_trick_or_treat()
        : SpellScriptLoader("spell_hallows_end_trick_or_treat")
    {
    }
...
public:
    spell_hallows_end_candy()
        : SpellScriptLoader("spell_hallows_end_candy")
    {
    }
...
{
public:
    spell_hallows_end_tricky_treat()
        : SpellScriptLoader("spell_hallows_end_tricky_treat")
    {
    }
...
public:
    spell_hallows_end_bucket_lands()
        : SpellScriptLoader("spell_hallows_end_bucket_lands")
    {
    }
...
{
public:
    npc_costumed_orphan_matron()
        : CreatureScript("npc_costumed_orphan_matron")
    {
    }

    struct npc_costumed_orphan_matronAI : public ScriptedAI
    {
        npc_costumed_orphan_matronAI(Creature* c)
            : ScriptedAI(c)
        {
        }
...
public:
    npc_soh_fire_trigger()
        : CreatureScript("npc_soh_fire_trigger")
    {
    }

    struct npc_soh_fire_triggerAI : public NullCreatureAI
    {
        npc_soh_fire_triggerAI(Creature* creature)
...
public:
    npc_hallows_end_soh()
        : CreatureScript("npc_hallows_end_soh")
    {
    }

    struct npc_hallows_end_sohAI : public ScriptedAI
    {
        npc_hallows_end_sohAI(Creature* creature)
            : ScriptedAI(creature)
        {
            pos     = 0;
            counter = 0;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                case 1:
                    me->MonsterYell("Prepare yourselves, the bells have tolled! Shelter your weak, your young and your old! Each of you shall pay the final sum! Cry for mercy; the reckoning has come!", LANG_UNIVERSAL, nullptr);
                ...
                case 3:
                {
                    ...
                    if (counter == 5)
                        me->MonsterYell("The sky is dark. The fire burns. You strive in vain as Fate's wheel turns.", LANG_UNIVERSAL, nullptr);
                    else if (counter == 10)
                        me->MonsterYell("The town still burns. A cleansing fire! Time is short, I'll soon retire!", LANG_UNIVERSAL, nullptr);
        ...
        void FinishEvent(bool failed)
        {
            if (failed)
            {
                me->MonsterYell("Fire consumes! You've tried and failed. Let there be no doubt, justice prevailed!", LANG_UNIVERSAL, nullptr);
            ...
            {
                me->MonsterYell("My flames have died, left not a spark! I shall send you now to the lifeless dark!", LANG_UNIVERSAL, nullptr);
        ...
        {
            me->MonsterYell("So eager you are, for my blood to spill. Yet to vanquish me, 'tis my head you must kill!", LANG_UNIVERSAL, nullptr);
...
{
public:
    npc_hallows_end_train_fire()
        : CreatureScript("npc_hallows_end_train_fire")
    {
    }

    struct npc_hallows_end_train_fireAI : public NullCreatureAI
    {
        npc_hallows_end_train_fireAI(Creature* creature)
            : NullCreatureAI(creature)
        {
        }
...
public:
    ...
    struct boss_headless_horsemanAI : public ScriptedAI
    {
        ...
        {
            summons.DespawnAll();
            me->MonsterSay("This end have I reached before. What new adventure lies in store?", LANG_UNIVERSAL, nullptr);
        ...
        {
            me->MonsterYell("Your body lies beaten, battered and broken. Let my curse be your own, fate has spoken.", LANG_UNIVERSAL, nullptr);
        ...
        void SpellHit(Unit* caster, const SpellInfo* spellInfo) override
        {
            if (spellInfo->Id == SPELL_THROW_HEAD_BACK)
            {
                ...
                me->MonsterYell("Here's my body, fit and pure! Now, your blackened souls I'll cure!", LANG_UNIVERSAL, nullptr);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.GetEvent())
            {
                case EVENT_HH_PLAYER_TALK:
                {
                    ...
                    {
                        case 1:
                            player->MonsterSay("Horseman rise...", LANG_UNIVERSAL, nullptr);
                            break;
                        case 2:
                            player->MonsterSay("Your time is nigh...", LANG_UNIVERSAL, nullptr);
                        ...
                        case 3:
                            ...
                            player->MonsterSay("You felt death once...", LANG_UNIVERSAL, nullptr);
                            me->MonsterSay("It is over, your search is done. Let fate choose now, the righteous one.", LANG_UNIVERSAL, nullptr);
                            me->PlayDirectSound(SOUND_AGGRO);
                            break;
                        case 4:
                            me->CastSpell(me, SPELL_SHAKE_CAMERA_MEDIUM, true);
                            player->MonsterSay("Now, know demise!", LANG_UNIVERSAL, nullptr);
                            events.PopEvent();
                            talkCount = 0;
                ...
                case EVENT_SUMMON_PUMPKIN:
                {
                    ...
                    {
                        me->MonsterSay("Soldiers arise, stand and fight! Bring victory at last to this fallen knight!", LANG_UNIVERSAL, nullptr);
...
{
public:
    boss_headless_horseman_head()
        : CreatureScript("boss_headless_horseman_head")
    {
    }

    struct boss_headless_horseman_headAI : public ScriptedAI
    {
        boss_headless_horseman_headAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (timer >= 30000)
            {
                ...
                me->MonsterTextEmote("Headless Horseman laughs", nullptr);
...
{
public:
    boss_headless_horseman_pumpkin()
        : CreatureScript("boss_headless_horseman_pumpkin")
    {
    }

    struct boss_headless_horseman_pumpkinAI : public ScriptedAI
    {
        boss_headless_horseman_pumpkinAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
