...
public:
    npc_pet_dk_ebon_gargoyle()
        : CreatureScript("npc_pet_dk_ebon_gargoyle")
    {
    }
    ...
    struct npc_pet_dk_ebon_gargoyleAI : ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (_despawnTimer > 4000)
            {
                ...
                if (_initialCastTimer >= 2000 && !me->HasUnitState(UNIT_STATE_CASTING | UNIT_STATE_LOST_CONTROL) && me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_CONTROLLED) == NULL_MOTION_TYPE && !me->HasSpellCooldown(SPELL_GARGOYLE_STRIKE))
                {
                    if (Unit* victim = me->GetVictim())
                    {
                        DoCast(victim, SPELL_GARGOYLE_STRIKE);
                        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_GARGOYLE_STRIKE))
                        {
                            uint32 castCooldown = spellInfo->CalcCastTime(me);
                            castCooldown += castCooldown <= 400 ? urand(300, 700) : urand(200, 550);
                            me->AddSpellCooldown(SPELL_GARGOYLE_STRIKE, 0, castCooldown);
                        }
                    }
                }
            }
...
public:
    npc_pet_dk_ghoul()
        : CreatureScript("npc_pet_dk_ghoul")
    {
    }

    struct npc_pet_dk_ghoulAI : public CombatAI
    {
        npc_pet_dk_ghoulAI(Creature* c)
            : CombatAI(c)
        {
        }
...
{
    npc_pet_dk_army_of_the_dead(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    void InitializeAI() override
    {
        ...
        if (auto minion = dynamic_cast<Minion*>(me))
            minion->SetFollowAngle(Position::RandomOrientation());

        scheduler.Schedule(400ms, [&](TaskContext func) {
            HandleCheckTarget();
            func.Repeat();
        });
    ...
    void HandleCheckTarget()
    {
        ...
        else
        {
            ...
            if (playerSelectedVictim && (playerSelectedVictim->IsInCombatWith(owner) || (playerSelectedVictim->GetCharmerOrOwnerPlayerOrPlayerItself() && owner->ToPlayer()->duel && owner->ToPlayer()->duel->opponent == playerSelectedVictim->GetCharmerOrOwnerPlayerOrPlayerItself())) && victimToAttack != playerSelectedVictim)
        ...
        bool const IsFollowingMaster = me->GetMotionMaster() && me->GetMotionMaster()->GetCurrentMovementGeneratorType() == MovementGeneratorType::FOLLOW_MOTION_TYPE;
...
struct npc_pet_dk_dancing_rune_weapon : ScriptedAI
{
    npc_pet_dk_dancing_rune_weapon(Creature* creature) : ScriptedAI(creature)
    {
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        values.AddSpellMod(SPELLVALUE_DAMAGE_PCT_OVERRIDE, 0.5f);
        me->AddSpellValueForUnit(47632, std::move(values));
    }

    // Called from spell_dk_dancing_rune_weapon_AuraScript::HandleEffectApply
    void SetGUID(uint64 const guid, int32 /*id*/) override
    {
        _targetGUID = guid;
    ...
    void AttackStart(Unit* victim) override
    {
        ...
        {
            summoner->SetInCombatWith(victim);
            victim->SetInCombatWith(summoner);
            victim->AddThreat(summoner, 0.0f);
            if (!victim->HasAura(49028, summoner->GetGUID()))
                summoner->AddAura(49028, victim);
        }
    }
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        if (Unit* target = ObjectAccessor::GetUnit(*me, _targetGUID); !target || !target->IsInWorld() || !target->IsAlive())
        {
            _changeTargetTimer.Update(static_cast<int32>(diff));
