...
public:
    npc_pet_gen_mojo()
        : CreatureScript("npc_pet_gen_mojo")
    {
    }

    struct npc_pet_gen_mojoAI : public ScriptedAI
    {
        npc_pet_gen_mojoAI(Creature* creature)
            : ScriptedAI(creature)
        {
        }
        ...
        void ReceiveEmote(Player* player, uint32 emote) override
        {
            ...
            if (emote != TEXT_EMOTE_KISS || !owner || owner->GetTypeId() != TYPEID_PLAYER || owner->ToPlayer()->GetTeam(CF_DENY) != player->GetTeam(CF_DENY))
            {
                return;
            }
...
{
public:
    npc_pet_gen_soul_trader_beacon()
        : CreatureScript("npc_pet_gen_soul_trader_beacon")
    {
    }
    ...
    struct npc_pet_gen_soul_trader_beaconAI : public ScriptedAI
    {
        uint64 ownerGUID;
        EventMap events;
        npc_pet_gen_soul_trader_beaconAI(Creature* c)
            : ScriptedAI(c)
        {
            events.Reset();
...
public:
    npc_pet_gen_argent_pony_bridle()
        : CreatureScript("npc_pet_gen_argent_pony_bridle")
    {
    }

    struct npc_pet_gen_argent_pony_bridleAI : public ScriptedAI
    {
        npc_pet_gen_argent_pony_bridleAI(Creature* c)
            : ScriptedAI(c)
        {
        ...
        void Reset() override
        {
            ...
            if (Unit* owner = me->GetCharmerOrOwner())
            {
                if (Player* player = owner->ToPlayer())
                {
                    ...
                    aura     = (player->GetTeam(CF_DENY) == TEAM_ALLIANCE ? SPELL_AURA_TIRED_S : SPELL_AURA_TIRED_G);
                    ...
                    // Generate Banners
                    uint32 mask = player->GetTeam(CF_DENY) ? RACEMASK_HORDE : RACEMASK_ALLIANCE;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            if (!creature->HasAura(player->GetTeam(CF_DENY) ? SPELL_AURA_TIRED_G : SPELL_AURA_TIRED_S))
            {
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_TRADE:
                ...
                spellId = player->GetTeam(CF_DENY) ? SPELL_AURA_SHOP_G : SPELL_AURA_SHOP_S;
            ...
            case GOSSIP_ACTION_BANK:
                ...
                spellId = player->GetTeam(CF_DENY) ? SPELL_AURA_BANK_G : SPELL_AURA_BANK_S;
            ...
            case GOSSIP_ACTION_MAILBOX: {
                ...
                spellId = player->GetTeam(CF_DENY) ? SPELL_AURA_POSTMAN_G : SPELL_AURA_POSTMAN_S;
        ...
        if (spellId && !creature->HasAura(spellId))
        {
            ...
            player->AddSpellCooldown(player->GetTeam(CF_DENY) ? SPELL_AURA_TIRED_G : SPELL_AURA_TIRED_S, 0, 3 * MINUTE * IN_MILLISECONDS + 4 * HOUR * IN_MILLISECONDS);
...
public:
    npc_pet_gen_target_following_bomb()
        : CreatureScript("npc_pet_gen_target_following_bomb")
    {
    }

    struct npc_pet_gen_target_following_bombAI : public NullCreatureAI
    {
        npc_pet_gen_target_following_bombAI(Creature* c)
            : NullCreatureAI(c)
        {
            checkTimer  = 0;
            bombSpellId = 0;
...
{
public:
    npc_pet_gen_gnomish_flame_turret()
        : CreatureScript("npc_pet_gen_gnomish_flame_turret")
    {
    }

    struct npc_pet_gen_gnomish_flame_turretAI : public ScriptedAI
    {
        npc_pet_gen_gnomish_flame_turretAI(Creature* c)
            : ScriptedAI(c)
        {
            checkTimer = 0;
        }
...
public:
    spell_pet_gen_valkyr_guardian_smite()
        : SpellScriptLoader("spell_pet_gen_valkyr_guardian_smite")
    {
    }
...
public:
    npc_pet_gen_imp_in_a_bottle()
        : CreatureScript("npc_pet_gen_imp_in_a_bottle")
    {
    }

    struct npc_pet_gen_imp_in_a_bottleAI : public NullCreatureAI
    {
        npc_pet_gen_imp_in_a_bottleAI(Creature* c)
            : NullCreatureAI(c)
        {
            _talkTimer = 0;
            _ownerGUID = 0;
...
{
public:
    npc_pet_gen_wind_rider_cub()
        : CreatureScript("npc_pet_gen_wind_rider_cub")
    {
    }

    struct npc_pet_gen_wind_rider_cubAI : public NullCreatureAI
    {
        npc_pet_gen_wind_rider_cubAI(Creature* c)
            : NullCreatureAI(c)
        {
            allowMove   = true;
            isFlying    = true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (allowMove || checkTimer > 2000)
            {
                ...
                if (Unit* owner = me->GetOwner())
                {
                    if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) == POINT_MOTION_TYPE || me->GetDistance(owner) < 1.0f)
                        return;
                    float x, y, z;
...
{
public:
    npc_pet_gen_plump_turkey()
        : CreatureScript("npc_pet_gen_plump_turkey")
    {
    }

    struct npc_pet_gen_plump_turkeyAI : public PassiveAI
    {
        npc_pet_gen_plump_turkeyAI(Creature* c)
            : PassiveAI(c)
        {
            goGUID     = 0;
            jumpTimer  = 0;
...
{
public:
    npc_pet_gen_toxic_wasteling()
        : CreatureScript("npc_pet_gen_toxic_wasteling")
    {
    }

    struct npc_pet_gen_toxic_wastelingAI : public PassiveAI
    {
        npc_pet_gen_toxic_wastelingAI(Creature* c)
            : PassiveAI(c)
        {
        }
...
public:
    npc_pet_gen_fetch_ball()
        : CreatureScript("npc_pet_gen_fetch_ball")
    {
    }

    struct npc_pet_gen_fetch_ballAI : public NullCreatureAI
    {
        npc_pet_gen_fetch_ballAI(Creature* c)
            : NullCreatureAI(c)
        {
        }
...
{
    npc_guardian_petAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
    ...
    Unit* GetValidTargetForGuardian() const
    {
        ...
        if (Unit* unit = owner->GetSelectedUnit())
        {
            ...
            Player const* playerSelected = unit->GetCharmerOrOwnerPlayerOrPlayerItself();
            if (playerSelected && owner->duel && owner->duel->opponent == playerSelected)
                return unit;
        }
...
{
    npc_warl_doomguardAI(Creature* creature)
        : ScriptedAI(creature)
    {
    }
...
{
    npc_warl_eye_of_kilrogg(Creature* creature)
        : ScriptedAI(creature)
    {
    }
