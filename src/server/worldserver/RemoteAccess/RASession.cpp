...
void RASession::Start()
{
    ...
    sLog->outRemote("Accepting RA connection from user %s (IP: %s)", username.c_str(), GetRemoteIpAddress().c_str());
    ...
    sLog->outRemote("User %s (IP: %s) authenticated correctly to RA", username.c_str(), GetRemoteIpAddress().c_str());
...
bool RASession::ProcessCommand(std::string& command)
{
    ...
    sLog->outRemote("Received command: %s", command.c_str());
    ...
    sWorld->QueueCliCommand(CliCommandHolder(this, command, &RASession::CommandPrint, &RASession::CommandFinished));
