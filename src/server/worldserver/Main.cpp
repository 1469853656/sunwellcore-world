...
#include "CustomMgr.h"
#ifndef _TRINITY_CORE_CUSTOM_CONFIG
#define _TRINITY_CORE_CUSTOM_CONFIG "worldcustom.conf"
#endif
...
extern int main(int argc, char** argv)
{
    ...
    char const* cfg_custom_file = _TRINITY_CORE_CUSTOM_CONFIG;
    ...
    if (!sConfigMgr->LoadMore(cfg_custom_file))
    {
        sLog->outError("Invalid or missing configuration file : %s", cfg_custom_file);
        sLog->outError("Verify that the file exists and has \'[worldcustom]' written in the top of the file!");
        return 1;
    }

    sLog->outString("Using configuration file %s.", cfg_file);
    sLog->outString("Using configuration file %s.", cfg_custom_file);
